    ...
    def _validate_post_processing_info(self):
        if IS_POST_PROCESSING_INCLUDED in self.payload and self.payload[IS_POST_PROCESSING_INCLUDED]:
            ...
            raise UserException(
                HTTPStatus.BAD_REQUEST,
                self.__class__.__name__,
                f"Invalid {POST_PROCESSING_TASK_NAME} value. No folder is present with the name {self.payload[POST_PROCESSING_INFO][POST_PROCESSING_TASK_NAME]} at {self.job_root_file_directory}.",
            )