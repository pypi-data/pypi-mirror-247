...
import subprocess
import os
from commons.redis_helper import get_by_key, persist_value, get_table_values
import shutil
from github import Github
import requests
import datetime
import os
from commons.setup import *
import configparser
...
def trigger_script(job, run_cmd, output_path):
...
def add_script(job, algo_block, output_path, job_id, background_flag):
    ...
    if background_flag:
        scheduler.add_job(trigger_script, args=[job, run_command, output_path])
    else:
        trigger_script(job, run_command, output_path)
