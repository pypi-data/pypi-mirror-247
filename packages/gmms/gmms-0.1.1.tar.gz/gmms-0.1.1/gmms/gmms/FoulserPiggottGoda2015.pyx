...
cdef get_arc_loc_cy(double hypo_lat,double hypo_lon):
...
@cython.nonecheck(False)
@cython.cdivision(True)
cdef get_mu_cy(int IM,double M,double hypo_depth,double R_rup,double Vs30,int ffore,int fback,int fins,int fint,int fnm,int frv):
...
@cython.nonecheck(False)
@cython.cdivision(True)
def FoulserPiggottGoda2015_cy(int IM,double M,double hypo_lat,double hypo_lon,double hypo_depth,
    ...
    [ffore,fback] = get_arc_loc_cy(hypo_lat,hypo_lon)

    for i in range(N_sites):
        mu_im[i]  = get_mu_cy(IM,M,hypo_depth,R_rup[i],Vs30[i],ffore,fback,fins,fint,fnm,frv)
        phi_im[i] = get_phi_cy(IM)
        tau_im[i] = get_tau_cy(IM)
