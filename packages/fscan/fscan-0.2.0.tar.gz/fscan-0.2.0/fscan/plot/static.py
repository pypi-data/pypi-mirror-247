...
def make_all_plots(targetPath, bandwidth, ptype):
    ...
    if len(specfiles) == 1:
        ...
        raise Exception(f"Expected 1 spect file for plot type {ptype}, found "
                        f"{len(specfiles)}")
...
def make_spectrogram(fname, title, f, data, Tsft, dateStart, dateEnd):
    ...
    im1 = plt.imshow(np.transpose(data),
                     aspect='auto',
                     origin='lower',
                     cmap=cmap,
                     extent=(mdates.date2num(dateStart),
                             mdates.date2num(dateEnd),
                             f[0],
                             f[-1]),
                     vmin=cutofflow,
                     vmax=cutoffhigh,
