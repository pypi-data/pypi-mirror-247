...
from .batch import MakeSFTJob, NormSFT, CoherenceFromSFT, write_dag
from .html import SummaryPage
...
from .utils.segments import find_segments
...
def epseg_setup(args, ep_info, segtype, channels):
    """
    ...
    epseg['segtype_path'] = os.path.join(
        os.path.abspath(args.SFTpath),
        f'{args.Tsft}s',
        '__'.join([x.replace(':', '_') for x in epseg['segtype']]))

    # basepath/Tsft/segment-type/duration/timestamp
    epseg['epoch_path'] = os.path.join(
        epseg['segtype_path'],
        ep_info['duration_tag'],
        ep_info['epoch_tag'])

    # basepath/Tsft/segment-type/duration/timestamp/segments.txt
    epseg['segfile'] = os.path.join(epseg['epoch_path'], 'segments.txt')
...
def main():
    ...
    parser.add_argument('-f', '--SFTpath', required=True, type=str,
    ...
    parser.add_argument('--include-extra-sub-band', type='bool', default=True,
                        help='include any remaining subband beyond the 100 Hz '
                             'chunks')
    parser.add_argument('--tracked-line-list', type=str, default=None,
    ...
    parser.add_argument('--request-disk', type=int, default=1024,
    ...
    parser.add_argument('-W', '--html-path', type=str,
    ...
    parser.add_argument('--band', type=float, default=1990,
                        help='Frequency band')
    ...
    initialParser = MultiConfParser(subsequentParser=parser)
    multiConfArgs = initialParser.parse_args()

    for args in multiConfArgs:
        ...
        for segtype_idx, (segtype, channels) in enumerate(
            ...
            for ep_idx, ep in enumerate(epochs_info):
                ...
                # filenames correct.)
                epseg_info = epseg_setup(args, ep, segtype, channels)
                ...
                if args.create_sfts and not no_data:
                ...
                if not no_data:
                    ...
                    write_dag(args, epseg_info, channels, all_ch_info)
