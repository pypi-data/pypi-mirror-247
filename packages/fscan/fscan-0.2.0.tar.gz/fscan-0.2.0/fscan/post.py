...
def convert_fscan_txt_to_npz(spects, args, metadata):
    """
    ...
    args: argparse dictionary
        output of post.__main__ argument parsing
    metadata: dictionary
        output of dtutils.parse_filepath
    """

    files = glob(os.path.join(args.parentPathInput, spects))
    for f in files:
        ...
        if temp_s.endswith("_PWA"):
            ...
            out = os.path.join(
                args.parentPathInput, ".".join(["_".join(n), "npz"]))
            gpstimes = data[0, 1:]
            vals = data[1:, 1:]
            freq = data[1:, 0]
            np.savez(out,
...
def main():
    ...
    convert_fscan_txt_to_npz("spec_*_timeaverage.txt", args, metadata)
    convert_fscan_txt_to_npz("spec_*_spectrogram.txt", args, metadata)
    convert_fscan_txt_to_npz("spec_*_coh.txt", args, metadata)
    # this covers both speclong file outputs
    convert_fscan_txt_to_npz("speclong_*.txt", args, metadata)
