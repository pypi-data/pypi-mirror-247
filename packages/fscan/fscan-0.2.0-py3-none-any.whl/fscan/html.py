...
from .utils.dtutils import (parse_filepath, datetime_to_gps, subfolder_format,
                            snap_to_day, snap_to_midnight, WE,
...
def fancybox_img(img, linkparams=dict(), lazy=False, **params):
    ...
    **params
    ...
    fmin = substrings[1]
    fmax = substrings[2]
    page.a(href=img, id_=f'{plt}_{fmin}_{fmax}', **aparams)
    ...
    page.img(id_=f'{plt}_{fmin}_{fmax}', **imgparams)
    page.a.close()
    return page()
        ...
        headline_plots.append(gwhtml.FancyPlot(
            os.path.join(
                metadata['channel-path'].replace(
                    public_html_folder_nobase, f"~{metadata['username']}"),
                'line_count_history.png')))
        headline_plots.append(gwhtml.FancyPlot(
            os.path.join(
                metadata['channel-path'].replace(
                    public_html_folder_nobase, f"~{metadata['username']}"),
                'line_count_histogram.png')))
...
def write_no_data_page(epseg_info, metadata, navbar):
    """
...
def write_fscan_page(ifo, epseg_info, ch_tup, no_data=False,
    ...
    if index_html_made is False:
        ...
        _ = write_special_index_page(mdata, nav, no_data=no_data)
