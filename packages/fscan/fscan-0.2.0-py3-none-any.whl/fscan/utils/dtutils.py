...
import numpy as np
from astropy.time import Time  # only for GPS conversion
...
def datetime_to_gps(dt):
    '''
    What it says on the tin, turn a python datetime object into a GPS time
    stamp.Depends on astropy.
    '''
    return int(Time(dt, scale='utc').gps)


def gps_to_datetime(GPS):
    return Time(GPS, format="gps", scale='utc').datetime
...
def subfolder_format(durationtag, t):
    ...
    # hour/min/sec
    if (t.second == 0 and t.minute == 0 and t.hour == 0) and all(
            [x not in durationtag for x in ["hours", "minutes", "seconds"]]):
...
def args_to_intervals(args):
    ...
    for key in ['analysisStart', 'analysisEnd']:
        if vargs[key]:
            vargs[key] = datestr_to_datetime(vargs[key])
    ...
    # averageDuration
    args.averageDuration, durationtag = deltastr_to_timedelta(
        args.averageDuration)
    if args.analysisDuration:
        args.analysisDuration, _ = deltastr_to_timedelta(
    ...
    if args.analysisDuration:
        if args.analysisStart:
            args.analysisEnd = args.analysisStart + args.analysisDuration
        elif args.analysisEnd:
            args.analysisStart = args.analysisEnd - args.analysisDuration
    if (args.analysisEnd) < (args.analysisStart + args.averageDuration):
        raise Exception(
    ...
    if "month" in args.snapToLast:
        args.analysisStart = snap_to_monthstart(args.analysisStart)
    # snap to day if a day is in args.snapToLast
    for i, (day, _) in enumerate(days.items()):
        if (day in args.snapToLast):
            args.analysisStart = snap_to_day(
                args.analysisStart, args.snapToLast, days)
    if "midnight" in args.snapToLast:
        args.analysisStart = snap_to_midnight(args.analysisStart)
    ...
    if args.greedy:
        quitpoint = args.analysisEnd
    else:
        quitpoint = args.analysisEnd - args.averageDuration
...
/home/ansel.neunzert/public_html/testing_data_v2/1800s/H1_DMT-ANALYSIS_READY/day/20200201/H1_PEM-CS_MAG_EBAY_LSCRACK_Z_DQ  # noqa
    ...
    for iback, substr in enumerate(substrs[::-1]):
        if substr[-1] == "s" and substr[:-1].isnumeric():
    ...
    if len(autopath) > 3:
        ...
        # Boolean if new format or old format of the file structure
        if new_structure := mdata['superdag-exists']:
            ...
            for idx, e in enumerate(mdata['multi-channel-sftdag-exists']):
                if e:
                    ...
                    sftopts = [
                        line.split('argList="')[-1].split('"tagstring')[0]
    ...
    if len(autopath) > 4:
        ...
        if len(postproclines) == 1:
            ...
            raise Exception(f"SUPERDAG.dag contains {len(postproclines)} lines"
                            " containing 'VARS PostProcess'; 1 expected")
