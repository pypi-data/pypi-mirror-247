...
from sona.utils.common import md5_content_hex, md5_hex
...
class StorageBase:
    ...
    def push_all(self, context_id, files: List[File], metadata=None) -> List[File]:
        pushed_files = [normalizer.encode(file) for file in files]
        return [self.push(context_id, file, metadata) for file in pushed_files]

    def push(self, context_id, file: File, metadata=None) -> File:
        ...
        remote_path = str(Path("storage") / today / filename)
        cached_path = self.cached_path.get(context_id, set())
