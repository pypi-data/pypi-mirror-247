...
class FilesAPI(GenericAPI):
    ...
    def get_all_from_location(self, space_id, path=None, file_type=None,
                              recurse=False):
        """
        ...
        ----------
        space_id : str
            The id of the workspace where the files of interest are stored.
        path : str
            The parent folder path of interest.  This will be a comma-separated
            list of ids up to and including the parent folder id.
            Ex:  ",uuid,uuid,uuid,"
        file_type : hyperthought.api.files.FilesAPI.FileType or None
            An enum instance specifying the type of files to get, i.e. files,
            folders, or both.  Will get both if None.
        recurse : bool
        ...
        def _get_all_from_location_flat(space_id, path, file_type=None):
            """Get all top-level files/folders from a specific location."""
            ...
            while True:
                new_files = self.get_from_location(
                    space_id=space_id,
                    path=path,
                    file_type=file_type,
                    start=start,
        ...
        def _queue_folder(folder):
            """Create partial functions to get all files within a folder."""
            # Set pagination parameters.
            start = 0
            length = 25

            while start < folder['items']:
                get_queue.put(
                    partial(
                        self.get_from_location,
                        space_id=space_id,
                        path=folder['path'] + folder['pk'] + ',',
                        start=start,
                        length=length
                    )
                )
        ...
        def _run_request(get_request):
            """Run get file request and queue any folders."""
            files = get_request()
            ...
            files_recursive.extend(files)

            active_threads.subtract()

        # Validate inputs.

        if not isinstance(space_id, str):
            raise ValueError(
                "The space_id parameter to get_all_from_location must be a "
                "string."
            )

        if path is not None and not isinstance(path, str):
            raise ValueError(
                "The path parameter to get_all_from_location must be a string "
                "or None."
            )

        if not isinstance(recurse, bool):
            raise ValueError(
                "The recurse parameter to get_all_from_location must be a "
                "boolean."
            )

        if recurse is False:

            files = _get_all_from_location_flat(
                space_id=space_id,
                path=path,
                file_type=file_type
            )
            ...
            print(
                "Getting all child items recursively, this may take "
                "several minutes..."
            )
            ...
            files_recursive = _get_all_from_location_flat(
                space_id=space_id,
                path=path,
            )
            ...
            while not get_queue.empty() or active_threads.count > 0:

                if get_queue.empty() or active_threads.count >= MAX_THREADS:
                    time.sleep(1)

                else:
                    active_threads.add()
                    get_request = get_queue.get()
            ...
            if file_type == self.FileType.FOLDERS_ONLY:
                return [
                    response_item for response_item in files_recursive if
                    response_item["ftype"] == "Folder"
                ]

            elif file_type == self.FileType.FILES_ONLY:
                return [
                    response_item for response_item in files_recursive if
                    response_item["ftype"] != "Folder"
                ]
