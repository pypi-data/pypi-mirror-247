...
def prepare_dataset(dataset, prompter, tokenizer, logger, cutoff_len, few_shot_size=None, fine_tune=False, diverse=False):
    huggingface_dataset = {}
    log_message(logger, f'Sampled dataset is {dataset["training"][["topic","stance","id"]].to_string()}')
    mapper = {0:"CON", 1:"PRO"}
    mapper_array = {0:["CON"], 1:["PRO"]}
    if not fine_tune:
        if few_shot_size:
            token_limit = 2048 // (few_shot_size -1)
        examples = format_examples(dataset["training"], tokenizer, mapper_array, token_limit, logger, alpaca_examples=True)
        log_message(logger, f"foramtting examples {examples}")
    for split in dataset:
        if fine_tune:
            huggingface_dataset[split] = generate_instructions(dataset[split], mapper)
        else:
            huggingface_dataset[split] = generate_instructions(dataset[split], mapper, examples)
    data = DatasetDict()
...
def evaluate(model, validate, test_data, tokenizer, logger):
    if validate:
    ...
    for step, (test_input) in enumerate(test_dataloader):
        ...
        for s in generation_output.sequences:
            ...
            label = output.split("### Response:")[1].strip()
...
def run_instructiona_fine_tuning_similar(config, params, experiment, validate, logger, topic_count, similarity, fine_tune=True, analyze_primes=False):
    all_test_preds = []
    all_test_labels = []
    cutoff_len = 256
    ...
    dataset = load_splits(experiment, oversample=False, topic_count=topic_count)
    if topic_count:
    ...
    if fine_tune:
        train_data, val_data, test_data = prepare_dataset(dataset, prompter, tokenizer, logger, cutoff_len, few_shot_size=config["few-shot-size"], fine_tune=fine_tune)
        if validate:
            test_data = val_data
    test_index = 0
    ...
    for _, record in dataset[experiment_type].iterrows():
        ...
        few_shots, scores = sample_similar_examples(record["id"], similarities, dataset["training"], few_shot_size)
        ...
        if fine_tune:
            ...
            train_data, _, _ = prepare_dataset(new_dataset, prompter, tokenizer, logger, cutoff_len, few_shot_size=config["few-shot-size"], fine_tune=fine_tune)
        else:
            new_dataset["test"] = dataset["test"]
            new_dataset["validation"] = dataset["validation"]
            train_data, val_data, test_data = prepare_dataset(new_dataset, prompter, tokenizer, logger, cutoff_len, few_shot_size=config["few-shot-size"], fine_tune=fine_tune)
            if validate:
                test_data = val_data
...
def run_instructional_fine_tuning(config, params, experiment, validate, logger, topic_count=None, sampling_strategy=None, similarity=None, fine_tune=True, analyze_primes=False):

    if sampling_strategy =="similar":
        return run_instructiona_fine_tuning_similar(config, params, experiment, validate, logger, topic_count, similarity, fine_tune=fine_tune, analyze_primes=analyze_primes)
    else:
        cutoff_len= 256
        path_template = get_template_path()
        model_name = config["model-name"]
        ...
        if topic_count:
            log_message(logger, f"training on {topic_count} topics")
        dataset = load_splits(experiment, oversample=True, topic_count=topic_count)
        if validate:
            experiment_type = "validation"
        else:
            experiment_type = "test"
        ...
        if sampling_strategy == "diverse":
            ...
            few_shots = sample_diverse_examples(experiment, experiment_type, config["few-shot-size"], topic_count=topic_count)
            if topic_count:
                log_message(logger, f"training on {topic_count} topics")
        ...
        train_data, val_data, test_data = prepare_dataset(dataset, prompter, tokenizer, logger, cutoff_len, few_shot_size=config["few-shot-size"], fine_tune=fine_tune, diverse=diverse)
        ...
        metrics = evaluate(model, validate, test_data, tokenizer, logger)
        log_metrics(logger, metrics, level=logging.WARNING)
        return metrics
