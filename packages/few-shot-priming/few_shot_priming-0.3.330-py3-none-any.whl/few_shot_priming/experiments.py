...
def adapt_vast(df):
    df = df [["topic_str", "text_s", "label", "new_id"]]
    df.rename(columns={"topic_str":"topic", "text_s": "text", "label": "stance", "new_id":"id"}, inplace=True)
    #df["topic"] = df["topic"].apply(lambda topic: " ".join(topic))
    return df[["topic", "stance", "text", "id"]]

def load_splits(experiment, oversample=True, validate=True, best_split=False, adapt=True, topic_count=None):
    """
    ...
    if topic_count:
        topics = df_training[topic_label].unique()
        chosen_topics = np.random.choice(topics, topic_count)
        df_training = df_training[df_training[topic_label].isin(chosen_topics)]
