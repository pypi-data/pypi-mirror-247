...
class ModelAdmin:
    ...
    def get_list_display(self, *args, **kwargs) -> list[RenderField]:
        return [
            RenderField(field_name=field_name, admin=self) for field_name in self.list_display
        ]
    ...
    def get_urlpatterns(self):
        ...
        if self.edit_view_class:
            urlpatterns.append(
                re_path(
                    rf"^{obj_pattern}/$" if obj_pattern else "", self.edit_view, name="edit"
                )
            )
    ...
    def _get_objects_count(self, qs):
        ...
        if self.use_estimated_count:
            return count_estimate(qs, precision=max(admin_settings.PAGINATION_CHOICES))
        return qs.count()
...
class RenderField(Generic[_Model]):
    def __init__(self, field_name: str, admin: ModelAdmin):
    ...
    @property
    def admin_field(self) -> Callable[[Any], str] | None:
    ...
    def get_label(self, form: _BaseForm | None = None) -> StrOrPromise:
        ...
        if self.model_field:
            return self.model_field.verbose_name  # type: ignore[union-attr]
