...
class TestFiQuS(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.model_names = ['MQXA', 'MCBRD_2d2a_2n2a_0i']
        ...
        for model_name in self.model_names:
            ...
            for column, atol in zip(columns, comparison_atol):
                if atol:
                    ...
                    comparison_result = np.isclose(np.mean(model_df[column]), np.mean(reference_df[column]), rtol=0, atol=atol, equal_nan=True).all()
