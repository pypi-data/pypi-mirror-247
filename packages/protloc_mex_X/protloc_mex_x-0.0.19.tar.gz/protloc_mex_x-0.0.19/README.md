...
Our current tool automatically detects whether your device uses a GPU or CPU. If your device supports GPU and has the GPU versions of Pytorch and transformers installed, it will utilize the GPU for feature representation inference. Otherwise, it will use the CPU. In future versions, we plan to modify the API to give users the choice between GPU and CPU. For more flexibility with our tool, please refer to version 0.0.20 and later.
