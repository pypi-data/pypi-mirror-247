...
class FunctionsDistribution(object):
    ...
    def adjust_demand(self, demand, prop, maxim, pdv_def):
        ...
        for loc in pdv_def:
            bol = True
            while bol:
                if demand.loc[loc].sum() > maxim.loc[loc].sum():
    ...
    def adjust_delivery(self, df):
        ...
        for location in list_origins:
            ...
            df_new["TransferLotSize_sum"] = np.ceil(df_new["TransferLotSize_sum"])
            ...
            for i in list_items:
                ...
                df_filter.loc[:, "TransferLotSizeRounded"] = df_filter.loc[:, "TransferLotSize"].apply(np.ceil)
                df_filter.loc[:, 'cumulative'] = df_filter['TransferLotSizeRounded'].cumsum()
                df_filter.loc[:, "TransferLotSize"] = np.where(df_filter["cumulative"] <= df_filter["TransferLotSize_sum"],
                                                                df_filter["TransferLotSizeRounded"], 0)
