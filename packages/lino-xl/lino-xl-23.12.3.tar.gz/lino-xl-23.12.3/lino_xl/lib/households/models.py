...
class Member(mixins.DateRange, Human, Born):
    class Meta:
        app_label = 'households'
    ...
    role = MemberRoles.field(default='child', blank=True, null=True)
    ...
    dependency = MemberDependencies.field(default='none')
    ...
    def __str__(self):
        if self.person_id is None:
            person = Human.__str__(self)
        else:
            person = str(self.person)
        if self.role is None:
            return person
        return "%s (%s)" % (person, self.role)
...
class MembersByPerson(Members):
    ...
    def get_table_summary(self, obj, ar):
        ...
        if self.insert_action is not None:
            sar = self.insert_action.request_from(ar, master_instance=obj)
