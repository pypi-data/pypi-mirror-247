...
class ContextNameType(click.ParamType):
    ...
    def shell_complete(self, ctx, param, incomplete):
        try:
            ...
            config = seaplane.config.SeaplaneConfig()
...
def create_context(context):
    ...
    key = config.keys[key_name] = seaplane.config.Key(key_name, key_value)
    context_obj = config.contexts[context] = seaplane.config.Context(context, key)
