...
class BaseFileLock:
    ...
    def __init__(self, lock_file, timeout = -1):
        ...
        # Whenever the lock is acquired, the counter is increased and the lock is only released, when this value is 0 again.
    ...
    async def acquire(self, timeout=None, poll_intervall=0.05):
        """
        Acquires the file lock or fails with a :exc:`LockTimeout` error.
        .. code-block:: python
            ...
            while True:
                with self._thread_lock:
                    if not self.is_locked:
                        logger.debug('Attempting to acquire lock %d on %s', lock_id, lock_filename)
                        self._acquire()

                if self.is_locked:
                    logger.info('Lock %d acquired on %s', lock_id, lock_filename)
                    break
                if timeout >= 0 and (time.time() - start_time) > timeout:  # pylint: disable=chained-comparison
                    logger.debug('Timeout on acquiring lock %d on %s', lock_id, lock_filename)
                    raise LockTimeout(self._lock_file)
                logger.debug(
                    'Lock %d not acquired on %s, waiting %s seconds ...',
        ...
        with self._thread_lock:
            if self.is_locked:
                ...
                if self._lock_counter == 0 or force:
                    lock_id = id(self)
                    lock_filename = self._lock_file
                    logger.debug('Attempting to release lock %d on %s', lock_id, lock_filename)
                    self._release()
                    self._lock_counter = 0
                    logger.info('Lock %d released on %s', lock_id, lock_filename)
