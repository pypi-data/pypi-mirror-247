...
def process_text(input_text_path: str, output_dir_text_path: str):
    ...
    text_dic = utils_file.load_dic_from_scp(text_path)
    res_list = []
    ...
    thread_runner.start()
    utils_file.write_dic_to_scp(all_output_dict, output_path)
...
def handle_work(dir, input_text_path, output_dir, output_file_name='text_process2'):
    ...
    text_dict = utils_file.load_dic_from_scp(input_text_path)
    ...
    for i in tqdm.tqdm(range(len(dict_list)), total=len(dict_list), desc='分解为若干小字典'):
        ...
        if os.path.exists(output_path_temp):
            ...
            continue
        utils_file.write_dic_to_scp(dict_temp, output_path_temp)
