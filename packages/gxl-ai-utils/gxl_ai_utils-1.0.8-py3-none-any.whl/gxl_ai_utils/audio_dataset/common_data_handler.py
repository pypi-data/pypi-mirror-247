...
def write_to_tar_file(data_list: list[tuple], tar_file_path: str, resample=16000, i=-1):
    ...
    finish_flag_file_path =os.path.splitext(tar_file_path)[0] + '.finish'
    utils_file.makedir_for_file_or_dir(tar_file_path)
    if os.path.exists(finish_flag_file_path):
        print(f'文件{finish_flag_file_path}已存在,跳过该shard的处理')
        return
    ...
    with open(finish_flag_file_path, 'w'):
        pass
    print(f"第{i}个shard处理完成,已创建空文件：{finish_flag_file_path}")
...
def make_shard_file(wav_scp_file_path: str, text_scp_file_path: str, output_dir: str, num_utt_per_shard: int = 1000,
    ...
    wav_dic = utils_file.load_dic_from_scp(wav_scp_file_path)
    data = []
    text_dic = utils_file.load_dic_from_scp(text_scp_file_path)
