...
def do_make_raw_list(wav_scp_path: str, text_scp_path: str,
    ...
    """
    do make raw list
    将wav_scp text_scp合并成一个jsonl:data.list
    :param wav_scp_path:
    :param text_scp_path:
    ...
    :return:
    """
    from .utils_file import load_dic_from_scp, write_dict_list_to_jsonl, makedir_for_file
    makedir_for_file(output_jsonl_path)
    wav_dict = load_dic_from_scp(wav_scp_path)
    text_dict = load_dic_from_scp(text_scp_path)
    segments_dict = {}
...
def do_make_shard_file(wav_scp_file_path: str, text_scp_file_path: str, output_dir: str, num_utt_per_shard: int = 1000,
    ...
    from .utils_file import load_dic_from_scp
...
def do_remove_blank_for_text_scp(text_scp_path: str, ):
    ...
    from .utils_file import load_dic_from_scp, write_dic_to_scp
    dic = load_dic_from_scp(temp_file)
    os.remove(temp_file)
    write_dic_to_scp(dic, text_scp_path)
