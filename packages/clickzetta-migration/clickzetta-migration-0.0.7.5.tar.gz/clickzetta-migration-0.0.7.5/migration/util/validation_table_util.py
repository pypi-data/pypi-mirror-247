...
def data_diff_validation(source_table: str, destination_table: str, source: Source, destination: Destination):
    try:
        ...
        diff_result = diff_tables(source_diff_table, destination_diff_table, algorithm=Algorithm.HASHDIFF, key_columns=pk_colmns,
...
def data_diff_validation_with_pks(source_table: str, destination_table: str, source: Source, destination: Destination, pks_str: str):
    try:
        source_db_type = source.name.lower()
        destination_db_type = destination.name.lower()
        ...
        extra_columns = [column.name for column in source.get_table_columns(source_table.split('.')[0], source_table.split('.')[1]) if column.name not in pk_columns]
        diff_result = diff_tables(source_diff_table, destination_diff_table, algorithm=Algorithm.HASHDIFF, key_columns=pk_columns,
                                  threaded=True, max_threadpool_size=6, extra_columns=tuple(extra_columns))
        result = []
        for sign, columns in diff_result:
            result.append(f'{sign} {columns}')
...
def pk_id_column_validation(source_table: str, destination_table: str, source: Source, destination: Destination, pk_id: str):
    try:
        max_min_id = source.execute_sql(f"select min({pk_id}), max({pk_id}) from {source_table}")[0]
        min_id = max_min_id[0]
        max_id = max_min_id[1]
        logger.info(f"{source_table}-source_min_id: {min_id}, source_max_id: {max_id}, prepare full table check.")
        cur_id = max_id
        cur_min_id = 0
        only_in_source = 0
        only_in_source_list = []
        ...
        while cur_id >= min_id:
            ...
            ids_source = set()
            source_result = source.execute_sql(f'select {pk_id} from {source_table} where {pk_id} <= {cur_id} order by {pk_id} desc limit 50000;')
            ...
            ids_dest = set()
            dest_result = destination.execute_sql(f'select {pk_id} from {destination_table} where {pk_id} >= {cur_min_id} and {pk_id} <={cur_id};')
...
def pk_all_column_validation(source_table: str, destination_table: str, source: Source, destination: Destination, pk_id: str):
    try:
        max_min_id = source.execute_sql(f"select min({pk_id}), max({pk_id}) from {source_table}")[0]
        ...
        logger.info(f"{source_table}-source_count: {source_count}, destination_count: {destination_count}, prepare full table check.")
        min_id = max_min_id[0]
        max_id = max_min_id[1]
        logger.info(f"{source_table}-source_min_id: {min_id}, source_max_id: {max_id}, prepare full table check.")
        cur_id = max_id
        ...
        while cur_id >= min_id:
            logger.debug(f'{datetime.now()} cur_id={cur_id}, ')
            ids_source = {}
            source_result = source.execute_sql(f'select {pk_id} as temp_id,* from {source_table} where {pk_id} <= {cur_id} order by {pk_id} desc limit 10000;')
            for r in source_result:
            ...
            ids_dest = {}
            dest_result = destination.execute_sql(f'select {pk_id} as temp_id,* from {destination_table} where {pk_id} >= {cur_min_id} and {pk_id} <={cur_id} order by {pk_id} desc;')
            ...
            cur_id = cur_min_id - 1
        return {'only_in_source': only_in_source, 'only_in_source_list': only_in_source_list,
                'only_in_dest': only_in_dest, 'only_in_dest_list': only_in_dest_list, 'both_in_but_diff': both_in_but_diff}
    except Exception as e:
        logger.error(f"pk-id validation failed, error: {e}")
        raise Exception('pk-id validation failed:', e)

def pk_id_column_validation_with_count(source_table: str, destination_table: str, source: Source, destination: Destination, pk_id: str, expect_count: int):
    try:
        max_min_id = source.execute_sql(f"select min({pk_id}), max({pk_id}) from {source_table}")[0]
        min_id = max_min_id[0]
        max_id = max_min_id[1]
        logger.info(f"{source_table}-source_min_id: {min_id}, source_max_id: {max_id}, expect_check_count: {expect_count}")
        cur_id = max_id
        cur_min_id = 0
        only_in_source = 0
        only_in_source_list = []
        ...
        while cur_id >= min_id and current_search_times < max_search_times:
            ...
            if current_search_times == max_search_times:
                logger.error(f"max_search_times reached, {source_table}-only_in_source: {only_in_source}, {destination_table}-only_in_dest: {only_in_dest}")
        return {'only_in_source': only_in_source, 'only_in_source_list': only_in_source_list,
