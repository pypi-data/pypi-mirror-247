...
from pingsafe_cli.cli.utils import get_home_path
from pingsafe_cli.version import build_type, version
...
def add_iac_flags(iac_parser):
    ...
    iac_parser.add_argument("--publish-result", dest="publish_result", action="store_true",
                            default=False, help="publish scan results to PingSafe")
...
def add_secret_flags(secret_parser):
    ...
    secret_parser.add_argument("--publish-result", dest="publish_result", action="store_true", default=False,
                               help="publish scan results to PingSafe(works only with --pull-request and --pre-commit)")
...
def add_config_global_flags(config_sub_parser):
    ...
    config_sub_parser.add_argument("--output-format", dest="output_format", metavar="",
                                   ...
                                   default=DefaultGlobalConfigurations.OUTPUT_FORMAT.value, choices=[OutputFormat.JSON, OutputFormat.CSV])
    config_sub_parser.add_argument("--output-file", dest="output_file", metavar="", default="",
                                   help="output file location")
    config_sub_parser.add_argument("--workers-count", dest="workers_count", default=DefaultGlobalConfigurations.WORKERS_COUNT.value, metavar="",
                                   help="worker count for concurrency")
    config_sub_parser.add_argument("--endpoint-url", dest="endpoint_url", default=APP_URL, metavar="",
                                   help="PingSafe Endpoint URL")
    config_sub_parser.add_argument("--on-crash-exit-code", dest="on_crash_exit_code", default=DefaultGlobalConfigurations.ON_CRASH_EXIT_CODE.value,
                                   metavar="", type=int,
