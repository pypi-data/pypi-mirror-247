...
from pingsafe_cli.cli.registry import CodeTypeSubParser, HttpMethod, TIME_FORMAT, SUPPORTED_FRAMEWORKS, \
    ...
    DOWNLOAD_CACHE_URL, PUBLISH_ISSUES_API, InvalidInput, MissingRequiredFlags, SUPPORTED_GIT_PROVIDERS, \
    DEFAULT_PINGSAFE_DIR, APP_URL, APP2_URL, LOCAL_URL, OutputFormat, SEVERITY_MAP
from pingsafe_cli.cli.utils import make_request, read_json_file, write_json_to_file, check_if_paths_exist, wrap_text, \
    get_config_path, get_version, print_output_on_file, get_severity_color, invalidate_cache, get_wrapping_length, \
    read_from_file, get_priority, get_output_file_and_format, get_sarif_payload
...
def iac_parser(args, cache_directory):
    ...
    global_config_data = read_json_file(global_file_path)
...
def evaluate_plugin_on_graph(directory_to_scan, plugin_data, graph_nodes, grouped_links_by_source,
    ...
    for node in graph_nodes:
        ...
        if evaluate_rego(node, plugin_data["rego"], cache_directory):
            results.append(generate_issue_meta(node, plugin_data, directory_to_scan))
...
def evaluate_rego(input_json, rego, cache_directory):
    ...
    try:
        ...
        if result.stderr:
            ...
            raise Exception(f"rego evaluation error: {err_msg}")
...
def iac_pre_evaluation(args, cache_directory):
    ...
    if check_if_upsert_required(iac_config_data, global_config_data):
        ...
        download_iac_cache(iac_cache_dir_path, global_config_data["api_token"], int(policy_version), endpoint_url)
...
def validate_iac_command(args):
    if args.publish_result and (args.branch == "" or args.repo_url == "" or args.repo_full_name == ""):
        raise MissingRequiredFlags(
            "Please provide mandatory flags (--branch, --repo-url, --repo-full-name) to publish results to PingSafe.")
    ...
    if args.repo_url:
        for provider in SUPPORTED_GIT_PROVIDERS:
            if provider.lower() in args.repo_url.lower():
                return
        raise InvalidInput("Git provider not supported yet. Please contact PingSafe support.")
...
def iac_post_evaluation(args, results, cache_directory):
    ...
    if len(filtered_results) > 0:
        ...
        if args.pre_commit and not args.quiet and not args.verbose:
            ...
            args.quiet = True
        filtered_results = sorted(filtered_results, key=get_priority)
        print_issue_on_console(filtered_results, args.quiet, args.verbose)
        save_issues_on_file(args, filtered_results, global_config_data)
        send_issues_to_pingsafe(args, filtered_results, global_config_data)
...
def download_iac_cache(iac_cache_dir_path, api_token, policy_version, endpoint_url):
    response = make_request(HttpMethod.GET, endpoint_url + DOWNLOAD_CACHE_URL, api_token)
...
def send_issues_to_pingsafe(args, results, global_config_data):
    ...
    LOGGER.info("Successfully published iac issues to PingSafe.")
...
def convert_issues_to_sarif(issues):
    rules = []
    results = []
    sarif_result = get_sarif_payload("PingSafe IAC Scanner")
