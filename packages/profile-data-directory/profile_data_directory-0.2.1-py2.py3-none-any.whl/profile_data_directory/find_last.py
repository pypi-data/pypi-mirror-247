...
# file_pattern = "output_v\d+.txt"  # Update the pattern accordingly
# file_extension = ".txt"  # Update the extension accordingly

# most_recent_file_path = find_most_recent_version(directory_path, file_pattern, file_extension)
...
@click.option('--isdir', is_flag=True, help=f"Optional: if True, will search for directories - default is '{DEFAULT_ISDIR}'")
...
def main(config_file: str, extension: str, indir: str, isdir: bool, logfile: str, no_details: bool, outdir: str, outfile: str, pattern: str, verbose: bool):
    ...
    if isdir is None:
        isdir = DEFAULT_ISDIR
        console.print(f"[yellow]--isdir was not specified and therefore was set to '{isdir}'[/]")
