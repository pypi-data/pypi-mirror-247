...
sys.path.append('C:\\Users\\emily\\Lib\\site-packages')
...
def init_sdr(ip):
    ...
    # Set sdr device with iio default value.
...
def amtFmcRfRef(ip="169.254.92.202",refClk="int",clkFreq=[]):
    ...
    inp = './amtFmcRfRef.sh '+refClk+" "+str(clkFreq)+'\n'
    send_cmd(sshob,inp,60)
...
def amtFmcTxConfig(rfPort="A", frequency=2400, rate=30.72, bw=18, txAtt=0, cyclic=True):
    ...
    logging.info("Set AD9364 Tx, rfPort="+rfPort+", frequency="+str(frequency)+" MHz, rate="+str(rate)+" MSPS, bw="+str(bw)+" MHz, txAtt="+str(txAtt)+" dB, cyclic="+str(cyclic))
    logger.removeHandler(ch)
    logger.removeHandler(fh)
amtFmcRfReset()
amtFmcTxConfig()
def amtFmcRfTxStart(data=[]):
    ...
    # Transmit data.
    sdr.tx(data)
    logging.info("Data transmitting...")
    logger.removeHandler(ch)
    logger.removeHandler(fh)
...
def amtFmcRxConfig(rfPort="A", frequency=2400, rate=30.72, bw=18, numOfSamples=16384, rxAAtt=0, rxGain=0):
    ...
    logging.info("Set AD9364 Rx, rfPort="+rfPort+", frequency="+str(frequency)+" MHz, rate="+str(rate)+" MSPS, bw="+str(bw)+" MHz, numOfSamples="+str(numOfSamples)+", rxGain="+str(rxGain)+" dB")
    logger.removeHandler(ch)
    logger.removeHandler(fh)
