...
class TinyB(object):
    ...
    async def promote_datasource(self, ds_name: str, data: Dict[str, Any]):
    ...
    async def deploy_environment(
        ...
        data_migrations: Optional[Any] = None,
    ):
        data = {
            "semver": semver,
        ...
        return await self._req(f"/v0/environments/{branch_id}/deployments", method="POST", data=data)

    async def branch_workspace_data(
        self,
        workspace_id: str,
        last_partition: Optional[str],
        all: Optional[str],
        ignore_datasources: Optional[str] = None,
    ):
        params = {}
        if last_partition:
    ...
    async def release_rm(
        self, workspace_id: str, semver: str, confirmation: Optional[str] = None, dry_run: bool = False
    ):
        params = {"force": "true" if confirmation else "false", "dry_run": "true" if dry_run else "false"}
        if confirmation:
