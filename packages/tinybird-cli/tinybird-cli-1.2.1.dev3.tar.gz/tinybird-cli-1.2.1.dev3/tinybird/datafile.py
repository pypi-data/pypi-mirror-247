...
class Deployment:
    def __init__(self, current_ws: Dict[str, Any], is_git_release: bool, tb_client: TinyB, dry_run: bool) -> None:
    ...
    async def detect_changes(
        ...
        if self.is_git_release:
            ...
            diffs = self.cli_git_release.diff_datafiles(self.current_release["commit"])
            click.echo(
                FeedbackManager.info_git_release_diffs(
                    workspace=self.current_ws["name"],
                    current_release_commit=self.current_release["commit"],
    ...
    def preparing_dependencies(self, changed, dep_map, latest_datasource_versions):
        if not self.is_git_release:
            return
...
def parse_datasource(filename: str, replace_includes: bool = True) -> Datafile:
...
def parse_pipe(filename: str, replace_includes: bool = True) -> Datafile:
    with open(filename) as file:
        s = file.read()
    basepath = os.path.dirname(filename)
...
def parse_token(filename: str, replace_includes: bool = True) -> Datafile:
    with open(filename) as file:
        s = file.read()
    basepath = os.path.dirname(filename)
...
class PipeChecker(unittest.TestCase):
    ...
    def _get_request_for_current(self):
        ...
        return requests.get(
            self.current_pipe_url, headers=headers, verify=not getenv_bool("TB_DISABLE_SSL_CHECKS", False)
        )

    def _get_request_for_checker(self):
        headers = {"Authorization": f"Bearer {self.token}"}
        return requests.get(
            self.checker_pipe_url, headers=headers, verify=not getenv_bool("TB_DISABLE_SSL_CHECKS", False)
        )
...
async def folder_push(
    ...
    deployment.preparing_dependencies(changed, dep_map, latest_datasource_versions)
...
async def format_datasource(
    ...
    doc: Datafile = parse_datasource(filename, replace_includes=replace_includes)
    file_parts: List[str] = []
    if for_diff:
