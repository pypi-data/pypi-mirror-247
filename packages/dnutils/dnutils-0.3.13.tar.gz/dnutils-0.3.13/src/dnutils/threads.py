...
class Lock:
    ...
    def acquire(self, blocking=True, timeout=None):
        ...
        try:
            while 1:
                if tid in self.__interrupt:
                    self.__interrupt.remove(tid)
                    raise ThreadInterrupt()
                # take the ownership of the lock in case it is not locked and
                # I am the designated next owner, if any
                if not self.locked() and ((self.__nextowner is None) or (self.__nextowner == tid)):
                    self.__owner = tid
                    self.__nextowner = None
                    return True
                elif not blocking:
                ...
                while 1:
                    ...
                    if (self.__blocking is None) or (self.__blocking == tid):
                        # __blocking is None means no other operation is in progress, we can proceed in this case.
                        # if __blocking is not None we are only allowed to proceed if we are the blocking thread
                        break
                    self.__lock.release()
                self.__blocking = None
    ...
    def interrupt(self, tid):
        ...
        try:
            self.__safe_acquire()
            if tid not in self.__waiters:
                return
            self.__interrupt.add(tid)
            self.__blocking = tid
            self.__waiters[tid].release()
        finally:
            self.__safe_release()
    ...
    def __safe_acquire(self):
        while 1:
            self.__lock.acquire()
            if self.__blocking is None: return
            # there is a release in progress, so we have to wait until it has finished
            self.__lock.release()
    ...
    def __release(self):
        ...
        self.__owner = None
        waiters = set(self.__waiters.keys())  # .difference(self.__releasing)
        if waiters:
            self.__nextowner = waiters.pop()
            self.__blocking = self.__nextowner
            self.__waiters[self.__nextowner].release()
    ...
    def __repr__(self):
        return '<%s.%s %s at 0x%s>' % (self.__class__.__module__,
                                       ...
                                       ('locked by thread %s' % self.__owner) if self.locked() else 'unlocked',
                                       hex(id(self)))
...
class Timer(Thread):
    ...
    def _checkrepeat(self):
        if self.repeat is None: return False
        if callable(self.repeat): return self.repeat()
        if self.repeat == 'inf': return True
        else: return self.repeat > self.runs
...
class _DummyEvent:
    # the _DummyEvent class replicates the interface of a normal Event
    # but has actually none of its functionality
    def set(self): pass
