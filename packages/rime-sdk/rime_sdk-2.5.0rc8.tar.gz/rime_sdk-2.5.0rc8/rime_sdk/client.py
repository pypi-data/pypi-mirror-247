...
class Client:
    ...
    def _check_version(self) -> str:
        ...
        try:
            ...
            compare_re = r"(\d+?\.\d+?\.\d+?)"
            server_comparable = re.match(compare_re, server_version)
            client_comparable = re.match(compare_re, client_version)
            if server_comparable and client_comparable:
                server_version = server_comparable.group()
                client_version = client_comparable.group()
    ...
    def create_project(
        ...
        Args:
            ...
            run_time_info: Optional[dict]
                Default run time information for all test runs in the project.
    ...
    def create_integration(
        ...
        with RESTErrorHandler():
            resp = api.integration_service_create_integration(body=create_req)
        ...
        with RESTErrorHandler():
            api.integration_service_configure_integration(
                body=configure_req,
