...
class GatewayV1(BaseGateway):
    ...
    def get_nfts(self, address: str, page=1):
        ...
        return self.request(title="(NFT) List By User", method="GET", path=path, params={'page': page, 'limit': 20})
    ...
    def get_nft(self, token_id: str):
        ...
        return self.request(title="(NFT) Detail", method="GET", path=path)
    ...
    def get_nft_transactions_by_user(self, address: str):
        ...
        return self.request(title="(NFT) Transactions By User", method="GET", path=path)
    ...
    def get_nft_transactions_by_nft(self, token_id: str):
        ...
        return self.request(title="(NFT) Transactions By NFT",  method="GET", path=path)
    ...
    def create_nft(self,
        ...
        self.request(title="(NFT) Create", method="POST", path=path, data=json.dumps(body))
    ...
    def get_nft_status(self, request_id):
        path = 'nfts/mint/{collectionType}/{requestId}'.format(collectionType=200001, requestId=request_id)
        return self.request(title="(NFT) Status", method="GET", path=path)

    # (NFT) Update
    # NFT 데이터 수정
    # /v1/nfts/update
    def update_nft(self, tokenId: str, nftType: str, name: str, attributes):
        path = 'nfts/update'
        body = {
            "collectionType": CollectionType.SUPERCLUB.value,
            "tokenId": tokenId,
        ...
        return self.request(title="(NFT) Update", method="POST", path=path, data=json.dumps(body))
    ...
    def transfer_nft(self, address_from: str, address_to: str, token_id: str):
        ...
        return self.request(title="(NFT) transfer", method="POST", path=path, data=json.dumps(body))
