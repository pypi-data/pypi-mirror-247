...
class FireDucksPandasCompat(
    ...
    def _slice(self, slobj, axis=0):
        reason = None
        if axis != 0:
            reason = f"unspported axis: {axis} for slicing"

        step = slobj.step or 1
        if step != 1:
            reason = f"unspported step: {step} for slicing"
    ...
    def __repr__(self):
        if self._fireducks_meta.is_cached():
            return utils._wrap(self._fireducks_meta.get_cache().__repr__())
        ...
        if need_truncation:
            ...
            tmp_str = tmp._fallback_call(
                "__repr__", __fireducks_reason="with truncated data"
            )
            res = tmp_str[:-truncated_shape_info_len] + actual_shape_info
            pandas.set_option("display.max_rows", maxr)
            return res
        else:
            return self._fallback_call(
                "__repr__", __fireducks_reason=f"with full data of size: {n}"
            )
    ...
    def describe(self, *args, **kwargs):
        ...
        # both pandas.DataFrame.describe and pandas.Series.describe
        # share same signature
        arg = utils.decode_args(args, kwargs, pandas.DataFrame.describe)
        reason = None
