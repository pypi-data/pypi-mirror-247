...
from arthub_login_widgets.filesystem import run_exe
...
class LoginBackend(object):
    def __init__(
        ...
        api_config = api_config_qq_test if dev_mode else api_config_qq
        self.open_api = OpenAPI(api_config, False, None, False)
    ...
    def is_login(self):
        ...
        if not token:
            return False
        return self._on_get_token(token)

    def popup_login(self, login_window_x=None, login_window_y=None):
        args = self._get_args()
        if (login_window_x is not None) and (login_window_y is not None):
            args.append("--window-center-x={}".format(login_window_x))
            args.append("--window-center-y={}".format(login_window_y))
        r = self._call_exe(args)
        if not r[0]:
            return False
        return bool(r[1])

    def popup_admin(self, login_window_x=None, login_window_y=None):
        args = self._get_args()
        if (login_window_x is not None) and (login_window_y is not None):
            args.append("--window-center-x={}".format(login_window_x))
            args.append("--window-center-y={}".format(login_window_y))
    ...
    def popup_introduction(self, login_window_x=None, login_window_y=None):
        args = self._get_args()
        if (login_window_x is not None) and (login_window_y is not None):
            args.append("--window-center-x={}".format(login_window_x))
            args.append("--window-center-y={}".format(login_window_y))
    ...
    def _call_exe(self, args):
        exit_code, stdout = run_exe(self._exe_path, args)
        if exit_code != 0:
            return False, None
    ...
    def _on_get_token(self, token):
        self.open_api.set_token(token, False)
        res = self.open_api.get_account_detail()
        if res.is_succeeded():
            self._account_detail = res.first_result()
