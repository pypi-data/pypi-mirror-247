...
def _parse_filter_replacement_countdown(countdown: List[int]) -> str:
    hours_minutes: int = countdown[0]
    hours: int = (hours_minutes & 0xff00) >> 8
    minutes: int = hours_minutes & 0xff
    days: int = countdown[1]
...
class S21Client:
    ...
    def _poll(self) -> ClimateDevice:
        ...
        filter_timer: List[int] = holding_registers[IR_CurFILTER_TIMER_start:IR_CurFILTER_TIMER_end+1]

        x = _parse_filter_replacement_countdown(filter_timer)
