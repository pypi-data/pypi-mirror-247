...
class SCRIPro_Multiome():
    ...
    def get_tf(self):
        ...
        store = pd.HDFStore(target_h5_path)
        ...
        def cal_tf_score(row):
            ...
            return (nb(super_gene_exp.loc[supercell_name, query_gene] - super_gene_mean[query_gene])/super_gene_std[query_gene]).mean() + (super_gene_exp.loc[supercell_name, tf_name] - super_gene_mean[tf_name])/ super_gene_std[tf_name]
