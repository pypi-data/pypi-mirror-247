...
SKILL = LanguageSkill.KEY_PHRASE_EXTRACTION


# When `parse_response == True`, return extracted key phrases (list[str]).
# Else, return raw API json output (dict).
@tool
def get_key_phrase_extraction(connection: CustomConnection,
                              language: str,
                              text: str,
                              max_retries: int = 5,
                              max_wait: int = 60,
