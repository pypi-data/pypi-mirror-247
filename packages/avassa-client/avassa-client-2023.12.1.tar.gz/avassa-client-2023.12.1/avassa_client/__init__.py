...
def _send_request(metod, session: Session, url, payload, user_agent=None, ssl_context=None):
    ...
    req = urllib.request.Request(url, method=metod, headers=headers, data=data)
    response = urllib.request.urlopen(req, context=ssl_context)
    return (response.getcode(), response.msg, response.getheaders(),
            response.read().decode('utf-8'))


def post_request(session: Session,
                 url, payload,
                 user_agent=None,
                 ssl_context=None):
    return _send_request('POST',
                         session,
                         url,
                         payload,
                         user_agent,
...
def put_request(session: Session,
                url, payload,
                user_agent=None,
                ssl_context=None):
    return _send_request('PUT',
                         session,
                         url,
                         payload,
                         user_agent,
...
def approle_login(host: str,
    ...
    try:
        ...
        (code, msg, headers, body) = post_request(
