...
def realtime_tick(ts_code: str = "000001.SZ", src: Optional[str] = "tx") -> pd.DataFrame:
    """
    历史分笔数据
    :param ts_code: 股票代码
    :type ts_code: str
    ...
    if src == "sina":
        return get_stock_sina_a_divide_amount(symbol)
    else:
        return get_stock_tx_a_divide_amount(symbol)


def get_stock_tx_a_divide_amount(symbol: str = "sz000001") -> pd.DataFrame:
    """
    腾讯财经-历史分笔数据
...
def get_stock_sina_a_divide_amount(symbol: str = "sz000001") -> pd.DataFrame:
    """
    腾新浪财经-历史分笔数据
...
if __name__ == '__main__':
    df = realtime_tick(ts_code="000001.SZ", src="tx")
    print(df)
