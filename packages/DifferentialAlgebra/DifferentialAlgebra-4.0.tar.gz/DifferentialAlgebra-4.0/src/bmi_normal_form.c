 ...
 */
  ...
  ba0_sscanf2 (bmi_string_op (1, callback), "%t[%careful_expanded_Qz]", &A);
#else
  ba0_sscanf2 (bmi_string_op (1, callback), "%t[%Qz]", &A);
#endif
  ba0_init_table (&R);
  ...
  while (R.size < A.size)
    {
      bad_normal_form_ratfrac_mod_intersectof_regchain
          ((baz_tableof_ratfrac) R.tab[R.size], A.tab[R.size],
          &tabC, (bap_polynom_mpz *) 0);
      R.size += 1;
    }
