...
/*
				bap_commuting_derivations_mpzm ();
*/
  ...
  while (!bap_outof_itermon_mpzm (&iter))
    {
      if (commuting_derivations)
        diff_monome_mpzm (&R1, &rgtot, &iter, s, nulles);
/*
	else
	    diff_monome_comrel_mpzm (&R1, &iter, s, nulles);
*/
...
*/
          ...
          if (d > 1)
            ...
            bap_write_creator_mpzm
                (&crea, &U, *bap_coeff_itermon_mpzm (iter));
        }
...
/*
 * texinfo: bap_zero_derivatives_of_tableof_parameter_mpzm
...
BAP_DLL void
bap_zero_derivatives_of_tableof_parameter_mpzm (
  ...
  while (i < nulles.size)
    {
      ...
      for (j = 0; j < T->size && !found; j++)
        {
          w = bap_leader_polynom_mpzm (T->tab[j]);
 ...
 */
  ...
  for (i = 0; i < nulles.size; i++)
    {
      bap_set_polynom_variable_mpzm (T->tab[T->size], nulles.tab[i], 1);
      T->size += 1;
    }
