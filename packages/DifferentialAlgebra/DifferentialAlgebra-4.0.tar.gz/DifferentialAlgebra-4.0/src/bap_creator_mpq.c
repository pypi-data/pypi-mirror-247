...
bap_begin_creator_mpq (
    bap_creator_mpq crea,
    bap_polynom_mpq A,
    bav_term T,
    enum bap_typeof_total_rank type,
    ba0_int_p table2of_monom_alloc)
{
  if (A == (bap_polynom_mpq) 0 || A->readonly)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
  bap_begin_creator_clot_mpq (&crea->crea, A->clot, T, table2of_monom_alloc);
...
/*
 * texinfo: bap_append_creator_mpq
...
BAP_DLL void
bap_append_creator_mpq (
    bap_creator_mpq crea,
    bap_polynom_mpq A,
    ba0_int_p table2of_monom_alloc)
{
  if (A == (bap_polynom_mpq) 0)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
  if (A->readonly)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
  bap_append_creator_clot_mpq (&crea->crea, A->clot, table2of_monom_alloc);
  crea->poly = A;
  crea->type = bap_exact_total_rank;
}

/*
 * texinfo: bap_write_creator_mpq
...
bap_write_creator_mpq (
    ...
    ba0_mpq_t c)
{
  bap_write_creator_clot_mpq (&crea->crea, T, c);
}

/* Writes - c*T on crea.  The coefficient c may be zero. */

/*
 * texinfo: bap_write_neg_creator_mpq
 * Write @math{- c\,T} on the creator.
 * The coefficient may be zero.
 */

BAP_DLL void
bap_write_neg_creator_mpq (
    bap_creator_mpq crea,
    bav_term T,
    ba0_mpq_t c)
{
  bap_write_neg_creator_clot_mpq (&crea->crea, T, c);
}

/*
 * texinfo: bap_is_write_allable_creator_mpq
 * Return @code{true} if the function @code{bap_write_all_creator_mpq}
 * can be applied to @var{crea} and @var{A}.
 */

BAP_DLL bool
bap_is_write_allable_creator_mpq (
    bap_creator_mpq crea,
    bap_polynom_mpq A)
{
  return crea->poly->clot->ordering == A->clot->ordering &&
      bap_are_disjoint_polynom_mpq (crea->poly, A) &&
...
/*
 * texinfo: bap_write_all_creator_mpq
...
/*
 * texinfo: bap_write_neg_all_creator_mpq
...
/*
 * texinfo: bap_write_mul_all_creator_mpq
...
bap_write_exquo_all_creator_mpz (
    ...
    ba0_mpz_t c)
{
  bap_write_exquo_all_creator_clot_mpq
      (&crea->crea, A->clot, c, A->seq.first, A->seq.after);
}
...
BAP_DLL void
bap_close_creator_mpq (
    bap_creator_mpq crea)
{
  bap_polynom_mpq A;

  bap_close_creator_clot_mpq (&crea->crea);
