    ...
    struct bmi_callback *callback)
{
  ...
  ba0_realloc_table ((ba0_table) & result, T.size);

  for (i = 0; i < T.size; i++)
    {
      result.tab[i] = baz_is_constant_ratfrac
          (T.tab[i], x, &bav_global.parameters);
      result.size = i + 1;
    }
#if ! defined (BMI_BALSA)
  {
...
#else
  {
    ...
    ba0_init_table ((ba0_table) & result_string);
    ba0_realloc_table ((ba0_table) & result_string, result.size);
    for (i = 0; i < result.size; i++)
      {
