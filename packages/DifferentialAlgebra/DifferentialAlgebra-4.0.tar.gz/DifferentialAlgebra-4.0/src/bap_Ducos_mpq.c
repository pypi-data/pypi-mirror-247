...
/*
	...
	if (rg.deg == 0)
	    ...
	    bap_addmulrk_polynom_mpq (&A, &bunk, &rg, &A);
    }
    if (rg.deg > 0)
    {	struct bav_term T;
	bav_init_term (&T);
...
bap_nsr3_Ducos_polynom_mpq (
    bap_tableof_polynom_mpq VR,
    bap_tableof_polynom_mpq VP,
    bap_tableof_polynom_mpq VQ,
    bap_polynom_mpq s,
    bap_polynom_mpq ss,
    bav_variable v)
{
    ...
    bav_variable v)
{
  ...
  if (bap_leader_polynom_mpq (P) != v || bap_leader_polynom_mpq (Q) != v)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
...
*/
  ...
  ba0_record (&M);
/*
ba0_printf ("algo_new (%Aq, %Aq, %v)\n", P, Q, v);
*/
  bap_init_readonly_polynom_mpq (&coeff);
  bap_init_polynom_mpq (&s);
  bap_init_polynom_mpq (&Z);

  bap_initial_polynom_mpq (&coeff, Q);
  delta = bap_leading_degree_polynom_mpq (P) -
      bap_leading_degree_polynom_mpq (Q);
...
*/
      delta = bap_leading_degree_polynom_mpq (A) -
          bap_degree_polynom_mpq (B, v);
    ...
    bav_variable v)
{
  ...
  if (bap_is_zero_polynom_mpq (P) || bap_is_zero_polynom_mpq (Q))
  ...
  else
    {
      ...
      if (degQ == 0)
        bap_pow_polynom_mpq (R, Q, degP);
      algo_new_mpq (R, P, Q, v);
      if (oppose)
        {
          bap_neg_polynom_mpq (Q, Q);
          Q->readonly = b;
...
*/
  ...
  bap_init_readonly_polynom_mpq (&coeff);
  bap_init_polynom_mpq (&s);
  VP = (bap_tableof_polynom_mpq) ba0_new_table ();
  VQ = (bap_tableof_polynom_mpq) ba0_new_table ();
  ba0_init_table ((ba0_table) & VZ);
  ba0_realloc2_table
      ((ba0_table) VP, VR->size, (ba0_new_function *) & bap_new_polynom_mpq);
  ...
  if (VQ->size == 1)
    bap_pseudo_division_polynom_mpq
        ((bap_polynom_mpq) 0, VQ->tab[0], (bav_Idegree *) 0, P, Q, v);
  else if (VQ->size == 2)
    {
      if (perm)
        {
      ...
      else
        {
          ...
          bap_pseudo_division_polynom_mpq
              ((bap_polynom_mpq) 0, VQ->tab[0], &e, P, Q, v);
          bap_initial2_polynom_mpq (&coeff, Q, v);
  ...
  for (;;)
    {
      ...
      BA0_SWAP (bap_tableof_polynom_mpq, VP, VQ);
