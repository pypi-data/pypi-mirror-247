    ...
    bap_clot_mpzm C)
{
  ...
  bap_init_set_termstripper (&A->tstrip, (bav_variable) - 1, C->ordering);
  A->readonly = false;
}

/*
 * texinfo: bap_init_polynom_mpzm
 * Initialize @var{A} to @math{0}.
 */

BAP_DLL void
bap_init_polynom_mpzm (
    bap_polynom_mpzm A)
{
  struct bav_term T;
  bap_clot_mpzm C;
  ...
  bav_init_term (&T);
  C = bap_new_clot_mpzm (&T);
  bap_init_polynom_clot_mpzm (A, C);
}

/* 
 * texinfo: bap_init_readonly_polynom_mpzm
 * Initialize @var{A} to @math{0} as a readonly polynomial.
 */

BAP_DLL void
bap_init_readonly_polynom_mpzm (
    bap_polynom_mpzm A)
{
  A->clot = (bap_clot_mpzm) 0;
  bav_init_term (&A->total_rank);
  A->access = bap_sequential_monom_access;
  A->seq.first = 0;
  A->seq.after = 0;
  bap_init_indexed_access (&A->ind);
  bap_init_set_termstripper (&A->tstrip, (bav_variable) - 1,
      bav_R_Iordering ());
  A->readonly = true;
}

/*
 * texinfo: bap_init_polynom_one_mpzm
 * Initialize @var{A} to @math{1}.
 */

BAP_DLL void
bap_init_polynom_one_mpzm (
    bap_polynom_mpzm A)
{
  ...
  ba0_record (&M);
  ba0_mpzm_init_set_ui (un, (unsigned long int) 1);
  ba0_pull_stack ();

  bav_init_term (&T);
  C = bap_new_clot_mpzm (&T);
  bap_init_polynom_clot_mpzm (A, C);
  bap_begin_creator_mpzm (&crea, A, &T, bap_exact_total_rank, 1);
  bap_write_creator_mpzm (&crea, &T, un);
  bap_close_creator_mpzm (&crea);
  ba0_restore (&M);
}

/*
 * texinfo: bap_init_polynom_crk_mpzm
    ...
    bav_rank rg)
{
  ...
  bap_init_polynom_clot_mpzm (A, C);
  bap_begin_creator_mpzm (&crea, A, &T, bap_exact_total_rank, 1);
  if (!ba0_mpzm_is_zero (c) || !bav_is_zero_rank (rg))
...
/****************************************************************************
 ...
 Toutes les fonctions parametrees par une clot sont static.
 ****************************************************************************/

static bap_polynom_mpzm
bap_new_polynom_clot_mpzm (
    bap_clot_mpzm C)
{
  bap_polynom_mpzm A;

  A = (bap_polynom_mpzm) ba0_alloc (sizeof (struct bap_polynom_mpzm));
  bap_init_polynom_clot_mpzm (A, C);
  return A;
}

/*
 * texinfo: bap_new_polynom_mpzm
 * Allocate a new polynomial, initialize it and return it.
 */

BAP_DLL bap_polynom_mpzm
bap_new_polynom_mpzm (
    void)
{
  struct bav_term T;
  bap_clot_mpzm C;
  ...
  bav_init_term (&T);
  C = bap_new_clot_mpzm (&T);
  return bap_new_polynom_clot_mpzm (C);
}

/*
 * texinfo: bap_new_readonly_polynom_mpzm
 * Allocate a new polynomial, initialize it as a readonly polynomial
 * and return it.
 */

BAP_DLL bap_polynom_mpzm
bap_new_readonly_polynom_mpzm (
    void)
{
  bap_polynom_mpzm A;

  A = (bap_polynom_mpzm) ba0_alloc (sizeof (struct bap_polynom_mpzm));
  bap_init_readonly_polynom_mpzm (A);
  return A;
}

/*
 * texinfo: bap_new_polynom_crk_mpzm
    ...
    void)
{
  ...
  A = bap_new_polynom_crk_mpzm (c, &rg);
  ba0_restore (&M);
  return A;
}
...
/*
 * texinfo: bap_set_polynom_crk_mpzm
...
/*
 * texinfo: bap_set_polynom_variable_mpzm
 * Assign @math{v^d} to @var{A}.
 * Exception @code{BA0_ERRALG} is raised if @var{A} has readonly mode.
 */

BAP_DLL void
bap_set_polynom_variable_mpzm (
    bap_polynom_mpzm A,
    bav_variable v,
    bav_Idegree d)
{
...
/* 
 * texinfo: bap_set_polynom_term_mpzm
 * Assign @var{T} to @var{A}.
 * Exception @code{BA0_ERRALG} is raised if @var{A} has readonly mode.
 */

BAP_DLL void
bap_set_polynom_term_mpzm (
    bap_polynom_mpzm A,
    bav_term T)
{
  struct bap_creator_mpzm crea;
...
/* 
 * texinfo: bap_set_polynom_monom_mpzm
...
/*
 * texinfo: bap_set_polynom_mpzm
 * Assign @var{B} to @var{A}.
 * The two polynomials are disjoint.
 * Exception @code{BA0_ERRALG} is raised if @var{A} has readonly mode.
 */

BAP_DLL void
bap_set_polynom_mpzm (
    bap_polynom_mpzm A,
    bap_polynom_mpzm B)
{
  struct bap_creator_mpzm crea;
  ...
  if (bap_is_zero_polynom_mpzm (B))
    bap_set_polynom_zero_mpzm (A);
  else if (!bap_are_disjoint_polynom_mpzm (A, B))
    {
      if (A == B)
        BA0_RAISE_EXCEPTION (BA0_ERRALG);
    ...
    {
      nbmon = bap_nbmon_polynom_mpzm (B) - A->clot->alloc;
      ...
      bap_begin_creator_mpzm
          (&crea, A, &B->total_rank, bap_exact_total_rank, nbmon);

      if (bap_is_write_allable_creator_mpzm (&crea, B))
        bap_write_all_creator_mpzm (&crea, B);
      else
        {
          bap_begin_itermon_mpzm (&iter, B);
...
/* 
 * texinfo: bap_set_readonly_polynom_mpzm
    ...
    bap_polynom_mpzm A)
{
  ...
  if (A->access == bap_sequential_monom_access)
  ...
  else
    {
      ...
      nbmon = bap_nbmon_polynom_mpzm (A);
...
/*
 * texinfo: bap_is_zero_polynom_mpzm
 * Return @code{true} if @var{A} is equal to @math{0}.
 */

BAP_DLL bool
bap_is_zero_polynom_mpzm (
    bap_polynom_mpzm A)
{
  return bap_nbmon_polynom_mpzm (A) == 0;
}

/* 
 * texinfo: bap_is_one_polynom_mpzm
 * Return @code{true} if @var{A} is equal to @math{1}.
 */

BAP_DLL bool
bap_is_one_polynom_mpzm (
    bap_polynom_mpzm A)
{
  struct bap_itermon_mpzm iter;
  ba0_mpzm_t *lc;
  ...
  return ba0_mpzm_is_one (*lc);
}

/* 
 * texinfo: bap_is_numeric_polynom_mpzm
 * Return @code{true} if @var{A} does not depend on any variable.
 */

BAP_DLL bool
bap_is_numeric_polynom_mpzm (
    bap_polynom_mpzm A)
{
  return bap_is_zero_polynom_mpzm (A) || bav_is_one_term (&A->total_rank);
}

/* 
 * texinfo: bap_is_univariate_polynom_mpzm
 * Return @code{true} if @var{A} depends on a single variable.
 */

BAP_DLL bool
bap_is_univariate_polynom_mpzm (
    bap_polynom_mpzm A)
{
...
/* 
 * texinfo: bap_is_variable_polynom_mpzm
 * Return @code{true} if @var{A} is equal to a variable.
 */

BAP_DLL bool
bap_is_variable_polynom_mpzm (
    bap_polynom_mpzm A)
{
  return bav_total_degree_term (&A->total_rank) == 1 &&
      bap_nbmon_polynom_mpzm (A) == 1;
}

/*
 * texinfo: bap_is_solved_polynom_mpzm
 * Return @code{true} if @var{A} has the form @math{v - B}
 * where @var{v} is the leading variable of @var{A} and @var{B}
 * is any polynomial.
 */

BAP_DLL bool
bap_is_solved_polynom_mpzm (
    bap_polynom_mpzm A)
{
  struct bap_polynom_mpzm init;
  struct ba0_mark M;
  bool b;
  ...
  if (bap_leading_degree_polynom_mpzm (A) != 1)
    return false;

  ba0_record (&M);
  bap_init_readonly_polynom_mpzm (&init);
  bap_initial_polynom_mpzm (&init, A);
  b = bap_is_numeric_polynom_mpzm (&init);
  ba0_restore (&M);
...
/*
 * texinfo: bap_is_derivative_minus_independent_polynom_mpzm
...
BAP_DLL bool
bap_is_derivative_minus_independent_polynom_mpzm (
    bap_polynom_mpzm A)
{
  ba0_int_p i;

  if (!bap_is_solved_polynom_mpzm (A))
    return false;
...
/*
 * bap_is_rank_minus_monom_polynom_mpzm
    ...
    bap_polynom_mpzm A)
{
  ...
  bap_init_readonly_polynom_mpzm (&init);
  bap_initial_polynom_mpzm (&init, A);
  return bap_is_one_polynom_mpzm (&init);
}

/* 
 * texinfo: bap_depend_polynom_mpzm
 * Return @code{true} if @var{A} depends on @var{v}.
 */

BAP_DLL bool
bap_depend_polynom_mpzm (
    bap_polynom_mpzm A,
    bav_variable v)
{
  ba0_int_p i;
...
/*
 * texinfo: bap_depend_only_polynom_mpzm
 * Return @code{true} if @var{A} depends only on the variables 
 * present in @var{T}. The rable @var{T} is supposed to be sorted
 * in ascending order.
*/

BAP_DLL bool
bap_depend_only_polynom_mpzm (
    bap_polynom_mpzm A,
    bav_tableof_variable T)
{
  ...
  if (i < A->total_rank.size)
    n = bav_R_variable_number (A->total_rank.rg[i].var);
  j = T->size - 1;
  if (j >= 0)
    m = bav_R_variable_number (T->tab[j]);
...
/*
 * texinfo: bap_compare_polynom_mpzm
    ...
    bap_polynom_mpzm B)
{
  ...
  while (!bap_outof_itermon_mpzm (&iterA) && !bap_outof_itermon_mpzm (&iterB))
    {
      ...
      if (resultat == ba0_eq &&
          !ba0_mpzm_are_equal (*bap_coeff_itermon_mpzm (&iterA),
  ...
  if (resultat == ba0_eq || resultat == ba0_equiv)
    {
      if (!bap_outof_itermon_mpzm (&iterA))
        resultat = ba0_gt;
      else if (!bap_outof_itermon_mpzm (&iterB))
        resultat = ba0_lt;
    }
  ba0_restore (&M);
  return resultat;
...
/* 
 * texinfo: bap_equal_polynom_mpzm
 * Return @code{true} if @var{A} and @var{B} are equal.
 */

BAP_DLL bool
bap_equal_polynom_mpzm (
    bap_polynom_mpzm A,
    bap_polynom_mpzm B)
{
  struct bap_itermon_mpzm iterA, iterB;
  ...
  while (!bap_outof_itermon_mpzm (&iterA))
    {
      ...
      if (!ba0_mpzm_are_equal (*bap_coeff_itermon_mpzm (&iterA),
              *bap_coeff_itermon_mpzm (&iterB)))
        {
          ba0_restore (&M);
          return false;
...
/* 
 * texinfo: bap_gt_rank_polynom_mpzm
 * Return @code{true} if @var{P} has strictly greater rank than @var{B}.
 * Both parameters are polynomials.
 */

BAP_DLL bool
bap_gt_rank_polynom_mpzm (
    void *P,
    void *Q)
{
  bap_polynom_mpzm A = (bap_polynom_mpzm) P;
  bap_polynom_mpzm B = (bap_polynom_mpzm) Q;
  struct bav_rank Arg, Brg;

  Arg = bap_rank_polynom_mpzm (A);
  Brg = bap_rank_polynom_mpzm (B);

  return bav_gt_rank (&Arg, &Brg);
}

/* 
 * texinfo: bap_lt_rank_polynom_mpzm
 * Return @code{true} if @var{P} has strictly lower rank than @var{B}.
 * Both parameters are polynomials.
 */

BAP_DLL bool
bap_lt_rank_polynom_mpzm (
    void *P,
    void *Q)
{
  bap_polynom_mpzm A = (bap_polynom_mpzm) P;
...
/* 
 * texinfo: bap_equal_rank_polynom_mpzm
 * Return @code{true} if @var{P} and @var{Q} have the same rank.
 */

BAP_DLL bool
bap_equal_rank_polynom_mpzm (
    bap_polynom_mpzm P,
    bap_polynom_mpzm Q)
{
  struct bav_rank Prg, Qrg;

  Prg = bap_rank_polynom_mpzm (P);
  Qrg = bap_rank_polynom_mpzm (Q);

  return bav_equal_rank (&Prg, &Qrg);
}

/*
 * texinfo: bap_are_disjoint_polynom_mpzm
...
/* 
 * texinfo: bap_mark_indets_polynom_mpzm
 * Assign @code{true} to the field @code{hack} of all variables
 * occuring in @var{A}.
 */

BAP_DLL void
bap_mark_indets_polynom_mpzm (
    bap_polynom_mpzm A)
{
  ba0_int_p i;

  for (i = 0; i < A->total_rank.size; i++)
    A->total_rank.rg[i].var->hack = true;
}

/*
 * texinfo: bap_involved_parameters_polynom_mpzm
 * Append to @var{R} all the parameters which occur in @var{A}, are listed
 * in @var{P} and are not already present in @var{R}. 
 */

/*
 * texinfo: bap_involved_parameters_polynom_mpzm
 * Append to @var{R} the parameters which occur in @var{A}, are listed
 * in @var{P} and are not already present in @var{R}. 
 */

BAP_DLL void
bap_involved_parameters_polynom_mpzm (
    bav_tableof_parameter R,
    bap_polynom_mpzm A,
    bav_tableof_parameter P)
{
  ...
  ba0_realloc_table ((ba0_table) R, P->size);
  for (i = 0; i < A->total_rank.size; i++)
    {
      v = A->total_rank.rg[i].var;
      if (bav_is_a_parameter (v->root, &j, P) &&
          !bav_is_a_parameter (v->root, 0, R))
        {
          ba0_realloc_table ((ba0_table) R, R->size + 1);
          R->tab[R->size] = P->tab[j];
          R->size += 1;
        }
    }
}
...
/*
 * Sets the field hack of the variables occuring in A to the
 * degree of A in that variable (provided that the existing value
 * of that field is less than this degree).
 */

/*
 * texinfo: bap_mark_ranks_polynom_mpzm
 * Assign to the field @code{hack} of all variables occuring 
 * in @var{A} their degrees in @var{A}. 
 * This function is often used in conjunction with
 * @code{bav_marked_ranks_term}.
 */

BAP_DLL void
bap_mark_ranks_polynom_mpzm (
    bap_polynom_mpzm A)
{
  ba0_int_p i;

  for (i = 0; i < A->total_rank.size; i++)
    A->total_rank.rg[i].var->hack = A->total_rank.rg[i].deg;
}

/*
 * texinfo: bap_set_total_rank_polynom_mpzm
 * This low-level function computes the @code{total_rank} field
 * of @var{A}.
 */

BAP_DLL void
bap_set_total_rank_polynom_mpzm (
    bap_polynom_mpzm A)
{
  struct bap_itermon_mpzm iter;
  struct bav_term T;
  ...
  bav_marked_ranks_term (&A->total_rank);
  bav_sort_term (&A->total_rank);
}

/*
 * texinfo: bap_reverse_polynom_mpzm
...
BAP_DLL void
bap_reverse_polynom_mpzm (
    bap_polynom_mpzm A)
{
  if (A->readonly)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);

  bap_reverse_clot_mpzm (A->clot);
}
...
struct quicksort_data
{
  ...
  struct bap_itermon_mpzm i;
  struct bap_itermon_mpzm j;
  struct bap_itermon_mpzm k;
  struct bav_term Ti;
  struct bav_term Tj;
  struct bav_term Tk;
    ...
    bap_polynom_mpzm R)
{
  ...
  bap_begin_itermon_mpzm (&qs->i, R);
  bap_begin_itermon_mpzm (&qs->j, R);
  bap_begin_itermon_mpzm (&qs->k, R);
 ...
 */
  ...
  if (r - l > 8)
    {
      i = (alpha[qs->zi] * (l + 2) + beta[qs->zi] * (r - 2)) /
          (alpha[qs->zi] + beta[qs->zi]);
      qs->zi = (qs->zi + 1) % 7;
      j = (alpha[qs->zj] * (l + 1) + beta[qs->zj] * (i - 1)) /
          (alpha[qs->zj] + beta[qs->zj]);
      qs->zj = (qs->zj + 3) % 7;
      k = (alpha[qs->zk] * (i + 1) + beta[qs->zk] * (r - 1)) /
          (alpha[qs->zk] + beta[qs->zk]);
      qs->zk = (qs->zk + 5) % 7;
      if (i == j || i == k || j == k)
        BA0_RAISE_EXCEPTION (BA0_ERRALG);
      bap_goto_itermon_mpzm (&qs->i, i);
      bap_goto_itermon_mpzm (&qs->j, j);
      bap_goto_itermon_mpzm (&qs->k, k);

      bap_term_itermon_mpzm (&qs->Ti, &qs->i);
      bap_term_itermon_mpzm (&qs->Tj, &qs->j);
      bap_term_itermon_mpzm (&qs->Tk, &qs->k);
/*
...
*/
  ...
  else
    {
      ...
      for (i = r - 1; !sorted && i >= l; i--)
        {
          sorted = true;
          bap_goto_itermon_mpzm (&qs->j, l);
          bap_term_itermon_mpzm (suiv, &qs->j);
          for (j = l; j <= i; j++)
            {
              BA0_SWAP (bav_term, cour, suiv);
              bap_set_iterator_indexed_access (&i_cour, &qs->j.iter_ix);
              bap_next_itermon_mpzm (&qs->j);
              bap_term_itermon_mpzm (suiv, &qs->j);
    ...
    bap_polynom_mpzm A)
{
  ...
  nbmonA = bap_nbmon_polynom_mpzm (A);
  if (nbmonA == 0)
    {
      ...
      else
        {
          ...
          for (i = 0; i < nbmonA; i++)
            bap_write_creator_indexed_access
                (&crea, bap_read_iterator_indexed_access (&iter));
        }
      bap_close_creator_indexed_access (&crea);
      bap_set_termstripper (&R->tstrip, &A->tstrip);
      bav_set_term (&R->total_rank, &A->total_rank);
  ...
  bap_append_termstripper (&R->tstrip, (bav_variable) - 1, bav_R_Iordering ());

  ba0_record (&M);
  bap_init_quicksort_data (&qs, R);
  quicksort_mpzm (0, nbmonA - 1, &qs);
  ba0_restore (&M);
  R->readonly = true;
}

/*
 * texinfo: bap_physort_polynom_mpzm
...
BAP_DLL void
bap_physort_polynom_mpzm (
    bap_polynom_mpzm A)
{
  if (A->access == bap_indexed_monom_access)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
  bap_sort_clot_mpzm (A->clot, A->seq.first, A->seq.after);
  bav_sort_term (&A->total_rank);
  bap_change_ordering_termstripper (&A->tstrip, bav_R_Iordering ());
  bap_change_ordering_clot_mpzm (A->clot, bav_R_Iordering ());
}

/* 
 * texinfo: bap_nbmon_polynom_mpzm
 * Return the number of monomials present in @var{A}.
 */

BAP_DLL ba0_int_p
bap_nbmon_polynom_mpzm (
    bap_polynom_mpzm A)
{
  if (A->access == bap_sequential_monom_access)
...
/*
 * texinfo: bap_leading_degree_polynom_mpzm
...
BAP_DLL bav_Idegree
bap_leading_degree_polynom_mpzm (
    bap_polynom_mpzm A)
{
  if (bap_is_numeric_polynom_mpzm (A))
    BA0_RAISE_EXCEPTION (BAP_ERRCST);
...
/*
 * texinfo: bap_total_order_polynom_mpzm
 * Return the sum of the orders of all the derivatives @var{A}
 * depends on.
 */

BAP_DLL bav_Iorder
bap_total_order_polynom_mpzm (
    bap_polynom_mpzm A)
{
...
/*
 * texinfo: bap_minimal_total_rank_polynom_mpzm
 * Assign to @var{T} the gcd of all the terms occuring in @var{A}.
 * Exception @code{BAP_ERRNUL} is raised if @var{A} is @math{0}.
 */

BAP_DLL void
bap_minimal_total_rank_polynom_mpzm (
    bav_term T,
    bap_polynom_mpzm A)
{
  struct bap_itermon_mpzm iter;
  ...
  while (!bap_outof_itermon_mpzm (&iter))
    {
      ...
      for (j = 0; j < U.size; j++)
        {
          while (A->total_rank.rg[i].var != U.rg[j].var)
            {
              A->total_rank.rg[i].var->hack = 0;
              i++;
            }
          if (A->total_rank.rg[i].var->hack > U.rg[j].deg)
            A->total_rank.rg[i].var->hack = U.rg[j].deg;
          i++;
        }
  ...
  for (i = j = 0; i < A->total_rank.size; i++)
    {
      v = A->total_rank.rg[i].var;
      if (v->hack > 0)
        {
...
/*
 * texinfo: bap_leading_term_polynom_mpzm
 * Assign to @var{T} the leading term of @var{A}.
 * Exception @code{BAP_ERRNUL} is raised if @var{A} is @math{0}.
 */

BAP_DLL void
bap_leading_term_polynom_mpzm (
    bav_term T,
    bap_polynom_mpzm A)
{
  struct bap_itermon_mpzm iter;
...
/* 
 * texinfo: bap_numeric_initial_polynom_mpzm
...
BAP_DLL ba0_mpzm_t *
bap_numeric_initial_polynom_mpzm (
    bap_polynom_mpzm A)
{
  struct bap_itermon_mpzm iter;
  ba0_mpzm_t *c;
...
/*
 * texinfo: bap_initial_and_reductum_polynom_mpzm
...
BAP_DLL void
bap_initial_and_reductum_polynom_mpzm (
    bap_polynom_mpzm initial,
    bap_polynom_mpzm reductum,
    bap_polynom_mpzm A)
{
  ...
  if (bap_is_numeric_polynom_mpzm (A))
    BA0_RAISE_EXCEPTION (BAP_ERRCST);
  if ((A == initial && reductum != BAP_NOT_A_POLYNOM_mpzm) ||
      (A == initial && A == reductum))
    BA0_RAISE_EXCEPTION (BA0_ERRNYP);
  bap_begin_itercoeff_mpzm (&iter, A, bap_leader_polynom_mpzm (A));
  if (initial != BAP_NOT_A_POLYNOM_mpzm)
    bap_coeff_itercoeff_mpzm (initial, &iter);
  if (reductum != BAP_NOT_A_POLYNOM_mpzm)
    {
      bap_next_itermon_mpzm (&iter.fin);
      bap_reductum_itermon_mpzm (&iter.fin, reductum);
    }
}

/*
 * texinfo: bap_initial_polynom_mpzm
...
BAP_DLL void
bap_initial_polynom_mpzm (
    bap_polynom_mpzm R,
    bap_polynom_mpzm A)
{
  bap_initial_and_reductum_polynom_mpzm (R, BAP_NOT_A_POLYNOM_mpzm, A);
}

/*
 * texinfo: bap_reductum_polynom_mpzm
...
BAP_DLL void
bap_reductum_polynom_mpzm (
    bap_polynom_mpzm R,
    bap_polynom_mpzm A)
{
  bap_initial_and_reductum_polynom_mpzm (BAP_NOT_A_POLYNOM_mpzm, R, A);
}

/*
 * texinfo: bap_initial_and_reductum2_polynom_mpzm
 * Variant of @code{bap_initial_and_reductum_polynom_mpzm}
...
BAP_DLL void
bap_initial_and_reductum2_polynom_mpzm (
    bap_polynom_mpzm initial,
    bap_polynom_mpzm reductum,
    bap_polynom_mpzm A,
    bav_variable v)
{
  if (bap_is_numeric_polynom_mpzm (A) || bap_leader_polynom_mpzm (A) != v)
    {
      if (initial != BAP_NOT_A_POLYNOM_mpzm)
        bap_set_readonly_polynom_mpzm (initial, A);
      if (reductum != BAP_NOT_A_POLYNOM_mpzm)
        bap_set_polynom_zero_mpzm (reductum);
    }
  else
    bap_initial_and_reductum_polynom_mpzm (initial, reductum, A);
}

/*
 * texinfo: bap_initial2_polynom_mpzm
 * Variant of @code{bap_initial_polynom_mpzm}.
 * See @code{bap_initial_and_reductum2_polynom_mpzm}.
 */

BAP_DLL void
bap_initial2_polynom_mpzm (
    bap_polynom_mpzm R,
    bap_polynom_mpzm A,
    bav_variable v)
{
  bap_initial_and_reductum2_polynom_mpzm (R, BAP_NOT_A_POLYNOM_mpzm, A, v);
}

/*
 * texinfo: bap_reductum2_polynom_mpzm
 * Variant of @code{bap_reductum_polynom_mpzm}.
 * See @code{bap_initial_and_reductum2_polynom_mpzm}.
 */

BAP_DLL void
bap_reductum2_polynom_mpzm (
    bap_polynom_mpzm R,
    bap_polynom_mpzm A,
    bav_variable v)
{
  bap_initial_and_reductum2_polynom_mpzm (BAP_NOT_A_POLYNOM_mpzm, R, A, v);
}

/*
 * texinfo: bap_coeff2_polynom_mpzm
    ...
    bav_Idegree d)
{
  if (bap_is_numeric_polynom_mpzm (A) || bap_leader_polynom_mpzm (A) != v)
    {
      if (d == 0)
        bap_set_readonly_polynom_mpzm (R, A);
      else
        bap_set_polynom_zero_mpzm (R);
    }
  else
    {
      ...
      bap_begin_itercoeff_mpzm (&iter, A, v);
      bap_seek_coeff_itercoeff_mpzm (R, &iter, &T);
      ba0_restore (&M);
    }
}

/*
 * texinfo: bap_lcoeff_and_reductum_polynom_mpzm
...
BAP_DLL void
bap_lcoeff_and_reductum_polynom_mpzm (
    bap_polynom_mpzm lcoeff,
    bap_polynom_mpzm reductum,
    bap_polynom_mpzm A,
    bav_variable v)
{
  struct bap_polynom_mpzm AA, lc, red;
  struct ba0_mark M;

  if ((lcoeff != BAP_NOT_A_POLYNOM_mpzm && lcoeff->readonly) ||
      (reductum != BAP_NOT_A_POLYNOM_mpzm && reductum->readonly))
    BA0_RAISE_EXCEPTION (BA0_ERRALG);

  if (bap_is_numeric_polynom_mpzm (A) ||
      (v != BAV_NOT_A_VARIABLE && v != bap_leader_polynom_mpzm (A)))
    {
      if (!bap_depend_polynom_mpzm (A, v))
        {
          if (lcoeff != BAP_NOT_A_POLYNOM_mpzm && lcoeff != A)
            bap_set_polynom_mpzm (lcoeff, A);
          if (reductum != BAP_NOT_A_POLYNOM_mpzm)
            bap_set_polynom_zero_mpzm (reductum);
        }
      else
        {
          ...
          bap_init_readonly_polynom_mpzm (&AA);
          bap_init_readonly_polynom_mpzm (&lc);
          bap_init_readonly_polynom_mpzm (&red);
          bap_sort_polynom_mpzm (&AA, A);
          bap_initial_and_reductum_polynom_mpzm
              (lcoeff != BAP_NOT_A_POLYNOM_mpzm ?
              &lc : BAP_NOT_A_POLYNOM_mpzm,
              reductum != BAP_NOT_A_POLYNOM_mpzm ?
              &red : BAP_NOT_A_POLYNOM_mpzm, &AA);

          bav_R_pull_ordering ();
          ba0_pull_stack ();
          if (lcoeff != BAP_NOT_A_POLYNOM_mpzm)
            bap_set_polynom_mpzm (lcoeff, &lc);
          if (reductum != BAP_NOT_A_POLYNOM_mpzm)
            bap_set_polynom_mpzm (reductum, &red);
...
/*
 * texinfo: bap_lcoeff_polynom_mpzm
 * Variant of @code{bap_lcoeff_and_reductum_polynom_mpzm}
 * for the leading coefficient only.
 */

BAP_DLL void
bap_lcoeff_polynom_mpzm (
    bap_polynom_mpzm lcoeff,
    bap_polynom_mpzm A,
    bav_variable v)
{
  bap_lcoeff_and_reductum_polynom_mpzm (lcoeff, BAP_NOT_A_POLYNOM_mpzm, A, v);
}

/*
 * texinfo: bap_coeff_and_reductum_polynom_mpzm
...
BAP_DLL void
bap_coeff_and_reductum_polynom_mpzm (
    bap_polynom_mpzm C,
    bap_polynom_mpzm R,
    bap_polynom_mpzm A,
    bav_variable v,
...
{
  struct bap_itermon_mpzm iter;
  struct bap_creator_mpzm creaC, creaR;
  struct bap_polynom_mpzm CC, RR;
  struct bav_term T, U;
  struct ba0_mark M;
  ba0_int_p i, nbmon;

  if (R == (bap_polynom_mpzm) 0)
    {
      bap_coeff_polynom_mpzm (C, A, v, d);
      return;
    }
  else if (R->readonly)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
  if (C != (bap_polynom_mpzm) 0 && C->readonly)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
  if (C == R)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);

  if (bap_is_numeric_polynom_mpzm (A) ||
      (v != BAV_NOT_A_VARIABLE && !bap_depend_polynom_mpzm (A, v)))
    {
      if (d == 0)
        {
          if (C != A && C != (bap_polynom_mpzm) 0)
            bap_set_polynom_mpzm (C, A);
          bap_set_polynom_zero_mpzm (R);
        }
      else
        {
          if (C != (bap_polynom_mpzm) 0)
            bap_set_polynom_zero_mpzm (C);
          if (R != A)
  ...
  if (C != A && R != A)
    {
      ...
      if (C != (bap_polynom_mpzm) 0)
        bap_begin_creator_mpzm
            (&creaC, C, &A->total_rank, bap_approx_total_rank, nbmon / 2 + 1);
      bap_begin_creator_mpzm (&creaR, R, &A->total_rank,
          bap_approx_total_rank, nbmon / 2 + 1);
      ba0_push_another_stack ();
    }
  else
    {
      bap_init_polynom_mpzm (&CC);
      bap_init_polynom_mpzm (&RR);
      if (C != (bap_polynom_mpzm) 0)
        bap_begin_creator_mpzm
            (&creaC, &CC, &A->total_rank, bap_approx_total_rank, nbmon / 2 + 1);
      bap_begin_creator_mpzm (&creaR, &RR, &A->total_rank,
          bap_approx_total_rank, nbmon / 2 + 1);
    }
          ...
          if (C != A && R != A)
            {
              ...
              if (C != (bap_polynom_mpzm) 0)
                bap_write_creator_mpzm
                    (&creaC, &U, *bap_coeff_itermon_mpzm (&iter));
              ba0_push_another_stack ();
            }
          else
            bap_write_creator_mpzm
          ...
          if (C != A && R != A)
            {
              ba0_pull_stack ();
              bap_write_creator_mpzm
                  (&creaR, &T, *bap_coeff_itermon_mpzm (&iter));
              ba0_push_another_stack ();
            }
          else
            bap_write_creator_mpzm
  ...
  if (C != A && R != A)
    {
      ba0_pull_stack ();
      if (C != (bap_polynom_mpzm) 0)
        bap_close_creator_mpzm (&creaC);
      bap_close_creator_mpzm (&creaR);
      ba0_push_another_stack ();
    }
  else
    {
  ...
  if (C == A || R == A)
    {
      if (C != (bap_polynom_mpzm) 0)
        bap_set_polynom_mpzm (C, &CC);
      bap_set_polynom_mpzm (R, &RR);
    }
  ba0_restore (&M);
}

/* 
 * texinfo: bap_degree_polynom_mpzm
 * Return the degree of @var{A} w.r.t. @var{v}.
 * Return @math{-1} if @var{A} is @math{0}.
 */

BAP_DLL bav_Idegree
bap_degree_polynom_mpzm (
    bap_polynom_mpzm A,
    bav_variable v)
{
  ba0_int_p i;
...
/*
 * texinfo: bap_coeff_polynom_mpzm
 * Assign to @var{C} the coefficient of @var{v^d} in @var{A}
 * (the leading variable of @var{A} is taken if @var{v} is 
 * @code{BAV_NOT_A_VARIABLE}).
 * The variable @var{v} is not required to be greater than
...
{
  struct bap_itercoeff_mpzm iter;
  struct bap_polynom_mpzm AA, coeff;
  struct bav_term T;
  struct ba0_mark M;
  ...
  if (bap_is_numeric_polynom_mpzm (A) ||
  ...
  else
    {
      ...
      bap_init_readonly_polynom_mpzm (&coeff);
      bav_init_term (&T);
      bav_set_term_variable (&T, bap_leader_polynom_mpzm (A), d);
      bap_begin_itercoeff_mpzm (&iter, A, bap_leader_polynom_mpzm (A));
      bap_seek_coeff_itercoeff_mpzm (&coeff, &iter, &T);
...
/*
 * texinfo: bap_total_degree_polynom_mpzm
 * Return the total degree of @var{A}.
 * Return @math{-1} if @var{A} is @math{0}.
 */

BAP_DLL bav_Idegree
bap_total_degree_polynom_mpzm (
    bap_polynom_mpzm A)
{
...
/*
 * texinfo: bap_replace_initial_polynom_mpzm
...
BAP_DLL void
bap_replace_initial_polynom_mpzm (
    bap_polynom_mpzm R,
    bap_polynom_mpzm A,
    bap_polynom_mpzm C)
{
  ...
  bap_begin_creator_mpzm (&crea, P, &T, bap_approx_total_rank,
      bap_nbmon_polynom_mpzm (A) + bap_nbmon_polynom_mpzm (C));

  rg = bap_rank_polynom_mpzm (A);
  bap_begin_itermon_mpzm (&itermon, C);
  ...
  bap_begin_itercoeff_mpzm (&iter, A, bap_leader_polynom_mpzm (A));
  bap_next_itermon_mpzm (&iter.fin);
  while (!bap_outof_itermon_mpzm (&iter.fin))
    {
      bap_term_itermon_mpzm (&T, &iter.fin);
      lc = bap_coeff_itermon_mpzm (&iter.fin);
      bap_write_creator_mpzm (&crea, &T, *lc);
      bap_next_itermon_mpzm (&iter.fin);
    }
  bap_close_creator_mpzm (&crea);
...
/*
 * texinfo: bap_separant_polynom_mpzm
    ...
    bap_polynom_mpzm A)
{
  ...
  P = bap_new_polynom_mpzm ();
  bap_begin_creator_mpzm (&crea, P, &T,
      bap_approx_total_rank, bap_nbmon_polynom_mpzm (A));
  ...
  for (;;)
    {
      if (T.rg[0].deg == 1)
        {
          bav_shift_term (&T, &T);
          bap_write_creator_mpzm (&crea, &T, *bap_coeff_itermon_mpzm (&iter));
        }
      else
        {
          ba0_mpzm_mul_ui (c, *bap_coeff_itermon_mpzm (&iter), T.rg[0].deg);
          T.rg[0].deg -= 1;
          bap_write_creator_mpzm (&crea, &T, c);
        }
      bap_next_itermon_mpzm (&iter);
      if (bap_outof_itermon_mpzm (&iter))
        break;
      bap_term_itermon_mpzm (&T, &iter);
...
/*
 * texinfo: bap_separant2_polynom_mpzm
    ...
    bav_variable v)
{
  ...
  if (!bap_depend_polynom_mpzm (A, v))
    bap_set_polynom_zero_mpzm (R);
  else if (bap_leader_polynom_mpzm (A) == v)
    bap_separant_polynom_mpzm (R, A);
  else
    {
      ba0_push_another_stack ();
      ba0_record (&M);
...
comp_polynom_ascending (
    ...
    const void *y)
{
  bap_polynom_mpzm A = *(bap_polynom_mpzm *) x;
  bap_polynom_mpzm B = *(bap_polynom_mpzm *) y;
  struct bav_term TA, TB;
  struct ba0_mark M;
  enum ba0_compare_code code;
  ...
  bav_init_term (&TA);
  bav_init_term (&TB);
  bap_leading_term_polynom_mpzm (&TA, A);
  bap_leading_term_polynom_mpzm (&TB, B);
  code = bav_compare_term (&TA, &TB);
  ba0_restore (&M);
...
/*
 * texinfo: bap_sort_tableof_polynom_mpzm
 * Sort @var{T} in ascending or descending order 
 * depending on @var{mode}.
 */

BAP_DLL void
bap_sort_tableof_polynom_mpzm (
    bap_tableof_polynom_mpzm T,
    enum ba0_sort_mode mode)
{
  switch (mode)
    {
    case ba0_descending_mode:
      qsort (T->tab, T->size, sizeof (bap_polynom_mpzm),
          &comp_polynom_descending);
      break;
...
static char _struct_polynom[] = "struct bap_polynom_mpzm";
static char _struct_polynom_rang[] = "bap_polynom_mpzm->total_rank";

BAP_DLL ba0_int_p
bap_garbage1_polynom_mpzm (
    void *AA,
    enum ba0_garbage_code code)
{
  bap_polynom_mpzm A = (bap_polynom_mpzm) AA;
  ba0_int_p n = 0;

  if (code == ba0_isolated)
    n += ba0_new_gc_info
        (A, sizeof (struct bap_polynom_mpzm), _struct_polynom);

  n += bap_garbage1_clot_mpzm (A->clot, ba0_isolated);
    ...
    enum ba0_garbage_code code)
{
  ...
  if (code == ba0_isolated)
    A = (bap_polynom_mpzm) ba0_new_addr_gc_info (AA, _struct_polynom);
  else
    A = (bap_polynom_mpzm) AA;

  A->clot = (bap_clot_mpzm) bap_garbage2_clot_mpzm (A->clot, ba0_isolated);

  if (A->total_rank.rg != (struct bav_rank *) 0)
    A->total_rank.rg = (struct bav_rank *) ba0_new_addr_gc_info
        (A->total_rank.rg, _struct_polynom_rang);
    ...
    void *AA)
{
  ...
  B = bap_new_polynom_mpzm ();
  bap_set_polynom_mpzm (B, A);
  return B;
}

/*
 * texinfo: bap_sizeof_polynom_mpzm
 * Return the number of bytes used to store @var{F}.
 */

BAP_DLL unsigned ba0_int_p
bap_sizeof_polynom_mpzm (
    bap_polynom_mpzm F)
{
  bap_polynom_mpzm P;
  struct ba0_mark A;
...
/*
 * texinfo: bap_switch_ring_polynom_mpzm
 * Apply @code{bav_switch_ring_variable} over all the variables occuring
...
BAP_DLL void
bap_switch_ring_polynom_mpzm (
    bap_polynom_mpzm A,
    bav_differential_ring R)
{
  bav_switch_ring_term (&A->total_rank, R);
  bap_switch_ring_termstripper (&A->tstrip, R);
  if (A->clot)
    bap_switch_ring_clot_mpzm (A->clot, R);
}
