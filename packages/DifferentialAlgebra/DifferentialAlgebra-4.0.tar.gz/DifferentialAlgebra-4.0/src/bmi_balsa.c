...
static struct bmi_balsa_object tjet_notation =
    { bmi_balsa_string_object, BMI_IX_tjet, false, 0 };
static struct bmi_balsa_object jet0_notation =
    { bmi_balsa_string_object, BMI_IX_jet0, false, 0 };
static struct bmi_balsa_object diff_notation =
    { bmi_balsa_string_object, BMI_IX_diff, false, 0 };
static struct bmi_balsa_object udif_notation =
    { bmi_balsa_string_object, BMI_IX_udif, false, 0 };
    ...
    M_INT size)
{
  ...
  L = (struct bmi_balsa_list *) malloc (sizeof (struct
          bmi_balsa_list));
 ...
 * The behaviour is reproduced by looking at the input/output functions
 */
    ...
    void)
{
  ...
  bav_get_settings_variable (&s, &f, 0, 0);

  if (s == &bav_scanf_sympy_Derivative_variable)
    return &Derivative_notation;
  else if (s == &bav_scanf_sage_D_variable)
    return &D_notation;
  else if (f == &bav_printf_jet_variable)
    return &jet_notation;
  else if (f == &bav_printf_jet_wesb_variable)
    return &tjet_notation;
  else if (f == &bav_printf_jet0_variable)
    return &jet0_notation;
  else if (f == &bav_printf_diff_variable)
    return &diff_notation;
  else
    {
      assert (f == &bav_printf_Diff_variable);
      return &udif_notation;
    }
}

/*
 * In DifferentialAlgebra, the tables are created at the MAPLE level.
 */

static struct bmi_balsa_table *new_table (
    ALGEB type,
    ALGEB ordering,
    ALGEB equations)
{
  ...
  T = (struct bmi_balsa_table *) malloc (sizeof (struct
          bmi_balsa_table));
  T->type = type;
