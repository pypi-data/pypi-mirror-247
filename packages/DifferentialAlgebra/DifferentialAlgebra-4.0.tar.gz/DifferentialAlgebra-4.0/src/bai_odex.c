    ...
    bai_odex_system S)
{
  ...
  ba0_init_table ((ba0_table) & S->lhs);
  ba0_init_table ((ba0_table) & S->rhs);
  ba0_init_table ((ba0_table) & S->params);
  ba0_init_table ((ba0_table) & S->commands);
}
...
{
  S->t = BAV_NOT_A_VARIABLE;
  ba0_reset_table ((ba0_table) & S->lhs);
  ba0_reset_table ((ba0_table) & S->rhs);
  ba0_reset_table ((ba0_table) & S->params);
  ba0_reset_table ((ba0_table) & S->commands);
}
...
BAI_DLL bai_odex_system
bai_new_odex_system (
    void)
{
  bai_odex_system S;

  S = (bai_odex_system) ba0_alloc (sizeof (struct bai_odex_system));
    ...
    bai_odex_system src)
{
  ...
  if (dst != src)
    {
      dst->t = src->t;
      ba0_set_table ((ba0_table) & dst->lhs, (ba0_table) & src->lhs);
      dst->rhs.size = 0;
      ba0_realloc2_table ((ba0_table) & dst->rhs, src->rhs.size,
          (ba0_new_function *) & baz_new_ratfrac);
      for (i = 0; i < src->rhs.size; i++)
        {
          baz_set_ratfrac (dst->rhs.tab[dst->rhs.size++], src->rhs.tab[i]);
        }
      ba0_set_table ((ba0_table) & dst->params, (ba0_table) & src->params);
      ba0_set_table ((ba0_table) & dst->commands, (ba0_table) & src->commands);
    }
}
...
bai_set_odex_system_tables (
    ...
    bav_tableof_variable depvars,
    baz_tableof_ratfrac rhs)
{
  ...
  if (!ba0_is_unique_table ((ba0_table) params) ||
      !ba0_is_unique_table ((ba0_table) commands) ||
      !ba0_is_unique_table ((ba0_table) depvars))
    BA0_RAISE_EXCEPTION (BAI_ERROXS);
/*
  ...
  for (i = 0; i < depvars->size; i++)
    {
      if (bav_total_order_variable (depvars->tab[i]) != 0)
        BA0_RAISE_EXCEPTION (BAI_ERROXS);
      if (ba0_member_table (depvars->tab[i], (ba0_table) params) ||
          ba0_member_table (depvars->tab[i], (ba0_table) commands))
        BA0_RAISE_EXCEPTION (BAI_ERROXS);
    }
/*
 ...
 */
  for (i = 0; i < params->size; i++)
    {
      if (ba0_member_table (params->tab[i], (ba0_table) commands))
        BA0_RAISE_EXCEPTION (BAI_ERROXS);
    }
/*
 ...
 */
  ...
  for (i = 0; i < rhs->size; i++)
    {
      bap_mark_indets_polynom_mpz (&rhs->tab[i]->numer);
      bap_mark_indets_polynom_mpz (&rhs->tab[i]->denom);
    }
  ...
  for (i = 0; i < T.size; i++)
    {
      if (!ba0_member_table (T.tab[i], (ba0_table) params) &&
          !ba0_member_table (T.tab[i], (ba0_table) commands) &&
          !ba0_member_table (T.tab[i], (ba0_table) depvars))
        BA0_RAISE_EXCEPTION (BAI_ERROXS);
    }
 ...
 */
  S->t = t;
  ba0_set_table ((ba0_table) & S->params, (ba0_table) params);
  ba0_set_table ((ba0_table) & S->commands, (ba0_table) commands);
  ba0_set_table ((ba0_table) & S->lhs, (ba0_table) depvars);
/*
 ...
 */
  ...
  S->rhs.size = 0;
  ba0_realloc2_table ((ba0_table) & S->rhs, rhs->size,
      (ba0_new_function *) & baz_new_ratfrac);
  for (i = 0; i < rhs->size; i++)
    {
    ...
    bad_regchain C)
{
  ...
  ba0_init_table ((ba0_table) & rhs);
  ba0_realloc2_table ((ba0_table) & rhs, depvars->size,
      (ba0_new_function *) & baz_new_ratfrac);
  bap_init_polynom_mpz (&leader);
  for (i = 0; i < depvars->size; i++)
    {
      if (bav_symbol_type_variable (depvars->tab[i]) !=
      ...
      bad_normal_form_polynom_mod_regchain
          (rhs.tab[rhs.size], &leader, C, (bap_polynom_mpz *) 0);
      rhs.size += 1;
    }
...
bai_scanf_odex_system (
    void *S0)
{
  bai_odex_system S = (bai_odex_system) S0;
 ...
 */
  ...
  ba0_init_table ((ba0_table) & Tparams);
  ba0_init_table ((ba0_table) & Tcommands);
  ba0_init_table ((ba0_table) & Tdepvars);
  ba0_init_table ((ba0_table) & Trhs);
          ...
          for (;;)
            {
              if (Tdepvars.size == Tdepvars.alloc)
                ba0_realloc_table
                    ((ba0_table) & Tdepvars, 2 * Tdepvars.size + 1);
              if (Trhs.size == Trhs.alloc)
                ba0_realloc2_table
                    ((ba0_table) & Trhs, 2 * Trhs.size + 1,
                    (ba0_new_function *) & baz_new_ratfrac);
/*
 * Temporarily, the Tdepvars elements are not forced to order zero.
 */
              ba0_scanf ("%v = %Qz",
                  &Tdepvars.tab[Tdepvars.size], Trhs.tab[Trhs.size]);
              Tdepvars.size += 1;
              Trhs.size += 1;
              ba0_get_token_analex ();
              if (!ba0_sign_token_analex (","))
  ...
  if (!indep)
    {
      if (bav_global.R.ders.size == 1)
        t = bav_R_derivation_index_to_derivation (0);
      else
        BA0_RAISE_EXCEPTION (BAI_ERROXS);
    }
/*
    ...
    enum ba0_garbage_code code)
{
  ...
  if (code == ba0_isolated)
    S = (bai_odex_system) ba0_new_addr_gc_info (S0, _odex);
  else
    S = (bai_odex_system) S0;
    ...
    void *S0)
{
  ...
  S = (bai_odex_system) ba0_alloc (sizeof (struct bai_odex_system));
  bai_init_odex_system (S);
  bai_set_odex_system (S, (bai_odex_system) S0);
  return S;
}
    ...
    void *z)
{
  ...
  if (bav_symbol_type_variable (v) == bav_independent_symbol)
    ...
    ba0_printf ("x[%d]", k);
  else if (ba0_member2_table (v, (ba0_table) & the_system->params, &k))
    ba0_printf ("p[%d]", k);
  else if (ba0_member2_table (v, (ba0_table) & the_system->commands, &k))
    ...
    bai_odex_system S)
{
  ...
  bav_get_settings_variable (0, &printf_variable, 0, 0);
  bav_get_settings_rank (&printf_rank);

  BA0_TRY
  {
    bav_set_settings_variable (0, &bai_printf_variable, 0, 0);
    ...
    bav_set_settings_variable (0, printf_variable, 0, 0);
