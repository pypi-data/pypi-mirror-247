...
{
  bad_initialized_global.regularize.strategy = strategy ? strategy :
...
bad_context (
    bad_tableof_quadruple tabG,
    bad_regchain C)
{
  if (C != (bad_regchain) 0)
    if (tabG != (bad_tableof_quadruple) 0)
...
bad_Euclid_mod_regchain (
    bap_tableof_tableof_polynom_mpz tabV,
    bad_tableof_quadruple tabG,
  ...
  switch (bad_initialized_global.regularize.strategy)
    ...
    case bad_subresultant_regularize_strategy:
      bad_Euclid_subresultant_mod_regchain
          (tabV, tabG, type, A, B, v, reginitA, reginitB, C, K, ddz);
      break;
...
*/
      ...
      ba0_pull_stack ();
      ba0_realloc2_table ((ba0_table) tabG, tabG->size + 1,
          (ba0_new_function *) & bad_new_quadruple);
      ba0_realloc2_table ((ba0_table) tabV, tabV->size + 1,
...
*/
      if (!bad_simplify_and_store_in_P_quadruple (H, &init, K))
        {
          tabG->size--;
          tabV->size--;
        }
      else
        {
          if (!bap_depend_polynom_mpz (&reductum, v))
            {
              if (!bad_simplify_and_store_in_P_quadruple (H, &reductum, K))
                {
                  tabG->size--;
                  tabV->size--;
                }
                ...
                {
                  ba0_realloc2_table ((ba0_table) W, 1,
            ...
            bad_Euclid_mod_regchain
                (tabV, tabG, type, A, &reductum, v, reginitA, false, C, K, ddz);
/*
...
*/
          BA0_SWAP (bad_quadruple,
              tabG->tab[tabG->size - 1], tabG->tab[index_G]);
          BA0_SWAP (bap_tableof_polynom_mpz,
              tabV->tab[tabV->size - 1], tabV->tab[index_V]);
        }
      consistent = bad_simplify_and_store_in_S_quadruple (G, &init, K);
      break;
      ...
      for (;;)
        {
          ...
          if (bap_depend_polynom_mpz (B, v) || context == bad_inverse_context)
            {
              if (!bad_is_a_reduced_to_zero_polynom_by_regchain
                  (&init, C, type_red))
                break;
            }
          if (context == bad_pardi_context &&
              ...
              (&init, &G->A, type_red))
            bad_simplify_and_store_in_P_quadruple (G, &init, K);
          ba0_push_another_stack ();
...
bad_basic_Euclid_subresultant_mod_regchain (
    ...
    bap_polynom_mpz *ddz)
{
  bad_quadruple G = (bad_quadruple) 0;  /* to avoid a warning */
  enum bad_typeof_context context;
  bap_tableof_polynom_mpz V;
  bap_polynom_mpz P, Q, Z;
  ...
  if (!reginitB)
    {
      ...
      switch (context)
        ...
        case bad_pardi_context:
          bad_ensure_regular_lcoeff
              (tabV, tabG, type, A, B, v, reginitA, C, K, ddz);
          break;
        case bad_rg_context:
          consistent = bad_ensure_regular_lcoeff
              (tabV, tabG, type, A, B, v, reginitA, C, K, ddz);
          break;
        }
      ba0_push_another_stack ();
...
*/
      if (bap_degree_polynom_mpz (Q, v) == 0)
      ...
      else
        {
          ...
          BA0_SWAP (bap_polynom_mpz, Z, Q);
          BA0_SWAP (bap_polynom_mpz, Z, P);
        }
      rankfall = false;
      while (consistent && !bap_is_zero_polynom_mpz (Q) && !rankfall)
        {
          ...
          switch (context)
            ...
            case bad_pardi_context:
              bad_ensure_regular_lcoeff
                  (tabV, tabG, type, P, Q, v, true, C, K, ddz);
              break;
            case bad_rg_context:
              if (bap_depend_polynom_mpz (Q, v))
                ...
                {
                  consistent = bad_simplify_and_store_in_P_quadruple (G, Q, K);
                  bap_set_polynom_zero_mpz (Q);
                }
              break;
            }
  ...
  if (!consistent)
    ...
    {
      ba0_realloc2_table
          ((ba0_table) V, 1, (ba0_new_function *) & bap_new_polynom_mpz);
    ...
    bap_polynom_mpz *ddz)
{
  bad_quadruple G;
  enum bad_typeof_context context;
  bap_tableof_polynom_mpz V, VZ, VP, VQ;
  ...
  VP = (bap_tableof_polynom_mpz) 0;
  VQ = (bap_tableof_polynom_mpz) 0;
  VZ = (bap_tableof_polynom_mpz) 0;
  ...
  if (!reginitQ)
    {
      ...
      switch (context)
        ...
        case bad_pardi_context:
          bad_ensure_regular_lcoeff
              (tabV, tabG, type, P, Q, v, reginitP, C, K, ddz);
          break;
        case bad_rg_context:
          consistent = bad_ensure_regular_lcoeff
              (tabV, tabG, type, P, Q, v, reginitP, C, K, ddz);
  ...
  if (consistent)
    {
      ...
      bap_init_readonly_polynom_mpz (&coeff);
      bap_init_polynom_mpz (&s);
      VP = (bap_tableof_polynom_mpz) ba0_new_table ();
      VQ = (bap_tableof_polynom_mpz) ba0_new_table ();
      VZ = (bap_tableof_polynom_mpz) ba0_new_table ();
      ba0_realloc2_table
          ((ba0_table) VP, n, (ba0_new_function *) & bap_new_polynom_mpz);
      ba0_realloc2_table ((ba0_table) VQ, n,
          (ba0_new_function *) & bap_new_polynom_mpz);
      ba0_realloc2_table ((ba0_table) VZ, n,
          (ba0_new_function *) & bap_new_polynom_mpz);
      VP->size = n;
      VQ->size = n;
      VZ->size = n;
      ...
      if (n == 1)
        bap_pseudo_division_polynom_mpz
            (BAP_NOT_A_POLYNOM_mpz, VQ->tab[0], (bav_Idegree *) 0, P, Q, v);
      else if (n == 2)
        {
          if (perm)
            {
            ...
            {
              bap_pseudo_division_polynom_mpz
                  (BAP_NOT_A_POLYNOM_mpz, VQ->tab[0], &d, P, Q, v);
              bap_initial2_polynom_mpz (&coeff, Q, v);
  ...
  while (consistent && !bap_is_zero_polynom_mpz (VQ->tab[0]))
    {
      ...
      switch (context)
        ...
        case bad_pardi_context:
          bad_ensure_regular_lcoeff
              (tabV, tabG, type, VP->tab[0], VQ->tab[0], v, true, C, K, ddz);
          break;
        case bad_rg_context:
          consistent = bad_ensure_regular_lcoeff
              (tabV, tabG, type, VP->tab[0], VQ->tab[0], v, true, C, K, ddz);
      ...
      if (rankfall)
        {
          ...
          if (n == 1)
            bap_pseudo_division_polynom_mpz
                (BAP_NOT_A_POLYNOM_mpz, VQ->tab[0], (bav_Idegree *) 0,
                &bunk, VQ->tab[0], v);
          else if (n == 2)
            {
              bap_pseudo_division_polynom_mpz
                  (BAP_NOT_A_POLYNOM_mpz, VQ->tab[0], &d, &bunk, VQ->tab[0], v);
              bap_initial2_polynom_mpz (&coeff, VQ->tab[0], v);
              bap_pow_polynom_mpz (VQ->tab[1], &coeff, d);
            }
          else
            {
              bap_pseudo_division_polynom_mpz
                  (VQ->tab[2], VQ->tab[0], &d, &bunk, VQ->tab[0], v);
              bap_neg_polynom_mpz (VQ->tab[2], VQ->tab[2]);
              bap_initial2_polynom_mpz (&coeff, VQ->tab[0], v);
              bap_pow_polynom_mpz (VQ->tab[1], &coeff, d);
            }
      ...
      else
        {
          ...
          BA0_SWAP (bap_tableof_polynom_mpz, VP, VQ);
  ...
  if (!consistent)
    ...
    {
      ba0_realloc2_table
          ((ba0_table) V, n, (ba0_new_function *) bap_new_polynom_mpz);
      for (i = 0; i < n; i++)
...
bad_Euclid_gcd_prem_and_factor_mod_regchain (
    ...
    bap_polynom_mpz *ddz)
{
  tabV = (bap_tableof_tableof_polynom_mpz) 0;
  G = (bad_tableof_quadruple) 0;
 ...
 */
      type_red = bad_defines_a_differential_ideal_regchain (C) ?
          bad_full_reduction : bad_algebraic_reduction;
 ...
 */
      ...
      bad_reduce_polynom_by_regchain (&P, (bap_product_mpz) 0, A, C,
 ...
 */
      for (i = 0; i < P.size; i++)
        bad_check_algebraic_regularity_mod_regchain
            (&P.tab[i].factor, C, (bap_listof_polynom_mpz) 0, K, ddz);
      ...
      if (bad_is_leader_of_regchain (v, C, &k))
        {
          ...
          if (dA < dk)
            ...
            bad_Euclid_check_algebraic_regularity_mod_regchain
                (A, C->decision_system.tab[k], v, C, S, K, ddz);
        }
    }
}
    ...
    bap_polynom_mpz *ddz)
{
  ...
  baz_gcd_prem_polynom_mpz (&R, (bap_product_mpz) 0, A, B, v);
  bad_reduce_easy_polynom_by_regchain (&R, &R, C, bad_algebraic_reduction);
  baz_factor_easy_polynom_mpz (&prod1, &R, S);

  if (bap_is_zero_product_mpz (&prod1))
    {
      if (ddz != (bap_polynom_mpz *) 0)
        {
          *ddz = B;
...
#undef BAD_DO_REDUCE
      ...
      bad_reduce_polynom_by_regchain
          (&prod2, (bap_product_mpz) 0, &prod1.tab[i].factor, C,
      ...
      if (bad_is_a_reduced_to_zero_polynom_by_regchain
      ...
      else
        {
          bad_ensure_nonzero_initial_mod_regchain
              (&prod1.tab[i].factor, &prod1.tab[i].factor,
              C, bad_algebraic_reduction);
      ...
      for (j = 0; j < prod2.size; j++)
        {
          if (!bap_depend_polynom_mpz (&prod2.tab[j].factor, v))
            bad_check_algebraic_regularity_mod_regchain
                (&prod2.tab[j].factor, C, S, K, ddz);
          else
            {
              bap_initial_polynom_mpz (&init, &prod2.tab[j].factor);
              bad_check_algebraic_regularity_mod_regchain (&init, C, S, K, ddz);
              bad_Euclid_check_algebraic_regularity_mod_regchain
                  (B, &prod2.tab[j].factor, v, C, S, K, ddz);
            }
        }
    }
  ba0_restore (&M);
 ...
 * of @math{S} are partially reduced w.r.t. @math{C}. This function stores 
    ...
    struct bad_base_field *K)
{
  ...
  volatile struct bap_polynom_mpz *g;
 ...
 */
  ...
  bad_set_intersectof_regchain_regchain
      (&todoC, tabC->inter.tab[tabC->inter.size]);

  ba0_init_table ((ba0_table) & todoS);
  ba0_realloc_table ((ba0_table) & todoS, 1);
 ...
 */
          ...
          ba0_pull_stack ();
          ba0_realloc2_table ((ba0_table) & tabC->inter,
 ...
 */
                vg = bap_leader_polynom_mpz ((bap_polynom_mpz) g);
 ...
 */
                ...
                if (ideal != (bad_regchain) 0)
                  consider_h = bad_is_a_reduced_to_zero_polynom_by_regchain
                      (h, ideal, type_red);
 ...
 */
                if (consider_h && !consider_g)
                  {
                    struct bap_polynom_mpz *tmp = (bap_polynom_mpz) g;
                    g = h;
                    h = tmp;
                    BA0_SWAP (bool,
                        consider_g,
 ...
 */
                if (consider_h)
                  D = (bad_regchain) bad_copy_regchain (C);
 ...
 */
                    ...
                    ba0_realloc_table ((ba0_table) & todoS, todoS.size + n);
                    for (int k = 0; k < n; k++)
 ...
 */
                    ba0_realloc_table
                        ((ba0_table) & todoC.inter, todoC.inter.size + 1);
 ...
 */
  ...
  while (bad_is_a_reducible_product_by_regchain
      (&Pbar, C, type_red, bad_all_derivatives_to_reduce, (ba0_int_p *) 0))
    {
      bad_reduce_product_by_regchain
          (&R, &H, &Pbar, C, type_red, bad_all_derivatives_to_reduce);
  ...
  baz_gcd_product_mpz ((bap_product_mpz) 0, &Pbar, &Qbar, &Pbar, &Qbar);
  ba0_pull_stack ();
...
bad_normal_form_ratfrac_mod_intersectof_regchain (
    baz_tableof_ratfrac tabNF,
    baz_ratfrac A,
    bad_intersectof_regchain tabC,
    bap_polynom_mpz *ddz)
{
  ba0_int_p i;

  ba0_realloc2_table ((ba0_table) tabNF,
      tabNF->size + tabC->inter.size, (ba0_new_function *) & baz_new_ratfrac);
  for (i = 0; i < tabC->inter.size; i++)
    {
      bad_normal_form_ratfrac_mod_regchain
          (tabNF->tab[tabNF->size], A, tabC->inter.tab[i], ddz);
      tabNF->size += 1;
    }
}
 ...
 * The chains which are stored in @var{tabC} are those w.r.t. which normal
 * forms can eventually be computed. The chains which are stored in @var{tabNUL}
 * are those w.r.t. which the denominator of @var{A} is zero. 
 * 
...
bad_normal_form_handling_exceptions_ratfrac_mod_regchain (
    baz_tableof_ratfrac tabNF,
    bad_intersectof_regchain tabC,
    bad_intersectof_regchain tabNUL,
    baz_ratfrac A)
{
  struct bad_intersectof_regchain S;
  volatile bad_regchain C;      /* volatile needed here */
  struct baz_ratfrac NF;
  struct bad_base_field K;
  volatile bap_polynom_mpz ddz = BAP_NOT_A_POLYNOM_mpz; /* to avoid a warning */
  struct ba0_mark M;

  ba0_realloc2_table
      ((ba0_table) tabNF, tabNF->size + 1,
 ...
 */
  ...
  {
    bad_normal_form_ratfrac_mod_regchain
        (tabNF->tab[tabNF->size], A, C, (bap_polynom_mpz *) & ddz);
    tabNF->size += 1;
  ...
  BA0_CATCH
  {
    if (ba0_global.exception.raised == BAD_EXRNUL)
      {
        if (tabNUL != (bad_intersectof_regchain) 0)
          bad_append_intersectof_regchain_regchain (tabNUL, C);
        tabC->inter.size -= 1;
        return;
      }
    else if (ba0_global.exception.raised != BAD_EXRDDZ)
 ...
 */
  ...
  C = tabC->inter.tab[tabC->inter.size - 1];
  tabC->inter.size -= 1;
  bad_set_intersectof_regchain_regchain (&S, C);
  baz_init_ratfrac (&NF);

  bad_handle_splitting_exceptions_regchain
      (&S, (struct bad_quench_map *) 0, ddz, (bad_regchain) 0, BAD_EXRDDZ, &K);
  ...
  while (S.inter.size > 0)
    {
      BA0_TRY
      {
        ...
        bad_normal_form_ratfrac_mod_regchain
            (&NF, A, C, (bap_polynom_mpz *) & ddz);

        BA0_CANCEL_EXCEPTION;

        ba0_pull_stack ();
        bad_append_intersectof_regchain_regchain (tabC, C);
        ba0_realloc2_table
            ((ba0_table) tabNF, tabNF->size + S.inter.size,
            (ba0_new_function *) & baz_new_ratfrac);
      ...
      BA0_CATCH
      {
        if (ba0_global.exception.raised == BAD_EXRNUL)
          ...
          {
            bad_handle_splitting_exceptions_regchain
                (&S, (struct bad_quench_map *) 0, ddz, (bad_regchain) 0,
                BAD_EXRDDZ, &K);
          }
        else
          BA0_RE_RAISE_EXCEPTION;
