    ...
    bap_indexed_access ind)
{
  ind->alloc = 0;
  ind->size = 0;
  ba0_init_table ((ba0_table) & ind->tab);
}
  ...
  while (n > 0)
    {
      ...
      ba0_realloc2_table ((ba0_table) & ind->tab,
          ind->tab.size + 1, (ba0_new_function *) & ba0_new_table);
...
{
  bap_indexed_access A = (bap_indexed_access) AA;
  ba0_int_p i, n = 0;
  ...
  if (A->tab.alloc > 0)
    n += ba0_new_gc_info
        (A->tab.tab, sizeof (void *) * A->tab.alloc, _indexed_tab);

  for (i = 0; i < A->tab.alloc; i++)
    {
      ba0_tableof_int_p tab = A->tab.tab[i];
      n += ba0_new_gc_info
          (tab, sizeof (struct ba0_tableof_int_p), _indexed_tab_i);
      n += ba0_new_gc_info
          (tab->tab, sizeof (ba0_int_p) * tab->alloc, _indexed_tab_i_tab);
    }
    ...
    enum ba0_garbage_code code)
{
  ...
  if (A->tab.alloc > 0)
    A->tab.tab = (ba0_tableof_int_p *) ba0_new_addr_gc_info
        (A->tab.tab, _indexed_tab);
  for (i = 0; i < A->tab.alloc; i++)
    {
      ba0_tableof_int_p tab;
      A->tab.tab[i] = (ba0_tableof_int_p) ba0_new_addr_gc_info
          (A->tab.tab[i], _indexed_tab_i);
      tab = A->tab.tab[i];
      tab->tab = (ba0_int_p *) ba0_new_addr_gc_info
          (tab->tab, _indexed_tab_i_tab);
    }
