...
/*
   Allocate a bap_table2of_monom_mint_hp with a capacity of at least n monomials.
   Initialize the coefficients.
*/

static bap_table2of_monom_mint_hp
new_table2of_monom_mint_hp (
    bap_termanager tgest,
    ba0_int_p n)
{
  bap_table2of_monom_mint_hp mont;
  ba0_int_p i;

  mont =
      (bap_table2of_monom_mint_hp) ba0_alloc (sizeof (struct
          bap_table2of_monom_mint_hp));
    ...
    bap_clot_mint_hp C)
{
  ...
  if (C == (bap_clot_mint_hp) 0)
    return;

  if (C->tab.size > C->tab.alloc || C->size > C->alloc)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);

  if (C->size > 0 && C->tab.tab == (bap_table2of_monom_mint_hp *) 0)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
  ...
  for (i = 0; i < C->tab.size; i++)
    {
      if (C->tab.tab[i] == (bap_table2of_monom_mint_hp) 0)
        BA0_RAISE_EXCEPTION (BA0_ERRALG);
  ...
  while (i < C->tab.alloc && C->tab.tab[i] != (bap_table2of_monom_mint_hp) 0)
    cumul_alloc += C->tab.tab[i++]->alloc;

  if (cumul_alloc != C->alloc)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
#else
  C = (bap_clot_mint_hp) 0;
#endif
}
...
/*
  ...
  The sub-arrays of bap_table2of_monom_mint_hp are shared.
*/
...
BAP_DLL bap_clot_mint_hp
bap_new_clot_mint_hp (
    bav_term T)
{
  bap_clot_mint_hp C;

  C = (bap_clot_mint_hp) ba0_alloc (sizeof (struct bap_clot_mint_hp));
...
bap_swap_itermon_clot_mint_hp (
    bap_itermon_clot_mint_hp I,
    bap_itermon_clot_mint_hp J)
{
  bap_table2of_monom_mint_hp mi, mj;
...
bap_goto_itermon_clot_mint_hp (
    ...
    ba0_int_p numero)
{
  bap_table2of_monom_mint_hp *tab;
  ba0_int_p primary, secondary;

  if (numero < 0 || numero >= iter->clot->size)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);

  tab = (bap_table2of_monom_mint_hp *) iter->clot->tab.tab;
  primary = 0;
  secondary = numero;
 ...
 * If some new arrays of type @code{bap_table2of_monom_mint_hp} are
...
/*
  Returns the bap_table2of_monom_mint_hp crea points to.
  Allocates a bap_table2of_monom_mint_hp if there is no place left.
  ...
  if (clot->alloc == crea->iter.num.combined)
    {
      if (clot->tab.alloc == crea->iter.num.primary)
        {
          bap_table2of_monom_mint_hp *new_tab;
          ba0_int_p n, new_alloc;

          new_alloc = 2 * clot->tab.alloc + 1;
          new_tab =
              (bap_table2of_monom_mint_hp *) ba0_alloc (sizeof (void *)
              * new_alloc);
          memcpy (new_tab, clot->tab.tab,
              clot->tab.alloc * sizeof (bap_table2of_monom_mint_hp));
          for (n = clot->tab.alloc; n < new_alloc; n++)
            new_tab[n] = (bap_table2of_monom_mint_hp) 0;
          clot->tab.alloc = new_alloc;
          clot->tab.tab = (bap_table2of_monom_mint_hp *) new_tab;
        }
      mont =
    ...
    ba0_int_p r)
{
  ...
  bap_table2of_monom_mint_hp d_mont, s_mont;
    ...
    bap_creator_clot_mint_hp crea)
{
  ...
  for (i = clot->tab.size; i < clot->tab.alloc; i++)
    {
      m = clot->tab.tab[i];
      if (m != (bap_table2of_monom_mint_hp) 0)
        m->size = 0;
    }
  check_clot_mint_hp (clot);
}
...
static char _struct_table2of_monom_zipterm[] =
    "bap_table2of_monom_mint_hp->zipterm";
static char _struct_table2of_monom_coeff[] = "bap_table2of_monom_mint_hp->coeff";

static ba0_int_p
garbage1_table2of_monom_mint_hp (
    bap_termanager tgest,
    bap_table2of_monom_mint_hp A,
    enum ba0_garbage_code code)
{
  ba0_int_p n = 0;

  if (A == (bap_table2of_monom_mint_hp) 0)
    return 0;

  if (code == ba0_isolated)
    n += ba0_new_gc_info
        (A, sizeof (struct bap_table2of_monom_mint_hp), _struct_mont);
  n += ba0_new_gc_info
...
static char _struct_clot_tab[] = "bap_clot_mint_hp->tab.tab";
...
bap_garbage1_clot_mint_hp (
    ...
    enum ba0_garbage_code code)
{
  bap_clot_mint_hp A = (bap_clot_mint_hp) AA;
  ba0_int_p i, n = 0;
  ...
  if (A->tab.tab)
    n += ba0_new_gc_info
        (A->tab.tab, sizeof (bap_table2of_monom_mint_hp) * A->tab.alloc,
        _struct_clot_tab);
    ...
    enum ba0_garbage_code code)
{
  ...
  if (code == ba0_isolated)
    A = (bap_table2of_monom_mint_hp) ba0_new_addr_gc_info (A, _struct_mont);

  A->zipterm = (bap_zipterm *) ba0_new_addr_gc_info
      (A->zipterm, _struct_table2of_monom_zipterm);

  A->coeff = (ba0_mint_hp_t *) ba0_new_addr_gc_info
    ...
    enum ba0_garbage_code code)
{
  ...
  if (code == ba0_isolated)
    A = (bap_clot_mint_hp) ba0_new_addr_gc_info (AA, _struct_clot);
  else
    A = (bap_clot_mint_hp) AA;

  if (A->tab.tab)
    A->tab.tab = (bap_table2of_monom_mint_hp *) ba0_new_addr_gc_info
        (A->tab.tab, _struct_clot_tab);

  bap_garbage2_termanager (&A->tgest, ba0_embedded);

  for (i = 0; i < A->tab.alloc; i++)
    A->tab.tab[i] = garbage2_table2of_monom_mint_hp
