...
/*
 * texinfo: bad_pardi
 * Computation of a regular differential chain by change of ranking.
 * Assign to @var{Cbar} a regular chain w.r.t. the ordering @var{rbar}
    ...
    bad_regchain C)
{
  ...
  bad_quadruple G;
  bad_intersectof_regchain tabCbar;
 ...
 */
  tabG = (bad_tableof_quadruple) ba0_new_table ();
  ba0_realloc2_table
      ((ba0_table) tabG, 1, (ba0_new_function *) & bad_new_quadruple);
  tabG->size = 1;
  ...
  bad_pardi_set_quadruple_regchain (G, C, &K);
/*
   Working variables
*/
  ...
  ba0_init_table ((ba0_table) & tabV);
  ba0_realloc_table ((ba0_table) & tabV, 1);
  tabV.tab[0] = &V;
  tabV.size = 1;
  ba0_init_table ((ba0_table) & V);
...
*/
  ...
  bad_set_strategy_selection_strategy
      (strategy, bad_lower_leader_first_selection_strategy);

  while (!bad_is_a_listof_rejected_critical_pair (G->D) ||
      G->P != (bap_listof_polynom_mpz) 0)
    {
 ...
 */
      ...
      if (!bad_is_leader_of_regchain (v, &G->A, &k))
        bad_complete_quadruple (tabG, &p, C, &K, strategy, tree);
      else
        {
          bad_Euclid_mod_regchain (&tabV, tabG, bad_basic_Euclid,
              G->A.decision_system.tab[k], &p, v,
              true, true, C, &K, (bap_polynom_mpz *) 0);
/*
 ...
 */
          if (bap_leading_degree_polynom_mpz (V.tab[0]) !=
            ...
            bad_complete_quadruple (tabG, V.tab[0], C, &K, strategy, tree);
        }
    }
/*
...
*/
  ...
  ba0_realloc_table ((ba0_table) & tabCbar->inter, 1);
      ...
      if (!bad_member_polynom_base_field (&init, K))
        {
          ...
          bad_simplify_and_store_in_S_quadruple (G, &pbar, K);
          bav_R_push_ordering (C->attrib.ordering);
        }
    ...
    bad_base_field K)
{
  ...
  while (rankfall)
    {
      ...
      for (i = 0; i < prod_init.size; i++)
        if (bad_is_a_reduced_to_zero_polynom_by_regchain
            (&prod_init.tab[i].factor, C, type_red))
          {
            reduced_to_zero = true;
            if (!bad_is_a_reduced_to_zero_polynom_by_regchain
                (&prod_init.tab[i].factor, &G->A, type_red))
              {
                ba0_pull_stack ();
                bad_simplify_and_store_in_P_quadruple
                    (G, &prod_init.tab[i].factor, K);
                ba0_push_another_stack ();
              }
          }
      if (reduced_to_zero)
          ...
          bap_leading_degree_polynom_mpz (p) > 1)
        {
          ...
          for (i = 0; i < prod_sep.size; i++)
            if (bad_is_a_reduced_to_zero_polynom_by_regchain
                (&prod_sep.tab[i].factor, C, type_red))
              {
                reduced_to_zero = true;
                if (!bad_is_a_reduced_to_zero_polynom_by_regchain
                    (&prod_sep.tab[i].factor, &G->A, type_red))
                  {
                    ba0_pull_stack ();
                    bad_simplify_and_store_in_P_quadruple
                        (G, &prod_sep.tab[i].factor, K);
                    ba0_push_another_stack ();
                  }
              }
          if (reduced_to_zero)
            {
              ...
              bap_prem_polynom_mpz
                  (p, (bav_Idegree *) 0, p, &sep, BAV_NOT_A_VARIABLE);
              ba0_push_another_stack ();
              rankfall = true;
            }
        }
  ...
  if (!bap_is_zero_polynom_mpz (p))
    {
      for (i = 0; i < prod_init.size; i++)
        bad_simplify_and_store_in_S_quadruple (G, &prod_init.tab[i].factor, K);
      if ((bad_defines_a_differential_ideal_regchain (&G->A) ||
              bad_has_property_regchain
              (&G->A, bad_squarefree_property)) &&
          bap_leading_degree_polynom_mpz (p) > 1)
