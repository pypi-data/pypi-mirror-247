    ...
    struct bmi_callback *callback)
{
  ...
  ba0_sscanf2 (ratfracs, "%t[%careful_expanded_Qz]", &T);
#else
  ba0_sscanf2 (ratfracs, "%t[%Qz]", &T);
#endif

/*
 * Do not apply the iterated method over independent rational fractions.
 * That would lead the algorithm in an infinite loop.
    if (iterated)
    {   for (i = 0; i < T.size; i++)
        {   if (baz_is_independent_ratfrac (T.tab [i], &bav_global.parameters))
                BA0_RAISE_EXCEPTION (BMI_ERRRBL);
        }
    }
 */
 ...
 */
  for (i = 0; i < T.size; i++)
    {
      P = (baz_tableof_ratfrac) ba0_new_table ();
      ba0_realloc2_table ((ba0_table) P, 2,
          (ba0_new_function *) & baz_new_ratfrac);
      if (!iterated)
        {
          baz_rat_bilge_mpz
              (P->tab[1], P->tab[0], T.tab[i], x, &bav_global.parameters);
          P->size = 2;
/*
 ...
 */
        ...
        {
          BA0_SWAP (baz_ratfrac, P->tab[P->size], T.tab[i]);
          P->size = 1;
          while (!baz_is_independent_ratfrac
              (P->tab[P->size - 1], &bav_global.parameters))
            {
              if (P->size >= P->alloc)
                ba0_realloc2_table ((ba0_table) P, 2 * P->size + 1,
                    (ba0_new_function *) & baz_new_ratfrac);
              baz_rat_bilge_mpz
                  (P->tab[P->size], P->tab[P->size - 1],
                  P->tab[P->size - 1], x, &bav_global.parameters);
              P->size += 1;
            }
 ...
 *      {   ALGEB res2;
 *          ba0_int_p j;
 *          P = (baz_tableof_ratfrac)result.tab [i];
 *          bmi_push_maple_gmp_allocators (); 
