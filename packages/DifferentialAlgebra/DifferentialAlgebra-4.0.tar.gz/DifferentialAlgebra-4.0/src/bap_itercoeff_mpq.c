    ...
    bav_term T)
{
  ...
  if ((R == A && B != BAP_NOT_A_POLYNOM_mpq) ||
      (R == B && A != BAP_NOT_A_POLYNOM_mpq))
  ...
  if (code == ba0_gt)
    ...
    {
      bap_seeklast_itermon_mpq (&iter.fin,
          &itercoeff_samerank_mpq, last_term, last_number);
...
bap_close_itercoeff_mpq (
    bap_itercoeff_mpq iter)
{
  iter = (bap_itercoeff_mpq) 0;
}
