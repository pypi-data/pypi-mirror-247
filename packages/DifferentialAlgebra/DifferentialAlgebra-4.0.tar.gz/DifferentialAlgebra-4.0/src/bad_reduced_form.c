    ...
    bap_polynom_mpz *ddz)
{
  ...
  if ((v == BAV_NOT_A_VARIABLE
          && bap_is_numeric_polynom_mpz (&RNF.denom))
 ...
 * Assign to @var{R} a reduced form of @var{F} w.r.t. @var{C}.
...
{
  bad_reduced_form_polynom_mod_regchain2
      (R, F, v, C, false, (bap_polynom_mpz *) 0);
    ...
    bad_intersectof_regchain tabC)
{
  ...
  ba0_reset_table ((ba0_table) tabR);
  ba0_realloc2_table ((ba0_table) tabR, tabC->inter.size,
      (ba0_new_function *) & baz_new_ratfrac);
  for (i = 0; i < tabC->inter.size; i++)
    {
      bad_reduced_form_polynom_mod_regchain2
          (tabR->tab[i], F, v, tabC->inter.tab[i],
          false, (bap_polynom_mpz *) 0);
