    ...
    bav_variable v)
{
  ...
  bap_begin_creator_mpq
      (&crea, &N, &term, bap_exact_total_rank,
  ...
  while (!bap_outof_itermon_mpz (&iter))
    {
      ...
      ba0_mpz_mul_ui (ba0_mpq_denref (q), ba0_mpq_denref (q), (unsigned long) d + 1);
    ...
    bav_tableof_parameter params)
{
  ...
  bav_zero_derivatives_of_tableof_parameter (&nulles, params);
 ...
 */
#if defined (RATBILGE_CHECK_INVARIANT)
      baz_diff_ratfrac (&W2, &R, x, &nulles);
      ...
      if (!baz_is_zero_ratfrac (&W2))
        BA0_RAISE_EXCEPTION (BA0_ERRALG);
#endif
      if (baz_is_independent_ratfrac (&F, params))
        {
/*
 ...
 */
          baz_integrate_with_remainder_mpz
              (&R2, &W2, &F, bav_R_symbol_to_variable (x));
      ...
      else if (bap_is_independent_polynom_mpz (&F.numer, params))
        {
/*
 ...
 */
          ...
          denom_independent = bap_is_independent_polynom_mpz (&F.denom, params);
          if (!denom_independent)
            vB = bap_leader_polynom_mpz (&F.denom);
          if (!denom_independent &&
              bav_R_variable_number (vN) <= bav_R_variable_number (vB))
            {
/*
 ...
 */
                          baz_add_ratfrac (&R, &R, &R2);
                          baz_reduce_ratfrac (&R, &R);
                          baz_diff_ratfrac (&R2, &R2, x, &nulles);
