    ...
    bool rw)
{
  ...
  if (bmi_is_regchain_op (1, callback))
    {
      fullset = bmi_bool_op (nops - 1, callback);
      if (!fullset)
        bad_remove_zero_derivatives_of_tableof_parameter_from_regchain
            (&C, &C, &bav_global.parameters);
      integer = true;
      ...
      for (i = T->size - 1; i >= 0; i--)
        {
          ...
          u = b ? BAV_NOT_A_VARIABLE : integer ?
      ...
      for (i = T->size - 1; i >= 0; i--)
        {
          ...
          b = integer ?
              ...
              (bav_tableof_parameter) 0) :
              bap_is_independent_polynom_mpq ((bap_polynom_mpq)
              T->tab[i], (bav_tableof_parameter) 0);
          p = b ? 0 : integer ?
              bap_total_order_polynom_mpz ((bap_polynom_mpz)
              T->tab[i]) :
  ...
  if (T->size == 0)
  ...
  else if (rw)
    {
      ...
      ba0_set_output_counter ();
      if (integer)
        bmi_printf_rewrite_rules_mpz ((bap_tableof_polynom_mpz) T, '[', ']');
      else
        bmi_printf_rewrite_rules_mpq ((bap_tableof_polynom_mpq) T, '[', ']');
      stres = ba0_persistent_malloc (ba0_output_counter () + 1);
      ba0_set_output_string (stres);
      if (integer)
        bmi_printf_rewrite_rules_mpz ((bap_tableof_polynom_mpz) T, '[', ']');
