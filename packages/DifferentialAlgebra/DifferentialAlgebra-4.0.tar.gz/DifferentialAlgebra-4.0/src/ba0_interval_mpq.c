...
BA0_DLL ba0_interval_mpq
ba0_new_interval_mpq (
    void)
{
  ba0_interval_mpq R;

  R = (ba0_interval_mpq) ba0_alloc (sizeof (struct ba0_interval_mpq));
 ...
 * Return @code{true} if all the elements of @var{I} are less than all
 * the elements of @var{J}. 
 */
 ...
 * Return @code{true} if the intersection of @var{I} and @var{J} is empty.
 */
 ...
 */
    ...
    {
      if (X->type == ba0_right_infinite_interval)
        return false;
      else
/*
 * X =             [Xa,          inf]
 ...
 * Assign the intersection of @var{I} and @var{J} to @var{X}.
 */
 ...
 * Return @code{true} if @var{I} is a subset of @var{J}.
 */
 ...
 * Assign @math{I \times J} to @math{X}.
 */
 ...
 * Assign @math{I \times q} to @math{X}.
 */
 ...
 * Assign @math{I \times n} to @math{X}.
 */
    ...
    ba0_interval_mpq Y)
{
  ...
  ba0_reset_table ((ba0_table) F);
  ba0_realloc2_table ((ba0_table) F, 2,
...
ba0_printf_interval_mpq (
    void *z)
{
  ba0_interval_mpq R = (ba0_interval_mpq) z;
    ...
    enum ba0_garbage_code code)
{
  ...
  if (code == ba0_isolated)
    R = (ba0_interval_mpq) ba0_new_addr_gc_info (z, _interval_mpq);
  else
    R = (ba0_interval_mpq) z;
    ...
    void *z)
{
  ...
  R = ba0_new_interval_mpq ();
  ba0_set_interval_mpq (R, (ba0_interval_mpq) z);
  return R;
}
