 ...
 * This data type permits to define the 
 * attributes associated to a regular chain.
 */

struct bad_attchain
{
// the ordering w.r.t. which the chain is defined
...
typedef struct bad_attchain *bad_attchain;
#   endif
...
#   if defined (BA0_OLDDEF)
typedef struct bad_splitting_control *bad_splitting_control;
#   endif
...
#   if defined (BA0_OLDDEF)
typedef struct bad_regchain *bad_regchain;
#   endif
...
#   if defined (BA0_OLDDEF)
typedef struct bad_tableof_regchain *bad_tableof_regchain;
#   endif
...
struct bad_base_field
{
// Indicate if the base field is differential or not
  bool differential;
// Indicate if the base field equations form an autoreduced set.
// If so, some computations are faster. Default value is false.
  bool assume_reduced;
...
#   if defined (BA0_OLDDEF)
typedef struct bad_base_field *bad_base_field;
#   endif
...
extern BAD_DLL void bad_force_algebraic_base_field (
    struct bad_base_field *);
...
#ifndef BAD_INTERSECTOF_REGCHAIN_H
...
#   if defined (BA0_OLDDEF)
typedef struct bad_intersectof_regchain *bad_intersectof_regchain;
#   endif
...
// multivariate gcd are computed to reduce the size of the power 
// product of initials and separants involved in the reduction process
  bad_gcd_prem_reduction_strategy,
// the same strategy as above is applied but the polynomial to
// reduce is factored and reduction is performed factor per factor
...
extern BAD_DLL bool bad_is_a_reduced_to_zero_product_by_regchain (
    struct bap_product_mpz *,
    struct bad_regchain *,
    enum bad_typeof_reduction);
...
extern BAD_DLL bool bad_is_a_partially_reduced_product_wrt_regchain (
    struct bap_product_mpz *,
    struct bad_regchain *);

extern BAD_DLL void bad_reduced_to_zero_derivatives_by_regchain (
    struct bav_tableof_variable *,
    struct bad_regchain *,
    enum bad_typeof_reduction);
...
{
// plain polynomials are preferred to polynomials arising from 
// critical pairs
...
#   if defined (BA0_OLDDEF)
typedef struct bad_selection_strategy *bad_selection_strategy;
#   endif
...
#   if defined (BA0_OLDDEF)
typedef struct bad_critical_pair *bad_critical_pair;
#   endif
...
#   if defined (BA0_OLDDEF)
typedef struct bad_listof_critical_pair *bad_listof_critical_pair;
#   endif
...
#   if defined (BA0_OLDDEF)
typedef struct bad_splitting_edge *bad_splitting_edge;
#   endif
...
#   if defined (BA0_OLDDEF)
typedef struct bad_tableof_splitting_edge *bad_tableof_splitting_edge;
#   endif
...
#   if defined (BA0_OLDDEF)
typedef struct bad_splitting_tree *bad_splitting_tree;
#   endif
...
#   if defined (BA0_OLDDEF)
typedef struct bad_quadruple *bad_quadruple;
#   endif
...
#   if defined (BA0_OLDDEF)
typedef struct bad_tableof_quadruple *bad_tableof_quadruple;
#   endif
...
#   if defined (BA0_OLDDEF)
typedef struct bad_listof_quadruple *bad_listof_quadruple;
#   endif
...
#ifndef BAD_LOW_POWER_THEOREM_H
...
#   if defined (BA0_OLDDEF)
typedef struct bad_preparation_term *bad_preparation_term;
#   endif
...
#   if defined (BA0_OLDDEF)
typedef struct bad_tableof_preparation_term *bad_tableof_preparation_term;
#   endif
...
#   if defined (BA0_OLDDEF)
typedef struct bad_preparation_equation *bad_preparation_equation;
#   endif
 ...
 * Used to pass some extra informations to a subfunction in
 ...
 */
  ...
  {
/* 
 * Statistical informations set by bad_Rosenfeld_Groebner and bad_pardi
 */
    time_t begin;
    time_t end;
