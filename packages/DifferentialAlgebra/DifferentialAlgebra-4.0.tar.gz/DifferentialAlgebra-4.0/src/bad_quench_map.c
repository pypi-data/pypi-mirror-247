...
{
  ba0_init_table ((ba0_table) prop);
  ba0_realloc_table ((ba0_table) prop, nb_max_elem);
    ...
    struct bad_regchain *A)
{
  ...
  if (bad_defines_a_differential_ideal_regchain (A))
    bad_init_satisfied_quench_property
        (&map->partially_autoreduced, nb_max_elem, k + 1);
  else
    bad_init_inactive_quench_property (&map->partially_autoreduced);
/*
 ...
 */
  if (bad_has_property_regchain (A, bad_primitive_property))
    bad_init_satisfied_except_quench_property
        (&map->primitive, nb_max_elem, nb_max_elem, k);
  else
    bad_init_inactive_quench_property (&map->primitive);
/*
    ...
    struct bad_quench_map *src)
{
  ...
  ba0_init_table ((ba0_table) & dst->partially_autoreduced);
  ba0_set_table ((ba0_table) & dst->partially_autoreduced,
      (ba0_table) & src->partially_autoreduced);
  ba0_init_table ((ba0_table) & dst->autoreduced);
  ba0_set_table ((ba0_table) & dst->autoreduced,
      (ba0_table) & src->autoreduced);
  ba0_init_table ((ba0_table) & dst->primitive);
  ba0_set_table ((ba0_table) & dst->primitive, (ba0_table) & src->primitive);
  ba0_init_table ((ba0_table) & dst->normalized);
  ba0_set_table ((ba0_table) & dst->normalized, (ba0_table) & src->normalized);
  ba0_init_table ((ba0_table) & dst->regular);
  ba0_set_table ((ba0_table) & dst->regular, (ba0_table) & src->regular);
  ba0_init_table ((ba0_table) & dst->squarefree);
  ba0_set_table ((ba0_table) & dst->squarefree, (ba0_table) & src->squarefree);
}
