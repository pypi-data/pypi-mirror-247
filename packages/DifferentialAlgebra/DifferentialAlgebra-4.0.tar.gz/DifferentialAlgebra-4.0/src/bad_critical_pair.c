...
BAD_DLL bad_critical_pair
bad_new_critical_pair (
    void)
{
  bad_critical_pair P;

  P = (bad_critical_pair) ba0_alloc (sizeof (struct bad_critical_pair));
...
*/
      if (bap_compare_polynom_mpz (P1, P2) == ba0_lt)
        {
          BA0_SWAP (bap_polynom_mpz, P1, P2);
          BA0_SWAP (bav_variable, u1, u2);
        }
/*
   P2 has lower rank than P1
*/
      if (u12 == u2)
        baz_gcd_prem_polynom_mpz (delta, (bap_product_mpz) 0, P1, P2, u2);
      else
        {
...
*/
          if (bap_depend_polynom_mpz (&PP2, u12))
            baz_gcd_prem_polynom_mpz
                (delta, (bap_product_mpz) 0, P1, &PP2, u12);
          else
 ...
 */
  a = bav_lcd_variable
      (bap_leader_polynom_mpz (&P->p), bap_leader_polynom_mpz (&P->q));
...
bad_printf_critical_pair (
    void *A)
{
  bad_critical_pair P = (bad_critical_pair) A;
    ...
    enum ba0_garbage_code code)
{
  ...
  if (code == ba0_isolated)
    P = (bad_critical_pair) ba0_new_addr_gc_info (A, _critical);
  else
    P = (bad_critical_pair) A;
    ...
    void *A)
{
  ...
  P = bad_new_critical_pair ();
  bad_set_critical_pair (P, (bad_critical_pair) A);
  return P;
}
...
bad_is_a_listof_rejected_critical_pair (
    bad_listof_critical_pair L)
{
  while (L != (bad_listof_critical_pair) 0)
    {
