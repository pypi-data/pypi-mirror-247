...
#define ba0_leaf_subformat 	ba0_global.format.leaf_subformat
#define ba0_htable		ba0_global.format.htable
#define ba0_nbelem_htable	ba0_global.format.nbelem_htable

/*
 * Initializes the table of the atomic formats and the H-table of the formats.
 * Sets the ba0_global.format.{scanf,printf}_value_var to zero.
 */
    ...
    void)
{
  ...
  ba0_init_table ((ba0_table) & ba0_leaf_subformat);
  ba0_realloc_table
      ((ba0_table) & ba0_leaf_subformat, BA0_SUBFORMAT_REALLOC_VALUE);

  ba0_init_table ((ba0_table) & ba0_htable);
  ba0_realloc_table ((ba0_table) & ba0_htable, BA0_SIZE_HTABLE_FORMAT);
  ba0_htable.size = ba0_htable.alloc;
  for (i = 0; i < ba0_htable.size; i++)
    ba0_htable.tab[i] = (struct ba0_pair *) 0;
  ba0_nbelem_htable = 0;
 ...
 * 
 * Creates a new atomic format, identified by the special character @code{%}
 ...
 * 
 * The previous function is a particular case of this one with @math{sizelt = -1}.
 */
    ...
    ba0_copy_function *c)
{
  ...
  for (i = 0; i < ba0_leaf_subformat.size; i++)
    if (strcmp (id, ba0_leaf_subformat.tab[i]->identificateur) == 0)
      BA0_RAISE_EXCEPTION (BA0_ERRALG);

  ba0_push_stack (&ba0_global.stack.format);

  if (ba0_leaf_subformat.size == ba0_leaf_subformat.alloc)
    ba0_realloc_table ((ba0_table) & ba0_leaf_subformat,
        ba0_leaf_subformat.alloc + BA0_SUBFORMAT_REALLOC_VALUE);

  pair = (struct ba0_pair *) ba0_alloc (sizeof (struct ba0_pair));
  pair->identificateur = id;
  pair->value = sf = (ba0_subformat) ba0_alloc (sizeof (struct ba0_subformat));
  ...
  ba0_leaf_subformat.tab[ba0_leaf_subformat.size++] = pair;
 ...
 * A backslash backslashs the meaning of the parenthesis.
 */
    ...
    ba0_int_p *index)
{
  ...
  f = (ba0_format) ba0_alloc (sizeof (struct ba0_format));
  f->text = (char *) ba0_alloc (textlen + 1);
  f->link = (ba0_subformat *) ba0_alloc (sizeof (ba0_subformat) * linknmb);
  f->linknmb = linknmb;
  itext = 0;
  ilink = 0;
      ...
      if (!backslash && is_open (chrin))
      ...
      else if (!backslash && chrin == '%')
        {
          ...
          if (predefined)
            {
              ba0_subformat l =
                  (ba0_subformat) ba0_alloc (sizeof (struct ba0_subformat));
              f->link[ilink++] = l;
          ...
          else
            {
              ...
              for (i = 0; i < ba0_leaf_subformat.size && !found; i++)
                {
                  pair = ba0_leaf_subformat.tab[i];
                  if (strcmp (pair->identificateur, code) == 0)
                    {
                      f->link[ilink++] = (ba0_subformat) pair->value;
                      found = true;
                    }
                }
              if (!found)
...
/*
 * Transforms the string to a format. 
 ...
 * Error BA0_ERROOM is raised if the H-table gets filled.
 */
