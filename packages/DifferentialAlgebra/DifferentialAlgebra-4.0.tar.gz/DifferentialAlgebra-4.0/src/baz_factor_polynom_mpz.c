...
Berlekamp_Hensel_lifting (
    bap_product_mpzm new_fact,
    bool *vrai_factor,
    bap_polynom_mpz new_P,
    bap_polynom_mpz P,
    bap_product_mint_hp fact,
    ba0_mint_hp p)
{
  ...
  W = (bap_polynom_mpzm) 0;
  A = (bap_polynom_mpzm) 0;
  Z = (bap_polynom_mpzm) 0;
  ...
  r = fact->size;
  F = (struct bap_polynom_mpzm *) ba0_alloc
      (sizeof (struct bap_polynom_mpzm) * r);
  for (i = 0; i < r; i++)
    {
...
*/
  ...
  for (i = 0; i < r; i++)
    bap_mods_polynom_mpzm ((bap_polynom_mpz) & F[i], &F[i]);
/*
for (i = 0; i < r; i++)
{   printf ("F [%d] = ", i);
    ba0_printf ("%Azm\n", &F [i]);
...
*/
  dF0 = bap_leading_degree_polynom_mpzm (&F[0]);
...
/*
   ...
   W [k] = F [k] * ... * F [r-1]
*/
      W = (struct bap_polynom_mpzm *) ba0_alloc
          (sizeof (struct bap_polynom_mpzm) * r);
      ...
      for (i = r - 2; i >= 1; i--)
        {
          ...
          bap_mods_polynom_mpzm ((bap_polynom_mpz) & W[i], &W[i]);
        }
/*
for (i = 1; i < r - 1; i++)
{   printf ("W [%d] = ", i);
...
*/
      A = (struct bap_polynom_mpzm *) ba0_alloc
          (sizeof (struct bap_polynom_mpzm) * r);
      for (i = 0; i < r - 1; i++)
        {
          ...
          bap_Euclidean_division_polynom_mpzm
              (BAP_NOT_A_POLYNOM_mpzm, &A[i], &A[i], &F[i]);
          bap_mods_polynom_mpzm ((bap_polynom_mpz) & A[i], &A[i]);
          bap_mul_polynom_mpzm (&D, &U, &D);
          bap_Euclidean_division_polynom_mpzm
              (BAP_NOT_A_POLYNOM_mpzm, &D, &D, &W[i + 1]);
        }
      bap_init_polynom_mpzm (&A[r - 1]);
      bap_mods_polynom_mpzm ((bap_polynom_mpz) & A[r - 1], &D);
/*
for (i = 0; i < r; i++)
{   printf ("A [%d] = ", i);
    ba0_printf ("%Azm\n", &A [i]);
...
*/
      ...
      bap_mods_polynom_mpzm ((bap_polynom_mpz) & D, &D);

      Z = (struct bap_polynom_mpzm *) ba0_alloc
          (sizeof (struct bap_polynom_mpzm) * r);
      for (i = 0; i < r; i++)
...
*/
          bap_exquo_polynom_numeric_mpz
              ((bap_polynom_mpz) & D, (bap_polynom_mpz) & D, old_q);
          for (i = 0; i < r; i++)
            {
/*
...
*/
              ...
              bap_Euclidean_division_polynom_mpzm
                  (BAP_NOT_A_POLYNOM_mpzm, &U, &U, &F[i]);
              bap_mods_polynom_mpzm ((bap_polynom_mpz) & U, &U);
              bap_mul_polynom_numeric_mpz
                  ((bap_polynom_mpz) & U, (bap_polynom_mpz) & U, old_q);
 ...
 */
      {
        bap_polynom_mpz X = (bap_polynom_mpz) & U;
        bool found = false;
        ...
        for (i = 0; i < r; i++)
          {
            if (vrai_factor[i])
              continue;
            if (i == 0)
              bap_set_polynom_mpz (X, (bap_polynom_mpz) & F[i]);
            else
              {
      ...
      for (i = 1; i < r - 1; i++)
        {
          ...
          bap_mods_polynom_mpzm ((bap_polynom_mpz) & Z[i], &Z[i]);
        }
/*
for (i = 0; i < r - 1; i++)
{   printf ("Z [%d] = ", i);
      ...
      for (i = 0; i < r; i++)
        {
          ...
          bap_Euclidean_division_polynom_mpzm
              (BAP_NOT_A_POLYNOM_mpzm, &V, &V, &F[i]);
          bap_mods_polynom_mpzm ((bap_polynom_mpz) & V, &V);
          bap_mul_polynom_numeric_mpz
              ((bap_polynom_mpz) & V, (bap_polynom_mpz) & V, old_q);
          ba0_mpzm_module_set (q, false);
    ...
    bap_polynom_mpz A)
{
  ...
  while (!found)
    {
      if (ba0_mpz_mod_ui (bunk, *lc, (unsigned long int) ba0_mint_hp_module) != 0)
        {
      ...
      if (!found)
        ba0_mint_hp_module_set
            (ba0_next_small_prime (ba0_mint_hp_module), true);
    }
  ba0_restore (&M);
 ...
 */
              ...
              bap_mods_polynom_mpzm ((bap_polynom_mpz) & F, &F);
              bap_normal_numeric_primpart_polynom_mpz
                  ((bap_polynom_mpz) & F, (bap_polynom_mpz) & F);
/*
   If F | A then
*/
              if (bap_is_factor_polynom_mpz (A, (bap_polynom_mpz) & F, A))
                {
/*
   A = A / F
...
*/
                  ...
                  ba0_pull_stack ();
                  combine_factors_univaries_mpzm
                      (prod, (bap_polynom_mpz) & F, k, a_combiner, l, n);
                  ba0_push_another_stack ();
/*
...
*/
  ...
  else
    {
      ...
      Berlekamp_Hensel_lifting (&new_factor, vrai_factor, &new_A, A,
          &prod_mod, p);
/*
          ...
          if (vrai_factor[i])
            {
              ...
              bap_mods_polynom_mpzm
                  ((bap_polynom_mpz) & new_factor.tab[i].factor,
              ...
              bap_mul_product_polynom_mpz (prod, prod,
                  (bap_polynom_mpz) & new_factor.tab[i].factor, k);
              ba0_push_another_stack ();
            }
          else
            n++;
...
/*
 * Assigns to point some new bav_point_int_p which does not annihilate
    ...
    baz_ideal_lifting lifting)
{
  ...
  bap_eval_polynom_point_int_p_mpz (&bunk, lifting->initial, &lifting->point);
  do
    {
    ...
    bap_power_mpz puis)
{
  ...
  volatile bap_polynom_mpz Ax, newAx, A, G;
 ...
 */
  x = baz_EZF_distinguished_variable (A);
 ...
 */
  bap_set_point_polynom_mpz ((ba0_point) & lifting.point, lifting.A, false);
/*
 ...
 */
  ...
  ba0_realloc2_table ((ba0_table) & nonzero, 1,
  ...
  ba0_init_table ((ba0_table) & nonzero_one_two);
  ba0_realloc_table
      ((ba0_table) & nonzero_one_two, lifting.factors_initial.size);
  ...
  factinit_mod_point =
      (ba0_mpz_t *) ba0_alloc
      (sizeof (ba0_mpz_t) * (1 + lifting.factors_initial.size));
      ...
      while (nb_success < nb_confirm && nb_factors != 1)
        {
          ...
          baz_yet_another_point_int_p_mpz
              (&lifting.point, &nonzero, &lifting.factors_initial,
              BAV_NOT_A_VARIABLE);

          baz_HL_begin_redistribute (&lifting, factinit_mod_point, newAx, cont);

          if (bap_equal_polynom_mpz (Ax, newAx))
            continue;
/*
 ...
 */
          BA0_TRY
          {
            baz_HL_integer_divisors
                ((ba0_mpz_t *) 0, &lifting, factinit_mod_point);
          }
          BA0_CATCH
          {
          ...
          bap_separant_polynom_mpz (G, newAx);
          baz_gcd_polynom_mpz
              (G, BAP_NOT_A_POLYNOM_mpz, BAP_NOT_A_POLYNOM_mpz, G, newAx);
          if (!bap_is_numeric_polynom_mpz (G))
            continue;

          BA0_SWAP (bap_polynom_mpz, Ax, newAx);
          baz_factor_polynom_mpz
              ((bap_product_mpz) & lifting.factors_mod_point, Ax);
 ...
 */
      BA0_TRY
      {
        baz_HL_end_redistribute (&lifting, factinit_mod_point + 1, Ax, cont);
      }
      BA0_CATCH
      {
        if (ba0_global.exception.raised != BAZ_EXHDIS)
...
*/
      ...
      for (i = 0; i < lifted_factors.size; i++)
        bap_normal_numeric_primpart_polynom_mpz
            (&lifted_factors.tab[i].factor, &lifted_factors.tab[i].factor);
      ba0_pull_stack ();
      bav_R_pull_ordering ();
      for (i = 0; i < lifted_factors.size; i++)
        {
          ...
          if (lc_negatif)
            {
              lc = bap_numeric_initial_polynom_mpz
                  (&lifted_factors.tab[i].factor);
              if (ba0_mpz_sgn (*lc) < 0)
                {
          ...
          bap_mul_product_polynom_mpz
              (prod, prod, &lifted_factors.tab[i].factor, k);
        }
      if (lc_negatif)
      ...
      if (A->total_rank.rg[i].deg == 1)
        {
          ...
          BA0_SWAP (bap_polynom_mpz, A, cont);
          i = 0;
        }
      else
        i++;
