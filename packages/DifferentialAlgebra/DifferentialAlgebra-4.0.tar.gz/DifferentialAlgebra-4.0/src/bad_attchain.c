 ...
 * regular chains (see @code{bad_intersectof_regchain}). 
 * Exception @code{BAD_ERRIAC} is raised if @var{A} and @var{B}
 * have differents orderings or are not both differential or not both non 
 * differential.
    ...
    bad_attchain B)
{
  ...
  if (bad_has_property_attchain
      (A, bad_differential_ideal_property) !=
      bad_has_property_attchain (B, bad_differential_ideal_property))
    BA0_RAISE_EXCEPTION (BAD_ERRIAC);
 ...
 * If no derivations are defined then @dfn{differential} is cleared.
 * If @dfn{differential} is set then @dfn{squarefree} is set.
 * If, moreover, at least two derivations are defined then
 *     @dfn{coherence} is set.
 * If @dfn{differential} is cleared or only one derivation
 *     is defined then @dfn{coherence} is cleared.
    ...
    bad_attchain A)
{
  if (bav_global.R.ders.size == 0)
    bad_clear_property_attchain (A, bad_differential_ideal_property);
  if (bad_has_property_attchain (A, bad_differential_ideal_property))
    {
      bad_set_property_attchain (A, bad_squarefree_property);
      if (bav_global.R.ders.size >= 2)
        bad_set_property_attchain (A, bad_coherence_property);
      else
        bad_clear_property_attchain (A, bad_coherence_property);
    }
  else
    bad_clear_property_attchain (A, bad_coherence_property);
    ...
    bad_attchain C)
{
  ...
  n = sizeof (properties) / sizeof (*properties);
  ba0_realloc_table ((ba0_table) T, T->size + n);
  for (i = 0; i < n; i++)
    {
