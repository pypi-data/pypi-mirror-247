 ...
 */
  ...
  bap_exquo_polynom_numeric_mpz (&A->numer, &A->numer, ba0_mpq_numref (icontent));

  baz_init_ratfrac (&B);
  bap_signed_numeric_content_polynom_mpz (ba0_mpq_denref (icontent), &A->denom);
  bap_exquo_polynom_numeric_mpz (&B.denom, &A->denom, ba0_mpq_denref (icontent));
  ...
  while (!bap_outof_itercoeff_mpz (&iter))
    {
      if (terms.size == terms.alloc)
        {
          ...
          ba0_realloc2_table ((ba0_table) & coeffs,
      ...
      baz_set_ratfrac_polynom (coeffs.tab[coeffs.size], &C);
    ...
    struct bmi_callback *callback)
{
  ...
  for (i = 0; i < X.size; i++)
    {
      if (v == BAV_NOT_A_VARIABLE ||
          bav_R_variable_number (X.tab[i]) >= bav_R_variable_number (v))
        larger = true;
      else
        lower = true;
    }
    ...
    struct bmi_callback *callback)
{
  ...
  struct baz_ratfrac A;
  struct bav_tableof_variable X, G;
  bav_variable v;
  ba0_int_p i;
 ...
 */
  bav_R_mark_variables (false);
  bap_mark_indets_polynom_mpz (&A.numer);
  bap_mark_indets_polynom_mpz (&A.denom);
  ba0_init_table ((ba0_table) & X);
  bav_R_marked_variables (&X, true);
  v = BAV_NOT_A_VARIABLE;
  for (i = 0; i < X.size; i++)
    {
      if (!bad_member_variable_base_field (X.tab[i], &K))
        {
          if (v == BAV_NOT_A_VARIABLE || bav_gt_variable (v, X.tab[i]))
            v = X.tab[i];
        }
    }
/*
