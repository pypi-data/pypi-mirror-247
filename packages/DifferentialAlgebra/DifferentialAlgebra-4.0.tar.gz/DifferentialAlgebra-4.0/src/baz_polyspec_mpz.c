...
/*
  bap_pull_clot_parameters_stack ();
 */
  ba0_restore (&M);
}
...
baz_yet_another_point_int_p_mpz (
    ...
    bap_tableof_polynom_mpz nonzero,
 ...
 */
  ...
  for (i = 0; i < nonzero->size; i++)
    bap_mark_indets_polynom_mpz (nonzero->tab[i]);
  for (i = 0; i < prod->size; i++)
    bap_mark_indets_polynom_mpz (&prod->tab[i].factor);
      ...
      do
        {
          ...
          for (i = 0; i < point->size; i++)
            {
              if (point->tab[i]->var != avoid &&
                  ((n == 0 && ba0_member_table
                          (point->tab[i]->var, (ba0_table) & T)) ||
                      (n == 1 && !ba0_member_table
                          (point->tab[i]->var, (ba0_table) & T))))
                {
                  ...
                  for (i = 0; !bad_point && i < nonzero->size; i++)
                    {
                      if (!bap_is_numeric_polynom_mpz (nonzero->tab[i]))
                        {
                          bap_eval_polynom_point_int_p_mpz
                      ...
                      if (!bap_is_numeric_polynom_mpz (&prod->tab[i].factor))
                        {
                          ...
                          bad_point = ba0_mpz_cmp_ui
                              (value, (unsigned long int) 2) <= 0;
                        }
                    }
                }
            }
    ...
    baz_ideal_lifting lifting)
{
  ...
  ba0_init_point ((ba0_point) & lifting->point);
...
baz_HL_printf_ideal_lifting (
    void *z)
{
  baz_ideal_lifting lif = (baz_ideal_lifting) z;
 ...
 */
  ...
  bap_realloc_product_mpz
      (&new_factors_initial, lifting->factors_mod_point.size);
...
/*
   foo = lcu
   bar = intermediate bav_variable 
 */
          lc = bap_numeric_initial_polynom_mpzm
              (&lifting->factors_mod_point.tab[j].factor);
          ba0_mpz_mul (foo, *lc, cont);
          bap_eval_polynom_point_int_p_mpz
              (&bar, &new_factors_initial.tab[j].factor, &lifting->point);
          ...
          bap_pow_polynom_mpz
              (&tmp, &lifting->factors_initial.tab[k].factor, m);
 ...
 */
  ...
  bap_set_product_mpz (&new_factors_mod_point,
      (bap_product_mpz) & lifting->factors_mod_point);
  if (lc_negatif)
    {
      j = lifting->factors_mod_point.size - 1;
      bap_neg_polynom_mpz
          (&new_factors_mod_point.tab[j].factor,
          (bap_polynom_mpz) & lifting->factors_mod_point.tab[j].factor);
    }
/*
      ...
      for (j = 0; j < new_factors_mod_point.size; j++)
        {
          lc = bap_numeric_initial_polynom_mpz
              (&new_factors_mod_point.tab[j].factor);
          bap_eval_polynom_point_int_p_mpz
              (&foo, &new_factors_initial.tab[j].factor, &lifting->point);
          ba0_mpz_divexact (foo, *lc, foo);
          bap_mul_polynom_numeric_mpz
              (&new_factors_initial.tab[j].factor,
 ...
 */
  ...
  bap_set_product_mpzm (&lifting->factors_mod_point,
      (bap_product_mpzm) & new_factors_mod_point);
  ba0_restore (&M);
}
    ...
    ba0_mpz_t cont)
{
  ...
  for (i = 0; i < lifting->factors_initial.size; i++)
    bap_eval_polynom_point_int_p_mpz (&factinit_mod_point[i + 1],
        &lifting->factors_initial.tab[i].factor, &lifting->point);
  bap_evalcoeff_polynom_point_int_p_mpz (R, lifting->A, &lifting->point);
    ...
    baz_ideal_lifting lifting)
{
  ...
  factinit_mod_point = (ba0_mpz_t *) ba0_alloc
...
baz_HL_ideal_Hensel_lifting (
    bap_product_mpz lifted_factors,
    baz_ideal_lifting lifting)
{
  ...
  A_eval = (struct bap_polynom_mpzm *) ba0_alloc
      (sizeof (struct bap_polynom_mpzm) * (nb_x + 1));
  lc_A_eval = (struct bap_polynom_mpzm *) ba0_alloc
      (sizeof (struct bap_polynom_mpzm) * (nb_x + 1) * nb_f);
 ...
 */
  ...
  sigma = (struct bap_polynom_mpzm *) ba0_alloc
      (sizeof (struct bap_polynom_mpzm) * nb_f);
  for (i = 0; i < nb_f; i++)
 ...
 */
  ...
  for (j = 0; j < nb_x; j++)
    {
      bap_mods_polynom_mpzm ((bap_polynom_mpz) & A_eval[j + 1], &A_eval[j + 1]);
      ...
      while (!bap_is_zero_polynom_mpzm (E) &&
          k <= bap_degree_polynom_mpzm
          (&A_eval[j + 1], lifting->point.tab[j]->var))
        {
          bap_mul_polynom_value_int_p_mpzm
              (monome, monome, lifting->point.tab[j]);
          bap_coeftayl_polynom_mpzm (C, E, lifting->point.tab[j], k);
          if (!bap_is_zero_polynom_mpzm (C))
            {
              lifting->point.size = j;
              bap_multi_Diophante_polynom_mpzm (sigma, &old_factors,
                  C, &lifting->point, maxdeg, lifting->p, lifting->l);
 ...
 */
              if (new_factors.size == 2)
                {
                  bap_polynom_mpzm temp1, temp2, psigma;
                  struct ba0_mark M;
                  ...
                  bap_mul_polynom_mpzm
                      (temp1, &sigma[0], &new_factors.tab[1].factor);
                  bap_mul_polynom_mpzm
                      (temp2, &sigma[1], &new_factors.tab[0].factor);
                  ...
                  while (i < new_factors.size && new_factors.size > 1)
                    {
                      bap_mods_polynom_mpzm
                          ((bap_polynom_mpz) & new_factors.tab[i].factor,
                          &new_factors.tab[i].factor);
                      if (bap_is_factor_polynom_mpz ((bap_polynom_mpz) &
                              A_eval[j + 1],
                              (bap_polynom_mpz) & new_factors.tab[i].factor, Z))
                        {
                          bap_set_polynom_mpz
                              ((bap_polynom_mpz) & A_eval[j + 1], Z);
 ...
 */
  bap_mods_product_mpzm ((bap_product_mpz) & new_factors, &new_factors);
  bap_expand_product_mpz (Z, (bap_product_mpz) & new_factors);
 ...
 */
  ...
  ba0_pull_stack ();
  bap_set_product_mpz (lifted_factors, (bap_product_mpz) & new_factors);
  ba0_restore (&M);
}
  ...
  if (degA < degB)
    {
      ...
      bav_R_variable_number (bap_leader_polynom_mpz (A)) <=
      bav_R_variable_number (x))
