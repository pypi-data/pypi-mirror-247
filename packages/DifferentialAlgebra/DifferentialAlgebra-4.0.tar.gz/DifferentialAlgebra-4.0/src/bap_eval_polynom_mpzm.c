...
/*
 * texinfo: bap_eval_polynom_numeric_mpzm
...
/*
 * texinfo: bap_eval_polynom_polynom_mpzm
 * Assign to @var{R} the polynomial obtained by evaluating @var{A} at @math{v=value}.
 */

BAP_DLL void
bap_eval_polynom_polynom_mpzm (
    bap_polynom_mpzm R,
    bap_polynom_mpzm A,
    bav_variable v,
    bap_polynom_mpzm val)
{
  struct bap_polynom_mpzm G, B, coeff, value;
  struct bap_itercoeff_mpzm iter;
  struct bav_term T;
  bav_Idegree d;
  bav_Iordering r;
    ...
    bav_value_int_p val)
{
  ...
  bap_eval_polynom_numeric_mpzm (R, A, val->var, c);
  ba0_restore (&M);
}
...
/*
 * texinfo: bap_eval_polynom_point_int_p_mpzm
 * Assign *@var{value} to @var{A} modulo @var{point}.
 */

BAP_DLL void
bap_eval_polynom_point_int_p_mpzm (
    ba0_mpzm_t *res,
    bap_polynom_mpzm A,
...
/*
 * texinfo: bap_evalcoeff_polynom_point_int_p_mpzm
...
BAP_DLL void
bap_evalcoeff_polynom_point_int_p_mpzm (
    bap_polynom_mpzm R,
    bap_polynom_mpzm A,
    bav_point_int_p point)
{
  ...
  while (!bap_outof_itercoeff_mpzm (&iter))
    {
      bap_coeff_itercoeff_mpzm (&C, &iter);
      bap_term_itercoeff_mpzm (&T, &iter);
      bap_eval_polynom_point_int_p_mpzm (&c, &C, point);
