...
/*
	...
	if (rg.deg == 0)
	    ...
	    bap_addmulrk_polynom_mpz (&A, &bunk, &rg, &A);
    }
    if (rg.deg > 0)
    {	struct bav_term T;
	bav_init_term (&T);
...
bap_nsr3_Ducos_polynom_mpz (
    bap_tableof_polynom_mpz VR,
    bap_tableof_polynom_mpz VP,
    bap_tableof_polynom_mpz VQ,
    bap_polynom_mpz s,
    bap_polynom_mpz ss,
    bav_variable v)
{
    ...
    bav_variable v)
{
  ...
  if (bap_leader_polynom_mpz (P) != v || bap_leader_polynom_mpz (Q) != v)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
...
*/
  ...
  ba0_record (&M);
/*
ba0_printf ("algo_new (%Az, %Az, %v)\n", P, Q, v);
*/
  bap_init_readonly_polynom_mpz (&coeff);
  bap_init_polynom_mpz (&s);
  bap_init_polynom_mpz (&Z);

  bap_initial_polynom_mpz (&coeff, Q);
    ...
    bav_variable v)
{
  ...
  if (bap_is_zero_polynom_mpz (P) || bap_is_zero_polynom_mpz (Q))
  ...
  else
    {
      ...
      if (degQ == 0)
        bap_pow_polynom_mpz (R, Q, degP);
      algo_new_mpz (R, P, Q, v);
      if (oppose)
        {
          bap_neg_polynom_mpz (Q, Q);
          Q->readonly = b;
...
*/
  ...
  ba0_realloc2_table
      ((ba0_table) VP, VR->size, (ba0_new_function *) & bap_new_polynom_mpz);
  ...
  if (VQ->size == 1)
    bap_pseudo_division_polynom_mpz
        ((bap_polynom_mpz) 0, VQ->tab[0], (bav_Idegree *) 0, P, Q, v);
  else if (VQ->size == 2)
    {
      if (perm)
        {
      ...
      else
        {
          ...
          bap_pseudo_division_polynom_mpz
              ((bap_polynom_mpz) 0, VQ->tab[0], &e, P, Q, v);
          bap_initial2_polynom_mpz (&coeff, Q, v);
