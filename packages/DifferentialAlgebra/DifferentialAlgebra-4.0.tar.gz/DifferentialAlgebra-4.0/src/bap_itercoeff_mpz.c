...
/****************************************************************************
 ITERATORS OF ba0_mpzICIENTS
 ****************************************************************************/

static enum bap_rank_code
itercoeff_samerank_mpz (
    bav_term R,
    bav_term last_term,
...
/*
 * texinfo: bap_begin_itercoeff_mpz
    ...
    bav_variable v)
{
  ...
  bap_begin_itermon_mpz (&I->debut, A);
  bap_term_itermon_mpz (&last_term, &I->debut);
  bap_begin_itermon_mpz (&I->fin, A);
  bap_seeklast_itermon_mpz (&I->fin, &itercoeff_samerank_mpz,
...
/*
 * texinfo: bap_next_itercoeff_mpz
    ...
    bap_itercoeff_mpz I)
{
  ...
  bav_init_term (&last_term);
  bap_set_itermon_mpz (&I->debut, &I->fin);
  bap_next_itermon_mpz (&I->debut);
  if (!bap_outof_itermon_mpz (&I->debut))
    {
      bap_term_itermon_mpz (&last_term, &I->debut);
      bap_seeklast_itermon_mpz (&I->fin, &itercoeff_samerank_mpz,
          &last_term, last_number);
...
/*
 * texinfo: bap_term_itercoeff_mpz
...
BAP_DLL void
bap_term_itercoeff_mpz (
    bav_term T,
    bap_itercoeff_mpz I)
{
  if (bap_outof_itercoeff_mpz (I))
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
  bap_term_itermon_mpz (T, &I->debut);
  bav_strip_term (T, T, I->last_variable->number.tab[bav_R_Iordering ()]);
}

/*
 * texinfo: bap_coeff_itercoeff_mpz
  ...
  if (I->poly->access == bap_sequential_monom_access)
    {
      ...
      A->seq.first = bap_number_itermon_clot_mpz (&I->debut.iter);
      A->seq.after = bap_number_itermon_clot_mpz (&I->fin.iter) + 1;
    }
  else
    {
...
/*
 * texinfo: bap_seek_coeff_itercoeff_mpz
    ...
    bav_term T)
{
  ...
  bap_seekfirst_itermon_mpz (&I->debut, &itercoeff_samerank_mpz,
      last_term, last_number);

  ba0_record (&M);
  bav_init_term (&U);
  bap_term_itermon_mpz (&U, &I->debut);
  ...
  if (I->poly->access == bap_sequential_monom_access)
    {
      ...
      A->access = bap_sequential_monom_access;
      A->seq.first = bap_number_itermon_clot_mpz (&I->debut.iter);
      A->seq.after = bap_number_itermon_clot_mpz (&I->fin.iter);
    }
  else
    {
      A->clot = I->poly->clot;
    ...
    bav_term T)
{
  ...
  if ((R == A && B != BAP_NOT_A_POLYNOM_mpz) ||
      (R == B && A != BAP_NOT_A_POLYNOM_mpz))
    BA0_RAISE_EXCEPTION (BA0_ERRNYP);

  if (T->size == 0)
    {
      if (A != BAP_NOT_A_POLYNOM_mpz && A != R)
        bap_set_readonly_polynom_mpz (A, R);
      if (B != BAP_NOT_A_POLYNOM_mpz && B != R)
        bap_set_polynom_zero_mpz (B);
      return;
    }
  else
    bap_begin_itercoeff_mpz (&iter, R, T->rg[T->size - 1].var);

  bap_begin_itermon_mpz (&iter.debut, R);
  ...
  ba0_record (&M);
  bav_init_term (&U);
  bap_term_itermon_mpz (&U, &iter.debut);
  ba0_pull_stack ();

  code = bav_compare_stripped_term (T, &U, last_number);
  if (code == ba0_gt)
    bap_set_itermon_mpz (&iter.fin, &iter.debut);
  else
    {
      bap_seeklast_itermon_mpz (&iter.fin,
          &itercoeff_samerank_mpz, last_term, last_number);
  ...
  if (A != BAP_NOT_A_POLYNOM_mpz)
    {
      bap_set_polynom_itercoeff_mpz (A, &iter);
    }
  if (B != BAP_NOT_A_POLYNOM_mpz)
    {
      bap_set_itermon_mpz (&iter.debut, &iter.fin);
      bap_end_itermon_mpz (&iter.fin, R);
      bap_next_itermon_mpz (&iter.fin);
      bap_set_polynom_itercoeff_mpz (B, &iter);
    }
}

/*
 * texinfo: bap_close_itercoeff_mpz
 * Close the iterator. 
 */

BAP_DLL void
bap_close_itercoeff_mpz (
    bap_itercoeff_mpz iter)
{
  iter = (bap_itercoeff_mpz) 0;
}
