...
bap_init_geobucket_mint_hp (
    bap_geobucket_mint_hp geo)
{
  ba0_init_table ((ba0_table) geo);
}
 ...
 */
              ...
              if (j >= geo->size)
                {
                  ba0_realloc2_table ((ba0_table) geo, j + 2,
              ...
              if (j != i)
                {
                  if (bap_is_zero_polynom_mint_hp (geo->tab[j]))
                    {
                      BA0_SWAP (bap_polynom_mint_hp, geo->tab[i], geo->tab[j]);
                      i -= 1;
                    }
                  else
                    {
                      bap_add_polynom_mint_hp
                          (geo->tab[j], geo->tab[j], geo->tab[i]);
    ...
    bap_polynom_mint_hp A)
{
  ...
  if (i >= geo->size)
    {
      ba0_realloc2_table ((ba0_table) geo, i + 2,
          (ba0_new_function *) & bap_new_polynom_mint_hp);
    ...
    bap_geobucket_mint_hp geo)
{
  ...
  iter = (bap_itermon_mint_hp) ba0_alloc
      (sizeof (struct bap_itermon_mint_hp) * geo->size);
