    ...
    bap_polynom_mpq A)
{
  ...
  bap_begin_itermon_clot_mpq (&I->iter, I->poly->clot);
  if (I->poly->access == bap_sequential_monom_access)
    bap_goto_itermon_clot_mpq (&I->iter, I->poly->seq.after - 1);
  else
    {
...
/*
 * texinfo: bap_outof_itermon_mpq
 * Return true if the iterator is outside the polynomial.
 */

BAP_DLL bool
bap_outof_itermon_mpq (
    bap_itermon_mpq I)
{
  if (I->poly->access == bap_sequential_monom_access)
    {
      ba0_int_p n = bap_number_itermon_clot_mpq (&I->iter);
...
/*
 * texinfo: bap_next_itermon_mpq
 * Move @var{I} to the next monomial.
 */

BAP_DLL void
bap_next_itermon_mpq (
    bap_itermon_mpq I)
{
  if (I->poly->access == bap_sequential_monom_access)
    bap_next_itermon_clot_mpq (&I->iter);
  else
    {
...
/*
 * texinfo: bap_prev_itermon_mpq
 * Move @var{I} to the previous monomial.
 */

BAP_DLL void
bap_prev_itermon_mpq (
    bap_itermon_mpq I)
{
  if (I->poly->access == bap_sequential_monom_access)
    bap_prev_itermon_clot_mpq (&I->iter);
  else
    {
...
BAP_DLL void
bap_goto_itermon_mpq (
    bap_itermon_mpq I,
    ba0_int_p n)
{
  if (n < 0 || n >= bap_nbmon_polynom_mpq (I->poly))
    BA0_RAISE_EXCEPTION (BA0_ERRALG);

  if (I->poly->access == bap_sequential_monom_access)
    bap_goto_itermon_clot_mpq (&I->iter, I->poly->seq.first + n);
  else
    {
      ...
      if (I->poly->access == bap_sequential_monom_access)
        {
          ...
          A->seq.first = bap_number_itermon_clot_mpq (&I->iter);
...
static void
seekfirst_sequential_itermon_poly_mpq (
    bap_itermon_mpq I,
    ...
    bav_Inumber last_number)
{
  ...
  bav_init_term (&T);
  bap_term_itermon_mpq (&T, I);
  code = (*f) (&T, last_term, last_number);
  if (code == bap_rank_too_high)
    {
// look on the right
      left = bap_number_itermon_clot_mpq (&I->iter);
...
*/
      ...
      right = bap_number_itermon_clot_mpq (&I->iter);
    }
  while (right - left > 1)
    {
      middle = (right + left) / 2;
      bap_goto_itermon_clot_mpq (&I->iter, middle);
      bap_term_itermon_mpq (&T, I);
      code = (*f) (&T, last_term, last_number);
      if (code == bap_rank_too_high)
        left = middle;
...
*/
  if (left != right)
    {
      if (left != bap_number_itermon_clot_mpq (&I->iter))
        bap_prev_itermon_clot_mpq (&I->iter);
      bap_term_itermon_mpq (&T, I);
      code = (*f) (&T, last_term, last_number);
      if (code != bap_rank_ok)
...
static void
seekfirst_indexed_itermon_poly_mpq (
    bap_itermon_mpq I,
    enum bap_rank_code (*f) (struct bav_term *,
        struct bav_term *,
        bav_Inumber),
...
/*
 * texinfo: bap_seekfirst_itermon_mpq
    ...
    bav_Inumber last_number)
{
  ...
  if (bap_is_zero_polynom_mpq (I->poly))
    BA0_RAISE_EXCEPTION (BAP_ERRNUL);
  ba0_record (&M);
  if (bap_outof_itermon_mpq (I))
    bap_begin_itermon_mpq (I, I->poly);
  if (I->poly->access == bap_sequential_monom_access)
    seekfirst_sequential_itermon_poly_mpq (I, f, last_term, last_number);
  else
    seekfirst_indexed_itermon_poly_mpq (I, f, last_term, last_number);
  ba0_restore (&M);
}

static void
seeklast_sequential_itermon_poly_mpq (
    bap_itermon_mpq I,
    enum bap_rank_code (*f) (struct bav_term *,
        struct bav_term *,
        bav_Inumber),
...
*/
  if (left != right)
    {
      if (right != bap_number_itermon_clot_mpq (&I->iter))
        bap_next_itermon_clot_mpq (&I->iter);
      bap_term_itermon_mpq (&T, I);
      code = (*f) (&T, last_term, last_number);
      if (code != bap_rank_ok)
...
static void
seeklast_indexed_itermon_poly_mpq (
    bap_itermon_mpq I,
    enum bap_rank_code (*f) (struct bav_term *,
        struct bav_term *,
        bav_Inumber),
...
/*
 * texinfo: bap_seeklast_itermon_mpq
    ...
    bav_Inumber last_number)
{
  ...
  if (bap_outof_itermon_mpq (I))
    bap_begin_itermon_mpq (I, I->poly);
  if (I->poly->access == bap_sequential_monom_access)
    seeklast_sequential_itermon_poly_mpq (I, f, last_term, last_number);
  else
    seeklast_indexed_itermon_poly_mpq (I, f, last_term, last_number);
  ba0_restore (&M);
}

/*
 * texinfo: bap_close_itermon_mpq
 * Close the iterator. 
 */

BAP_DLL void
bap_close_itermon_mpq (
    bap_itermon_mpq iter)
{
  iter = (bap_itermon_mpq) 0;
}
