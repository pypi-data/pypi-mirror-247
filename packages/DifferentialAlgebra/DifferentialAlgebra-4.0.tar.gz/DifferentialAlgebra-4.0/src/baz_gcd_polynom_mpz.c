 ...
 *
 * Returns a small prime strictly less than prime, which does
 * not annihilate c1.
 */
...
/*
 * Computes the gcd of univariate, numerically primitive, polynomials
 ...
 */
  {
    bav_term T = &polys->tab[0]->total_rank;
    bav_variable v;
    if (T->size != 1)
      BA0_RAISE_EXCEPTION (BA0_ERRALG);
    v = T->rg[0].var;
 ...
 */
  p = baz_univariate_gcd_determine_small_prime
      (ba0_largest_small_prime () + 2, c1);
  ba0_mint_hp_module_set (p, true);
/*
 * Abar = polys mod p
 */
  ba0_init_table ((ba0_table) & Abar);
  ba0_realloc2_table ((ba0_table) & Abar, polys->size,
 ...
 */
  ...
  bap_mods_polynom_mpzm ((bap_polynom_mpz) & Ghat, &Ghat);
  bap_numeric_primpart_polynom_mpz
      ((bap_polynom_mpz) & Ghat, (bap_polynom_mpz) & Ghat);
  ...
  for (;;)
    {
      ...
      for (i = 0; i < polys->size && divisor_found; i++)
        divisor_found = bap_is_factor_polynom_mpz
            (polys->tab[i], (bap_polynom_mpz) & Ghat, BAP_NOT_A_POLYNOM_mpz);
      if (divisor_found)
        break;
 ...
 */
        {
          ...
          bap_mul_polynom_numeric_mpz
              ((bap_polynom_mpz) & Ghat, (bap_polynom_mpz) & Ghat, utilde);
          bap_mul_polynom_numeric_mpz ((bap_polynom_mpz) & Gtilde,
              (bap_polynom_mpz) & Gtilde, ubar);
          bap_add_polynom_mpz ((bap_polynom_mpz) & Ghat,
              (bap_polynom_mpz) & Gtilde, (bap_polynom_mpz) & Ghat);
          bap_polynom_mpz_to_mpzm (&Gtilde, (bap_polynom_mpz) & Ghat);
          bap_mul_polynom_numeric_mpzm (&Ghat, &Gtilde, c1);
          bap_mods_polynom_mpzm ((bap_polynom_mpz) & Ghat, &Ghat);
          bap_numeric_primpart_polynom_mpz
              ((bap_polynom_mpz) & Ghat, (bap_polynom_mpz) & Ghat);
 ...
 */
  ba0_pull_stack ();
  if (G != BAP_NOT_A_POLYNOM_mpz)
    bap_set_polynom_mpz (G, (bap_polynom_mpz) & Ghat);
  ba0_restore (&M);
}
 ...
 * variable.
 * Exception @code{BA0_ERRALG} is raised if @var{polys} contains less
 * than two elements.
 */
    ...
    bap_tableof_polynom_mpz polys)
{
  ...
  ba0_record (&M);
/*
 * Computes the numeric contents of the polynomials (in cont)
 *              primitive parts of the polynomials (in primitive_polys)
 *              gcd of the numeric contents (in gcd_cont)
 */
  ba0_init_table ((ba0_table) & cont);
  ba0_realloc2_table ((ba0_table) & cont, polys->size,
      (ba0_new_function *) & ba0_new_mpz);
  cont.size = polys->size;
/*
 * primitive_polys is not initialized before the end of the next loop
 */
  ba0_init_table ((ba0_table) & primitive_polys);
  ba0_realloc_table ((ba0_table) & primitive_polys, polys->size);
      ...
      if (ba0_mpz_cmp_ui (cont.tab[i], 1) == 0)
      ...
      else
        {
          ...
          bap_exquo_polynom_numeric_mpz
              (primitive_polys.tab[i], polys->tab[i], cont.tab[i]);
        }
    }
/*
 * gcd_cont
...
baz_gcdheu_sort_by_ascending_nbmon (
    bap_tableof_polynom_mpz A,
    bap_tableof_polynom_mpz B)
{
  ba0_int_p n, i, p, q;
  bool modified;
      ...
      for (i = 0; i < n; i++)
        {
          ...
          if (p > q)
            {
              BA0_SWAP (bap_polynom_mpz, A->tab[i], A->tab[i + 1]);
              BA0_SWAP (bap_polynom_mpz, B->tab[i], B->tab[i + 1]);
              modified = true;
            }
        }
    }
    ...
    bap_tableof_polynom_mpz polys)
{
  ...
  ba0_init_table ((ba0_table) & maxnorm);
  ba0_realloc2_table ((ba0_table) & maxnorm, polys->size,
      (ba0_new_function *) & ba0_new_mpz);
  for (i = 0; i < polys->size; i++)
...
baz_gcdheu_tableof_polynom_mpz (
    bap_polynom_mpz G,
    bap_tableof_polynom_mpz polys0,
 ...
 */
  ...
  ba0_realloc_table ((ba0_table) & polys, polys0->size);
 ...
 */
  ...
  ba0_realloc2_table ((ba0_table) & cont, polys.size,
      (ba0_new_function *) & ba0_new_mpz);
  ba0_init_table ((ba0_table) & term);
  ba0_realloc2_table ((ba0_table) & term, polys.size,
      (ba0_new_function *) & bav_new_term);
  ba0_init_table ((ba0_table) & primitive_polys);
  ba0_realloc2_table ((ba0_table) & primitive_polys, polys.size,
  ...
  for (i = 0; i < polys.size; i++)
    {
      ...
      bap_exquo_polynom_term_mpz
          (primitive_polys.tab[i], polys.tab[i], term.tab[i]);
      bap_exquo_polynom_numeric_mpz
          (primitive_polys.tab[i], primitive_polys.tab[i], cont.tab[i]);
    }
  cont.size = polys.size;
 ...
 */
  ...
  ba0_init_table ((ba0_table) & evaluated_polys);
  ba0_realloc2_table ((ba0_table) & evaluated_polys,
  ...
  for (nbloops = 0; nbloops < 6 && !found && !stop; nbloops++)
    {
      if (baz_gcdheu_too_complex_problem
          (toocomplex, ksi, &primitive_polys, v, &X))
        stop = true;
      else
        {
/*
 ...
 */
          ...
          for (i = 0; i < primitive_polys.size && !a_root_was_hit; i++)
            {
              bap_eval_polynom_numeric_mpz
                  (evaluated_polys.tab[i], primitive_polys.tab[i], v, ksi);
 ...
 */
              baz_gcdheu_tableof_polynom_mpz
                  (&gcd_evaluated_polys, &evaluated_polys, toocomplex);
/*
 * lift gcd_evaluated_polys and division test
 */
              baz_genpoly_polynom_mpz
                  (&lifted_gcd, &gcd_evaluated_polys, ksi, v);
              bap_normal_numeric_primpart_polynom_mpz
                  (&lifted_gcd, &lifted_gcd);
              if (bap_is_factor_tableof_polynom_mpz
                  (&lifted_gcd, &primitive_polys))
                found = true;
              else
                {
/*
 ...
 */
                  for (i = 0; i < evaluated_polys.size; i++)
                    bap_exquo_polynom_mpz
                        (evaluated_polys.tab[i],
 ...
 */
                  baz_gcdheu_sort_by_ascending_nbmon
                      (&evaluated_polys, &primitive_polys);
                  for (i = 0; i < evaluated_polys.size && found; i++)
                    {
                      baz_genpoly_polynom_mpz
                          (&lifted_evaluated_poly,
                          evaluated_polys.tab[i], ksi, v);
                      if (!bap_is_zero_polynom_mpz
                          (&lifted_evaluated_poly)
                          &&
                          bap_is_factor_polynom_mpz
                          (primitive_polys.tab[i],
                              &lifted_evaluated_poly, &lifted_gcd))
                        found =
                            bap_is_factor_tableof_polynom_mpz
                            (&lifted_gcd, &primitive_polys);
                    }
                }
            }
 ...
 */
  W->size = 0;
  ba0_realloc_table ((ba0_table) W, A->total_rank.size);
 ...
 */
  ba0_init_table ((ba0_table) & weights);
  ba0_realloc2_table ((ba0_table) & weights, polys->size,
 ...
 */
  ba0_init_table ((ba0_table) & bestvar);
  ba0_realloc_table ((ba0_table) & bestvar, polys->size);
  for (i = 0; i < polys->size; i++)
    {
      ...
      for (i = 0; i < n; i++)
        {
          if (weights.tab[i]->tab[bestvar.tab[i]] >
            ...
            {
              BA0_SWAP (bap_polynom_mpz, polys->tab[i], polys->tab[i + 1]);
              BA0_SWAP (ba0_tableof_int_p, weights.tab[i], weights.tab[i + 1]);
 ...
 */
  ...
  ba0_mpz_mul_2exp (lb, lb, 4 * (unsigned long) ba0_mpz_sizeinbase (max_maxnorm, 2));
  ba0_pull_stack ();
...
baz_EZG_set_modulus_part_ideal_lifting (
    baz_ideal_lifting lifting,
    ba0_mpz_t floor_prime,
    bap_tableof_polynom_mpz uni_polys)
{
 ...
 */
  ba0_init_table ((ba0_table) & lcoeff);
  ba0_realloc_table ((ba0_table) & lcoeff, uni_polys->size);
 ...
 */
  ...
  bap_polynom_mpz_to_mpzm
      (&lifting->factors_mod_point.tab[0].factor, gcd_uni_polys);
 ...
 */
  ...
  ba0_init_table ((ba0_table) & cof_uni_poly_modp);
  ba0_realloc_table ((ba0_table) & cof_uni_poly_modp, uni_polys->size);
  ...
  while (i < uni_polys->size && !found)
    {
      ...
      bap_Euclid_polynom_mpzm
          (&g_modp, cof_uni_poly_modp.tab[i], gcd_uni_polys_modp);
  ...
  if (found)
    {
      ...
      bap_set_polynom_mpzm
          (&lifting->factors_mod_point.tab[1].factor, cof_uni_poly_modp.tab[i]);
      ba0_restore (&M);
      return;
    }
/*
 ...
 */
  ...
  bap_set_polynom_mpzm
      (&lifting->factors_mod_point.tab[1].factor, &cof_comblin_modp);
  ba0_restore (&M);
}
    ...
    bool giveup)
{
  ...
  volatile bap_polynom_mpz gcd;

  struct bav_term term_one;
  struct bav_rank rg;
  volatile bav_variable x = BAV_NOT_A_VARIABLE;
  ...
  for (i = 0; i < polys.size; i++)
    {
      ...
      polys.tab[i] = (bap_polynom_mpz) bap_copy_polynom_mpz (&tmp);
    }
/*
 * lifting.point receives [y1 = 0, ..., yk = 0] (x is shifted out)
 */
  bap_set_point_polynom_mpz ((ba0_point) & lifting.point, polys.tab[0], false);
/*
    ba0_delete_coord_point 
		((ba0_point)&lifting.point, (ba0_point)&lifting.point, 0);
*/
/*
 ...
 */
  ...
  baz_content_tableof_polynom_mpz (&gcd_cont, &polys, x, giveup);
  ...
  ba0_init_table ((ba0_table) & polys1);
  ba0_realloc_table ((ba0_table) & polys1, 1);
  polys1.size = 1;
  bap_init_product_mpz (&tmp_prod);
  for (i = 0; i < polys.size; i++)
    {
      polys1.tab[0] = polys.tab[i];
      baz_content_tableof_polynom_mpz (&tmp_prod, &polys1, x, giveup);
 ...
 */
  ba0_init_table ((ba0_table) & nonzero);
  ba0_realloc2_table ((ba0_table) & nonzero, 2 * polys.size,
      (ba0_new_function *) & bap_new_readonly_polynom_mpz);
  bav_init_term (&term_one);
  for (i = 0; i < polys.size; i++)
    {
      bap_begin_itercoeff_mpz (&iter, polys.tab[i], x);
      bap_coeff_itercoeff_mpz (nonzero.tab[nonzero.size], &iter);
      bap_seek_coeff_itercoeff_mpz
          (nonzero.tab[nonzero.size + 1], &iter, &term_one);
      bap_coeff_itercoeff_mpz (nonzero.tab[nonzero.size + 1], &iter);
      nonzero.size += 2;
    }
  bap_set_product_one_mpz (&tmp_prod);

  ba0_init_table ((ba0_table) & uni_polys);
  ba0_realloc2_table ((ba0_table) & uni_polys, polys.size,
      (ba0_new_function *) & bap_new_polynom_mpz);
      ...
      while (degre_x > 0 && nb_success < nb_confirm)
        {
          ...
          baz_yet_another_point_int_p_mpz (&lifting.point, &nonzero,
              &tmp_prod, BAV_NOT_A_VARIABLE);

          ba0_reset_table ((ba0_table) & uni_polys);
          for (i = 0; i < polys.size; i++)
            {
              bap_evalcoeff_polynom_point_int_p_mpz
                  (uni_polys.tab[i], polys.tab[i], &lifting.point);
              uni_polys.size += 1;
            }
 ...
 */
      ...
      bap_normal_numeric_primpart_polynom_mpz (gcd, gcd);
      if (bap_is_factor_tableof_polynom_mpz (gcd, &polys))
        {
          ba0_pull_stack ();
          bav_R_pull_ordering ();
          bap_sort_polynom_mpz (gcd, gcd);
          bap_mul_product_polynom_mpz (G, G, gcd, 1);
          bav_R_push_ordering (r);
          ba0_push_another_stack ();
          found = true;
          continue;
...
baz_init_factored_polynom_mpz (
    baz_factored_polynom_mpz A)
{
  bap_init_product_mpz (&A->outer);
  bap_init_polynom_mpz (&A->poly);
}

static baz_factored_polynom_mpz
baz_new_factored_polynom_mpz (
    void)
{
  baz_factored_polynom_mpz A;

  A = (baz_factored_polynom_mpz) ba0_alloc
...
baz_printf_factored_polynom_mpz (
    void *AA)
{
  baz_factored_polynom_mpz A = (baz_factored_polynom_mpz) AA;
    ...
    baz_gcd_data gcd_data)
{
  ...
  ba0_reset_table ((ba0_table) & gcd_data->F);
}
...
baz_realloc_gcd_data (
    ...
    ba0_int_p n)
{
  ba0_realloc2_table ((ba0_table) & gcd_data->F, n,
...
baz_printf_gcd_data (
    void *GG)
{
  baz_gcd_data gcd_data = (baz_gcd_data) GG;
  ba0_int_p i;
...
comp_factored_polynom_ascending (
    ...
    const void *y)
{
  baz_factored_polynom_mpz A = *(baz_factored_polynom_mpz *) x;
  baz_factored_polynom_mpz B = *(baz_factored_polynom_mpz *) y;
  ...
  switch (mode)
    ...
    case ba0_ascending_mode:
      qsort (gcd_data->F.tab, gcd_data->F.size,
          sizeof (baz_factored_polynom_mpz), &comp_factored_polynom_ascending);
      break;
    }
  i = 0;
  while (i < gcd_data->F.size - 1)
    {
      if (bap_equal_polynom_mpz
          (&gcd_data->F.tab[i]->poly, &gcd_data->F.tab[i + 1]->poly))
        ba0_delete_table ((ba0_table) & gcd_data->F, i);
      else
        i += 1;
    }
}
    ...
    bav_tableof_variable Y)
{
  ...
  ba0_init_table ((ba0_table) & Z);
  ba0_realloc_table ((ba0_table) & X, bav_global.R.vars.size);
  ba0_realloc_table ((ba0_table) & Z, bav_global.R.vars.size);
 ...
 */
      ...
      ba0_reset_table ((ba0_table) & Z);
      ba0_reset_table ((ba0_table) & X);
      for (i = 0; i < gcd_data->F.size; i++)
        {
      ...
      for (i = 0; i < Z.size; i++)
        {
          v = Z.tab[i];
          if (v->hack != gcd_data->F.size ||
              (Y != (bav_tableof_variable) 0 &&
                  ba0_member_table (v, (ba0_table) Y)))
            {
              X.tab[X.size] = v;
              X.size += 1;
            }
              ...
              while (!bap_outof_itercoeff_mpz (&iter))
                {
                  ...
                  bap_exquo_polynom_term_mpz
                      (&gcd_data->F.tab[gcd_data->F.size]->poly, &C, &T);
/*
 * Beware to the ordering of the result
 */
                  bav_R_pull_ordering ();
                  bap_physort_polynom_mpz
                      (&gcd_data->F.tab[gcd_data->F.size]->poly);
                  bav_sort_term (&Touter);

                  bap_mul_product_term_mpz
                      (&gcd_data->F.tab[gcd_data->F.size]->outer,
                      &gcd_data->F.tab[i]->outer, &Touter);
                  bap_exquo_polynom_numeric_mpz
                      (&gcd_data->F.tab[gcd_data->F.size]->poly,
                      &gcd_data->F.tab[gcd_data->F.size]->poly, cont);
                  bap_mul_product_numeric_mpz
                      (&gcd_data->F.tab[gcd_data->F.size]->outer,
      ...
      if (!bap_is_zero_polynom_mpz (polys->tab[i]))
        {
          bap_minimal_total_rank_polynom_mpz
              (term.tab[term.size], polys->tab[i]);
          ba0_pull_stack ();
          bap_exquo_polynom_term_mpz
              (&gcd_data->F.tab[gcd_data->F.size]->poly,
  ...
  for (i = 0; i < term.size; i++)
    {
      ...
      ba0_pull_stack ();
      bap_mul_product_term_mpz
          (&gcd_data->F.tab[i]->outer, &gcd_data->F.tab[i]->outer, term.tab[i]);
      ba0_push_another_stack ();
    }
  ba0_pull_stack ();
 ...
 */
  ba0_init_table ((ba0_table) & cont);
  ba0_realloc2_table ((ba0_table) & cont, gcd_data->F.size,
      (ba0_new_function *) & ba0_new_mpz);
  for (i = 0; i < gcd_data->F.size; i++)
    {
      bap_signed_numeric_content_polynom_mpz
          (cont.tab[i], &gcd_data->F.tab[i]->poly);
      ba0_pull_stack ();
      bap_exquo_polynom_numeric_mpz
          (&gcd_data->F.tab[i]->poly, &gcd_data->F.tab[i]->poly, cont.tab[i]);
      ba0_push_another_stack ();
    }
  ...
  for (i = 0; i < cont.size; i++)
    {
      ...
      ba0_pull_stack ();
      bap_mul_product_numeric_mpz
          (&gcd_data->F.tab[i]->outer, &gcd_data->F.tab[i]->outer, cont.tab[i]);
      ba0_push_another_stack ();
    }
  ba0_pull_stack ();
 ...
 */
  baz_replace_by_coefficients_gcd_data (gcd_data, (bav_tableof_variable) 0);
}
    ...
    bap_polynom_mpz A)
{
  ...
  ba0_realloc2_table ((ba0_table) & T, gcd_data->F.size,
      (ba0_new_function *) & bap_new_polynom_mpz);
  ba0_init_table ((ba0_table) & is_factor);
  ba0_realloc_table ((ba0_table) & is_factor, gcd_data->F.size);
  ba0_reset_table ((ba0_table) is_modified);
  is_true_factor = true;
  for (i = 0; i < gcd_data->F.size; i++)
    {
      is_factor.tab[is_factor.size] = bap_is_factor_polynom_mpz
          (&gcd_data->F.tab[i]->poly, A, T.tab[T.size]);
      ...
      for (i = 0; i < gcd_data->F.size; i++)
        {
          ...
          is_factor.tab[i] = bap_is_factor_polynom_mpz
              (&gcd_data->F.tab[i]->poly, A, T.tab[i]);
        }
    }
  for (i = 0; i < gcd_data->F.size; i++)
    {
      while (is_factor.tab[i])
        {
          bap_mul_product_polynom_mpz
              (&gcd_data->F.tab[i]->outer, &gcd_data->F.tab[i]->outer, A, 1);
          bap_set_polynom_mpz (&gcd_data->F.tab[i]->poly, T.tab[i]);
          is_factor.tab[i] = bap_is_factor_polynom_mpz
              (&gcd_data->F.tab[i]->poly, A, T.tab[i]);
        }
    ...
    baz_gcd_data gcd_data)
{
  ...
  for (i = 1; i < gcd_data->F.size && it_divides; i++)
    it_divides = bap_is_factor_polynom_mpz
        (A, &gcd_data->F.tab[i]->poly, BAP_NOT_A_POLYNOM_mpz);
  return it_divides;
}
 ...
 */
  ...
  ba0_realloc_table ((ba0_table) Y, vars->size);
  if (vars->size == 1)
    {
/*
 ...
 */
      ...
      ba0_init_table ((ba0_table) & unipolys);
      ba0_realloc2_table ((ba0_table) & unipolys, gcd_data->F.size,
          (ba0_new_function *) & bap_new_readonly_polynom_mpz);
      unipolys.size = gcd_data->F.size;

      for (j = 0; j < gcd_data->F.size; j++)
        bap_set_readonly_polynom_mpz
            (unipolys.tab[j], &gcd_data->F.tab[j]->poly);
      ...
      if (!bap_is_one_polynom_mpz (&gcd))
        {
          for (j = 0; j < gcd_data->F.size; j++)
            bap_exquo_polynom_mpz
                (&gcd_data->F.tab[j]->poly, &gcd_data->F.tab[j]->poly, &gcd);
          bap_mul_product_polynom_mpz
              (&gcd_data->common, &gcd_data->common, &gcd, 1);
        }
/*
 ...
 */
      bap_init_polynom_mpz (&gcd);
      ba0_init_table ((ba0_table) & polys);
      ba0_realloc2_table ((ba0_table) & polys, gcd_data->F.size,
          (ba0_new_function *) & bap_new_readonly_polynom_mpz);
      polys.size = gcd_data->F.size;

      ba0_init_point ((ba0_point) & point);
      bap_set_point_polynom_mpz
          ((ba0_point) & point, &gcd_data->F.tab[0]->poly, true);
/*
 * nonzero = polynomials which should not annihilate
 */
      ba0_init_table ((ba0_table) & nonzero);
      ba0_init_table ((ba0_table) & unipolys);
      ba0_realloc2_table ((ba0_table) & nonzero, gcd_data->F.size,
          (ba0_new_function *) & bap_new_polynom_mpz);
      ba0_realloc2_table ((ba0_table) & unipolys, gcd_data->F.size,
          (ba0_new_function *) & bap_new_polynom_mpz);
      nonzero.size = gcd_data->F.size;
      unipolys.size = gcd_data->F.size;
      ba0_init_table ((ba0_table) & is_modified);
      ba0_realloc_table ((ba0_table) & is_modified, gcd_data->F.size);
      bap_init_product_mpz (&tmp_prod);
...
//              bav_sort_point_int_p (&point);
          ...
          else
            {
              for (j = 0; j < gcd_data->F.size; j++)
                bap_set_readonly_polynom_mpz
                    (polys.tab[j], &gcd_data->F.tab[j]->poly);
            }
/*
 ...
 */
              for (j = 0; j < gcd_data->F.size; j++)
                {
                  bap_evalcoeff_polynom_point_int_p_mpz
                      (unipolys.tab[j], polys.tab[j], &point);
                  if (bap_is_zero_polynom_mpz (unipolys.tab[j]))
                    BA0_RAISE_EXCEPTION (BA0_ERRALG);
                  bap_numeric_primpart_polynom_mpz
                      (unipolys.tab[j], unipolys.tab[j]);
                }
              baz_gcd_univariate_primitive_tableof_polynom_mpz
                  (&gcd, &unipolys);
/*
                  ...
                  if (v != ld)
                    {
                      ...
                      b = baz_is_true_factor_gcd_data (&is_modified,
                          gcd_data, &gcd);
                      bav_R_push_ordering (r);
                    }
                  else
 ...
 */
          ...
          if (all_gcd_are_one || v_disappeared)
            {
              if (ba0_member_table (v, (ba0_table) Y))
                BA0_RAISE_EXCEPTION (BA0_ERRALG);
              Y->tab[Y->size] = v;
              Y->size += 1;
            }
...
baz_gcd_polynom_mpz (
    bap_polynom_mpz G,
    bap_polynom_mpz cofA,
    bap_polynom_mpz cofB,
    bap_polynom_mpz A,
    bap_polynom_mpz B)
{
  ...
  ba0_record (&M);
  ba0_init_table ((ba0_table) & polys);
  ba0_realloc_table ((ba0_table) & polys, 2);
  polys.tab[0] = A;
  polys.tab[1] = B;
  polys.size = 2;
 ...
 */
      ba0_pull_stack ();
      bap_mul_product_polynom_mpz
          (G, &gcd_data.common, &gcd_data.F.tab[0]->poly, 1);
    }
  else
    {
      BA0_TRY
 ...
 */
        bap_init_polynom_mpz (&gcd);
        ba0_init_table ((ba0_table) & polys2);
        ba0_realloc_table ((ba0_table) & polys2, gcd_data.F.size);
        for (i = 0; i < gcd_data.F.size; i++)
 ...
 */
      ...
      if (v == BAV_NOT_A_VARIABLE)
        BA0_RAISE_EXCEPTION (BA0_ERRALG);
/*
 * v is the maximum of the variables occuring in polys0
 */
      nbcoeff = 0;
      for (i = 0; i < polys0->size; i++)
        {
 ...
 */
  ...
  ba0_record (&M);
  ba0_init_table ((ba0_table) & polys);
  ba0_realloc2_table ((ba0_table) & polys, nbcoeff,
      (ba0_new_function *) & bap_new_readonly_polynom_mpz);
  for (i = 0; i < polys0->size; i++)
    {
 ...
 * highest derivative occuring in @var{polys0}, if @var{v} is zero).
 ...
 */
  ...
  ba0_record (&M);
  ba0_init_table ((ba0_table) & polys1);
  ba0_realloc_table ((ba0_table) & polys1, polys0->size);
 ...
 */
          ...
          ba0_init_table ((ba0_table) & polys2);
          ba0_realloc2_table ((ba0_table) & polys2, polys1.size,
              (ba0_new_function *) & bap_new_readonly_polynom_mpz);
    ...
    bav_variable v)
{
  ...
  ba0_init_table ((ba0_table) & polys);
  ba0_realloc_table ((ba0_table) & polys, 1);
  polys.tab[0] = A;
  polys.size = 1;
...
comp_nbmon_ascending_polynom_mpz (
    ...
    const void *y)
{
  bap_polynom_mpz A = *(bap_polynom_mpz *) x;
  bap_polynom_mpz B = *(bap_polynom_mpz *) y;
  ba0_int_p nA, nB;
    ...
    bool giveup)
{
  ...
  if (bap_is_numeric_polynom_mpz (A0) ||
      (bap_is_univariate_polynom_mpz (A0) &&
 ...
 */
      ...
      bap_init_product_mpz (&cont);
      baz_content_tableof_polynom_mpz
          (&cont, &polys, BAV_NOT_A_VARIABLE, giveup);
/*
...
/*
 * Factors A0. Result in prod.
 * Relies on gcd computations only (content, Yun).
 * Tries the polynomials of F0 for gcd.
 */

/*
 * texinfo: baz_factor_easy_polynom_mpz
 * Assign to @var{prod} a factorization of @var{A} which is not too difficult
 * to compute and not necessarily irreducible. The input list @var{F} involves
 * polynomials which may help factoring @var{A} by gcd computations.
 */
...
baz_factor_easy_polynom_mpz (
    bap_product_mpz prod,
    bap_polynom_mpz A0,
    bap_listof_polynom_mpz F0)
{
  struct bap_tableof_polynom_mpz polys, F;
  struct bap_product_mpz prod1, prod2, prod3;
 ...
 */
  ba0_init_table ((ba0_table) & F);
  if (F0 != (bap_listof_polynom_mpz) 0)
    {
      ba0_set_table_list ((ba0_table) & F, (ba0_list) F0);
      qsort (F.tab, F.size, sizeof (bap_polynom_mpz),
          &comp_nbmon_ascending_polynom_mpz);
    }
/*
 * Get rid of the numerical content and of the obvious factors. 
 * A is now primitive numerically
 */
...
/*
 * Look for common factors between A and the elements of F.
 * Result in prod1 (initialized with the obvious factors of A0).
 */
  bap_init_product_mpz (&prod1);
  bap_mul_product_term_mpz (&prod1, &prod1, &T);
  bap_init_product_mpz (&prod2);
 ...
 */
          ...
          for (j = 0; j < prod2.size; j++)
            {
              baz_factor_easy2_polynom_mpz
                  (&prod3, &prod2.tab[j].factor, giveup);
...
/*
 * Apply factor_easy2 over A. 
 */
  baz_factor_easy2_polynom_mpz (&prod3, &A, giveup);
  ba0_pull_stack ();
  bap_mul_product_mpz (prod, &prod1, &prod3);
