 ...
 * Exception BAP_EXHNCP is raised if te computation failed.
 */
 ...
 * Exception @code{BAP_EXHNCP} is raised if te computation failed.
 */
    ...
    bav_Idegree k)
{
  ...
  if (k == 1)
    ...
    {
      bap_polynom_mpzm A_mod_p, B_mod_p, U_mod_p, V_mod_p, G_mod_p,
...
*/
      ...
      bap_polynom_mpz_to_mpzm (A_mod_p, (bap_polynom_mpz) A);
      bap_polynom_mpz_to_mpzm (B_mod_p, (bap_polynom_mpz) B);
      U_mod_p = bap_new_polynom_mpzm ();
      V_mod_p = bap_new_polynom_mpzm ();
      G_mod_p = bap_new_polynom_mpzm ();
      bap_extended_Euclid_polynom_mpzm
          (U_mod_p, V_mod_p, G_mod_p, A_mod_p, B_mod_p);
      ...
      for (j = 1; j <= k - 1; j++)
        {
          ...
          bap_mul_polynom_mpz (F, (bap_polynom_mpz) newU, (bap_polynom_mpz) A);
          bap_sub_polynom_mpz (E, E, F);
          bap_mul_polynom_mpz (F, (bap_polynom_mpz) newV, (bap_polynom_mpz) B);
          bap_sub_polynom_mpz (E, E, F);
          bap_exquo_polynom_numeric_mpz (E, E, ba0_mpzm_module);
/*
   On pourrait prendre mod p plutot que mod p^j
*/
          bap_polynom_mpz_to_mpzm ((bap_polynom_mpzm) E, E);

          bap_mul_polynom_mpzm (Uhat, U_mod_p, (bap_polynom_mpzm) E);
          bap_mul_polynom_mpzm (Vhat, V_mod_p, (bap_polynom_mpzm) E);
          bap_Euclidean_division_polynom_mpzm (Q, Uhat, Uhat, B_mod_p);
          bap_mul_polynom_mpzm (Q, Q, A_mod_p);
          bap_add_polynom_mpzm (Vhat, Vhat, Q);
          bap_mul_polynom_numeric_mpz ((bap_polynom_mpz) Uhat,
              (bap_polynom_mpz) Uhat, ba0_mpzm_module);
          bap_mul_polynom_numeric_mpz ((bap_polynom_mpz) Vhat,
              (bap_polynom_mpz) Vhat, ba0_mpzm_module);
          bap_add_polynom_mpz ((bap_polynom_mpz) newU,
              (bap_polynom_mpz) newU, (bap_polynom_mpz) Uhat);
          bap_add_polynom_mpz ((bap_polynom_mpz) newV,
              (bap_polynom_mpz) newV, (bap_polynom_mpz) Vhat);
 ...
 * in te Taylor expansion of @math{A} at @math{@emph{val} = (x,\,\alpha)}.
 */
  ...
  if (val->var != bap_leader_polynom_mpzm (A))
    {
      ...
      bap_change_ordering_termstripper (&B.tstrip, bav_R_Iordering ());
    }
  ba0_pull_stack ();
...
BAP_DLL void bap_quorem_polynom_mpzm 
		(bap_polynom_mpzm Q, bap_polynom_mpzm R, 
		 bav_rank rg, bap_polynom_mpzm A, bap_polynom_mpzm B)
{   struct bav_rank rgA, rgB, rgbar;
    bap_polynom_mpzm newQ, newR, C;
    struct bav_term T;
    ba0_mpz_t c;
    struct ba0_mark M;
...
*/
  ...
  B = (struct bap_polynom_mpzm *) ba0_alloc
...
*/
      gamma = (struct bap_polynom_mpzm *) ba0_alloc
          (sizeof (struct bap_polynom_mpzm) * 2);
      ...
      for (j = 0; j < nb_f - 1; j++)
        {
          ...
          bap_multi_Diophante_polynom_mpzm
              (gamma, &temp_prod, &B[nb_f - 1], &point, 0, p, k);
...
*/
  ...
  else
    {
      ...
      bap_Bezout_polynom_mpzm
          (U, V, G, &prod->tab[1].factor, &prod->tab[0].factor, p, k);
...
ba0_put_string ("=============================\n");
*/
  bap_evalcoeff_polynom_point_int_p_mpzm (&A, &A, point);
  if (!bap_is_zero_polynom_mpzm (&A))
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
...
*/
  gamma = (struct bap_polynom_mpzm *) ba0_alloc
      (sizeof (struct bap_polynom_mpzm) * nb_f);
  delta = (struct bap_polynom_mpzm *) ba0_alloc
      (sizeof (struct bap_polynom_mpzm) * nb_f);
  for (j = 0; j < nb_f; j++)
    {
  ...
  if (nb_x > 0)
    {
      bav_Idegree m;
      bap_polynom_mpzm A, monome;
      bap_polynom_mpzm E, F, new_C;
...
*/
      ba0_init_point ((ba0_point) & new_point);
      point->size--;
      ba0_set_point ((ba0_point) & new_point, (ba0_point) point);
      point->size++;
      ...
      for (j = 0; j < nb_f; j++)
        {
          bap_eval_polynom_value_int_p_mpzm (&new_prod.tab[j].factor,
              &prod->tab[j].factor, point->tab[point->size - 1]);
        }
      new_prod.size = prod->size;

      new_C = bap_new_polynom_mpzm ();
      bap_eval_polynom_value_int_p_mpzm (new_C, C, point->tab[point->size - 1]);

      bap_multi_Diophante_polynom_mpzm
          (gamma, &new_prod, new_C, &new_point, maxdeg, p, k);
/*
      ...
      for (m = 1; m <= maxdeg && !bap_is_zero_polynom_mpzm (E); m++)
        {
          bap_mul_polynom_value_int_p_mpzm
              (monome, monome, point->tab[point->size - 1]);
...
*/
              bap_multi_Diophante_polynom_mpzm
                  (delta, &new_prod, new_C, &new_point, maxdeg, p, k);
/*
...
*/
  ...
  else
    {
      ...
      bap_mark_ranks_polynom_mpzm (C);
      for (j = 0; j < nb_f; j++)
        bap_mark_ranks_polynom_mpzm (&prod->tab[j].factor);
      bav_init_term (&T);
      bav_marked_ranks_term (&T);
