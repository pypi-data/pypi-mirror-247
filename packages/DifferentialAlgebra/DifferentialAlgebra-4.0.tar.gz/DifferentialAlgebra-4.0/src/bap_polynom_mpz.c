...
static bap_polynom_mpz
bap_new_polynom_clot_mpz (
    bap_clot_mpz C)
{
  bap_polynom_mpz A;

  A = (bap_polynom_mpz) ba0_alloc (sizeof (struct bap_polynom_mpz));
  bap_init_polynom_clot_mpz (A, C);
  return A;
}
...
bap_gt_rank_polynom_mpz (
    ...
    void *Q)
{
  bap_polynom_mpz A = (bap_polynom_mpz) P;
  bap_polynom_mpz B = (bap_polynom_mpz) Q;
...
/*
 * texinfo: bap_involved_parameters_polynom_mpz
...
BAP_DLL void
bap_involved_parameters_polynom_mpz (
...
/*
 * texinfo: bap_mark_ranks_polynom_mpz
    ...
    bap_polynom_mpz A)
{
  ...
  bap_change_ordering_clot_mpz (A->clot, bav_R_Iordering ());
}
    ...
    bap_polynom_mpz A)
{
  ...
  if (bap_is_numeric_polynom_mpz (A))
    BA0_RAISE_EXCEPTION (BAP_ERRCST);
  if ((A == initial && reductum != BAP_NOT_A_POLYNOM_mpz) ||
    ...
    bav_variable v)
{
  ...
  if (bap_is_numeric_polynom_mpz (A) ||
      (v != BAV_NOT_A_VARIABLE && v != bap_leader_polynom_mpz (A)))
    {
      if (!bap_depend_polynom_mpz (A, v))
      ...
      else
        {
          ...
          bap_initial_and_reductum_polynom_mpz
              (lcoeff != BAP_NOT_A_POLYNOM_mpz ?
              &lc : BAP_NOT_A_POLYNOM_mpz,
              reductum != BAP_NOT_A_POLYNOM_mpz ?
              &red : BAP_NOT_A_POLYNOM_mpz, &AA);
    ...
    bav_Idegree d)
{
  ...
  if (R == (bap_polynom_mpz) 0)
    ...
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
  if (C != (bap_polynom_mpz) 0 && C->readonly)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
  if (C == R)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);

  if (bap_is_numeric_polynom_mpz (A) ||
      (v != BAV_NOT_A_VARIABLE && !bap_depend_polynom_mpz (A, v)))
    {
      if (d == 0)
        {
          if (C != A && C != (bap_polynom_mpz) 0)
            bap_set_polynom_mpz (C, A);
          bap_set_polynom_zero_mpz (R);
        }
      else
        {
          if (C != (bap_polynom_mpz) 0)
  ...
  if (C != A && R != A)
    {
      ...
      bap_begin_creator_mpz (&creaR, R, &A->total_rank,
          ...
          if (C != A && R != A)
            {
              ...
              if (C != (bap_polynom_mpz) 0)
                bap_write_creator_mpz
                    (&creaC, &U, *bap_coeff_itermon_mpz (&iter));
              ba0_push_another_stack ();
            }
          else
            bap_write_creator_mpz
          ...
          if (C != A && R != A)
            {
              ba0_pull_stack ();
              bap_write_creator_mpz
                  (&creaR, &T, *bap_coeff_itermon_mpz (&iter));
              ba0_push_another_stack ();
            }
          else
            bap_write_creator_mpz
    ...
    bap_polynom_mpz A)
{
  ...
  P = bap_new_polynom_mpz ();
  bap_begin_creator_mpz (&crea, P, &T,
      bap_approx_total_rank, bap_nbmon_polynom_mpz (A));
    ...
    enum ba0_sort_mode mode)
{
  switch (mode)
    ...
    case ba0_descending_mode:
      qsort (T->tab, T->size, sizeof (bap_polynom_mpz),
          &comp_polynom_descending);
      break;
...
static char _struct_polynom_rang[] = "bap_polynom_mpz->total_rank";
...
{
  bap_polynom_mpz A = (bap_polynom_mpz) AA;
  ba0_int_p n = 0;

  if (code == ba0_isolated)
    n += ba0_new_gc_info
        (A, sizeof (struct bap_polynom_mpz), _struct_polynom);
    ...
    enum ba0_garbage_code code)
{
  ...
  if (code == ba0_isolated)
    A = (bap_polynom_mpz) ba0_new_addr_gc_info (AA, _struct_polynom);
  else
    A = (bap_polynom_mpz) AA;

  A->clot = (bap_clot_mpz) bap_garbage2_clot_mpz (A->clot, ba0_isolated);
