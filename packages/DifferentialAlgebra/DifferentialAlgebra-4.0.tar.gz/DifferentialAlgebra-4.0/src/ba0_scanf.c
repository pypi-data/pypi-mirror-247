...
/*
   ...
   A mark is set in another heap before parsing the list
*/
  ba0_push_another_stack ();
  ba0_record (&M);
  ba0_pull_stack ();
    ...
    bool struct_list_in_another_stack)
{
  ...
  for (;;)
    {
      ba0_value res;
      res = ba0_scanf_value (f, po, pf, (ba0_value) 0);
...
*/
  ...
  if (point == (ba0_point) 0)
    point = ba0_new_point ();
  ba0_set_table_list ((ba0_table) point, L);
...
*/
  ...
  n = ba0_length_list (L);
  m = 0;
  for (LL = L; LL != (ba0_list) 0; LL = LL->next)
    {
      o = ba0_length_list ((ba0_list) LL->value);
      m = BA0_MAX (m, o);
    }
  if (M == (ba0_matrix) 0)
    M = ba0_new_matrix ();
  ...
  while (f->text[i] != '\0')
    {
      ...
      if (backslash || isalpha ((int) f->text[i]))
      ...
      else if (f->text[i] != '%')
        {
          ...
          buffer[0] = f->text[i];
/*
	    if (!backslash && f->text [i] == ':' && f->text [i+1] != '\0')
	    {	buffer [1] = f->text [i+1];
		buffer [2] = '\0';
		i += 2;
	    } else
...
/*
   Varant of the above one, which reads the first token to be read.
*/
