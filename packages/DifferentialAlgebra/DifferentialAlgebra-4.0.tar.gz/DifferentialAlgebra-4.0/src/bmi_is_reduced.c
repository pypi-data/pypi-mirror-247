    ...
    struct bmi_callback *callback)
{
  ...
  ba0_sscanf2 (bmi_string_op (1, callback), "%t[%careful_expanded_Aq]", &polys);
#else
  ba0_sscanf2 (bmi_string_op (1, callback), "%t[%Aq]", &polys);
#endif
  ba0_init_table ((ba0_table) & result);
  ba0_realloc_table ((ba0_table) & result, polys.size);
...
#else
  ba0_sscanf2 (bmi_string_op (3, callback), "%t[%Az]", &C.decision_system);
#endif
