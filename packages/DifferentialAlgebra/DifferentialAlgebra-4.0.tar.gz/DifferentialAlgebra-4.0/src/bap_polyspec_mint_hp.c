...
/*-
\subsection{Polynomials with machine modular coefficients}

They are polynomials with {\tt ba0_mint_hp} coefficients.
Coefficients are in positive representation.
-*/

/*-
\paragraph{void bap_polynom_mpq_to_mint_hp 
		(bap_polynom_mint_hp A, bap_polynom_mpq B)}
Sets $A$ to $B \mod {(\mbox{\tt ba0_mint_hp_module})}$.
-*/
...
bap_polynom_mpq_to_mint_hp (
    ...
    bap_polynom_mpq B)
{
  A = (bap_polynom_mint_hp) 0;
  B = (bap_polynom_mpq) 0;
  BA0_RAISE_EXCEPTION (BA0_ERRNYP);
}

/*-
\paragraph{void bap_polynom_mpz_to_mint_hp (bap_polynom_mint_hp R, bap_polynom_mpz A)}
Sets $R$ to  $A \mod {(\mbox{\tt ba0_mint_hp_module})}$.
-*/
...
*/
      c = (ba0_mint_hp) ba0_mpz_fdiv_ui
          (*bap_coeff_itermon_mpz (&iter), ba0_mint_hp_module);
      if (c != 0)
        {
...
\paragraph{void bap_random_eval_polynom_mpz_to_mint_hp 
	(bap_polynom_mint_hp R, bap_polynom_mpz A, ba0_unary_predicate* f)}
Sets
$R$ to $A \mod (p, x_k - \alpha_k)$ where~$p$ 
is {\em mint_hp_module}, the
$x_k$ are the the derivatives occuring in~$A$ which satisfy~$f$ and the
 ...
 * the @math{x_k} denote the derivatives occuring in @var{A} which satisfy
  ...
  for (i = 0; i < A->total_rank.size; i++)
    {
      ...
      evaluer[u->index] = (*f) (u);
      if (evaluer[u->index])
        value[u->index] = bav_random_eval_variable_to_mint_hp (u);
    }
      ...
      for (i = 0; i < T.size; i++)
        {
          ...
          if (evaluer[u->index])
            {
              ba0_mint_hp d = ba0_pow_mint_hp (value[u->index], T.rg[i].deg);
...
/*-
\paragraph{void bap_Berlekamp_mint_hp (bap_product_mint_hp prod, bap_polynom_mint_hp P)}
 ...
 * Denote @var{n} the modulu @code{ba0_mint_hp_module}.
    ...
    bap_polynom_mint_hp P)
{
  ...
  struct bap_polynom_mint_hp *F = (bap_polynom_mint_hp) 0, B, G;
  struct ba0_mark M;
...
*/
  F = (struct bap_polynom_mint_hp *) ba0_alloc
      (sizeof (struct bap_polynom_mint_hp) * (r + 1));
