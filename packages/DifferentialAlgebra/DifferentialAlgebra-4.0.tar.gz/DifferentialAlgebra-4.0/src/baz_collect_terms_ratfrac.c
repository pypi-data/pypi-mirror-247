    ...
    baz_ratfrac G)
{
  return bav_equal_term (&F->numer.total_rank, &G->numer.total_rank)
      ...
      && bap_nbmon_polynom_mpz (&F->numer) ==
      bap_nbmon_polynom_mpz (&G->numer)
...
baz_preprocess_for_collect (
    ba0_mpz_t contnum,
    ba0_mpz_t contden,
    bap_polynom_mpz ppnum,
    bap_polynom_mpz ppden,
    ba0_int_p *yet,
    baz_ratfrac F)
{
...
baz_collect_terms_tableof_ratfrac (
    baz_tableof_ratfrac collected_coeffs,
    baz_tableof_ratfrac collected_terms,
    baz_tableof_ratfrac coeffs,
    baz_tableof_ratfrac terms)
{
  ...
  ba0_init_table ((ba0_table) & tmp_coeffs);
  ba0_init_table ((ba0_table) & tmp_terms);
  ...
  if (collected_coeffs == coeffs && collected_terms == terms)
    ...
    {
      in_place = false;
      ba0_set_table ((ba0_table) & tmp_coeffs, (ba0_table) coeffs);
      ba0_set2_table ((ba0_table) & tmp_terms, (ba0_table) terms,
 ...
 * be modified.
 */
  ba0_init_table ((ba0_table) & ppnum);
  ba0_init_table ((ba0_table) & ppden);
  ba0_init_table ((ba0_table) & contnum);
  ba0_init_table ((ba0_table) & contden);
  ba0_init_table ((ba0_table) & yet);
  ba0_realloc2_table ((ba0_table) & ppnum, coeffs->size,
      (ba0_new_function *) & bap_new_polynom_mpz);
  ppnum.size = coeffs->size;
  ba0_realloc2_table ((ba0_table) & ppden, coeffs->size,
      (ba0_new_function *) & bap_new_polynom_mpz);
  ppden.size = coeffs->size;
  ba0_realloc2_table ((ba0_table) & contnum, coeffs->size,
      (ba0_new_function *) & ba0_new_mpz);
  contnum.size = coeffs->size;
  ba0_realloc2_table ((ba0_table) & contden, coeffs->size,
      (ba0_new_function *) & ba0_new_mpz);
  contden.size = coeffs->size;
  ba0_realloc_table ((ba0_table) & yet, coeffs->size);
      ...
      while (j < yet.size)
        {
          if (in_place ?
              baz_are_likely_to_be_collected
              (coeffs->tab[i], coeffs->tab[j]) :
              baz_are_likely_to_be_collected
              (tmp_coeffs.tab[i], tmp_coeffs.tab[j]))
            {
              if (!yet.tab[i])
                baz_preprocess_for_collect
                    (contnum.tab[i], contden.tab[i],
                    ppnum.tab[i], ppden.tab[i], &yet.tab[i],
                    in_place ? coeffs->tab[i] : tmp_coeffs.tab[i]);
              if (!yet.tab[j])
                baz_preprocess_for_collect
                    (contnum.tab[j], contden.tab[j],
...
*/
                  if (in_place)
                    {
                      ...
                      baz_mul_ratfrac_numeric_mpq
                          (terms->tab[j], terms->tab[j], alpha);
                      baz_add_ratfrac
                          (terms->tab[i], terms->tab[i], terms->tab[j]);
                      ba0_delete_table ((ba0_table) coeffs, j);
                      ba0_delete_table ((ba0_table) terms, j);
                      ba0_push_another_stack ();
                    }
                  else
                    {
                      baz_mul_ratfrac_numeric_mpq
                          (tmp_terms.tab[j], tmp_terms.tab[j], alpha);
                      baz_add_ratfrac
                          (tmp_terms.tab[i], tmp_terms.tab[i],
                          tmp_terms.tab[j]);
                      ba0_delete_table ((ba0_table) & tmp_coeffs, j);
                      ba0_delete_table ((ba0_table) & tmp_terms, j);
                    }
                  ba0_delete_table ((ba0_table) & ppnum, j);
                  ba0_delete_table ((ba0_table) & ppden, j);
                  ba0_delete_table ((ba0_table) & contnum, j);
                  ba0_delete_table ((ba0_table) & contden, j);
                  ba0_delete_table ((ba0_table) & yet, j);
                }
              else
                j += 1;
            }
  ...
  if (!in_place)
    {
      ba0_set_table ((ba0_table) collected_coeffs, (ba0_table) & tmp_coeffs);
      ba0_set2_table ((ba0_table) collected_terms,
          (ba0_table) & tmp_terms,
          (ba0_new_function *) & baz_new_ratfrac,
          (ba0_set_function *) & baz_set_ratfrac);
    }
