...
#   if defined BMI_MEMCHECK
      if (gmp.maple_sp < 0)
        {
          fprintf (stderr,
              "bmi fatal error: MAPLE stack underflow\n");
          exit (1);
        }
#   endif
    }
...
#   if defined (MAPLE11) || defined (MAPLE12) || defined (BMI_BALSA)
  ...
  if (gmp.sp != 0 || blad_alloc != &ba0_gmp_alloc)
    {
      fprintf (stderr,
          "bmi fatal error: BLAD gmp allocators expected\n");
      fprintf (stderr, "file %s, line %d\n", f, l);
      fprintf (stderr, "actual = %lx, blad = %lx, maple = %lx\n",
          (unsigned long) blad_alloc,
          (unsigned long) &ba0_gmp_alloc,
          (unsigned long) gmp.maple_alloc);
...
#   if defined (MAPLE11) || defined (MAPLE12) || defined (BMI_BALSA)
  ...
  if (maple_alloc != gmp.maple_alloc)
    {
      fprintf (stderr,
          "bmi fatal error: MAPLE gmp allocators expected\n");
      fprintf (stderr, "file %s, line %d\n", f, l);
      exit (1);
    }
#   else
...
{ bool protect;
  ...
  gmp.kv = kv;
  ba0_get_settings_gmp (0, &protect, &mpz_string);
  ba0_set_settings_gmp (&bmi_mp_set_memory_function, protect, mpz_string);
...
#   if defined (BMI_DO_PUSH_MAPLE)
      MaplePushGMPAllocators
          (gmp.kv, gmp.maple_alloc, gmp.maple_realloc,
          gmp.maple_free);
#   endif
    }
#endif
...
{
#if defined (MAPLE11) || defined (MAPLE12) || defined (BMI_BALSA)
  ba0_mp_set_memory_functions (&ba0_gmp_alloc, &ba0_gmp_realloc,
      &ba0_gmp_free);
#else
  gmp.maple_sp += 1;
#   if defined (_MSC_VER)
  MaplePushGMPAllocators
      (gmp.kv, &bmi_ba0_gmp_alloc, &bmi_ba0_gmp_realloc,
      &bmi_ba0_gmp_free);
#   else
  MaplePushGMPAllocators
