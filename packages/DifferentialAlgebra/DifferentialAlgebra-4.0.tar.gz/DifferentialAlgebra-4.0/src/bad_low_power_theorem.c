...
bad_init_preparation_term (
    bad_preparation_term T)
{
  ba0_init_table ((ba0_table) & T->z);
  ba0_init_table ((ba0_table) & T->theta);
  ba0_init_table ((ba0_table) & T->deg);
}

static bad_preparation_term
bad_new_preparation_term (
    void)
{
  bad_preparation_term T;

  T = (bad_preparation_term) ba0_alloc (sizeof (struct bad_preparation_term));
    ...
    ba0_int_p n)
{
  if (T->theta.alloc < n)
    {
      ba0_realloc_table ((ba0_table) & T->z, n);
      ba0_realloc2_table
          ((ba0_table) & T->theta, n, (ba0_new_function *) & bav_new_term);
      ba0_realloc_table ((ba0_table) & T->deg, n);
    }
}
...
bad_set_preparation_term_one (
    bad_preparation_term T)
{
  ba0_reset_table ((ba0_table) & T->z);
  ba0_reset_table ((ba0_table) & T->theta);
  ba0_reset_table ((ba0_table) & T->deg);
}

static void
bad_set_preparation_term (
    bad_preparation_term T,
    bad_preparation_term U)
{
  ba0_int_p i;
  ...
  if (T != U)
    {
      ...
      ba0_set_table ((ba0_table) & T->z, (ba0_table) & U->z);
      ba0_reset_table ((ba0_table) & T->theta);
      ...
      ba0_set_table ((ba0_table) & T->deg, (ba0_table) & U->deg);
    }
}
...
{
  struct bav_rank rg;
  bav_variable v, fakevar;
  bav_symbol fakesym;
  bool first;
  struct ba0_mark M;
  ba0_int_p i;
  ba0_scanf_function *scanfsym;
  ba0_printf_function *printsym;
 ...
 */
  if (bav_global.R.deps.size == 0)
    BA0_RAISE_EXCEPTION (BA0_ERRNYP);
  v = bav_global.R.vars.tab[bav_global.R.deps.tab[0]];
/*
 ...
 */
  bav_get_settings_symbol (&scanfsym, &printsym);
  if (printsym == &bav_printf_numbered_symbol)
    bav_set_settings_symbol (scanfsym, &bav_printf_default_symbol);
/*
 * Prints a term
 */
  first = true;
 ...
 * Restore
 */
  bav_set_settings_symbol (scanfsym, printsym);
  ba0_restore (&M);
}
    ...
    bad_regchain A)
{
  ...
  bad_reduced_to_zero_derivatives_by_regchain
      (&nulles, &K->relations, bad_full_reduction);
    ...
    bad_preparation_term V)
{
  ...
  for (i = 0; i < V->z.size; i++)
    bad_mul_preparation_term_theta_z
        (&R, &R, V->theta.tab[i], V->z.tab[i], V->deg.tab[i]);
  ba0_pull_stack ();
...
bad_init_preparation_equation (
    bad_preparation_equation E)
{
  bap_init_product_mpz (&E->H);
  ba0_init_table ((ba0_table) & E->coeffs);
  ba0_init_table ((ba0_table) & E->terms);

  E->F = (bap_polynom_mpz) 0;
  E->A = (bad_regchain) 0;
  E->K = (bad_base_field) 0;
}
...
BAD_DLL bad_preparation_equation
bad_new_preparation_equation (
    void)
{
  bad_preparation_equation E;

  E = (bad_preparation_equation) ba0_alloc
      (sizeof (struct bad_preparation_equation));
...
bad_realloc_preparation_equation (
    bad_preparation_equation P,
    ba0_int_p n)
{
  ba0_realloc2_table ((ba0_table) & P->coeffs, n,
      (ba0_new_function *) & bap_new_polynom_mpz);
  ba0_realloc2_table ((ba0_table) & P->terms, n,
...
bad_printf_preparation_equation (
    void *A)
{
  bad_preparation_equation E = (bad_preparation_equation) A;
  ba0_int_p i, n;
  bool first;

  if (E->F == (bap_polynom_mpz) 0)
    return;

  n = E->K->relations.decision_system.size;
 ...
 */
  ...
  bad_reduced_to_zero_derivatives_by_regchain
      (&nulles, &E->K->relations, bad_full_reduction);

  bap_diff2_polynom_mpz (&B, A->tab[z], &theta, &nulles);
 ...
 */
  bad_set_preparation_equation_polynom2
      (&Eq, &Q, (ba0__mpz_struct *) 0, E->A, E->K);
 ...
 */
  ba0_delete_table ((ba0_table) & E->coeffs, k);
  ba0_delete_table ((ba0_table) & E->terms, k);
  ...
  for (i = 0; i < Eq.coeffs.size; i++)
    {
      ...
      bad_mul_preparation_term (E->terms.tab[E->terms.size],
          Eq.terms.tab[i], &termk);
...
/*
 * Computes most of a preparation equation for F and stores it in E since
 * the coeffs [i] are not guaranteed to be regular w.r.t. A.
 */
 ...
 */
          if (bad_is_a_reducible_polynom_by_regchain
              (E->coeffs.tab[i], &K->relations, bad_full_reduction,
 ...
 */
          ...
          else if (bad_is_a_reducible_polynom_by_regchain
              (E->coeffs.tab[i], A, bad_full_reduction,
                  bad_all_derivatives_to_reduce, &rg, &z))
            {
              bad_reduce_preparation_equation (E, i, z, &rg);
/*
      ...
      for (i = 0; i < E->terms.size - 1; i++)
        {
          ...
          if (dj < di)
            {
              BA0_SWAP (bad_preparation_term, E->terms.tab[i], E->terms.tab[j]);
              BA0_SWAP (bap_polynom_mpz, E->coeffs.tab[i], E->coeffs.tab[j]);
              swap = true;
            }
        }
    }
...
bad_low_power_theorem_simplify_intersectof_regchain (
    bad_intersectof_regchain S0,
    bad_intersectof_regchain T0,
    bad_base_field K)
{
  struct bad_preparation_equation E;
  struct bad_intersectof_regchain S, T, aux;
  bad_regchain general_component, A;
  bap_polynom_mpz F, ddz;
  ba0_mpz_t denom;
  struct ba0_mark M;
  volatile ba0_int_p i, n;

  if (T0->inter.size == 0)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
  n = K->relations.decision_system.size;
  general_component = T0->inter.tab[0];
  if (general_component->decision_system.size != n + 1)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
  F = general_component->decision_system.tab[n];
      ...
      if (A->decision_system.size == n + 1)
        {
          ...
          BA0_CATCH
          {
            ...
            bad_handle_splitting_exceptions_regchain
                (&aux, (struct bad_quench_map *) 0,
                ddz, (bad_regchain) 0, BAD_EXRDDZ, K);
