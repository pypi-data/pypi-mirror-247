...
BAV_DLL bav_value_interval_mpq
bav_new_value_interval_mpq (
    void)
{
  bav_value_interval_mpq value;

  value = (bav_value_interval_mpq) ba0_new_value ();
...
bav_set_value_interval_mpq (
    bav_value_interval_mpq dst,
    bav_value_interval_mpq src)
{
  if (src != dst)
    {
      dst->var = src->var;
...
BAV_DLL bav_point_interval_mpq
    ...
    void)
{
  ...
  point =
      (bav_point_interval_mpq) ba0_alloc (sizeof (struct
...
bav_realloc_point_interval_mpq (
    ...
    ba0_int_p n)
{
  ba0_realloc2_table ((ba0_table) point, n,
...
bav_set_point_interval_mpq (
    bav_point_interval_mpq dst,
    bav_point_interval_mpq src)
{
  ba0_int_p i;
    ...
    ba0_interval_mpq X)
{
  ...
  if (ba0_bsearch_point (v, (ba0_point) point, &i) != BA0_NOT_A_VALUE)
  ...
  else
    {
      ...
      ba0_insert_table ((ba0_table) point, i, value);
#if defined (DEBUG)
      if (!ba0_is_sorted_point ((ba0_point) point))
    ...
    ba0_interval_mpq X)
{
  ...
  if (ba0_bsearch_point (v, (ba0_point) dst, &i) != BA0_NOT_A_VALUE)
  ...
  else
    {
      ...
      value->var = v;
      ba0_set_interval_mpq (value->value, X);
      ba0_insert_table ((ba0_table) dst, i, value);
#if defined (DEBUG)
      if (!ba0_is_sorted_point ((ba0_point) dst))
        BA0_RAISE_EXCEPTION (BA0_ERRALG);
#endif
    }
}
...
bav_intersect_point_interval_mpq (
    bav_point_interval_mpq P,
    bav_point_interval_mpq Q,
    bav_point_interval_mpq R)
{
  ba0_int_p i;
  ...
  if (P != Q && P != R)
    ...
    BA0_SWAP (bav_point_interval_mpq, Q, R);
...
bav_bisect_point_interval_mpq (
    bav_tableof_point_interval_mpq T,
    bav_point_interval_mpq point,
    ba0_int_p coord)
{
  ba0__mpq_struct *a, *b;
  ...
  ba0_realloc2_table ((ba0_table) T, 2 * T->size + 2,
...
bav_set_tableof_point_interval_mpq (
    bav_tableof_point_interval_mpq dst,
    bav_tableof_point_interval_mpq src)
{
  ba0_int_p i;
