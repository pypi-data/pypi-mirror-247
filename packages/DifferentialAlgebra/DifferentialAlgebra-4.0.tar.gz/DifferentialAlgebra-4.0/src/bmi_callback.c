 ...
 */
  callback->ordering =
      ToMapleName (kv, BMI_IX_ordering, (M_BOOL) true);
  MapleGcProtect (callback->kv, callback->ordering);
  callback->equations =
      ToMapleName (kv, BMI_IX_equations, (M_BOOL) true);
  MapleGcProtect (callback->kv, callback->equations);
  callback->notation =
      ToMapleName (kv, BMI_IX_notation, (M_BOOL) true);
 ...
 */
  result = IsMapleString (callback->kv, a)
      || IsMapleName (callback->kv, a);
    ...
    struct bmi_callback *callback)
{
  ...
  MapleGcProtect (callback->kv, b);
  table =
      EvalMapleProc (callback->kv, callback->op, 2, b, callback->arg);
