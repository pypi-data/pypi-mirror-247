...
BAP_DLL bap_power_mpq
bap_new_power_mpq (
    void)
{
  bap_power_mpq P;

  P = (bap_power_mpq) ba0_alloc (sizeof (struct bap_power_mpq));
...
bap_scanf_power_mpq (
    void *A)
{
  bap_power_mpq P = (bap_power_mpq) A;
  bool exponent;
    ...
    ba0_int_p n)
{
  ...
  if (n > prod->alloc)
    {
      newtab = (struct bap_power_mpq *) ba0_alloc
          (sizeof (struct bap_power_mpq) * n);
...
BAP_DLL bap_product_mpq
bap_new_product_mpq (
    void)
{
  bap_product_mpq P;

  P = (bap_product_mpq) ba0_alloc (sizeof (struct bap_product_mpq));
...
static char _struct_product_num[] = "bap_product_mpq->num_factor._mp_d";
#else
#   if defined (BAD_FLAG_mpq)
static char _struct_product_num_num[] =
    "bap_product_mpq->num_factor._mp_num._mp_d";
static char _struct_product_num_den[] =
    "bap_product_mpq->num_factor._mp_den._mp_d";
#   endif
#endif
static char _struct_product_tab[] = "bap_product_mpq->tab";
...
{
  bap_product_mpq A = (bap_product_mpq) AA;
  ba0_int_p i, n = 0;
  ba0__mpz_struct *p;

  if (code == ba0_isolated)
    n += ba0_new_gc_info
        (A, sizeof (struct bap_product_mpq), _struct_product);
...
#   endif
  ...
  n += ba0_new_gc_info (A->tab,
      A->alloc * sizeof (struct bap_power_mpq), _struct_product_tab);
  for (i = 0; i < A->alloc; i++)
    ...
    enum ba0_garbage_code code)
{
  ...
  if (code == ba0_isolated)
    A = (bap_product_mpq) ba0_new_addr_gc_info (AA, _struct_product);
  else
    A = (bap_product_mpq) AA;
  ...
  A->tab = (struct bap_power_mpq *) ba0_new_addr_gc_info
    ...
    void *A)
{
  ...
  bap_set_product_mpq (B, (bap_product_mpq) A);
  return B;
}
...
bap_scanf_product_mpq (
    void *A)
{
  bap_product_mpq prod = (bap_product_mpq) A;
  bap_power_mpq P;
  ba0_list L;
  ba0_int_p i;
  bool negatif = false;
...
bap_printf_product_mpq (
    void *A)
{
  bap_product_mpq P = (bap_product_mpq) A;
  ba0_mpq_t bunk, *lc;
  ba0_int_p i;
  struct ba0_mark M;
