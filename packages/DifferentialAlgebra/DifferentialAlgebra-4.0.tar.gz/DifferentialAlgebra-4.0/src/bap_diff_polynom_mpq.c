...
#include "bap_diff_polynom_mpq.h"

#define BAD_FLAG_mpq

/*
 * texinfo: bap_is_constant_polynom_mpq
 * Return @code{true} if the derivative of @var{A} w.r.t. @var{s} is zero, else
 * @code{false}. If @var{s} is zero, then returns @code{true} if the derivatives 
 * of @var{A} w.r.t. all independent variables are zero, else @code{false}.
 */

BAP_DLL bool
bap_is_constant_polynom_mpq (
    bap_polynom_mpq A,
    bav_symbol s,
      ...
      for (i = 0; i < A->total_rank.size && is_constant; i++)
        {
          ...
          if (bav_symbol_type_variable (w) == bav_independent_symbol)
            is_constant = false;
          else if (P != (bav_tableof_parameter) 0 &&
...
/*
 * Returns true if A does not depend on dependent variables (i.e. does
 * not involve derivatives, unless they are parameters).
 */

BAP_DLL bool
bap_is_independent_polynom_mpq (
    bap_polynom_mpq A,
    bav_tableof_parameter P)
{
  bav_variable w;
...
/*
 * texinfo: bap_diff_polynom_mpq
 * Assign to @var{R} the polynomial obtained by differentiating @var{A}
 * w.r.t. @var{s}. Rewrites to zero any monomial involving a derivative of
 * any element of @var{nulles}.
 */

BAP_DLL void
bap_diff_polynom_mpq (
    bap_polynom_mpq R,
    bap_polynom_mpq A,
    bav_symbol s,
...
*/
  ...
  while (!bap_outof_itermon_mpq (&iter))
    {
      if (commuting_derivations)
        diff_monome_mpq (&R1, &rgtot, &iter, s, nulles);
/*
	else
	    diff_monome_comrel_mpq (&R1, &iter, s, nulles);
*/
    ...
    bav_tableof_variable nulles)
{
  ...
  bap_begin_creator_mpq (&crea, R, rgtot, bap_approx_total_rank, T.size);
/*
...
*/
          ...
          if (d > 1)
            {
              ba0_push_another_stack ();
              ba0_mpq_mul_ui (c, *bap_coeff_itermon_mpq (iter),
                  (unsigned long int) d);
              ba0_pull_stack ();
            ...
            bap_write_creator_mpq
                (&crea, &U, *bap_coeff_itermon_mpq (iter));
        }
...
/*
 * texinfo: bap_diff2_polynom_mpq
 * The term @var{theta} encode a derivation operator (every variable
 * should correspond to a derivation). 
 * Assigns to @var{R} the polynomial obtained by differentiating @var{A}
 * w.r.t. @var{theta}. Rewrites to zero any monomial involving a derivative of
 * any element of @var{nulles}.
 */

BAP_DLL void
bap_diff2_polynom_mpq (
    bap_polynom_mpq R,
    bap_polynom_mpq A,
    bav_term T,
    bav_tableof_variable nulles)
      ...
      for (i = 0; i < T->size; i++)
        {
          for (d = 0; d < T->rg[i].deg; d++)
            {
              bap_diff_polynom_mpq
                  (R, first ? A : R, T->rg[i].var->root, nulles);
              first = false;
            }
        }
    }
...
/*
 * texinfo: bap_involved_derivations_polynom_mpq
...
BAP_DLL void
bap_involved_derivations_polynom_mpq (
              ...
              if (v->order.tab[j] > 0)
                {
                  ...
                  if (!ba0_member_table (x, (ba0_table) T))
                    {
...
/*
 * texinfo: bap_zero_derivatives_of_tableof_parameter_mpq
...
BAP_DLL void
bap_zero_derivatives_of_tableof_parameter_mpq (
  ...
  while (i < nulles.size)
    {
      ...
      for (j = 0; j < T->size && !found; j++)
        {
          w = bap_leader_polynom_mpq (T->tab[j]);
          found = bav_is_derivative (v, w);
        }
      if (found)
        ba0_delete_table ((ba0_table) & nulles, i);
      else
        i += 1;
    }
/*
 * Append nulles to T.
 */
  ba0_realloc2_table ((ba0_table) T, T->size + nulles.size,
      (ba0_new_function *) & bap_new_polynom_mpq);
  for (i = 0; i < nulles.size; i++)
    {
      bap_set_polynom_variable_mpq (T->tab[T->size], nulles.tab[i], 1);
      T->size += 1;
    }
