...
ba0_compare_tab_elts (
    ...
    const void *b)
{
  ba0_gc_info *aa = (ba0_gc_info *) a;
  ba0_gc_info *bb = (ba0_gc_info *) b;
    ...
    ba0_int_p n)
{
  ...
  M = ba0_current;
  for (i = 0; i < n; i++)
    ba0_tab[i] = (ba0_gc_info) ba0_alloc_mark (&M, sizeof (struct ba0_gc_info));
}
...
/***********************************************************************
 ...
 Runs over the data structures to be garbaged.
    ...
    enum ba0_garbage_code code)
{
  ...
  switch (f->code)
    ...
    case ba0_list_format:
      n = ba0_garbage1_list (f->u.node.op->link[0], (ba0_list) objet, code);
      break;
    case ba0_table_format:
      n = ba0_garbage1_table (f->u.node.op->link[0], (ba0_table) objet, code);
      break;
    case ba0_array_format:
      n = ba0_garbage1_array (f->u.node.op->link[0], (ba0_array) objet, code);
      break;
    case ba0_matrix_format:
      n = ba0_garbage1_matrix (f->u.node.op->link[0], (ba0_matrix) objet, code);
      break;
    case ba0_value_format:
      n = ba0_garbage1_value (f->u.node.op->link[0], (ba0_value) objet, code);
      break;
    case ba0_point_format:
      n = ba0_garbage1_point (f->u.node.op->link[0], (ba0_point) objet, code);
      break;
    }
  return n;
}
...
/***********************************************************************
 ...
 Runs over the garbaged data structure in order to update internal pointers.
...
static ba0_list ba0_garbage2_list (
    ba0_subformat,
    ba0_list,
    enum ba0_garbage_code code);
static ba0_table ba0_garbage2_table (
    ba0_subformat,
    ba0_table,
    enum ba0_garbage_code code);
static ba0_array ba0_garbage2_array (
    ba0_subformat,
    ba0_array,
    enum ba0_garbage_code code);
static ba0_matrix ba0_garbage2_matrix (
    ba0_subformat,
    ba0_matrix,
    enum ba0_garbage_code code);
static ba0_value ba0_garbage2_value (
    ba0_subformat,
    ba0_value,
    enum ba0_garbage_code code);
static ba0_point ba0_garbage2_point (
    ba0_subformat,
    ba0_point,
    enum ba0_garbage_code code);
    ...
    enum ba0_garbage_code code)
{
  switch (f->code)
    ...
    case ba0_leaf_format:
      ...
      break;
    case ba0_list_format:
      objet = ba0_garbage2_list (f->u.node.op->link[0], (ba0_list) objet, code);
      break;
    case ba0_table_format:
      objet = ba0_garbage2_table
          (f->u.node.op->link[0], (ba0_table) objet, code);
      break;
    case ba0_array_format:
      objet = ba0_garbage2_array
          (f->u.node.op->link[0], (ba0_array) objet, code);
      break;
    case ba0_matrix_format:
      objet = ba0_garbage2_matrix
          (f->u.node.op->link[0], (ba0_matrix) objet, code);
      break;
    case ba0_value_format:
      objet = ba0_garbage2_value
          (f->u.node.op->link[0], (ba0_value) objet, code);
      break;
    case ba0_point_format:
      objet = ba0_garbage2_point
          (f->u.node.op->link[0], (ba0_point) objet, code);
      break;
    }
  return objet;
}
    ...
    enum ba0_garbage_code code)
{
  ...
  L = Head = (ba0_list) ba0_new_addr_gc_info (Head, _struct_list);
      ...
      if (L->next != (ba0_list) 0)
        {
          if (code == ba0_isolated)
            L = L->next =
                (ba0_list) ba0_new_addr_gc_info (L->next, _struct_list);
          else
            L = L->next;
        }
      else
    ...
    enum ba0_garbage_code code)
{
  ...
  if (code == ba0_isolated)
    T = (ba0_table) ba0_new_addr_gc_info (T, _struct_table);
    ...
    enum ba0_garbage_code code)
{
  ...
  if (code == ba0_isolated)
    point = (ba0_point) ba0_new_addr_gc_info (point, _struct_point);

  if (point->alloc > 0)
    {
      point->tab =
          (ba0_value *) ba0_new_addr_gc_info (point->tab, _struct_point_tab);
      for (i = 0; i < point->alloc; i++)
        point->tab[i] = (ba0_value) ba0_garbage2_value
...
{
  if (code == ba0_isolated)
    value = (ba0_value) ba0_new_addr_gc_info (value, _struct_value);
    ...
    enum ba0_garbage_code code)
{
  ...
  if (code == ba0_isolated)
    A = (ba0_array) ba0_new_addr_gc_info (A, _struct_array);
    ...
    enum ba0_garbage_code code)
{
  ...
  if (code == ba0_isolated)
    T = (ba0_matrix) ba0_new_addr_gc_info (T, _struct_matrix);
...
/*
   ...
   created with the relevant informations.
    ...
    char *text)
{
  ...
  if (ba0_user_provided_mark.index_in_cells < old_index_in_cells ||
    ...
    {
      g = (ba0_gc_info) ba0_alloc (sizeof (struct ba0_gc_info));
...
/*
  ...
  ba0_gc_info is read. Its new_addr field provides the new address of
    ...
    char *text)
{
  ...
  g = (ba0_gc_info) ba0_alloc_mark (&ba0_current, sizeof (struct ba0_gc_info));
...
/*
   step 2
*/
      ba0_tab = (ba0_gc_info *) ba0_alloc (sizeof (ba0_gc_info) * n);
      ba0_fill_tab (n);
      qsort (ba0_tab, n, sizeof (ba0_gc_info), &ba0_compare_tab_elts);
/*
   step 3
*/
