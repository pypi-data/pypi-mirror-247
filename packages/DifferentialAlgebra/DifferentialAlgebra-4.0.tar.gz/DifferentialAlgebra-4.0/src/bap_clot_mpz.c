...
/****************************************************************************
 bap_table2of_monom_mpz
 ****************************************************************************/

/*
   Allocate a bap_table2of_monom_mpz with a capacity of at least n monomials.
   Initialize the coefficients.
*/

static bap_table2of_monom_mpz
new_table2of_monom_mpz (
    bap_termanager tgest,
    ba0_int_p n)
{
  bap_table2of_monom_mpz mont;
  ba0_int_p i;

  mont =
      (bap_table2of_monom_mpz) ba0_alloc (sizeof (struct
          bap_table2of_monom_mpz));
  ba0_t2_alloc (sizeof (ba0_mpz_t), sizeof (bap_zipterm), n,
    ...
    bap_clot_mpz C)
{
  ...
  if (C->size > 0 && C->tab.tab == (bap_table2of_monom_mpz *) 0)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
  ...
  for (i = 0; i < C->tab.size; i++)
    {
      if (C->tab.tab[i] == (bap_table2of_monom_mpz) 0)
        BA0_RAISE_EXCEPTION (BA0_ERRALG);
  ...
  while (i < C->tab.alloc && C->tab.tab[i] != (bap_table2of_monom_mpz) 0)
    cumul_alloc += C->tab.tab[i++]->alloc;
...
/*
  ...
  The sub-arrays of bap_table2of_monom_mpz are shared.
*/
...
BAP_DLL void
bap_reverse_clot_mpz (
    bap_clot_mpz C)
{
  struct bap_itermon_clot_mpz I, J;
  ba0_int_p i;
  struct ba0_mark M;

  ba0_record (&M);
  bap_begin_itermon_clot_mpz (&I, C);
  bap_end_itermon_clot_mpz (&J, C);
  for (i = 0; i < C->size / 2; i++)
    {
      bap_swap_itermon_clot_mpz (&I, &J);
      bap_next_itermon_clot_mpz (&I);
      bap_prev_itermon_clot_mpz (&J);
    }
  ba0_restore (&M);
}

/*
 * texinfo: bap_change_ordering_clot_mpz
 * Assign @var{r} to the field @code{ordering} of @var{C}.
 * This low-level function is used by @code{bap_physort_polynom_mpz} only.
 */

BAP_DLL void
bap_change_ordering_clot_mpz (
    bap_clot_mpz C,
    bav_Iordering r)
{
  C->ordering = r;
...
/*
 * texinfo: bap_end_itermon_clot_mpz
 * Set @var{iter} on the last monomial of @var{clot}.
 * If @var{clot} is empty, @var{iter} is set outside the @var{clot}.
 */

BAP_DLL void
bap_end_itermon_clot_mpz (
    bap_itermon_clot_mpz iter,
    bap_clot_mpz clot)
{
  check_clot_mpz (clot);
  iter->clot = clot;
  if (bap_is_zero_clot_mpz (clot))
    {
      iter->num.primary = -1;
      iter->num.combined = -1;
    }
...
/*
 * texinfo: bap_outof_itermon_clot_mpz
 * Return @code{true} if @var{iter} is set outside its clot.
 */

BAP_DLL bool
bap_outof_itermon_clot_mpz (
    bap_itermon_clot_mpz iter)
{
...
/*
 * texinfo: bap_next_itermon_clot_mpz
...
BAP_DLL void
bap_next_itermon_clot_mpz (
    bap_itermon_clot_mpz iter)
{
  bap_table2of_monom_mpz m;

  if (bap_outof_itermon_clot_mpz (iter))
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
...
/*
 * texinfo: bap_prev_itermon_clot_mpz
...
/*
 * texinfo: bap_number_itermon_clot_mpz
 * Return the number of the current monomial of @var{iter}.
 */

BAP_DLL ba0_int_p
bap_number_itermon_clot_mpz (
    bap_itermon_clot_mpz iter)
{
...
/*
 * texinfo: bap_term_itermon_clot_mpz
...
/*
 * texinfo: bap_coeff_itermon_clot_mpz
...
/*
 * texinfo: bap_swap_itermon_clot_mpz
...
BAP_DLL void
bap_swap_itermon_clot_mpz (
    bap_itermon_clot_mpz I,
    bap_itermon_clot_mpz J)
{
  bap_table2of_monom_mpz mi, mj;
  if (bap_outof_itermon_clot_mpz (I) || bap_outof_itermon_clot_mpz (J))
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
  ...
  ba0_mpz_swap (mi->coeff[I->num.secondary], mj->coeff[J->num.secondary]);
}

/*
 * texinfo: bap_goto_itermon_clot_mpz
...
BAP_DLL void
bap_goto_itermon_clot_mpz (
    bap_itermon_clot_mpz iter,
    ba0_int_p numero)
{
  bap_table2of_monom_mpz *tab;
  ba0_int_p primary, secondary;

  if (numero < 0 || numero >= iter->clot->size)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);

  tab = (bap_table2of_monom_mpz *) iter->clot->tab.tab;
  primary = 0;
  secondary = numero;
...
/*
 * texinfo: bap_begin_creator_clot_mpz
 ...
 * If some new arrays of type @code{bap_table2of_monom_mpz} are
 * required during the creation process, they will have 
 * @var{table2of_monom_alloc} entries.
 */

BAP_DLL void
bap_begin_creator_clot_mpz (
...
/*
 * texinfo: bap_append_creator_clot_mpz
...
/*
  Returns the bap_table2of_monom_mpz crea points to.
  Allocates a bap_table2of_monom_mpz if there is no place left.
  If as many used monomials as allocated monomials
     one needs to allocate a new mont_mpz
     Now, if tab.tab is full then it must be
        resized before allocating the mont_mpz.
  else
     there is no need to allocated a new mont_mpz
     it may be useful to move to the next mont_mpz.
*/

static bap_table2of_monom_mpz
mont_address_creator_clot_mpz (
    bap_creator_clot_mpz crea)
{
  bap_clot_mpz clot;
  bap_table2of_monom_mpz mont;
  ...
  if (clot->alloc == crea->iter.num.combined)
    {
      if (clot->tab.alloc == crea->iter.num.primary)
        {
          bap_table2of_monom_mpz *new_tab;
          ba0_int_p n, new_alloc;

          new_alloc = 2 * clot->tab.alloc + 1;
          new_tab =
              (bap_table2of_monom_mpz *) ba0_alloc (sizeof (void *)
              * new_alloc);
          memcpy (new_tab, clot->tab.tab,
              clot->tab.alloc * sizeof (bap_table2of_monom_mpz));
          for (n = clot->tab.alloc; n < new_alloc; n++)
            new_tab[n] = (bap_table2of_monom_mpz) 0;
          clot->tab.alloc = new_alloc;
          clot->tab.tab = (bap_table2of_monom_mpz *) new_tab;
        }
      mont =
          new_table2of_monom_mpz (&clot->tgest, crea->table2of_monom_alloc);
...
/*
   There is a slight difference with bap_next_itermon_clot_mpz (comparison
   with alloc instead of size though it amounts to the same). There is no
   overflow test.
*/

static void
next_creator_clot_mpz (
    bap_creator_clot_mpz crea)
{
  bap_table2of_monom_mpz mont;
...
/*
 * texinfo: bap_write_creator_clot_mpz
    ...
    ba0_mpz_t c)
{
  ...
  mont = mont_address_creator_clot_mpz (crea);

  bap_set_zipterm_term_termanager
      (&crea->iter.clot->tgest, &mont->zipterm[crea->iter.num.secondary], T);
  ba0_mpz_set (mont->coeff[crea->iter.num.secondary], c);

  next_creator_clot_mpz (crea);
}

/*
 * texinfo: bap_write_neg_creator_clot_mpz
 * Write the monomial @math{-c\,T} in the clot processed by @var{crea}.
 * The coefficient @var{c} is allowed to be zero (then nothing is done).
 * The creator then moves to the next available monomial entry.
 */
...
BAP_DLL void
bap_write_neg_creator_clot_mpz (
    bap_creator_clot_mpz crea,
    bav_term T,
    ba0_mpz_t c)
{
  ...
  bap_set_zipterm_term_termanager
      (&crea->iter.clot->tgest, &mont->zipterm[crea->iter.num.secondary], T);
  ba0_mpz_neg (mont->coeff[crea->iter.num.secondary], c);

  next_creator_clot_mpz (crea);
}

/*
 * texinfo: bap_write_term_creator_clot_mpz
...
BAP_DLL void
bap_write_term_creator_clot_mpz (
    bap_creator_clot_mpz crea,
    bav_term T)
{
  bap_table2of_monom_mpz mont;
    ...
    ba0_int_p r)
{
  ...
  bap_table2of_monom_mpz d_mont, s_mont;
  ...
  bap_begin_itermon_clot_mpz (&iter, clot);
  bap_goto_itermon_clot_mpz (&iter, l);

  for (i = l; i < r; i++)
    {
      s_mont = iter.clot->tab.tab[iter.num.primary];
      s_clef = s_mont->zipterm[iter.num.secondary];
      d_mont = mont_address_creator_clot_mpz (crea);
      ...
      ba0_mpz_set (d_mont->coeff[crea->iter.num.secondary],
...
/* 
 * texinfo: bap_write_neg_all_creator_clot_mpz
...
BAP_DLL void
bap_write_neg_all_creator_clot_mpz (
    bap_creator_clot_mpz crea,
    bap_clot_mpz clot,
    ba0_int_p l,
    ba0_int_p r)
  ...
  for (i = l; i < r; i++)
    {
      ...
      bap_set_zipterm_zipterm_termanager
          (&crea->iter.clot->tgest, d_clef, &iter.clot->tgest, s_clef);
      ba0_mpz_neg (d_mont->coeff[crea->iter.num.secondary],
          s_mont->coeff[iter.num.secondary]);
      bap_next_itermon_clot_mpz (&iter);
      next_creator_clot_mpz (crea);
    }
...
BAP_DLL void
bap_write_mul_all_creator_clot_mpz (
    bap_creator_clot_mpz crea,
    bap_clot_mpz clot,
    ba0_mpz_t c,
    ba0_int_p l,
  ...
  for (i = l; i < r; i++)
    {
      ...
      bap_set_zipterm_zipterm_termanager
          (&crea->iter.clot->tgest, d_clef, &iter.clot->tgest, s_clef);
      ba0_mpz_mul (d_mont->coeff[crea->iter.num.secondary],
          s_mont->coeff[iter.num.secondary], c);
      if (ba0_mpz_is_zero (d_mont->coeff[crea->iter.num.secondary]))
        BA0_RAISE_EXCEPTION (BA0_ERRALG);
      bap_next_itermon_clot_mpz (&iter);
      next_creator_clot_mpz (crea);
    }
...
BAP_DLL void
bap_goto_creator_clot_mpz (
    bap_creator_clot_mpz crea,
    ba0_int_p numero)
{
  bap_goto_itermon_clot_mpz (&crea->iter, numero);
}
...
{
  struct bap_itermon_clot_mpz l;
  struct bap_itermon_clot_mpz r;
  struct bap_itermon_clot_mpz i;
  struct bap_itermon_clot_mpz j;
  struct bap_itermon_clot_mpz k;
  struct bav_term Tl;
  struct bav_term Tr;
  struct bav_term Ti;
  struct bav_term Tj;
...
static void
bap_init_quicksort_data_mpz (
    struct quicksort_data *qs,
    bap_clot_mpz clot)
{
  bap_begin_itermon_clot_mpz (&qs->l, clot);
  bap_begin_itermon_clot_mpz (&qs->r, clot);
  bap_begin_itermon_clot_mpz (&qs->i, clot);
  bap_begin_itermon_clot_mpz (&qs->j, clot);
  bap_begin_itermon_clot_mpz (&qs->k, clot);
  bav_init_term (&qs->Tl);
  bav_init_term (&qs->Tr);
  bav_init_term (&qs->Ti);
  bav_init_term (&qs->Tj);
  ...
  if (r - l > 8)
    {
      ...
      if (i == j || i == k || j == k)
        BA0_RAISE_EXCEPTION (BA0_ERRALG);
      bap_goto_itermon_clot_mpz (&qs->l, l);
      bap_term_itermon_clot_mpz (&qs->Tl, &qs->l);
      bap_goto_itermon_clot_mpz (&qs->r, r);
      bap_term_itermon_clot_mpz (&qs->Tr, &qs->r);
      bap_goto_itermon_clot_mpz (&qs->i, i);
      bap_term_itermon_clot_mpz (&qs->Ti, &qs->i);
      bap_goto_itermon_clot_mpz (&qs->j, j);
      bap_term_itermon_clot_mpz (&qs->Tj, &qs->j);
      bap_goto_itermon_clot_mpz (&qs->k, k);
      bap_term_itermon_clot_mpz (&qs->Tk, &qs->k);
/*
      ...
      if (code == ba0_gt)
        {
          ...
          if (code == ba0_gt)
            {
              bap_swap_itermon_clot_mpz (&qs->l, &qs->j);
              bap_swap_itermon_clot_mpz (&qs->r, &qs->i);
/* T [i] > T [j] > T [k] */
          ...
          else
            {
              ...
              if (code == ba0_gt)
                {
                  bap_swap_itermon_clot_mpz (&qs->l, &qs->k);
                  bap_swap_itermon_clot_mpz (&qs->r, &qs->i);
/* T [i] > T [k] > T [j] */
                ...
                {
                  bap_swap_itermon_clot_mpz (&qs->l, &qs->i);
                  bap_swap_itermon_clot_mpz (&qs->r, &qs->k);
/* T [k] > T [i] > T [j] */
...
/* T [k] > T [j] > T [i] */
          ...
          else
            {
              ...
              if (code == ba0_gt)
                {
                  bap_swap_itermon_clot_mpz (&qs->l, &qs->k);
                  bap_swap_itermon_clot_mpz (&qs->r, &qs->j);
/* T [j] > T [k] > T [i] */
                  bav_set_term (&qs->pivot, &qs->Tk);
                }
              else
...
*/
      ...
      do
        {
          bap_swap_itermon_clot_mpz (&qs->i, &qs->j);
          do
            {
              i += 1;
              bap_next_itermon_clot_mpz (&qs->i);
              bap_term_itermon_clot_mpz (&qs->Ti, &qs->i);
          ...
          do
            {
              j -= 1;
              bap_prev_itermon_clot_mpz (&qs->j);
              bap_term_itermon_clot_mpz (&qs->Tj, &qs->j);
      ...
      while (i < j);
      bap_swap_itermon_clot_mpz (&qs->i, &qs->r);
      quicksort_clot_mpz (l, i - 1, qs);
      quicksort_clot_mpz (i + 1, r, qs);
    }
  else
    {
      bool sorted;
      ...
      for (i = r - 1; !sorted && i >= l; i--)
        {
          sorted = true;
          bap_goto_itermon_clot_mpz (&qs->i, l);
          bap_term_itermon_clot_mpz (cour, &qs->i);
          bap_goto_itermon_clot_mpz (&qs->j, l + 1);
          bap_term_itermon_clot_mpz (suiv, &qs->j);
          ...
          for (j = l + 1; j <= i; j++)
            {
              qs->i = qs->j;
              bap_next_itermon_clot_mpz (&qs->j);
              BA0_SWAP (bav_term, cour, suiv);
              bap_term_itermon_clot_mpz (suiv, &qs->j);
              code = bav_compare_term (cour, suiv);
              if (code == ba0_lt)
 ...
 * This low-level function is used by @code{bap_physort_polynom_mpz}.
 */
    ...
    ba0_int_p r)
{
  ...
  init_set_clot_mpz (&old_clot, clot);
  bap_begin_itermon_clot_mpz (&iter, &old_clot);
  bap_goto_itermon_clot_mpz (&iter, l);

  bav_init_term (&T);
  bav_set_term (&T, &clot->tgest.total_rank);
  bav_sort_term (&T);
  bap_begin_creator_clot_mpz (&crea, clot, &T, 0);

  bap_goto_creator_clot_mpz (&crea, l);

  for (i = l; i < r; i++)
    {
      bap_term_itermon_clot_mpz (&T, &iter);
      bav_sort_term (&T);
      bap_write_term_creator_clot_mpz (&crea, &T);
      bap_next_itermon_clot_mpz (&iter);
    }

  bap_init_quicksort_data_mpz (&qs, clot);
  quicksort_clot_mpz (l, r - 1, &qs);
  ba0_restore (&M);
}
...
static char _struct_mont[] = "struct bap_table2of_monom_mpz";
static char _struct_table2of_monom_zipterm[] =
    "bap_table2of_monom_mpz->zipterm";
static char _struct_table2of_monom_coeff[] = "bap_table2of_monom_mpz->coeff";

static ba0_int_p
garbage1_table2of_monom_mpz (
    bap_termanager tgest,
    bap_table2of_monom_mpz A,
    enum ba0_garbage_code code)
{
  ba0_int_p n = 0;

  if (A == (bap_table2of_monom_mpz) 0)
    return 0;

  if (code == ba0_isolated)
    n += ba0_new_gc_info
        (A, sizeof (struct bap_table2of_monom_mpz), _struct_mont);
  n += ba0_new_gc_info
      (A->zipterm, sizeof (bap_zipterm) * A->alloc,
      _struct_table2of_monom_zipterm);
  n += ba0_new_gc_info (A->coeff, sizeof (ba0_mpz_t) * A->alloc,
...
#if ! defined (BAD_FLAG_mint_hp)
  {
    ba0_int_p i;
    for (i = 0; i < A->alloc; i++)
      n += ba0_garbage1_mpz (&A->coeff[i], ba0_embedded);
  }
#endif
  return n;
}
...
static char _struct_clot[] = "struct bap_clot_mpz";
static char _struct_clot_tab[] = "bap_clot_mpz->tab.tab";

BAP_DLL ba0_int_p
bap_garbage1_clot_mpz (
    void *AA,
    enum ba0_garbage_code code)
{
  bap_clot_mpz A = (bap_clot_mpz) AA;
  ba0_int_p i, n = 0;

  if (code == ba0_isolated)
    n += ba0_new_gc_info (A, sizeof (struct bap_clot_mpz), _struct_clot);

  if (A->tab.tab)
    n += ba0_new_gc_info
        (A->tab.tab, sizeof (bap_table2of_monom_mpz) * A->tab.alloc,
        _struct_clot_tab);

  n += bap_garbage1_termanager (&A->tgest, ba0_embedded);

  for (i = 0; i < A->tab.alloc; i++)
    n += garbage1_table2of_monom_mpz (&A->tgest, A->tab.tab[i], ba0_isolated);

  return n;
}

static bap_table2of_monom_mpz
garbage2_table2of_monom_mpz (
    bap_termanager tgest,
    bap_table2of_monom_mpz A,
    enum ba0_garbage_code code)
{
  ...
  if (code == ba0_isolated)
    A = (bap_table2of_monom_mpz) ba0_new_addr_gc_info (A, _struct_mont);

  A->zipterm = (bap_zipterm *) ba0_new_addr_gc_info
      (A->zipterm, _struct_table2of_monom_zipterm);

  A->coeff = (ba0_mpz_t *) ba0_new_addr_gc_info
...
#if ! defined (BAD_FLAG_mint_hp)
  {
    ba0_int_p i;
    for (i = 0; i < A->alloc; i++)
      ba0_garbage2_mpz (&A->coeff[i], ba0_embedded);
  }
#endif
  return A;
}
    ...
    enum ba0_garbage_code code)
{
  ...
  if (code == ba0_isolated)
    A = (bap_clot_mpz) ba0_new_addr_gc_info (AA, _struct_clot);
  else
    A = (bap_clot_mpz) AA;

  if (A->tab.tab)
    A->tab.tab = (bap_table2of_monom_mpz *) ba0_new_addr_gc_info
        (A->tab.tab, _struct_clot_tab);

  bap_garbage2_termanager (&A->tgest, ba0_embedded);

  for (i = 0; i < A->tab.alloc; i++)
    A->tab.tab[i] = garbage2_table2of_monom_mpz
