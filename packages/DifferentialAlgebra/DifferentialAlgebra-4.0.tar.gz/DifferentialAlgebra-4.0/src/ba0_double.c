 ...
 * Allocate a @code{double} in the current stack and returns its address.
 */
    ...
    void *z)
{
  ...
  ba0_get_settings_analex (&nbtokens);
  if (nbtokens < 10)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);

  ba0_init_table ((ba0_table) & tokseq);
  ba0_push_another_stack ();
  ba0_record (&M);
  ba0_realloc_table ((ba0_table) & tokseq, 10);
  ba0_pull_stack ();
