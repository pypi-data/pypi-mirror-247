    ...
    bad_regchain C)
{
  ...
  bap_product_mpz H;
  bap_polynom_mpz U_1, U_3, V_1, V_3, Q, R, S;
  ba0_int_p counter;
  struct ba0_mark M;
 ...
 * R = R / Q
 */
      BA0_SWAP (bap_polynom_mpz, U_1, V_1);
      if (counter < 2)
        {
          U_3 = V_3;
        ...
        BA0_SWAP (bap_polynom_mpz, U_3, V_3);
/*
 * A bit complicated stuff in order not to override A0 and B0
 */
      BA0_SWAP (bap_polynom_mpz, V_1, S);
      BA0_SWAP (bap_polynom_mpz, V_3, R);
/*
 * U_1 = V_1
 * U_3 = V_3
 * V_1 = S
 ...
 */
      if (bap_is_independent_polynom_mpz (U3, (bav_tableof_parameter) 0))
        break;
    ...
    bap_polynom_mpz *ddz)
{
  ...
  volatile bap_polynom_mpz ddzbar;
  volatile bap_polynom_mpz *_ddzbar;
  bav_Iordering r;
 ...
 */
  ...
  _ddzbar = ddz == (bap_polynom_mpz *) 0 ?
      (volatile bap_polynom_mpz *) 0 : (volatile bap_polynom_mpz *) &ddzbar;
 ...
 */
            bad_algebraic_invert_polynom_mod_regchain_zero_dimensional
                (&U1, &U3, &Abar.tab[i].factor, &Cbar,
                (bap_polynom_mpz *) _ddzbar);
          }
          BA0_CATCH
          {
/*
 ...
 */
            bav_R_pull_ordering ();
            bap_sort_polynom_mpz (ddzbar, ddzbar);
/*
 ...
 */
            ...
            bap_initial_polynom_mpz (&init_ddzbar, ddzbar);
            bad_check_regularity_polynom_mod_regchain (&init_ddzbar, C, K, ddz);
/*
 * re-raise the BAD_EXRDDZ exception
 */
            *ddz = bap_new_polynom_mpz ();
            bap_set_polynom_mpz (*ddz, ddzbar);
            BA0_RAISE_EXCEPTION2 (BAD_EXRDDZ, "%Az", (void **) ddz);
          }
          BA0_ENDTRY;
/*
    ...
    bap_polynom_mpz *ddz)
{
  ...
  bad_reduce_polynom_by_regchain
      (&P, &H, A, C, bad_full_reduction, bad_all_derivatives_to_reduce);
  ...
  if (bad_defines_a_differential_ideal_regchain (C))
  ...
  else
    {
      if (bad_is_a_reduced_to_zero_polynom_by_regchain
          (A, C, bad_algebraic_reduction))
        BA0_RAISE_EXCEPTION (BAD_EXRNUL);

      ba0_push_another_stack ();
      ba0_record (&M);
      bap_init_product_mpz (&P);
      baz_factor_easy_polynom_mpz (&P, A, (bap_listof_polynom_mpz) 0);
      ba0_pull_stack ();
      bad_algebraic_invert_product_mod_regchain (U, G, &P, C, K, ddz);
      ba0_restore (&M);
    }
  baz_gcd_product_mpz ((bap_product_mpz) 0, U, G, U, G);
  ...
  for (i = 0; i < A->size; i++)
    {
      if (A->tab[i].exponent > 0)
        {
          bad_invert_polynom_mod_regchain
              (&Uhat, &Ghat, &A->tab[i].factor, C, K, ddz);
    ...
    bad_regchain C)
{
  ...
  ba0_realloc2_table
      ((ba0_table) & T, 2, (ba0_new_function *) & bap_new_polynom_mpz);
  T.size = 2;
      ...
      if (bap_depend_polynom_mpz (first ? A : Gbar, v))
        {
          ...
          first = false;
          BA0_SWAP (bap_polynom_mpz, T.tab[0], Gbar);
