  ...
  if (tgest->total_rank.alloc > 0)
    {
      tgest->zipping.nbbits =
          (unsigned char *) ba0_alloc
          (sizeof (unsigned char) * tgest->total_rank.alloc);
      tgest->zipping.mask =
          (unsigned ba0_int_p *) ba0_alloc
          (sizeof (unsigned ba0_int_p) * tgest->total_rank.alloc);
    }
    ...
    bool *reinit)
{
  ...
  if (tgest->total_rank.alloc > old_total_rank_alloc)
    {
      tgest->zipping.nbbits = (unsigned char *) ba0_alloc
          (sizeof (unsigned char) * tgest->total_rank.alloc);
      tgest->zipping.mask = (unsigned ba0_int_p *) ba0_alloc
          (sizeof (unsigned ba0_int_p) * tgest->total_rank.alloc);
    }
  compute_zipping (tgest);
    ...
    ba0_int_p n)
{
  ...
  if (tgest->zipping.alloc > 1)
    for (i = 0; i < n; i++)
      tab[i] = (bap_zipterm) ba0_alloc
          (sizeof (ba0_int_p) * tgest->zipping.alloc);
}
...
bap_set_zipterm_zipterm_termanager (
    bap_termanager dst_tgest,
    bap_zipterm *dst_zipterm,
    bap_termanager src_tgest,
...
*/
  if (!bav_is_factor_term (&tgest->total_rank, T, (bav_term) 0))
...
*/
  tab = tgest->zipping.alloc <= 1 ?
      (ba0_int_p *) (void *) zipterm : (ba0_int_p *) (void *) *zipterm;
  i = j = k = 0;
  while (j < T->size)
    {
      m = 0;
...
*/
  tab = tgest->zipping.alloc <= 1 ?
      (ba0_int_p *) & zipterm : (ba0_int_p *) (void *) zipterm;
  i = j = k = 0;
...
static char _termanager[] = "bap_termanager->total_rank.rg";
static char _termanager_nbbits[] = "bap_termanager->zipping.nbbits";
static char _termanager_mask[] = "bap_termanager->zipping.mask";
...
{
  bap_termanager T = (bap_termanager) TT;
  ba0_int_p n = 0;
  ...
  if (T->total_rank.alloc > 0)
    {
      ...
      n += ba0_new_gc_info
          (T->zipping.nbbits,
          ba0_ceil_align (sizeof (unsigned char) *
              T->total_rank.alloc), _termanager_nbbits);
    ...
    enum ba0_garbage_code code)
{
  ...
  if (T->total_rank.alloc > 0)
    {
      T->total_rank.rg = (struct bav_rank *) ba0_new_addr_gc_info
          (T->total_rank.rg, _termanager);
      T->zipping.nbbits = (unsigned char *) ba0_new_addr_gc_info
          (T->zipping.nbbits, _termanager_nbbits);
      T->zipping.mask = (unsigned ba0_int_p *) ba0_new_addr_gc_info
          (T->zipping.mask, _termanager_mask);
    }
  return (void *) 0;
}
    ...
    enum ba0_garbage_code code)
{
  ...
  if (tgest->zipping.alloc <= 1)
    BA0_RAISE_EXCEPTION (BA0_ERRALG);
  n += ba0_new_gc_info
      ((void *) key, sizeof (ba0_int_p) * tgest->zipping.alloc, _zipterm);
  return n;
}
