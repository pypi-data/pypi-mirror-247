...
bai_reset_parameters (
    struct bai_parameters *params)
{
  ba0_table names = (ba0_table) params->names;
...
{
  ba0_table Pnames, Qnames;

  if (P != Q)
    {
      Pnames = (ba0_table) P->names;
      Qnames = (ba0_table) Q->names;
...
bai_set_parameters_tableof_variable (
    struct bai_parameters *params,
    bav_tableof_variable idents)
{
  bai_realloc_parameters (params, idents->size);
  ba0_set_table ((ba0_table) params->names, (ba0_table) idents);
}
...
bai_reset_commands (
    struct bai_commands *commands)
{
  ba0_table names = (ba0_table) commands->names;
  ...
  if (P != Q)
    {
      ...
      if (Pnames->alloc < Qnames->alloc)
        {
          P->fptrs = (bai_command_function **) ba0_alloc
              (sizeof (bai_command_function *) * Qnames->alloc);
        }
    ...
    ba0_int_p n)
{
  ...
  names = (ba0_table) commands->names;
  if (names->alloc < n)
    {
      new_fptrs = (bai_command_function **) ba0_alloc
          (sizeof (bai_command_function *) * n);
    ...
    bav_tableof_variable idents)
{
  ...
  ba0_set_table ((ba0_table) commands->names, (ba0_table) idents);
}
