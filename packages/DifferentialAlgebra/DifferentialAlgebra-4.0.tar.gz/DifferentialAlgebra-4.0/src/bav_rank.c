...
bav_init_rank (
    bav_rank rg)
{
  rg = (bav_rank) 0;            /* to avoid an annoying warning */
}
    ...
    void)
{
  ...
  rg = (bav_rank) ba0_alloc (sizeof (struct bav_rank));
  bav_init_rank (rg);
  return rg;
}
 ...
 * w.r.t. the current ordering and, in case of equality,
 * the degrees.
 */
    ...
    bav_rank sg)
{
  ...
  if (bav_is_zero_rank (sg) || bav_is_constant_rank (sg))
    return false;
  nr = bav_R_variable_number (rg->var);
  ns = bav_R_variable_number (sg->var);
  if (nr < ns)
    return true;
  else if (nr == ns)
...
/*
 * texinfo: bav_inv_comp_rank
 * A comparison function used by @code{qsort} for sorting tables
 * of rank by increasing order.
 */
...
bav_inv_comp_rank (
    ...
    const void *y)
{
  bav_rank r = (bav_rank) x;
  bav_rank s = (bav_rank) y;
  bav_Inumber n, m;
