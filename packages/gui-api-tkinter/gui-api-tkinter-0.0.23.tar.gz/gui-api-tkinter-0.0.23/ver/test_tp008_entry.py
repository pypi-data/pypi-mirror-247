...
class TestTp008(unittest.TestCase):
    ...
    def test_tp008(self):  # pylint: disable=too-many-statements
        pth.proto.protocol('tp-008', 'check "set_text" function on Entry widget')
        ...
        widget_path = ['window1', 'page1_frame', 'button_frame', 'entry1']
        ...
        pth.proto.step('verify the content of the Entry widget has changed')
        item = svc.th.search(widget_path)
        pth.ver.verify_equal('Entry', item['class'])
        pth.ver.verify_equal('entry1', item['name'])
        pth.ver.verify_equal('entry1abcd', item['value'], reqids=['SRS-092', 'SRS-091'])
        pth.ver.verify_equal('normal', item['state'])

        # === set_text()
        msg += 'e'
        pth.proto.step('set_text() on Entry widget using search path')
        ...
        svc.th.get_screen()
        item = svc.th.search(widget_path)
        pth.ver.verify_equal('entry1abcde', item['value'], reqids=['SRS-092', 'SRS-091'])

        # === set_text_at()
        msg += 'f\ng'
        pth.proto.step('set_text_at() on Entry widget using raw x, y coordinates')
        ...
        pth.proto.step('verify the contents of the Entry widget have changed to "abcdefg"')
        svc.th.get_screen()
        item = svc.th.search(widget_path)
        # Note: newlines are ignored
        pth.ver.verify_equal('entry1abcdefg', item['value'], reqids=['SRS-092', 'SRS-091'])

        # === clear text
        pth.proto.step('enter an empty string into the Entry widget')
        ack_nak = svc.th.set_text_at(x, y, '')
        pth.ver.verify_equal('ack', ack_nak['value'])

        pth.proto.step('verify the contents of the Entry widget have changed to ""')
        svc.th.get_screen()
        item = svc.th.search(widget_path)
        pth.ver.verify_equal('', item['value'], reqids=['SRS-092', 'SRS-091'])
