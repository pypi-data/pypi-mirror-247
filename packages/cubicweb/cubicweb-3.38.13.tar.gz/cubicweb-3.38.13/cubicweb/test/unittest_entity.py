...
class EntityTC(CubicWebTC):
    ...
    def test_copy(self):
        with self.admin_access.web_request() as req:
            req.create_entity("Tag", name="x")
            p = req.create_entity("Personne", nom="toto")
            oe = req.create_entity("Note", type="x")
            req.execute(
                "SET T ecrit_par U WHERE T eid %(t)s, U eid %(u)s",
                {"t": oe.eid, "u": p.eid},
            )
            req.execute("SET TAG tags X WHERE X eid %(x)s", {"x": oe.eid})
            e = req.create_entity("Note", type="z")
    ...
    def test_copy_with_composite(self):
        with self.admin_access.web_request() as req:
            ...
            e = req.execute("Any X WHERE X eid %(x)s", {"x": req.user.eid}).get_entity(
                0, 0
            )
    ...
    def test_related_cache_both(self):
        with self.admin_access.web_request() as req:
            user = req.execute(
                "Any X WHERE X eid %(x)s", {"x": req.user.eid}
            ).get_entity(0, 0)
            req.execute(
                'INSERT EmailAddress X: X address "toto@logilab.org", U use_email X WHERE U login "admin"'
            )[0][0]
    ...
    def test_related_limit(self):
        with self.admin_access.web_request() as req:
            p = req.create_entity("Personne", nom="di mascio", prenom="adrien")
            for tag in "abcd":
    ...
    def test_related_targettypes(self):
        with self.admin_access.web_request() as req:
            p = req.create_entity("Personne", nom="Loxodonta", prenom="Babar")
            n = req.create_entity("Note", type="scratch", ecrit_par=p)
            t = req.create_entity("Tag", name="a tag", tags=(p, n))
            req.cnx.commit()
        with self.admin_access.web_request() as req:
    ...
    def test_cw_instantiate_relation(self):
        with self.admin_access.web_request() as req:
            p1 = req.create_entity("Personne", nom="di")
            p2 = req.create_entity("Personne", nom="mascio")
            t = req.create_entity("Tag", name="t0", tags=[])
            self.assertCountEqual(t.tags, [])
            t = req.create_entity("Tag", name="t1", tags=p1)
            self.assertCountEqual(t.tags, [p1])
            t = req.create_entity("Tag", name="t2", tags=p1.eid)
            self.assertCountEqual(t.tags, [p1])
            t = req.create_entity("Tag", name="t3", tags=[p1, p2.eid])
    ...
    def test_cw_instantiate_reverse_relation(self):
        with self.admin_access.web_request() as req:
            t1 = req.create_entity("Tag", name="t1")
            t2 = req.create_entity("Tag", name="t2")
            p = req.create_entity("Personne", nom="di mascio", reverse_tags=t1)
            self.assertCountEqual(p.reverse_tags, [t1])
            p = req.create_entity("Personne", nom="di mascio", reverse_tags=t1.eid)
            self.assertCountEqual(p.reverse_tags, [t1])
            p = req.create_entity(
    ...
    def test_related_rql_base(self):
        ...
        with self.admin_access.web_request() as req:
            p = req.create_entity("Personne", nom="pouet")
            self.assertEqual(
    ...
    def test_unrelated_rql_security_2(self):
        with self.admin_access.web_request() as req:
            email = req.execute('INSERT EmailAddress X: X address "hop"').get_entity(
                0, 0
            )
    ...
    def test_unrelated_rql_constraints_edition_subject(self):
        with self.admin_access.web_request() as req:
            person = req.create_entity("Personne", nom="sylvain")
            rql = person.cw_unrelated_rql("connait", "Personne", "subject")[0]
            self.assertEqual(
                rql,
                "Any O,AA,AB,AC ORDERBY AA DESC WHERE "
    ...
    def test_unrelated_rql_s_linkto_s(self):
        with self.admin_access.web_request() as req:
            ...
            soc = req.create_entity("Societe", nom="logilab")
    ...
    def test_unrelated_rql_o_linkto_s(self):
        with self.admin_access.web_request() as req:
            ...
            self.vreg["etypes"].etype_class("Personne").fetch_attrs = ()
            person = req.create_entity("Personne", nom="florent")
    ...
    def test_unrelated_rql_s_linkto_s_no_info(self):
        with self.admin_access.web_request() as req:
            person = self.vreg["etypes"].etype_class("Personne")(req)
            self.vreg["etypes"].etype_class("Personne").fetch_attrs = ()
            req.create_entity("Societe", nom="logilab")
    ...
    def test_unrelated_rql_s_linkto_s_unused_info(self):
        with self.admin_access.web_request() as req:
            person = self.vreg["etypes"].etype_class("Personne")(req)
            self.vreg["etypes"].etype_class("Personne").fetch_attrs = ()
            other_p = req.create_entity("Personne", nom="titi")
    ...
    def test_unrelated_base(self):
        with self.admin_access.web_request() as req:
            p = req.create_entity("Personne", nom="di mascio", prenom="adrien")
            e = req.create_entity("Tag", name="x")
    ...
    def test_unrelated_limit(self):
        with self.admin_access.web_request() as req:
            e = req.create_entity("Tag", name="x")
            req.create_entity("Personne", nom="di mascio", prenom="adrien")
            req.create_entity("Personne", nom="thenault", prenom="sylvain")
            self.assertEqual(
...
du :eid:`1:*ReST*`"""
    ...
    def test_nonregr_relation_cache(self):
        with self.admin_access.web_request() as req:
            p1 = req.create_entity("Personne", nom="di mascio", prenom="adrien")
            req.create_entity("Personne", nom="toto")
    ...
    def test_cw_set_relations(self):
        with self.admin_access.web_request() as req:
            person = req.create_entity("Personne", nom="chauvat", prenom="nicolas")
            note = req.create_entity("Note", type="x")
            note.cw_set(ecrit_par=person)
            note = req.create_entity("Note", type="y")
            note.cw_set(ecrit_par=person.eid)
            self.assertEqual(len(person.reverse_ecrit_par), 2)

    def test_absolute_url_empty_field(self):
        with self.admin_access.web_request() as req:
            card = req.create_entity("Card", wikiid="", title="test")
    ...
    def test_create_and_compare_entity(self):
        ...
        with access.web_request() as req:
            p1 = req.create_entity("Personne", nom="fayolle", prenom="alexandre")
            p2 = req.create_entity("Personne", nom="campeas", prenom="aurelien")
            note = req.create_entity("Note", type="z")
            p = req.create_entity(
                "Personne",
                nom="di mascio",
