...
def setUpModule(*args):
    ...
    config = ApptestConfiguration("data-rewrite", __file__)
    config.bootstrap_cubes()
    schema = config.load_schema()
    schema.add_relation_def(
        RelationDefinition(
            subject="Card", name="in_state", object="State", cardinality="1*"
        )
    )
    rqlhelper = RQLHelper(schema, special_relations={"eid": "uid", "has_text": "fti"})
...
def eid_func_map(eid):
    return {1: "CWUser", 2: "Card", 3: "Affaire"}[eid]
    ...
    for v, exprs in sorted(snippets_map.items()):
        ...
        for snippet in exprs:
            ...
            if isinstance(snippet, str):
                snippet = mock_object(
                    snippet_rqlst=parse("Any X WHERE " + snippet).children[0],
...
class RQLRewriteTC(TestCase):
    ...
    * "has_<ACTION>_permission" ?
    ...
    def test_base_var(self):
        constraint = (
            "X in_state S, U in_group G, P require_state S,"
        ...
        self.assertEqual(
            rqlst.as_string(),
            "Any C WHERE C is Card, B eid %(D)s, "
    ...
    def test_multiple_var(self):
        ...
        rewrite(
            rqlst,
            {("C", "X"): (card_constraint,), ("S", "X"): affaire_constraints},
            kwargs,
        )
    ...
    def test_or(self):
        ...
        rqlst = parse(
            "Any S WHERE S owned_by C, C eid %(u)s, S is in (CWUser, CWGroup)"
        )
    ...
    def test_optional_var_4(self):
        ...
        self.assertEqual(
            rqlst.as_string(),
            "Any X,LA,Y WHERE LA? documented_by X, LA concerne Y, B eid %(C)s, "
    ...
    def test_ambiguous_optional_same_exprs(self):
        ...
        rqlst = parse(
            "Any A,AR,X,CD WHERE A concerne X?, A ref AR, "
            "A eid %(a)s, X creation_date CD"
        )
        rewrite(rqlst, {("X", "X"): ("X created_by U",)}, {"a": 3})
        self.assertEqual(
            rqlst.as_string(),
            "Any A,AR,X,CD WHERE A concerne X?, A ref AR, A eid %(a)s "
    ...
    def test_ambiguous_optional_same_exprs_constant(self):
        rqlst = parse(
            "Any A,AR,X WHERE A concerne X?, A ref AR, "
            "A eid %(a)s, X creation_date TODAY"
        )
        rewrite(rqlst, {("X", "X"): ("X created_by U",)}, {"a": 3})
        self.assertEqual(
            rqlst.as_string(),
            "Any A,AR,X WHERE A concerne X?, A ref AR, A eid %(a)s "
    ...
    def test_optional_var_inlined_imbricated_error(self):
        ...
        c2 = "X inlined_card O, O require_permission P"
        rqlst = parse(
            "Any C,A,R,A2,R2 WHERE A? inlined_card C, A ref R,"
    ...
    def test_optional_var_inlined_linked(self):
        c1 = "X require_permission P"
        rqlst = parse(
            "Any A,W WHERE A inlined_card C?, C inlined_note N, " "N inlined_affaire W"
        )
        rewrite(rqlst, {("C", "X"): (c1,)}, {})
        self.assertEqual(
            rqlst.as_string(),
    ...
    def test_relation_non_optimization_2(self):
        ...
        rewrite(
            rqlst,
            {
                ("A", "X"): ("X inlined_card C, C inlined_note N, N owned_by U",),
    ...
    def test_unsupported_constraint_1(self):
        ...
        self.assertRaises(
            Unauthorized, rewrite, rqlst, {("T", "X"): (trinfo_constraint,)}, {}
        )
    ...
    def test_unsupported_constraint_2(self):
        ...
        self.assertEqual(
            rqlst.as_string(),
            "Any U,T WHERE U is CWUser, T wf_info_for U, "
    ...
    def test_unsupported_constraint_3(self):
        ...
        rewrite(
            rqlst,
            {("T", "X"): (trinfo_constraint, 'X in_group G, G name "managers"')},
            {},
        )
        self.assertEqual(rqlst.as_string(), "XXX dunno what should be generated")
    ...
    def test_add_ambiguity_exists(self):
        ...
        self.assertEqual(
            rqlst.as_string(),
            "Any X WHERE X is Affaire, ((EXISTS(X concerne A, A is Division)) "
    ...
    def test_add_ambiguity_outerjoin(self):
        ...
        self.assertEqual(
            rqlst.as_string(),
            "Any X,C WHERE X? documented_by C, C is Card "
    ...
    def test_rrqlexpr_nonexistant_subject_1(self):
        ...
        self.assertEqual(rqlst.as_string(), "Any C WHERE C is Card")
        rqlst = parse("Card C")
    ...
    def test_rrqlexpr_nonexistant_subject_3(self):
        ...
        self.assertEqual(
            rqlst.as_string(),
            "Any C WHERE C is Card, A eid %(B)s, "
    ...
    def test_rrqlexpr_nonexistant_subject_5(self):
        ...
        self.assertEqual(
            rqlst.as_string(),
            "Any C WHERE C is Card, EXISTS(C owned_by A, A is CWUser)",
        )
    ...
    def test_rqlexpr_not_relation_1_1(self):
        ...
        self.assertEqual(
            rqlst.as_string(),
            "Any A WHERE NOT EXISTS(A documented_by C, "
    ...
    def test_rqlexpr_not_relation_1_2(self):
        ...
        self.assertEqual(
            rqlst.as_string(),
            "Any A WHERE NOT EXISTS(A documented_by C, C is Card), A is Affaire, "
    ...
    def test_rqlexpr_multiexpr_outerjoin(self):
        ...
        self.assertEqual(
            rqlst.as_string(),
            "Any A WHERE A documented_by C?, A is Affaire "
    ...
    def test_nonregr_is_instance_of(self):
        ...
        rqlst = parse(
            "Any O WHERE S use_email O, S is CWUser, O is_instance_of EmailAddress"
        )
    ...
    def test_ambiguous_constraint_not_exists(self):
        ...
        rqlst = parse("Any P WHERE NOT P require_state S")
        rewrite(
            rqlst,
            {("P", "S"): (state_constraint,), ("S", "O"): (state_constraint,)},
            {},
        )
        self.assertMultiLineEqual(
            rqlst.as_string(),
            "Any P WHERE NOT P require_state S, "
...
class RewriteFullTC(CubicWebTC):
    ...
    def test_ambiguous_optional_same_exprs(self):
        ...
        with self.temporary_permissions(
            (edef1, {"read": (ERQLExpression("X owned_by U"),)}),
            (edef2, {"read": (ERQLExpression("X owned_by U"),)}),
            (edef3, {"read": (ERQLExpression("X owned_by U"),)}),
        ):
            union = self.process(
                "Any A,AR,X,CD WHERE A concerne X?, A ref AR, X creation_date CD"
            )
            self.assertEqual(
                "Any A,AR,X,CD WHERE A concerne X?, A ref AR, A is Affaire "
                "WITH X,CD BEING (Any X,CD WHERE X creation_date CD, "
    ...
    def test_ambiguous_optional_diff_exprs(self):
        ...
        edef3 = self.schema["Note"]
        with self.temporary_permissions(
            (edef1, {"read": (ERQLExpression("X created_by U"),)}),
            (edef2, {"read": ("users",)}),
            (edef3, {"read": (ERQLExpression("X owned_by U"),)}),
        ):
            union = self.process(
    ...
    def test_xxxx(self):
        ...
        with self.temporary_permissions(
            (edef1, {"read": (read_expr,)}), (edef2, {"read": (read_expr,)})
        ):
            union = self.process(
                "Any X,AA,AC,AD ORDERBY AD DESC "
    ...
    def test_question_mark_attribute_snippet(self):
        ...
        try:
            with self.temporary_permissions(
                (self.schema["Affaire"], {"read": (ERQLExpression('X ref "blah"'),)}),
                (
                    self.schema["Note"],
                    {
                        "read": (
                ...
                self.assertEqual(
                    "Any A,COUNT(N) GROUPBY A WHERE A is Affaire "
...
class RQLRelationRewriterTC(TestCase):
    ...
    def test_base_rule(self):
        ...
        self.assertEqual("Any X WHERE X contributor S", rqlst.as_string())

    def test_complex_rule_1(self):
        rules = {
            "illustrator_of": (
                "C is Contribution, C contributor S, "
    ...
    def test_complex_rule_2(self):
        ...
        rule_rewrite(rqlst, rules)
        self.assertEqual(
            "Any A WHERE EXISTS(C is Contribution, "
            "C contributor A, C manifestation B, "
            'C role D, D name "illustrator")',
            rqlst.as_string(),
        )
    ...
    def test_rewrite2(self):
        rules = {
            "illustrator_of": "C is Contribution, C contributor S, "
            'C manifestation O, C role R, R name "illustrator"'
        }
        rqlst = rqlhelper.parse(
            "Any A,B WHERE A illustrator_of B, C require_permission R, S"
            "require_state O"
        )
        rule_rewrite(rqlst, rules)
        self.assertEqual(
    ...
    def test_rewrite3(self):
        ...
        rqlst = rqlhelper.parse(
            "Any A,B WHERE E require_permission T, A illustrator_of B"
        )
        rule_rewrite(rqlst, rules)
        self.assertEqual(
            "Any A,B WHERE E require_permission T, "
    ...
    def test_rewrite4(self):
        ...
        rqlst = rqlhelper.parse(
            "Any A,B WHERE C require_permission R, A illustrator_of B"
        )
        rule_rewrite(rqlst, rules)
        self.assertEqual(
            "Any A,B WHERE C require_permission R, "
    ...
    def test_rewrite_with(self):
        ...
        rqlst = rqlhelper.parse(
            "Any A,B WITH A, B BEING(Any X, Y WHERE X illustrator_of Y)"
        )
        rule_rewrite(rqlst, rules)
        self.assertEqual(
            "Any A,B WITH A,B BEING "
    ...
    def test_rewrite_with2(self):
        ...
        rqlst = rqlhelper.parse(
            "Any A,B WHERE T require_permission C "
    ...
    def test_rewrite_with3(self):
        rules = {"participated_in": "S contributor O"}
        rqlst = rqlhelper.parse(
            "Any A,B WHERE A participated_in B "
            "WITH A, B BEING(Any X,Y WHERE X contributor Y)"
        )
        rule_rewrite(rqlst, rules)
        self.assertEqual(
            "Any A,B WHERE A contributor B WITH A,B BEING "
    ...
    def test_rewrite_union(self):
        ...
        rqlst = rqlhelper.parse(
            "(Any A,B WHERE A illustrator_of B) UNION"
    ...
    def test_rewrite_exists(self):
        ...
        rqlst = rqlhelper.parse(
            "(Any A,B WHERE A illustrator_of B, " "EXISTS(B is ArtWork))"
        )
        rule_rewrite(rqlst, rules)
        self.assertEqual(
            "Any A,B WHERE EXISTS(B is ArtWork), "
    ...
    def test_rewrite_exists2(self):
        ...
        rqlst = rqlhelper.parse(
            "(Any A,B WHERE B contributor A, EXISTS(A illustrator_of W))"
        )
        rule_rewrite(rqlst, rules)
        self.assertEqual(
            "Any A,B WHERE B contributor A, "
    ...
    def test_rewrite_exists3(self):
        ...
        rqlst = rqlhelper.parse(
            "(Any A,B WHERE A illustrator_of B, EXISTS(A illustrator_of W))"
        )
        rule_rewrite(rqlst, rules)
        self.assertEqual(
            "Any A,B WHERE EXISTS(C is Contribution, C contributor A, "
    ...
    def test_rewrite_groupby(self):
        rules = {"participated_in": "S contributor O"}
        rqlst = rqlhelper.parse(
            "Any SUM(SA) GROUPBY S " "WHERE P participated_in S, P manifestation SA"
        )
        rule_rewrite(rqlst, rules)
        self.assertEqual(
            "Any SUM(SA) GROUPBY S WHERE P manifestation SA, P contributor S",
            rqlst.as_string(),
        )
...
class RQLRelationRewriterCWTC(CubicWebTC):
    ...
    def test_base_rule(self):
        with self.admin_access.client_cnx() as cnx:
            art = cnx.create_entity("ArtWork", name="Les travailleurs de la Mer")
            role = cnx.create_entity("Role", name="illustrator")
            vic = cnx.create_entity("Person", name="Victor Hugo")
            cnx.create_entity(
                "Contribution", code=96, contributor=vic, manifestation=art, role=role
            )
            rset = cnx.execute("Any X WHERE X illustrator_of S")
            self.assertEqual(
                ["Victor Hugo"], [result.name for result in rset.entities()]
            )
            rset = cnx.execute(
                "Any S WHERE X illustrator_of S, X eid %(x)s", {"x": vic.eid}
            )
            self.assertEqual(
