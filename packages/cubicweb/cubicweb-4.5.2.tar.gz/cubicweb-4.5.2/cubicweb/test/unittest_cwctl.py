...
class CubicWebCtlTC(unittest.TestCase):
    ...
    @patch(
        ...
        side_effect=unittest_cwconfig.iter_entry_points,
    )
    def test_list(self, mock_iter_entry_points):
        ListCommand(None).run([])
        stream_result = self.stream.getvalue()
        self.assertNotIn("cubicweb_", stream_result)
        sys.stderr.write(stream_result)
...
class CubicWebShellTC(CubicWebTC):
    def test_process_script_args_context(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            mih.cmd_process_script(
                join(self.datadir, "scripts", "script1.py"), funcname=None
            )
...
class InstanceCommandTest(unittest.TestCase):
    def setUp(self):
        ...
        config_patcher = patch.object(
            cwcfg, "config_for", return_value=self.fake_config
        )
    ...
    def test_set_loglevel(self):
        ...
        for log_level in LOG_LEVELS:
            ...
            self.fake_config.global_set_option.assert_called_with(
                "log-threshold", log_level.upper()
            )
    ...
    def test_set_dblevel(self):
        ...
        for dbg_flag in DBG_FLAGS:
            ...
            total_value += getattr(server, f"DBG_{dbg_flag}")
