...
from cubicweb import rdf, Unauthorized
...
def _get_entity_from_eid(request):
    try:
        entity = EntityResource.from_eid()(request).rset.one()
    except Unauthorized:
        raise HTTPNotFound()
    return entity


def _get_entity_from_identifier(request):
    try:
        entity = ETypeResource.from_match("etype")(request)[
            request.matchdict["identifier"]
        ].rset.one()
    except Unauthorized:
        raise HTTPNotFound()
    return entity
...
def download_context_from_eid(request):
    entity = _get_entity_from_eid(request)
    return BinaryResource(entity)


def download_context_from_identifier(request):
    entity = _get_entity_from_identifier(request)
    return BinaryResource(entity)
...
def rdf_context_from_eid(request):
    ...
    if mime_type is None:
        return HTMLResource(entity)
    return RDFResource(entity, mime_type)


def rdf_context_from_identifier(request):
    mime_type = negociate_mime_type(request, rdf.RDF_MIMETYPE_TO_FORMAT)
    entity = _get_entity_from_identifier(request)
    if mime_type is None:
        return HTMLResource(entity)
...
class ETypeResource:
    ...
    def __getitem__(self, value):
        (rest_attr, needcheck) = self.cls.cw_rest_attr_info()
        resource = EntityResource(self.request, self.cls, rest_attr, value)
        rset = resource.rset
        if rset.rowcount and (not needcheck or rset.rowcount == 1):
            return resource
    ...
    def rset(self):
        ...
        rset = self.request.cw_cnx.execute(rql)
        return rset
