...
from cubicweb.toolsutils import fill_templated_file, CommandHandler, underline_title
...
class AllInOneCreateHandler(serverctl.RepositoryCreateHandler, WebCreateHandler):
...
class PyramidStartHandler(InstanceCommand):
    ...
    def restart_with_reloader(self, filelist_path):
        ...
        while True:
            ...
            try:
                ...
                if proc is not None:
                    proc.terminate()
                    self.info("Waiting for the server to stop. Hit CTRL-C to exit")
                    exit_code = proc.wait()
    ...
    def configfiles(self, cwconfig):
        ...
        for f in ("sources", "logging.conf", "pyramid.ini", "pyramid-debug.ini"):
    ...
    def start_instance(self, appid):
        ...
        cwconfig.in_debug_mode = self["debug"] or cwconfig["debug"]

        self._needreload = False
        autoreload = self["reload"] or cwconfig.in_debug_mode

        filelist_path = os.path.join(cwconfig.apphome, ".pyramid-reload-files.list")
        ...
        if autoreload:
            ...
            self.install_reloader(
                self["reload-interval"], extra_files, filelist_path=filelist_path
            )

        # if no loglevel is specified and --debug is here, set log level at debug
        if self["loglevel"] is None and cwconfig.in_debug_mode:
        ...
        url_scheme = "https" if cwconfig["base-url"].startswith("https") else "http"
        repo = app.application.registry["cubicweb.repository"]
        try:
            self.info(
                f"Application started. Point your browser to {cwconfig['base-url']}"
            )
            waitress.serve(
                app,
                host=host,
    ...
    def check_reload(self):
        ...
        for filename in filenames:
            ...
            if filename not in self.module_mtimes:
                ...
                print(f"{filename} changed; reloading...")
                return False
