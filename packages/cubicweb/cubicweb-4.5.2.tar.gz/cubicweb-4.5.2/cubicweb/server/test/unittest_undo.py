...
class UndoableTransactionTC(CubicWebTC):
    def setup_database(self):
        with self.admin_access.repo_cnx() as cnx:
            self.totoeid = self.create_user(
                cnx, "toto", password="toto", groups=("users",), commit=False
            ).eid
    ...
    def check_transaction_deleted(self, cnx, txuuid):
        ...
        cu = cnx.system_sql(f"SELECT * from tx_entity_actions WHERE tx_uuid='{txuuid}'")
        self.assertFalse(cu.fetchall())
        cu = cnx.system_sql(
            f"SELECT * from tx_relation_actions WHERE tx_uuid='{txuuid}'"
        )
        self.assertFalse(cu.fetchall())
    ...
    def test_undo_api(self):
        ...
        with self.admin_access.client_cnx() as cnx:
            ...
            self.assertRaises(NoSuchTransaction, cnx.transaction_info, "hop")
            self.assertRaises(NoSuchTransaction, cnx.transaction_actions, "hop")
            self.assertRaises(NoSuchTransaction, cnx.undo_transaction, "hop")
            ...
            self.assertEqual(
                {actions[4].rtype, actions[5].rtype},
                {"in_state", "created_by"},
            )
            for i in (4, 5):
                a = actions[i]
        ...
        with self.new_access("anon").client_cnx() as cnx:
            self.assertRaises(NoSuchTransaction, cnx.transaction_info, self.txuuid)
            self.assertRaises(NoSuchTransaction, cnx.transaction_actions, self.txuuid)
            self.assertRaises(NoSuchTransaction, cnx.undo_transaction, self.txuuid)
            txs = cnx.undoable_transactions()
    ...
    def test_undoable_transactions(self):
        with self.admin_access.client_cnx() as cnx:
            toto = self.toto(cnx)
            cnx.create_entity(
                "EmailAddress", address="toto@logilab.org", reverse_use_email=toto
            )
            txuuid1 = cnx.commit()
            toto.cw_delete()
            txuuid2 = cnx.commit()
            ...
            self.assertEqual(len(txs), 0)
            txs = undoable_transactions(etype="EmailAddress", action="D", public=False)
            self.assertEqual(len(txs), 1)
            self.assertEqual(txs[0].uuid, txuuid2)
    ...
    def test_undo_deletion_base(self):
        with self.admin_access.client_cnx() as cnx:
            ...
            self.assertEqual(
                toto.cw_adapt_to("IEmailable").get_email(), "toto@logilab.org"
            )
            self.assertEqual(
                [(p.pkey, p.value) for p in toto.reverse_for_user],
                [("ui.default-text-format", "text/rest")],
            )
            self.assertEqual([g.name for g in toto.in_group], ["users"])
            self.assertEqual(
                [et.name for et in toto.related("is", entities=True)], ["CWUser"]
            )
            self.assertEqual([et.name for et in toto.is_instance_of], ["CWUser"])
            # undoing shouldn't be visble in undoable transaction, and the undone
            # transaction should be removed
            txs = cnx.undoable_transactions()
            self.assertEqual(len(txs), 2)
            self.assertRaises(NoSuchTransaction, cnx.transaction_info, txuuid)
        with self.admin_access.repo_cnx() as cnx:
    ...
    def test_undo_deletion_integrity_1(self):
        with self.admin_access.client_cnx() as cnx:
            ...
            c = cnx.create_entity("Card", title="hop", content="hop")
            p = cnx.create_entity("Personne", nom="louis", fiche=c)
            cnx.commit()
            c.cw_delete()
            txuuid = cnx.commit()
            c2 = cnx.create_entity("Card", title="hip", content="hip")
            p.cw_set(fiche=c2)
            cnx.commit()
            self.assertUndoTransaction(
                cnx,
                txuuid,
                [
    ...
    def test_undo_deletion_integrity_2(self):
        with self.admin_access.client_cnx() as cnx:
            ...
            g = cnx.create_entity("CWGroup", name="staff")
            ...
            self.assertUndoTransaction(
                ...
                txuuid,
                [
                    "Can't restore relation in_group, object entity "
            ...
            self.assertEqual(
                cm.exception.errors,
                {
                    "in_group-subject": "at least one relation in_group is "
    ...
    def test_undo_creation_1(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            for eid in (p.eid, c.eid):
                self.assertFalse(
                    cnx.system_sql(f"SELECT * FROM entities WHERE eid={eid}").fetchall()
                )
                self.assertFalse(
                    cnx.system_sql(
                        f"SELECT 1 FROM owned_by_relation WHERE eid_from={eid}"
                    ).fetchall()
                )
                # added by sql in hooks (except when using dataimport)
                self.assertFalse(
                    cnx.system_sql(
                        f"SELECT 1 FROM is_relation WHERE eid_from={eid}"
                    ).fetchall()
                )
                self.assertFalse(
                    cnx.system_sql(
                        f"SELECT 1 FROM is_instance_of_relation WHERE eid_from={eid}"
                    ).fetchall()
                )
            self.check_transaction_deleted(cnx, txuuid)
    ...
    def test_undo_creation_integrity_1(self):
        with self.admin_access.client_cnx() as cnx:
            ...
            txuuid = cnx.commit()
            email = cnx.create_entity("EmailAddress", address="tutu@cubicweb.org")
            prop = cnx.create_entity(
                "CWProperty", pkey="ui.default-text-format", value="text/html"
            )
    ...
    def test_undo_inline_rel_remove_ko(self):
        ...
        with self.admin_access.client_cnx() as cnx:
            ...
            self.assertUndoTransaction(
                ...
                txuuid,
                [
                    "Can't restore relation fiche, object entity %d doesn't exist anymore."
                    % c.eid
                ],
            )
            cnx.commit()
        ...
        with self.admin_access.repo_cnx() as cnx:
            self.assertIsNone(
                cnx.system_sql(
                    f"SELECT cw_fiche FROM cw_Personne WHERE cw_eid={p.eid}"
                ).fetchall()[0][0]
            )
    ...
    def test_undo_inline_rel_add_ok(self):
        ...
        with self.admin_access.client_cnx() as cnx:
            c = cnx.create_entity("Card", title="hop", content="hop")
            p = cnx.create_entity("Personne", nom="louis")
            cnx.commit()
            p.cw_set(fiche=c)
            txuuid = cnx.commit()
            self.assertUndoTransaction(cnx, txuuid)
    ...
    def test_undo_inline_rel_delete_ko(self):
        with self.admin_access.client_cnx() as cnx:
            c = cnx.create_entity("Card", title="hop", content="hop")
            txuuid = cnx.commit()
            cnx.create_entity("Personne", nom="louis", fiche=c)
            cnx.commit()
            integrityerror = (
                self.repo.system_source.dbhelper.dbapi_module.IntegrityError
            )
    ...
    def test_undo_inline_rel_replace_ok(self):
        ...
        with self.admin_access.client_cnx() as cnx:
            c1 = cnx.create_entity("Card", title="hop", content="hop")
            c2 = cnx.create_entity("Card", title="hip", content="hip")
            p = cnx.create_entity("Personne", nom="louis", fiche=c1)
            cnx.commit()
            p.cw_set(fiche=c2)
            txuuid = cnx.commit()
            self.assertUndoTransaction(cnx, txuuid)
