...
from cubicweb.server.schema2sql import (
    eschema2sql,
    rschema2sql,
    unique_index_name,
    sql_type,
...
logger = logging.getLogger("server.migractions")
...
class ServerMigrationHelper(MigrationHelper):
    ...
    def __init__(
        ...
        if config is not None and (cnx or connect):
            ...
            CW_EVENT_MANAGER.bind(
                "after-registry-reload", ClearGroupMap.mih_register, repo
            )
        ...
        if not schema and not config.quick_start:
            insert_lperms = (
                self.repo.get_versions()["cubicweb"] < (3, 14, 0)
                and "localperms" in config.available_cubes()
            )
            if insert_lperms:
                cubes = config._cubes
    ...
    def set_cnx(self):
        try:
            ...
            logger.info(
                f"use login/password '{login}/*******' found in repo configuration "
                f"(from {self.repo.config.apphome})"
            )
        except KeyError:
            logger.info(
                f"failed to get login/password from repo config (from "
                f"{self.repo.config.apphome}), fallback to ask it from input"
            )
        ...
        while True:
            ...
            try:
                ...
                print(
                    f"failed to get another login/password from standard input (probably because "
                    f"this is executed from a script)\nChange the login/password of admin in the "
                    f"config file (in {self.repo.config.apphome}) to match that."
                )
                print("aborting...")
                sys.exit(0)

    def cube_upgraded(self, cube, version):
        self.cmd_set_property(f"system.version.{cube.lower()}", str(version))
        self.commit()
    ...
    def backup_database(self, backupfile=None, askconfirm=True, format="native"):
        ...
        backupfile = backupfile or osp.join(
            instbkdir, f"{config.appid}-{timestamp}.tar.gz"
        )
        # check backup has to be done
        if osp.exists(backupfile) and not self.confirm(
            f"Backup file {backupfile} exists, overwrite it?"
        ):
            print("-> no backup done.")
            return
        elif askconfirm and not self.confirm(f"Backup {config.appid} database?"):
            print("-> no backup done.")
            return
    ...
    def restore_database(self, backupfile, drop=True, askconfirm=True, format="native"):
        ...
        if not osp.exists(backupfile):
            raise ExecutionError(f"Backup file {backupfile} doesn't exist")
        if askconfirm and not self.confirm(
            f"Restore {self.config.appid} database from {backupfile} ?"
        ):
            return
        # unpack backup
        tmpdir = tempfile.mkdtemp()
            ...
            for name in bkup.getnames():
                if name[0] in "/.":
                    raise ExecutionError(
                        'Security check failed, path starts with "/" or "."'
                    )
        ...
        try:
            ...
            print(f"-> error trying to restore {source.uri} [{exc}]")
            if not self.confirm(
                "Continue anyway?", default="n", pdb=True, traceback=traceback_
            ):
                raise SystemExit(1)
        finally:
            shutil.rmtree(tmpdir)
    ...
    def cmd_exec_event_script(self, event, cube=None, funcname=None, *args, **kwargs):
        ...
        if cube:
            ...
            apc = osp.join(cubepath, "migration", f"{event}.py")
        elif kwargs.pop("apphome", False):
            apc = osp.join(self.config.apphome, "migration", f"{event}.py")
        else:
            apc = osp.join(self.config.migration_scripts_dir(), f"{event}.py")
        if osp.exists(apc):
    ...
    def cmd_install_custom_sql_scripts(self, cube=None):
        ...
        sql_scripts = glob(osp.join(directory, f"*.{driver}.sql"))
        for fpath in sql_scripts:
            print("-> installing", fpath)
            with open(fpath) as fobj:
                sql_code = fobj.read()
            failed = sqlexec(sql_code, self.cnx.system_sql, False, delimiter=";;")
            if failed:
    ...
    def _synchronize_permissions(self, erschema, teid):
        ...
        if "update" in erschema.ACTIONS or erschema.final:
            # entity type
            exprtype = "ERQLExpression"
        else:
            # relation type
            exprtype = "RRQLExpression"
        ...
        for action in erschema.ACTIONS:
            perm = f"{action}_permission"
            # handle groups
            newgroups = list(erschema.get_groups(action))
            for geid, gname in self.rqlexec(
                "Any G, GN WHERE T %s G, G name GN, " "T eid %%(x)s" % perm,
                {"x": teid},
                ask_confirm=False,
            ):
                if gname not in newgroups:
                    if not confirm or self.confirm(
                        f"Remove {action} permission of {erschema} to {gname}?"
                    ):
                        self.rqlexec(
                            f"DELETE T {perm} G WHERE G eid %(x)s, T eid {teid}",
                            {"x": geid},
                            ask_confirm=False,
                        )
                else:
            ...
            for gname in newgroups:
                if not confirm or self.confirm(
                    f"Grant {action} permission of {erschema} to {gname}?"
                ):
                    try:
                        self.rqlexec(
                            f"SET T {perm} G WHERE G eid %(x)s, T eid {teid}",
                            {"x": gm[gname]},
                            ask_confirm=False,
                        )
                    except KeyError:
                        self.error(
                            "can grant %s perm to unexistant group %s", action, gname
                        )
            # handle rql expressions
            newexprs = dict(
                (expr.expression, expr) for expr in erschema.get_rqlexprs(action)
            )
            for expreid, expression in self.rqlexec(
                f"Any E, EX WHERE T {perm} E, E expression EX, T eid {teid}",
                ask_confirm=False,
            ):
                if expression not in newexprs:
                    if not confirm or self.confirm(
                        "Remove %s expression for %s permission of %s?"
                        % (expression, action, erschema)
                    ):
                        # deleting the relation will delete the expression entity
                        self.rqlexec(
                            f"DELETE T {perm} E WHERE E eid %(x)s, T eid {teid}",
                            {"x": expreid},
                            ask_confirm=False,
                        )
                else:
            ...
            for expression in newexprs.values():
                ...
                if not confirm or self.confirm(
                    f"Add {expr} expression for {action} permission of {erschema}?"
                ):
                    self.rqlexec(
                        "INSERT RQLExpression X: X exprtype %%(exprtype)s, "
                        "X expression %%(expr)s, X mainvars %%(vars)s, T %s X "
                        "WHERE T eid %%(x)s" % perm,
                        {
                            "expr": expr,
                            "exprtype": exprtype,
                            "vars": ",".join(sorted(expression.mainvars)),
                            "x": teid,
                        },
                        ask_confirm=False,
                    )

    def _synchronize_rschema(
        self, rtype, syncrdefs=True, syncperms=True, syncprops=True
    ):
        ...
        * permissions if `syncperms`
        ...
        rschema = self.fs_schema.relation_schema_for(rtype)
        reporschema = self.repo.schema.relation_schema_for(rtype)
        if syncprops:
            assert reporschema.eid, reporschema
            self.rqlexecall(
                ss.updaterschema2rql(rschema, reporschema.eid),
        ...
        if rschema.rule:
            ...
            for subj, obj in rschema.relation_definitions:
                if (subj, obj) not in reporschema.relation_definitions:
                    continue
                if rschema in VIRTUAL_RTYPES:
                    continue
                self._synchronize_rdef_schema(
                    subj, rschema, obj, syncprops=syncprops, syncperms=syncperms
                )

    def _synchronize_eschema(
        self, etype, syncrdefs=True, syncperms=True, syncprops=True
    ):
        ...
        * permissions if `syncperms`
        ...
        repoeschema = self.repo.schema.entity_schema_for(etype)
        try:
            eschema = self.fs_schema.entity_schema_for(etype)
        except KeyError:
        ...
        if syncprops:
            ...
            if repospschema and not espschema:
                self.rqlexec(
                    "DELETE X specializes Y WHERE X is CWEType, X name %(x)s",
                    {"x": str(repoeschema)},
                    ask_confirm=False,
                )
            elif not repospschema and espschema:
                self.rqlexec(
                    "SET X specializes Y WHERE X is CWEType, X name %(x)s, "
                    "Y is CWEType, Y name %(y)s",
                    {"x": str(repoeschema), "y": str(espschema)},
                    ask_confirm=False,
                )
            self.rqlexecall(
                ss.updateeschema2rql(eschema, repoeschema.eid),
                ask_confirm=self.verbosity >= 2,
            )
        if syncperms:
        ...
        if syncrdefs:
            for rschema, targettypes, role in eschema.relation_definitions(True):
                if rschema in VIRTUAL_RTYPES:
                    continue
                if role == "subject":
                    if rschema not in repoeschema.subject_relations.values():
                        continue
                    subjtypes, objtypes = [etype], targettypes
                else:  # role == 'object'
                    if rschema not in list(repoeschema.object_relations.values()):
                        continue
                    subjtypes, objtypes = targettypes, [etype]
                self._synchronize_rschema(
                    rschema, syncrdefs=False, syncprops=syncprops, syncperms=syncperms
                )
                if (
                    rschema.rule
                ):  # rdef for computed rtype are infered hence should not be
                    # synchronized
                    continue
                reporschema = self.repo.schema.relation_schema_for(rschema)
        ...
        if syncprops:  # need to process __unique_together__ after rdefs were processed
            ...
            for entity in schemaentity.related(
                "constraint_of", "object", targettypes=("CWUniqueTogetherConstraint",)
            ).entities():
            ...
            for name in removed:
                self.rqlexec(
                    "DELETE CWUniqueTogetherConstraint C WHERE C name %(name)s",
                    {"name": name},
                )

            def possible_unique_constraint(cols):
                for name in cols:
                    rschema = repoeschema.subject_relations.get(name)
                    if rschema is None:
                        print(
                            "dont add %s unique constraint on %s, missing %s"
                            % (",".join(cols), eschema, name)
                        )
                        return False
                    if not (rschema.final or rschema.inlined):
                        print(
                            "dont add %s unique constraint on %s, %s is neither final nor inlined"
                            % (",".join(cols), eschema, name)
                        )
                        return False
                return True
    ...
    def _synchronize_rdef_schema(
        self, subjtype, rtype, objtype, syncperms=True, syncprops=True
    ):
        ...
        * permissions
        ...
        if rschema.rule:
            raise ExecutionError(
                "Cannot synchronize a relation definition for a "
        ...
        rdef = rschema.relation_definition(subjtype, objtype)
        if rdef.infered:
            return  # don't try to synchronize infered relation defs
        repordef = reporschema.relation_definition(subjtype, objtype)
        confirm = self.verbosity >= 2
        if syncprops:
            # properties
            self.rqlexecall(ss.updaterdef2rql(rdef, repordef.eid), ask_confirm=confirm)
            # constraints
            ...
            for cstr in oldconstraints:
                self.rqlexec(
                    "DELETE CWConstraint C WHERE C eid %(x)s",
                    {"x": cstr.eid},
                    ask_confirm=confirm,
                )
            ...
            self.rqlexecall(
                ss.constraints2rql(cstrtype_map, newconstraints, repordef.eid),
                ask_confirm=confirm,
            )
    ...
    def cmd_add_cubes(self, cubes, update_database=True):
        ...
        for cube in newcubes:
            self.cmd_set_property(
                "system.version." + cube, self.config.cube_version(cube)
            )
        ...
        for rschema in newcubes_schema.relations():
            if rschema.type in VIRTUAL_RTYPES:
                continue
            existingschema = self.repo.schema.relation_schema_for(rschema.type)
            for fromtype, totype in rschema.relation_definitions:
                ...
                if (
                    ...
                    in existingschema.relation_definitions
                    or rschema.relation_definitions[(fromtype, totype)].infered
                ):
                    continue
    ...
    def cmd_drop_cube(self, cube, removedeps=False):
        ...
        for rschema in fsschema.relations():
            if rschema in removedcubes_schema and rschema in reposchema:
                ...
                # other cubes
                for fromtype, totype in rschema.relation_definitions:
                    if (fromtype, totype) not in removedcubes_schema[
                        rschema.type
                    ].relation_definitions and (fromtype, totype) in reposchema[
                        rschema.type
        ...
        for cube in reversed(removedcubes):
            ...
            self.rqlexec(
                "DELETE CWProperty X WHERE X pkey %(pk)s",
                {"pk": "system.version." + cube},
                ask_confirm=False,
            )
        self.commit()
    ...
    def cmd_add_attribute(self, etype, attrname, attrtype=None, commit=True):
        ...
        if attrtype is None:
            rschema = self.fs_schema.relation_schema_for(attrname)
    ...
    def cmd_drop_attribute(self, etype, attrname, commit=True):
        ...
        """
        try:
            rschema = self.repo.schema.relation_schema_for(attrname)
            attrtype = rschema.objects(etype)[0]
        except KeyError:
            print(f"warning: attribute {etype} {attrname} is not known, skip deletion")
        else:
    ...
    def cmd_rename_attribute(self, etype, oldname, newname, commit=True):
        ...
        rql = f"SET X {newname} VAL WHERE X is {etype}, X {oldname} VAL"
        card = eschema.relation_definition(newname).cardinality[0]
        if card == "1":
            rql += f", NOT X {oldname} NULL"
    ...
    def cmd_add_entity_type(self, etype, auto=True, commit=True):
        ...
        if etype in instschema and not (eschema.final and eschema.eid is None):
            print(f"warning: {etype} already known, skip addition")
            return
        confirm = self.verbosity >= 2
        ...
        if specialized:
            try:
                ...
                raise ExecutionError(
                    "trying to add entity type but parent type is "
        ...
        for spschema in eschema.specialized_by(recursive=False):
            ...
            if instspschema.specializes() != eschema:
                self.rqlexec(
                    "SET D specializes P WHERE D eid %(d)s, P name %(pn)s",
                ...
                for rschema, tschemas, role in spschema.relation_definitions(True):
                    for tschema in tschemas:
                        ...
                        if role == "subject":
                            ...
                            if (
                                rschema.final
                                and rschema in instspschema.subject_relations
                            ):
                            ...
                            subjschema = tschema
                            objschema = spschema
                        if rschema.relation_definition(
                            subjschema, objschema
                        ).infered or (
                            instschema.has_relation(rschema)
                            and (subjschema, objschema)
                            in instschema[rschema].relation_definitions
                        ):
                            continue
                        self.cmd_add_relation_definition(
        ...
        if auto:
            ...
            self.commit()
            added = []
            for rschema in eschema.subject_relations.values():
                ...
                for targetschema in rschema.objects(etype):
                    ...
                    ss.execschemarql(
                        execute, rdef, ss.rdef2rql(rdef, cstrtypemap, groupmap)
                    )
    ...
    def cmd_drop_entity_type(self, etype, commit=True):
        ...
        self.rqlexec(
            "DELETE CWEType X WHERE X name %(etype)s",
            {"etype": etype},
            ask_confirm=self.verbosity >= 2,
        )
        if commit:
    ...
    def cmd_rename_entity_type(self, oldname, newname, attrs=None, commit=True):
        ...
        if oldname not in schema:
            print(f"warning: entity type {oldname} is unknown, skip renaming")
            return
        # if merging two existing entity types
        if newname in schema:
            assert (
                oldname in ETYPE_NAME_MAP
            ), f"{oldname} should be mapped to {newname} in ETYPE_NAME_MAP"
            if attrs is None:
                attrs = ",".join(
                    SQL_PREFIX + rschema.type
                    for rschema in schema[newname].subject_relations.values()
            ...
            new = schema.entity_schema_for(newname)
            oldeid = self.rqlexec(
                "CWEType ET WHERE ET name %(on)s", {"on": oldname}, ask_confirm=False
            )[0][0]
            # backport old type relations to new type
            # XXX workflows, other relations?
            for r1, rr1 in [("from_entity", "to_entity"), ("to_entity", "from_entity")]:
                self.rqlexec(
                    "SET X %(r1)s NET WHERE X %(r1)s OET, "
            ...
            for rtype in ("is", "is_instance_of"):
                self.sqlexec(
                    "UPDATE %s_relation SET eid_to=%s WHERE eid_to=%s"
                    % (rtype, new.eid, oldeid),
                    ask_confirm=False,
                )
            ...
            for rdeftype in ("CWRelation", "CWAttribute"):
                ...
                entities = [
                    self.cnx.entity_from_eid(eid, rdeftype) for eid in thispending
                ]
                self.repo.system_source.delete_info_multi(self.cnx, entities)
                self.sqlexec(
                    "DELETE FROM cw_%s WHERE cw_from_entity=%%(eid)s OR "
                    "cw_to_entity=%%(eid)s" % rdeftype,
                    {"eid": oldeid},
                    ask_confirm=False,
                )
                    ...
                    if role == "object":
                        if rschema.inlined:
                            for eschema in ttypes:
                                sqls.append(
                                    "DELETE FROM cw_%s WHERE cw_%s IN(%%s)"
                                    % (eschema, rschema)
                                )
                        else:
                            sqls.append(
                                "DELETE FROM %s_relation WHERE eid_to IN(%%s)" % rschema
                            )
                    elif not rschema.inlined:
                        sqls.append(
                            "DELETE FROM %s_relation WHERE eid_from IN(%%s)" % rschema
                        )
                    for sql in sqls:
                        self.sqlexec(sql % thiseids, ask_confirm=False)
            # remove the old type: use rql to propagate deletion
            self.rqlexec(
                "DELETE CWEType ET WHERE ET name %(on)s",
                {"on": oldname},
                ask_confirm=False,
            )
        # elif simply renaming an entity type
            ...
            self.rqlexec(
                "SET ET name %(newname)s WHERE ET is CWEType, ET name %(on)s",
    ...
    def cmd_add_relation_type(self, rtype, addrdef=True, commit=True):
        ...
        if rtype in reposchema:
            print(f"warning: relation type {rtype} is already known, skip addition")
        elif rschema.rule:
        ...
        if not rschema.rule and addrdef:
            ...
            for subj, obj in rschema.relation_definitions:
                if not (reposchema.has_entity(subj) and reposchema.has_entity(obj)):
                    continue
            ...
            if rtype in META_RTYPES:
                ...
                for etype in self.repo.schema.entities():
                    if etype not in self.fs_schema:
                        ...
                        rdef.subject = etype
                        rdef.rtype = self.repo.schema.relation_schema_for(rschema)
                        rdef.object = self.repo.schema.entity_schema_for(objtype)
                        ss.execschemarql(execute, rdef, ss.rdef2rql(rdef, cmap, gmap))
        if commit:
            self.commit()
    ...
    def cmd_drop_relation_type(self, rtype, commit=True):
        ...
        """
        self.rqlexec(
            f"DELETE CWRType X WHERE X name {rtype!r}", ask_confirm=self.verbosity >= 2
        )
        self.rqlexec(
            f"DELETE CWComputedRType X WHERE X name {rtype!r}",
            ask_confirm=self.verbosity >= 2,
        )
        if commit:
            self.commit()
    ...
    def cmd_rename_relation_type(self, oldname, newname, commit=True, force=False):
        ...
        if oldname in self.fs_schema and not force:
            if not self.confirm(
                "Relation %s is still present in the filesystem schema,"
        ...
        if not self.repo.schema[oldname].rule:
            self.rqlexec(
                f"SET X {newname} Y WHERE X {oldname} Y",
                ask_confirm=self.verbosity >= 2,
            )
    ...
    def cmd_add_relation_definition(self, subjtype, rtype, objtype, commit=True):
        ...
        if rschema.rule:
            raise ExecutionError(
                f"Cannot add a relation definition for a computed relation ({rschema})"
            )
        if rtype not in self.repo.schema:
            self.cmd_add_relation_type(rtype, addrdef=False, commit=False)
        if (subjtype, objtype) in self.repo.schema.relation_schema_for(
            rtype
        ).relation_definitions:
            print(
                "warning: relation %s %s %s is already known, skip addition"
        ...
        ss.execschemarql(
            self.cnx.execute,
            rdef,
            ss.rdef2rql(rdef, self.cstrtype_mapping(), self.group_mapping()),
        )
        if commit:
            self.commit()

    def _get_rdef(self, rschema, subjtype, objtype):
        return self._set_rdef_eid(rschema.relation_definitions[(subjtype, objtype)])

    def _set_rdef_eid(self, rdef):
        for attr in ("relation_type", "subject", "object"):
            schemaobj = getattr(rdef, attr)
            if getattr(schemaobj, "eid", None) is None:
                schemaobj.eid = self.repo.schema[schemaobj].eid
                assert schemaobj.eid is not None, "%s has no eid while adding %s" % (
                    schemaobj,
                    rdef,
                )
        return rdef
    ...
    def cmd_drop_relation_definition(self, subjtype, rtype, objtype, commit=True):
        ...
        """
        rschema = self.repo.schema.relation_schema_for(rtype)
        if rschema.rule:
            raise ExecutionError(
                f"Cannot drop a relation definition for a computed relation ({rschema})"
            )
        ...
        if rschema.final:
            ...
            etype = "CWRelation"
        rql = (
            'DELETE %s X WHERE X from_entity FE, FE name "%s",'
            'X relation_type RT, RT name "%s", X to_entity TE, TE name "%s"'
        )
        self.rqlexec(
            rql % (etype, subjtype, rtype, objtype), ask_confirm=self.verbosity >= 2
        )
        if commit:
            self.commit()

    def cmd_sync_schema_props_perms(
        self, ertype=None, syncperms=True, syncprops=True, syncrdefs=True, commit=True
    ):
        ...
        if ertype is not None:
            ...
            for etype in self.repo.schema.entities():
                if etype.eid is None:
                    ...
                    continue
                self._synchronize_eschema(
                    etype, syncrdefs=syncrdefs, syncprops=syncprops, syncperms=syncperms
                )
        if commit:
            self.commit()

    def cmd_change_relation_props(
        self, subjtype, rtype, objtype, commit=True, **kwargs
    ):
        ...
        if subjtype and subjtype != "Any":
            restriction.append(f'X from_entity FE, FE name "{subjtype}"')
        if objtype and objtype != "Any":
            restriction.append(f'X to_entity TE, TE name "{objtype}"')
        if rtype and rtype != "Any":
            restriction.append(f'X relation_type RT, RT name "{rtype}"')
        assert restriction
        values = []
        for k, v in kwargs.items():
            values.append(f"X {k} %({k})s")
        rql = f"SET {','.join(values)} WHERE {','.join(restriction)}"
    ...
    def cmd_set_size_constraint(self, etype, rtype, size, commit=True):
        ...
        oldvalue = None
        for constr in (
            self.repo.schema.entity_schema_for(etype)
            .relation_definition(rtype)
            .constraints
        ):
        ...
        if oldvalue is None and size is not None:
            ceid = self.rqlexec(
                "INSERT CWConstraint C: C value %(v)s, C cstrtype CT "
                'WHERE CT name "SizeConstraint"',
                {"v": SizeConstraint(size).serialize()},
                ask_confirm=self.verbosity >= 2,
            )[0][0]
            self.rqlexec(
                "SET X constrained_by C WHERE X from_entity S, X relation_type R, "
            ...
            if size is not None:
                self.rqlexec(
                    "SET C value %%(v)s WHERE X from_entity S, X relation_type R,"
                ...
                self.rqlexec(
                    "DELETE X constrained_by C WHERE X from_entity S, X relation_type R,"
                    'X constrained_by C, C cstrtype CT, CT name "SizeConstraint",'
                    'S name "%s", R name "%s"' % (etype, rtype),
                    ask_confirm=self.verbosity >= 2,
                )
    ...
    def cmd_add_workflow(
        self, name, wfof, default=True, commit=False, ensure_workflowable=True, **kwargs
    ):
        """
        create a new workflow and links it to entity types
         ...
         :rtype: `Workflow`
        """
        wf = self.cmd_create_entity("Workflow", name=name, **kwargs)
        if not isinstance(wfof, (list, tuple)):
            wfof = (wfof,)

        def _missing_wf_rel(etype):
            return f"missing workflow relations, see make_workflowable({etype})"

        for etype in wfof:
            eschema = self.repo.schema[etype]
            if ensure_workflowable:
                assert "in_state" in eschema.subject_relations, _missing_wf_rel(etype)
                assert "custom_workflow" in eschema.subject_relations, _missing_wf_rel(
                    etype
                )
                assert "wf_info_for" in eschema.object_relations, _missing_wf_rel(etype)
            rset = self.rqlexec(
                "SET X workflow_of ET WHERE X eid %(x)s, ET name %(et)s",
                {"x": wf.eid, "et": etype},
                ask_confirm=False,
            )
            assert rset, f"unexistant entity type {etype}"
            if default:
                self.rqlexec(
    ...
    def cmd_get_workflow_for(self, etype):
        ...
        rset = self.rqlexec(
            "Workflow X WHERE ET default_workflow X, ET name %(et)s", {"et": etype}
        )
    ...
    def cmd_storage_changed(self, etype, attribute):
        ...
        rset = self.rqlexec(f"Any X WHERE X is {etype}", ask_confirm=False)
    ...
    def cmd_update_etype_fti_weight(self, etype, weight):
        if self.repo.system_source.dbdriver == "postgres":
            self.sqlexec(
                "UPDATE appears SET weight=%(weight)s "
                "FROM entities as X "
    ...
    def cmd_update_bfss_path(self, old_path, new_path, commit=True):
        ...
        """
        changes = []
        for f_eid, fspath in self.rqlexec("Any F, FSPATH(D) WHERE F is File, F data D"):
    ...
    @contextmanager
    def cmd_dropped_constraints(
        self, etype, attrname, cstrtype=None, droprequired=False
    ):
        ...
        """
        rdef = self.fs_schema.entity_schema_for(etype).relation_definition(attrname)
        original_constraints = rdef.constraints
        # remove constraints
        if cstrtype:
            rdef.constraints = [
                cstr
                for cstr in original_constraints
    ...
    def sqlexec(self, sql, args=None, ask_confirm=True):
        ...
        level actions
        """
        if not ask_confirm or self.confirm(f"Execute sql: {sql} ?"):
            try:
    ...
    def rqlexec(self, rql, kwargs=None, build_descr=True, ask_confirm=False):
        ...
        for rql, kwargs in rql:
            if kwargs:
                ...
                msg = rql
            if not ask_confirm or self.confirm(f"Execute rql: {msg} ?"):
                try:
    ...
    def cmd_change_attribute_type(self, etype, attr, newtype, commit=True):
        ...
        * the actual schema won't be updated until next startup
        """
        rschema = self.repo.schema.relation_schema_for(attr)
        oldschema = rschema.objects(etype)[0]
        rdef = rschema.relation_definition(etype, oldschema)
        sql = (
            "UPDATE cw_CWAttribute "
            "SET cw_to_entity=(SELECT cw_eid FROM cw_CWEType WHERE cw_name='%s')"
            "WHERE cw_eid=%s"
        ) % (newtype, rdef.eid)
        self.sqlexec(sql, ask_confirm=False)
        dbhelper = self.repo.system_source.dbhelper
        newrdef = self.fs_schema.relation_schema_for(attr).relation_definition(
            etype, newtype
        )
        ...
        dbhelper.change_col_type(
            cursor, f"cw_{etype}", f"cw_{attr}", sqltype, allownull
        )
        if commit:
            self.commit()
            ...
            del rschema.relation_definitions[(eschema, oldschema)]
            rschema.relation_definitions[(eschema, newschema)] = rdef
    ...
    def cmd_add_entity_type_table(self, etype, commit=True):
        ...
        """
        dbhelper = self.repo.system_source.dbhelper
        for sql in eschema2sql(
            dbhelper, self.repo.schema.entity_schema_for(etype), prefix=SQL_PREFIX
        ):
            self.sqlexec(sql)
        if commit:
            self.commit()
    ...
    def cmd_add_relation_type_table(self, rtype, commit=True):
        ...
        and a sql database
        """
        for sql in rschema2sql(self.repo.schema.relation_schema_for(rtype)):
            self.sqlexec(sql)
        if commit:
            self.commit()
...
class ForRqlIterator:
    ...
    def _get_rset(self):
        ...
        if kwargs:
            ...
            msg = rql
        if self.ask_confirm:
            if not self._h.confirm(f"Execute rql: {msg} ?"):
                raise StopIteration
        try:
