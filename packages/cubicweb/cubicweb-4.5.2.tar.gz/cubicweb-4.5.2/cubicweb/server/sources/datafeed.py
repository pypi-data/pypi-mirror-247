...
from cubicweb import ObjectNotFound, SourceException, _
...
class DataFeedSource(AbstractSource):
    ...
    options = (
        (
            "synchronize",
            {
                ...
                "default": True,
                "help": (
                    "Is the repository responsible to automatically import "
        ...
        (
            "synchronization-interval",
            {
                ...
                "default": "5min",
                "help": (
                    "Interval in seconds between synchronization with the "
        ...
        (
            "max-lock-lifetime",
            {
                ...
                "default": "1h",
                "help": (
                    "Maximum time allowed for a synchronization to be run. "
        ...
        (
            "delete-entities",
            {
                ...
                "default": False,
                "help": (
                    "Should already imported entities not found anymore on the "
        ...
        (
            "use-cwuri-as-url",
            {
                ...
                "help": (
                    "Use cwuri (i.e. external URL) for link to the entity "
    ...
    def init(self, source_entity):
        ...
        self.synchro_interval = timedelta(
            seconds=typed_config["synchronization-interval"]
        )
    ...
    def update_latest_retrieval(self, cnx):
        ...
        cnx.execute(
            "SET X latest_retrieval %(date)s WHERE X eid %(x)s",
    ...
    def release_synchronization_lock(self, cnx):
        cnx.execute("SET X in_synchronization NULL WHERE X eid %(x)s", {"x": self.eid})
        cnx.commit()
    ...
    def pull_data(self, cnx, force=False, raise_on_error=False, sync=True, **kwargs):
        ...
        try:
            if kwargs.get("async") is not None:
                warn(
                    "[3.27] `async` is reserved keyword in py3.7 use `sync` param instead",
                    DeprecationWarning,
                )
    ...
    def _async_pull_data(self, cnx, force, raise_on_error):
        ...
        def _synchronize_source(repo, source_eid, import_log_eid):
            with repo.internal_cnx() as cnx:
                ...
                source._pull_data(
                    cnx, force, raise_on_error, import_log_eid=import_log_eid
                )
    ...
    def _pull_data(self, cnx, force=False, raise_on_error=False, import_log_eid=None):
        ...
        try:
            ...
            importlog.record_error(msg % (f'source "{self.uri}"'))
            self.error(msg, self)
            stats = {}
        else:
        ...
        if stats.get("created"):
            importlog.record_info(f"added {len(stats['created'])} entities")
        if stats.get("updated"):
            importlog.record_info(f"updated {len(stats['updated'])} entities")
...
class DataFeedParser(AppObject):
    ...
    def retrieve_url(self, url):
        ...
        * other will be considered as plain content, useful for testing purpose
        ...
        try:
            ...
            self.source.info(f"Using cwclientlib for {url}")
            resp = cnx.get(url)
        ...
        for url in urls:
            ...
            try:
                ...
                self.import_log.record_error(
                    f"could not pull data while processing {url}: {exc}"
                )
                error = True
                ...
                self.exception("error while processing %s: %s", url, exc)
                error = True
        return error
    ...
    def update_if_necessary(self, entity, attrs):
        ...
        if not mdate or mdate > entity.modification_date:
            attrs = dict((k, v) for k, v in attrs.items() if v != getattr(entity, k))
            if attrs:
                entity.cw_set(**attrs)
...
class DataFeedXMLParser(DataFeedParser):
    ...
    def is_deleted(self, extid, etype, eid):
        ...
        try:
            ...
            cnx = cwproxy_for(url)
            cnx.timeout = self.source.http_timeout
            self.source.info(f"Using cwclientlib for checking {url}")
