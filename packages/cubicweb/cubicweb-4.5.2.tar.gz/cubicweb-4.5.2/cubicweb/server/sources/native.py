...
def sql_or_clauses(sql, clauses):
    ...
    if restrclauses:
        restr = f"{' AND '.join(restrclauses)} AND ({' OR '.join(clauses)})"
    else:
        restr = f"({' OR '.join(clauses)})"
    return f"{select} WHERE {restr}"
...
def rdef_table_column(rdef):
    ...
    the database
    """
    return (SQL_PREFIX + str(rdef.subject), SQL_PREFIX + str(rdef.relation_type))
...
def _undo_check_relation_target(tentity, rdef, role):
    ...
    if card in "?1" and tentity.related(rdef.relation_type, role):
        msg = tentity._cw._(
            "Can't restore %(role)s relation %(rtype)s to entity %(eid)s which "
            "is already linked using this relation."
        )
        raise _UndoException(
            msg
            % {"role": neg_role(role), "rtype": rdef.relation_type, "eid": tentity.eid}
        )
...
def _undo_rel_info(cnx, subj, rtype, obj):
    ...
    for role, eid in (("subject", subj), ("object", obj)):
        try:
            ...
            raise _UndoException(
                msg % {"role": cnx._(role), "rtype": cnx._(rtype), "eid": eid}
            )
    sentity, oentity = entities
    try:
        rschema = cnx.vreg.schema.relation_schema_for(rtype)
        rdef = rschema.relation_definitions[(sentity.cw_etype, oentity.cw_etype)]
    except KeyError:
        msg = cnx._(
        ...
        raise _UndoException(msg % {"rtype": cnx._(rtype), "subj": subj, "obj": obj})
...
AND (EXISTS(SELECT 1 FROM tx_entity_actions AS TEA
     ...
     OR EXISTS(SELECT 1 FROM tx_relation_actions as TRA
        ...
        % {"txuuid": cnx.transaction_data["undoing_uuid"], "eid": eid}
    ).fetchone()


class DefaultEidGenerator:
    ...
    def _create_eid(self, count):
        ...
        try:
            cursor = cnx.cursor()
            for sql in source.dbhelper.sqls_increment_numrange(
                "entities_id_seq", count
            ):
                cursor.execute(sql)
            ...
            self.cnx = None
            source.exception(
                "create eid failed in an unforeseen way on SQL statement %s", sql
            )
            raise
        else:
            cnx.commit()
...
class NativeSQLSource(SQLAdapterMixIn, AbstractSource):
    ...
    options = (
        ...
        (
            ...
            {
                "type": "string",
                "default": CubicWebConfiguration.mode == "user"
                and getlogin()
                or "cubicweb",
                "help": "database user",
                "group": "native-source",
    ...
    def __init__(self, repo, source_config, *args, **kwargs):
        ...
        # sql generator
        self._rql_sqlgen = rql2sql.SQLGenerator(
            self.schema, self.dbhelper, ATTR_MAP.copy()
        )
        # full text index helper
        self.do_fti = not repo.config["delay-full-text-indexation"]
        # sql queries cache
        self._cache = cachetools.LFUCache(repo.config["rql-cache-size"])
    ...
    def check_config(self, source_entity):
        if source_entity.config:
            msg = _(
                "Configuration of the system source goes to "
                "the 'sources' file, not in the database"
            )
            raise ValidationError(
                source_entity.eid, {role_name("config", "subject"): msg}
            )
    ...
    def clear_caches(self, eids, etypes):
        ...
        if eids is None:
            ...
            for eid, etype in zip(eids, etypes):
                cache.pop(f"Any X WHERE X eid {eid}", None)
                cache.pop(f"Any {eid}", None)
                if etype is not None:
                    cache.pop(f"Any X WHERE X eid {eid}, X is {etype}", None)
    ...
    def map_attribute(self, etype, attr, cb, sourcedb=True):
        self._rql_sqlgen.attr_map[f"{etype}.{attr}"] = (cb, sourcedb)

    def unmap_attribute(self, etype, attr):
        self._rql_sqlgen.attr_map.pop(f"{etype}.{attr}", None)

    def set_storage(self, etype, attr, storage):
        storage_dict = self._storages.setdefault(etype, {})
        storage_dict[attr] = storage
        self.map_attribute(etype, attr, storage.callback, storage.is_source_callback)
    ...
    def storage(self, etype, attr):
        ...
        try:
            ...
            raise Exception(f"no custom storage set for {etype}.{attr}")
    ...
    def compile_rql(self, rql, sols):
        ...
        set_qdata(self.schema.relation_schema_for, rqlst, ())
        return rqlst

    def set_schema(self, schema):
        """set the instance'schema"""
        self._cache = cachetools.LFUCache(self.repo.config["rql-cache-size"])
    ...
    def syntax_tree_search(
        self, cnx, union, args=None, cachekey=None, rql_query_tracing_token=None
    ):
            ...
            for entity in entities:
                with self._fixup_cw(cnx, entity):
                    if event == "deleted":
                        ...
                        if attr in edited:
                            handler = getattr(storage, f"entity_{event}")
    ...
    def add_entity(self, cnx, entity):
        ...
        with self._storage_handler(cnx, entity, "added"):
            ...
            if cnx.ertype_supports_undo(entity.cw_etype):
                self._record_tx_action(
                    cnx, "tx_entity_actions", "C", etype=entity.cw_etype, eid=entity.eid
                )
    ...
    def update_entity(self, cnx, entity):
        ...
        with self._storage_handler(cnx, entity, "updated"):
            ...
            sql = self.sqlgen.update(SQL_PREFIX + entity.cw_etype, attrs, ["cw_eid"])
            self.doexec(cnx, sql, attrs)
    ...
    def delete_entity(self, cnx, entity):
        ...
        with self._storage_handler(cnx, entity, "deleted"):
            if cnx.ertype_supports_undo(entity.cw_etype):
                attrs = [
                    SQL_PREFIX + r.type
                    for r in entity.e_schema.subject_relations.values()
    ...
    def _add_relations(self, cnx, rtype, subj_obj_list, inlined=False):
        ...
        if inlined is False:
            attrs = [
                {"eid_from": subject, "eid_to": object}
                for subject, object in subj_obj_list
            ]
            sql.append((self.sqlgen.insert(f"{rtype}_relation", attrs[0]), attrs))
            ...
            for subj_etype, subj_obj_list in etypes.items():
                attrs = [
                    {"cw_eid": subject, SQL_PREFIX + rtype: object}
                    for subject, object in subj_obj_list
                ]
                sql.append(
                    (
                        self.sqlgen.update(SQL_PREFIX + etype, attrs[0], ["cw_eid"]),
                        attrs,
                    )
                )
    ...
    def delete_relation(self, cnx, subject, rtype, object):
        ...
        rschema = self.schema.relation_schema_for(rtype)
        ...
        if (
            rschema.inlined
            and cnx.deleted_in_transaction(subject)
    ...
    def _delete_relation(self, cnx, subject, rtype, object, inlined=False):
        ...
        if inlined:
            ...
            sql = f"UPDATE {table} SET {column}=NULL WHERE {SQL_PREFIX}eid=%(eid)s"
            attrs = {"eid": subject}
        else:
            attrs = {"eid_from": subject, "eid_to": object}
            sql = self.sqlgen.delete(f"{rtype}_relation", attrs)
        self.doexec(cnx, sql, attrs)

    def doexec(
        self, cnx, query, args=None, rollback=True, rql_query_tracing_token=None
    ):
        """Execute a query.
        it's a function just so that it shows up in profiling
        """
        ...
        try:
            ...
            if self.repo.config.mode != "test":
                ...
                # db schema
                self.info(
                    "sql: %r\n args: %s\ndbms message: %r", query, args, ex.args[0]
                )
            if rollback:
                try:
                    cnx.cnxset.rollback()
            ...
            if any(
                ...
                for arg in ex.args:
                    ...
                    if mo is not None:  # sqlite in use
                        ...
                        rtypes = [c.strip()[3:] for c in mo.group(1).split(",")]
                    ...
                    if mo is not None:
                        # postgresql
                        raise ViolatedConstraint(cnx, cstrname=mo.group(0), query=query)
                    if arg.startswith("CHECK constraint failed:"):
                        # sqlite3 (new)
                        raise ViolatedConstraint(
                            cnx, cstrname=arg.split(":", 1)[1].strip(), query=query
                        )
                    mo = re.match("^constraint (cstr.*) failed$", arg)
                    if mo is not None:
                        # sqlite3 (old)
                        raise ViolatedConstraint(cnx, cstrname=mo.group(1), query=query)
            ex.args = (
                "when doing the query '%s' with the args '%s' got the error '%s'"
    ...
    def update_rdef_column(self, cnx, rdef):
        ...
        if not self.dbhelper.alter_column_support:
            self.error(
                "backend can't alter %s.%s to %s%s",
                table,
                column,
                coltype,
            ...
            return
        self.dbhelper.change_col_type(
            LogCursor(cnx.cnxset.cu), table, column, coltype, allownull
        )
        self.info(
            "altered %s.%s: now %s%s",
            table,
    ...
    def update_rdef_null_allowed(self, cnx, rdef):
        ...
        table, column = rdef_table_column(rdef)
        coltype, allownull = rdef_physical_info(self.dbhelper, rdef)
        self.dbhelper.set_null_allowed(
            LogCursor(cnx.cnxset.cu), table, column, coltype, allownull
        )
    ...
    def eid_type(self, cnx, eid):  # pylint: disable=E0202
        ...
        sql = f"SELECT type FROM entities WHERE eid={eid}"
        try:
    ...
    def add_info(self, cnx, entity, source):
        ...
        self._handle_insert_entity_sql(
            cnx, self.sqlgen.insert("entities", attrs), attrs
        )
    ...
    def undoable_transactions(self, cnx, ueid=None, **actionfilters):
        ...
        if ueid is not None:
            restr["tx_user"] = ueid
        sql = self.sqlgen.select(
            "transactions", restr, ("tx_uuid", "tx_time", "tx_user")
        )
        if actionfilters:
            ...
            for key, val in actionfilters.items():
                if key == "etype":
                    ...
                    raise AssertionError(f"unknow filter {key}")
            ...
            if trarestr or (genrestr and not tearestr):
                ...
                if "eid_from" in trarestr:
                    ...
                    trasql = sql_or_clauses(
                        trasql, ["eid_from = %(eid_from)s", "eid_to = %(eid_to)s"]
                    )
                trasql += " AND transactions.tx_uuid=tx_relation_actions.tx_uuid"
                subqsqls.append(f"EXISTS({trasql})")
            if tearestr or (genrestr and not trarestr):
                ...
                subqsqls.append(f"EXISTS({teasql})")
            if restr:
                sql += f" AND {' OR '.join(subqsqls)}"
            else:
                sql += f" WHERE {' OR '.join(subqsqls)}"
    ...
    def tx_actions(self, cnx, txuuid, public):
        ...
        sql = self.sqlgen.select(
            "tx_entity_actions",
            restr,
            ("txa_action", "txa_public", "txa_order", "etype", "eid", "changes"),
        )
        cu = self.doexec(cnx, sql, restr)
        actions = [
            tx.EntityAction(a, p, o, et, e, c and pickle.loads(self.binary_to_str(c)))
            for a, p, o, et, e, c in cu.fetchall()
        ]
        sql = self.sqlgen.select(
            "tx_relation_actions",
            restr,
            ("txa_action", "txa_public", "txa_order", "rtype", "eid_from", "eid_to"),
        )
        cu = self.doexec(cnx, sql, restr)
        ...
        with cnx.deny_all_hooks_but("integrity", "activeintegrity", "undo"):
            with cnx.security_enabled(read=False):
                for action in reversed(self.tx_actions(cnx, txuuid, False)):
                    undomethod = getattr(self, f"_undo_{action.action.lower()}")
                    errors += undomethod(cnx, action)
        # remove the transactions record
        self.doexec(cnx, f"DELETE FROM transactions WHERE tx_uuid='{txuuid}'")
        if errors:
    ...
    def _save_attrs(self, cnx, entity, attrs):
        ...
        for column in attrs:
            # [3:] remove 'cw_' prefix
            attr = column[3:]
            if not eschema.subject_relations[attr].final:
                continue
    ...
    def _reedit_entity(self, entity, changes, err):
        ...
        eschema = entity.e_schema
        getrschema = eschema.subject_relations
        for column, value in changes.items():
            ...
            try:
                ...
                err(
                    cnx._(
                        "can't restore relation %(rtype)s of entity %(eid)s, "
            ...
            if not rschema.final:
                ...
                # rschema is an inlined relation
                    ...
                    try:
                        ...
                        err(
                            cnx._(
                                "can't restore entity %(eid)s of type %(eschema)s, "
    ...
    def _undo_d(self, cnx, action):
        ...
        try:
            ...
            err(f"can't restore entity {eid} of type {etype}, type no more supported")
            return errors
    ...
    def _undo_r(self, cnx, action):
        ...
        try:
            ...
            for role, entity in (("subject", sentity), ("object", oentity)):
                try:
                    ...
                    continue
        if not errors:
            self.repo.hm.call_hooks(
                "before_add_relation", cnx, eidfrom=subj, rtype=rtype, eidto=obj
            )
            # add relation in the database
            self._add_relations(cnx, rtype, [(subj, obj)], rdef.relation_type.inlined)
            # set related cache
            cnx.update_rel_cache_add(subj, rtype, obj, rdef.relation_type.symmetric)
            self.repo.hm.call_hooks(
                "after_add_relation", cnx, eidfrom=subj, rtype=rtype, eidto=obj
            )
        return errors
    ...
    def _undo_c(self, cnx, action):
        ...
        self.doexec(cnx, f"DELETE FROM is_relation WHERE eid_from={eid}")
        self.doexec(cnx, f"DELETE FROM is_instance_of_relation WHERE eid_from={eid}")
        self.doexec(cnx, f"DELETE FROM cw_source_relation WHERE eid_from={eid}")
    ...
    def _undo_u(self, cnx, action):
        ...
        try:
            ...
            err(
                cnx._(
                    "can't restore state of entity %s, it has been " "deleted inbetween"
                )
                % action.eid
            )
            return errors
        ...
        sql = self.sqlgen.update(
            SQL_PREFIX + entity.cw_etype, action.changes, ["cw_eid"]
        )
        self.doexec(cnx, sql, action.changes)
    ...
    def _undo_a(self, cnx, action):
        ...
        try:
            ...
            rschema = rdef.relation_type
            if rschema.inlined:
                sql = "SELECT 1 FROM cw_{} WHERE cw_eid={} and cw_{}={}".format(
                    sentity.cw_etype,
                    subj,
                    rtype,
                    obj,
                ...
                sql = (
                    "SELECT 1 FROM {}_relation WHERE eid_from={} and eid_to={}".format(
                        rtype,
                        subj,
                        obj,
                    )
        ...
        if not errors:
            self.repo.hm.call_hooks(
                "before_delete_relation", cnx, eidfrom=subj, rtype=rtype, eidto=obj
            )
            ...
            self.repo.hm.call_hooks(
                "after_delete_relation", cnx, eidfrom=subj, rtype=rtype, eidto=obj
            )
        return errors
    ...
    def need_fti_indexation(self, etype):
        eschema = self.schema.entity_schema_for(etype)
    ...
    def fti_index_entities(self, cnx, entities):
        ...
        try:
            ...
            for entity in entities:
                cursor_index_object(
                    entity.eid, entity.cw_adapt_to("IFTIndexable"), cursor
                )
        except Exception:  # let KeyboardInterrupt / SystemExit propagate
...
def sql_schema(driver):
    ...
    yield from helper.sql_create_numrange("entities_id_seq").split(";")
    yield from (
        """
...
def grant_schema(user, set_owner=True):
    ...
    for table in (
        ...
        if set_owner:
            yield f"ALTER TABLE {table} OWNER TO {user};"
        yield f"GRANT ALL ON {table} TO {user};"


class BaseAuthentifier:
...
class LoginPasswordAuthentifier(BaseAuthentifier):
    ...
    auth_rql = (
        "Any X WHERE X is CWUser, X login %(login)s, X upassword %(pwd)s, "
    ...
    def authenticate(self, cnx, login, password=None, **kwargs):
        ...
        try:
            ...
            if pwd is not None and pwd.getvalue():
                ...
                if newhash:
                    cnx.system_sql(
                        "UPDATE %s SET %s=%%(newhash)s WHERE %s=%%(login)s"
                        % (
                            SQL_PREFIX + "CWUser",
...
class EmailPasswordAuthentifier(BaseAuthentifier):
    def authenticate(self, cnx, login, **authinfo):
        ...
        rset = cnx.execute(
            "Any L WHERE U login L, U primary_email M, " "M address %(login)s",
            {"login": login},
            build_descr=False,
        )
        if rset.rowcount != 1:
...
class DatabaseIndependentBackupRestore:
    ...
    * a list of rows (as tuples with one element per column)
    ...
    def write_sequence(self, archive, seq):
        ...
        archive.writestr(f"sequences/{seq}", serialized)
    ...
    def write_numrange(self, archive, numrange):
        ...
        archive.writestr(f"numrange/{numrange}", serialized)

    def write_table(self, archive, table):
        nb_lines_sql = f"SELECT COUNT(*) FROM {table}"
        ...
        if rowcount > 0:
            for i, start in enumerate(range(0, rowcount, blocksize)):
                ...
                self.logger.debug(
                    "wrote rows %d to %d (out of %d) to %s.%04d",
                    start,
                    start + len(rows) - 1,
                    rowcount,
                    table,
    ...
    def restore(self, backupfile):
        ...
        self.cnx = self.get_connection()
        self.cursor = self.cnx.cursor()
        sequences, numranges, tables, table_chunks = self.read_metadata(
            archive, backupfile
        )
    ...
    def read_metadata(self, archive, backupfile):
        ...
        if formatinfo.strip() != "1.1":
            ...
            raise ValueError(f"Unknown format in {backupfile}: {formatinfo}")
        ...
        if archive_versions != db_versions:
            ...
            if new_cubes:
                self.logger.critical(
                    "In the db:\n%s",
                    "\n".join(f"{cube}: {ver}" for cube, ver in sorted(new_cubes)),
                )
            old_cubes = archive_versions - db_versions
            if old_cubes:
                self.logger.critical(
                    "In the archive:\n%s",
                    "\n".join(f"{cube}: {ver}" for cube, ver in sorted(old_cubes)),
                )
    ...
    def read_sequence(self, archive, seq):
        seqname, columns, rows = pickle.loads(archive.read(f"sequences/{seq}"))
        assert seqname == seq
        assert len(rows) == 1
        assert len(rows[0]) == 1
        value = rows[0][0]
    ...
    def read_numrange(self, archive, numrange):
        rangename, columns, rows = pickle.loads(archive.read(f"numrange/{numrange}"))
    ...
    def read_table(self, archive, table, filenames):
        ...
        for filename in filenames:
            ...
            if not rows:
                continue
            insert = self.sql_generator.insert(table, dict(zip(columns, rows[0])))
            for row in rows:
    ...
    def _get_versions(self):
        ...
        for pkey, value in self.cursor.fetchall():
            if pkey.startswith("system.version"):
