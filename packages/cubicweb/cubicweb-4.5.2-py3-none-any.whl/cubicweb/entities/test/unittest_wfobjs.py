    ...
    def test_duplicated_transition(self):
        with self.admin_access.shell() as shell:
            ...
            self.assertEqual(
                {
                    "name": "%(KEY-rtype)s is part of violated unicity constraint",
                    "transition_of": "%(KEY-rtype)s is part of violated unicity constraint",
                    "": "some relations violate a unicity constraint",
                },
                cm.exception.errors,
            )
        ...
        with self.admin_access.repo_cnx() as cnx:
            with cnx.security_enabled(write=False):
                ...
                self.assertEqual(
                    cm.exception.errors,
                    {
                        "in_state-subject": "state doesn't belong to entity's workflow. "
    ...
    def test_swf_exit_consistency(self):
        with self.admin_access.shell() as shell:
            ...
            self.assertEqual(
                cm.exception.errors,
                {
                    "subworkflow_exit-subject": "can't have multiple exits on the same state"
                },
            )
