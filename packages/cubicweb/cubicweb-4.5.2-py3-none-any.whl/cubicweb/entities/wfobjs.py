...
class Workflow(AnyEntity):
    ...
    def is_default_workflow_of(self, etype):
        ...
        entity type
        """
        return any(et for et in self.reverse_default_workflow if et.name == etype)
    ...
    def iter_workflows(self, _done=None):
        ...
        yield self
        _done.add(self.eid)
        for tr in self._cw.execute(
            "Any T WHERE T is WorkflowTransition, " "T transition_of WF, WF eid %(wf)s",
            {"wf": self.eid},
        ).entities():
    ...
    def state_by_name(self, statename):
        rset = self._cw.execute(
            "Any S, SN WHERE S name SN, S name %(n)s, " "S state_of WF, WF eid %(wf)s",
    ...
    def state_by_eid(self, eid):
        rset = self._cw.execute(
            "Any S, SN WHERE S name SN, S eid %(s)s, " "S state_of WF, WF eid %(wf)s",
            {"s": eid, "wf": self.eid},
        )
        if rset:
            return rset.get_entity(0, 0)
    ...
    def transition_by_name(self, trname):
        rset = self._cw.execute(
            "Any T, TN WHERE T name TN, T name %(n)s, "
    ...
    def transition_by_eid(self, eid):
        rset = self._cw.execute(
            "Any T, TN WHERE T name TN, T eid %(t)s, "
            "T transition_of WF, WF eid %(wf)s",
            {"t": eid, "wf": self.eid},
        )
        if rset:
    ...
    def add_state(self, name, initial=False, **kwargs):
        ...
        self._cw.execute(
            "SET S state_of WF WHERE S eid %(s)s, WF eid %(wf)s",
            {"s": state.eid, "wf": self.eid},
        )
        if initial:
            assert not self.initial, f"Initial state already defined as {self.initial}"
            self._cw.execute(
                "SET WF initial_state S " "WHERE S eid %(s)s, WF eid %(wf)s",
                {"s": state.eid, "wf": self.eid},
            )
        return state
    ...
    def _add_transition(
        ...
        self._cw.execute(
            "SET T transition_of WF " "WHERE T eid %(t)s, WF eid %(wf)s",
        ...
        for state in fromstates:
            if hasattr(state, "eid"):
                state = state.eid
            self._cw.execute(
                "SET S allowed_transition T " "WHERE S eid %(s)s, T eid %(t)s",
                {"s": state, "t": tr.eid},
            )
    ...
    def add_transition(
        self, name, fromstates, tostate=None, requiredgroups=(), conditions=(), **kwargs
    ):
        """add a transition to this workflow from some state(s) to another"""
        tr = self._add_transition(
            "Transition", name, fromstates, requiredgroups, conditions, **kwargs
        )
        if tostate is not None:
            if hasattr(tostate, "eid"):
                tostate = tostate.eid
            self._cw.execute(
                "SET T destination_state S " "WHERE S eid %(s)s, T eid %(t)s",
                {"t": tr.eid, "s": tostate},
            )
        return tr

    def add_wftransition(
        self,
        name,
        subworkflow,
        fromstates,
        ...
        **kwargs,
        ...
        tr = self._add_transition(
            "WorkflowTransition", name, fromstates, requiredgroups, conditions, **kwargs
        )
        if hasattr(subworkflow, "eid"):
            subworkflow = subworkflow.eid
        assert self._cw.execute(
            "SET T subworkflow WF WHERE WF eid %(wf)s,T eid %(t)s",
    ...
    def replace_state(self, todelstate, replacement):
        ...
        execute(
            "SET X in_state NS WHERE X in_state OS, " "NS eid %(ns)s, OS eid %(os)s",
            args,
        )
        execute(
            "SET X from_state NS WHERE X from_state OS, "
            "OS eid %(os)s, NS eid %(ns)s",
            args,
        )
        execute(
            "SET X to_state NS WHERE X to_state OS, " "OS eid %(os)s, NS eid %(ns)s",
            args,
        )
...
class BaseTransition(AnyEntity):
    ...
    def may_be_fired(self, eid):
        ...
        if groups:
            ...
            if matches:
                if DBG:
                    print(
                        f"may_be_fired: {self.name!r} may fire: user matches {groups}"
                    )
                return matches
            if "owners" in groups and user.owns(eid):
                if DBG:
                    print(f"may_be_fired: {self.name!r} may fire: user is owner")
                return True
    ...
    def set_permissions(self, requiredgroups=(), conditions=(), reset=True):
        ...
        if reset:
            self._cw.execute(
                "DELETE T require_group G WHERE T eid %(x)s", {"x": self.eid}
            )
            self._cw.execute("DELETE T condition R WHERE T eid %(x)s", {"x": self.eid})
        for gname in requiredgroups:
            rset = self._cw.execute(
                "SET T require_group G " "WHERE T eid %(x)s, G name %(gn)s",
                {"x": self.eid, "gn": gname},
            )
            assert rset, f"{gname} is not a known group"
...
class WorkflowTransition(BaseTransition):
    ...
    def add_exit_point(self, fromstate, tostate):
        ...
        if tostate is None:
            self._cw.execute(
                "INSERT SubWorkflowExitPoint X: T subworkflow_exit X, "
    ...
    def get_exit_point(self, entity, stateeid):
        ...
        if tostateeid is None:
            ...
            return (
                entity.cw_adapt_to("IWorkflowable")
                .subworkflow_input_trinfo()
                .previous_state
            )
...
class TrInfo(AnyEntity):
    ...
    fetch_attrs, cw_fetch_order = fetch_config(
        ["creation_date", "comment"], pclass=None
    )  # don't want modification_date
...
class IWorkflowableAdapter(EntityAdapter):
    ...
    def subworkflow_input_trinfo(self):
        ...
        for trinfo in self.workflow_history:
            if (
                trinfo.transition
                and trinfo.previous_state.workflow.eid != trinfo.new_state.workflow.eid
            ):
                # entering or leaving a subworkflow
                if (
                    subwfentries
                    and subwfentries[-1].new_state.workflow.eid
    ...
    def _get_transition(self, tr):
        ...
        if isinstance(tr, str):
            ...
            assert _tr is not None, f"not a {self.__regid__} transition: {tr}"
            tr = _tr
        return tr
    ...
    def fire_transition_if_possible(self, tr, comment=None, commentformat=None):
        ...
        """
        tr = self._get_transition(tr)
        if any(tr_ for tr_ in self.possible_transitions() if tr_.eid == tr.eid):
    ...
    def change_state(self, statename, comment=None, commentformat=None, tr=None):
        ...
        if hasattr(statename, "eid"):
            ...
            if state is None:
                raise WorkflowException(f"not a {self.__regid__} state: {statename}")
            stateeid = state.eid
