...
def wsgi_application_from_cwconfig(
    ...
    if debugtoolbar:
        try:
            ...
            logging.exception(
                "Error: you tried to activate the debug toolbar but you have missing "
                "dependencies: %s" % e
            )
            ...
            config.add_route(
                "debug_display_source_code", DEBUG_DISPLAY_SOURCE_CODE_PATH
            )
            config.add_view(
                debug_display_source_code, route_name="debug_display_source_code"
            )
...
def includeme(config):
    ...
    if cwconfig is None:
        debugmode = asbool(config.registry.settings.get("cubicweb.debug", False))
    ...
    if (
        "cubicweb.pyramid.rest_api.include_html" in cubicweb_includes
        or "cubicweb.pyramid.rest_api.include_rdf" in cubicweb_includes
    ):
        config.include("cubicweb.pyramid.rest_api")

    for name in cubicweb_includes:
        config.include(name)
    ...
    config.set_default_csrf_options(
        require_csrf=asbool(
            config.registry.settings.get("cubicweb.pyramid.enable_csrf", "yes")
        )
    )
