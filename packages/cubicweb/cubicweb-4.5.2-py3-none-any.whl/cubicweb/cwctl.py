...
from cubicweb import ConfigurationError, ExecutionError, BadCommandUsage, utils, repoapi
from cubicweb.cwconfig import (
    CubicWebConfiguration as cwcfg,
    instance_configuration,
)
from cubicweb.server import set_debug
from cubicweb.server.repository import Repository, get_cnxset
...
CWCTL = CommandLine(
    ...
    version=cw_version,
    check_duplicated_command=False,
)
...
def wait_process_end(pid, maxtry=10, waittime=1):
    ...
    while nbtry < maxtry:
        ...
        raise ExecutionError(f"can't kill process {pid}")
...
class InstanceCommand(Command):
    ...
    @property
    def _cnx(self):
        return repoapi.get_repository(config=self.cwconfig).internal_cnx()
    ...
    def run(self, args):
        ...
        cmdmeth = getattr(self, f"{self.name}_instance")
        ...
        try:
            ...
            sys.stderr.write(f"instance {appid} not {self.actionverb}: {ex}\n")
            status = 4
            ...
            sys.stderr.write(f"{self.name} aborted\n")
        ...
        if status != 0 and self.config.pdb:
            ...
            if traceback_ is not None:
                ...
                print(
                    "WARNING: Could not access to the traceback because the command return "
...
class ListCommand(Command):
    ...
    List available installed cubes and registered instances.

    If given, the optional argument allows to restrict listing only a category of items.
    """

    name = "list"
    arguments = "[all|cubes|instances]"
    options = (
        (
            "verbose",
            {"short": "v", "action": "store_true", "help": "display more information."},
        ),
    )
    ...
    def run(self, args):
        ...
        if mode == "all":
            print(f"CubicWeb {cwcfg.cubicweb_version()} ({cwcfg.mode} mode)")
            print()
            ...
            try:
                ...
                for cube in cube_names:
                    try:
                        ...
                        tinfo = None
                        tversion = f"[missing cube information: {ex}]"
                    print(f"* {cube.ljust(namesize)} {tversion}")
        ...
        if mode in ("all", "instances"):
            ...
            if instances:
                print(f"Available instances ({regdir}):")
                for appid in instances:
                    print(f"* {appid}")
            else:
                print(f"No instance available in {regdir}")
            print()
            ...
            if cfgpb.errors:
                ...
                for op, cube, version, src in cfgpb.errors:
                    if op == "add":
                        ...
                        print(f"is not installed, but required by {src}")
                    else:
                        print(
...
def check_options_consistency(config):
    if config.automatic and config.config_level > 0:
        raise BadCommandUsage(
            "--automatic and --config-level should not be " "used together"
        )
...
class CreateInstanceCommand(Command):
    ...
    def run(self, args):
        ...
        try:
            templdirs = [cwcfg.cube_dir(cube) for cube in cubes]
        except ConfigurationError as ex:
            print(ex)
            print("\navailable cubes:", end=" ")
        ...
        print("\n" + underline_title(f"Creating the instance {appid}"))
        create_dir(config.apphome)
        # cubicweb-ctl configuration
        if not self.config.automatic:
            print(
                "\n" + underline_title(f"Configuring the instance ({configname}.conf)")
            )
        ...
        if not self.config.automatic:
            sections = set(
                sect.lower()
                for sect, opt, odict in config.all_options()
                if "type" in odict and odict.get("level", 0) <= self.config.config_level
            )
            for section in sections:
                if section not in ("main", "email", "web"):
                    print("\n" + underline_title(f"{section} options"))
                    config.input_config(section, self.config.config_level)
        # write down configuration
        config.save()
        print(f"-> generated config {config.main_config_file()}")
        # handle i18n files structure
        # in the first cube given
        from cubicweb import i18n

        langs = [
            lang for lang, _ in i18n.available_catalogs(join(templdirs[0], "i18n"))
        ]
        errors = config.i18ncompile(langs)
        if errors:
            print("\n".join(errors))
            if self.config.automatic or not ASK.confirm(
                "error while compiling message catalogs, " "continue anyway ?"
            ):
        ...
        if config["uid"]:
            ...
            print(f"set {config['uid']} as owner of the data directory")
            chown(config.appdatahome, config["uid"])
        print(f"\n-> creation done for {repr(config.apphome)[1:-1]}\n")
    ...
    def run(self, args):
        ...
        try:
            ...
            if ex.errno != errno.ENOENT:
                raise
        print(f"-> instance {appid} deleted.")
    ...
    options = InstanceCommand.options + (
        (
            "force-cube-version",
            {
                "short": "t",
                "type": "named",
                "metavar": "cube1:X.Y.Z,cube2:X.Y.Z",
                "default": None,
...
(recommended). If 'all' is given, will connect to all defined sources.",
    ...
    def upgrade_instance(self, appid):
        print("\n" + underline_title(f"Upgrading the instance {appid}"))
        ...
        for cube in config.cubes():
            ...
            try:
                ...
                config.error(f"no version information for {cube}")
                continue
        ...
        if self.config.fs_only or toupgrade:
            for cube, fromversion, toversion in toupgrade:
                print(
                    f"-> migration needed from {fromversion} to {toversion} for {cube}"
                )
            with mih.cnx:
            ...
            print(f"-> no data migration needed for instance {appid}.")
...
class ListVersionsInstanceCommand(InstanceCommand):
    ...
    def versions_instance(self, appid):
        ...
        config = instance_configuration(appid)
        # We need to initialize only a few things to have all cube versions.
        # So we can't use regular methods, since we only need a Repository
        # with a configuration, a connection and a schema.
        # Thus, we'll be able to list all cube versions of the instance,
        # even if some cubes aren't installed, which is impossible
        # if we simply use config.repository().get_versions().
        repo = Repository(config)
        repo.cnxsets = get_cnxset(repo.config, repo.system_source, bootstrap=True)
        repo.set_schema(repo.config.load_bootstrap_schema(), resetvreg=False)
        vcconf = repo.get_versions(checkversions=False)
            ...
            with mih.cnx:
                with mih.cnx.security_enabled(False, False):
                    if args:
                        ...
                        scripts, args = (
                            self.cmdline_parser.largs[1:],
                            self.cmdline_parser.rargs,
                        )
            ...
            for key, value in self.config.param.items():
                try:
                    ...
                    raise ConfigurationError(
                        f'unknown configuration key "{key}" for mode {appcfg.name}'
                    )
            appcfg.save()
