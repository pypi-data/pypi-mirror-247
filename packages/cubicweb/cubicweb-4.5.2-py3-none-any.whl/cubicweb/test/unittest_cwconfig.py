...
from cubicweb.cwconfig import CubicWebConfiguration, _expand_modname
...
def temp_config(appid, instance_dir, cubes):
    ...
    old = (cls._INSTANCES_DIR, sys.path[:], sys.meta_path[:])
...
def iter_entry_points(group):
    """Mock importlib.metadata.entry_points to yield EntryPoint from
    ...
    for pkgname in os.listdir(libpython):
        ...
        yield EntryPoint(pkgname[len(prefix) :], pkgname, dist=Distribution(location))
...
class CubicWebConfigurationTC(BaseTestCase):
    ...
    def test_available_cubes(self, mock_iter_entry_points):
        ...
        mock_iter_entry_points.assert_called_once_with(group="cubicweb.cubes")

    def test_reorder_cubes(self):
        # forge depends on email and file and comment
        # email depends on file
        self.assertEqual(
            self.config.reorder_cubes(["file", "email", "forge"]),
            ("forge", "email", "file"),
        )
        self.assertEqual(
            self.config.reorder_cubes(["email", "file", "forge"]),
            ("forge", "email", "file"),
        )
        self.assertEqual(
            self.config.reorder_cubes(["email", "forge", "file"]),
            ("forge", "email", "file"),
        )
        self.assertEqual(
            self.config.reorder_cubes(["file", "forge", "email"]),
            ("forge", "email", "file"),
        )
        self.assertEqual(
            self.config.reorder_cubes(["forge", "file", "email"]),
            ("forge", "email", "file"),
        )
        self.assertEqual(
            self.config.reorder_cubes(("forge", "email", "file")),
            ("forge", "email", "file"),
        )
    ...
    def _test_reorder_cubes_recommends(self, comment_pkginfo):
        ...
        try:
            ...
            self.assertEqual(
                self.config.reorder_cubes(("forge", "email", "file", "comment")),
                ("forge", "email", "comment", "file"),
            )
            self.assertEqual(
                self.config.reorder_cubes(("forge", "email", "comment", "file")),
                ("forge", "email", "comment", "file"),
            )
            self.assertEqual(
                self.config.reorder_cubes(("forge", "comment", "email", "file")),
                ("forge", "email", "comment", "file"),
            )
            self.assertEqual(
                self.config.reorder_cubes(("comment", "forge", "email", "file")),
                ("forge", "email", "comment", "file"),
            )
        finally:
    ...
    def test_expand_cubes(self):
        self.assertEqual(
            self.config.expand_cubes(("email", "comment")),
            ["email", "comment", "file"],
        )
    ...
    def test_config_value_from_environment_str(self):
        ...
        try:
            self.assertEqual(self.config["base-url"], "https://www.cubicweb.org")
        finally:
...
class ModnamesTC(unittest.TestCase):
    ...
    def test_expand_modnames(self, libdir):
        ...
        self.assertEqual(
            list(_expand_modname("lib.b", recursive=False)),
            [
                ("lib.b", join(tempdir, "b", "__init__.py")),
                ("lib.b.a", join(tempdir, "b", "a.py")),
                ("lib.b.c", join(tempdir, "b", "c.py")),
    ...
    def test_schema_modnames(self, libdir):
        ...
        with temp_config("data1", instance_dir, ("foo",)) as config:
            self.assertEqual(
                pkgutil.find_loader("schema").get_filename(),
                join(libdir, "_instance_dir", "data1", "schema.py"),
            )
            self.assertEqual(config.schema_modnames(), expected + [("data", "schema")])
        # app doesn't have schema file
        with temp_config("data2", instance_dir, ("foo",)) as config:
            self.assertEqual(
                pkgutil.find_loader("schema").get_filename(), join(libdir, "schema.py")
            )
    ...
    def test_appobjects_modnames(self, libdir):
        ...
        expected = [
            ...
            "cubicweb.hooks",
            "cubicweb.hooks.bookmark",
            "cubicweb.hooks.email",
            "cubicweb.hooks.integrity",
            "cubicweb.hooks.metadata",
            "cubicweb.hooks.notification",
            "cubicweb.hooks.security",
            "cubicweb.hooks.synccomputed",
            "cubicweb.hooks.syncschema",
            "cubicweb.hooks.syncsession",
            "cubicweb.hooks.syncsources",
            "cubicweb.hooks.workflow",
            "cubicweb.sobjects",
            "cubicweb.sobjects.ldapparser",
            "cubicweb.sobjects.notification",
            "cubicweb.sobjects.services",
            "cubicweb.sobjects.supervising",
        ...
        # data1 has entities
        with temp_config("data1", instance_dir, ("foo",)) as config:
            config.cube_appobject_path = {"entities", "hooks"}
            self.assertEqual(config.appobjects_modnames(), expected + ["entities"])
        # data2 has hooks
        with temp_config("data2", instance_dir, ("foo",)) as config:
            config.cube_appobject_path = {"entities", "hooks"}
            self.assertEqual(config.appobjects_modnames(), expected + ["hooks"])
