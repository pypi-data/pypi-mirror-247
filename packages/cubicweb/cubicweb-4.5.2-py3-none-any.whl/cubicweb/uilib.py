...
def rql_for_eid(eid):
    ...
    """
    return f"Any X WHERE X eid {eid}"
...
def print_tztime(value, req, props, displaytime=True):
    return ustrftime(value, req.property_value("ui.time-format")) + " UTC"
...
def print_tzdatetime(value, req, props, displaytime=True):
    if displaytime:
        return ustrftime(value, req.property_value("ui.datetime-format")) + " UTC"
    return ustrftime(value, req.property_value("ui.date-format"))
...
def css_em_num_value(vreg, propname, default):
    ...
    if propvalue.endswith("em"):
        try:
            ...
            vreg.warning("css property %s looks malformed (%r)", propname, propvalue)
    else:
        vreg.warning(
            "css property %s should use em (currently is %r)", propname, propvalue
        )
    return default
...
try:
    from cubicweb_web.ext.rest import rest_publish  # pylint: disable=W0611
except ImportError:
...
def _subst_rql(view, obj):
    ...
    return f'<a href="{view._cw.build_url(rql=rql)}">{descr}</a>'
...
def soup2xhtml(data, encoding):
    ...
    xmltree = etree.HTML(CLEANER.clean_html(f"<div>{data}</div>"))
...
class _JSCallArgs(_JSId):
    ...
    def __str__(self):
        ...
        if self.parent:
            return f"{self.parent}({','.join(args)})"
        return ",".join(args)
...
def tooltipize(text, tooltip, url=None):
    ...
    url = url or "#"
    return f'<a href="{url}" title="{tooltip}">{text}</a>'


def toggle_action(nodeid):
    """builds a HTML link that uses the js toggleVisibility function"""
    return f"javascript: toggleVisibility('{nodeid}')"


def toggle_link(nodeid, label):
    """builds a HTML link that uses the js toggleVisibility function"""
    return f'<a href="{toggle_action(nodeid)}">{label}</a>'
...
def rest_traceback(info, exception):
    ...
    for stackentry in traceback.extract_tb(info[2]):
        res.append("\tFile %s, line %s, function %s" % tuple(stackentry[:3]))
        if stackentry[3]:
            data = xml_escape(stackentry[3])
            res.append(f"\t  {data}")
    res.append("\n")
    try:
        res.append(f"\t Error: {exception}\n")
    except Exception:
        pass
    return "\n".join(res)


def html_traceback(info, exception, title="", encoding="ISO-8859-1", body=""):
    ...
    if body:
        ...
        strings.append("</div>")
    if title:
        strings.append(f'<h1 class="error">{xml_escape(title)}</h1>')
    try:
        strings.append(
            '<p class="error">%s</p>'
    ...
    for index, stackentry in enumerate(stacktb):
        strings.append(
            '<b>File</b> <b class="file">%s</b>, <b>line</b> '
        ...
        try:
            ...
            for name, value in local_context.items():
                ...
                info = f'<span class="name">{name}</span>={value}, '
            ...
            strings.append(f"[{toggle_link(boxid, '+')}]")
            strings.append(
                '<div id="%s" class="pycontext hidden">%s</div>'
