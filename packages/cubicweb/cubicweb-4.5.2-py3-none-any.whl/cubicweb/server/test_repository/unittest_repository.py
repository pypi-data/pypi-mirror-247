...
from yams import register_base_type, ValidationError, unregister_base_type
...
class RepositoryTC(CubicWebTC):
    ...
    def test_unique_together_constraint(self):
        with self.admin_access.repo_cnx() as cnx:
            cnx.execute(
                'INSERT Societe S: S nom "Logilab", S type "SSLL", S cp "75013"'
            )
            ...
            self.assertEqual(
                {
                    "cp": "%(KEY-rtype)s is part of violated unicity constraint",
                    "nom": "%(KEY-rtype)s is part of violated unicity constraint",
                    "type": "%(KEY-rtype)s is part of violated unicity constraint",
    ...
    def test_unique_together_schema(self):
        person = self.repo.schema.entity_schema_for("Personne")
        self.assertEqual(len(person._unique_together), 1)
        self.assertCountEqual(person._unique_together[0], ("nom", "prenom", "inline2"))
    ...
    def test_connect(self):
        with self.repo.internal_cnx() as cnx:
            self.assertTrue(
                self.repo.authenticate_user(
                    cnx, self.admlogin, password=self.admpassword
                )
            )
            self.assertRaises(
                AuthenticationError,
            ...
            self.assertRaises(
                AuthenticationError, self.repo.authenticate_user, cnx, self.admlogin
            )
            self.assertRaises(
                AuthenticationError, self.repo.authenticate_user, cnx, None
            )
    ...
    def test_login_upassword_accent(self):
        with self.admin_access.repo_cnx() as cnx:
            cnx.execute(
                ...
                {"login": "barnabé", "passwd": "héhéhé".encode()},
            )
            cnx.commit()
            repo = self.repo
            self.assertTrue(
                repo.authenticate_user(cnx, "barnabé", password="héhéhé".encode())
            )
        ...
        with self.admin_access.repo_cnx() as cnx:
            with self.temporary_appobjects(ValidationErrorAfterHook):
                self.assertRaises(
                    ValidationError,
                    cnx.execute,
                    'SET X name "toto" WHERE X is CWGroup, X name "guests"',
                )
    ...
    def test_initial_schema(self):
        ...
        self.assertListEqual(
            ...
            [
                r.type
                for r in schema.entity_schema_for("CWAttribute").ordered_relations()
                if r.type not in notin
            ],
        )

        self.assertEqual(schema.entity_schema_for("CWEType").main_attribute(), "name")
        self.assertEqual(schema.entity_schema_for("State").main_attribute(), "name")

        constraints = (
            schema.relation_schema_for("name")
            .relation_definition("CWEType", "String")
            .constraints
        )
        ...
        constraints = (
            schema.relation_schema_for("relation_type")
            .relation_definition("CWAttribute", "CWRType")
            .constraints
        )
        ...
        ownedby = schema.relation_schema_for("owned_by")
        self.assertEqual(ownedby.objects("CWEType"), ("CWUser",))

    def test_public_api(self):
        self.assertEqual(self.repo.get_schema(), self.repo.schema)
        self.assertEqual(
            self.repo.source_defs(),
            {"system": {"type": "native", "uri": "system", "use-cwuri-as-url": False}},
        )
        # .properties() return a result set
        self.assertEqual(
            self.repo.properties().rql,
    ...
    def test_delete_if_singlecard1(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            p1 = cnx.create_entity("Personne", nom="toto")
            cnx.execute(
                "SET A todo_by P WHERE A eid %(x)s, P eid %(p)s",
                {"x": note.eid, "p": p1.eid},
            )
            rset = cnx.execute("Any P WHERE A todo_by P, A eid %(x)s", {"x": note.eid})
            self.assertEqual(len(rset), 1)
            p2 = cnx.create_entity("Personne", nom="tutu")
            cnx.execute(
                "SET A todo_by P WHERE A eid %(x)s, P eid %(p)s",
                {"x": note.eid, "p": p2.eid},
            )
    ...
    def test_delete_if_object_inlined_singlecard(self):
        with self.admin_access.repo_cnx() as cnx:
            c = cnx.create_entity("Card", title="Carte")
            cnx.create_entity("Personne", nom="Vincent", fiche=c)
            cnx.create_entity("Personne", nom="Florent", fiche=c)
            cnx.commit()
            self.assertEqual(len(c.reverse_fiche), 1)

    def test_delete_computed_relation_nonregr(self):
        with self.admin_access.repo_cnx() as cnx:
            c = cnx.create_entity("Personne", nom="Adam", login_user=cnx.user.eid)
            cnx.commit()
            c.cw_delete()
            cnx.commit()
        ...
        class DummyBeforeHook(Hook):
            ...
            def __call__(self):
                ...
                if self.entity.eid not in pendings:
                    ...
                    self.entity.cw_set(alias="foo")

        with self.admin_access.repo_cnx() as cnx:
            with self.temporary_appobjects(DummyBeforeHook):
                addr = cnx.create_entity("EmailAddress", address="a@b.fr")
                addr.cw_set(address="a@b.com")
                rset = cnx.execute(
                    "Any A,AA WHERE X eid %(x)s, X address A, X alias AA",
                    {"x": addr.eid},
                )
                self.assertEqual(rset.rows, [["a@b.com", "foo"]])
    ...
    def test_cw_set_in_before_add(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            with self.temporary_appobjects(DummyBeforeHook):
                ...
                self.assertRaises(
                    AssertionError, cnx.create_entity, "EmailAddress", address="a@b.fr"
                )
    ...
    def test_multiple_edit_cw_set(self):
        ...
        class DummyBeforeHook(Hook):
            ...
            def __call__(self):
                ...
                self._test.assertFalse(
                    "invoiced" in self.entity.cw_edited,
        ...
        with self.admin_access.repo_cnx() as cnx:
            with self.temporary_appobjects(DummyBeforeHook):
                cnx.create_entity("Affaire", ref="AFF01")
                cnx.create_entity("Affaire", ref="AFF02")
    ...
    def test_user_friendly_error(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            with self.temporary_appobjects(MyIUserFriendlyUniqueTogether):
                s = cnx.create_entity("Societe", nom="Logilab", type="ssll", cp="75013")
                cnx.commit()
                with self.assertRaises(ValidationError) as cm:
                    cnx.create_entity("Societe", nom="Logilab", type="ssll", cp="75013")
                self.assertEqual(cm.exception.errors, {"hip": "hop"})
                cnx.rollback()
                cnx.create_entity("Societe", nom="Logilab", type="ssll", cp="31400")
                with self.assertRaises(ValidationError) as cm:
                    s.cw_set(cp="31400")
    ...
    def test_attribute_cache(self):
        with self.admin_access.repo_cnx() as cnx:
            bk = cnx.create_entity("Bookmark", title="index", path="/")
            cnx.commit()
            self.assertEqual(bk.title, "index")
            bk.cw_set(title="root")
    ...
    def test_delete_entity_with_inlined_relation(self):
        ...
        with self.admin_access.cnx() as cnx:
            ...
            p0 = cnx.create_entity("Personne", nom="p0")
            p1 = cnx.create_entity("Personne", nom="p1", personne_inlined=p0)
            cnx.commit()
            ...
            p1 = cnx.create_entity("Personne", nom="p1")
...
class SchemaDeserialTC(CubicWebTC):
    ...
    def test_deserialization_base(self):
        ...
        * _unique_together__ content"""
        ...
        try:
            ...
            with self.admin_access.repo_cnx() as cnx:
                cu = cnx.system_sql(
                    f"SELECT {namecol} FROM {table} WHERE {finalcol} is NULL"
                )
                ...
                sql = (
                    "SELECT etype.cw_eid, etype.cw_name, cstr.cw_eid, rel.eid_to "
                ...
                self.assertCountEqual(
                    person._unique_together[0], ("nom", "prenom", "inline2")
                )
    ...
    def test_custom_attribute_param(self):
        ...
        try:
            ...
            attr = pes.relation_definition("custom_field_of_jungle")
...
class FTITC(CubicWebTC):
    def test_fulltext_container_entity(self):
        with self.admin_access.repo_cnx() as cnx:
            assert (
                self.schema.relation_schema_for("use_email").fulltext_container
                == "subject"
            )
            toto = cnx.create_entity("EmailAddress", address="toto@logilab.fr")
            cnx.commit()
            ...
            cnx.execute(
                'DELETE X use_email Y WHERE X login "admin", Y eid %(y)s',
                {"y": toto.eid},
            )
            cnx.commit()
            rset = cnx.execute("Any X WHERE X has_text %(t)s", {"t": "toto"})
            self.assertEqual(rset.rows, [])
            tutu = cnx.create_entity("EmailAddress", address="tutu@logilab.fr")
            cnx.user.cw_set(use_email=tutu)
            cnx.commit()
            rset = cnx.execute("Any X WHERE X has_text %(t)s", {"t": "tutu"})
            self.assertEqual(rset.rows, [[cnx.user.eid]])
            tutu.cw_set(address="hip@logilab.fr")
            cnx.commit()
            rset = cnx.execute("Any X WHERE X has_text %(t)s", {"t": "tutu"})
            self.assertEqual(rset.rows, [])
    ...
    def test_no_uncessary_ftiindex_op(self):
        with self.admin_access.repo_cnx() as cnx:
            cnx.create_entity("Workflow", name="dummy workflow", description="huuuuu")
            self.assertFalse(
                any(
                    x
                    for x in cnx.pending_operations
        ...
        with self.admin_access.repo_cnx() as cnx:
            inserted = [
                ...
                for r in cnx.execute(
                    "Any K ORDERBY K " 'WHERE P pkey K, P pkey ~= "system.version.%"'
                )
            ]
            self.assertEqual(
                inserted,
                [
                    "system.version.basket",
                    "system.version.card",
                    "system.version.comment",
                    "system.version.cubicweb",
                    "system.version.file",
                    "system.version.localperms",
                    "system.version.tag",
                ],
            )
...
class InlineRelHooksTC(CubicWebTC):
        ...
        with self.temporary_appobjects(EcritParHook):
            with self.admin_access.repo_cnx() as cnx:
                ...
                self.assertEqual(
                    CALLED,
                    [
                        ("before_add_relation", eidn, "ecrit_par", eidp),
                ...
                self.assertEqual(
                    CALLED,
                    [
                        ("before_delete_relation", eidn, "ecrit_par", eidp),
                        ("after_delete_relation", eidn, "ecrit_par", eidp),
                    ],
                )
                CALLED[:] = ()
                eidn = cnx.execute('INSERT Note N: N ecrit_par P WHERE P nom "toto"')[
                    0
                ][0]
                self.assertEqual(
                    CALLED,
    ...
    def test_unique_contraint(self):
        with self.admin_access.repo_cnx() as cnx:
            toto = cnx.create_entity("Personne", nom="toto")
            a01 = cnx.create_entity("Affaire", ref="A01", todo_by=toto)
            cnx.commit()
            cnx.create_entity("Note", type="todo", inline1=a01)
            cnx.commit()
            cnx.create_entity("Note", type="todo", inline1=a01)
            with self.assertRaises(ValidationError) as cm:
                cnx.commit()
            self.assertEqual(
                cm.exception.errors,
                {
                    "inline1-subject": "RQLUniqueConstraint S type T, S inline1 A1, "
    ...
    def test_add_relations_at_creation_with_del_existing_rel(self):
        with self.admin_access.repo_cnx() as cnx:
            person = cnx.create_entity(
                "Personne", nom="Toto", prenom="Lanturlu", sexe="M"
            )
...
class PerformanceTest(CubicWebTC):
    ...
    def test_composite_deletion(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            for i in range(2000):
                p = cnx.create_entity(
                    "Personne", nom="Doe%03d" % i, prenom="John", sexe="M"
                )
                personnes.append(p)
            abraham = cnx.create_entity(
                "Personne", nom="Abraham", prenom="John", sexe="M"
            )
    ...
    def test_session_add_relations(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            relations = [
                ("personne_composite", [(abraham.eid, p.eid) for p in personnes])
            ]
    ...
    def test_session_add_relations_inlined(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            add_relations = cnx.add_relations
            relations = [
                ("personne_inlined", [(abraham.eid, p.eid) for p in personnes])
            ]
            add_relations(relations)
            cnx.commit()
            t1 = time.time()
    ...
    def test_optional_relation_reset_1(self):
        with self.admin_access.repo_cnx() as cnx:
            p1 = cnx.create_entity("Personne", nom="Vincent")
            p2 = cnx.create_entity("Personne", nom="Florent")
            w = cnx.create_entity("Affaire", ref="wc")
