...
from cubicweb import AuthenticationError, Binary
...
You can set multiple groups by separating them by a comma.",
    ...
    def check_urls(self, source_entity):
        ...
        if len(urls) > 1:
            raise ValidationError(
                source_entity.eid, {"url": _("can only have one url")}
            )
    ...
    def init(self, source_entity):
        ...
        self._authenticate = getattr(self, f"_auth_{self.authmode}")
        ...
        self.base_filters = [
            f"(objectclass={replace_filter(o)})" for o in typedconfig["user-classes"]
        ]
        ...
        self.group_base_filters = [
            f"(objectClass={replace_filter(o)})" for o in typedconfig["group-classes"]
        ]
    ...
    def authenticate(self, cnx, login, password=None, **kwargs):
        ...
        searchfilter = [
            f"({replace_filter(self.user_login_attr)}={replace_filter(login)})"
        ]
        searchfilter.extend(self.base_filters)
        searchstr = f"(&{''.join(searchfilter)})"
        # first search the user
        try:
            user = self._search(
                cnx, self.user_base_dn, self.user_base_scope, searchstr
            )[0]
        except IndexError:
            # no such user
            raise AuthenticationError()
    ...
    def _search(self, cnx, base, scope, searchstr="(objectClass=*)", attrs=()):
        """make an ldap query"""
        self.debug(
            "ldap search %s %s %s %s %s", self.uri, base, scope, searchstr, list(attrs)
        )
        ...
        if not ldapcnx.search(
            base, searchstr, search_scope=scope, attributes=set(attrs) - {"dn"}
        ):
            return []
        result = []
