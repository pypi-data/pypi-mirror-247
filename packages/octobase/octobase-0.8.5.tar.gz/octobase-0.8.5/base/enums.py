...
class Enum:
  ...
  def _AddOption(self, option):
    ...
    if self.parent:
      ...
      self.parent.enum._AddOption(option)
  ...
  def _UnderstandNames(self, names):
    ...
    if isinstance(names, Option):     # from DefineNested()
      ...
      self.codename = self.parent.enum.codename + base.utils.CollapseWhitespace(names.shortname, to='')
      self.constname    = self.parent.enum.constname + '_' + names.shortconst
...
class Option:
  ...
  def __contains__(self, x):
    ...
    if self == y:
      return True
    return self.children and any(y in z for z in self.children)
  ...
  def _FromDefinition(enum, definition):
    ...
    shortname       = name
    if enum.parent:
      name          = ' '.join((name, enum.parent.name))

    zelf.enum       = enum
    zelf.name       = name
    zelf.shortname  = shortname
    zelf.constname  = '_'.join((enum.constname, const))
