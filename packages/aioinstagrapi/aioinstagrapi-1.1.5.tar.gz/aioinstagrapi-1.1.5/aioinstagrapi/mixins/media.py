...
from aioinstagrapi.models import Location, Media, UserShort, Usertag
from aioinstagrapi.utils import InstagramIdCodec, json_value
...
class MediaMixin:
    ...
    async def media_info_a1(self, media_pk: str, max_id: str = None) -> Media:
        ...
        data = await self.public_a1_request(
    ...
    async def user_medias_gql(
        ...
        while True:
            ...
            medias_page, end_cursor = await self.user_medias_paginated_gql(
    ...
    async def user_videos_v1(self, user_id: str, amount: int = 0) -> List[Media]:
        """
        ...
        while True:
            try:
                medias_page, next_max_id = await self.user_videos_paginated_v1(
    ...
    async def user_medias_v1(self, user_id: str, amount: int = 0) -> List[Media]:
        """
        ...
        while True:
            try:
                medias_page, next_max_id = await self.user_medias_paginated_v1(
                    user_id, amount, end_cursor=next_max_id
                )
            except PrivateError as e:
                raise e
    ...
    async def user_medias_paginated(
        ...
        try:
            if end_cursor and "_" in end_cursor:
                ...
                raise EndCursorIsV1
            try:
                medias, end_cursor = await self.user_medias_paginated_gql(
            ...
            if isinstance(e, EndCursorIsV1):
                ...
                self.logger.exception(e)
            medias, end_cursor = await self.user_medias_paginated_v1(
                user_id, amount, end_cursor=end_cursor
            )
    ...
    async def user_medias(self, user_id: str, amount: int = 0, sleep: int = 0) -> List[Media]:
        """
        Get a user's media
        ...
        try:
            try:
                medias = await self.user_medias_gql(user_id, amount, sleep)
            except ClientLoginRequired as e:
                if not self.inject_sessionid_to_public():
                    raise e
                medias = await self.user_medias_gql(user_id, amount, sleep)  # retry
        except PrivateError as e:
            raise e
        except Exception as e:
            if not isinstance(e, ClientError):
    ...
    async def user_clips_v1(self, user_id: str, amount: int = 0) -> List[Media]:
        """
        ...
        while True:
            try:
                medias_page, next_max_id = await self.user_clips_paginated_v1(
    ...
    async def user_clips(self, user_id: str, amount: int = 0) -> List[Media]:
        """
        ...
        amount = int(amount)
        user_id = int(user_id)
        return await self.user_clips_v1(user_id, amount)
    ...
    async def media_unarchive(self, media_id: str) -> bool:
        ...
        bool
            A boolean value
        """
        return await self.media_archive(media_id, revert=True)
    ...
    async def usertag_medias_v1(self, user_id: str, amount: int = 0) -> List[Media]:
        """
    ...
    async def usertag_medias(self, user_id: str, amount: int = 0) -> List[Media]:
        """
        ...
        user_id = int(user_id)
        try:
            medias = await self.usertag_medias_gql(user_id, amount)
        except ClientError:
    ...
    async def media_pin(self, media_pk: str, revert: bool = False):
        ...
        result = await self.private_request(f"users/{name}_timeline_media/", data)
        return result["status"] == "ok"
