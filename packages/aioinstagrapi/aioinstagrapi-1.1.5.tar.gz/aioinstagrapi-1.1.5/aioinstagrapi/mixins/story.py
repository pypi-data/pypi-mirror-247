...
from aioinstagrapi.exceptions import ClientNotFoundError, StoryNotFound, UserNotFound
from aioinstagrapi.extractors import (
    extract_story_gql,
    extract_story_v1,
    extract_user_short,
)
from aioinstagrapi.models import Story, UserShort


class StoryMixin:
    _stories_cache = {}  # pk -> object

    async def story_pk_from_url(self, url: str) -> str:
        """
    ...
    # async def story_info_gql(self, story_pk: str):
    #     # GQL havent video_url :-(
    #     return self.media_info_gql(self, str(story_pk))

    async def story_stream(self, user_id : int) -> List[Story]:
        stories = []
        data = {
            "exclude_media_ids":"[]",
        ...
        response = await self.private_request('feed/reels_media_stream/', data=data)
        for story in response['reels'][str(user_id)]['items']:
            stories.append(extract_story_v1(story))
    ...
    async def user_stories_v1(self, user_id: str, amount: int = None) -> List[Story]:
        ...
        user_id = int(user_id)
        reel = (
            (await self.private_request(f"feed/user/{user_id}/story/", params=params)).get(
                "reel"
            )
            or {}
        )
    ...
    async def story_download_by_url(
        ...
        assert fname, (
            ...
            """Read the documentation https://subzeroid.github.io/aioinstagrapi/usage-guide/story.html"""
        )
    ...
    async def story_like(self, story_id: str, revert: bool = False, container : str = 'reel_follow_timeline') -> bool:
        """
        Like a story
