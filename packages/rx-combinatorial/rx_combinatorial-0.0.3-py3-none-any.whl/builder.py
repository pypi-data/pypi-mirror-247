...
class builder:
    ...
    def _get_scales_dir(self):
        if   self._cache_path is None:
            scales_dir = f'{os.environ["CALDIR"]}/scales/{self._vers}' 
        elif not os.path.isfile(self._cache_path): 
            scales_dir = f'{os.environ["CALDIR"]}/scales/{self._vers}' 
        else:
            try:
                tf=tarfile.open(self._cache_path)
                tf.close()
            except tarfile.ReadError:
                log.error(f'File {self._cache_path} be opened')
                raise

            scales_dir = self._cache_path.replace('.tar.gz', '')

        return scales_dir
    #------------------------------
    def _build_tarball(self):
        if self._cache_path is None or os.path.isfile(self._cache_path):
            return

        cache_dir = self._cache_path.replace('.tar.gz', '')
        os.makedirs(cache_dir, exist_ok=True)
        l_json = glob.glob(f'{self._scales_dir}/*.json')
        for src_path in l_json:
            file_name = os.path.basename(src_path)
            trg_path  = f'{cache_dir}/{file_name}'
            shutil.copy(src_path, trg_path)

        with tarfile.open(self._cache_path, 'w:gz') as tar:
            cache_dir_name = os.path.basename(cache_dir)
            tar.add(cache_dir, arcname=cache_dir_name)
    #------------------------------
    def _load_pars(self):
    ...
    def _add_corrected_par(self, d_par, d_par_all, unc):
        log.info(f'Applying corrections for unc: {unc}')

        log.info(f'{"Param":<10}{"->":10}{"Value":>10}{"Correction":>15}')
        for name in d_par_all:
            ...
            if unc in [0, -1] or err_sr == 0:
                log.info(f'{name:<10}{"->":10}{val_sr:>10.3e}{"0 ":>15}')
            ...
            if   unc == None:
                val_sr = val_sr - dval
                log.info(f'{name:<10}{"->":10}{val_sr:>10.3e}{dval:>15.3e}')
            elif isinstance(unc, dict):
                serr   = 0  if name not in unc else unc[name] * err
                val_sr = val_sr - dval + serr
                log.info(f'{name:<10}{"->":10}{val_sr:>10.3e}{dval:>15.3e}{"+":10}{serr:>10.3e}')
            else:
    ...
    def _build_pars(self, d_par_val, preffix):
        ...
        for name, (low, hig, val, err) in d_par_val.items():
            if err == 0:
                ...
                d_par[name] = zfit.param.Parameter(f'{name}_{preffix}', val, low, hig)
    ...
    def _build_cons(self, d_par_val, d_par_obj):
        ...
        return d_con
    #------------------------------
    def get_pdf(self, obs=None, unc=None, preffix=None, name=None): 
        '''
        Parameters
        -------------------
