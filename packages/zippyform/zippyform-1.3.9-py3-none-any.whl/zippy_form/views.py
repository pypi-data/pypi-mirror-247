...
from zippy_form.utils import FORM_STATUS, FORM_FIELD_STATUS, FIELD_TYPES, FORM_SUBMISSION_STATUS, \
    ...
    WEBHOOK_STATUS_DETAILS ,FORM_TYPE_DETAILS, FORM_TYPE, convert_utc_to_timezone,PAYMENT_MODE,get_stripe_secret_key,PAYMENT_GATEWAYS,APPLICATION_TYPE,PAYMENT_TYPE,DEFAULT_STRIPE_APPLICATION_FEE_AMOUNT
...
def get_account_details(req, account_id):
    ...
    account_payment_settings_detail = {
        ...
        "stripe_connect_redirect_url":account.stripe_connect_redirect_url,
...
def update_account_profile_details(req,account_id):
    ...
    if req.data.get('is_payment_collect_enabled') is not None and req.data.get(
        ...
        account.stripe_connect_redirect_url = ''
        account.save()
        response_data = {}
...
def get_form_payment_settings(req, form_id, payment_gateway=None, payment_mode=None):
    ...
    if form_payment_setting:
        ...
        if form_payment_setting.payment_gateway == PAYMENT_GATEWAYS[0][0]:
            is_stripe_connected = AccountPaymentSettings.objects.filter(account=form_payment_setting.account.id, payment_gateway=form_payment_setting.payment_gateway,payment_mode=form_payment_setting.payment_mode).exists()
            ...
            response_data['stripe']['show_connect'] = show_connect
            ...
            if selected_payment_gateway == PAYMENT_GATEWAYS[0][0]:
                ...
                if not secret_key:
                    return Response({
                        ...
                        "data": {"secret_key": ["Please add Stripe Secret Key to the application before updating Payment Settings"]}
                    }, status=status.HTTP_400_BAD_REQUEST)
            ...
            if application_type == APPLICATION_TYPE[1][0]:
                ...
                if account_payment_settings and account_payment_settings.key:
                    ...
                    if selected_payment_gateway == PAYMENT_GATEWAYS[0][0]:
                        ...
                        return Response({
                            "status": "validation_error",
                            "data": {"connect" : ["Please Connect to your Stripe Account before updating Payment Settings"]}
                        },status=status.HTTP_400_BAD_REQUEST)
                    else:
                        # Payment Gateway - When Working On New Payment Gateway, Add New Payment Gateway Secret Key Here
                        ...
                        return Response({
                            "status": "validation_error",
                            "data": {"secret_key" : ["Secret Key required"]}
                        },status=status.HTTP_400_BAD_REQUEST)
...
def form_payment_settings_stripe_connect(req):
    ...
    if is_payment_collect_enabled:
        ...
        response_data = {}
        response_data['redirect_url'] = account.stripe_connect_redirect_url

        return Response({"status": "success", "data": response_data, "msg": "Stripe Connected Successfully"},
                        status=status.HTTP_200_OK)
    else:
...
def submit_form(req, form_id, step_id, submission_id=None):
    ...
    if current_step_order > 1 and submission_id is None:
            ...
            if _is_payment_collect_enabled:
                ...
                if form_payment_settings:
                    ...
                    if primary_payment_gateway == PAYMENT_GATEWAYS[0][0]:
                        ...
                        checkout = payment.checkout(payment_type, application_fee_amount, line_items, after_payment_redirect_url, form_id, form_submission.id)
    ...
    form_data_for_gsheet["Status"] = FORM_SUBMISSION_STATUS_DETAILS[str(form_submission.status)]
...
def webhook_stripe(req):
    ...
    if stripe_payment_status == 'complete':
        ...
        try:
            log_payment_status(form_submission, form_submission_sub_total, form_submission_tax_amount, tax_percentage,
                            form_submission_total, stripe_payment_status)
        except Exception as e:
            print(str(e))
    else:
...
def log_payment_status(form_submission, form_submission_sub_total, form_submission_tax_amount, tax_percentage, form_submission_total,stripe_payment_status):
    """
    Log Payment Status
    """
    form_submission_payment_details = FormSubmissionPaymentDetails()
    form_submission_payment_details.form_submission = form_submission
    form_submission_payment_details.sub_total = (form_submission_sub_total / 100)
    form_submission_payment_details.tax_amount = (form_submission_tax_amount / 100)
    if form_submission_tax_amount > 0:
        form_submission_payment_details.tax_percentage = tax_percentage
    form_submission_payment_details.total = (form_submission_total / 100)
    form_submission_payment_details.payment_gateway_status = stripe_payment_status
    form_submission_payment_details.save()
