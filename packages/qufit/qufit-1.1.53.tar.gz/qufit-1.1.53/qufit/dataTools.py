...
def func(paras,b0,numofinterp=1001):
    voffset, vperiod, ejs, ec, d = paras
    sign = 1 if b0>=voffset else -1
    x = np.linspace(-vperiod/2,vperiod/2,numofinterp) + voffset
    tmp = np.pi*(x-voffset)/vperiod
    f01 = np.sqrt(8*ejs*ec)*(np.sqrt(np.abs(np.cos(tmp))*(1+d**2*np.tan(tmp)**2))-1) * (sign*np.sign(x-voffset)) + np.sqrt(8*ejs*ec) -ec
    return x, f01

def freq2flux(delta,f0,b0,paras,numofinterp=1001):
    
    bias, f01 = func(paras,b0,numofinterp)
...
def bitstring(level=2,repeat=2,dtype='str'):
    x = product(range(level),repeat=repeat)
    if dtype == 'str':
        return iter([''.join(str(i) for i in j) for j in x])
    else:
        return x
...
def find_circle(s_st,target=0,stdnum=[[1,1]],n_clusters=2,figsize=(9,14)):
