...
from ara_tools.artefact_link_updater import ArtefactLinkUpdater
...
class ArtefactRenamer:
    ...
    def rename(self, old_name, new_name, classifier):
        ...
        sub_directory = self.file_system.path.join("ara",Classifier.get_sub_directory(classifier))
        ...
        self.link_updater.update_links_in_related_artefacts(old_name, new_name, dir_path)


  
    def _update_title_in_artefact(self, artefact_path, new_title, classifier):
    # Format the new title: replace underscores with spaces and capitalize the first letter
        formatted_new_title = new_title.replace('_', ' ').capitalize()
