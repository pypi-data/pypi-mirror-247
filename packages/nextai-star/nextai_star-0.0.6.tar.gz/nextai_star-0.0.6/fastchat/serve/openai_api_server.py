...
def create_usage_record(usage: UsageInfo, metadata: Union[EmbeddingsResponse, ChatCompletionResponse, CompletionResponse], token: str):
    requests.post('{}/public/clusters/{}/usage-record/'.format(app_settings.auth_server, app_settings.cluster_id), json={
        "usage": usage.dict(),
...
def create_background_tasks(background_tasks: BackgroundTasks, usage: UsageInfo, metadata: Union[EmbeddingsResponse, ChatCompletionResponse, CompletionResponse], token: Optional[str] = None):
    background_tasks.add_task(create_usage_record, usage=usage, metadata=metadata, token=token)
...
app = fastapi.FastAPI()
headers = {"User-Agent": "NextAI API Server"}
...
async def get_worker_address(model_name: str) -> str:
    ...
    """
    controller_address = app_settings.controller_address
    if app_settings.worker_address is None:
...
@app.get("/v1/models")
async def show_available_models(token: Annotated[str, Depends(check_api_key)],):
    controller_address = app_settings.controller_address
...
async def create_chat_completion(request: ChatCompletionRequest, token: Annotated[str, Depends(check_api_key)], background_tasks: BackgroundTasks):
    ...
    if request.stream:
        generator = chat_completion_stream_generator(
            request.model, gen_params, request.n, worker_addr, background_tasks, token=token
        )
    ...
    create_background_tasks(background_tasks, usage=response.usage, metadata=ChatCompletionResponse(model=request.model, id=response.id, choices=[], usage=response.usage), token=token)
    return response


async def chat_completion_stream_generator(
    model_name: str, gen_params: Dict[str, Any], n: int, worker_addr: str, background_tasks: BackgroundTasks, token: Optional[str] = None
    ...
    prompt_tokens = encoder.encode(gen_params["prompt"]).__len__()
    for i in range(n):
        ...
        async for content in generate_completion_stream(gen_params, worker_addr):
            ...
            if len(delta_text) == 0:
                ...
                completion_tokens += encoder.encode(delta_text).__len__()
            choice_data = ChatCompletionResponseStreamChoice(
                index=i,
    ...
    create_background_tasks(background_tasks, usage=usage, metadata=ChatCompletionResponse(id=id, model=model_name, choices=[], usage=usage), token=token)
    yield "data: [DONE]\n\n"

@app.post("/v1/completions")
async def create_completion(request: CompletionRequest, token: Annotated[str, Depends(check_api_key)], background_tasks: BackgroundTasks):
    error_check_ret = await check_model(request)
    if error_check_ret is not None:
        return error_check_ret
    error_check_ret = check_requests(request)
    ...
    if request.stream:
        generator = generate_completion_stream_generator(
            request, request.n, worker_addr, background_tasks, token=token
        )
        return StreamingResponse(generator, media_type="text/event-stream")
    else:
        text_completions = []
        ...
        create_background_tasks(background_tasks, response.usage, CompletionResponse(
            model=request.model, choices=[], usage=response.usage, id=response.id, created=response.created
        ), token=token)
        return response


async def generate_completion_stream_generator(
    request: CompletionRequest, n: int, worker_addr: str, background_tasks: BackgroundTasks, token: str
):
    ...
    finish_stream_events = []
    prompt_tokens = 0
    # Get prompt tokens using the encoder. Right now using gpt-35 model to calculate
    for text in request.prompt:
        prompt_tokens += encoder.encode(text).__len__()
        for i in range(n):
            previous_text = ""
            gen_params = await get_gen_params(
                request.model,
    ...
    create_background_tasks(background_tasks, usage=usage, metadata=CompletionResponse(id=id, model=model_name, usage=usage, choices=[]), token=token)
    yield "data: [DONE]\n\n"
...
async def create_embeddings(request: EmbeddingsRequest, token: Annotated[str, Depends(check_api_key)], background_tasks: BackgroundTasks, model_name: str = None):
    ...
    create_background_tasks(background_tasks, response.usage, EmbeddingsResponse(data=[], model=request.model,
    usage=response.usage), token)
...
async def create_chat_completion(request: APIChatCompletionRequest, token: Annotated[str, Depends(check_api_key)], background_tasks: BackgroundTasks):
    """Creates a completion for the chat message"""
    error_check_ret = await check_model(request)
    if error_check_ret is not None:
        return error_check_ret
    ...
    for i, content in enumerate(all_tasks):
        ...
        for usage_key, usage_value in task_usage.dict().items():
            setattr(usage, usage_key, getattr(usage, usage_key) + usage_value)
    response = ChatCompletionResponse(model=request.model, choices=choices, usage=usage)
    create_background_tasks(background_tasks, response.usage, ChatCompletionResponse(model=request.model, choices=[], usage=response.usage, id=response.id, created=response.created), token=token)
    return response
...
def create_openai_api_server():
    parser = argparse.ArgumentParser(
        description="NextAI ChatGPT-Compatible RESTful API server."
    )
    ...
    parser.add_argument(
        "--worker-address", type=str
    )
    parser.add_argument(
        "--cluster", type=str, help="id of the cluster"
    )
    parser.add_argument(
        "--auth-server", type=str, help="token authentication server"
    )
    parser.add_argument(
    ...
    app_settings.auth_server = args.auth_server
    app_settings.worker_address = args.worker_address
