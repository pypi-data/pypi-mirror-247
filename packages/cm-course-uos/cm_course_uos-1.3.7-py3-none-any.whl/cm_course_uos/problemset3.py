...
class StroopNetwork:
    ...
    ### Forward Pass ###
    def forward(color_input, word_input, task_input, return_activations=False, weights=None):
        """
        ...
        activation_ch = StroopNetwork.color_hidden_activation(
            color_input, task_input, Weights, bias)

        # Compute activation of word hidden layer
        activation_wh = StroopNetwork.word_hidden_activation(
            word_input, task_input, Weights, bias)

        # Compute activation of output layer
        activation_output = StroopNetwork.output_activation(
            activation_ch, activation_wh, Weights, bias)
    ...
    def compute_weight_updates_hidden_to_out(hidden_activation,
        ...
        output_derivative = StroopNetwork.logistic_derivative(
            output_activation)
    ...
    def initialize_inputs():
        ...
        # Random binary color input
        color_input = np.array(random.choice(possible_inputs))
        # Random binary word input
        word_input = np.array(random.choice(possible_inputs))
        # Random binary task input
        task_input = np.array(random.choice(possible_inputs))
    ...
    def simulate_stroop(color_input, word_input, task_input):
        ...
        response_probabilities = StroopNetwork.forward(
            color_input, word_input, task_input)
    ...
    def generate_training_data(n_trials):
        ...
        for trials in range(n_trials):
            ...
            response_probabilities = StroopNetwork.simulate_stroop(
                color_input, word_input, task_input)
            training_data.append(
                [color_input, word_input, task_input, response_probabilities])
    ...
    def train_stroop(traning_data, n_epochs=100, learning_rate=0.01):
        ...
        Weights = StroopNetwork.TweakedWeights
        ...
        for color_input, word_input, task_input, target_output in training_data:
            ...
            result = StroopNetwork.forward(
                color_input, word_input, task_input, return_activations=True, weights=Weights)

            error = (result["response"] - target_output) ** 2
            MSE_loss = error

            color_activation = StroopNetwork.color_hidden_activation(
                color_input, task_input, Weights, bias)
            word_activation = StroopNetwork.word_hidden_activation(
                word_input, task_input, Weights, bias)
            output_activation = StroopNetwork.output_activation(
                color_activation, word_activation, Weights, bias)

            update_c_h_to_out = StroopNetwork.compute_weight_updates_hidden_to_out(
                color_activation, output_activation, error)
            update_c_in_to_c_h = StroopNetwork.compute_weight_updates_in_to_hidden(
                color_input, Weights.c_h_to_out, color_activation, error)
...
def test_forward(student_function):
    c_ins = [[0, 1], [1, 0]]
    t_ins = [[0, 1], [1, 0]]
    w_ins = [[0, 1], [0, 1]]
    bias = -3
