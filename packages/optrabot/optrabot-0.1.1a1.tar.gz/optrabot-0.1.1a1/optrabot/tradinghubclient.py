...
class TradinghubClient():
		...
		try:
			...
			if response.text != '\"\"' and response.text != '':
				...
				if signalTime == None or self._signalIsOutdated(signalTime):
					logger.error('Signal is outdated already or Signal timestamp is invalid!')
					await self._scheduleNextPoll()
					return
				...
				try:
					...
					if not self._meetsMinimumPremium(limitPrice):
						...
						self._slShortTrade = None
						self._tpTrade = None
						logger.debug("Account: {} Trade placed: {}", order.account, self._entryTrade)
	...
	async def onOrderStatusEvent(self, trade: Trade):
		if trade == self._entryTrade:
			...
			if trade.orderStatus.status == OrderStatus.Cancelled:
				...
				logger.info('Entry Order has been filled (Qty: {}) and trade is running now.', trade.order.filledQuantity)
				self._position = True
	...
	async def _placeTakeProfitAndStop(self, entryTrade: Trade):
		...
		self._ironFlyStopPrice = OptionHelper.roundToTickSize(fillPrice * -1.16)
		logger.info('Stop LossPrice : {}', self._ironFlyStopPrice)
	...
	async def _monitorPosition(self):
		...
		logger.debug('Long Legs value {} Current Short SL Price: {} Desired Short SL Pice: {}', longLegsValue, currentStopPrice, desiredStopPrice)
		if currentStopPrice != desiredStopPrice:
			self._slShortTrade.order.auxPrice = desiredStopPrice
			logger.debug('Adjusting Stop Loss price to {}', desiredStopPrice)
