...
class OptraBot():
	...
	async def onDisconnected(self):
		logger.error('Disconnected from TWS, attempting to reconnect in 30 seconds ...')
		self._tradingEnabled = False
	...
	async def _checkMarketData(self):
		...
		if util.isNan(ticker.marketPrice()):
			logger.error("IB returned no SPX price but just NaN value. Trading is deactivated!")
			return
		...
		strikePrice = OptionHelper.roundToStrikePrice(ticker.marketPrice())
