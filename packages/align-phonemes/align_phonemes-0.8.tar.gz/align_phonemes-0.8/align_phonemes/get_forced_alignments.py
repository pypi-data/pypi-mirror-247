...
def get_forced_alignments(block_label_path, block_wav_path, output_transcription_json_path, transcription_method="Critical_Error", Critical_Error=0.3, verbose=False):
    # Get the trials directory location 
    current_dir = os.path.dirname(os.path.abspath(__file__))
    trials_directory = os.path.join(current_dir, "trials")

    trial_directory = parse_block(block_wav_path, block_label_path, trials_directory, verbose=verbose, clean_trials=True)
