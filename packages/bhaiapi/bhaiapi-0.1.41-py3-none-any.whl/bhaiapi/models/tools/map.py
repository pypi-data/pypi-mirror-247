...
class BhaiMapsDirections:
    ...
    @property
    def sections(self) -> List[BhaiMapsRoadSection]:
        return [BhaiMapsRoadSection(s) for s in self._map[1]]
...
class BhaiMapContent(UserContent):
    ...
    @property
    def points(self) -> List[BhaiMapsPoint]:
        return (
            [BhaiMapsPoint(point) for point in self._input_list[0][1]]
            if self._input_list[0]
            else []
        )

    @property
    def directions(self) -> Optional[BhaiMapsDirections]:
        return BhaiMapsDirections(self._input_list[1]) if self._input_list[1] else None
