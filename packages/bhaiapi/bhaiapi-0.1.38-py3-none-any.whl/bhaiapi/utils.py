...
import requests
from bhaiapi.constants import IMG_UPLOAD_HEADERS
...
def upload_image(image: bytes, filename: str = "Photo.jpg"):
    """
    Upload image into bhai bucket on Google API, do not need session.
...
def extract_bhai_cookie(cookies: bool = False) -> dict:
    """
    Extracts the specified Bhai cookie(s) from the browser's cookies.

    This function searches for the specified Bhai cookies in various web browsers
    ...
    Returns:
        dict: A dictionary containing the extracted Bhai cookies.
