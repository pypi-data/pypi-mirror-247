...
from bhaiapi.constants import SESSION_HEADERS
from bhaiapi.utils import extract_bhai_cookie


class BhaiCookies(Bhai):
    """
    Bhai class for interacting with the Bhai API using cookies for authentication.
    """
    ...
    def __init__(
        ...
        """
        Initialize the BhaiCookies instance.

        Args:
            cookie_dict (dict, optional): Dictionary containing Bhai cookies.
    ...
    def _get_snim0e(self) -> str:
        """
        Get the SNlM0e value from the Bhai API response.
    ...
    def get_answer(self, input_text: str) -> dict:
        """
        Get an answer from the Bhai API for the given input text.

        Args:
            input_text (str): Input text for the query.

        Returns:
            dict: Answer from the Bhai API in the following format:
                {
                    "content": str,
                    "conversation_id": str,
                    "response_id": str,
    ...
    def speech(self, input_text: str, lang: str = "hi-IN") -> dict:
        """
        Get speech audio from Bhai API for the given input text.
        ...
        >>>   "__Secure-1PSIDTS": ""
        >>> }
        >>> bhai = BhaiCookies(cookie_dict=cookies)
        >>> audio = bhai.speech("hello!")
        >>> with open("bhai.ogg", "wb") as f:
        >>>     f.write(bytes(audio['audio']))

        Args:
            input_text (str): Input text for the query.
            lang (str, optional, default = "hi-IN"): Input language for the query.
    ...
    def ask_about_image(
        ...
        >>> bhai_answer = bhai.ask_about_image("what is in the image?", image)['content']
        >>> print(bhai_answer)
    ...
    def export_conversation(self, bhai_answer, title: str = "") -> dict:
        """
        Get Share URL for specific answer from bhai
        ...
        >>> }
        >>> bhai = BhaiCookies(cookie_dict=cookies)
        >>> bhai_answer = bhai.get_answer("hello!")
        >>> url = bhai.export_conversation(bhai_answer, title="Export Conversation")
        >>> print(url['url'])

        Args:
            bhai_answer (dict): bhai_answer returned from get_answer
        ...
        """
        return super().export_conversation(bhai_answer, title)
    ...
    def export_replit(
        ...
        **kwargs,
        ...
        >>> }
        >>> bhai = BhaiCookies(cookie_dict=cookies)
        >>> bhai_answer = bhai.get_answer("code python to print hello world")
        >>> url = bhai.export_replit(bhai_answer['code'], bhai_answer['program_lang'])
        >>> print(url['url'])
...
class BhaiAsyncCookies(BhaiAsync):
    """
    Bhai class for interacting with the Bhai API using httpx[http2]
    """
    ...
    def __init__(
        ...
        """
        Initialize the Bhai instance.

        Args:
            cookie_dict (dict, optional): Bhai cookies.
            timeout (int, optional, default = 20): Request timeout in seconds.
            proxies (dict, optional): Proxy configuration for requests.
            conversation_id (str, optional): Conversation ID.
            google_translator_api_key (str, optional): Google cloud translation API key.
    ...
    async def _get_snim0e(self) -> str:
        """
        The _get_snim0e function is used to get the SNlM0e value from the Bhai website.
    ...
    async def get_answer(self, input_text: str) -> dict:
        ...
        >>>             "__Secure-1PSIDTS": ""
        >>>         }
        >>>     bhai = BhaiAsyncCookies(cookie_dict=cookies)
        >>>     response = await bhai.get_answer("나와 내 동년배들이 좋아하는 뉴진스에 대해서 알려줘")
    ...
    async def speech(self, input_text: str, lang: str = "hi-IN") -> dict:
        """
        Get speech audio from Bhai API for the given input text.
    ...
    async def ask_about_image(
        ...
        """
        Send Bhai image along with question and get answer async mode
        ...
        >>>     bhai = BhaiAsyncCookies(cookie_dict=cookies)
        >>>     image = open('image.jpg', 'rb').read()
        >>>     bhai_answer = await bhai.ask_about_image("what is in the image?", image)
        >>>     print(bhai_answer['content'])
        >>>
        >>> asyncio.run(main())
    ...
    async def export_conversation(self, bhai_answer, title: str = "") -> dict:
        """
        Get Share URL for specific answer from bhai
        ...
        >>>         }
        >>>     bhai = BhaiAsyncCookies(cookie_dict=cookies)
        >>>     bhai_answer = await bhai.get_answer("hello!")
        >>>     url = await bhai.export_conversation(bhai_answer, title="Export Conversation")
        >>>     print(url['url'])
        >>>
        >>> asyncio.run(main())
        ...
        """
        return await super().export_conversation(bhai_answer, title)
    ...
    async def export_replit(
        ...
        **kwargs,
        ...
        >>>         }
        >>>     bhai = BhaiAsyncCookies(cookie_dict=cookies)
        >>>     bhai_answer = await bhai.get_answer("code python to print hello world")
        >>>     url = await bhai.export_replit(bhai_answer['code'], bhai_answer['program_lang'])
        >>>     print(url['url'])
        >>>
        >>> asyncio.run(main())
