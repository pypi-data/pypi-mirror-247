...
from bhaiapi.models.result import BhaiResult
...
class Bhai:
    """
    Bhai class for interacting with the Google Bhai.
    """
    ...
    def _get_token(self, token: str, token_from_browser: bool) -> str:
        ...
        if token:
            ...
            extracted_cookie_dict = extract_bhai_cookie(cookies=False)
    ...
    def ask(
        ...
        res = BhaiResult(resp_json)
        if not res.drafts:
            res = BhaiResult(jsons[-2])
    ...
    def get_answer(
        ...
        >>> response = bhai.get_answer("나와 내 동년배들이 좋아하는 뉴진스에 대해서 알려줘")
        >>> print(response['content'])

        >>> response = bhai.get_answer("Show me grocery stores close to the entrance to Grand Teton National Park and give me ideas for good snacks to bring hiking", tool=Tool.GOOGLE_MAPS)
        >>> print(response['content'])
        ...
        bhai_answer = build_bhai_answer(
            parsed_answer, images, program_lang, code, resp.status_code
        )

        # Update params
        self.conversation_id, self.response_id, self.choice_id = (
            bhai_answer["conversation_id"],
            bhai_answer["response_id"],
            bhai_answer["choices"][0]["id"],
        )
        self._reqid += 100000

        # [Optional] Execute code
        if self.run_code and bhai_answer["code"] is not None:
            try:
                piro = bhai_answer["code"]
                code = f"```python\n{piro}\n```"
                console.print(Markdown(code))
                exec(bhai_answer["code"])
            except Exception:
                pass
    ...
    def export_conversation(self, bhai_answer, title: str = "") -> dict:
        ...
        """
        conv_id = bhai_answer["conversation_id"]
        resp_id = bhai_answer["response_id"]
        choice_id = bhai_answer["choices"][0]["id"]
        params = {
            "rpcids": "fuVx7",
            "source-path": "/",
            "bl": TEXT_GENERATION_WEB_SERVER_PARAM,
    ...
    def ask_about_image(
        ...
        """
        Send Bhai image along with question and get answer
    ...
    def export_replit(
        ...
        **kwargs,
        ...
        >>> token = 'xxxxxx'
        >>> bhai = Bhai(token=token)
        >>> bhai_answer = bhai.get_answer("Give me python code to print hello world")
