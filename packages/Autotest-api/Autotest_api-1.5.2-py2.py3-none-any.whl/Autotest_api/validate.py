...
def check_elements_in_list(list1, list2):
    """断言一个list [1, 2]成员全部在另外一个list[1, 2, 3]"""
...
def contains(check_value, expect_value):
    if isinstance(check_value, (list, tuple, dict, str)):
        result1 = expect_value in check_value
        result2 = check_elements_in_list(expect_value, check_value)
        result = result1 or result2
        assert result, f'{expect_value} in {check_value}'
    else:
        # 数字类型包含
...
def contained_by(check_value, expect_value):
    if isinstance(expect_value, (list, tuple, dict, str)):
        result1 = check_value in expect_value
        result2 = check_elements_in_list(check_value, expect_value)
        result = result1 or result2
        assert result, f'{check_value} in {expect_value}'
    else:
        # 数字类型包含
