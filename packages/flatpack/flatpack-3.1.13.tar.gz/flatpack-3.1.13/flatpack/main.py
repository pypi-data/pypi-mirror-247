...
def fpk_generate_text_with_image(prompt, image_url, model_name="microsoft/git-base-coco", max_length=64):
    """Generate text response based on an image URL and a text prompt."""
    ...
    # Preprocess text
    inputs = processor(text=prompt, images=image, return_tensors="pt", max_length=max_length)

    # Generate text response
    sample = model.generate(**inputs, max_length=max_length)
    response = processor.tokenizer.decode(sample[0], skip_special_tokens=True)
                ...
                try:
                    try:
                        ...
                        interface = gr.Interface(
                            ...
                            inputs=["text", gr.inputs.Image(type="pil", label="Optional Image")],
                            outputs="text",
                            title="flatpack.ai"
                        )
