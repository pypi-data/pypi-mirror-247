...
class JAverager(object):
    def __init__(self):
        ...
        self._fft = np.array([], dtype=complex)
    ...
    def samples_to_time(self, in_samples):
        return np.array(in_samples).astype(float) / self.fs - self.time_offset

    def time_to_samples(self, in_time):
        return np.rint((in_time + self.time_offset) * self.fs).astype(int)
    ...
    def add_sweep(self, new_sweep=np.array([]), split_id=None):
        try:
            ...
            if self.total_sweeps == 0:
                ...
                self._fft = np.zeros((_fft_size, self.splits), dtype=complex)
    ...
    def do_blanking(self,  data: np.array = None):
        _end_point = np.minimum(self.blanking_duration * self.fs, data.shape[0]).astype(int)
