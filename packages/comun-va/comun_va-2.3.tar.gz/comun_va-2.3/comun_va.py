...
def get_image_from_ip_camera(camera_url: str, frames_to_calibrate: int = 1):
    ...
    try:
        ...
        raise Exception(f"Error, no se ha podido obtener una imagen de la camara IP: {str(e)}")
...
def run_model_with_frame(img_frame, class_names: dict, model, output=False, limit_gpu=False,
                         rescaled_size=(256, 256)) -> tuple[str, str]:
    """
    ...
    try:
        ...
        match_score = str(predictions[0][index])[:4]
