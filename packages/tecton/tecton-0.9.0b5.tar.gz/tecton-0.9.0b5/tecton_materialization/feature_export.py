...
def _get_features_in_range(
    ...
    # TODO (vitaly): remove adding 1 second once read api valid_to is no longer inclusive in the query start
    tecton_df = fv.get_features_in_range(
        start_time=effective_start_time + datetime.timedelta(seconds=1), end_time=effective_end_time, from_source=False
    )
    ...
    df = df.withColumn(
        valid_from(), when(col(valid_from()) < effective_start_time, effective_start_time).otherwise(col(valid_from()))
    )
    # TODO (vitaly): remove clipping valid_to() once read api does this by default
    df = df.withColumn(
        valid_to(),
        when(col(valid_to()) > effective_end_time, effective_end_time).otherwise(
            coalesce(col(valid_to()), lit(effective_end_time))
        ),
    )
