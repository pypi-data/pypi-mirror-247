...
EMR_SUPPORTED_SPARK = {SparkVersions.Emr6_5, SparkVersions.Emr6_7, SparkVersions.Emr6_9}
DATABRICKS_SUPPORTED_SPARK = {SparkVersions.Dbr10_4, SparkVersions.Dbr9_1, SparkVersions.Dbr11_3}
...
# A timedelta that can also be initialized with a string like '1d12h30m45s'.
CoercibleTimedelta = Annotated[datetime.timedelta, pydantic.BeforeValidator(_string_to_timedelta)]

# A datetime that can be also be initialized with a date. Needed because YAML parsing values like "2022-11-04" produces
# date objects.
CoercibleDatetime = Annotated[datetime.datetime, pydantic.BeforeValidator(_date_to_datetime)]
...
# A dict that can be initialized with either a string or a dict. Will be coerced to a dict.
# TODO(jake): Could use a Pydantic validator to eagerly validate that the json can be converted to a proto struct.
# TODO(jake): Would preferable to use immutable types, but the proto struct creation expects dicts/lists.
CoercibleJsonStruct = Annotated[Dict[str, Any], pydantic.BeforeValidator(_str_to_json_struct)]


class _DefaultClusterConfig(BaseTectonConfig):
...
class ExistingClusterConfig(BaseTectonConfig):
...
class EMRClusterConfig(BaseTectonConfig):
...
with warnings.catch_warnings():
    # Ignore pydantic "json" shadowing warning. Pydantic 2.0 kept this attribute around for legacy support only.
    warnings.filterwarnings("ignore", module="pydantic")

    class EMRJsonClusterConfig(BaseTectonConfig):
    ...
    class DatabricksJsonClusterConfig(BaseTectonConfig):
...
class DatabricksClusterConfig(BaseTectonConfig):
...
class RiftClusterConfig(BaseTectonConfig):
...
class SparkConfigWrapper(BaseTectonConfig):
...
class ParquetConfig(BaseTectonConfig):
...
class DeltaConfig(BaseTectonConfig):
    ...
    time_partition_size: Optional[CoercibleTimedelta] = datetime.timedelta(hours=24)
    subdirectory_override: Optional[str] = None
...
class OfflineStoreConfig(BaseTectonConfig):
    ...
    staging_table_format: Union[DeltaConfig, ParquetConfig] = pydantic.Field(
        default_factory=DeltaConfig, discriminator="kind"
    )
    publish_full_features: bool = False
    publish_start_time: Optional[CoercibleDatetime] = None
...
class DynamoConfig(BaseTectonConfig):
...
class RedisConfig(BaseTectonConfig):
...
class BigtableConfig(BaseTectonConfig):
...
class MonitoringConfig(BaseTectonConfig):
...
class LoggingConfig(BaseTectonConfig):
    """
    ...
    @pydantic.field_validator("sample_rate", mode="before")
    def int_to_float(cls, v):
        """Allow users to define sample rates as ints (0 or 1) and convert the value to a float automatically."""
        if isinstance(v, int):
            return float(v)
        return v
...
class OutputStream(BaseTectonConfig):
...
class DatetimePartitionColumn(BaseTectonConfig):
...
with warnings.catch_warnings():
    # Ignore pydantic "schema" shadowing warning. Pydantic 2.0 kept this attribute around for legacy support only.
    warnings.filterwarnings("ignore", module="pydantic")

    class RequestSource(BaseTectonConfig):
...
# TODO(jake): Should inherit from BaseTectonConfig.
...
class LifetimeWindow(BaseTectonConfig):
...
class TimeWindow(BaseTectonConfig):
...
class CacheConfig(BaseTectonConfig):
...
class Aggregation(BaseTectonConfig):
    """
    ...
    @pydantic.field_validator("time_window", mode="before")
