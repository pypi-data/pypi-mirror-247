...
def plot_visualizing_inference_results(infer_image_path, scores , save_dir, file_names, args):
    """
    Args:
        test_imgs (ndarray): shape (N, 3, h, w)
        scores (ndarray): shape (N, h, w)
    ...
    for i in range(len(scores)):
        ...
        fig = plt.figure(figsize=(args.origin_size[0]*0.001, args.origin_size[1]*0.001), dpi=100)
        ax = plt.Axes(fig, [0., 0., 1., 1.])
        ax.set_axis_off()
        fig.add_axes(ax)
        ...
        img = Image.open(infer_image_path)
        img = img.resize((args.origin_size[0], args.origin_size[1]))
        img = np.array(img)
        
        ax.imshow(heat_map, cmap='OrRd', norm=norm,interpolation='none')
        ax.imshow(img, cmap='gray', alpha=0.7, interpolation='none')
        fig.savefig(os.path.join(save_dir, os.path.basename(infer_image_path[:-4]) + '.png'), dpi=100)
        plt.close()
