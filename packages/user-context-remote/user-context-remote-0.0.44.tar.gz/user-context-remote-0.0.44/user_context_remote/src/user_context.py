...
# TODO Shall we remove the src?
from sdk.src.mini_logger import MiniLogger
# TODO How can remove the src?
...
class UserContext:
    ...
    def _initialize(self, user_identifier: str = None, password: str = None, user_jwt: str = None):
        ...
        self._get_user_data_login_response(validate_jwt_response=data)
        ...
        if UserContext._instance is None:
            if user_identifier is None:
                try:
                    ...
                    MiniLogger.error(
                        "Exception missing PRODUCT_USER_IDENTIFIER in .env file - ERROR - " + str(e))
            if password is None:
                try:
                    password = os.getenv("PRODUCT_PASSWORD")
                except Exception as e:
                    MiniLogger.error(
                        "Exception missing PRODUCT_PASSWORD in .env file - ERROR -" + str(e))
    ...
    def get_effective_profile_preferred_lang_code(self) -> str:
        GET_CURENT_LANG_CODE_METHOD_NAME = "get_effective_profile_preferred_lang_code"
        # MiniLogger.start(GET_CURENT_LANG_CODE_METHOD_NAME)
        # MiniLogger.end(GET_CURENT_LANG_CODE_METHOD_NAME,object={"lang_code": self.lang_code})
    ...
    def _set_effective_profile_preferred_lang_code(self, lang_code: LangCode) -> None:
        ...
        self.lang_code = lang_code.value
    ...
    def get_real_last_name(self) -> str:
        # TODO Change all those to private
    ...
    def get_effective_subscription_id(self) -> None:
        ...
        MiniLogger.warning("user-context-python-package "+_GET_EFFECTIVE_SUBSCRIPTION_ID_METHOD_NAME +
                           " Please make sure get_effective_subscription_id() subscription_id="+str(self.subscription_id))
        ...
        try:
            ...
            if output.status_code != HTTPStatus.OK:
                MiniLogger.error(_AUTHENTICATE_BY_USER_IDENTIFICATION_AND_PASSWORD_METHOD_NAME +
                                 " output.status_code != HTTPStatus.OK " + output.text)
            ...
            MiniLogger.end(
                _AUTHENTICATE_BY_USER_IDENTIFICATION_AND_PASSWORD_METHOD_NAME)
            raise
    ...
    def _authenticate_by_user_jwt(self, user_jwt: str) -> str:
        ...
        MiniLogger.end(_AUTHENTICATE_BY_USER_JWT_METHOD_NAME,
                       object={"validate_jwt_response": output})
        return output

    def _get_user_data_login_response(self, validate_jwt_response: str) -> None:
        _GET_USER_DATA_LOGIN_RESPONSE_METHOD_NAME = "get_user_data_login_response"
        # validate_jwt_response created in Authentication Local REST-API https://github.com/circles-zone/authentication-local-restapi-typescript-serverless-com/edit/dev/auth-restapi-serverless-com/src/services/auth-service/auth-service-impl.ts
        data_json = validate_jwt_response.json()['data']
        if "userDetails" in data_json:
            ...
            if "subscriptionId" in userDetails:
                ...
                MiniLogger.end(_GET_USER_DATA_LOGIN_RESPONSE_METHOD_NAME + " Got subscription_id from the cloud. subscription_id=" +
