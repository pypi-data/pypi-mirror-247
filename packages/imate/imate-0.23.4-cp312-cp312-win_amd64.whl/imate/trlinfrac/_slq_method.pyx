...
from ..functions cimport pyFunction, Function, Homographic
...
def slq_method(
    ...
    **Symmetric Input Matrix:**
    ...
    The `slq` method requires the input matrix of :func:`imate.trlinfrac` function
    ...
    .. code-block:: python
        ...
        >>> # Import packages
        >>> from imate import trlinfrac

        >>> # Compute the trace-exponential
        >>> trlinfrac(A, gram=False, p=2.5, method='slq')
        277.00578301560057
    ...
    **Gramian Matrix:**
    ...
    Passing ``gram=True`` to :func:`imate.trlinfrac` function uses the Gramian
    ...
    .. code-block:: python
        ...
        >>> trlinfrac(B, gram=True, p=2.5, method='slq')
        275.15518680196476
    ...
    **Verbose output:**
    ...
    .. literalinclude:: ../_static/data/imate.trlinfrac.slq-verbose-1.txt
        :language: python
    ...
    **Output information:**
    ...
    .. code-block:: python
        ...
        >>> ld, info = trlinfrac(A, method='slq', return_info=True)
        >>> print(ld)
        138.6294361119891
    ...
    **Plotting:**
    ...
    .. code-block:: python
        ...
        >>> A = toeplitz(2, 1, size=1000000, gram=True)
        >>> trlinfrac(A, method='slq', min_num_samples=20, max_num_samples=80,
        ...       error_rtol=2e-4, confidence_level=0.95,
        ...       outlier_significance_level=0.001, plot=True)

    .. image:: ../_static/images/plots/trlinfrac_slq_convergence_1.png
        :align: center
        :class: custom-dark
    ...
    **Matrix operator:**
    ...
    pass the matrix `A` to the `trlinfrac` function.
    ...
    An advantage of passing `Aop` (instead of `A`) to the `trlinfrac` function
    ...
    **Computation on GPU:**
    ...
    .. code-block:: python
        ...
        >>> # Compute trace-exponential of Aop
        >>> trlinfrac(Aop, method='slq', gpu=True)
        141.52929878934194
    ...
    calling the above `trlinfrac` function (or another function) again on this
    ...
    .. code-block:: python
        ...
        >>> # Compute trace-exponential of Aop
        >>> trlinfrac(Aop, method='slq', gpu=True, gram=True)
        141.52929878934194
    ...
    **Affine matrix operator:**
    ...
    The object :math:`\\mathbf{A}(t)` can be passed to `trlinfrac` function with
    ...
    .. code-block:: python
        ...
        >>> trlinfrac(Aop, method='slq', parameters=t, min_num_samples=20,
    ...
    .. literalinclude:: ../_static/data/imate.trlinfrac.slq-verbose-2.txt
        :language: python

    The output of the plot is shown below. Each colored curve corresponds to
    a parameter in ``t = [-1, 0, 1]``.

    .. image:: ../_static/images/plots/trlinfrac_slq_convergence_2.png
        :align: center
        :width: 80%
        :class: custom-dark
    """
    ...
    # Check arguments
    check_arguments(coeff, return_info)

    # Define matrix function
    cdef Function* matrix_function = new Homographic(
...
def check_arguments(coeff, return_info):
    """
    Checks the type and value of the parameters.
    """

    # Check coeff
    if (not isinstance(coeff, (tuple, list, numpy.array))):
        raise TypeError('"coeff" should be a tuple, list, or numpy array.')
    elif len(coeff) != 4:
        raise ValueError('"coeff" should be an array or list of length 4.')
    if (coeff[0] * coeff[3] == coeff[1] * coeff[2]):
        raise ValueError('"coeff" of the linear fractional transformation ' +
                         'are degenerate.')
