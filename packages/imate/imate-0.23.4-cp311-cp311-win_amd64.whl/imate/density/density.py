...
def density(
    ...
    Note that the input matrix should have real eigenvalues, requiring the
    input matrix to be symmetric. However, if ``gram`` is set to `True`, the
    matrix can be non-symmetric or even non-square.
    ...
    A : numpy.ndarray, scipy.sparse, :class:`imate.Matrix`, or \
        ...
        :ref:`slq method <imate.density.slq>`. If ``method=cholesky``, the
        matrix `A` should be positive-definite. If ``gram=False``, the input
        matrix `A` should be symmetric. If ``gram=True``, the input matrix can
        be non-square.
    ...
    method : {'eigenvalue', 'slq'}, default='eigenvalue'
        ...
        * :ref:`eigenvalue <imate.density.eigenvalue>`
        * :ref:`slq <imate.density.slq>`
    ...
    * :ref:`eigenvalue <imate.density.eigenvalue>`: uses spectral
      decomposition.
      Suitable for small matrices (:math:`n < 2^{12}`). The solution is exact.
    * :ref:`slq <imate.density.slq>`: uses stochastic Lanczos quadrature
    ...
    **Sparse matrix:**
    ...
    .. code-block:: python
        ...
        >>> # Import packages
        >>> from imate import toeplitz, density
    ...
    **Output information:**
    ...
    .. code-block:: python
        ...
        >>> ld, info = density(A, return_info=True)
        >>> print(ld)
        138.6294361119891
    ...
    **Matrix operator:**
    ...
    :ref:`slq method <imate.density.slq>`.
    ...
    **Affine matrix operator:**
    ...
    in :ref:`slq method <imate.density.slq>`.
    ...
    .. code-block:: python
        ...
        >>> density(Aop, method='slq', parameters=t)
