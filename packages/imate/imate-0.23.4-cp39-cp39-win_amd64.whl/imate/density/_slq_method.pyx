...
def slq_method(
    ...
    where :math:`f` is the Gaussian function given by

    .. math::

        f(\\lambda \\vert \\mu, \\sigma) = \\frac{1}{\\sqrt{2 \\pi} \\sigma}
        \\mathrm{exp} \\left( -\\frac{1}{2}
        \\frac{(\\lambda - \\mu)^2}{\\sigma^2} \\right).

    When the parameter :math:`\\sigma` is chosen small enough, the above
    Gaussian function effectively approximates the Dirac's Delta function
    :math:`\\delta(\\lambda - \\mu)`, and the trace of this quantity
    approximates the spectral density of the given matrix at
    :math:`\\lambda = \\mu`.
    ...
    mu : float, default=1.0
        The parameter :math:`\\mu` of the Gaussian function.

    sigma : float, default=1.0
        The parameter :math:`\\sigma` of the Gaussian function.
    ...
    **Symmetric Input Matrix:**
    ...
    The `slq` method requires the input matrix of :func:`imate.density` function
    ...
    .. code-block:: python
        ...
        >>> density(A, gram=False, p=2.5, method='slq')
        277.00578301560057
    ...
    **Gramian Matrix:**
    ...
    Passing ``gram=True`` to :func:`imate.density` function uses the Gramian
    ...
    .. code-block:: python
        ...
        >>> density(B, gram=True, p=2.5, method='slq')
        275.15518680196476
    ...
    **Verbose output:**
    ...
    .. literalinclude:: ../_static/data/imate.density.slq-verbose-1.txt
        :language: python
    ...
    **Output information:**
    ...
    .. code-block:: python
        ...
        >>> ld, info = density(A, method='slq', return_info=True)
        >>> print(ld)
        138.6294361119891
    ...
    **Large matrix:**
    ...
    .. code-block:: python
        ...
        >>> ld, info = density(A, method='slq', min_num_samples=100,
    ...
    **Plotting:**
    ...
    .. code-block:: python
        ...
        >>> A = toeplitz(2, 1, size=1000000, gram=True)
        >>> density(A, method='slq', min_num_samples=20, max_num_samples=80,
        ...       error_rtol=2e-4, confidence_level=0.95,
        ...       outlier_significance_level=0.001, plot=True)

    .. image:: ../_static/images/plots/density_slq_convergence_1.png
        :align: center
        :class: custom-dark
    ...
    **Matrix operator:**
    ...
    pass the matrix `A` to the `density` function.

    .. code-block:: python
        :emphasize-lines: 8

        >>> # Import matrix operator
        >>> from imate import toeplitz, density, Matrix
        ...
        >>> density(Aop, method='slq')
        141.52929878934194

    An advantage of passing `Aop` (instead of `A`) to the `density` function
    ...
    **Computation on GPU:**
    ...
    .. code-block:: python
        ...
        >>> # Compute trace-exponential of Aop
        >>> density(Aop, method='slq', gpu=True)
        141.52929878934194
    ...
    calling the above `density` function (or another function) again on this
    ...
    .. code-block:: python
        ...
        >>> # Compute trace-exponential of Aop
        >>> density(Aop, method='slq', gpu=True, gram=True)
        141.52929878934194
    ...
    **Affine matrix operator:**
    ...
    The object :math:`\\mathbf{A}(t)` can be passed to `density` function with
    ...
    .. code-block:: python
        ...
        >>> from imate import toeplitz, density, AffineMatrixFunction
        ...
        >>> density(Aop, method='slq', parameters=t, min_num_samples=20,
    ...
    .. literalinclude:: ../_static/data/imate.density.slq-verbose-2.txt
        :language: python

    The output of the plot is shown below. Each colored curve corresponds to
    a parameter in ``t = [-1, 0, 1]``.

    .. image:: ../_static/images/plots/density_slq_convergence_2.png
        :align: center
        :width: 80%
        :class: custom-dark
    """
...
def check_arguments(mu, sigma, return_info):
    """
