...
from ..functions cimport pyFunction, Function, Exponential
...
def slq_method(
    ...
    .. math::
        ...
        \\mathrm{trace} \\; \\mathrm{exp}
        \\left(c (\\mathbf{A}^{\\intercal}\\mathbf{A})^p \\right).
    ...
    coeff : float, default=1.0
        The coefficient :math:`c` in
        :math:`\\mathrm{trace} \\mathrm{exp} \\left(c \\mathbf{A}^p \\right)`.
    ...
    **Symmetric Input Matrix:**
    ...
    The `slq` method requires the input matrix of :func:`imate.trexp` function
    ...
    .. code-block:: python
        ...
        >>> trexp(A, gram=False, p=2.5, method='slq')
        277.00578301560057
    ...
    **Gramian Matrix:**
    ...
    Passing ``gram=True`` to :func:`imate.trexp` function uses the Gramian
    ...
    .. code-block:: python
        ...
        >>> trexp(B, gram=True, p=2.5, method='slq')
        275.15518680196476
    ...
    **Verbose output:**
    ...
    .. literalinclude:: ../_static/data/imate.trexp.slq-verbose-1.txt
        :language: python
    ...
    **Output information:**
    ...
    .. code-block:: python
        ...
        >>> ld, info = trexp(A, method='slq', return_info=True)
        >>> print(ld)
        138.6294361119891
    ...
    **Large matrix:**
    ...
    .. code-block:: python
        ...
        >>> ld, info = trexp(A, method='slq', min_num_samples=100,
    ...
    **Plotting:**
    ...
    .. code-block:: python
        ...
        >>> A = toeplitz(2, 1, size=1000000, gram=True)
        >>> trexp(A, method='slq', min_num_samples=20, max_num_samples=80,
        ...       error_rtol=2e-4, confidence_level=0.95,
        ...       outlier_significance_level=0.001, plot=True)

    .. image:: ../_static/images/plots/trexp_slq_convergence_1.png
        :align: center
        :class: custom-dark
    ...
    **Matrix operator:**
    ...
    pass the matrix `A` to the `trexp` function.

    .. code-block:: python
        :emphasize-lines: 8

        >>> # Import matrix operator
        >>> from imate import toeplitz, trexp, Matrix
        ...
        >>> trexp(Aop, method='slq')
        141.52929878934194

    An advantage of passing `Aop` (instead of `A`) to the `trexp` function
    ...
    **Computation on GPU:**
    ...
    calling the above `trexp` function (or another function) again on this
    ...
    .. code-block:: python
        ...
        >>> # Compute trace-exponential of Aop
        >>> trexp(Aop, method='slq', gpu=True, gram=True)
        141.52929878934194
    ...
    **Affine matrix operator:**
    ...
    The object :math:`\\mathbf{A}(t)` can be passed to `trexp` function with
    ...
    .. code-block:: python
        ...
        >>> from imate import toeplitz, trexp, AffineMatrixFunction
        ...
        >>> trexp(Aop, method='slq', parameters=t, min_num_samples=20,
    ...
    .. literalinclude:: ../_static/data/imate.trexp.slq-verbose-2.txt
        :language: python

    The output of the plot is shown below. Each colored curve corresponds to
    a parameter in ``t = [-1, 0, 1]``.

    .. image:: ../_static/images/plots/trexp_slq_convergence_2.png
        :align: center
        :width: 80%
        :class: custom-dark
    """
    ...
    cdef Function* matrix_function = new Exponential(coeff)
