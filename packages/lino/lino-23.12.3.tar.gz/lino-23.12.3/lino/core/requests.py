...
class BaseRequest(object):
    ...
    # """
    #
    # Used in permalink URIs creation. Set this to `True` if the front_end root
    # has an hash router. Ex: `lino_react`.
    #
    # """
    ...
    def __init__(self, request=None, parent=None, hash_router=None,
    ...
    def setup(self,
        ...
        #     if self.master_instance is None:
        #         raise Exception("20230426 a {}".format(self.master))

        if self.master is not None and self.master_instance is None:
            # master_instance might have been inherited from parent
            if master_instance is None:
                master_instance = self.get_master_instance(self.master, master_key, master_type)
            ...
            if pk and pk != '-99998'  and pk != '-99999':
                ...
                if obj is None:
                    raise exceptions.ObjectDoesNotExist(
                        "Invalid primary key {0} for {1}".format(
                            pk, self.actor))
    ...
    def get_master_instance(self, master, mk, mt):
        ...
        if master is not None:
            ...
            if settings.SITE.is_installed('contenttypes'):
                if (issubclass(master, models.Model)
                    ...
                    try:
                        ...
                        # master = UnresolvedModel  # (master, mt)
                        raise exceptions.BadRequest(
                            "Invalid master type {}".format(mt))
                        # pass
                        # master is None
            pk = mk
            # if pk == '':
            ...
            if True:
                ...
                # if master_instance is None:
                #     master_instance = UnresolvedModel(master, pk)
                    # raise exceptions.ObjectDoesNotExist(
                    # raise exceptions.BadRequest(
                    #     "No master {0}, {1} for {2}".format(
                    #         master, pk, self.actor))
...
signature as `django.core.mail.EmailMessage`.
        ...
        if '@example.com' in sender:
            ...
            self.logger.debug(
                "Ignoring email because sender is %s", sender)
            self.logger.debug(PRINT_EMAIL.format(
...
class ActionRequest(BaseRequest):
    ...
    def create_phantom_rows(self, **kw):
        ...
        #     return
        if self.create_kw is None or self.actor.hide_editing(self.get_user().user_type) \
