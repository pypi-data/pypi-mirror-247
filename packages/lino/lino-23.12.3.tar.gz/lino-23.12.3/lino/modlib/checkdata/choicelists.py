...
class Checker(dd.Choice):
    ...
    def update_unbound_problems(cls, ar, **kwargs):
        ...
        todo, done = cls.self.update_problems(ar, **kwargs)
        msg = "Found {0} and fixed {1} checkdata messages for {2}."
    ...
    def update_problems(self, ar, obj=None, delete=True, fix=False):
        Message = rt.models.checkdata.Message
        if delete:
            # if obj is None:
            #     flt = {
            #         Message.owner.ct_field.name + "__isnull": True,
            #         Message.owner.fk_field.name + "__isnull": True
            #     }
            # else:
            #     flt = gfk2lookup(Message.owner, obj, checker=self)
            flt = gfk2lookup(Message.owner, obj, checker=self)
            qs = Message.objects.filter(**flt)
            qs.delete()
        ...
        for fixable, msg in self.get_checkdata_problems(obj, fix):
            ...
            if fixable and fix:
                ...
                todo.append(msg)
            if obj is None:
                ar.logger.info("- {} : {}".format(self.value, msg))
            else:
                ar.logger.info("- {} : {}".format(obj, msg))
        ...
        if len(todo):
            ...
            with translation.override(lang):
                if obj is None:
                    for msg in todo:
                        prb = Message(message=str(msg), checker=self, user=user)
                        prb.full_clean()
                        prb.save()
                else:
                    msg = '\n'.join([str(s) for s in todo])
                    prb = Message(owner=obj, message=msg, checker=self, user=user)
                    prb.full_clean()
                    prb.save()
        return (todo, done)
