...
from jf_ingest.config import IngestionConfig, IssueMetadata, JiraConfig
...
def get_jira_connection(config: JiraConfig, creds, max_retries=3) -> JIRA:
    kwargs = {
...
def _project_is_accessible(jira_connection: JIRA, project_id: str, project_key: str) -> bool:
    """Helper function to determine if a project is accessible or not

    Works by hitting the search (issues) API endpoint for a project. IF no error is raised, we have access to that project.

    If a 400 is raised, we catch and release and return false. Any other exception is raised

    Args:
        jira_connection (JIRA): A Jira Connection Object
        project_id (str): A list of project IDs (str)

    Returns:
        bool: True if we have access to a project
    """
    try:
        ...
        if (
            e.status_code == 400
            and e.text == f"A value with ID '{project_id}' does not exist for the field 'project'."
        ):
            logging_helper.log_standard_error(
                logging.WARNING,
                msg_args=[f"{project_key} (ID: {project_id})"],
                error_code=2112,
            )
            return False
        else:
...
def download_projects_and_versions_and_components(
    ...
    projects = [
        ...
        and _project_is_accessible(jira_connection, proj.id, proj.key)
    ]
...
def download_issues(
    ...
    Args:
        ...
        jellyfish_issue_metadata (list[IssueMetadata]): A list of Jellyfish Issue Metadata, useful for
        determining if we already have issues or if issues got delted
    ...
    Returns:
        tuple[list[dict], list[int]]: This function returns two values (two lists) that represent the issues
        we downloaded AND a list of IDs that got deleted from the remote JIRA source
    """
    #######################################################################
    # Pull Jira Issue Meta Data
    #######################################################################

    # For Jira Issue operations we need to determine the batch size that
    # the JIRA provider will limit us to
    jira_issues_batch_size = get_jira_search_batch_size(jira_connection=jira_connection)
    logger.debug(f"All issue operations will use a batch_size of {jira_issues_batch_size}")
    ...
    # First, detect which issues have been deleted on JIRA and track them.
    # To find these, take the difference between sets of Issues in JF
    # vs issues in Jira
    deleted_issue_ids = get_ids_from_difference_of_issue_metadata(
        source=jellyfish_issue_metadata,
        dest=issue_metadata_from_jira,
    )
    ...
    out_of_date_issue_ids = get_out_of_date_issue_ids(
        ...
        issue_metadata_from_jellyfish=jellyfish_issue_metadata,
    ...
    logger.info(
        ...
        f"{len(deleted_issue_ids)} issues have been deleted, "
    ...
    jira_issues = pull_jira_issues_by_jira_ids(
        jira_connection=jira_connection,
    ...
    logger.info(
        f"Done downloading issues. Found {len(jira_issues)} issues to download, and {len(deleted_issue_ids)} to delete"
    )
    return jira_issues, deleted_issue_ids
...
def get_jira_search_batch_size(jira_connection: JIRA, optimistic_batch_size: int = 1000) -> int:
    """A helper function that gives us the batch size that the
    ...
    Args:
        jira_connection (JIRA): A Jira Connection Object
        optimistic_batch_size (int, optional): An optimistic batch size. Defaults to 1000.
...
def _download_issue_page(
    ...
    while True:
        try:
            # Get Issues
            issues: list[Issue] = retry_for_429s(
                jira_connection.search_issues,
                jql_query,
            ...
            issues_raw: list[dict] = [issue.raw for issue in issues]

            # Potentially expand the changelogs
            issues_raw = _expand_changelog(jira_connection, issues_raw, batch_size)
...
def pull_jira_issues_by_jira_ids(
    ...
    Returns:
        list[dict]: A list of issues, which should be the same length of the jira_ids list we provided
    """
    ...
    logger.debug(f"Found {len(all_issues)}/{len(jira_ids)} issues!")
    return all_issues
...
def download_all_issue_metadata(
    ...
    if missing_parent_id_set:
        ...
        parent_issues = pull_jira_issues_by_jira_ids(
            jira_connection=jira_connection,
