...
from turcar import get_runner, get_workbench, misc_utils, tktextext
from turcar.common import InlineCommand, UserError, get_dirs_children_info
from turcar.languages import tr
from turcar.misc_utils import get_menu_char, running_on_mac_os, running_on_windows, sizeof_fmt
...
class BaseFileBrowser(ttk.Frame):
    ...
    def update_node_data(self, node_id, name, data):
        ...
        if data["isdir"]:
            ...
            elif self.should_open_name_in_turcar(name):
    ...
    def on_double_click(self, event):
        ...
        if kind == "file":
            if self.should_open_name_in_turcar(name):
                self.open_file(path)
            else:
    ...
    def on_secondary_click(self, event):
        ...
        if ext == '.pdf':
            ...
            self.menu.add_command(
                label='打开pdf文件', command=lambda: self.open_pdf(self.get_selected_path())
            )
        elif ext == '.mp4':
            self.menu.delete(0, "end")
            self.menu.add_command(
                label='播放视频', command=lambda: self.video_play(self.get_selected_path())
            )
        else:
    ...
    def open_pdf(self, path):
        ...
        # get_workbench().show_view("PdfView", set_focus=False)
        # get_workbench().event_generate("PdfView", content=path)
    def video_play(self, path):
        get_workbench().show_view("VideoPlay", set_focus=False)
        get_workbench().event_generate("VideoPlay", path=path)
    ...
    def add_first_menu_items(self, context):
        ...
        if context == "button":
            ...
            if selected_kind == "dir":
                ...
                self.menu.add_command(
                    label=tr("Open in turcar"), command=lambda: self.open_file(selected_path)
                )
    ...
    def open_extension_dialog(self, extension: str) -> None:
        ...
        turcar_choice = tr("Open in turcar's text editor")

        current_index = (
            1 if get_workbench().get_option(get_file_handler_conf_key(extension)) == "turcar" else 0
        )

        choice = ask_one_from_choices(
            title=tr("Configure %s files") % extension,
            question=tr(
                "What to do with a %s file when you double-click it in turcar's file browser?"
            )
            % extension,
            choices=[system_choice, turcar_choice],
        ...
        get_workbench().set_option(
            ...
            "system" if choice == system_choice else "turcar",
        )
        # update icons
        self.refresh_tree()
    ...
    def should_open_name_in_turcar(self, name):
        ext = self.get_extension_from_name(name)
        return get_workbench().get_option(get_file_handler_conf_key(ext), "system") == "turcar"
...
class BaseRemoteFileBrowser(BaseFileBrowser):
    ...
    def open_path_with_system_app(self, path):
        messagebox.showinfo(
            ...
            + "\n\n"
            + tr(
                "If it is a text file, then you can configure it to open in turcar "
...
class DialogRemoteFileBrowser(BaseRemoteFileBrowser):
    ...
    def should_open_name_in_turcar(self, name):
        # In dialog, all file types are to be opened in turcar
        return True
