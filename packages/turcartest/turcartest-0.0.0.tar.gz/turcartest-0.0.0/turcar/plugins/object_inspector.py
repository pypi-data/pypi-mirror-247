...
import turcar.memory
from turcar import get_runner, get_workbench, ui_utils
from turcar.common import InlineCommand
from turcar.languages import tr
from turcar.memory import MemoryFrame
from turcar.misc_utils import shorten_repr
from turcar.tktextext import TextFrame
from turcar.ui_utils import CustomToolbutton, ems_to_pixels
...
class ObjectInspector(ttk.Frame):
    ...
    def _show_object_by_id(self, object_id, via_navigation=False):
        ...
        if self.winfo_ismapped() and self.object_id != object_id:
            ...
            self._set_title("object @ " + turcar.memory.format_object_id(object_id))
    ...
    def set_object_info(self, object_info):
        ...
        if object_info is None or "error" in object_info:
            ...
            self._set_title(
                ...
                + " @ "
                + turcar.memory.format_object_id(object_info["id"])
            )
...
class ElementsInspector(turcar.memory.MemoryFrame, ContentInspector):
    def __init__(self, master):
        ContentInspector.__init__(self, master)
        turcar.memory.MemoryFrame.__init__(
    ...
    def set_object_info(self, object_info):
        ...
        for element in object_info["elements"]:
            ...
            self.tree.set(node_id, "id", turcar.memory.format_object_id(element.id))
            self.tree.set(
                node_id, "value", shorten_repr(element.repr, turcar.memory.MAX_REPR_LENGTH_IN_GRID)
            )
            index += 1
...
class DictInspector(turcar.memory.MemoryFrame, ContentInspector):
    def __init__(self, master):
        ContentInspector.__init__(self, master)
        turcar.memory.MemoryFrame.__init__(
    ...
    def set_object_info(self, object_info):
        ...
        for key, value in object_info["entries"]:
            node_id = self.tree.insert("", "end")
            self.tree.set(node_id, "key_id", turcar.memory.format_object_id(key.id))
            self.tree.set(
                node_id, "key", shorten_repr(key.repr, turcar.memory.MAX_REPR_LENGTH_IN_GRID)
            )
            self.tree.set(node_id, "id", turcar.memory.format_object_id(value.id))
            self.tree.set(
                node_id, "value", shorten_repr(value.repr, turcar.memory.MAX_REPR_LENGTH_IN_GRID)
            )
...
class AttributesFrame(turcar.memory.VariablesFrame):
    def __init__(self, master):
        turcar.memory.VariablesFrame.__init__(self, master)
    ...
    def show_selected_object_info(self):
        ...
        if repr_str == "<bound_method>":
            from turcar.plugins.micropython import MicroPythonProxy
