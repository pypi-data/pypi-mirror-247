...
def load_plugin():
    old_data_dir = os.path.join(os.path.expanduser("~"), ".turcar")
    if os.path.exists(old_data_dir):
        ...
        def doit():
            ...
            answer = askyesno(
                ...
                "turcar versions before 3.0 (and first 3.0 betas) used to keep "
                + "configuration, logs and such in '%s'" % old_data_dir
                + ". "
                + "Since 3.0 this data is kept in a new location: '%s'.\n\n" % turcar_USER_DIR
                + "If you don't intend to use older turcar versions anymore, "
        ...
        get_workbench().add_command(
            "delolddatadir", "tools", "Clean up turcar 2.1 data folder ...", doit, group=110
        )
