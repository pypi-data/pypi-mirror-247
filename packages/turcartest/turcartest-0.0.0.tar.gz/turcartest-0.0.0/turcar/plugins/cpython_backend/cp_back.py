...
from turcar.backend import MainBackend, logger
...
_REPL_HELPER_NAME = "_turcar_repl_print"

_CONFIG_FILENAME = os.path.join(turcar.turcar_USER_DIR, "backend_configuration.ini")
...
class MainCPythonBackend(MainBackend):
    def __init__(self, target_cwd, options):
        ...
        self._main_dir = os.path.dirname(sys.modules["turcar"].__file__)
        ...
        # required in shell mode and also required to overwrite turcar location dir
    ...
    def _init_command_reader(self):
        ...
        # https://github.com/turcar/turcar/issues/1363
        pass
    ...
    def _handle_immediate_command(self, cmd: ImmediateCommand) -> None:
        ...
        was reverted for CPython https://github.com/turcar/turcar/issues/1363.
    ...
    def _load_plugins(self):
        # built-in plugins
        try:
            import turcar.plugins.backend  # pylint: disable=redefined-outer-name
        except ImportError:
            # May happen eg. in ssh session
            return

        self._load_plugins_from_path(turcar.plugins.backend.__path__, "turcar.plugins.backend.")

        # 3rd party plugins from namespace package
        try:
            import turcarcontrib.backend  # @UnresolvedImport
        except ImportError:
            # No 3rd party plugins installed
            pass
        else:
            self._load_plugins_from_path(turcarcontrib.backend.__path__, "turcarcontrib.backend.")
    ...
    def _cmd_FastDebug(self, cmd):
        self.switch_env_to_script_mode(cmd)
        from turcar.plugins.cpython_backend.cp_tracers import FastTracer
    ...
    def _cmd_Debug(self, cmd):
        ...
        from turcar.plugins.cpython_backend.cp_tracers import NiceTracer
    ...
    def export_value(self, value, max_repr_length=5000):
        ...
        try:
            ...
            # See https://bitbucket.org/plas/turcar/issues/584/problem-with-turcars-back-end-obj-no
            rep = "??? <repr error>"
...
def format_exception_with_frame_info(e_type, e_value, e_traceback, shorten_filenames=False):
    """Need to suppress turcar frames to avoid confusion"""
        ...
        if tb is not None:
            ...
            for entry in traceback.extract_tb(tb):
                ...
                if (
                    ...
                    and "turcar/backend" not in entry.filename.replace("\\", "/")
                    and (
...
def in_debug_mode():
    return os.environ.get("turcar_DEBUG", False) in [1, "1", True, "True", "true"]
