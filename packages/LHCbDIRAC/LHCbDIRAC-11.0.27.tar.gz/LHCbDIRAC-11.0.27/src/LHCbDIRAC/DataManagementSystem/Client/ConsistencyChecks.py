...
class ConsistencyChecks(DiracConsistencyChecks):
    ...
    def getReplicasPresenceFromDirectoryScan(self, lfns, typeStr="files"):
        ...
        for dirN in sorted(dirs):
            ...
            self.__logVerbose(f"Obtained {len(lfnsFound)} files in {time.time() - startTime1:.1f} seconds")
            if compare:
                ...
                if resChunk["OK"] and not resChunk["Value"]["Failed"]:
                    ...
                    for lfn in lfnChunk:
                        ...
                        if lfn in descDict:
                            ...
                            multi = {ft: ftc for ft, ftc in ft_count[lfn].items() if ftc > 1}
                            if multi:
                    ...
                    progressBar.comment(
                        f"Error getting daughters for {len(lfnChunk)} files, retry", resChunk["Message"]
                    )
        prStr = ""
    ...
    def getDescendants(self, lfns, status=""):
        ...
        if filesWithDescendants:
            ...
            if setNotPresent:
                ...
                progressBar.endLoop(
                    message="found %d descendants of %d daughters"
                ...
                for lfn in set(filesWithDescendants):
                    ...
                    if not descToCheck:
                        ...
                        multi = {ft: ftc for ft, ftc in ft_count.items() if ftc > 1}
