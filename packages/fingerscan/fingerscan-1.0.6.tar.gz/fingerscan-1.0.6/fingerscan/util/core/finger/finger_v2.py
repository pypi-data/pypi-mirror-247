...
from fingerscan.export.file_path import FileUtils, PathInit


class FingerScan(object):
    ...
    @staticmethod
    def _finger_file_data():
        """
        获取finger文件指纹数据
        :return:
        """
        blacklist_file_name = PathInit.SCRIPT_PATH() + "/fingerscan/export/config"
        log.warning(blacklist_file_name)
        finger_path = f'{blacklist_file_name}/iotscan.json'
        with open(finger_path, 'r', encoding='utf-8') as file:
            finger_list = json.load(file)
        return finger_list
    ...
    def _run_get_finger(self, url: str, title: str, icon_hash: str, banner: str, body: str):
        ...
        finger_list = self._finger_file_data().get('finger')
    ...
    def _make_request(self, url, path, req_method):
        ...
        for _ in range(3):
            try:
                # 请求制定path路径应该不跟随请求
                resp = getattr(requests, req_method)(url=url + path, headers=self.headers, timeout=10, verify=False,
    ...
    def get_finger(self, res: requests.Response, content: str):
        """
        获取识别指纹
        ...
        banner = str(res.headers)
        body = res.text
        url = res.request.url
        cms = self._run_get_finger(url=url, title=title, icon_hash=icon_hash, banner=banner, body=body)
