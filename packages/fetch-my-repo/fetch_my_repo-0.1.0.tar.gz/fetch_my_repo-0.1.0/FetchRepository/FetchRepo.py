...
class FetchRepo:
    ...
    def __download_and_extract_repo(self, https_link, branch=None, headers=None):
    ...
    def __read_ignore_files(self, file_path):
        ignore_files = []
        with open(file_path, 'r') as f:
            ignore_files = [line.strip() for line in f.readlines()]
        return ignore_files

    def __exclude_files(self, file_path, ignore_files):
        for excluded_file in ignore_files:
            if excluded_file.lower() in file_path.lower():
                return True
        return False

    def __convert_to_json(self, repo_content, root_dir, ignore_files):
        result = {}
        if not repo_content:
            return result
        ...
        with tempfile.TemporaryDirectory() as temp_dir, \
            ...
            for root, _, files in os.walk(temp_dir):
                ...
                for file in files:
                    ...
                    if self.__exclude_files(file_path, ignore_files):
                        continue

                    with open(os.path.join(root, file), 'rb') as f:
                        file_content = f.read()
                        if self.__is_binary(file_content):
                            continue
                        file_content = file_content.decode('utf-8', errors='replace')

                    components = file_path.split(os.path.sep)
                    current_obj = current_dir
                    for component in components[:-1]:
                        current_obj = current_obj.setdefault(component, {})
                    current_obj[components[-1]] = file_content

        return result

    def __print_repo_content_json(self, repo_content, root_dir, ignore_files):
        json_structure = self.__convert_to_json(repo_content, root_dir, ignore_files)
        formatted_json = json.dumps(json_structure, indent=2, separators=(',', ': '), sort_keys=True)

        # Split the formatted JSON into lines and concatenate with the desired indentation
        formatted_lines = ('\n').join(formatted_json.splitlines())
        print(formatted_lines)

    def __get_github_repo_branches(self, repo_link, headers=None):
    ...
    def harvest_github_repo(self, repo_link, branch=None, access_token=None):
        ...
        ignore_files = self.__read_ignore_files(ignore_file_path)

        if not branch:
            branches = self.__get_github_repo_branches(repo_link, headers=self.headers)
            if not branches:
        ...
        repo_content = self.__download_and_extract_repo(f"{repo_link}/archive/{branch}.tar.gz", headers=self.headers)
        if not repo_content:
            print("Failed to download and extract repository content.")
            return None

        root_directory = os.getcwd()
        self.__print_repo_content_json(repo_content, root_directory, ignore_files)
