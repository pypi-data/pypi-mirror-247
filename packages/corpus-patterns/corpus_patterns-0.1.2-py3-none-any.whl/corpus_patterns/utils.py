...
class Rule(BaseModel):
    ...
    def check_path(self, asset_folder: Path, filename: str) -> Path:
        if not asset_folder.exists():
            raise Exception(f"Missing {asset_folder=}")
        return asset_folder.joinpath(filename)

    def phrases_from_disk(self, asset_folder: Path) -> list[str]:
        """Get rule's phrases from expected location."""
        file = self.check_path(asset_folder, self.phrase_path)
        if not file.exists():
            return []
        return make_uniform_lines(file)

    def convert_phrases_to_patterns(self, asset_folder: Path) -> list[dict[str, Any]]:
        """Use `phrases_from_disk` to construct a list of patterns."""
        objs = self.phrases_from_disk(asset_folder)
        patterns = self.create_patterns(objs)
        return patterns

    def matchers_to_disk(self, asset_folder: Path) -> Path:
        """Overwrites conventional patterns.json file in a proper folder."""
        file = self.check_path(asset_folder, self.matcher_path)
        srsly.write_json(path=file, data=self.matchers)
        return file

    def matchers_from_disk(self, asset_folder: Path) -> list[list[dict[str, Any]]]:
    ...
    def convert_matchers_to_patterns(self, asset_folder: Path) -> list[dict[str, Any]]:
        """Use `matchers_from_disk` to construct a list of patterns."""
        objs = self.matchers_from_disk(asset_folder)
        patterns = self.create_patterns(objs)
        return patterns

    def collect_patterns(self, asset_folder: Path):
        phrases = self.convert_phrases_to_patterns(asset_folder)
        matchers = self.convert_matchers_to_patterns(asset_folder)
