...
def handle_get(options, _, config):
    ...
    if options.json:
        ...
        options.json_out.write(json.dumps(plan.as_dict(), indent=4))
    else:
        print(
...
def handle_list(options, _, config):
    ...
    if options.json:
        ...
        options.json_out.write(json.dumps([p.as_dict() for p in plans], indent=4))
    else:
        while True:
            previous_pt = None
            for plan in plans:
            ...
            if sys.stdout.isatty():
                ...
                plans, n_token = fetch_next_non_interactive(
                    Plan, config, url, n_token, options.limit
                )
    return 0


def get_pc_headers(headers):
    headers_dict = {}
...
def handle_submit(cmdargs, extra_arguments, config):
    ...
    if cmdargs.plan_callback_header:
        if not cmdargs.plan_callback:
            ...
            pc_headers = get_pc_headers(cmdargs.plan_callback_header)
...
def handle_create(cmdargs, extra_arguments, config):
