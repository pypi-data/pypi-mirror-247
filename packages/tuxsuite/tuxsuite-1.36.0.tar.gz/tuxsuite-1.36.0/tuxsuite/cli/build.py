...
def handle_list(options, _, config):
    ...
    if options.json:
        ...
        options.json_out.write(json.dumps([b.as_dict() for b in builds], indent=4))
    else:
        while True:
            previous_pt = None
            for build in builds:
            ...
            if sys.stdout.isatty():
                ...
                builds, n_token = fetch_next_non_interactive(
                    Build, config, url, n_token, options.limit
                )
    return 0
...
def setup_parser(parser):
    ...
    p.add_argument("uid")
    p.add_argument("--json", action="store_true", help="Output json build to stdout")
    p.add_argument(
        "--json-out",
