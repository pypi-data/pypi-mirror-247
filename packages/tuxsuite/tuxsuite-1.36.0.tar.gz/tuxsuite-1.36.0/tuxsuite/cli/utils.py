...
def fetch_next_non_interactive(cmd_cls, cfg, url, next_token, limit):
    ret = get(cfg, url, params={"start": next_token})
    next_token = ret.json()["next"]
    res = [cmd_cls.new(**r) for r in ret.json()["results"][:limit]]
    if not next_token:
...
def load_plan(plan_file):
    try:
        if re.match(r"^https?://", str(plan_file)):
            respose = GET(plan_file)
            if respose.status_code != 200:
                error(f"Failed to get plan file: {respose.text}")
            contents = respose.text
        else:
