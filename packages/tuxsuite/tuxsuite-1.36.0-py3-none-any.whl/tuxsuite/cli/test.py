...
def handle_list(options, _, config):
    ...
    if options.json:
        ...
        options.json_out.write(json.dumps([t.as_dict() for t in tests], indent=4))
    else:
        while True:
            previous_pt = None
            for test in tests:
            ...
            if sys.stdout.isatty():
                ...
                tests, n_token = fetch_next_non_interactive(
                    Test, config, url, n_token, options.limit
                )
    return 0
...
def test_cmd_options(sp):
    ...
    sp.add_argument(
        ...
        help=(
            ...
            "Valid values 'x86_64', 'm6a.large', 'm6g.large', 'm6id.large', 'm6gd.large'. "
...
def setup_parser(parser):
    ...
    t.add_argument("uid")
    t.add_argument("--json", action="store_true", help="Output json test to stdout")
    t.add_argument(
        "--json-out",
