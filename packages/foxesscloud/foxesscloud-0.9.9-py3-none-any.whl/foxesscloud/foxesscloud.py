...
def signed_header(path, login = 0):
    ...
    headers['Signature'] = hashlib.md5(fr"{path}\r\n{headers['Token']}\r\n{headers['Lang']}\r\n{headers['Timestamp']}".encode('UTF-8')).hexdigest() + '.5245784'
    if debug_setting > 1:
        print(f"path = {path}")
...
def get_token():
    global username, password, fox_user_agent, time_zone, token_store, device_list, device, device_id, debug_setting, token_save, token_renewal, messages
    if token_store is None:
        token_store = {'token': None, 'valid_from': None, 'valid_for': token_renewal, 'user_agent': fox_user_agent, 'lang': 'en', 'time_zone': time_zone}
    if messages is None:
        get_messages()
...
def get_device(sn=None):
    ...
    response = signed_post(path="/generic/v0/device/list", data=json.dumps(query))
    if response.status_code != 200:
...
def get_remote_setting():
    ...
    params = {id_name: id_code}
    response = signed_get(path="/bus/device/inverter/remoteSetting", params=params)
    if response.status_code != 200:
