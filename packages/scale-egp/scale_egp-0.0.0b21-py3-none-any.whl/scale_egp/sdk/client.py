...
from scale_egp.sdk.models.user_info import UserInfoResponse, get_user_info
...
class EGPClient:
    ...
    def __init__(
        ...
        *,
        log_curl_command: Optional[bool] = None,
        **kwargs: Dict[str, Any],
    ):
        """
        Args:
        ...
        self.log_curl_command = (
            log_curl_command
            if log_curl_command is not None
        ...
        if self.account_id is None:
            # TODO: if there are more accounts, taking the first one might not be the most intuitive logic
            self.account_id = self.user_info().accounts[0].account_id

    def user_info(self) -> UserInfoResponse:
        return get_user_info(self.httpx_client, self.endpoint_url, self.log_curl_command)
