    ...
    def delete_items(self, item_ids : List[int], trees: int = 16) -> None:
        ...
        def rebuild_vectors(item_id):
            ...
            def download_and_upload(i, plus_one):
                if plus_one == 1:
                    metadata = json.loads(self.cloud_manager.download_text_from_cloud(cloud_name(self.vault, self.map[str(i - plus_one)], self.user, self.api, meta=True)))
                    metadata['item_id'] = i - plus_one
                    self.cloud_manager.upload_to_cloud(cloud_name(self.vault, self.map[str(i - plus_one)], self.user, self.api, meta=True), json.dumps(metadata))
