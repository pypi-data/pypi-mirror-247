...
def main():
    ENV.setdefault("BDUSS","W1haW9uVVFIRURZQTMtfmZJLUwzaWd5Tm9FUm95QjIxSThiMERqNzBNTUQxWVJsSVFBQUFBJCQAAAAAAAAAAAEAAACJI7b1TGlaaGVuMDYyODU5AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAANIXWUDSF1la")
    if ('BDUSS' not in ENV):
        print("未配置BDUSS")
        return
    b = ENV['BDUSS'].split('#')
    ...
    for n, i in enumerate(b):
        print("开始签到第" + str(n+1) + "个用户" + i)
        tbs = get_tbs(i)
        favorites = get_favorite(i)
        for j in favorites:
            time.sleep(random.randint(1,5))
            client_sign(i, tbs, j["id"], j["name"])
        print("完成第" + str(n+1) + "个用户签到")
