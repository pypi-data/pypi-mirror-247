...
class WordInfo(object):
    ...
    def calculate_entropy(self, cnt_dict) -> float:
        """
        计算左熵和右熵
        Args:
            cnt_dict:
    ...
    def compute(self, length):
        ...
        """
        self.freq /= length
        self.left = self.calculate_entropy(self.left)
        self.right = self.calculate_entropy(self.right)

    def computeAggregation(self, words_dict):
        """
        计算词的凝聚度
        @param words_dict：候选词的词频
        """
        parts = self.gen_subparts(self.text)
        if len(parts) > 0:
            self.aggregation = min(self.freq / words_dict[p1_p2[0]].freq / words_dict[p1_p2[1]].freq for p1_p2 in parts)

    def gen_subparts(self, string):
        """
        划分字符串，生成所有可能的组合。比如给定字符串 "abcd"，生成：[("a", "bcd"), ("ab", "cd"), ("abc", "d")]
        对于长度为1的字符串，返回空列表
        """
        length = len(string)
...
class WordDiscoverer(object):
    ...
    def sorted_suffix_index(self, doc, max_word_len):
        """
        Treat a suffix as an index where the suffix begins.
        Then sort these indexes by the suffixes.
        """
        length = len(doc)
        indexes = ((i, j) for i in range(0, length) for j in range(i + 1, min(i + 1 + max_word_len, length + 1)))
    ...
    def genWords(self, doc):
        ...
        suffix_indexes = self.sorted_suffix_index(doc, self.max_word_len)
        word_cands = {}
    ...
    def genWords2(self, doc):
        ...
        values = word_cands.values()
        for v in values:
            v.left = v.calculate_entropy(v.left)
            v.right = v.calculate_entropy(v.right)
        return values
