            ...
            if 'updated_at' not in column_names:
                ...
                if result_dict['only_in_source'] > 0 and result_dict['only_in_dest'] == 0:
                    ...
                    if result:
                        ...
                        cdc_ts_result = destination.execute_sql(f"select max(server_ts),min(server_ts) from {result[0][0]}.`{cdc_table}`")[0]
                        ...
                        if cdc_min_server_ts > update_by_list[-1]:
                            ...
                            st.success(f"max server_ts <= min updated_at, check if cdc event is being consumed normally.")
                        else:
