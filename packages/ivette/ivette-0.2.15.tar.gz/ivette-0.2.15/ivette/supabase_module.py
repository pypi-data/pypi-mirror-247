...
@http_request
def get_next_job(server_id: str | None = None):
...
def update_job(job_id: str,
    ...
    *,
    thermo_data: ThermoData | None = None,
):
    ...
    if status == 'pending' and species_id:
        ...
        if response.data[0].get('status') == 'in progress':
            server_table.update({'status': 'turn off'}).eq('id', response.data[0].get('serverId')).execute()
