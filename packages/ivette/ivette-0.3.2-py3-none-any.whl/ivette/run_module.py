...
def run_nwchem(job_id, nproc):
    ...
    if nproc:
        command = [
            f"mpirun -np {nproc} --use-hwthread-cpus --allow-run-as-root $NWCHEM_TOP/bin/$NWCHEM_TARGET/nwchem tmp/{job_id}"]
    else:
        command = [
            f"mpirun -map-by core --use-hwthread-cpus --allow-run-as-root $NWCHEM_TOP/bin/$NWCHEM_TARGET/nwchem tmp/{job_id}"]
        ...
        if package in ["GAMESS US", "NWChem"] and (check_gamess_installation if package == "GAMESS US" else is_nwchem_installed):
            ...
            try:
                ...
                while not job_done:
                    ...
                    if should_exit:
                        print("Turning off requested, interrupting job...",
                              flush=True)
                ...
                if str(exc) == "Turning off requested":
