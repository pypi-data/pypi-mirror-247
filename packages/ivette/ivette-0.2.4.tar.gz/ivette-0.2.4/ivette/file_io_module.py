...
{'''driver
  maxiter 200
end''' if operation.upper() in ['OPTIMIZE', 'FREQ'] else ''}
        ...
        for geometry_match in geometry_patterns:
            ...
            for row in rows:
                geometry_data.append({
                    ...
                    "X": round(float(row[3]) / 1.8897259885789, len((str(row[3]).split(".")[1]))),
                    "Y": round(float(row[4]) / 1.8897259885789, len((str(row[3]).split(".")[1]))),
                    "Z": round(float(row[5]) / 1.8897259885789, len((str(row[3]).split(".")[1]))),
                })
