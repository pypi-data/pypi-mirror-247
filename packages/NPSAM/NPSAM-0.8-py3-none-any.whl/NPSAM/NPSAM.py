...
def download_weights(model):
    weights = {
        ...
        'FastSAM': [
            'https://drive.google.com/u/0/uc?id=1m1sjY4ihXBU1fZXdQ-Xdj-mDltW-2Rqv&export=download&confirm=t&uuid=bacf82f5-6461-44a5-8594-e534dd1b0509&at=AB6BwCD_YbFjLvGr0XiZVPtCQieh:1703072969660',
            'FastSAM.pt']
    }
    directory = os.path.dirname(NPSAM.__file__)

    if not os.path.exists(directory):
        print('NPSAM is not correctly installed')
        return
...
def choose_SAM_model(SAM_model):
    ...
    if SAM_model == 'auto':
        for model, type_name in model_types.items():
            ...
            if any(fname.startswith(checkpoint_file) for fname in available_SAM_models):
                ...
                return f'sam_vit_{type_name[1]}.pth', type_name[0]
    elif SAM_model in {'base', 'large', 'huge'}:
        checkpoint_file = f'sam_vit_{model_types.get(SAM_model)[1]}.pth'
        if any(fname.startswith(checkpoint_file) for fname in available_SAM_models):
            return f'sam_vit_{model_types.get(SAM_model)[1]}.pth', model_types.get(SAM_model)[0]
        else:
            _input = input(
    ...
    _input = input('No SAM weights were found. Do you want to download them? y/n: ')
    if _input.lower() == 'y':
        download_weights('huge')
        return 'sam_vit_h_4b8939.pth', 'vit_h'
...
class ImageFilter:
    ...
    def start_plot(self, image, labels):
        ...
        self.ax['left2'].imshow(self.filtered_masks_rebinned, cmap=self.label_cmap, interpolation='nearest', vmin=0,
                                vmax=self.vmax)
