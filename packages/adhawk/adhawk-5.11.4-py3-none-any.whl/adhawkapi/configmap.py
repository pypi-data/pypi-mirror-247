...
class ModelToReg(ConfigHandlerInterface):
    ...
    def write(self, value, api, configmodel, tracker_id, config, **extra_args):
        api.set_register(self._regkey, value)

    def read(self, api, configmodel, tracker_id, config, **extra_args):
        return api.get_register(self._regkey)
...
class ModelToBlob(ConfigHandlerInterface):
    ...
    def _get_blob_context(self, _api):  # pylint: disable=no-self-use
    ...
    def write(self, value, api, configmodel, tracker_id, config, **extra_args):
        ...
        blobversion = BlobVersion[self._blobtype.name].value
        try:
            blob = blob_parsers.create_blob(self._blobtype, blobversion, ctxt, value)
        except ValueError as exc:
            logging.warning(exc)
            return
        err = api.write_blob(self._blobtype, blob)
        if err != AckCodes.SUCCESS:
            logging.warning(f'Unable to write {config}')

    def read(self, api, configmodel, tracker_id, config, **extra_args):
        ctxt = self._get_blob_context(api)  # pylint: disable=assignment-from-none
        err, blob = api.read_blob(self._blobtype)
        if err != AckCodes.SUCCESS:
            return {}
        try:
            return blob_parsers.parse_blob(self._blobtype, ctxt, blob)
        except ValueError as exc:
            logging.warning(exc)
            return {}
...
class ModelToModuleCalBlob(ModelToBlob):
    '''Interface between the model and the module cal blob'''

    def _get_blob_context(self, _api):
        return blob_parsers.ModuleCalContext(defaults.MAX_EYES)
...
class ModelToSelectionRegister(ModelToReg):
    ...
    def read(self, api, configmodel, tracker_id, config, **extra_args):
        register_value = api.get_register(self._regkey)
...
class PdGain(ConfigHandlerInterface):
    ...
    def write(self, value, api, configmodel, tracker_id, config, **extra_args):
        ...
        api.set_register(regkey, value)

    def read(self, api, configmodel, tracker_id, config, **extra_args):
        if registers.SpecCapability.SHARED_DETECTOR_CAPABILITY in \
                api.get_register(registers.SPEC_CAPABILITY, control=True):
        ...
        regkey = self._regmap[extra_args['pd_id']]
        return api.get_register(regkey)
...
class AutotuneControl(ConfigHandlerInterface):
    ...
    def write(self, value, api, configmodel, tracker_id, config, **extra_args):
        ...
        api.set_register(self._regkey, regvalues)
...
class PdBitFields(ConfigHandlerInterface):
    ...
    def write(self, value, api, configmodel, tracker_id, config, **extra_args):
        ...
        for pd_id in range(max_pds):
            ...
            if value:
                choices.add(pd_id)
        api.set_register(self._regkey, choices)

    def read(self, api, configmodel, tracker_id, config, **extra_args):
        choices = api.get_register(self._regkey)
...
class DetectorType(ModelToReg):
    ...
    def write(self, value, api, configmodel, tracker_id, config, **extra_args):
        ...
        api.set_register(registers.SPEC_PD_TYPE, regvalues)
...
class PupilPdGain(ConfigHandlerInterface):
    ...
    def write(self, value, api, configmodel, tracker_id, config, **extra_args):
        ...
        api.set_register(self._regmap[extra_args['pd_id']], regvalues)
    ...
    def read(self, api, configmodel, tracker_id, config, **extra_args):
        if registers.SpecCapability.SHARED_DETECTOR_CAPABILITY in \
            ...
            # hardware does not support this, just return whats in the model
            return configmodel.get_value(trackermodel.construct_path(tracker_id, config, **extra_args))
        regvalues = api.get_register(self._regmap[extra_args['pd_id']])
...
def cache_and_restore(regapi, reglist):
    ...
    cache = []
    for reg in reglist:
        cache.append(regapi.get_register(reg))
    try:
        yield cache
    except Exception:  # pylint: disable=broad-except
        for reg, cache in zip(reglist, cache):
            regapi.set_register(reg, cache)
        raise
...
class LaserConfig(ConfigHandlerInterface):
    ...
    def write(self, value, api, configmodel, tracker_id, config, **extra_args):
        if config == configs.TRACKER_LASERCURRENTPCT:
            max_val = api.get_register(registers.SPEC_MAX_VCSEL_CURRENT)
            value = normalize.normalize(value, (0, 100), (0, max_val))

        # update the hardware
        api.set_register(registers.GENERAL1_LASER_CURRENT, value)

    def read(self, api, configmodel, tracker_id, config, **extra_args):
        value = api.get_register(registers.GENERAL1_LASER_CURRENT)
        if config == configs.TRACKER_LASERCURRENTPCT:
            max_val = api.get_register(registers.SPEC_MAX_VCSEL_CURRENT)
...
class TrackerSize(ConfigHandlerInterface):
    ...
    def write(self, value, api, configmodel, tracker_id, config, **extra_args):
        ...
        if config.endswith('pct'):
            ...
            spec_min_val = api.get_register(self._spec_min_reg)
            spec_max_val = api.get_register(self._spec_max_reg)
        ...
        with cache_and_restore(api, [self._min_reg, self._max_reg]):
            # update the hardware
            api.set_register(self._min_reg, min_val)
            api.set_register(self._max_reg, max_val)

    def read(self, api, configmodel, tracker_id, config, **extra_args):
        min_val = api.get_register(self._min_reg)
        max_val = api.get_register(self._max_reg)
        if config.endswith('pct'):
...
_CONFIG_HANDLER_MAP = {
    configs.GENERAL_APIVERSION: APIVersion(registers.GENERAL1_API_VERSION),
    configs.GENERAL_FIRMWAREVERSION: ModelToReg(registers.SPEC_BUILD),
    configs.GENERAL_NUMTRACKERS: NumTrackers(registers.GENERAL1_MULTI_DEVICES),
    configs.GENERAL_ACTIVEEYES: ActiveEyes(registers.SPEC_OCULAR_MODE),
    configs.EYETRACKER_PUPIL_OFFSET: ModelToReg(registers.GENERAL1_PUPIL_OFFSET),
    configs.EYETRACKER_PCCR_ENABLE: ModelToRegBool(registers.GENERAL1_PCCR_ENABLE),
    configs.EYETRACKER_REALTIME_Z_ENABLE: ModelToRegBool(registers.GENERAL1_REALTIME_Z_ENABLE),
    configs.EYETRACKER_POWER_CYCLING_PERIOD: ModelToReg(registers.GENERAL1_POWER_CYCLING_PERIOD),
    configs.EYETRACKER_POWER_CYCLING_DUTY: ModelToReg(registers.GENERAL1_POWER_CYCLING_DUTY),
    ...
    configs.TRACKER_DEADTIME_ENABLE: DeadTimeEnableConfig(registers.GENERAL1_DEAD_TIME_MODE),
    ...
    configs.AUTOTUNE_XRANGE_STEP: ModelToReg(registers.AUTOTUNE_X_STEP),
    ...
    configs.AUTOTUNE_YRANGE_STEP: ModelToReg(registers.AUTOTUNE_Y_STEP),
    ...
    configs.BLOB_TUNING_MODULE_CAL: ModelToModuleCalBlob(BlobType.MODULE_CAL),
