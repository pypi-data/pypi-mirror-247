''' binocular gaze stream for AdHawk Backend
...
- Run the application
  e.g python frontend.py frontend_binocular
...
class FrontendData:
    ...
    def __init__(self):
        ...
        self._api.register_stream_handler(adhawkapi.PacketType.PER_EYE_GAZE, self._handle_gazevecs_data)
        self._api.start(connect_cb=self._handle_connect)

    def _handle_connect(self, error):
        if not error:
            streams = [adhawkapi.PacketType.PER_EYE_GAZE]
            for stream in streams:
                self._api.set_stream_control(stream, 120, callback=(lambda *args: None))

    def _handle_gazevecs_data(self, timestamp, xright, yright, zright, xleft, yleft, zleft):
