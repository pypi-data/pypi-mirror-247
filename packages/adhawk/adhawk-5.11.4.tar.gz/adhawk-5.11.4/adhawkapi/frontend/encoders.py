...
from ..publicapi import (MarkerSequenceMode, PacketType, ProcedureType, PropertyType,
                         SystemControlType, CameraUserSettings)
...
def encode(packet_type, *args):
    ...
    try:
        ...
        if not encoder:
            assert not args
            encoder = (lambda: struct.pack('<B', packet_type.value))
    return encoder(*args)
...
_ENCODERS = {
    ...
    PacketType.SET_STREAM_ENABLE:
        (lambda stream_bitmap: struct.pack('<BI', PacketType.SET_STREAM_ENABLE, stream_bitmap)),
    PacketType.START_CAMERA:
        (lambda cam_index, res_index, correct_distortion: struct.pack('<BHHB', PacketType.START_CAMERA.value, cam_index,
                                                                      res_index, correct_distortion)),
    PacketType.START_VIDEO_STREAM:
        (lambda ip, port: struct.pack('<B4sH', PacketType.START_VIDEO_STREAM.value, inet_aton(ip), port)),
    PacketType.STOP_VIDEO_STREAM:
        (lambda ip, port: struct.pack('<B4sH', PacketType.STOP_VIDEO_STREAM.value, inet_aton(ip), port)),
    ...
    PacketType.SET_STREAM_RATE:
        (lambda rate: struct.pack('<Bf', PacketType.SET_STREAM_RATE, rate)),
