...
def process_period(boleta: dict):
    ...
    init_date = datetime(day=pid, month=int(month), year=int(year))
    duration_map = {"Mensual": 1, "Trimestral": 3, "Cuatrimestral": 4, "Semestral": 6, "Anual": 12}
    months = duration_map.get(boleta['plan_cobro'])
...
def format_factura_afip(
    ...
    factura['FeDetReq'] = {
        'FECAEDetRequest': {
            ...
            'DocTipo': TIPO_DOCUMENTO.get(tipo_doc),
            'DocNro': documento,
            ...
            'FchVtoPago': today_argentina().strftime("%Y%m%d"),
            'MonId': 'PES',
            'MonCotiz': 1,
        }
    }

    if tipo_cbte == 8 or tipo_cbte == 3:
...
def format_factura_db(
    ...
    factura_formateada = {
        'data_factura':{
            ...
            'fecha_factura': datetime.strptime(factura['FeDetReq']['FECAEDetRequest']['CbteFch'], "%Y%m%d"),
    ...
    if factura['FeCabReq']['CbteTipo'] == 8 or factura["FeCabReq"]["CbteTipo"] == 3:
