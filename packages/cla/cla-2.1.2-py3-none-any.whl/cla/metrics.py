...
if ENABLE_R:
    try:
        import rpy2 # require the R runtime
...
def run_multiclass_clfs_presplit(X_train, y_train, X_test = None, y_test = None, clfs = 'all', show = True):
    '''
...
def run_multiclass_clfs(X, y, clfs = 'all', split = .3, show = True):
    ...
    '''
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=split, random_state = 2, stratify=y)
    return run_multiclass_clfs_presplit(X_train, y_train, X_test, y_test, clfs = clfs, show = show)