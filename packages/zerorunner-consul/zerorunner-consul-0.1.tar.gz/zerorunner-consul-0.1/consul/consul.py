...
from consul.cb import CB
from consul.check import Check
from consul.exception import ConsulException
...
class Consul(object):
    ...
    class Agent(object):
        ...
        def services(self, token=None):
            ...
            返回在本地代理中注册的所有服务。
这些服务要么通过配置文件提供，要么
使用HTTP API动态添加。需要注意的是
代理已知的服务可能与那些不同
由目录报告。这通常是由于正在进行的更改
而没有选出领导人。代理执行活动
反熵，所以在大多数情况下，一切都是同步的
几秒钟内.
            """
            headers = {}
            token = token or self.agent.token
...
几秒钟之内。
        ...
        def members(self, wan=False, token=None):
            """
            返回此代理当前看到的所有成员。这可能
根据代理的不同，使用Catalog的节点api来检索集群
广泛一致的成员观点。
对于在服务器模式下运行的代理，将*wan*设置为*True*将返回
WAN成员的列表，而不是LAN成员的列表
违约
            """
            params = []
            headers = {}
