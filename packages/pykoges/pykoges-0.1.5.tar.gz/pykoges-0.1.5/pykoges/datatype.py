...
class Questions:
    ...
    @staticmethod
    def __filter(self, f):
        return Questions(list(filter(f, self.list)))

    @staticmethod
    def __find(self, f):
        return next(filter(f, self.list), None)
    ...
    def has_code(self, code):
        if isinstance(code, list) or isinstance(code, set):
            res = Questions()
            for x in code:
                q = self.__find(lambda y: y.survey_code.endswith(f"_{x.lower()}"))
                res.append(q)
            return res
        return self.__filter(lambda x: x.survey_code.endswith(f"_{code.lower()}"))

    def has_text(self, code):
        if isinstance(code, list):
            res = Questions()
            for x in code:
                q = self.__find(lambda y: x in y.question_text)
                res.append(q)
        return self.__filter(lambda x: code in x.question_text)
    ...
    def __getattr__(self, name):
        if name not in self.__dict__:
            self.__dict__[name] = self.__extract_attr(name)
