...
def one(value: Result[T | list[T]] | T | list[T], default_value: Any | None = None) -> T:
    return (
...
def nl(value: str = "", count: int = 1, reversed: bool | None = False) -> str:
    nl_text: str = "\n" * count
...
def js(value: tuple[Any | None] | list[Any | None]) -> str:
    return j(value, " ")
...
class EnumTool:
    ...
    @staticmethod
    def get_by_value(
        enum_class: Enum, value: Any, default_value: Any | None = None
    ) -> Any | None:
...
class ListTool:
    ...
    def to_dict(
        ...
        return {
            item if modificator is None else modificator(item): None for item in value
        }
...
class ParameterList:
    ...
    def next(
        self, object: Any | None = None, default_value: Any | None = None
    ) -> Any | None:
...
class DateTimeTool:
    ...
    def add_months(sourcedate, months):
        ...
        return timedelta(datetime.date(year, month, day) """
    ...
    def datetime_or_date_from_string(
        ...
        if value.find(CONST.DATETIME_SPLITTER) != -1 and not as_date:
            return result
        return result.date()
...
class ResultTool:
    ...
    def filter(
        ...
        try:
            ...
            result.data = DataTool.filter(filter_function, data)
...
def while_excepted(check_action: Callable[[None], Any | None]) -> Any | None:
    def action() -> Any | None:
        try:
            return check_action()
