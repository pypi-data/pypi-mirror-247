...
class ServiceListener:
    ...
    def listen_for(
        ...
        PIH.SERVICE.ADMIN.serve(
            ...
            lambda command_name, pl: handler(
                command_name, pl, self
            ),
...
class UserOutput(UserOutputBase):
    def result(
        ...
        base_location_list = PIH.DATA.FORMAT.location_list(root_location, False)
...
class Output(OutputBase):
    ...
    def free_marks_by_group_for_result(
        ...
        self.table_with_caption_last_title_is_centered(
            result,
            f'Свободные карты доступа для группы доступа "{b(group_name)}":',
            use_index,
        )
...
class Input(InputBase):
    ...
    def index(
        ...
        while True:
            if (
                not has_error
                and label_function is not None
                and length > 1
                or use_free_input
            ):
                for index, item in enumerate(data):
            ...
            selected_index = PIH.DATA.EXTRACT.decimal(
                value := self.input(index_caption)
                if n(text_after)
...
class PIH:
    ...
    class ERROR:
        ...
        def detect_interruption(message: str | None = None) -> bool:
            try:
                ...
                DataTool.check_not_none(message, lambda: PIH.output.red(message))
        ...
        @staticmethod
        def global_except_hook(error: Any):
    ...
    class UPDATE:
        ...
        def for_service(
            ...
            return result and PIH.UPDATE.modules(modules, modules_update, show_output)

        @staticmethod
        def modules(
            value: list[str] | None, update: bool = True, show_output: bool = True
        ) -> bool:
            result: bool = True
        ...
        def command_for_module(
            ...
            return [
                ...
                version, lambda: [j((module_name, "==", version))], [module_name, "-U"]
            )
        ...
        def module_exists(module_name: str) -> bool:
            ...
            return process_result.stdout.find(module_name) != -1
        ...
        def install_module(
            ...
            command_list: list[str] = PIH.UPDATE.command_for_module(
                module_name, version
            )
            process_result: CompletedProcess = PIH.EXECUTOR.execute(
                command_list, show_output
    ...
    class EXECUTOR:
        ...
        def python_path(host: str | None = None) -> str:
            ...
            variable_list: list[str] = if_else(
                n(host),
                lambda: PIH.OS.environment_variable(WINDOWS.ENVIROMENT_VARIABLES.PATH),
        ...
        def create_command_for_psexec(
            ...
            if not local_host:
                ...
                login = login or PIH.DATA.VARIABLE.value(LINK.ADMINISTRATOR_LOGIN)
        ...
        def create_command_for_start_service(
            ...
            def local_action(value):
                return if_else(local, None, value)
        ...
        def process_is_exists(
            ...
            password = password or PIH.DATA.VARIABLE.value(LINK.ADMINISTRATOR_PASSOWORD)
            if ne(host):
                command_list += ["/s", host]
    ...
    class EVENT:
        ...
        class BUILDER:
            ...
            def polibase_person_email_was_added(
                ...
                return PIH.EVENT.BUILDER.create_event(
                    ...
                    DataTool.check_not_none(
                        person_for_search, lambda: (None, person_for_search.pin)
                    ),
                )
            ...
            def polibase_person_set_suitable_card_registry_folder(
                ...
                return PIH.EVENT.BUILDER.create_event(
                    ...
                    lambda: (
                        ...
                        None
                        if n(name)
                        else PIH.DATA.FORMAT.polibase_person_card_registry_folder(name),
                        None if n(registrator) else registrator.FullName,
                        None if n(registrator) else registrator.pin,
                    ),
                )
        ...
        def it_notify_about_create_temporary_mark(
            ...
            PIH.EVENT.it_notify_about_mark_creation(True, full_name, tab_number)
        ...
        def on_event(
            ...
            def thread_handler() -> None:
                ServiceListener().listen_for(
                    ...
                    lambda _, pl, service_listener: handler(
                        pl, service_listener
                    ),
                )
        ...
        def wait_server_start(
            ...
            def internal_handler(
                pl: ParameterList, listener: ServiceListener
            ) -> None:
        ...
        def on_robocopy_job_complete(
            handler_or_robocopy_job_name: Callable[[str, int, ServiceListener], None]
            | str
        ) -> bool | None:
            class DH:
    ...
    class SERVICE:
        ...
        def call(
            ...
            def long_operation_action() -> None:
                ...
                if not DH.stop_long_operation_action:
                    PIH.SERVICE.call_listeners_on_long_operation()

            if ne(long_operation_duration):
                long_operation_thread: Thread = Thread(target=long_operation_action)
            ...
            def action() -> str | None:
                ...
                DH.stop_long_operation_action = True
                return result
        ...
        def get_support_host_list(
            ...
            return DataTool.filter(
                lambda item: item.name.startswith(
                    j(
                        (
                            PIH.SERVICE.SUPPORT_NAME_PREFIX,
                            ServiceRoles.description(role_or_information).name,
                        )
                    )
                ),
        ...
        class ADMIN:
            ...
            def kill_all(
                ...
                def kill(host: str, local: bool, via_standart_tools: bool) -> None:
                    if local:
                        PIH.EXECUTOR.kill_python_process(host, via_standart_tools)
                    else:
            ...
            def create_developer_service_description(
                ...
                if n(port) or port == ServiceRoles.DEVELOPER.value.port:
            ...
            def create_event_listener_service_description(
                ...
                return ServiceDescription(
                    name=":".join(
                        [PIH.SERVICE.EVENT_LISTENER_NAME_PREFIX, host, str(port)]
                    ),
            ...
            def serve(
                ...
                if PIH.SERVICE.check_on_host_chanchable(service_description):
                    ...
                    PIH.ERROR.service_host_is_unchanachable(service_description.host)
            ...
            def validate(
                ...
                if ne(service_information_list):
                    ...
                    with ThreadPoolExecutor(max_workers=length) as executor:
                        ...
                        service_description_list: list[
                            ServiceDescription
                        ] = DataTool.filter(
                            lambda item: item.visible_for_admin,
                            DataTool.map(
                                lambda item: ServiceRoles.description(item),
                                ServiceRoles,
                            ),
                        )
        ...
        def check_on_accessibility(
            ...
            return ne(PIH.SERVICE.get_information(role_or_information, cached))
    ...
    class DATA(DataTool, StringTool, ListTool, DateTimeTool, EnumTool, FullNameTool):
        class JOURNAL:
            ...
            @staticmethod
            def tag_by_id(value: int) -> Tag | None:
                data: IconedOrderedNameCaptionDescription | None = one(
                    DataTool.filter(
                        lambda item: item[0] == value, DataTool.to_list(Tag)
                    )
                )
                return None if n(data) else Tag(data)
        ...
        class VARIABLE:
            ...
            class ENVIRONMENT:
                @staticmethod
                def value(name: str | None = None) -> Any | None:
                    return PIH.DATA.VARIABLE.value(name, True)
            ...
            def value_with_section(
                ...
                environment_variable_handler: Callable[
                    [str], Any
                ...
                def get_value(
                    variable_name: str, section: PIH.DATA.VARIABLE.SECTIONS
                ) -> Any | None:
        ...
        class EXTRACT:
            ...
            class EVENT:
                ...
                def action(parameters: dict[str, Any]) -> ActionWasDone:
                    action: ActionWasDone = DataTool.fill_data_from_source(
                        ActionWasDone(),
                        PIH.EVENT.get_parameter(Events.ACTION_WAS_DONE, parameters),
                        copy_by_index=True,
                    )
                ...
                @staticmethod
                def get(pl: ParameterList) -> Events:
                    event_content: Any | list[Any] = (
                        pl.list[0]
                        if PIH.DATA.CHECK.has_subscribtion_result(pl)
                        else pl.list
                    )
            ...
            def subscribtion_result(
                ...
                return (
                    None
                    if e(pl.list)
                    or not PIH.DATA.CHECK.has_subscribtion_result(pl)
                    else extract()
                )
        ...
        class FORMAT:
            ...
            """@staticmethod
            def result(value: Result[T | list[T]], every_item: Callable[[FieldItem, T], str]) -> str:
                string_result_list: list[str] = []
                ResultTool.every(lambda item: every_item(valie.fields, item), value)
                return j(string_result_list)"""
            ...
            def user_principal_name(login: str) -> str:
                return j((login, AD.DOMAIN_MAIN), EMAIL.SPLITTER)
            ...
            def email(
                ...
                if use_default_domain and value.find(EMAIL.SPLITTER) == -1:
                    value = PIH.DATA.FORMAT.email(
                        j((value, CONST.SITE_ADDRESS), EMAIL.SPLITTER)
                    )
                if email_correction:
                    for char in '"(),:;<>[\\] ':
                        value = value.replace(char, "")
                    value = value.replace("/", ".")
                    email_name, email_domain = value.split(EMAIL.SPLITTER)
                    PIH.RESULT.NOTES.execute_file("@email_correction")
                    for email_correction_item in EMAIL_CORRECTION:
                        if email_domain in email_correction_item[0]:
                            ...
                            break
                    value = j((email_name, email_domain), EMAIL.SPLITTER)
                return value.lower()
    ...
    class RESULT(ResultTool):
        class JOURNALS:
            @staticmethod
            def get(
                value: JournalType | None = None, tag: Tag | None = None
                ...
                def convert(event: EventDS) -> JournalRecord:
                    ...
                    return DataTool.fill_data_from_source(
                        JournalRecord(
                            event.timestamp, applicant_user=user
        ...
        class EVENTS:
            ...
            def get(
                ...
                def extract_function(data: Any) -> EventDS:
                ...
                return DataTool.to_result(
                    PIH.SERVICE.call_command(
                        ...
                        if n(event_type)
                            ...
                            event_type.name,
                            PIH.EVENT.BUILDER.create_parameters_map(
                                event_type, parameters, check_for_parameters_count=False
                            ),
                            timestamp=timestamp,
                        ),
                    ),
            ...
            def get_last(
                ...
                return PIH.RESULT.EVENTS.get(event_type, parameters, timestamp)

        class NOTES:
            cache: Result[list[Note]] | None = None
            ...
            @staticmethod
            def get(id: str, force_update: bool = False) -> Result[Note]:
                return DataTool.to_result(
                    PIH.SERVICE.call_command(
                        ServiceCommands.get_note, (id, force_update)
                    ),
                    Note,
                )

            @staticmethod
            def by_label(value: str, force_update: bool = False) -> Result[list[Note]]:
                return DataTool.to_result(
                    PIH.SERVICE.call_command(
                        ServiceCommands.get_note_list_by_label, (value, force_update)
                    ),
                    Note,
                )
            ...
            def files(
                ...
                result: Result[list[Note]] | None = None
                search_function: Callable[[str | None], Result[list[Note]]] = (
                    lambda item: True if n(name) else item.title.find(name) != -1
                )
                if ne(name) and name.startswith("@@"):
                    cached = True
                if n(PIH.RESULT.NOTES.cache):
                    cached = False
                if cached:
                    result = ResultTool.filter(
                        search_function, PIH.RESULT.NOTES.cache, True
                    )
                if not cached or A.R.is_empty(result):
                    PIH.RESULT.NOTES.cache = PIH.RESULT.NOTES.by_label(
                        CONST.NOTES.FILE_SECTION, force_fetch
                    )
                    result = ResultTool.filter(
                        search_function, PIH.RESULT.NOTES.cache, True
                    )
            ...
            def execute_file(
                ...
                return PIH.RESULT.NOTES.execute(
                    one(
                        PIH.RESULT.NOTES.files(file_search_request, cached, force_fetch)
                    ).text,
                    parameters,
                    stdout_redirect,
                    catch_exceptions,
        ...
        class WORKSTATION:
            ...
            def by_login(value: str) -> Result[list[Workstation]]:
                ...
                if PIH.CHECK.USER.exists_by_login(value):
                    workstation_list_result: Result[
                        list[Workstation]
                    ] = DataTool.to_result(
                        PIH.SERVICE.call_command(
        ...
        class TIME_TRACKING:
            ...
            def create(
                ...
                def get_date_or_time(entity: TimeTrackingEntity, date: bool) -> str:
                    return DataTool.check_not_none(
                        entity,
                        lambda: entity.TimeVal.split(CONST.DATETIME_SPLITTER)[not date],
                    )
            ...
            def by_any(value: Any, active: bool | None = None) -> Result[list[User]]:
                ...
                if isinstance(value, Mark):
                    ...
                    try:
                        ...
                        pass
                    if PIH.DATA.CHECK.decimal(value):
                        init_or_add(lambda: by_number(value))
                    if PIH.CHECK.WORKSTATION.name(value):
                        init_or_add(
                            lambda: ResultTool.as_list(
    ...
    class CHECK:
        ...
        class RESOURCE:
            ...
            def accessibility_by_ping_with_port(
                ...
                return PIH.CHECK.RESOURCE.accessibility_by_ping(
                    j((address_or_ip, port), ":"), host, count, check_for_all
                )
            ...
            def accessibility_by_ping(
                ...
                return PIH.EXECUTOR.psping(address_or_ip, host, count, check_for_all)
        ...
        class POLIBASE:
            ...
            @staticmethod
            def person_exists_by_pin(value: int) -> bool:
                try:
                    return ne(PIH.RESULT.POLIBASE.person_by_pin(value))
                except NotFound as _:
                    return False
        ...
        class COMPUTER:
            ...
            def windows_service_running(name: str, host: str) -> bool | None:
                accessable: bool | None = PIH.CHECK.COMPUTER.accessibility(host)
                if n(accessable):
                    return None
        ...
        class WHATSAPP:
            class WAPPI:
                ...
                def get_message_list(
                    ...
                    key: str = "message"
                    if not has_error:
                        if A.D.is_in(response_result, key):
    ...
    class CARD_REGISTRY:
        ...
        def get_statistics() -> list[CardRegistryFolderStatistics]:
            ...
            return PIH.RESULT.map(map_function, A.R_DS.execute(query)).data
        ...
        def get_suitable_folder_name_for_person(
            ...
            return DataTool.get_first_item(
                PIH.RESULT.map(map_function, A.R_DS.execute(query)).data
            ).name
        ...
        def set_folder_for_person(
            ...
            if set_by_polibase:
                ...
                if already_set_by_polibase:
                    ...
                    PIH.ACTION.POLIBASE.set_card_registry_folder(
                        value, person, registrator_person
                    )
                return True
    ...
    class ACTION:
        class JOURNAL:
            ...
            def add(
                ...
                journal_type_value: tuple[
                    int, OrderedNameCaptionDescription
                ] = EnumTool.get(journal_type)
                tag_value: tuple[
                    int, IconedOrderedNameCaptionDescription
                ] = EnumTool.get(tag)
                PIH.EVENT.send(
        ...
        class EVENTS:
            ...
            def register(
                ...
                if remove_before:
                    parameters_for_remove: dict[str, Any] | None = parameters
                    key_name: tuple[str] | None = EnumTool.get(value).key_name
                    if nn(key_name):
                        parameters_for_remove = {}
                        for key_name_item in key_name:
                            parameters_for_remove[key_name_item] = parameters[
                                key_name_item
                            ]
                    PIH.ACTION.EVENTS.remove(value, parameters_for_remove)
                invisible_list: tuple[int] | None = A.D.get(value).invisible_list
                if nn(A.D.get(value).invisible_list):
                    visible_parameters: dict[str, Any] = {}
                    for index, parameter_key in enumerate(parameters):
                        if index not in invisible_list:
                            visible_parameters[parameter_key] = parameters[
                                parameter_key
                            ]
        ...
        class MOBILE_INPUT_OUTPUT:
            @staticmethod
            def send_command_to(
                value: str, recipient: str | Enum, chat_id: str | None = None
            ) -> None:
...
class A:
    ...
    CT = CONST
    CT_CMDT = COMMAND_TYPE
    CT_LNK = LINK
    CT_EMAIL = EMAIL_COLLECTION
...
@staticmethod
def delay(action: Callable[[None], None], value: float) -> None:
