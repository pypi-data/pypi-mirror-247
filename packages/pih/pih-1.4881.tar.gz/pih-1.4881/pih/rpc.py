...
from pih.tools import ParameterList, BitMask as BM, j, nn, n
...
class RPC:
    ...
    class UnaryService(pb2_grpc.UnaryServicer):
        ...
        def internal_call_handler(
            ...
            try:
                ...
                return (
                    None
                    if A.D_C.empty(self.call_handler)
    ...
    class Service(IService):
        ...
        def create_subscribtions(
            ...
            for description_item in (
                self.subscribtions_map if A.D_C.empty(information) else [information]
            ):
    ...
    def call_service(
        ...
        information: ServiceInformationBase | None = (
            role_or_information
            if role_or_information is None
            else A.CT_SR.description(role_or_information)
        )
        try:
            ...
            if A.D_C.empty(timeout):
                if (
                    A.D_C.empty(RPC.service_information)
