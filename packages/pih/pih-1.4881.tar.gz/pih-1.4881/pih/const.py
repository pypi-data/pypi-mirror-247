...
class HOSTS:
    ...
    class POLIBASE:
        ...
        NAME: str = "fmvpolibase1"
        ALIAS: str = "polibase"
        IP: str = "192.168.100.3"

    class POLIBASE_TEST:
        NAME: str = "fmvpolibase2"
...
class PATH_POLIBASE:
    ...
    def get_person_folder(pin: int, test: bool = False) -> str:
        ...
        return os.path.join(
            os.path.join(f"//{root}", "polibaseData", "PERSONS"), str(pin)
        )
...
class COMMAND_TYPE(Enum):
    POLIBASE: tuple[str] = (
        "Запрос к базе данный Polibase",
        "polibase",
        "полибейс",
        "oracle",
    )
    DATA_SOURCE: tuple[str] = ("Запрос к базе данных", "ds")
...
class Events(Enum):
    ...
    NEW_FILE_DETECTED: EventDescription = EventDescription(
        ...
        LogMessageChannels.IT,
        LogMessageFlags.SILENCE,
        (ParamItem("path", "Путь к файлу"),),
    )

    NEW_POLIBASE_DOCUMENT_DETECTED: EventDescription = EventDescription(
        "Новый Polibase документ",
        LogMessageChannels.IT,
        LogMessageFlags.NORMAL,
        (
            ParamItem("path", "Путь к файлу"),
            PARAM_ITEMS.PERSON_PIN,
            ParamItem("document_name", "Имя документа"),
        ),
    )
