from typing import TYPE_CHECKING, Any, Dict, List, Type, TypeVar, Union

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..types import UNSET, Unset

if TYPE_CHECKING:
    from ..models.v0_developer_error import V0DeveloperError


T = TypeVar("T", bound="V0ValidateResponse")


@_attrs_define
class V0ValidateResponse:
    """
    Attributes:
        request_errors (Union[Unset, List['V0DeveloperError']]):
        validation_errors (Union[Unset, List['V0DeveloperError']]):
        updated_validation_yaml (Union[Unset, str]):
    """

    request_errors: Union[Unset, List["V0DeveloperError"]] = UNSET
    validation_errors: Union[Unset, List["V0DeveloperError"]] = UNSET
    updated_validation_yaml: Union[Unset, str] = UNSET
    additional_properties: Dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> Dict[str, Any]:
        request_errors: Union[Unset, List[Dict[str, Any]]] = UNSET
        if not isinstance(self.request_errors, Unset):
            request_errors = []
            for request_errors_item_data in self.request_errors:
                request_errors_item = request_errors_item_data.to_dict()

                request_errors.append(request_errors_item)

        validation_errors: Union[Unset, List[Dict[str, Any]]] = UNSET
        if not isinstance(self.validation_errors, Unset):
            validation_errors = []
            for validation_errors_item_data in self.validation_errors:
                validation_errors_item = validation_errors_item_data.to_dict()

                validation_errors.append(validation_errors_item)

        updated_validation_yaml = self.updated_validation_yaml

        field_dict: Dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update({})
        if request_errors is not UNSET:
            field_dict["requestErrors"] = request_errors
        if validation_errors is not UNSET:
            field_dict["validationErrors"] = validation_errors
        if updated_validation_yaml is not UNSET:
            field_dict["updatedValidationYaml"] = updated_validation_yaml

        return field_dict

    @classmethod
    def from_dict(cls: Type[T], src_dict: Dict[str, Any]) -> T:
        from ..models.v0_developer_error import V0DeveloperError

        d = src_dict.copy()
        request_errors = []
        _request_errors = d.pop("requestErrors", UNSET)
        for request_errors_item_data in _request_errors or []:
            request_errors_item = V0DeveloperError.from_dict(request_errors_item_data)

            request_errors.append(request_errors_item)

        validation_errors = []
        _validation_errors = d.pop("validationErrors", UNSET)
        for validation_errors_item_data in _validation_errors or []:
            validation_errors_item = V0DeveloperError.from_dict(validation_errors_item_data)

            validation_errors.append(validation_errors_item)

        updated_validation_yaml = d.pop("updatedValidationYaml", UNSET)

        v0_validate_response = cls(
            request_errors=request_errors,
            validation_errors=validation_errors,
            updated_validation_yaml=updated_validation_yaml,
        )

        v0_validate_response.additional_properties = d
        return v0_validate_response

    @property
    def additional_keys(self) -> List[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
