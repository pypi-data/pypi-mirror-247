...
class LLMDeployment:
    ...
    def generate(
        ...
        data = {
            "inputs": prompt,
            "parameters": options if isinstance(options, dict) else dict(),
        }
        ...
        if self._adapter:
            if isinstance(self._adapter, HuggingFaceLLM):
                ...
                # Temporarily set this as requests to shared LLMs will still be routed to the API Gateway.
                if self.data.is_shared:
                    data["parameters"]["adapter_id"] = f"{self.adapter.repo.name}/{self.adapter.version}"
                else:
                    data["parameters"][
                        "adapter_id"
        ...
        res = self.session.post_json_serving(
            endpoint=f"/{self.session.tenant}/deployments/v2/llms/{self.name}/generate",
            json=data,
        )
        return GeneratedResponse(
            prompt=prompt,
            response=res.get("generated_text", ""),
            model_name=self.name,
            generated_tokens=res.get("details", {}).get("generated_tokens", None),
            prefill=res.get("details", {}).get("prefill", None),
            tokens=res.get("details", {}).get("tokens", None),
            finish_reason=res.get("details", {}).get("finish_reason", None),
            seed=res.get("details", {}).get("seed", None),
            best_of_sequences=res.get("details", {}).get("best_of_sequences", None),
        )
    ...
    def prompt(
        ...
        if not deployment_ready:
            print(
                f"Target LLM deployment `{self.name}` is not active yet. "
                f"Call `LLMDeployment.get_events()` to get the status of your deployment.",
            )
...
class LLMDeploymentJob:
    ...
    def get(self) -> LLMDeployment:
        self._session.get_llm_deployment_events_until_with_logging(
            ...
            error_cond=lambda resp: "BackOff" in [r.get("eventType", None) for r in resp.get("ComputeEvents", [])]
            or resp.get("deploymentStatus", None) in ("failed", "canceled", "stopped"),
        )
...
def deploy_llm(
    ...
    custom_args = [
        ...
        for arg in [
            ...
            ("--max-total-tokens", max_total_tokens or 2048),
    ...
    auto_suspend_seconds = auto_suspend_seconds or 3600
    model_params = {
    ...
    base_msg = "Important: this is a dedicated deployment for which Predibase charges per hour of usage."
    if auto_suspend_seconds == 0:
        msg = "It will remain active until you choose to delete the deployment."
    else:
        msg = (
            f"It will remain active until either the autoscaling scales down after {auto_suspend_seconds} "
            f"seconds. You can also delete the deployment to completely shut it down."
        )
    print(base_msg + " " + msg + "\n")
