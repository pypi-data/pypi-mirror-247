...
class StradaFunction():
    ...
    def __call__(self, **kwargs):
        # Log function call
        DebugLogger.log_http_object(HttpObjectType.FUNCTION.name, {"function_name": self.function_name, "function_args": kwargs})
        ...
        **kwargs
        ...
        if response.ok:  # HTTP status code 200-299
            try:
                ...
                DebugLogger.log_function_call(instance_name=instance_name, app_name=app_name, log_entry={"status": StradaResponseStatus.SUCCESS.name, "input": kwargs, "output": response_data})
        else:
            response_data = None
            error_message = None
            if response.headers.get("Content-Type") == "application/json" or response.headers.get("content-type") == "application/json":
            ...
            DebugLogger.log_function_call(instance_name=instance_name, app_name=app_name, log_entry={"status": StradaResponseStatus.ERROR.name, "input": kwargs, "output": response_data, "error": error_message})
            error = StradaError(
