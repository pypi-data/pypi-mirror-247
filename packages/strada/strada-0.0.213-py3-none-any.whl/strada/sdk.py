        ...
        **kwargs
        ...
        if response.ok:  # HTTP status code 200-299
            try:
                ...
                DebugLogger.log_function_call(instance_name=instance_name, app_name=app_name, log_entry={"status": StradaResponseStatus.SUCCESS.name, "input": kwargs, "output": json_response})
        else:
            response_data = None
            error_message = None
            if response.headers.get("Content-Type") == "application/json" or response.headers.get("content-type") == "application/json":
            ...
            DebugLogger.log_function_call(instance_name=instance_name, app_name=app_name, log_entry={"status": StradaResponseStatus.ERROR.name, "input": kwargs, "output": response_json, "error": error_message})
