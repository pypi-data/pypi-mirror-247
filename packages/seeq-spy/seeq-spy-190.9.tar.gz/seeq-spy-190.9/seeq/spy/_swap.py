...
def swap(items: pd.DataFrame, assets: pd.DataFrame, *, partial_swaps_ok: bool = False,
    ...
    for index, row in items.iterrows():
        ...
        try:
            ...
            if len(swappable_assets) == 0:
                item_dict['Result'] = ("This item does not depend on any assets, either as the item's parent or "
    ...
    for swap_group in swap_groups:
        ...
        for item_dependency_output, dependencies_with_relevant_assets in item_list:
            ...
            def _on_success(_row_index, _job_result):
                status.df.loc[_row_index, _job_result.keys()] = _job_result.values()
    ...
    status.df = pd.DataFrame(status_entries)
    ...
    error_count = len(result_df[result_df['Result'] != 'Success'])
    success_count = len(result_df[result_df['Result'] == 'Success'])
    if error_count > 0:
...
def _construct_header_column(df: pd.DataFrame):
    ...
    df['Header'] = pd.Series(header_column)
