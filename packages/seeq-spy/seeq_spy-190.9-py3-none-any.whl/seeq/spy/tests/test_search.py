...
def test_redacted_search_by_id():
    if not _login.is_server_version_at_least(62):
        ...
        # 403 error.
        return
    # An ID that doesn't actually exist is equivalent to an item without permission when using the search endpoint
    id_that_does_not_exist = _common.new_placeholder_guid()

    # Verify this blows up with a default `errors` setting
    reason = "The item was not found or the acting user does not have permission to read it"
    with pytest.raises(ApiException, match=reason):
        spy.search({'ID': id_that_does_not_exist}, all_properties=True)

    # And that it works for errors='catalog'
    status = spy.Status(errors='catalog')
    search_results = spy.search({'ID': id_that_does_not_exist}, all_properties=True, status=status)

    # The dataframe should have an ID, but no other properties
    assert len(search_results.index) == 1
    assert search_results.at[0, 'ID'] == id_that_does_not_exist

    # And the status should have a warning
    assert len(status.warnings) == 1
    warning = list(status.warnings)[0]
    assert id_that_does_not_exist in warning
...
def test_search_limit():
    ...
    all_example_data = spy.search({'Name': 'Area *_*', 'Datasource Name': 'Example Data'}, limit=10)
