    ...
    def persist_vm_module(self, vm_module: _binding.VmModule) -> "TracedModule":
        ...
        if flatbuffer_blob:
            save_path = os.path.join(
                self.trace_path, self.get_unique_name(f"{name}.vmfb")
            )
...
class TracedModule:
    ...
    def serialize(self):
        ...
        if self._vmfb_path:
            ...
            module_record["path"] = os.path.relpath(
                self._vmfb_path, self._parent.trace_path
            )
        else:
...
class ContextTracer:
    ...
    def __init__(
        self, parent: Tracer, is_dynamic: bool, modules: Sequence[TracedModule]
    ):
        self._parent = parent
        self._modules = list(modules)  # type: List[TracedModule]
        self._frame_count = 0
        self._file_path = os.path.join(
            parent.trace_path, parent.get_unique_name("calls.yaml")
        )
