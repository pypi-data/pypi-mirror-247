...
class AriaDigitalTwinDataPathsProvider:
    """
    This class is to load all data file paths from ADT data structure given a sequence path. Each ADT collection sequence may contain more than one Aria device wearers. The data associatedwith each Aria device is called a subsequence: 
    ├── sequencePath
    │   ├── subsequence1_Name
    │   │   ├── 2d_bounding_box.csv
    ...
    │   │   ├── ...
    │   └── metadata.json
    This class allows you use dataset root path and sequence name to query all Aria devices in a sequence and to query all data associated with an Aria device.
    """
...
class AriaDigitalTwinDataProvider:
    ...
    def get_aria_3d_pose_by_timestamp_ns(self, device_timestamp_ns: int, time_query_options: projectaria_tools.core.sensor_data.TimeQueryOptions = ...) -> Aria3dPoseDataWithDt:
        ...
        """
    def get_aria_all_streams(self) -> list[...]:
        """
        Get a list of all VRS stream ids from the Aria recording. StreamId is the uniqueidentifier to query different sensor data in ADT DataProvider.
        """
