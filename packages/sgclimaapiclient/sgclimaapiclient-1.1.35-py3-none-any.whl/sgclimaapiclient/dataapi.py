...
from settings import DATA_API_TOKEN
    ...
    async def fetch_parquet_blob_storage_async(self, filename, start_date, end_date, pids_list):
        ...
        async with svc_client:
            ...
            try:
                ...
                pids_list_columns = [col for col in pids_list if
                                     col in filtered_table.column_names] if pids_list else None
                if pids_list_columns and len(pids_list_columns) > 0:
                    pids_list_columns.append("ts")
                    filtered_table = filtered_table.select(pids_list_columns)
                else:
                    filtered_table = filtered_table.select(filtered_table.column_names)
...
if __name__ == '__main__':
    ...
    sgclimaapiclient = SGClimaDataAPI(token=DATA_API_TOKEN)

    parquets = sgclimaapiclient.download_weather_station_data_parquet(
        station_id=2511174,
        aggregation='raw',
