...
def evolve(site: ISite):
    ...
    try:
        ...
        for context in find_objects_providing(site, IPortalContext):
            ...
            if page is not None:
                LOGGER.info(f"Updating portlets configuration for {context!r}")
                page_annotations = IAnnotations(page)
                portlets = page_annotations.get(PORTLETS_CONFIGURATION_KEY)
                if portlets is not None:
                    for configuration in portlets.values():
                        configuration.__parent__ = page
    finally:
