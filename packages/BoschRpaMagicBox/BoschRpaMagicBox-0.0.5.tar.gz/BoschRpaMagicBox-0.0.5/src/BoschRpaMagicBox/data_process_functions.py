...
# def open_sap(system_desc: str):
#     """this function is used for open SAP system
#
#     Args:
#         system_desc（str）: This is the system description
#     """
...
#         sapguiauto = None
#         return
#     application.OpenConnection(system_desc, True)
...
# def execute_created_script(script_file_path: str, done_file_path: str = '', is_paste: bool = False, paste_content_list: list = None,
#                            check_method='done_indicator', save_folder_path='', file_name='', wait_time=30,
#                            open_sap_method: str = 'system_description',
#                            system_code: str = '', sys_desc: str = ''):
#     """This function is used to log in sap and execute related SAP guiXT script
#
#
#     Args:
#         wait_time(int): This is the time interval to wait sap job done
#         open_sap_method(str): system_description or system_code
#         check_method(str): done_indicator or file_download
#         save_folder_path(str): This is the folder path of downloaded files
#         file_name(str): This is the file name of downloaded file
#         script_file_path(str): This is the file path of sap script
#         done_file_path(str): This is the done indicator file path
#         sys_desc(str): This is the file path of script
#         is_paste(bool): Whether paste content to SAP
#         paste_content_list(list): he content list need to be pasted to SAP
#         system_code(str): POE,PRP ....
#     """
#     print('Open SAP......')
#     # self.open_sap('QPT: Abnahme PT-Power')
#     if open_sap_method == 'system_description':
#         open_sap(sys_desc)
#     else:
#         open_sap_with_system_code(system_code)
#     sleep(20)
#     if is_paste and paste_content_list:
#         paste_content = '\r\n'.join(paste_content_list)
#         pyperclip.copy(paste_content)
#     os.system('start guixt input=ok:process="{file_path}"'.format(file_path=script_file_path))
#     if check_method == 'done_indicator':
#         check_done_indicators_complete(done_file_path, f'Check Download Done for {done_file_path}', wait_time=wait_time)
#     else:
#         check_file_download_status(save_folder_path, file_name)
...
# def create_pivot_table(target_workbook, data_source_sheet_name, pivot_table_sheet_name, pt_name, pt_rows, pt_cols, pt_filters, pt_fields,
#                        pt_table_location, pt_filter_location):
#     """This function is used to create pivot tables
#
#     Args:
#         target_workbook(Book): This is the workbook to create pivot tables
#         data_source_sheet_name(str): This is the name of data source sheet
#         pivot_table_sheet_name(str): This is the name of pivot table worksheet
#         pt_name(str): This is the name of pivot table
#         pt_rows(list):This is the list of pivot rows
#         pt_cols(list): This is the list of pivot columns
#         pt_filters(list): This is the lisf of pivot filters
#         pt_fields(list(list)): This is the list of pivot fields which will be show in pivot table data area.
#                                e.g. [['Document Number', 'Document Number Count', win32c.xlCount, '0']]
#         pt_table_location(list): This is the location of pivot table starts
#         pt_filter_location(list): This is the location of pivot filter starts
#     """
#
#     # pivot table location, the left-up corner location
#     win32c = win32com.client.constants
#
#     target_workbook = target_workbook.api
#     data_source_sheet = target_workbook.Sheets(data_source_sheet_name)
#     pivot_table_sheet = target_workbook.Sheets(pivot_table_sheet_name)
#
#     # grab the pivot table source data
#     pivot_cache = target_workbook.PivotCaches().Create(SourceType=win32c.xlPageField, SourceData=data_source_sheet.UsedRange)
#
#     # create the pivot table object
#     target_cell = pivot_table_sheet.Cells(pt_table_location[0], pt_table_location[1])
#     pivot_cache.CreatePivotTable(TableDestination=pivot_table_sheet.Range(target_cell, target_cell), TableName=pt_name)
#
#     # select the pivot table work sheet and location to create the pivot table
#     pivot_table_sheet.Select()
#     pivot_table_sheet.Cells(pt_filter_location[0], pt_filter_location[1]).Select()
#
...
#             pivot_table_sheet.PivotTables(pt_name).PivotFields(value).Orientation = field_r
#             pivot_table_sheet.PivotTables(pt_name).PivotFields(value).Position = i + 1
#
#     # Sets the Values of the pivot table
#     for field in pt_fields:
#         pivot_table_sheet.PivotTables(pt_name).AddDataField(pivot_table_sheet.PivotTables(pt_name).PivotFields(field[0]), field[1],
#                                                             field[2]).NumberFormat = field[3]
#
#     # Visibility True or False
#     pivot_table_sheet.PivotTables(pt_name).ShowValuesRow = True
#     pivot_table_sheet.PivotTables(pt_name).ColumnGrand = True
