    ...
    async def upload(self, bucket_name, resp, file_name):
        ...
        try:
            async with aioboto3.Session().client('s3',
                ...
                try:
                    # 上传字节流文件对象到S3存储桶中
                    res = await client.upload_fileobj(byte_stream_file_obj, bucket_name, file_name)
                    await resp.aclose()
    ...
    async def upload_bytes(self, bucket_name, file_bytes, file_name):
        ...
        try:
            ...
            async with aioboto3.Session().client('s3',
                                                 ...
                                                 aws_secret_access_key=self.aws_secret_access_key) as client:
                # 上传字节流文件对象到S3存储桶中
                await client.upload_fileobj(byte_stream_file_obj, bucket_name, file_name)
                logger.info(f'Uploaded {file_name} to {bucket_name}')
                return True
        except botocore.exceptions.ClientError as e:
            logger.error(f'Error uploading {file_name}: {e}')
    ...
    async def upload_large(self, bucket_name, resp, file_name):
        ...
        try:
            async with aioboto3.Session().client('s3',
                                                 ...
                                                 aws_secret_access_key=self.aws_secret_access_key) as client:
                # 逐块上传流数据到S3存储桶中
                upload_id = (await client.create_multipart_upload(Bucket=bucket_name, Key=file_name)).get('UploadId')
                part_number = 1
                uploaded_parts = []
