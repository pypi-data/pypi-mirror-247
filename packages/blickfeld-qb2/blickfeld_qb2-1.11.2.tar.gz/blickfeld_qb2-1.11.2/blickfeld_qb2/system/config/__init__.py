...
class NetworkWireless(betterproto.Message):
    ...
    simultanously. [NOTE] Always use the validate method of the network service
    to ensure that the given credentials are correct. If the credentials are
    incorrect or the wireless access point is down, the fallback IP address can
...
class ScanPatternVertical(betterproto.Message):
    ...
    * This section defines the movement of the vertical mirror within one
    ...
    vertical FoV. The vertical FoV is limited by the optical components. Note:
    Due to the time required for one scan line, the rounded even number of scan
    lines directly scales inverse proportionally with the frame rate.
    """
