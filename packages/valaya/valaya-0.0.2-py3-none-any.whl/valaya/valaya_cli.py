...
if args.signup is not None:
    ...
    ip, port, user = valaya.get_config().values()

    pw = pwinput.pwinput(prompt='Current password: ')

    user = valaya.User(ip, port, user, pw)
...
if user:
    ...
    try:
        userobj = valaya.User(ip, port, user, pw, key_pw)
    except Exception as e:
        print('Error: ' + str(e))
        quit()
else:
...
def main():
    while True:
        args = input(f'{user} /{userobj.c_dir} ❯ ').split()
        ...
        if not cmd:
            ...
            try:
                ...
                files = userobj.list(args.path, stats=args.long)
            ...
            try:
                args = parser.parse_args(args)
                userobj.move(args.source, args.dest)
            except Exception as e:
                print('Error: ' + str(e))
        elif cmd == 'rm':
            ...
            try:
                args = parser.parse_args(args)
                userobj.remove(args.file, args.force)
            except Exception as e:
                print('Error: ' + str(e))
        elif cmd == 'dl':
            ...
            try:
                args = parser.parse_args(args)
                userobj.download(args.source, args.dest)
            except Exception as e:
                print('Error: ' + str(e))
        elif cmd == 'ul':
            ...
            try:
                args = parser.parse_args(args)
                userobj.upload(args.source, args.dest)
            except Exception as e:
                print('Error: ' + str(e))
        elif cmd == 'quota':
