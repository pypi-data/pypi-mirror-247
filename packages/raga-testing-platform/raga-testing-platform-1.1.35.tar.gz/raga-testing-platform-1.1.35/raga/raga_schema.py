...
class DARules():
    ...
    def add(self, technique:str, Stage:str, type:Optional[str]="", variance_limit:Optional[tuple]=(), mean:Optional[float]=None, p:Optional[float]=None):
        from raga import REQUIRED_ARG_V2
        assert isinstance(technique, str) and technique, f"{REQUIRED_ARG_V2.format('technique', 'str')}"
        assert isinstance(Stage, str) and Stage, f"{REQUIRED_ARG_V2.format('Stage', 'str')}"
        rules = {"technique": technique, "Stage": Stage}
        if(type!=None):
            rules["type"] = type
        if(variance_limit!=None):
            rules["variance_limit"] = variance_limit
        if(mean!=None):
            rules["mean"] = mean
        if(p!=None):
            rules["p"] = p
