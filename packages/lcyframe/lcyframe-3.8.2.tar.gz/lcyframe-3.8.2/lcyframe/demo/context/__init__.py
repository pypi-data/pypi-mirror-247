...
class InitContext(object):
    ...
    def get_context(cls, config_name=None):
        ...
        if not os.path.isabs(cls.config_file):
            cls.config_file = os.path.abspath(os.path.join(os.path.dirname(__file__), cls.config_file))
        cls.config = dict(EnvYAML(cls.config_file))
        # cls.config = yaml2py.load_config(cls.config_file)
