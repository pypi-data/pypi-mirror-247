...
def enter_series(page=None, mode=None):
    ...
    if page and in_series(page, mode):
        return
    if page and page.name in [consts.carhunt, consts.legendary_hunt]:
...
@retry(max_attempts=3)
def enter_career(page=None, mode=None):
    """进入生涯"""
    if not mode:
        mode = globals.CONFIG["模式"]
    if page and in_series(page, mode):
...
def _enter_daily_events(page=None, mode=0):
    ...
    if event_type == "特殊":
        ...
        if page and page.name in [
            consts.world_series,
            consts.limited_series,
            consts.trial_series,
        ]:
            pro.press_group([Buttons.B, Buttons.ZL], 2)
        else:
            reset_to_career()
...
@retry(max_attempts=3)
def enter_custom_event(page=None):
    """进入自定义任务"""
