...
class TagsModel(QStandardItemModel):
    ...
    def set_photo_collection(self, photo_collection):
        self._initialize_data(photo_collection)

    def _initialize_data(self, photo_collection):
        self.clear()
        self.appendRow(QStandardItem(""))
        for tag in photo_collection.get_list_of_all_tags():
...
class PhotoInfoPanel(QWidget):
    class SinglePhotoInfo(QWidget):
        def __init__(self, parent=None, **kwargs):
            ...
            self.creation_timestamp_property = QLabeledProperty("Creation Date")
            ...
            grid_layout.addWidget(self.creation_timestamp_property, 2, 0)
            grid_layout.addWidget(self.gps_coordinates_property, 3, 0)
        ...
        def set_photo(self, photo):
            ...
            self.creation_timestamp_property.set_text(f"{photo.creation_timestamp}")
...
class PhotoAssistant(QMainWindow):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        with importlib.resources.path("photoassistant", "photoassistant-icon.png") as icon_path:
        ...
        # signals and slots
        photo_view_control_panel.sort_function_selected_signal.connect(self.async_sort_photos)
        photo_view_control_panel.apply_filters_signal.connect(self.async_filter_photos)
        photo_view.photos_selected_signal.connect(info_panel.set_photos_selected)
        photo_view.photos_selected_signal.connect(lambda photos: photo_viewer.update_photo(photos[0]) if len(photos) == 1 else None)
        ...
        photo_view.photo_delete_signal.connect(self.async_delete_photo)
    ...
    @staticmethod
    def async_busy_indicator_dialog_execute_decorator(message):
        # convenience decorator for 'busy_indicator_dialog.execute'
        def _busy_indicator_wrapped(func):
            def _busy_indicator_func(self_, *args, **kwargs):
                return self_.busy_indicator_dialog.execute(message, func, self_, *args, **kwargs)
            return _busy_indicator_func
        return _busy_indicator_wrapped
    ...
    def keyPressEvent(self, event):
        if event.modifiers() == Qt.KeyboardModifier.ControlModifier:
            ...
            if event.key() == Qt.Key.Key_R:
                if len(self.photos_selected) == 1:
                    self.rotate_photo(self.photos_selected[0])
                return
        super().keyPressEvent(event)
    ...
    def _initialize_menu_bar(self):
        ...
        open_photos_action = QAction("Open &Photos", self)
        open_photos_action.setStatusTip("Open photos.")
        integrate_opened_photos_action = QAction("Integrate Opened Photos and Meta Data", self)
        integrate_opened_photos_action.setStatusTip("Integrate currently opened photos and their meta data.")
        ...
        open_photos_action.triggered.connect(self._choose_photos_to_open)
        integrate_opened_photos_action.triggered.connect(self._integrate_opened_photos)
        ...
        file_menu.addAction(open_photos_action)
        file_menu.addAction(integrate_opened_photos_action)
    ...
    def create_photo_collection(self, directory):
        try:
            ...
            assert collection_root is None, f"Collections can not be nested. The conflicting collection is '{collection_root}'."
    ...
    @async_busy_indicator_dialog_execute_decorator("Open photo collection.")
    def async_open_photo_collection(self, directory):
        try:
            collection_root = PhotoCollectionManager.find_collection(directory)
            assert collection_root is not None, f"Collection can not be found at '{directory}'. Please use the menu to create a collection first!"
        except PhotoCollectionManager.CollectionError as e:
    ...
    @async_busy_indicator_dialog_execute_decorator("Open photos.")
    def async_open_photos(self, path):
        try:
            assert self.photo_collection.collection_root is not None, "Photos can only be opened after a Collection is opened. Please open a Collection first!"
        ...
        self.busy_indicator_dialog.execute(
            ...
            lambda: self.photo_collection.open_photos(path, sort_key=self.PHOTOS_SORT_FUNCTIONS[self.photos_sort_key])
        ).wait()
        num_photos = len(self.photo_collection.photo_index)
        self.busy_indicator_dialog.execute(
            f"Read tags from {num_photos} photos.",
            lambda: self.tags_model_qt.set_photo_collection(self.photo_collection)
        ).wait()
        return self._async_update_photo_index().wait()

    @async_busy_indicator_dialog_execute_decorator("Update visualized photos.")
    def _async_update_photo_index(self):
        # This function must be executed in a thread seperate from the main thread
        # The 'async_busy_indicator_dialog_execute_decorator' does this implicitly
        if self.photo_collection.collection_root is None or self.photo_collection.photo_index is None:
            return  # decorator will return BusyIndicatorDialog.ExecuteResult(None)
        ...
        def _tags_include_match(photo):
            ...
            for include_tag_pattern in include_tag_patterns:
                if any(re.match(include_tag_pattern, tag) for tag in photo_tags):
                    return True
            return False
        def _tags_exclude_match(photo):
            ...
            for exclude_tag_pattern in exclude_tag_patterns:
                if any(re.match(exclude_tag_pattern, tag) for tag in photo_tags):
                    return True
            return False

        if len(include_tag_patterns) > 0:
            photo_index_copy = self.busy_indicator_dialog.execute(
    ...
    @async_busy_indicator_dialog_execute_decorator("Sort visualized photos.")
    def async_sort_photos(self, sort_key="filename"):
        assert sort_key in self.PHOTOS_SORT_FUNCTIONS
        self.photos_sort_key = sort_key
        return self._async_update_photo_index().wait()

    @async_busy_indicator_dialog_execute_decorator("Filter visualized photos.")
    def async_filter_photos(self, include_filter_tags, exclude_filter_tags):
    ...
    @async_busy_indicator_dialog_execute_decorator("Delete photo.")
    def async_delete_photo(self, photo):
        with self.photo_data_model_qt.removing_photo(photo):
            self.busy_indicator_dialog.execute(
                f"Delete photo '{photo.filename}'.",
                lambda: self.photo_collection.delete_photo(photo),
            ).wait()
            self.photo_data_model_qt.photo_index.remove(photo) # copy of photo_collection.photo_index
    ...
    def set_program_status_info_label_text(self):
        ...
        if self.photo_collection.photo_index is not None:
            photos_root = self.photo_collection.photo_index.root_path
            photos_str = f"Photos: '{photos_root}'"
            if self.photo_index is not None:
                num_photos = len(self.photo_index)
                photos_str = f"Photos: '{photos_root}' ({num_photos} photos)"
    ...
    def _choose_photo_collection_to_create(self):
        ...
        try:
            collection_root = PhotoCollectionManager.find_collection(directory)
            assert collection_root is None, f"Collections can not be nested. The conflicting collection is '{collection_root}'. More information can be found in menu \"Help\""
        except PhotoCollectionManager.CollectionError as e:
    ...
    def _choose_photo_collection_to_open(self):
        ...
        if directory != collection_root:
            InfoQMessageBox(self, f"The directory chosen is contained within a collection at '{collection_root}' which will be loaded now.").exec()

        self.async_open_photo_collection(directory)
    ...
    def _choose_photos_to_open(self):
        ...
        try:
            ...
            assert os.path.isdir(directory), f"The path {directory} does not exist. Please choose an existing path to open photos!"
        except AssertionError as e:
            ErrorQMessageBox(self, str(e)).exec()
            return

        self.async_open_photos(directory)

    def _integrate_opened_photos(self):
        text = (
        ...
        if info.exec() == info.StandardButton.Ok:
            ...
            self._async_update_photo_index()

    def _get_readme_text(self, mark=None):
        text = ""
        with importlib.resources.path("photoassistant", "README.md") as readme_path:
        ...
        if mark is not None:
            ...
            text = re.sub(r"!\[Image\]\(photoassistant/photoassistant-icon.png\)", lambda _: f"![Image]({self.icon_path})", text)
        return text.strip()
