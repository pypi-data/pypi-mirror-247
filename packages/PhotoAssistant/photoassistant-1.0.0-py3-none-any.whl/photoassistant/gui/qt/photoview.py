...
class PhotoView(QAbstractItemView):
    ...
    def update_photo_tile_size(self):
        ...
        photo_tile_extra_height = 0
        if hasattr(self.itemDelegate(), "photo_view_text_height"):
            photo_tile_extra_height = getattr(self.itemDelegate(), "photo_view_text_height")
        self.photo_tile_height = self.photo_tile_width + photo_tile_extra_height
...
class PhotoAssistantPhotoView(PhotoView):
    ...
    def setModel(self, photo_data_model):
        super().setModel(photo_data_model)
        photo_data_model.modelReset.connect(
            lambda: self.selectionModel().select(photo_data_model.index(0, 0, self.rootIndex()), QItemSelectionModel.SelectionFlag.Select)
        )

    def selectionChanged(self, selected, deselected):
        photos_selected = [index.data(Qt.ItemDataRole.UserRole) for index in selected.indexes()]
    ...
    def keyPressEvent(self, event):
        if event.modifiers() == Qt.KeyboardModifier.ControlModifier:
            ...
            if event.key() in (Qt.Key.Key_Delete, Qt.Key.Key_Backspace):
                ...
                #
                # selection_indices = self.selectionModel().selection().indexes()
