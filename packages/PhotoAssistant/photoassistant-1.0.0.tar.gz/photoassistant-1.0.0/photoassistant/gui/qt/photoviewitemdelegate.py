...
class PhotoViewItemDelegate(QStyledItemDelegate):
    def __init__(self, *args, **kwargs):
        ...
        self.photo_view_text_height = 35

        with importlib.resources.path("photoassistant.gui.resources", "icon-loading.png") as icon_path:
    ...
    def paint_with_pixmap(self, painter, option, index, pixmap=None):
        ...
        # original function QStyledItemDelegate.paint such that we can reuse a
        # lot of code:
        # - setClipRegion(...)
        # - draw background:
        #   - drawPrimitive(QStyle.PrimitiveElement.PE_PanelItemViewItem, ...)
        # - draw focus rect:
        #   - subElementRect(QStyle.SubElement.SE_ItemViewItemFocusRect, ...)
        #   - drawPrimitive(QStyle.PrimitiveElement.PE_FrameFocusRect, ...)
        # We could even draw the entire widget as in the original function with
        ...
        image_rect = option.rect.adjusted(
            PhotoViewItemDelegate.IMAGE_MARGIN,
            PhotoViewItemDelegate.IMAGE_MARGIN,
            -PhotoViewItemDelegate.IMAGE_MARGIN,
            -(PhotoViewItemDelegate.IMAGE_MARGIN + self.photo_view_text_height),
        )
        text_rect = QRect(
            image_rect.x(),
            image_rect.bottom(),
        ...
        # draw focus rect
        if QStyle.StateFlag.State_HasFocus in opt.state:
            opt_focus_rect = QStyleOptionFocusRect()
            for attr in ["state", "direction", "rect", "fontMetrics", "palette", "styleObject"]:
                setattr(opt_focus_rect, attr, getattr(opt, attr))
            opt_focus_rect.rect = style.subElementRect(QStyle.SubElement.SE_ItemViewItemFocusRect, opt, widget)
            opt_focus_rect.state |= QStyle.StateFlag.State_KeyboardFocusChange 
            opt_focus_rect.state |= QStyle.StateFlag.State_Item
            cg = QPalette.ColorGroup.Normal if (QStyle.StateFlag.State_Enabled in opt.state) else QPalette.ColorGroup.Disabled
            cr = QPalette.ColorRole.Highlight if (QStyle.StateFlag.State_Selected in opt.state) else QPalette.ColorRole.Window
            opt_focus_rect.backgroundColor = opt.palette.color(cg, cr)
            style.drawPrimitive(QStyle.PrimitiveElement.PE_FrameFocusRect, opt_focus_rect, painter, widget)
