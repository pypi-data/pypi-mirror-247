...
class QLabeledProperty(QModernStyleGroupBoxContainer):
    ...
    def __init__(self, title, parent=None):
        ...
        self.layout().addWidget(self._property_label, 0, 0)
...
class QPixmapLoader:
    ...
    def load(path, max_size=None, orientation_correction=None):
        try:
            ...
            with importlib.resources.path("photoassistant.gui.resources", "icon-broken-image.png") as icon_path:
...
class BusyIndicatorDialog(QDialog):
    ...
    class MessageUpdateModule(threading.Thread):
        ...
        MINIMUM_MESSAGE_DISPLAY_TIME = 1 # in seconds
        MESSAGE_DISPLAY_DELAY = 0.5 # in seconds
        ...
        def run(self):
            time.sleep(self.MESSAGE_DISPLAY_DELAY)
            while True:
    ...
    def execute(self, message, func, *args, **kwargs):
        ...
        def _target():
            with self.execute_thread_lock:
                self.message_update_module = self.MessageUpdateModule()
