...
class UsageApi(object):
    ...
    def get_usage(self, **kwargs):
        """
        Get usage records for the specified time range. Results will be sorted by date if aggregating by day, otherwise by amount descending and limited to the first 500 records
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        to be invoked when receiving the response.
        >>> def callback_function(response):
