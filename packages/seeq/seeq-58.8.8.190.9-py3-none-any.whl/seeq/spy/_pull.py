...
def pull(items, *, start=None, end=None, grid='15min', header='__auto__', group_by=None,
    ...
    if shape == 'samples' and not (at_least_one_signal or (at_least_one_asset and calculation_is_signal)):
    ...
    while True:
        ...
        if shape == 'capsules' and at_least_one_signal:
            ...
            this_capsule_count, next_start, continuation_token, result_df, http_headers = \
                _pull_condition_via_formula_api(session,
                                                calculation_to_use,
                                                shape,
                                                capsule_properties,
                                                current_start,
...
def _pull_condition_via_formula_api(session: Session, calculation_to_use, shape, capsule_properties, current_start,
                                    continuation_token, index_to_use, item_id, item_name, offset, pd_end, tz, result_df,
                                    column_names):
    ...
    if shape == 'samples':
        ...
        df_to_append = pd.DataFrame(capsule_dict_list, columns=column_names[item_id])
...
def _pull_condition_as_a_table(session: Session, capsule_properties, current_start, query_df, item_id, item_name,
    ...
    for row in formula_table:
        ...
        for column_name in additional_columns_for_this_row:
            ...
            if column_name not in column_names[item_id]:
    ...
    df_to_append = pd.DataFrame(capsule_df_rows, columns=column_names[item_id])
