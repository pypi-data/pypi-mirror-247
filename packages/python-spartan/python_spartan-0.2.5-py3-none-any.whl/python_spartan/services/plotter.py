...
class PlotterService:
    def __init__(self, name: str):
        ...
        self.destination_folder = "plotters"
        self.source_stub = os.path.join(
            self.home_directory, "stubs", "plotter", "default.stub"
        )
    ...
    def create_plotter_file(self):
        ...
        if not os.path.exists(self.file_path):
            ...
            try:
                ...
                print(f"An error occurred while writing the new plotter file: {e}")
        else:
            print(f"File '{self.file_path}' already exists. Skipping creation.")

    def delete_plotter_file(self):
