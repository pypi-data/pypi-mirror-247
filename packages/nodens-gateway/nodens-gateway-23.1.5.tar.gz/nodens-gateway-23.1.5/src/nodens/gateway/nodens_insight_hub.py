...
def on_subscribe_MS(unused_client, unused_userdata, mid, granted_qos):
    nodens.logger.debug('MINDSPHERE: on_subscribe: mid {}, qos {}'.format(mid, granted_qos))

def on_connect_MS(client, userdata, flags, rc):
    global CONNECTION_ON
    nodens.logger.debug('MINDSPHERE: on_connect: {} userdata: {}. flags: {}. rc: {}'.format(mqtt.connack_string(rc), userdata, flags, rc))
    CONNECTION_ON = 1

def on_disconnect_MS(client, userdata, rc):
    global CONNECTION_ON
    nodens.logger.debug('MINDSPHERE: on_disconnect: {} userdata: {}.'.format(mqtt.connack_string(rc), userdata))
    CONNECTION_ON = 0
...
def on_unsubscribe_MS(client, userdata, mid):
    nodens.logger.debug('MINDSPHERE: on_unsubscribe: mid {}. userdata: {}.'.format(mid, userdata))
...
dir = "C:/Users/kraja/OneDrive/Documents/MindSphere/Certificates/"
...
def get_mindconnect_client(
    ...
    nodens.logger.debug("Setting up client...")
    client = mqtt.Client("us2dev_nodens_test")
...
class sendMindConnect:
    ...
    def send_mindconnect_payload(self, 
        ...
        while CONNECTION_ON == 0:
            nodens.logger.warning("Not connected yet")
            sleep(delay_time)

        if CONNECTION_ON == 1:
            # Message
            nodens.logger.debug("Sending message to MindConnect...")
            # Set values
            ...
            nodens.logger.debug("Detected: {}. Activity type to send: {} at {}".format(activity_detected, activity_type, timestamp))
            ...
            nodens.logger.debug("Sending to Insight Hub...")
            self.mqtt_client.publish(self.topic_timeseries_send,payload=json.dumps(send_packet),qos=0) # TODO: update topic
            self.mqtt_client.loop_stop()

            # Message
            nodens.logger.debug("...message sent.")
        else:
            nodens.logger.warning("Not connected yet 2")
            sleep(1)
