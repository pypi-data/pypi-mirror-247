...
def get_config(config, SECTION, CONFIG, CONFIG_str):
    try:
        ...
        output = CONFIG
        logger.debug('{} not specified in config file. Default value used.'.format(CONFIG_str))
    else:
        output = config.get(SECTION, CONFIG_str).partition('#')[0].rstrip()
        logger.debug('CONFIG: {} = {}'.format(CONFIG_str, config.get(SECTION, CONFIG_str).partition('#')[0].rstrip()))
...
def parse_config(config_file, EntryWays, rcp, cp, client):
    ...
    if config_file is None:
        logger.warning('No config file. Default values used.')
    elif config_file.is_file():
        config = configparser.RawConfigParser()
        config.read(config_file)
        logger.debug(config['Sensor target']['SENSOR_ID'])
        ...
        try:
            ...
            logger.debug('{} not specified in config file. Default value used: {}'.format('SENSOR_TOPIC', rcp.SENSOR_TOPIC))
        else:
            rcp.SENSOR_TOPIC = config.get('Sensor target', 'SENSOR_TOPIC').partition('#')[0].rstrip()
        if not bool(rcp.SENSOR_TOPIC):
            rcp.SENSOR_TOPIC = '#'
        logger.debug('Topic = {}'.format(rcp.SENSOR_TOPIC))
    ...
    if rcp.RADAR_SEND_FLAG:
        ...
        if sv.radar_dim == 2:
            ...
            logger.debug(rcp.config_radar[12])
        ...
        if sv.radar_dim == 2:
            ...
            logger.debug(rcp.config_radar[10])
        ...
        if sv.radar_dim == 2:
            ...
            logger.debug(rcp.config_radar[15])
        elif sv.radar_dim == 3:
            # Static - 22
            i = 0
            while True:
                if i == len(rcp.config_radar):
                    logger.warning("Config error: {} not found!)".format("staticBoundaryBox "))
                    break
                    ...
                    logger.debug(rcp.config_radar[i])
                    break
                else:
                    i+=1
            ...
            while True:
                if i == len(rcp.config_radar):
                    logger.warning("Config error: {} not found!)".format("boundaryBox "))
                    break
            ...
            while True:
                if i == len(rcp.config_radar):
                    logger.warning("Config error: {} not found!)".format("presenceBoundaryBox "))
                    break
    ...
    if rcp.ENTRY_FLAG:
        ...
        for i in range(len(temp)):
            ...
            if len(temp_split) == 2:
                ...
                logger.debug('Entryway x: {},{}.'.format(float(temp_split[0]),float(temp_split[1])))
            else:
                logger.warning('WARNING! Incorrect format provided: {}. Format should be two numbers separated with a comma, e.g. \'-1,1\''.format(temp[i]))
        ...
        for i in range(len(temp)):
            ...
            if len(temp_split) == 2:
                (temp_parse.append(float(temp_split[0])))
                (temp_parse.append(float(temp_split[1])))
                logger.debug('Entryway y: {},{}.'.format(float(temp_split[0]),float(temp_split[1])))
            else:
                logger.warning('WARNING! Incorrect format provided: {}. Format should be two numbers separated with a comma, e.g. \'-1,1\''.format(temp[i]))
...
class sensor_version:
    ...
    def parse(self, str):
        ...
        if len(str) > 0:
            ...
            if str[0] == 'C':
                ...
                logger.debug("VERSIONING ERROR. Mismatched Wi-Fi version. Expected 'CXX...'. Detected %s.", str)
                self.radar_dim = 2
            if (str[8] == 'R' and str[10] == 'D'):
                ...
                if (temp == 2 or temp == 3):
                    ...
                    logger.debug("VERSIONING ERROR. Mismatched RADAR dimensions. Expected '...RXD...'. Detected %s.", str)
            else:
                logger.debug("VERSIONING ERROR. Mismatched RADAR version. Expected 'RXD...' where X=2/3. Detected %s. Revert to 2D.", str)
                self.radar_dim = 2
            print(self.wifi_version, self.radar_version)
        else:
            logger.debug("VERSIONING ERROR. No version provided. Defaulting to 2D config.")
            self.radar_dim = 2
    def request(self, client, root_topic, sensor_target):
        ...
        logger.debug("Published sensor version request")
        temp = 0
        while (1):
            if self.string != []:
                logger.debug("Version received. Version = {}. Dimensions = {}.".format(self.string, self.radar_dim))
                break
            elif temp < 20:
                logger.debug("Waiting... {}".format(self.string))
                temp += 1
                time.sleep(0.2)
            else:
                logger.debug("No response to version request. Continue...")
                break
...
class OccupantHist:
        ...
        if (sensor_id in ew.id):
            ...
            try:
                if abs(sx0-sx1)+abs(sy0-sy1) > 0:
                    # if len(ew.x[ind_e]) == 0:
                    #     logger.debug('No entries defined')
                    for i in range(int(len(ew.x[ind_e])/2)):
                        ...
                        if (0<=t<=1) and (0<=u<=1):
                            if (sx0**2 + sy0**2) > (sx1**2 + sy1**2):
                                ew.count[ind_e] += 1
                                logger.info('Entered room at entry {} with (t,u)=({},{}). Occupancy = {}.'.format(i,t,u,ew.count[ind_e]))
                            else:
                                ew.count[ind_e] -= 1
                                if ew.count[ind_e] < 0:
                                    ew.count[ind_e] = 0
                                    logger.warning('Warning! Count dropped below 0 and was reset.')
                                logger.info('Leaving room at entry {} with (t,u)=({},{}). Occupancy = {}.'.format(i,t,u,ew.count[ind_e]))
            except:
                logger.warning('Entryway update issue. (sx0,sx1)=({},{}). (sy0,sy1)=({},{})'.format(sx0,sx1,sy0,sy1))
    ...
    def activity_detection(self, sensor_id, track_id, tot_dist_thresh=1, max_dist_thresh=1):
        ...
        try:
            ...
            logger.error("len(xd) != len(yd). xh={}. yh={}.".format(self.xh, self.yh))
    ...
    def update(self, raw):
        ...
        if self.X != []:
            if (np.sqrt((X-self.X)**2 + (Y-self.Y)**2)) > 1:
                logger.debug("RESET VS: {}".format((np.sqrt((X-self.X)**2 + (Y-self.Y)**2))))
                self.__init__()
        ...
        if self.heart_rate_raw[0] != None:
            ...
            logger.debug(self.heart_rate, self.heart_rate_raw[0])
...
class ud:
    ...
    def signature(raw, Nud, spec_length, Nchirps):
        ...
        if Nchirps > spec_length:
            Nchirps = spec_length
            logger.warning("Nchirps should be <= spec_length")
...
def convert_4_to_1(arg1, *argv):
    ...
    if  isinstance(arg1, (list,)):   
        try:
            ...
            logger.error("convert_4_to_1 error. arg1: {}".format(arg1))
            output = 65536
    else:
        output = arg1
        x = 1
...
class parseTLV:
        ...
        while (j < self.packet_len):
            if (data[j:j+4] == [6,0,0,0]):
                ...
                if (j <= self.packet_len):
                    ...
                    logger.debug("(X,Y): ({:.1f},{:.1f}). HR:{:.1f}. BR: {:.1f}.".format(self.vs.X, self.vs.Y,  self.vs.heart_rate, self.vs.breathing_rate))
                    logger.debug(" B dev: {}".format(self.vs.breathing_deviation[0:3]))
                    logger.debug(" B vote: {}".format(self.vs.breathing_vote[0:3]))
    ...
    def tlvN(self, data, j):
        ...
        if (lenN == 65536):
            logger.warning("Data packet TLV length error. j: {}. len: {}.".format(j,len(data)))
        dataN = data[j:j+lenN]
        j += lenN
...
class sendCMDtoSensor(object):
    ...
    def request_version(rcp,cp,sv):
        ...
        logger.info("Published sensor version request to {}".format(rcp.SENSOR_TOPIC))
        temp = 0
        while (1):
            if sv.string != []:
                logger.info("Version received. Version = {}. Dimensions = {}.".format(sv.string, sv.radar_dim))
                break
            elif temp < 20:
                logger.info("Waiting... {}".format(sv.string))
                temp += 1
                time.sleep(0.2)
            else:
                logger.info("No response to version request. Continue...")
                break
...
class MessagePipeline:
    ...
    def clear(self, index):
        if index < len(self.sensor_id):
            ...
            logger.warning("Sensor index %d does not exist", index)
...
logger.info(si.connected_sensors)