...
elly_a = 5900268983
my_tid = 5491025132
my_tids = ['5900268983', '6179455648', '6236215930',

    '5754810063', '5491025132', '5360564451', '6281795468', ]
...
ferey_instagram = 'https://www.instagram.com/ferey_page'
...
short_description = f"""👩🏽‍💻 New @telegram neuro-#marketing bots

Connect: t.me/FereyDemoBot
🇬🇧🇨🇳🇦🇪🇪🇸🇷🇸🇫🇷
"""
...
BOT_CNOTIFICATION_ = '☑☐☑☐'
BOT_CUSER_ = '☑☐☐'
BOT_VARS_ = '{"BOT_PROMO": "#911", "BOT_CHANNEL": 0, "BOT_CHANNELTID": 0, "BOT_GROUP": 0, "BOT_GROUPTID": 0, "BOT_GEO": 0, "BOT_TZ": "+00:00", "BOT_DT": "", "BOT_LZ": "en", "BOT_LC": "en"}'
...
trg_utms = ['360', 'ad', 'advert', 'aesthetic', 'ai', 'air', 'alpha', 'beta', 'big', 'blog', 'boost', 'bot', 'box',
    'brainstorm', 'bravo', 'buy', 'calc', 'care', 'cascade', 'change', 'channel', 'charm', 'chat', 'chn', 'click',
    'cloud', 'club', 'codex', 'color', 'command', 'connect', 'copy', 'course', 'day', 'demo', 'dev', 'discord',
    'discount', 'doc', 'docum', 'document', 'download', 'dream', 'dzen', 'echo', 'element', 'event', 'facebook', 'fast',
    'fest', 'festival', 'field', 'file', 'first', 'fit', 'fix', 'flow', 'fly', 'focus', 'for', 'forest', 'free',
    'fresh', 'fun', 'future', 'get', 'gift', 'giga', 'git', 'go', 'grade', 'grape', 'guide', 'hackathon', 'have',
    'here', 'high', 'hill', 'hot', 'hub', 'ig', 'infinite', 'info', 'intensive', 'landing', 'last', 'lead', 'league',
    'learn', 'lesson', 'level', 'life', 'like', 'lime', 'limit', 'link', 'linkedin', 'load', 'look', 'map', 'marathon',
    'master', 'mastermind', 'max', 'medium', 'meet', 'method', 'mind', 'mix', 'mobile', 'more', 'msg', 'neuro', 'new',
    'newsletter', 'night', 'nike', 'node', 'note', 'nova', 'number', 'ocean', 'omega', 'one', 'orange', 'paint',
    'pinterest', 'plane', 'platform', 'play', 'podcast', 'portal', 'post', 'present', 'press', 'pro', 'promo', 'puzzle',
    'quick', 'ran', 'rate', 'read', 'ready', 'real', 'reality', 'realm', 'red', 'reddit', 'refer', 'referral', 'review',
    'round', 'run', 'safe', 'salute', 'save', 'scale', 'school', 'sea', 'seminar', 'seo', 'service', 'session', 'set',
    'share', 'shop', 'sky', 'smart', 'snow', 'social', 'soundcloud', 'special', 'spotify', 'spring', 'star', 'stat',
    'status', 'stellar', 'stone', 'subscribe', 'summer', 'sun', 'symposium', 'tap', 'taplink', 'target', 'team', 'tech',
    'technology', 'tele', 'telegram', 'telegraph', 'telescope', 'ten', 'testdrive', 'text', 'this', 'tiktok', 'time',
    'top', 'training', 'trend', 'trigger', 'true', 'try', 'turbo', 'unity', 'university', 'unsubscribe', 'up', 'utm',
    'vimeo', 'vip', 'wait', 'wave', 'web', 'webinar', 'webmaster', 'website', 'week', 'win', 'winter', 'wordpress',
    'work', 'workbook', 'workshop', 'wow', 'x', 'yes', 'you', 'you2b', 'youtube', 'yt', 'zip']
    ...
    try:
        ...
        if message.text == '⬅️ Prev':
            ...
            if offer_id:
                ...
                await db_change(sql, (
                chat_id, offer_text, offer_file_type, file_name_part, offer_file_id, offer_file_id_note, offer_button,
                offer_isbutton, offer_tgph_link, offer_istgph, offer_dt, offer_tz, 1, offer_id,), BASE_D)
            else:
                ...
                await db_change(sql, (
                chat_id, offer_text, offer_file_type, file_name_part, offer_file_id, offer_file_id_note, offer_button,
                offer_isbutton, offer_tgph_link, offer_istgph, offer_dt, offer_tz, 1,), BASE_D)
...
async def format_text_md(txt, is_web=False):
    ...
    try:
        ...
        tmp_arr = re.split(r'\s+', result)
        if len(tmp_arr) > 2 and not tmp_arr[1].startswith(('<', '#', '$', '=', '*', '_', '|', '[', '~', '{', '`')):
        ...
        if not is_web:
            ...
            while i > 0:
                ...
                if (0 < r_i < len(tmp_arr) - 1 and len(word) >= 4 and not word.startswith(
                    ...
                    map(lambda c: c.isascii() and not c.isalnum(), word))):
                    # print(f'python: {word}')
                    # result = result.replace(word, f"```py\n{word}```")
                    word_pattern = re.escape(word)
                    result = re.sub(rf'\b{word_pattern}\b', f"```py\n{word}```", result)
                    break
...
async def format_text(txt, is_web=False):
    ...
    try:
        ...
        while i > 0:
            ...
            word = tmp_arr[r_i]
            if (0 < r_i < len(tmp_arr) - 1 and len(word) >= 4 and not word.startswith(
                ('<', '#', '$', '=', 'href')) and not any(map(lambda c: c.isascii() and not c.isalnum(), word))):
                # print(f'italic: {word}')
        ...
        if not is_web:
            ...
            while i > 0:
                ...
                if (0 < r_i < len(tmp_arr) - 1 and len(word) >= 4 and not word.startswith(
                    ...
                    word_pattern = re.escape(word)
                    result = re.sub(rf'\b{word_pattern}\b', f"<pre>py\n{word}</pre>", result)
                    break
        ...
        while cnt >= 0:
            try:
                ...
                if '\n' in result:
                    ...
                    result = result.replace('[^1^]', '').replace('[1]','').replace('[^2^]', '').replace('[2]','').replace('[^3^]', '').replace('[3]','').replace('[^4^]', '').replace('[4]','')
                    result = result.replace('https://bing.com/search?q=', '').replace('""', '').replace('dw.com', '').replace('Википедия', '')
                    result = result.replace(' , ', ', ').replace('\n: ', '').replace('\n\n: ', '').replace(' — ', ' - ').replace('/ru/', '')
                    result = result.replace('%D0%', '').replace('%D1%', '').replace('%87%', '').replace('%82%', '').replace('%BE%', '').replace('%B0%', '').replace('%B5%', '').replace('%B1%', '').replace('%8B%', '').replace('%BA%', '').replace('%80%', '').replace('%8E%', '').replace('()', '').replace('https://www.', '').replace(' .', '.')
                    result = result.replace('Источник: ', '').replace('8782BE', '').replace('82B0BABEB5', '').replace('B1BE828B', '').replace('B8', '').replace('BAB0BA','').replace('BEBDB8', '').replace('80B0B1BE82B08E82', '').replace('a-41716550', '').replace(' – DW – ', '')
                    result = result.strip()
            ...
            try:
                if item['type'] == 'tgph':
                    ...
                    if not item['prompt'].startswith('https://'):
                        ...
                        data_.append({'type': item['type'], 'prompt': file_content_hex, 'extra': 'file_path', 'file_name': file_name})
                    else:
                    ...
                    data_.append({'type': item['type'], 'prompt': prompt, 'count': count, 'file_name': file_name, 'file_content': file_content})
                elif item['type'] == 'tts':
                    ...
                    if isinstance(prompt, str):
                        prompt = [
                            ...
                            {"role": "user", "content": f"Translate the following text (to `{lc}`-ISO language code): {prompt}"}
                        ]
            ...
            for item in data_:
                try:
                    ...
                    if item['type'] == 'tgph':
                        ...
                        try:
                            ...
                            while cnt >= 0:
                                try:
                                    ...
                                    result.append({'type': item['type'], 'answer': f"https://telegra.ph{res[0]['src']}"})
                                    break
                                except Exception as e:
                                    logger.info(log_ % str(e))
                        ...
                        while cnt >= 0:
                            ...
                            try:
                                ...
                                if item['prompt'] == 'variation':
                                    ...
                                    try:
                                        res = await client.images.create_variation(
                        ...
                        # if os.path.exists(dst_mp3): os.remove(dst_mp3)
                    elif item['type'] == 'stt':
                        is_res = False
                        text = ''
                        dt_ = datetime.datetime.utcnow().strftime("%d-%m-%Y_%H-%M-%S-%f.mp3")
...
async def create_invoice_link(BOT_TID, BOT_LC, msg_text, msg_btns, BASE_D):
    ...
    try:
        ...
        result = await extra_bot.create_invoice_link(title=msg_media['title'], description=msg_media['description'],
            payload=msg_media['payload'], provider_token=msg_media['provider_token'], currency=msg_media['currency'],
            prices=msg_media['prices'], max_tip_amount=msg_media['max_tip_amount'],
...
async def region_web(bot, username, ENT_TID, ENT_LC, POST_ID, POST_TYPE, POST_TEXT, POST_LNK, POST_BUTTON, ENT_TYPE,
    ...
    try:
        ...
        html_web = html_template.format(m_html, txt_html, btn_html, msg_views, ENT_LINK, ENT_USERNAME, ENT_TID, POST_ID, POST_LNKS)
        if POST_ID:
    ...
    try:
        ...
        while cnt >= 0:
            try:
                POST_TEXT = POST_TEXT if POST_TEXT else str_empty
                POST_TEXT = (
                    POST_TEXT.replace('<tg-spoiler>', '').replace('</tg-spoiler>', '').replace('<span>', '').replace(
                        '</span>', ''))
