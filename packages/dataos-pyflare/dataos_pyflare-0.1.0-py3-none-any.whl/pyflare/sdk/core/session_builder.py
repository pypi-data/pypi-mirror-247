...
from pyflare.sdk.utils.generic_utils import resolve_dataos_address
...
class SparkSessionBuilder:
    ...
    def with_spark_conf(self, conf) -> SparkSessionBuilder:
        ...
        os.environ[SPARK_APP_NAME] = app_name[-1] if app_name else constants.SPARK_APP_NAME_PREFIX
        return self
...
def load(name, format, driver=None, query=None, options=None):
    """
        ...
        Args:
            ...
            optional params -
            driver (str): driver to read source
    ...
    heimdallClient = spark._jvm.HeimdallClient.Builder().url(os.environ.get("Heimdall_URL", "")).apikey(g_dataos_token).build()
    data_govern_jvm = spark._jvm.DataGovernor.getInstance(heimdallClient)
