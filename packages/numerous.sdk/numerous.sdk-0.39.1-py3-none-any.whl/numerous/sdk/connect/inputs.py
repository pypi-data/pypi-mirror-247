...
class InputStream:
    ...
    def read_variables(self, key: datetime) -> dict[str, float]:
        return {var.key: self._read_variable(var, key) for var in self._input_variables}
    ...
    def _create_stream(self, key: Optional[datetime]) -> Iterator[spm_pb2.DataList]:
        ...
        if key is None:
            ...
            start_time = (
                key.timestamp()
                - self._input_source.offset
                - maximum_variable_offset
                - self._repeat_offset
            )
...
class InputReader(Iterable[tuple[datetime, dict[str, float]]]):
    ...
    def __init__(
        ...
        loaded_last_read_key = self.load_last_read_key()
        self._next_key = (
            self._start if loaded_last_read_key is None else loaded_last_read_key
        )
        self._step = step
        self._repeat_spacing = repeat_spacing
    ...
    def store_last_read_key(self) -> None:
        if self._last_read_key is None:
            return

        self._spm_stub.SetScenarioCustomMetaData(
            request=spm_pb2.ScenarioCustomMetaData(
                ...
                key=f"{self._identity.job_id}.hibernated_at_job_read_time",
                meta=json.dumps(self._last_read_key.isoformat()),
            )
        )

    def load_last_read_key(self) -> Optional[datetime]:
        hibernated_at_job_read_time = self._spm_stub.GetScenarioCustomMetaData(
            request=spm_pb2.ScenarioCustomMetaData(
                project=self._identity.project_id,
                scenario=self._identity.scenario_id,
                execution=self._execution_id,
        ...
        if not hibernated_at_job_read_time:
            return None
        else:
            try:
                return datetime.fromisoformat(json.loads(hibernated_at_job_read_time))
