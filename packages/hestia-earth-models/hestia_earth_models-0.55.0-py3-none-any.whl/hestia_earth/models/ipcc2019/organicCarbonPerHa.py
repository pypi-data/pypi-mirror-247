...
def _should_run_annual_temperature_factors(
    ...
    should_run = all([
        all((len(inner[_InnerKey.TEMPERATURES]) == 12 for inner in grouped_data.values())),
...
def _should_run_annual_water_factors(
    ...
    should_run = all([
        all((len(inner[_InnerKey.PRECIPITATIONS]) == 12 for inner in grouped_data.values())),
        all((len(inner[_InnerKey.PETS]) == 12 for inner in grouped_data.values())),
        all((len(inner[_InnerKey.IS_IRRIGATEDS]) == 12 for inner in grouped_data.values())
            ) if run_with_irrigation else True,
        len(grouped_data.keys()) >= MIN_RUN_IN_PERIOD,
        _check_consecutive(grouped_data.keys()),
