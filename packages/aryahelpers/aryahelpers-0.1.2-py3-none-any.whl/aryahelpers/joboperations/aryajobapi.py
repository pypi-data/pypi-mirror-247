"""Flask api to peroform automated Arya job operations"""
...
from aryahelpers.config import CONFIG, COPY_JOB_LIMIT, CANDIDATE_TABLES
...
class AryaJobOperations():
    ...
    def __init__(self, **kwargs):
        ...
        self.db_key, self.db_name, self.base_table, self.jobid_key, self.guid_key = \
            tuple(CANDIDATE_TABLES["toprocess"]["base"].values())
    ...
    def _get_dbkey_and_joburl(self, env: str):
        ...
        if env == 'qa':
            return 'database', f"{CONFIG['arya_job_urls']['qa']['jobs_v3.1']}"
        return 'database.staging', f"{CONFIG['arya_job_urls']['staging']['jobs_v3.1']}"
    ...
    def validate_copy_jobs_payload(self, payload):
        ...
        if payload:
            ...
            if job_ids and len(job_ids) <= COPY_JOB_LIMIT and users_lens_set == {2}:
                if set(source_and_target_envs) <= {'qa', 'staging'}:
    ...
    def copy_single_job(self, job_id):
        ...
        if target_job:
            ...
            if self.to_retain_job and self.target_env == 'qa':
                MySQLHelpers.transact_to_jobsretain_tbl(self.db_key, 'candidate_reservoir', [target_job_id], 'insert')
    ...
    @elapsedTime
    def arya_sourced_jobs_record(self, filter_dict={}, save_path=False):
        ...
        try:
            db_key, db_name, jobid_key = self.db_key, self.db_name, self.jobid_key
            ...
            for idx, jid in enumerate(job_ids):
                ...
                arya_jobs_record.update({jid: job_record})
...
@app.route('/copy_jobs', methods=['POST'])
def copy_jobs():
    """The copy job route. Example request payload is shown below.
    {
        "job_ids": [388036, 388004],
...
@app.route('/jobs_record', methods=['POST'])
def jobs_record():
    """The Arya sourced jobs route. Example request payload is shown below.
    {
    ...
    try:
        ...
        joboperations_obj.arya_sourced_jobs_record(filter_dict, save_path)
