...
from aryahelpers.config import CONFIG
...
class MySQLHelpers(object):
    ...
    def get_TopN_JTR(db_key, jobid, topN=None):
        """Get JTR candidates"""
        base_query = "SELECT job_id, candidate_name,  format(candidate_score, 2) AS candidate_score, \
    ...
    def _get_job_metadata(db_key: str, job_id: int):
        ...
        query_str = "SELECT CreatedById, CONVERT_TZ(CreatedDate,'+00:00','+05:30') AS CreatedDate, CONVERT_TZ(\
                LastUpdateTimeStamp,'+00:00','+05:30') AS UpdateDate FROM job WHERE JobId={};".format(job_id)
        query_res = MySQLManager.execute_query(query_str, (), **CONFIG[db_key]['arya'])
        if not query_res:
            return job_metadata
        user_id = query_res[0]["CreatedById"]
        create_date = query_res[0]["CreatedDate"].strftime("%d-%b-%Y, %H:%M:%S IST")
        update_date = query_res[0]["UpdateDate"].strftime("%d-%b-%Y, %H:%M:%S IST")
    ...
    def extract_arya_jobdetails(db_key: str, job_id: int):
        ...
        try:
            ...
            if client_company and job_title:
                ...
                arya_jobdetails = merge_dicts((arya_jobdetails, job_metadata))
