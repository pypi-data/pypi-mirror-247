...
def test_save_audio(reference_stipa_waveform, tmp_path):
    # Test saving a waveform to file

    smaller_waveform = AudioWaveform(reference_stipa_waveform.timeseries[:1000],
                                     sample_rate=reference_stipa_waveform.sample_rate,
                                     name='test_waveform')

    out_name = os.path.join(f"{tmp_path}", "test_waveform.wav")
    smaller_waveform.save_waveform_to_file(filename=out_name)

    assert os.path.exists(out_name)
    assert os.path.getsize(out_name) > 0

    # fail if the format isn't allowed
    with pytest.raises(WaveformOperationException):
        smaller_waveform.save_waveform_to_file(filename=out_name, format='not_a_format')

    # fail if the subtype isn't allowed
    with pytest.raises(WaveformOperationException):
        smaller_waveform.save_waveform_to_file(filename=out_name, format='WAV', subtype='not_a_subtype')
...
def test_match_lengths():
    waveformA = AudioWaveform(np.arange(-100, 100.5, 1), sample_rate=8000)
    waveformB = AudioWaveform(np.arange(-100, 100.5, 2), sample_rate=8000)
    assert len(waveformA.timeseries) != len(waveformB.timeseries)
    waveformA, waveformB = AudioWaveform.match_lengths(waveformA, waveformB)
    assert len(waveformA.timeseries) == len(waveformB.timeseries)


def test_match_length_sample_rate_warning(caplog):
    waveformA = AudioWaveform(np.arange(-100, 100.5, 1), sample_rate=8000)
    waveformB = AudioWaveform(np.arange(-100, 100.5, 2), sample_rate=4000)
    waveformA, waveformB = AudioWaveform.match_lengths(waveformA, waveformB)
    assert waveformB.sample_rate != waveformA.sample_rate
    assert "You are matching the lengths of two waveforms with different sample rates." in caplog.text
...
def test_get_total_waveform_duration():
    waveform = AudioWaveform(np.arange(100), sample_rate=1)
    assert waveform.get_total_waveform_duration() == 100

    waveform.sample_rate = 0
    with pytest.raises(ZeroDivisionError):
        waveform.get_total_waveform_duration()


def test_moving_std():
    waveform_length = 2880000
    test_vals = np.random.uniform(low=-0.7, high=0.7, size=waveform_length)
    test_waveform = AudioWaveform(test_vals, sample_rate=1000)

    segment_length_sec = 0.01

    # Do moving std with chunk size larger than the array (no breaking up required)
    moving_std = test_waveform.get_moving_std(std_window_size_sec=segment_length_sec, chunk_size=waveform_length * 2)

    num_samples = int(segment_length_sec * test_waveform.sample_rate)

    # test some values
    for offset in np.arange(0, 100, 5):
        assert test_waveform.timeseries[offset:offset + num_samples].std() == moving_std[offset]

    # Do moving std with chunk size smaller than the array (breaking up required)
    moving_std2 = test_waveform.get_moving_std(std_window_size_sec=segment_length_sec,
                                               chunk_size=int(waveform_length / 10))

    assert len(moving_std) == len(test_waveform.timeseries)
    assert len(moving_std) == len(moving_std2)

    # despite different chunking, we should get the same results
    assert np.array_equal(moving_std2, moving_std)
