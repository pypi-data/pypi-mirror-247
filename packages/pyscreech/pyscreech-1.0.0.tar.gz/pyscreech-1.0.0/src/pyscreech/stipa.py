...
class StipaCalculator(ScoreCalculator):
    ...
    def calculate_score_from_waveforms(self,
        ...
        Args:
            ...
            butter_order (int): Butterworth filter order used in octave band filter
            firwin_filter_params (FirwinFilterParameters): low pass filter parameters
            log_details (bool): logs modulation transfer indices at each octave band for the results
        ...
        if log_details:
            logger.info(f"Octave band centers (Hz):    {OCTAVE_BAND_CENTERS}")
            logger.info(f"Modulation transfer indices: {modulation_transfer_index}")
