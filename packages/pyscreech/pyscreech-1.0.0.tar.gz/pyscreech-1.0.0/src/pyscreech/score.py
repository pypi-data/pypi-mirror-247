...
class ScoreCalculator(abc.ABC):
    ...
    def calculate_multiple_random(self,
        ...
        Args:
            ...
            use_std (bool): whether to use a moving standard deviation to sync waveforms (True) or the waveforms
                themselves (False)
        ...
        if duration_sec == 0:
            # if duration of zero is requested, use the full waveform, but first match lengths
            input_waveform, reference_waveform = AudioWaveform.match_lengths(waveformA=input_waveform,
                                                                             waveformB=reference_waveform,
                                                                             reset_start_times=True)
            duration_sec = input_waveform.get_total_waveform_duration()
    ...
    def calculate_multiple_sequential(self,
        ...
        if sync_waveforms:
            input_waveform, reference_waveform = AudioWaveform.get_synced_waveforms(waveformA=input_waveform,
                                                                                    waveformB=reference_waveform,
                                                                                    use_std=use_std)
        ...
        if num_input_segments < num_segments or num_ref_segments < num_segments:
            msg = (f"\nNumber of input segments ({num_input_segments}) or number of reference "
                   f"segments ({num_ref_segments}) is less than requested "
                   f"number ({num_segments}).\n"
                   f"Total reference recording duration:      {reference_waveform.get_total_waveform_duration()} sec\n"
                   f"Total input recording duration:          {input_waveform.get_total_waveform_duration()} sec\n"
                   f"Segment duration (offset) requested:     {duration_sec} sec ({offset_sec} sec)\n"
                   f"Number of sequential segments requested: {num_segments} sec\n"
                   )
            raise ScoreCalculationException(msg)
