...
###############################################################################
    ...
    10) varsmooth() to convolve a spectrum with a variable sigma using FFT
...
class synthetic_photometry:
    ...
    Notes
    -----
    When using the E-MILES SSP models, note that some spectra have no NIR
    spectrum at age < 1 Gyr. For details on the caveats see `Vazdekis et al. (2016)
    ...
    def __init__(self, spectra, lam_spec, bands,
        ...
        if filters_file is None:
            ...
            filters_file = path.join(ppxf_dir, "examples", "FILTER.RES.txt")
...
def synthetic_photometry_one_band(spectra, lam_spec, band, redshift, filters_file):
    """
...
def read_filter(band, filters_file):
    ...
    j, lam_resp, resp = np.array([ln.split() for ln in lines], float).T

    return lam_resp, resp

###############################################################################

def varsmooth(x, y, sig_x, xout=None, oversample=1, shift=0):
    """    
    ...
    shift : float
        Constant shift to apply to the vector `y` in units of `x`. 
        The output becomes `y = y(x - shift)`. The shift is performed by
        changing the complex analytic Fourier Transform of the Gaussian,
        without resampling.
    ...
    if np.isscalar(sig_x):
        dx = np.diff(x)
        assert np.all(dx[0] == dx), "`x` must be uniformly spaced, when `sig_x` is a scalar"
        n = len(x)
