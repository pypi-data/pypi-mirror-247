...
def annotate_vcf_with_table(record: pysam.libcbcf.VariantRecord, vcf: pysam.VariantFile, frequency_table: pd.DataFrame):    
    ...
    if record.info.get("ISEQ_MANE_ANN"):
        protein_change = record.info.get("ISEQ_MANE_ANN")[0].split("|")[10]
    else:
        protein_change = record.info.get("ISEQ_REPORT_ANN")[0].split("|")[10]
    df = frequency_table[(frequency_table['GNOMAD_ID'] == gnomad) & (frequency_table['Protein_change_abbreviation'] == protein_change)]
    if not df.empty:
...
def main():
    ...
    parser.add_argument('--studies', type=str, required=False, nargs='+', help='StudyIds')
    ...
    studies = [x for x in args.studies if x != ''] if args.studies else []
