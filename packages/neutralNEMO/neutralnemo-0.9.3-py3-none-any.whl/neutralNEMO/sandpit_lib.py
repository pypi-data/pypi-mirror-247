...
from neutralNEMO import grid, surf
import os
...
hgd = grid.load_hgriddata( mm_path )

neutral_grid = grid.build_nemo_hgrid( hgd, iperio=False, jperio = False)

zgd = grid.load_zgriddata( mm_path)

tsd = surf.load_tsdata( t_path, zgd, to_varname="votemper", so_varname="vosaline")

surf_dataset = surf.find_omega_surfs( tsd, neutral_grid , zgd, [200., 3000.], [14,10], [6,9], [-1,-1], eos="gsw",
                                      ITER_MAX=10, calc_veronis=True, calc_potsurf=True)
