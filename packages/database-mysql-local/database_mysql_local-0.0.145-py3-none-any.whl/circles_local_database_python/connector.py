...
load_dotenv()
from .cursor import (DATABASE_MYSQL_PYTHON_PACKAGE_COMPONENT_ID,  # noqa E402, E501
...
class Connector:
    ...
    def connect(schema_name: str):
        ...
        # TODO What shall we do if schema_name in connections_pool and
        # connections_pool[schema_name].connection.is_NOT_connected():, I think we shall reconnect and not create a new entry.
        if schema_name in connections_pool and connections_pool[schema_name].connection.is_connected():
    ...
    def __init__(self, schema_name, host=None, user=None, password=None) -> None: # noqa E501
        if not all([os.getenv("RDS_HOSTNAME"), os.getenv("RDS_USERNAME"), os.getenv("RDS_PASSWORD")]): # noqa E501
            logger.error(
                "Error: Add RDS_HOSTNAME, RDS_USERNAME and RDS_PASSWORD to .env") # noqa E501
        ...
        if not (self.host.endswith("circ.zone") or self.host.endswith("circlez.ai")): # noqa E501
            logger.error(
                f"Warning: Your RDS_HOSTNAME={self.host} which is not what is expected") # noqa E501
    ...
    def set_schema(self, new_schema) -> None:
        ...
        if self.connection and self.connection.is_connected():
            ...
            logger.error(
                "Connection is not established. The database will be used on the next connect.") # noqa E501
        logger.end()
