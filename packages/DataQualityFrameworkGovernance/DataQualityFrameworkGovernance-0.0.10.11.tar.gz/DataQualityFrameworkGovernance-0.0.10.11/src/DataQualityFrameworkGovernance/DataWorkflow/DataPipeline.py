...
def processing_framework(json_config_file, output_csv):
    #clearing csv file
    df_empty = pd.DataFrame(columns=pd.read_csv(output_csv, nrows=0).columns)
            ...
            if selected_function is not None and callable(selected_function):
                ...
                for parameters in parameter_sets:
                    if parameters.get('enabled', True):
                        ...
                        #flag to check calculate value No and variable declarations to null values
                        flagParam = 0 
                        param_pairs = []
                        df_namelist = [] #df name list empty
                        df_pathlist = [] # df path list empty
                        df_namelist_str = None
                        df_pathlist_str = None
                        param_all_for_description = None
                        ...
                        for param_key, param_value in filtered_params.items():
                            ...
                            if param_key == "calculate" and param_value == "No":
                                ...
                                previous_data = pd.read_csv(output_csv, encoding='utf-8')
                                #appending with previous data
                                if not previous_data.empty:
                                    resError = pd.concat([previous_data, resError], ignore_index=True)
                                
                                resError.to_csv(output_csv, index=False)
                                print(f"{datetime.now()} : [ERROR] Function call with error, saved it in {output_csv}")
                        ...
                        if flagParam == 0:
                            ...
                            if output_csv:
                                if flagParam == 0:
                                    ...
                                    #appending with previous data
                                    if not previous_data.empty:
                                        result = pd.concat([previous_data, result], ignore_index=True)

                                    result.to_csv(output_csv, index=False)
                                    print(f"{datetime.now()} : [SUCCESS] Function call completed for {selected_function} and saved it in {output_csv}")
                    else:
                        print(f"{datetime.now()} : Parameter set is disabled.")
            else:
                print(f"{datetime.now()} : Function '{function_path}' not found or not callable.")
        else:
            print(f"{datetime.now()} : Function is disabled.")

#if __name__ == "__main__":
#    json_config_file = '/Users/rajithprabhakaran/Library/Mobile Documents/com~apple~CloudDocs/Documents/Python/Python Package/src/DataQualityFrameworkGovernance/DataWorkflow/dq_pipeline_config.json'
#    output_csv = '/Users/rajithprabhakaran/Library/Mobile Documents/com~apple~CloudDocs/Documents/Python/Output.csv'
#    processing_framework(json_config_file, output_csv)
