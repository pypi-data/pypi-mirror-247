...
#from .utilities import parameter_setting, get_cell_gene_neighbors, get_cell_positive_pairs
...
stKeep.get_cell_positive_pairs(adata2, args)
	

print('5---calculate cell encoding')
stKeep.RNA_encoding_train(args, adata2 )
	

print('6---save cell-gene neighborhoods')
stKeep.get_cell_gene_neighbors(adata2, args)


print('7---extracting histologicald features from H&E images')
stKeep.tiling(adata2, args.tillingPath, target_size = args.image_size)
if args.Hismodel == 'SimCLR':
	stKeep.train_simCLR_sImage( args, args.outPath )
	stKeep.extract_representation_simCLR_model( args, adata2 ) 
else:
	stKeep.extract_representation_resnet50_model(args, adata2)
