...
def summarize_public_Huggingface_textsplitter(self, text: str, verbose_option: int) -> str:
    ...
    while (not(Response_IsOK))and(WhileCounter<self.MaxCallRepeat):
        ...
        try:
            ...
            summary_ids = self.huggingface_model.generate(inputs["input_ids"], max_length=self.MaxSummaryLength*2,length_penalty=2.0, num_beams=4, early_stopping=True, do_sample=True, temperature=Actual_Temperature)
        ...
        if Response_IsOK:
            ...
            if (not(isnotcapped))and(length_ratio>0.75):

                # Then, the returned summary does not end with a dot, while it is normal text and, actually, should end in a dot:
                Response_IsOK = False
                
                if (verbose_option>=0):
                    print("Huggingface returned a summary that contained capped text (not ending in a dot)")
                ...
                if (verbose_option>=0):
                    if isinstance(self.logfile, io.TextIOBase):
                        if self.logfile.writable():
                            self.logfile.write("Huggingface Call " + str(self.callcounter) + " returned a summary that did not end with a dot. As such, the text is probably capped and we will re-try.\n")
