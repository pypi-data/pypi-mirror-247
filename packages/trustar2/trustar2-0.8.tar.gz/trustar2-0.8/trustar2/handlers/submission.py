...
class Submission(SearchHandler):
    ...
    def __repr__(self):
        sub_title = self.payload_params.get(SubmissionEnum.TITLE.value)
        sub_external_id = self.payload_params.get(SubmissionEnum.EXTERNAL_ID.value)

        return f"{typename(self)}(title={sub_title}, external_id={sub_external_id})"
