...
class Observables(SearchHandler):
    ...
    def search(self):
        ...
        url = f"{self.base_url}{self._search_endpoint}"
    ...
    def get_from_submission(self, submission_id, id_type = ID_Types.INTERNAL.value):
        ...
        :returns: Query Object.
        """
        url = f"{self.base_url}{self._get_from_submission_endpoint}"
    ...
    def _validate_tags_length(self):
        ...
        if are_too_may_tags:
            raise AttributeError(f"Tags are limited to {MAX_TAGS} per observable")
