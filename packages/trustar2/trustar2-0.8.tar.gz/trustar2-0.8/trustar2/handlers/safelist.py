...
class Safelist(BaseHandler):
    ...
    def _verify_entry(self, entry):
        ...
        if entity_type not in obs_types:
            raise AttributeError(f"Entity type should be one of the following: {obs_types}")
    ...
    def get_safelist_libraries(self):
        ...
        """
        result = Query(self.config,
                       self.summaries_endpoint,
                       Methods.GET).set_params(self.payload_params).execute()
        data = result.json()
    ...
    def create_entries(self):
        ...
        result = Query(self.config,
                       self.details_endpoint,
                       Methods.PATCH).set_params(self.payload_params).execute()
        data = result.json()
        if result.status_code == STATUS_OK:
            data = SafelistLibrary.from_dict(data)
    ...
    def create_safelist(self):
        ...
        result = Query(self.config,
                       self.summaries_endpoint,
                       Methods.POST).set_params(self.payload_params).execute()
        data = result.json()
        if result.status_code == STATUS_OK:
            data = SafelistLibrary.from_dict(data)
    ...
    def delete_entry(self, entry_guid):
        ...
        result = Query(self.config,
                       endpoint,
                       Methods.DELETE).set_params(self.payload_params).execute()
        return TruStarResponse(status_code=result.status_code,
                               data="OK" if result.status_code < 300 else "ERROR")
