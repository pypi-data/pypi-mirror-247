...
class PrioritizedIndicator(Base):
    ...
    def __init__(self, guid, enclave_guid, workflow_guid, observable, priority_score,
                 attributes, user_tags, submission_tags, score_contexts, created, updated,
                 processed_at, safelisted):
    ...
    def __repr__(self):
        return f"{typename(self)}(type={self.observable.type}, value={self.observable.value})"


    @classmethod
    def from_dict(cls, ioc_dict):
        observable = ioc_dict.get(OBSERVABLE)
        observable_obj = Entity.observable(observable.get(TYPE), observable.get(VALUE))
        ...
        return cls(
            ...
            safelisted=ioc_dict.get(IndicatorEnum.SAFELISTED.value, False)
        )
