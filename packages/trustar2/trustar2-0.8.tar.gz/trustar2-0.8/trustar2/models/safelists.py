...
class SafelistLibrary(Base):
    ...
    def __repr__(self):
        return f"{typename(self)}(name={self.name}, guid={self.guid})"
    ...
    def from_dict(cls, safelist_dict): 
        ...
        return cls(
            ...
            entries=[SafelistEntry.from_dict(e) for e in entries] if entries else [],
        )
...
class SafelistEntry(Base):
    ...
    def __repr__(self):
        return f"{typename(self)}(entity={self.entity}, type={self.type})"
