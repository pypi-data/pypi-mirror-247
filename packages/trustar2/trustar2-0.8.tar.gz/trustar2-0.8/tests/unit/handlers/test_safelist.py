...
SAFELIST_SUMMARIES_URL = TRUSTAR_API + "/safelist-libraries"
SAFELIST_LIBRARY = "test-library-guid-1"
SAFELIST_DETAILS_URL = SAFELIST_SUMMARIES_URL + "/" + SAFELIST_LIBRARY
...
def test_get_safelist_summaries(mocked_request, safelist, safelist_summaries_json):
    mocked_request.get(SAFELIST_SUMMARIES_URL, status_code=200, json=safelist_summaries_json)
    response = safelist.get_safelist_libraries()
    current_safelists = [s.serialize() for s in response.data]
    assert current_safelists == safelist_summaries_json
    assert len(current_safelists) == len(safelist_summaries_json)


def test_get_safelist_details_successfully(mocked_request, safelist, safelist_details_json):
    mocked_request.get(SAFELIST_DETAILS_URL, status_code=200, json=safelist_details_json)
    response = safelist.set_library_guid(SAFELIST_LIBRARY).get_safelist_details()
    current_safelist = response.data.serialize()
    all_required_values_are_equal = all(current_safelist.get(k) == safelist_details_json.get(k)
...
def test_create_entries_successfully(mocked_request, safelist, safelist_details_json):
    mocked_request.patch(SAFELIST_DETAILS_URL, status_code=200, json=safelist_details_json)
    response = safelist \
...
def test_create_safelist_library_successfully(mocked_request, safelist, safelist_summaries_json):
    mocked_request.post(SAFELIST_SUMMARIES_URL, status_code=200, json=safelist_summaries_json[0])
    response = safelist.set_library_name("test-library-name-1").create_safelist()
    current_safelist = response.data.serialize()
    all_required_values_are_equal = all(current_safelist.get(k) == safelist_summaries_json[0].get(k)
                                            for k in current_safelist.keys())
...
def test_delete_entry_successfully(mocked_request, safelist):
    ...
    url = SAFELIST_DETAILS_URL + "/" + entry_to_be_deleted
...
def test_delete_safelist_library_successfully(mocked_request, safelist):
    mocked_request.delete(SAFELIST_DETAILS_URL, status_code=200)
