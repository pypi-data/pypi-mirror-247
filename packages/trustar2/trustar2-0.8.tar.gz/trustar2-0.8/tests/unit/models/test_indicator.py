...
def test_indicator_deserialization(indicator_json):
    json_indicator = Indicator.from_dict(indicator_json)
    assert json_indicator.observable.type == URL
    assert json_indicator.observable.value == "verybadurl"
    assert json_indicator.confidence_score == LOW_CONFIDENCE
    assert json_indicator.malicious_score == BENIGN_SCORE
    assert json_indicator.properties == {"propertyKey": "propertyValue"}
    assert json_indicator.valid_from == FROM_TIMESTAMP
    assert json_indicator.valid_to == TO_TIMESTAMP

    assert json_indicator.attributes[0].type == THREAT_ACTOR
    assert json_indicator.attributes[0].value == "ActorName"
    assert json_indicator.attributes[0].valid_from == FROM_TIMESTAMP
    assert json_indicator.attributes[0].valid_to == TO_TIMESTAMP
    assert json_indicator.attributes[0].confidence_score == LOW_CONFIDENCE

    assert json_indicator.attributes[1].type == MALWARE
    assert json_indicator.attributes[1].value == "MalwareName"
    assert json_indicator.attributes[1].valid_from == FROM_TIMESTAMP
    assert json_indicator.attributes[1].valid_to == TO_TIMESTAMP
    assert json_indicator.attributes[1].confidence_score == MEDIUM_CONFIDENCE

    assert json_indicator.related_observables[0].type == IP4
    assert json_indicator.related_observables[0].value == "2.2.2.2"
    assert json_indicator.related_observables[0].valid_from == FROM_TIMESTAMP
    assert json_indicator.related_observables[0].valid_to == TO_TIMESTAMP
    assert json_indicator.related_observables[0].confidence_score == LOW_CONFIDENCE

    assert json_indicator.related_observables[1].type == URL
    assert json_indicator.related_observables[1].value == "wwww.relatedUrl.com"
    assert json_indicator.related_observables[1].valid_from == FROM_TIMESTAMP
    assert json_indicator.related_observables[1].valid_to == TO_TIMESTAMP
    assert json_indicator.related_observables[1].confidence_score == HIGH_CONFIDENCE

    assert json_indicator.tags == ["importantTag", "anotherTag"]
