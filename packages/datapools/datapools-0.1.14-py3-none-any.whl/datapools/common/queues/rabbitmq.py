...
class RestAPI:
    def __init__(self, connection_url):
        ...
        self.url = f"http://{p.username}:{p.password}@{p.hostname}:15672/api/"  # TODO: port should be configurable
    ...
    async def get_queue(self, queue_name):
        # TODO: rest api port should be configurable
                ...
                if self.role == QueueRole.Receiver:
                    ...
                    if "messages" in queue:
                        if time.time() - last_log > 5:
                            last_log = time.time()
                            logger.info(
                                f'=================== receiver queue size {self.queue_name} {queue["messages"]} {queue["messages_unacknowledged"]=}'
                            )
                        if (
                            queue["messages"] == 0
            ...
            while not await self.is_stopped():
                ...
                try:
                    ...
                    if (
                        ...
                        for rk in list(self.params.routing_key):
