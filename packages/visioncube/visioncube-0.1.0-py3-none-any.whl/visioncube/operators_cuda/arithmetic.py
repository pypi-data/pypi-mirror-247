...
class Add(AbstractTransform):
    ...
    def _apply(self, sample):
        ...
        sample.image = torch.clip(sample.image + value, 0, 255)
...
class Multiply(AbstractTransform):
    ...
    def _apply(self, sample):
        ...
        sample.image = torch.clip(sample.image * mul, 0, 255)
...
class Solarize(AbstractTransform):
    ...
    def __init__(self, threshold: Union[float, Sequence] = 255.0) -> None:
        """Solarize, 反转变换, 算术变换
    ...
    def _apply(self, sample):
        ...
        sample.image = TF.solarize(sample.image.int(), threshold=threshold).float()
        return sample
...
class AdditiveGaussianNoise(AbstractTransform):
    ...
    def _apply(self, sample):
        ...
        sample.image = torch.clip(noisy_img, min=0, max=255)
