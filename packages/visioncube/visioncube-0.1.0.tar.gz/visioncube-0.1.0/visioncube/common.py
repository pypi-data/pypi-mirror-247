...
class AbstractTransform(object):
    ...
    def __init__(self, use_gpu=False):
        self.sample_module = "visioncube.operators_cuda" if use_gpu else "visioncube.operators"

    def __call__(self, doc):

        convert = isinstance(doc, dict)
        if convert:
            Sample = getattr(import_module(self.sample_module), "Sample")
            sample = Sample(doc)
        else:
            sample = doc

        out = self._apply(sample)
        return out if not convert else sample.output()
...
class AbstractSample(object):
    ...
    def __init__(self, doc):
        ...
        self.bboxes_checked = False
        self.heatmap_checked = False
        self.keypoints_checked = False
