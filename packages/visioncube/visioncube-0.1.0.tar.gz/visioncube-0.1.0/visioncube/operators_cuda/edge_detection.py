...
class Laplacian(AbstractTransform):
    ...
    def _apply(self, sample):
        ...
        sample.image = torch.abs(image).clip_(min=0, max=255)
...
class Roberts(AbstractTransform):
    ...
    def _apply(self, sample):
        ...
        out_x = F.conv2d(gray_image, kernel_x)
        out_y = F.conv2d(gray_image, kernel_y)

        sample.image = torch.sqrt(out_x ** 2 + out_y ** 2)
