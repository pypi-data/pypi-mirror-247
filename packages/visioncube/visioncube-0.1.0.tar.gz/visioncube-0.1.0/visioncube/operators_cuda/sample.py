...
from visioncube.functional_cuda.imageio import hwc_to_chw, chw_to_hwc
...
class Sample(AbstractSample):
    ...
    def _handle_image(self, image):
        ...
        image = hwc_to_chw(image).to(torch.float32)
    ...
    def _handle_heatmap(self, heatmap):
        ...
        heatmap = hwc_to_chw(heatmap).to(torch.float32)
    ...
    def _out_image(self):
        return chw_to_hwc(self.image).to(torch.uint8).cpu().numpy()
    ...
    def _out_heatmap(self):
        return chw_to_hwc(self.heatmap).to(torch.uint8).cpu().numpy()
