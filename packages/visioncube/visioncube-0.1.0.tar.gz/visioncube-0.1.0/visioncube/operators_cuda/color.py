...
class Brightness(AbstractTransform):
    ...
    def __init__(self, factor: float = 1.0) -> None:
        ...
        Args:
            factor: Brightness factor, 亮度系数, [0.0, 2.0, 0.1], 1.0
        """
        super().__init__(use_gpu=True)
    ...
    def _apply(self, sample):
        ...
        image = sample.image.int()
        sample.image = TF.adjust_brightness(image, brightness_factor=self.factor).float()
...
class Saturation(AbstractTransform):
    ...
    def __init__(self, factor: float = 1.0) -> None:
        ...
        Args:
            factor: Saturation factor, 饱和度系数, [0.0, 2.0, 0.1], 1.0
        """
        super().__init__(use_gpu=True)
        self.factor = eval_arg(factor, None)
    ...
    def _apply(self, sample):
        ...
        image = sample.image.int()
        sample.image = TF.adjust_saturation(image, saturation_factor=self.factor).float()
...
class Contrast(AbstractTransform):
    ...
    def __init__(self, factor: float = 1.0) -> None:
        ...
        Args:
            factor: Contrast factor, 对比度系数, [0.0, 2.0, 0.1], 1.0
        """
        super().__init__(use_gpu=True)
        self.factor = eval_arg(factor, None)
    ...
    def _apply(self, sample):
        ...
        image = sample.image.int()
        sample.image = TF.adjust_contrast(image, contrast_factor=self.factor).float()
...
class Sharpness(AbstractTransform):
    ...
    def _apply(self, sample):
        ...
        image = sample.image.int()
        sample.image = TF.adjust_sharpness(image, sharpness_factor=self.factor).float()
...
class Gamma(AbstractTransform):
    ...
    def _apply(self, sample):
        ...
        sample.image = (255 * gain * (sample.image / 255) ** gamma).clamp(0, 255)
...
class Equalize(AbstractTransform):
    ...
    def _apply(self, sample):
        ...
        sample.image = TF.equalize(image).float()
...
class Posterize(AbstractTransform):
    ...
    def _apply(self, sample):
        ...
        image = sample.image.type(torch.uint8)
        sample.image = TF.posterize(image, bits=self.bits).float()
...
class CLAHE(AbstractTransform):
    ...
    def _apply(self, sample):
        ...
        sample.image = K.enhance.equalize_clahe(
...
class RandomBrightness(AbstractTransform):
    ...
    def _apply(self, sample):
        ...
        image = sample.image.int()
        sample.image = self.operator(image).float()
...
class RandomHue(AbstractTransform):
    ...
    def _apply(self, sample):
        ...
        image = sample.image.to(torch.uint8)
        sample.image = self.operator(image).float()
