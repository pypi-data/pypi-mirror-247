...
class CallingConvention(IntEnum):
    ...
    def __str__(self):
        if self is CallingConvention.Default:
            return "Default"
        if self is CallingConvention.ParameterStruct:
...
class CommandBufferModeBitfield(IntFlag):
    ...
    def __str__(self):
        ...
        if self is CommandBufferModeBitfield.OneShot:
            return "OneShot"
        if self is CommandBufferModeBitfield.Nested:
            return "Nested"
...
class DescriptorSetLayoutFlags(IntFlag):
    ...
    def __str__(self):
        if len(self) > 1:
            return "|".join(map(str, self))
        if self is DescriptorSetLayoutFlags.None_:
            return "None"
        if self is DescriptorSetLayoutFlags.Indirect:
            return "Indirect"
        raise ValueError("Unknown DescriptorSetLayoutFlags enum entry.")



@register_attribute_builder("HAL_DescriptorSetLayoutFlagsAttr")
def _hal_descriptorsetlayoutflagsattr(x, context):
    return _ods_ir.IntegerAttr.get(_ods_ir.IntegerType.get_signless(32, context=context), int(x))
...
@register_attribute_builder("anonymous_478")
def _anonymous_478(x, context):
    return _ods_ir.IntegerAttr.get(_ods_ir.IntegerType.get_signless(32, context=context), int(x))
