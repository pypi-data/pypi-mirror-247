...
class Network:
    ...
    def request(self, method:str, input:dict={}, tmpSession:bool=False, count_try:int = 0, max_try:int=2):
        ...
        if platform in ["rubx", "rubikax"]:
            ...
            if apiVersion < 6:
                client["app_version"] = "3.0.4"
        
        elif platform in ["android"]:
            client:dict = Configs.clients["android"]
            if apiVersion < 6:
                client["package"] = "ir.resaneh1.iptv"
                client["app_version"] = "3.0.9"
    ...
    def download(self, accessHashRec:str, fileId:str, dcId:str, size:int, fileName:str, chunkSize:int=262143):
        ...
        while True:
            lastIndex = int(header["start-index"]) + chunkSize if int(header["start-index"]) + chunkSize < size else size - 1
