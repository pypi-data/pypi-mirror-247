...
class Methods:
    ...
    def __init__(self, sessionData:dict, platform:str, apiVersion:int, proxy:str, timeOut:int, showProgressBar:bool) -> None:
        ...
        ) if sessionData else Cryption(auth=Tools.randomTmpSession())
    ...
    def baseSendFileInline(self, objectGuid:str, file:str, text:str, messageId:str, fileName:str, type:dict, time:int=None, performer:str=None) -> dict:
        ...
        if type in ["Music", "Voice"]:
            input["file_inline"]["time"] = (time or Tools.getVoiceDuration(uploadData["file"])) * (1000 if type == "Voice" else 1)

            if type == "Music":
                input["file_inline"]["music_performer"] = performer or Tools.getMusicArtist(uploadData["file"])
