...
def git_push(repo, project=None):
    if repo.head.commit.diff("origin/" + repo.active_branch.name):
        ...
        time.sleep(10)
        if project:
            check_target_pipeline_status(project)
        else:
...
def git_push_folder(repo_name="", server="", branch=None):
    if not DEFAULT_GIT_DIR.is_dir():
        log.info("Nothing to push")
        return True
    git.repo = git.Repo.init(DEFAULT_GIT_DIR)
    if branch:
        git_checkout(branch)
    return git_push(git.repo, repo_name, server)
