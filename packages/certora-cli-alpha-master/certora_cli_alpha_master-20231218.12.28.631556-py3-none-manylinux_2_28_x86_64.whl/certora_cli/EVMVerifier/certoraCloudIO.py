...
class CloudVerification:
    ...
    def print_output_links(self) -> None:
        ...
        if Ctx.is_supported_server(self.context):
            ...
            print(f"{prefix} {Util.SupportedServers.PRODUCTION.value}")
        print(f"Once the job is completed, the results will be available at {self.reportUrl}", flush=True)
    ...
    def __compress_and_upload_zip_files(self) -> bool:
        ...
        if self.context.mode == Util.Mode.TAC:
        ...
        elif self.context.mode == Util.Mode.SOLANA:
            ...
            for arg in jar_args:
                ...
                if arg == '-solanaInlining':
                    ...
                    is_file = True
            result = compress_files(self.ZipFilePath, *paths, short_output=self.context.short_output)
        else:
    ...
    def cli_verify_and_report(self, cl_args: str, wait_for_results: str) -> bool:
        ...
        if wait_for_results == str(Vf.WaitForResultOptions.NONE):
            job_list.save_data()
