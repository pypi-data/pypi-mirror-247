...
class CompilerLangVy(CompilerLang, metaclass=Singleton):
    ...
    def collect_storage_layout_info(file_abs_path: str,
            ...
            with Path(storage_layout_stderr_name).open('w+') as stderr:
                try:
                    ...
                    if not Path(storage_layout_output_file_name).exists():
                        ast_logger.warn(f"Could not find storage layout file {storage_layout_output_file_name}, giving up on storage layout info")
                        return data
                    with Path(storage_layout_output_file_name).open('r') as output_file:
                        ...
                        # and also another piece of nonsense. from 0.2.16 until 0.3.6, we didn't have the 'storage_layout' key.
        ...
        # vyper 0.3.0: "userBalances": {"type": "HashMap[address, uint256][address, uint256]", "location": "storage", "slot": 2}
    ...
    def collect_source_type_descriptions_and_funcs(asts: Dict[str, Dict[str, Dict[int, Any]]],
                                                   ...
                                                   build_arg_contract_file: str) -> \
        Tuple[List[CT.Type], List[CompilerLangFunc]]:
        # legacy version did the verification against data['abi']. so we're going to run it too and use it if it contains
        # more functions
        ...
        try:
            ...
            if legacy_type_descriptions_and_funcs is not None and \
                ...
                ast_logger.info(
                    "Legacy collector returned more functions. Using legacy results instead. (You may use an older vyper version)")
