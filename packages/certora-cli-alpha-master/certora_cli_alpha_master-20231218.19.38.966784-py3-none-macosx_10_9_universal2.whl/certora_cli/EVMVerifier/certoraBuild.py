...
import EVMVerifier.certoraMutateGenerator as MutGen
...
class CertoraBuildGenerator:
    ...
    def check_for_errors_and_warnings(self, data: Dict[str, Any]) -> None:
        if "errors" in data:
            ...
            if len(severe_errors) > 0:
                for i, e in enumerate(severe_errors):
                    ...
                    if self.context.contract_compiler_skip_severe_warning_as_error:
    ...
    def collect_for_file(self,
        ...
        if not is_vyper:
            ...
            main_path = route_func(self.context.solc_allow_path)
            ...
            if self.context.packages is not None:
                ...
                self.context.remappings = list(map(lambda remap: f'{remap[0]}={remap[1]}', rerouted_remapping_pairs))
        ...
        if self.context.generate_mutation_conf:
            MutGen.generate_mutation_conf(self.context, build_arg_contract_file, compiler_ver_to_run)
        ...
        for c in sdc.contracts:
            for f in c.internal_funcs.union(c.public_funcs):
                ...
                if len(mods) > 0:
                    ...
                    should_compress = (self.context.function_finder_mode is None or
                                       self.context.function_finder_mode == Vf.FunctionFinderMode.DEFAULT.name)
    ...
    def resolve_slot(self, primary_contract: str, slot_name: str) -> str:
        ...
        asm_collect_cmd = f'{solc_ver_to_run} -o {Util.get_certora_config_dir()}/ --overwrite --asm ' \
                          f'--allow-paths "{self.context.solc_allow_path}" "{Util.abs_posix_path(file)}"'
        if self.context.packages is not None:
            asm_collect_cmd = f"{asm_collect_cmd} {' '.join(self.context.packages)}"

        Util.run_compiler_cmd(asm_collect_cmd, f"{primary_contract}.asm",
                              Path(os.getcwd()))
...
class CertoraVerifyGenerator:
    def __init__(self, build_generator: CertoraBuildGenerator, context: CertoraContext):
        self.context = context
...
def build(context: CertoraContext, ignore_spec_syntax_check: bool = False) -> None:
    ...
    try:
        ...
        certora_build_generator.certora_verify_generator = CertoraVerifyGenerator(certora_build_generator, context)
