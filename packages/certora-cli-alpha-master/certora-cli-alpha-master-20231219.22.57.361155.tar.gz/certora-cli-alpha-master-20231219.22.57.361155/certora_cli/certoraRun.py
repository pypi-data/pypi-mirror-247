        ...
        if context.is_solana:
            if context.local:
                ...
                run_result = Util.run_jar_cmd(check_cmd, compare_with_tool_output, logger_topic="verification",
                                              print_output=True)
                # For solana, we don't check types so build time is zero.
            ...
            if not context.local and not context.build_only:
                """
            ...
            if not (context.build_only or context.generate_mutation_conf) and exit_code == 0:
