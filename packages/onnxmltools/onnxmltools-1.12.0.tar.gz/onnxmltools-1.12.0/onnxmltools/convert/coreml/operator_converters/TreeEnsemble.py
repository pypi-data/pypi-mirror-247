...
def get_onnx_tree_mode(cm_tree_behavior):
    ...
    raise ValueError(
        "CoreML tree node behavior not supported {0}".format(cm_tree_behavior)
    )
...
def get_onnx_tree_post_transform(cm_tree_post_transform):
    if cm_tree_post_transform in COREML_TREE_POST_TRANSFORM_TO_ONNX_TREE_POST_TRANSFORM:
        return COREML_TREE_POST_TRANSFORM_TO_ONNX_TREE_POST_TRANSFORM[
            cm_tree_post_transform
        ]
    raise ValueError(
        "CoreML tree post transform not supported {0}".format(cm_tree_post_transform)
    )
...
def convert_tree_ensemble_model(scope, operator, container):
    ...
    if raw_model.WhichOneof("Type") == "treeEnsembleClassifier":
        ...
        if (
            raw_model.treeEnsembleClassifier.WhichOneof("ClassLabels")
            == "int64ClassLabels"
        ):
            class_labels = list(
                int(i) for i in raw_model.treeEnsembleClassifier.int64ClassLabels.vector
            )
            ...
            class_labels = list(
                s.encode("utf-8")
                for s in raw_model.treeEnsembleClassifier.stringClassLabels.vector
            )
    ...
    leaf_treeids = [
        node.treeId
        for node in nodes
        if 6 == node.nodeBehavior
        for _ in node.evaluationInfo
    ]
    leaf_nodeids = [
        node.nodeId
        for node in nodes
    ...
    if raw_model.WhichOneof("Type") == "treeEnsembleRegressor":
        ...
        if len(operator.inputs) > 1:
            ...
            container.add_node(
                ...
                inputdimensions=[
                    variable.type.shape[1] for variable in operator.inputs
                ],
            )
            container.add_node(
                op_type,
        ...
        for variable in operator.outputs:
            ...
            if (
                raw_model.description.predictedProbabilitiesName != ""
                and raw_model.description.predictedProbabilitiesName
                == variable.raw_name
            ):
