...
from xmind2testcasezen import const
from xmind2testcasezen.metadata import TestCase, TestStep, TestSuite
from xmind2testcasezen.parser_xmind import ParseXmind
...
class ParseOld(ParseXmind):
    ...
    def parse_testsuites(self, xmind_content_dict):
        """convert xmind file to `xmind2testcasezen.metadata.TestSuite` list"""
        suites = []
        ...
        for sheet in xmind_content_dict:
            ...
            if sub_topics:
                ...
                logging.warning(
                    "This is a blank sheet(%s), should have at least 1 sub topic(test suite)", sheet["title"]
                )
                continue
    ...
    def filter_empty_or_ignore_topic(self, topics):
        ...
        result = [
            ...
            if not (
                topic["title"] is None
                or topic["title"].strip() == ""
                or topic["title"][0] in const.config["ignore_char"]
            )
        ]
    ...
    def sheet_to_suite(self, root_topic):
        ...
        suite.label = root_topic["label"]
        suite.sub_suites = []

        for suite_dict in root_topic["topics"]:
            # suite_dict["label"] = suite_dict["label"] or suite.label
            suite_dict["label"] = self.merge_test_value(suite.label, suite_dict["label"])
    ...
    def parse_testsuite(self, suite_dict):
        ...
        testsuite.label = suite_dict["label"]
        testsuite.testcase_list = []
        logging.debug("start to parse a testsuite: %s", testsuite.name)

        for cases_dict in suite_dict.get("topics", []):
            # cases_dict["label"] = cases_dict["label"] or testsuite.label
            cases_dict["label"] = self.merge_test_value(testsuite.label, cases_dict["label"])
    ...
    def parse_a_testcase(self, case_dict, parent):
        ...
        testcase.label = self.merge_test_value(self.get_test_value(topics, "label"))
    ...
    def gen_testcase_title(self, topics, separator=None):
    ...
    def parse_test_steps(self, step_dict_list):
        steps = []
        step_dict_list = self.gen_test_steps(step_dict_list)

        for step_num, step_list in enumerate(step_dict_list, 1):
            step_dict = step_list[0]
            step_dict.update({"title": self.gen_testcase_title(step_list, " > "), "topics": step_list[-1]["topics"]})
    ...
    def get_test_value(self, datas, key):
        # return list_get([data[key] for data in datas if data[key]], -1, [])
        return list(funcy.flatten([data[key] for data in datas if data[key]]))

    def merge_test_value(self, *datas):
        result_dict = OrderedDict()
        for item in funcy.flatten([i or [] for i in datas]):
            key, value = item.replace("：", ":").split(":", 1)
            result_dict[key] = value
        return [f"{key}：{value}" for key, value in result_dict.items()]

    def gen_test_steps(self, node_list, label=None):
        def find_leaf_nodes(node_list, label=None):
            if not node_list:
                return

            for node in node_list:
                if "topics" in node and node["topics"]:
                    # 递归遍历子节点，并将子节点的结果作为新的分支
                    for branch in find_leaf_nodes(node["topics"], label):
                        yield [deepcopy(node)] + branch
                else:
                    # 当前节点是末节点，返回包含源数据的分支
                    yield [deepcopy(node)]

        result = []
        for step_list in [i[:-1] if len(i) > 1 else i for i in find_leaf_nodes(node_list)]:
            if step_list not in result:
