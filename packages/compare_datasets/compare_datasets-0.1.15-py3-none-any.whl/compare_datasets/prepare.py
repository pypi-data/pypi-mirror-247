...
class PrepareForComparison:
    ...
    def testCounts(self):
        ...
        self.report[
            "count_report"
        ] = f"COUNT COMPARISON: {stringify_result(self.result['count_result']['result'])}\n{tabulate(row_and_column_counts, headers='keys', tablefmt='psql')}"
    ...
    def testColumnNames(self):
        ...
        self.report[
            "column_names_report"
        ] = f"COLUMNS COMPARISON: {stringify_result(self.result['column_names_result'])}\n{tabulate(self.column_comparison, headers='keys', tablefmt='psql')}"
    ...
    def __verify_keys__(self):
        ...
        if not is_tested_valid:
            raise KeyError(f"One or more key column(s) provided {self.key} do not exist in the tested dataframe.")
        if not is_expected_valid:
            raise KeyError(f"One or more key column(s) provided {self.key} do not exist in the expected dataframe.")
