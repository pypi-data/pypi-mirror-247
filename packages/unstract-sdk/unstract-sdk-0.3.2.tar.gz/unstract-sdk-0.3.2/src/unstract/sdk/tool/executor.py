...
class ToolExecutor:
    ...
    def validate_and_get_settings(self, args: argparse.Namespace) -> dict[str, Any]:
    ...
    def validate_and_get_params(self, args: argparse.Namespace) -> dict[str, Any]:
        ...
        if args.params is None:
            self.tool.stream_error_and_exit("--params are required for RUN command")
        try:
            ...
            self.tool.stream_error_and_exit(f"Params are not a valid JSON: {str(e)}")
        except ValidationError as e:
    ...
    def execute(self, args: argparse.Namespace) -> None:
        ...
        if command in Command.static_commands():
            ...
            self.tool.run(params, settings, args.workflow_id)
        else:
