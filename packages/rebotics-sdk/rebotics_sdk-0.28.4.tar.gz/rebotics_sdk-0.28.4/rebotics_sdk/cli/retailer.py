...
from ..constants import CVAT_EXPORT_WORKSPACE
...
@pass_rebotics_context
def export_to_cvat(ctx, processing_ids, image_quality, segment_size, workspace):
    if ctx.verbose:
    ...
    try:
        if ctx.verbose:
            click.echo('Sending request...')
        response = ctx.provider.export_to_cvat(processing_ids, image_quality, segment_size, workspace)
