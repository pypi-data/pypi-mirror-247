...
if TYPE_CHECKING:
    ...
    from app import APIDefinition, StereotechCloud
...
class WebRequest:
    ...
    def get_scopes(self) -> Optional[Dict[str, Any]]:
        return self.scopes
...
class WebsocketManager(APITransport, NotificationTransport):
    def __init__(self, server: Server) -> None:
        ...
        self.rpc.register_method(
            "server.websocket.id", self._handle_id_request)
    ...
    def _generate_local_callback(self,
        ...
        async def func(ws: WebSocket, **kwargs) -> Any:
            result = await callback(
                WebRequest(endpoint, kwargs, action=request_method, conn=ws,
    ...
    def remove_websocket(self, ws: WebSocket) -> None:
        ...
        if old_ws is not None:
            ...
            if not self.websockets:
                self.server.send_event('server:websocket_removed', {
                    'websocket': ws.uid})
    ...
    def notify_websockets(self,
        ...
        if data != SENTINEL:
            msg['params'] = [data]
        intersect = list(self.websockets.keys())
        if len(ws_list) > 0:
            intersect = list(set(self.websockets.keys()) & set(ws_list))
        for ws_key in intersect:
            ws = self.websockets[ws_key]
...
class WebSocket(WebSocketHandler, Subscribable):
    def initialize(self) -> None:
        app: StereotechCloud = self.settings['parent']
    ...
    def check_origin(self, origin: str) -> bool:
        if not super(WebSocket, self).check_origin(origin):
            auth: AuthComp = self.server.lookup_component(
                'authorization', None)
            if auth is not None:
