...
STEREOTECH_CLOUD_PATH = os.path.join(os.path.dirname(__file__), '..')
...
class StereotechCloudLoggingHandler(logging.handlers.TimedRotatingFileHandler):
    def __init__(self, app_args: Dict[str, Any], **kwargs) -> None:
        ...
        self.rollover_info: Dict[str, str] = {
            'header': f"{'-'*20}StereotechCloud Log Start{'-'*20}"
        }
...
def get_software_version() -> str:
    ...
    try:
        version = retreive_git_version(STEREOTECH_CLOUD_PATH)
    except Exception:
        vfile = pathlib.Path(os.path.join(
            STEREOTECH_CLOUD_PATH, "stereotech_cloud/.version"))
        if vfile.exists():
            try:
...
def setup_logging(app_args: Dict[str, Any]
                             ...
                             Optional[StereotechCloudLoggingHandler]]:
    ...
    if app_args.get('log_file', ""):
        file_hdlr = StereotechCloudLoggingHandler(
...
def verify_source(path: str = STEREOTECH_CLOUD_PATH) -> Optional[Tuple[str, bool]]:
...
async def check_scope_access(web_request: WebRequest) -> WebRequest:
        logging.info('Enter check scope middleware')
        if (
            web_request.current_user is not None
            and not isinstance(web_request.current_user, str)
            and not set(web_request.current_user.get('role', {}).get("scopes", []))
            & set(web_request.scopes[web_request.get_action().lower()])
            and len(web_request.scopes[web_request.get_action().lower()]) != 0
        ):
            raise ServerError("Access denied", 403)
        return web_request