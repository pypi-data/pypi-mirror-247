...
case "${prev}" in
    linode-cli | linode | lin)
        COMPREPLY=( $(compgen -W "account availability betas events payment-methods service-transfers users databases domains images linodes kernels stackscripts lke longview managed networking firewalls vlans nodebalancers object-storage profile security-questions sshkeys phone regions tickets tags volumes _spec --help" -- ${cur}) )
        return 0
        ;;
    account)
        ...
        ;;
        availability)
        COMPREPLY=( $(compgen -W "getAvailability getAccountAvailability --help" -- ${cur}) )
        return 0
        ;;
        betas)
        ...
        ;;
        nodebalancers)
        COMPREPLY=( $(compgen -W "list create view update delete configs-list config-create config-view config-update config-delete config-rebuild nodes-list node-create node-view node-update node-delete firewalls --help" -- ${cur}) )
        return 0
        ;;
        object-storage)
