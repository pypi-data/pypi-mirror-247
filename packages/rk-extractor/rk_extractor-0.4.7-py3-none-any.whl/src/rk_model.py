...
class rk_model(model):
    ...
    def _preffix_to_info(self, preffix):
        regex = '(ee|mm)_(r1|r2p1|2017|2018)_(TOS|TIS)_.*'
        ...
        year = {'r1' : '2012', 'r2p1' : '2016', '2017' : '2017', '2018' : '2018'}[dset]
        trig = {'mm_TOS' : 'MTOS', 'mm_TIS' : 'MTOS', 'ee_TOS' : 'ETOS', 'ee_TIS' : 'GTIS'} [f'{chan}_{trg}']
    ...
    def _get_entries_from_json(self, kind=None, trig=None, year=None):
        json_path = files('extractor_data').joinpath(f'{self._norm_vers}/{year}_{trig}.json')
    ...
    def _get_signal(self, preffix, nent=None):
        ...
        year = '2018'
        sig, d_cns = hqm_model.get_signal_shape(preffix=preffix, year=year, trigger=trig)
    ...
    def _get_rare(self, preffix, nent=None, kind=None):
        ...
        if   kind == 'bp':
            pdf = hqm_model.get_Bu2Ksee_shape(preffix=preffix, year=year, trigger=trig)
        elif kind == 'bd':
            pdf = hqm_model.get_Bd2Ksee_shape(preffix=preffix, year=year, trigger=trig)
        else:
            log.error(f'Invalid kind: {kind}')
            raise ValueError

        nrare= self._get_entries(nent, f'{kind}ks', trigger=trig, year=year)
        nbkg = zfit.Parameter(f'nr{kind}_{preffix}', nrare, 0, 100000)
        pdf.set_yield(nbkg)

        return pdf 
    #---------------------------------------------------------------
    def _get_prec(self, preffix, nent=None):
        year, trig = self._preffix_to_info(preffix)
        log.warning(f'Using 2018 PRec background for {year}')
        year = '2018'

        pdf = hqm_model.get_part_reco(preffix=preffix, year=year, trigger=trig)
    ...
    def _get_msid(self, preffix):
        dset, trig    = self._get_dset_trig(preffix)
        if trig == 'GTIS':
            log.warning(f'Missing mis-id component for: {preffix}')
            return

        bld= msid(name=preffix, version='v1', obs=self._obs)
        bld.load_model()
        bld.fix_mode = 'all'
        pdf=bld.build_model() 
    ...
    def _get_pdf(self, preffix='', nent=None):
        ...
        if   preffix.startswith('ee_'):
            erbp  = self._get_rare(preffix, nent=nent, kind='bp')
            erbd  = self._get_rare(preffix, nent=nent, kind='bd')
            eprc  = self._get_prec(preffix, nent=nent           )
            emis  = self._get_msid(preffix)

            l_pdf = [cbkg, erbp, erbd, eprc] if emis is None else [cbkg, erbp, erbd, eprc, emis]
        ...
        if not self._skip_signal:
            ...
            log.warning(f'Not adding signal to fit model')
