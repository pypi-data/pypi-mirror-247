...
class normalizer:
    ...
    def _initialize(self):
        ...
        zut.print_pdf(self._model, txt_path=f'{self._out_dir}/pdf/{self._dset}_{self._trig}.txt', d_const=self._d_const)
    ...
    def _get_mass(self, l_dpath):
        ...
        arr_mass_lo = arr_mass[(arr_mass < low[1])]
        arr_mass_hi = arr_mass[(arr_mass > hig[0])]

        arr_mass_cn = arr_mass[(arr_mass > low[1]) & (arr_mass < hig[0])]
        arr_mass_cn = numpy.random.uniform(low=low[1], high=hig[0], size=arr_mass_cn.shape)
    ...
    def out_dir(self, value):
        ...
        self._out_dir = value
    #--------------------------------------
    def _plot_fit(self, data=None, name=None, result=None, l_range=None):
        obj= zfp(data=data, model=self._model, result=result)
        obj.plot(nbins=self._nbins, ranges=l_range, stacked=False)
    ...
    def get_fit_result(self):
        ...
        res   = obj.fit(ntries=5, ranges=[low, hig], d_const=self._d_const)

        self._plot_fit(data=arr_mass, name=f'{self._trig}_{self._dset}', result=res, l_range=[low, hig])
