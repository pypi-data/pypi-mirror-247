...
class rk_model(model):
    ...
    def _fix_shape(self, pdf):
        s_par = pdf.get_params(is_yield=False)
        for par in s_par:
            par.floating=False
    #---------------------------------------------------------------
    def _get_combinatorial(self, preffix):
    ...
    def _get_rare(self, preffix, nent=None, kind=None):
        ...
        if   kind == 'bpks':
            pdf = hqm_model.get_Bu2Ksee_shape(name=r'$B^+\to K^{*0}ee$', year=year, trigger=trig)
        elif kind == 'bdks':
            pdf = hqm_model.get_Bd2Ksee_shape(name=r'$B_0\to K^{*+}ee$', year=year, trigger=trig)
        elif kind == 'bsph':
            pdf = hqm_model.get_Bd2Ksee_shape(name=r'$B_s\to \phi  ee$', year=year, trigger=trig)
        else:
            log.error(f'Invalid kind: {kind}')
            raise ValueError
    ...
    def _get_prec(self, preffix, nent=None):
        ...
        pdf = hqm_model.get_part_reco(name='$PRec + \psi(2S) K^+$', preffix=preffix, year=year, trigger=trig)
    ...
    def _get_msid(self, preffix):
        ...
        bld= msid(name='Mis-ID', version='v1', obs=self._obs)
        bld.load_model()
        bld.fix_mode = 'all'
        pdf=bld.build_model() 
    ...
    def _get_pdf(self, preffix='', nent=None):
        ...
        cbkg      = self._get_combinatorial(preffix)
        ...
        if   preffix.startswith('ee_'):
            ...
            l_pdf = [cbkg, erbp, erbd, erbs, eprc] if emis is None else [cbkg, erbp, erbd, erbs, eprc, emis]
        elif preffix.startswith('mm_'):
            l_pdf = [cbkg]
        else:
            log.error(f'Invalid preffix: {preffix}')
    ...
    def _plot_model(self, key, mod):
        ...
        obj.plot(nbins=50, ext_text=key, stacked=True)
