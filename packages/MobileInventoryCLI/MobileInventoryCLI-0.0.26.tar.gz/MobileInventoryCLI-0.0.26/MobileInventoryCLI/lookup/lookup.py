									...
									while True:
										try:
											...
											for field in data[itemNum].__table__.columns:
												...
												edit_field=input("Edit {field}[{fieldData}]({type}) [y/n/quit/back]: ".format(field=field.name,fieldData=d,type=field.type))
												while True:
													if edit_field.lower() in ['y','n','quit','back']:
														...
														edit_field=input("Edit {field}({type}) [y/n/quit/back]: ".format(field=d,type=field.type))

											customfields=session.query(tbl.ItemCustomField).filter(tbl.ItemCustomField.ItemId==data[itemNum].ItemId).all()
											if len(customfields) < 1:
												...
												for cf in customfields:
													...
													edit=input("{}({})={} [y/n/back/quit]: ".format(attrs.Name,index[attrs.Type],cf.Value))
													if edit == "y":
														...
														if new_value == "#quit#":
															...
															if attrs.Type == 1:
																...
																new_value=str(new_value)
															setattr(cf,field.name,new_value)
															session.commit()
															session.flush()
															session.refresh(cf)
