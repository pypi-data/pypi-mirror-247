...
from arraylake.chunkstore import Chunkstore, mk_chunkstore
...
class AsyncClient:
    ...
    def _init_chunkstore(self, repo_id: DBID, bucket: Optional[Bucket]) -> Chunkstore:
        ...
        if bucket is None:
            ...
            s3_uri = f"s3://{bucket.name}/{repo_id.hex()}/"  # the final / is critical
            client_kws = dict(bucket.extra_config)
            if bucket.endpoint_url is not None:
                client_kws["endpoint_url"] = bucket.endpoint_url

        return mk_chunkstore(s3_uri, inline_threshold_bytes=inline_threshold_bytes, **client_kws)
    ...
    async def get_or_create_repo(self, name: str, bucket_nickname: Optional[str] = None) -> AsyncRepo:
...
class Client:
    ...
    def get_or_create_repo(self, name: str, bucket_nickname: Optional[str] = None) -> Repo:
        """Get a repo by name. Create the repo if it doesn't already exist.

        Args:
            name: Full name of the repo (of the form [ORG]/[REPO])
        """
        arepo = sync(self.aclient.get_or_create_repo, name, bucket_nickname)
        return Repo(arepo)
