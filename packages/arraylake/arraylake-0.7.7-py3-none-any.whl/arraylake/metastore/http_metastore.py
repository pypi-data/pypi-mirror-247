...
from typing import Any, AsyncGenerator, Generator, Mapping, Optional, Sequence, TypeVar
...
class HttpMetastore(ArraylakeHttpClient, Metastore):
    ...
    async def list_databases(self) -> Sequence[Repo]:
        ...
        handle_response(response)
        return [Repo(**doc) for doc in response.json()]

    async def create_database(self, name: str, bucket_nickname: Optional[str] = None):
        """
        Params:
...
class HttpMetastoreDatabase(ArraylakeHttpClient, MetastoreDatabase):
    ...
    async def rebase(self, base_commit: CommitID | None, session_id: SessionID, upstream_branch: BranchName) -> CommitID:
        ...
        # passyng base_commit=& as query parameter doesn't work, it gets parsed as an empty DBID
    ...
    async def create_session(self, session_request: NewSession) -> SessionInfo:
        response = await self._request("POST", f"{self._repo_path}/sessions", json=session_request)
        handle_response(response)

        return SessionInfo.parse_obj(response.json())
    ...
    async def update_session_expiration(self, session_id: SessionID, expires_in: timedelta) -> SessionInfo:
        request = {"expires_in": expires_in}
        response = await self._request("PUT", f"{self._repo_path}/sessions/{session_id}", json=request)
        handle_response(response)
