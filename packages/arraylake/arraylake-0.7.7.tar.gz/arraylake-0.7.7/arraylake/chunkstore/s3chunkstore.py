...
from typing import TYPE_CHECKING, Callable, Deque, Dict, Mapping, Optional, Set, Tuple
...
class S3Chunkstore(Chunkstore):
    ...
    _known_key_cache: Deque

    def __init__(self, uri: str, inline_threshold_bytes: int = 0, **client_kws):
        """
        Args:
    ...
    def __getstate__(self):
        return self.uri, self.inline_threshold_bytes, self.client_kws

    def __setstate__(self, state):
        self.uri, self.inline_threshold_bytes, self.client_kws = state
        self._set_props()
        self._setup_chunk_key_cache()
    ...
    async def add_chunk(self, data: bytes, *, hash_method: Optional[str] = None) -> ReferenceData:
        ...
        chunk_ref = ReferenceData(uri=uri, offset=0, length=length, hash=hash)

        if not inline and token not in self._known_key_cache:
            resp = await self._session_client.put_object(Bucket=self._bucket, Key=key, Body=data)
            self._known_key_cache.append(token)
            await logger.adebug(resp)
