...
# TODO: add types
def retry_on_exception(exceptions: Tuple[Type[Exception], ...], n: int):
...
class ArraylakeHttpClient:
    ...
    def __init__(self, api_url: str, hint: Optional[str] = None, token: Optional[str] = None):
        ...
        self._default_headers = {
            ...
            "arraylake-feature-managed-sessions": str(config_obj.config.get("server_managed_sessions", False)),
        }
    ...
    @retry_on_exception((httpx.RemoteProtocolError, httpx.ConnectError), 3)  # type: ignore  # TODO we need to type retry_on_exception first
    ...
    async def get_user(self) -> Union[ApiTokenInfo, UserInfo]:
        ...
        if config_obj.config.get("user.diagnostics", True):
            try:
                ...
                response = await self._request("POST", "user/diagnostics", json=diagnostics.dict())
                handle_response(response)
