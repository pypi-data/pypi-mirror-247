...
def pipeline_to_dict(obj):
    if issubclass(type(obj), definitions.Definition):
        return pipeline_to_dict({obj.name : vars(obj)})
    elif isinstance(obj, Dict):
        return { k: pipeline_to_dict(v) for k, v in obj.items() }
    elif isinstance(obj, List):
        return [ pipeline_to_dict(o) for o in obj ]
    elif issubclass(type(obj), message.Message):
        return pipeline_to_dict(json_format.MessageToDict(obj, preserving_proto_field_name=True))
    return obj
...
def print_response(ctx: typer.Context, data):
  ...
  if output == 'objects':
    ...
    if issubclass(type(data), list) and len(data) > 0:
      type_check = data[0]
    elif issubclass(type(data), dict) and data.keys():
      type_check = list(data.keys())[0]
    ...
    if getattr(data, '__dict__', None):
      json_str = json.dumps(pipeline_to_dict(data), sort_keys=False, indent=int(ctx.obj.indent) if ctx.obj.indent else 2, cls=cls)
    else:
