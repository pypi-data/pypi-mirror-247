...
def check_that_hhmmss_start_is_before_end(start_time: str,
    ...
    if len(start_time.split(':')) != 3:
        ...
        raise InvalidInputError(f'Invalid time-stamp: ({end_time}). HH:MM:SS or HH:MM:SS.MS format required')
...
def check_if_valid_rgb_tuple(data: Tuple[int, int, int]) -> bool:
    check_instance(source=check_if_valid_rgb_tuple.__name__, instance=data, accepted_types=tuple)
    check_iterable_length(source=check_if_valid_rgb_tuple.__name__, val=len(data), exact_accepted_length=3)
    for i in range(len(data)):
        check_int(name='RGB value', value=data[i], max_value=255, min_value=0, raise_error=True)
