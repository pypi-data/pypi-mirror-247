...
class PiotrFeatureExtractor(ConfigReader,
    ...
    def __init__(self,
        ...
        self.session_timer, self.plot = SimbaTimer(start=True), plot
    ...
    def run(self):
        ...
        for file_cnt, file_path in enumerate(self.outlier_corrected_paths):
            ...
            white_animal_polygons = GeometryMixin().multiframe_bodyparts_to_polygon(data=white_animal_df_arr, pixels_per_mm=pixels_per_mm, parallel_offset=20, verbose=True, video_name=video_name, animal_name='white', core_cnt=CORE_CNT)
            black_animal_polygons = GeometryMixin().multiframe_bodyparts_to_polygon(data=black_animal_df_arr, pixels_per_mm=pixels_per_mm, parallel_offset=20, verbose=True, video_name=video_name, animal_name='black', core_cnt=CORE_CNT)
            ...
            if self.plot:
                GeometryPlotter(config_path=self.config_path, geometries=[white_animal_polygons, black_animal_polygons], core_cnt=CORE_CNT, video_name=video_name).run()
