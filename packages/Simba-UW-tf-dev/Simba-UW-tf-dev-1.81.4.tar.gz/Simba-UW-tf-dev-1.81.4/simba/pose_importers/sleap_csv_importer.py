...
from simba.utils.read_write import write_df, get_video_meta_data, get_fn_ext, clean_sleap_filename
...
class SLEAPImporterCSV(ConfigReader, PoseImporterMixin):
    ...
    def __init__(self,
        ...
        if self.animal_cnt > 1:
            self.data_and_videos_lk = self.link_video_paths_to_data_paths(data_paths=self.input_data_paths, video_paths=self.video_paths, filename_cleaning_func=clean_sleap_filename)
    ...
    def run(self):
        for file_cnt, (video_name, video_data) in enumerate(self.data_and_videos_lk.items()):
            output_filename = clean_sleap_filename(filename=video_name)
            ...
            if INSTANCE_SCORE in data_df.columns: data_df = data_df.drop([INSTANCE_SCORE], axis=1)
            idx = data_df.iloc[:, :2]
        ...
        stdout_success(msg=f'{len(list(self.data_and_videos_lk.keys()))} file(s) imported to the SimBA project (project_folder/csv/input_csv directory)', source=self.__class__.__name__)
