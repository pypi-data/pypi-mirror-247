...
class SimbaProjectPopUp(ConfigReader, PopUpMixin):
    ...
    def __init__(self,
        ...
        button_analyzeDirection_bp = Button(processmovementdupLabel, text="ANALYZE DIRECTIONALITY BETWEEN BODY PARTS",fg="purple", command=lambda: DirectionAnimalToBodyPartSettingsPopUp(config_path=self.config_path))
        button_visualizeDirection_bp = Button(processmovementdupLabel, text="VISUALIZE DIRECTIONALITY BETWEEN BODY PARTS", fg="black",command=lambda: DirectingAnimalToBodyPartVisualizerPopUp(config_path=self.config_path))
        ...
        button_outliercorrection = Button(label_outliercorrection,text='RUN OUTLIER CORRECTION', fg='green', command=lambda: threading.Thread(target=self.correct_outlier).start())
        button_skipOC = Button(label_outliercorrection,text='SKIP OUTLIER CORRECTION (CAUTION)',fg='red', command=lambda: threading.Thread(target=self.initiate_skip_outlier_correction).start())
        ...
        targeted_clip_annotator_note = Label(targeted_clip_annotator_frm,text='A bout annotator that creates annotated clips from videos associated with ML results.')
        targeted_clip_annotator_btn = Button(targeted_clip_annotator_frm, text='Select video',command= lambda: select_labelling_video_targeted_clips(config_path=self.config_path))
        ...
        self.csvfile = FileSelect(label_model_validation, 'SELECT DATA FEATURE FILE', color='blue', lblwidth=30, file_types=[('SimBA CSV', '*.csv'), ('SimBA PARQUET', '*.parquet')])
        ...
        button_runmachinemodel = Button(label_runmachinemodel,text='RUN MODELS', fg='green', compound='left', image=self.btn_icons['clf']['img'], command= lambda: threading.Thread(target=self.runrfmodel).start())
    ...
    def run_feature_extraction(self):
        ...
        if self.user_defined_var.get():
            custom_feature_extractor = CustomFeatureExtractor(extractor_file_path=self.scriptfile.file_path,
