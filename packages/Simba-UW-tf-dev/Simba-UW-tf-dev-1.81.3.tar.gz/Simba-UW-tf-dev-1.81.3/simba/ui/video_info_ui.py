...
from simba.utils.enums import Paths, ConfigKey, Dtypes, Keys, Links, Formats, TagNames
...
class VideoInfoTable(ConfigReader, PopUpMixin):
    ...
    def __check_that_value_is_numeric(self, value=None, value_name=None, video_name=None):
        if str(value).isdigit():
            ...
            raise ParametersFileError(msg=f'The {value_name} setting for video {video_name} is set to {value} in the project_folder/logs/video_info. Please set it to a numeric value.', source=self.__class__.__name__)
    ...
    def __append_videos_from_video_folder(self):
        for cnt, name in enumerate(self.video_paths):
            ...
            if isinstance(self.video_info_df, pd.DataFrame):
                prior_data = None
                try:
                    prior_data = self.read_video_info(video_name=video_basename, raise_error=False)[0]
    ...
    def __check_no_zero_px_per_mm_values(self):
        for index, row in self.video_df.iterrows():
            try:
                ...
                raise ParametersFileError(msg=f'The pixels/mm for video {row["Video"]} is not a numeric value. Please set pixels per mm using the "Calculate distance" button before proceding.', source=self.__class__.__name__)
            if px_per_mm_float <= 0:
                InvalidValueWarning(msg=f'The pixels/mm for video {row["Video"]} is set to zero. Please calculate pixels per mm using the "Calculate distance" button before proceding.', source=self.__class__.__name__)
    ...
    def create_window(self):
        ...
        self.col_widths = ['5', self.max_char_vid_name+5,'18','18','18','18', '18', '18']
        ...
        self.duplicate_distance_btn = Button(self.video_frm, text="Duplicate index 1 distance/mm (CAUTION!)", fg="red", command=lambda: self.__duplicate_idx_1_distance_mm())
        ...
        Label(self.execute_frm, text=f'THE PROJECT CONTAINS {len(self.videos.keys())} VIDEOS', fg='blue', font=Formats.LABELFRAME_HEADER_FORMAT.value, padx=20).grid(row=0, column=1, sticky=NW)
