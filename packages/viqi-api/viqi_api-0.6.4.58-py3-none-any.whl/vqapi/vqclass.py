...
from bq.metadoc.formats import Metadoc, anyxml_to_etree, RESERVED_TAGNAMES
...
class VQCollection:
    ...
    def __init__(self, sess, from_query = None, from_tags = None):
        ...
        self._from_tags = from_tags   # from_tags = ("resource type", {'tag_query':'...', 'tag_order':'...'})
...
class VQModule(VQResource):
    ...
    def inputs(self) -> list[tuple[str, str, object]]:
        ...
        Returns:
            list of (input name, input type, default value)
        """
        self._refresh(lvls=sys.maxsize)
        return [
        ...
        for param_name, param_val in kwargs.items():
            ...
            if not skip_typecheck:
                if (
                    ...
                    or (
                        isinstance(single_param_val, (int, float, complex)) and not isinstance(single_param_val, bool)
                    ...
                    if (
                        not isinstance(single_param_val, str)
                        or all(ptype in ("number", "boolean") or ptype in RESERVED_TAGNAMES or ptype.startswith("list") 
