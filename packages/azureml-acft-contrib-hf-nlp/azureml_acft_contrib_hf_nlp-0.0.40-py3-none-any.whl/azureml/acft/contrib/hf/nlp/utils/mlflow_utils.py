...
def replace_mlflow_weights_with_pytorch_weights_zero3(pytorch_model_save_path: str, mlflow_model_save_path: str):
    ...
    # Identify files other than py files
    all_files_except_code_files = find_files_with_inc_excl_pattern(str(dst_folder), exclude_pat=".py$")
    logger.info("Removing the dummy files created with mlflow.save")
    for file_name in all_files_except_code_files:
...
class SaveMLflowModelCallback(TrainerCallback):
    ...
    def on_train_end(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs):
        ...
        if state.is_world_process_zero:
            ...
            if self.mlflow_flavor == MLFLOW_FLAVORS.TRANSFORMERS:
                self.save_oss_flavor_mlflow_model(model=model, tokenizer=tokenizer, py_code_files=py_code_files)
            else:
                # save hftransformers or hftransformers model
                self.save_hftransformers_mlflow_model(acft_model=acft_model, tokenizer=tokenizer, model=model, misc_conf=misc_conf, py_code_files=py_code_files)
    ...
    def save_hftransformers_mlflow_model(self, acft_model, tokenizer, model, misc_conf, py_code_files):
        """
