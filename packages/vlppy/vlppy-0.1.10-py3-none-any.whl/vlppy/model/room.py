...
class Room:
    ...
    def regular_wall_angle(self, wall, shape):
        """获取规则墙壁反射单元(法向量)的方位角和倾斜角
        Params
            wall: 墙壁序列号
            shape: 角度形状
        Return
            angles: 墙壁反射单元(法向量)的方位角和倾斜角(单位:度)
        """
    ...
    def rough_wall_angle(self, wall, shape):
        ...
        alpha, beta = alpha.flatten(), beta.flatten()                  # 2*(l1*w1,1) -> 2*(l1*w1)
