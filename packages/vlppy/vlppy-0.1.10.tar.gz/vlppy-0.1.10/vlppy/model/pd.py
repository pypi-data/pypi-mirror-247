...
class PD:
    ...
    def recv_nlos_led_signal(self, led:LED, room:Room):
        ...
        for i, (w_pos, angle) in enumerate(room.get_reflect_wall_args()):
            ...
            # 反射(NLOS)子链路信道增益  
            Hw = (((led.m + 1) * self.Ar) / (2 * np.pi**2 * d1**2 * d2**2)) * rho * Aw * cos_led_t_angle_rad**led.m * cos_wall_r_angle_rad * cos_wall_t_angle_rad *  cos_pd_r_angle_rad * self.Ts * self.G_Con  # (l*w*h,l1*w1)
