...
def 路径格式化(路径: str, 分隔符: str = None) -> tuple[str, str]:
    """
...
class 语义日期模板:
    ...
    def __str__(self) -> str:
        ...
        if 时间语义:
            ...
            语义 = f'{月数.__abs__()}月{"前" if 月数 < 0 else "后"}'
        elif 周数 != 0:
...
class 打印模板:
    ...
    def 准备表格(self, 对齐控制串: str = None, 列宽控制表: list[int] = None):
        ...
        :param 对齐控制串: 一个包含 l c r 左 中 右 的字符串，分别控制对应列的对齐方式，l: 左对齐, c: 居中对齐, r: 右对齐, 例如 'l左cr'; 超出对齐控制串的列,由最后一个对齐控制字控制
...
class 交互选项类:
    ...
    @property
    def 说明(self) -> str:
        return str(self._说明 if self._说明 else '').strip()
    # endregion
...
class ssh接口类:
    ...
    def 有效(self) -> bool:
        ...
        if isinstance(self.__端口号, str) and self.__端口号.isnumeric():
            self.__端口号 = int(self.__端口号)
        if not (isinstance(self.__端口号, int) and 1 <= self.__端口号 <= 65535):
            self.__端口号 = 22
    ...
    def 端口号(self, 端口号: int):
        旧端口号 = self.__端口号
        if isinstance(端口号, str) and 端口号.isnumeric():
            端口号 = int(端口号)
        if not (isinstance(端口号, int) and 1 <= 端口号 <= 65535):
            端口号 = 22
        self.__端口号 = 端口号
    ...
    @property
    def 可达(self) -> bool:
        if not self.__可达 and self.__测试次数 < 1 and self.__主机地址 and self.__端口号:
    ...
    def ssh接口补全(self,
        ...
        def ip端口格式检测(待检测端口: str) -> bool:
            ...
            if not 待检测端口:
                return False
            if not 待检测端口.isnumeric():
                return False
            if 0 < int(待检测端口) <= 65535:
                return True
            else:
                return False
...
class 搜索接口基类:
    """
    您可以通过该基类构建个性化的搜索接口,在子类中,您需要实现如下函数功能:
    def 可用性判定(self) -> bool:  # 您需要提供您的搜索接口中可用性判定的判定逻辑
    def 搜索(self,
           搜索关键字: str = None,
           大小写敏感: bool = False,
           参考路径: str = None,
           ...
           搜文档: bool = True,
           搜路径: bool = False,
           画板: 打印模板 = None) -> 搜索结果类:  # 您需要提供您的搜索接口的搜索功能实现逻辑,并返回要求的结果对象
    
    class everything搜索接口类(搜索接口基类) 和 class sshFind搜索接口类(搜索接口基类) 是两个基于本基类实现的搜索接口,是不错的参考
    """
    ...
    # region 访问器
    @property
    def 在windows系统中(self) -> bool:
        return 在nt系统中()

    @property
    def 在linux系统中(self) -> bool:
        return 在posix系统中()
    ...
    # 提供一个判定搜索接口是否可用的统一方法,子类需要重写该方法
    def 可用性判定(self) -> bool:
        return False

    # 通过使用搜索接口,来尝试搜索指定的路径,如果可以搜索到,则该路径存在, 子类中可以重写该方法
    # 例如对于 everything 接口,由于确认其在window平台运行,所以可以重写该方法,使用 os.path.isdir() 来判定
    def 存在路径(self, 路径: str, 画板: 打印模板 = None) -> bool:
        ...
        # 使用搜索路径搜索该路径
        搜索结果 = self.搜索(搜索关键字=路径, 搜文档=False, 搜路径=True, 画板=画板)

        # 统一转换为小写,以便进行比较
        路径 = 路径.lower().replace('\\', '/').strip()
        if 搜索结果.总数 < 1:
            # 没有搜到内容
            return False
        else:
            for 结果 in 搜索结果.结果列表:
                if 结果.lower().replace('\\', '/').strip().startswith(路径):
                    return True
        return False

    # 通过使用搜索接口,来尝试搜索指定的文档,如果可以搜索到,则该文档存在, 子类中可以重写该方法
    # 例如对于 everything 接口,由于确认其在window平台运行,所以可以重写该方法,使用 os.path.isfile() 来判定
    def 存在文档(self, 文档: str, 画板: 打印模板 = None) -> bool:
        ...
        # 使用搜索路径搜索该文档
        搜索结果 = self.搜索(搜索关键字=文档, 搜文档=True, 搜路径=False, 画板=画板)
        ...
        if 搜索结果.总数 < 1:
            ...
            for 结果 in 搜索结果.结果列表:
                if 文档 == 结果.lower().replace('\\', '/').strip():
                    return True
        return False

    # 通过使用搜索接口, 搜索并列出指定路径下的所有文档,返回文档全名的列表,如果没有,则返回空列表
    def 列出文档(self, 路径: str, 限制数量: int = 0, 画板: 打印模板 = None) -> list[str]:
        画板 = 画板 if isinstance(画板, 打印模板) else 打印模板()
        ...
        路径 = str(路径 if 路径 else '').strip().replace('\\', '/')
        路径, 路径分隔符 = 路径格式化(路径=路径)
        路径 = 路径.rstrip(f'{路径分隔符}*').rstrip('*')
        if not 路径:
            return 文档列表

        关键字: str = f'{路径}{"" if 路径分隔符 == 路径 else 路径分隔符}*'

        搜索结果 = self.搜索(搜索关键字=关键字, 搜文档=True, 搜路径=False, 画板=画板)

        if isinstance(限制数量, str) and 限制数量.isnumeric():
            限制数量 = int(限制数量)
        ...
        if 搜索结果.总数 > 0:
            ...
            for 序号 in range(len(搜索结果.结果列表)):
                if 文档数量 >= 限制数量:
                    ...
                    if not 替换路径:
                        if '\\' in 文档全名:
                            替换路径 = 路径.lower().replace('/', '\\')
                        elif '/' in 文档全名:
                            替换路径 = 路径.lower().replace('\\', '/')
                        else:
                            替换路径 = 路径.lower()

                    文档名 = 文档全名.lower().replace(替换路径, '').lstrip('\\').lstrip('/')
                    if '\\' in 文档名 or '/' in 文档名:
                        # 如果文档名中存在路径分隔符,则说明这个文档是在子路径下
                        continue
                    elif 文档名.startswith('~$'):
                        # 这是回收站里的文档,不处理
                        continue
    ...
    def 列出子路径(self, 路径: str, 限制数量: int = 0, 画板: 打印模板 = None) -> list[str]:
        画板 = 画板 if isinstance(画板, 打印模板) else 打印模板()
        ...
        搜索结果 = self.搜索(搜索关键字=关键字, 搜文档=False, 搜路径=True, 画板=画板)
        ...
        if 搜索结果.总数 > 0:
            ...
            for 序号 in range(len(搜索结果.结果列表)):
                if 路径数量 >= 限制数量:
                    ...
                    路径名 = 路径全名.lower().replace(替换路径, '').lstrip('\\').lstrip('/')
                    if '\\' in 路径名 or '/' in 路径名:
                        # 如果路径名中存在路径分隔符,则说明这个文档是在子路径下
                        continue
                    elif 路径名.startswith('~$'):
                        # 这是回收站里的路径,不处理
                        continue
    ...
    def 搜索(self,
        ...
        画板.提示错误('搜索功能未实现, 这里是基类的搜索, 不会返回任何搜索结果')

        return 搜索结果类()


class everything搜索接口类(搜索接口基类):
    """
    本everything搜索接口是根据everything的http接口实现的,需要您的everything支持http服务
    """

    def __init__(self,
                 地址: str = None,
                 端口: int = 0):
        super(everything搜索接口类, self).__init__(地址=地址,
                                                   端口号=端口)

    def 可用性判定(self) -> bool:
        # 提供搜索接口可用性判定逻辑
        if self.服务在线:
            self.ssh接口.主机地址 = str(self.ssh接口.主机地址 if self.ssh接口.主机地址 else '').strip()
            if self.ssh接口.主机地址 and 1 <= self.ssh接口.端口号 <= 65535:
                return True
        return False
    ...
    def 搜索(self,
        ...
        参考路径 = 路径格式化(路径=参考路径, 分隔符='\\')[0].rstrip('\\')
        画板.调试消息(f'参考路径: {参考路径}')

        指定find参数 = str(指定find参数 if 指定find参数 else '').strip()
        画板.调试消息(f'指定find参数: {指定find参数}')
        ...
        if isinstance(限定数量, str) and 限定数量.isnumeric():
            限定数量 = int(限定数量)
        ...
        # 如果条件满足,则执行搜索过程
        if self.可用 and 搜索关键字:
            # 关于 everything 的 url 参数,可参考: https://www.voidtools.com/support/everything/http/
            # 由于everything只在window下用,不支持 / 路径分隔符,所以需要把关键字中的 / 替换为 \
            搜索关键字 = 搜索关键字.replace('/', '\\')  # everything 适用于windows平台,路径分隔符是 \
            搜索url: str = f'http://{self.ssh接口.主机地址}:{self.ssh接口.端口号}/?search={搜索关键字}' \
            ...
            try:
                ...
                会话.trust_env = False
                everything搜索结果 = 会话.get(搜索url, timeout=500)
                ...
                画板.消息(f'推荐您参考文档 https://ynrx7b5i1u.feishu.cn/docx/CKO8d2NlTo62yVxkUKmcMSWBnDf '
...
class posix搜索接口类(搜索接口基类):
    """
    这是一个基于 ssh 管道,通过 paramiko.SSHClient() 执行 find 命令,并分析返回结果来进行的搜索
    """
    ...
    def __init__(self,
        ...
        if isinstance(ssh接口, ssh接口类):
            super(posix搜索接口类, self).__init__(名称='未知' if not ssh接口.主机名 else ssh接口.主机名,
                                                  地址=ssh接口.主机地址,
                                                  端口号=ssh接口.端口号,
                                                  用户名=ssh接口.用户名,
                                                  密码=ssh接口.密码)
        else:
            super(posix搜索接口类, self).__init__(名称='未知',
                                                  地址=地址,
                                                  端口号=端口,
                                                  用户名=用户名,
                                                  密码=密码)
    ...
    def 存在路径(self, 路径: str, 画板: 打印模板 = None) -> bool:
        ...
        路径, 路径分隔符 = 路径格式化(路径=路径, 分隔符='/')
        if not 路径:
            return False
        if '*' in 路径:  # 如果在路径中存在通配符,则直接否定
            return False

        shell指令: str = f"[ -d {路径} ] && echo Existsdyy || echo Noexists"
        ...
        if self.ssh接口.有效:
            ...
            try:
                ...
                ssh管道.connect(hostname=self.ssh接口.主机地址,
                                username=self.ssh接口.用户名,
                                password=self.ssh接口.密码,
                                port=self.ssh接口.端口号)
                stdin, stdout, stderr = ssh管道.exec_command(shell指令, timeout=100)
                shell结果 = stdout.read().decode(encoding='utf-8').strip()
                shell错误 = stderr.read().decode(encoding='utf-8').strip()

                画板.调试消息(f'shell结果是: {shell结果}')
                画板.调试消息(f'shell错误是: {shell错误}')

                if 'Existsdyy' in shell结果:  # 路径存在
                    路径存在 = True
            except Exception as exp:
                画板.调试错误(f'执行shel指令是遇到异常: {exp}')
            finally:
                if isinstance(ssh管道, _paramiko.client.SSHClient):
                    ssh管道.close()
            return 路径存在
        else:  # ssh 接口无效
            if self.在windows系统中:  # 如果当前在 windows 系统中, 无法执行 posix 指令
                return False
            if self.在linux系统中:  # 通过执行 shell 指令来判断是否存在
                路径存在: bool = False
                try:
                    指令结果 = _subprocess.run(shell指令,
                                               stdout=_subprocess.PIPE,
                                               stderr=_subprocess.PIPE,
                                               shell=True,
                                               text=True)
                    shell结果 = str(指令结果.stdout).strip()
                    shell错误 = str(指令结果.stderr).strip()

                    画板.调试消息(f'shell结果是: {shell结果}')
                    画板.调试消息(f'shell错误是: {shell错误}')

                    if 'Exists' in shell结果:  # 路径存在
                        路径存在 = True
                except Exception as exp:
                    画板.调试错误(f'执行shel指令是遇到异常: {exp}')
                return 路径存在

        # 默认返回为 False, 即默认文档不存在
        return False
    ...
    def 存在文档(self, 文档: str, 画板: 打印模板 = None) -> bool:
        ...
        if not 文档:
            ...
            if 文档[-1] in ['/', '\\']:  # 这是个路径,不是文档
                return False
        if '*' in 文档:  # 文档中存在通配符,则直接否定
            return False

        文档, 路径分隔符 = 路径格式化(路径=文档, 分隔符='/')

        shell指令: str = f"[ -e {文档} ] && echo Existsdyy || echo Noexists"
        画板.调试消息(f'shell指令是: {shell指令}')
        if self.ssh接口.有效:
            # 通过ssh接口判断文档是否存在
            文档存在: bool = False
    ...
    def 搜索(self,
        ...
        if '/' in 搜索关键字:
            真正关键字 = 搜索关键字.split('/')[-1]
            参考路径 = 搜索关键字.replace(真正关键字, '')
            搜索关键字 = 真正关键字

        大小写敏感 = True if 大小写敏感 else False
        画板.调试消息(f'大小写敏感: {大小写敏感}')

        参考路径 = 路径格式化(路径=参考路径, 分隔符='/')[0]
        参考路径 = 参考路径 if 参考路径 else '/'
        画板.调试消息(f'参考路径: {参考路径}')
        ...
        if self.可用 and 搜索关键字:
            ...
            if 搜索关键字:
                if 指定find参数:
                    find指令 = f'find {指定find参数}'
                elif 搜文档:
                    # 如果被要求搜文档，则处理搜文档逻辑，不care是否要求搜路径
                    if '*' == 搜索关键字:
                        find指令 = f'find {参考路径} -maxdepth 1 -type f'
                    else:
                        find指令 = f'find {参考路径} -{"" if 大小写敏感 else "i"}name "{搜索关键字}" -type f'
                elif 搜路径:
                    if '*' == 搜索关键字:
                        find指令 = f'find {参考路径} -maxdepth 1 -type d'
                    else:
                        find指令 = f'find {参考路径} -{"" if 大小写敏感 else "i"}name "{搜索关键字}" -type d'
            ...
            if find指令:
                try:
                    if self.ssh接口.无效 and self.在linux系统中:
                        # 如果 ssh 接口无效, 且当前在linux中, 直接进行 find 搜索
                        ...
                        # 如果ssh接口有效, 则通过 ssh 管道进行 find 搜索
                        ssh管道 = _paramiko.SSHClient()
                        ssh管道.set_missing_host_key_policy(_paramiko.AutoAddPolicy())
                        ssh管道.connect(hostname=self.ssh接口.主机地址,
                                        username=self.ssh接口.用户名,
                                        password=self.ssh接口.密码,
                                        port=self.ssh接口.端口号)
                        stdin, stdout, stderr = ssh管道.exec_command(find指令, timeout=1000)
                        find结果 = stdout.read().decode(encoding='utf-8').strip()
                except Exception as exp:
                    搜索结果.状态码 = 1
                    搜索结果.错误消息 = str(exp)
                    self.服务在线 = False
...
class 本地搜索接口类(搜索接口基类):
    def __init__(self,
                 everything地址: str = None,
                 everything端口: int = 0):
        super(本地搜索接口类, self).__init__(地址=everything地址,
                                             端口号=everything端口)
    ...
    # 重新定义方法,使用 os.path.isfile 来判断是否存在文档
    def 存在文档(self, 文档: str, 画板: 打印模板 = None) -> bool:
        画板 = 画板 if isinstance(画板, 打印模板) else 打印模板()
        画板.执行位置(self.__class__, self.存在文档)
        ...
        if not 文档:
            return False
        if 文档[-1] in ['/', '\\']:  # 这是一个路径,不是个文档
            return False

        文档, 路径分隔符 = 路径格式化(路径=文档)
        if '*' in 文档:  # 文档中存在通配符,则直接否定
            return False

        return _os.path.isfile(文档)

    # 重新定义方法,使用 os.path.isDir 来判断是否存在路径
    def 存在路径(self, 路径: str, 画板: 打印模板 = None) -> bool:
        画板 = 画板 if isinstance(画板, 打印模板) else 打印模板()
        画板.执行位置(self.__class__, self.存在路径)
    ...
    # 重新定义方法,使用os.scandir 方法扫描指定路径下的文档
    def 列出文档(self, 路径: str, 限制数量: int = 0, 画板: 打印模板 = None) -> list[str]:
        画板 = 画板 if isinstance(画板, 打印模板) else 打印模板()
        画板.执行位置(self.__class__, self.列出文档)
        ...
        文档数量: int = 0
        for 节点 in _os.scandir(路径):
            if 文档数量 >= 限制数量:
                break
            else:
                全名: str = _os.path.join(路径, 节点.name)
    ...
    def 列出子路径(self, 路径: str, 限制数量: int = 0, 画板: 打印模板 = None) -> list[str]:
        ...
        for 节点 in _os.scandir(路径):
            if 路径数量 >= 限制数量:
                ...
                全名 = _os.path.join(路径, 节点.name)
                if _os.path.isdir(全名):
                    路径列表.append(全名)
                    路径数量 += 1
        return 路径列表

    # 根据平台,自动判断使用 everything or find 进行搜索
    def 搜索(self,
             搜索关键字: str = None,
             大小写敏感: bool = False,
             参考路径: str = None,
        ...
        if self.在windows系统中:
            # windows 下,调用 everything 搜索接口进行搜索
            everything搜索 = everything搜索接口类(地址=self.主机地址, 端口=self.端口号)
            everything搜索.服务在线 = self.服务在线

            搜索结果 = everything搜索.搜索(搜索关键字=搜索关键字,
                                           大小写敏感=大小写敏感,
                                           参考路径=参考路径,
                                           指定find参数=指定find参数,
                                           搜文档=搜文档,
                                           ...
                                           限定数量=限定数量,
                                           画板=画板)
            self.服务在线 = everything搜索.服务在线
            return 搜索结果
        elif self.在linux系统中:
            # linux下,调用 posix 搜索接口进行搜索
            posix搜索 = posix搜索接口类()
            posix搜索.服务在线 = self.服务在线

            搜索结果 = posix搜索.搜索(搜索关键字=搜索关键字,
                                      大小写敏感=大小写敏感,
                                      参考路径=参考路径,
                                      指定find参数=指定find参数,
                                      搜文档=搜文档,
...
class 交互接口类:
    ...
    def 添加选项(self,
                 选项: list or str or 交互选项类,
                 代号: list or str or int = None,
                 备注: list or str = None,
                 修饰方法: list[callable] or callable = None) -> int:
        """
        添加选项，以供交互时提供给用户选择
        :param 选项: 需要添加的选项
            ...
            if 选项数量 > 0:
                ...
                for 选项索引 in range(选项数量):
                    if isinstance(选项[选项索引], 交互选项类):
                        ...
                        说明: str = ''
                        if 代号备注索引 < 备注数量:
                            说明 = str(备注列表[代号备注索引] if 备注列表[代号备注索引] else '').strip()

                        修饰方法 = 修饰方法列表[选项索引]

                        代号备注索引 += 1

                        待添加选项列表.append(交互选项类(代号=代号值, 选项=选项名称, 说明=说明, 修饰方法=修饰方法))
        else:
            # 统一把 选项,代号, 备注 识为字符串
            if 选项:
    ...
    def 发起选项交互(self,
        ...
        交互结果: 交互接口类._交互结果类 = 交互接口类._交互结果类()
        交互结果.选项 = []
        交互结果.代号 = []
        ...
        def 展示选项(操作说明: list[str] or str = None,
            ...
            def 绿化处理(内容: str) -> str:
                ...
                if 兴趣字集:
                    for 字 in 兴趣字集:
                        if 字:
                            # 把字做为一个匹配模板来进行匹配绿化内容,并设置忽略大小写
                            匹配结果: list[str] = _re.findall(字, 绿化内容, flags=_re.I)
                            if 匹配结果:
            ...
            选项字典: dict[str, str] = {}
            画板.准备表格(对齐控制串='rl')
            画板.添加分隔行(提示文本='交互选项表', 适应窗口=True, 修饰方法=黄字)
            if 操作说明信息:
                # 把操作说明输出在选项的前面
                for 说明 in 操作说明信息:
                    if 说明:
                    ...
                    if 临时代号:
                        ...
                        if 临时代号:
                            ...
                            if 选项.说明:
                                备注存在 = True
                                if callable(选项.修饰方法):
                                    画板.添加一行(临时代号, 绿化处理(选项.选项), 选项.说明).修饰行(方法=选项.修饰方法)
                                else:
                                    画板.添加一行(临时代号, 绿化处理(选项.选项), 选项.说明)
                            else:
                                if callable(选项.修饰方法):
                                    画板.添加一行(临时代号, 绿化处理(选项.选项)).修饰行(方法=选项.修饰方法)
                                else:
                                    画板.添加一行(临时代号, 绿化处理(选项.选项))

            # 定制标题行
            if 备注存在:
                画板.修改指定行(行号=标题行, 列表=['代号', '选项', '备注']).修饰行(青字)
            else:
                画板.修改指定行(行号=标题行, 列表=['代号', '选项']).修饰行(青字)
        ...
        选项表: dict[str, str] = 展示选项(操作说明=操作说明,
                                          兴趣字=兴趣字,
                                          画板=画板)
        选项代号表: list[str] = list(选项表.keys())
        if not 选项代号表:
            return 交互结果
        else:
            while True:
                ...
                if not 输入文本:
                    ...
                    if not 多选:
                        ...
                        if 输入文本切割:
                            ...
                            for 文本 in 输入文本切割:
                                if '-' in 文本 and not (
                                        (文本 in 选项代号表) or 文本.startswith('-') or 文本.endswith('-')):
                                    # 这是一个范围指定项, 例如 'a-z', '1-8' 等
                                    范围值表: list[str] = [值.strip() for 值 in 文本.split('-') if 值.strip()]
                                    if len(范围值表) < 2:  # 要求至少存在两个值,一个下界值, 一个上界值
                                        画板.消息(f'指定的代号范围 {红字(文本)} 无效, 请重新输入')
                                        存在无效值 = True
                                        break
                                    代号下界: str = 范围值表[0]  # 第一个值被识别为代号下界
                                    代号上界: str = 范围值表[1]  # 第二个值被识别为代号上界

                                    代号下界索引: int = -1
                                    代号上界索引: int = -1
                                    if 代号下界 in 选项代号表:
                                        代号下界索引 = 选项代号表.index(代号下界)
                                    else:
                                        画板.消息(f'不存在选项代号 {红字(代号下界)}, 请重新输入')
                                    if 代号上界 in 选项代号表:
                                        代号上界索引 = 选项代号表.index(代号上界)
                                    else:
                                        画板.消息(f'不存在选项代号 {红字(代号上界)}, 请重新输入')

                                    if 代号上界索引 < 0 or 代号下界索引 < 0:  # 代号下界或者上界不存在
                                        存在无效值 = True
                                        break
                                    elif 代号下界索引 == 代号上界索引:  # 代号下界与代号上界相同
                                        选择的代号列表.append(代号下界)
                                    else:
                                        for 索引 in range(min(代号下界索引, 代号上界索引),
                                                          max(代号下界索引, 代号上界索引) + 1):
                                            选择的代号列表.append(选项代号表[索引])
                                else:
                                    if 文本 in 选项代号表:
                                        选择的代号列表.append(文本)
                                    else:
    ...
    def 指定选择路径(cls,
        ...
        while True:
            ...
            if '0' == 用户输入:
                ...
                if not isinstance(搜索接口, 搜索接口基类) or 搜索接口.不可用:
                    # if 搜索接口不是符合条件的类型, 或者搜索接口不可用, 则无法判断路径是否存在,清空用户输入并返回
                    路径 = ''
                    break
                elif 搜索接口.存在路径(路径=路径, 画板=画板.副本):
                    # if 用户输入的路径存在,则直接返回用户输入
                    break
                else:
                        ...
                        if 可用路径列表.总数 > 0:
                            ...
                            if 路径列表:
                                ...
                                if 可用路径列表.截断 or 截断标记:
                                    if 可用路径列表.截断:
                                        交互端.添加选项(代号=红字('...'),
                                                        选项=红字(f'搜索结果未完全呈现, 共 >={搜索条数限制} 条'))
                                    else:
                                        交互端.添加选项(代号=红字('...'),
                                                        选项=红字(f'搜索结果未完全呈现, 共 {可用路径列表.总数} 条'))

                                # 添加一个额外选项,供用户反悔
                                交互端.添加选项(选项='-')
                                交互端.添加选项(代号='r', 选项='重新指定/搜索', 修饰方法=黄字)
                                if 功能选项:
                                    for 选项 in 功能选项:
                                        选项.修饰方法 = 黄字
                                        交互端.添加选项(选项=选项)
    ...
    def 指定选择文档(cls,
        ...
        输入提示 = str(输入提示 if 输入提示 else '').strip()
        输入提示 = 输入提示 if 输入提示 else '请指定文档或者关键字: '
        ...
        while True:
            ...
            if '0' == 用户输入:
                ...
                if not isinstance(搜索接口, 搜索接口基类) or 搜索接口.不可用:
                    ...
                    文档列表 = []
                    break
                elif 搜索接口.存在文档(文档=文档列表[0], 画板=画板.副本):
                    # 搜索接口判定文档存在, 则返回该文档
                    break
                else:
                    # 如果判定文档不存在
                            ...
                            if 文档列表:
                                ...
                                if 可用文档列表.截断 or 截断标记:
                                    if 可用文档列表.截断:
                                        ...
                                        交互端.添加选项(代号=红字('...'),
                                                        选项=红字(f'搜索结果未完全呈现, 共 {可用文档列表.总数} 条'))
                                ...
                                # 没有满足约束条件的搜索结果,请用户重新输入, 以便重新搜索
                                画板.消息(
                                    '不存在满足条件的文档选项, 可能的原因是搜索关键字过于宽泛,您可以提供更多关键字信息以便重试')
    ...
    def 选择文档(初始路径: str = None,
        ...
        # region 初始化参考路径
        参考路径, 路径分隔符 = 路径格式化(初始路径)

        # 提供用户选择文档的交互
        while True:
            if (not 参考路径) or (
                    isinstance(搜索接口, 搜索接口基类) and 搜索接口.可用 and not 搜索接口.存在路径(路径=参考路径,
                                                                                                   画板=画板.副本)):
                # 如果参考路径不存在, 或者可以判断参考路径不存在
                画板.消息(f"当前路径(本地)为: .")
                参考路径 = 交互接口类.指定选择路径(输入提示='请指定文档路径或者关键字(0: 退出操作): ',
                                                   搜索接口=搜索接口,
                                                   排除规则=路径排除规则,
                                                   画板=画板)
                if '0' == 参考路径:
            ...
            if not 文档列表:
                ...
                # 引导用户重新指定操作路径
                参考路径 = 交互接口类.指定选择路径(输入提示='请指定文档路径或关键字(0: 退出操作): ',
                                                   搜索接口=搜索接口,
                                                   画板=画板)
                if '0' == 参考路径:
                    # 如果用户选择了退出,则退出
        ...
        if 画板.正在调试 and (已匹配的参数 or 未匹配的参数):
            ...
            if 已匹配的参数:
                ...
                for 参数 in 已匹配的参数.values():
                    画板.添加一行(参数.名称, 参数.类型, 参数.值, 参数.提示)
            if 未匹配的参数:
                画板.添加分隔行(提示文本='以下参数未匹配成功', 适应窗口=True, 修饰方法=红字)
