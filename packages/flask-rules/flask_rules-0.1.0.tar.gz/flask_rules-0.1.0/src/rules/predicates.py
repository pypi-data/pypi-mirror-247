...
class Predicate:
    ...
    def __repr__(self) -> str:
        return f"<{type(self).__name__}:{str(self)} object at {hex(id(self))}>"
    ...
    def __and__(self, other) -> Predicate:
        ...
        return type(self)(AND, f"({self.name} & {other.name})")

    def __or__(self, other) -> Predicate:
        def OR(*args):
            return self._combine(other, operator.or_, args)

        return type(self)(OR, f"({self.name} | {other.name})")

    def __xor__(self, other) -> Predicate:
        def XOR(*args):
            return self._combine(other, operator.xor, args)

        return type(self)(XOR, f"({self.name} ^ {other.name})")
    ...
    def _apply(self, *args) -> bool | None:
