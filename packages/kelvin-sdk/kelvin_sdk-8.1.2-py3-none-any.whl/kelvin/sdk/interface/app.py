...
@typechecked
def app_build(app_dir: str, fresh_build: bool = False, build_args: Dict[str, str] = {}) -> OperationResponse:
    """
    ...
    return _project_build(app_dir=app_dir, fresh_build=fresh_build, build_args=build_args)


@typechecked
def app_deploy(app_dir: str, fresh_build: bool = False) -> OperationResponse:
    """
    ...
    """
    from kelvin.sdk.lib.apps.local_apps_manager import project_deploy as _project_deploy

    return _project_deploy(app_dir=app_dir, fresh_build=fresh_build)


@typechecked
def app_undeploy(
    app_dir: Optional[str], name: Optional[str], version: Optional[str], assets: Optional[List[str]]
) -> OperationResponse:
    """
    ...
    app_dir: Optional[str]
        the path where the application is hosted (defaults to current directory)
    name: Optional[str]
        name of the application
    version: Optional[str]
        version of the application (eg: 1.0.1)
    assets: Optional[List[str]]
        list of assets to undeploy the application from
    ...
    """
    from kelvin.sdk.lib.api.app_manager import get_undeploy_info_from_config, undeploy_app

    if not (name and version and assets):
        name, version, assets = get_undeploy_info_from_config(app_dir or ".")

    return undeploy_app(name, version, assets)
...
def app_images_list(should_display: bool = False) -> OperationResponse:
    ...
    """
    from kelvin.sdk.lib.emulation.emulation_manager import get_local_appregistry_images
...
@typechecked
def app_image_config(app_name_with_version: str, output_dir: str) -> OperationResponse:
    """
    ...
    """
    from kelvin.sdk.lib.apps.local_apps_manager import app_image_config as _app_image_config

    return _app_image_config(app_name_with_version=app_name_with_version, output_dir=output_dir)
