...
def read_app_py(path: str) -> str:
    ...
    try:
        ...
        raise Exception(f"app.py not found in {directory}") from None
...
def publish(
    ...
    # Read app config
    config_reader = ConfigReader(path, dryrun)
...
def _create_deployment_by_source_code(
    ...
    assets = _tar_assets(
        path, app_config.files or DEFAULT_FILES, REQUIRED_FILES
    )
