...
class MarimoV1Client:
    ...
    def create_deployment(
        self, *, application_id: ApplicationId, code: str, requirements_txt: str, assets: typing.IO
    ) -> Deployment:
        _response = httpx.request(
            "POST",
            urllib.parse.urljoin(f"{self._environment}/", "api/v1/deployments"),
            data=jsonable_encoder({"applicationId": application_id, "code": code, "requirementsTxt": requirements_txt}),
            files={"assets": assets},
            headers=remove_none_from_headers(
                {"Authorization": f"Bearer {self._token}" if self._token is not None else None}
            ),
        ...
        async with httpx.AsyncClient() as _client:
            _response = await _client.request(
                ...
                data=jsonable_encoder(
                    {"applicationId": application_id, "code": code, "requirementsTxt": requirements_txt}
                ),
                files={"assets": assets},
                headers=remove_none_from_headers(
                    {"Authorization": f"Bearer {self._token}" if self._token is not None else None}
