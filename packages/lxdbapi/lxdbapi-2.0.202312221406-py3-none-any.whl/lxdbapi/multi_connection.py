    ...
    while retry > 0:
        ...
        try:
            ...
            if 'Broken pipe' in msg or 'IO Error' in msg or 'Connection refused' in msg \
                ...
                raise RuntimeError("Execution failed: {}".format(ex), ex)
    return result
...
class MultiConnection(MultiConnectionI):
    ...
    def set_session(self, autocommit=None, readonly=None, useTZ=False):
        _invoke_with_reconnect(self, None, lambda: self._delegator.set_session(autocommit, readonly, useTZ))
    ...
    def autocommit(self, value):
        if self.autocommit is not value:
            _invoke_with_reconnect(self, None, lambda: self._delegator.autocommit(value))
            with self._do_reconnect_lock:
                self._is_auto_commit = self._delegator.autocommit
