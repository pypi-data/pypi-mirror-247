...
# ====================================================================================================================
# 装饰器方法
def Dao(entity, return_type=tuple, auto_single_record: bool = True):
    """对数据访问类使用的装饰器，用于绑定数据类和数据访问类。

    Args:
        entity: 该数据访问类对应的数据类。
        return_type: [tuple, dict, dataclass, your_entity], 设置select语句查询结果的数据类型。
...
def __return_entity(entity, cursor, result, is_list=True):
    ...
    if not all(field in entity_fields for field in cursor_fields):
        raise ValueError("Cursor fields are not a subset of entity fields.")
    ...
    def create_entity(row):
        _entity = entity.__new__(entity)
        for field, value in zip(field_mapping, row):
            setattr(_entity, field, value)
        return _entity
    ...
    if cursor_fields == entity_fields:
        ...
        # 映射cursor字段到entity字段
        field_mapping = [field for field in cursor_fields if field in entity_fields]
