...
def test_row_factory(init_db: Database):
    ...
    start = time.time()
    results3 = db.dao_student.select_all3()
    end = time.time() - start
    print(f'entity_fast耗费时间：{end}, 查询结果数量；{len(results3)}')

    start = time.time()
    results4 = db.dao_student.select_all4()
    end = time.time() - start
    print(f'entity_part耗费时间：{end}, 查询结果数量；{len(results4)}')

    start = time.time()
    results5 = db.dao_student.select_all5()
    end = time.time() - start
    print(f'entity_not_match耗费时间：{end}, 查询结果数量；{len(results5)}')

    assert results2 == results3 == results5

    start = time.time()
    results6 = db.dao_student.select_all()
    end = time.time() - start
    print(f'dataclass耗费时间：{end}, 查询结果数量；{len(results6)}')
...
def test_is_selecting_single_field():
    # 测试用例
    test_cases = [
