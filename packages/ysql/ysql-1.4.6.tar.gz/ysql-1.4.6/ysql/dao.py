...
# ====================================================================================================================
# 装饰器方法
def Dao(entity, data_type=tuple, fetch_one: bool = True, enable_substitute: bool = True):
    """对数据访问类使用的装饰器，用于绑定数据类和数据访问类。

    Args:
        entity: 该数据访问类对应的数据类。
        data_type: [tuple, dict, dataclass, namedtuple, your_entity], 设置select语句查询结果的数据类型。
        fetch_one: 自动识别并返回单条select记录。
        enable_substitute: 启用表名替代符 '__' ，自动将其转换为绑定的entity的类名。
    ...
    def decorator(cls):
        ...
        cls.cursor = None
        cls.entity = entity
        cls.fetch_one = fetch_one
        cls.enable_substitute = enable_substitute
        cls.data_type = data_type if data_type != dataclass else _DATACLASS
        # 新增方法
        cls.execute = execute
...
def Sql(sql: str, data_type=None):
    """执行sql语句的装饰器，传入sql语句的同时会自动实现被装饰函数。

    Args:
        sql: 固定字符串sql语句。
        data_type: [tuple, dict, dataclass, namedtuple, your_entity], 设置select语句查询结果的数据类型，
                    此参数优先级更高，若Dao和Sql同时设置该参数，则遵循Sql的data_type。

    Return:
        使用select时，将结合data_type自动返回查询结果。
...
class DaoFunc:
    ...
    def execute(self, sql: str, args=None, data_type=None):
        """执行sql语句"""
        pass
...
def execute(self, sql: str, args=None, data_type=None):
    ...
    Args:
        ...
        args: 执行该sql需要的参数。
        data_type: [tuple, dict, dataclass, namedtuple, your_entity], 设置select语句查询结果的数据类型，
                    此参数优先级更高，若Dao和execute同时设置该参数，则遵循execute的data_type。
    ...
    return __fetch_result(self=self, sql=sql, cursor=self.cursor, data_type=data_type)
...
def __return_one_field(result, data_type, is_list=True):
    """提取出单字段并返回，并处理bool类型"""
    if data_type != bool:
        if is_list:
...
def __is_fetch_one(sql: str) -> bool:
    """判断sql语句中是否明确仅取一条记录"""
    return any([sql.endswith(flag) for flag in RETURN_FLAG])


def __fetch_result(self, sql, cursor, data_type):
    """确定返回类型"""
    lower_sql = sql.lower()

    _data_type = copy.deepcopy(self.data_type)
    if data_type is not None:
        _data_type = data_type

    if self.fetch_one and __is_fetch_one(lower_sql):
        is_list = False
    ...
    # 不做转换
    if not result or _data_type == tuple:
        return result

    elif is_entity(_data_type):
        return __return_entity(entity=_data_type, cursor=cursor, result=result, is_list=is_list)

    elif _data_type in _ENABLED_TYPES_WITHOUT_NONE:
        return __return_one_field(result=result, data_type=_data_type, is_list=is_list)

    elif _data_type == _DATACLASS:
    ...
    elif _data_type == namedtuple:
        return __return_namedtuple(cursor=cursor, result=result, is_list=is_list)

    else:
        raise TypeError(
            f"Dao的查询结果格式设置错误，应该属于: 'tuple' 'dict' 'dataclass' 'entity'，但得到的是: {_data_type}")
