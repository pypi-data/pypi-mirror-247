...
def Dao(entity, return_type=tuple, auto_single_record: bool = True):
    ...
    Args:
        entity: 该数据访问类对应的数据类
        return_type: 设置select语句查询结果的数据类型
...
# ====================================================================================================================
# Dao类新增的方法
def execute_sql(self, sql: str, args=None, return_type=None):
    """执行sql语句"""
...
def __return_entity(entity, result, is_list=True):
    """设置查询结果为传入的entity数据类"""
    ...
    if is_list:
        return [create_entity(row) for row in result]
    return create_entity(result)
...
def __fetch_result(self, sql, cursor, return_type):
    ...
    elif is_entity(return_type):
        return __return_entity(entity=return_type, result=result, is_list=is_list)
