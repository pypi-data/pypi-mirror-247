...
from ysql.entity import Constraint, _parse_constraints, _ENABLED_TYPES, is_entity, _ENABLED_TYPES_WITHOUT_NONE

# ====================================================================================================================
# 模块常量
TABLE_SUBSTITUTE = '__'  # 约定的表名代替符，在sql语句中凡是涉及本表表名的，均可采取此代替符，内部可自动替换。
RETURN_FLAG = ('limit 1', 'limit 1;')  # 在sql语句中包含此字符，则直接返回单条记录，不再返回列表。
...
# ====================================================================================================================
# 装饰器方法
def Dao(entity, return_type=tuple, auto_single_record: bool = True, enable_table_name_substitute: bool = True):
    """对数据访问类使用的装饰器，用于绑定数据类和数据访问类。

    Args:
        entity: 该数据访问类对应的数据类。
        return_type: [tuple, dict, dataclass, namedtuple, your_entity], 设置select语句查询结果的数据类型。
        auto_single_record: 自动识别并返回单条select记录。
        enable_table_name_substitute: 启用表名替代符 '__' 将自动转换为绑定的entity的类名。
    ...
    def decorator(cls):
        ...
        cls.cursor = None
        cls.entity = entity
        cls.auto_single_record = auto_single_record
        cls.enable_table_name_substitute = enable_table_name_substitute
...
def Sql(sql: str, return_type=None):
    """执行sql语句的装饰器，传入sql语句的同时会自动实现被装饰函数。

    Args:
        sql: 固定字符串sql语句。
        return_type: [tuple, dict, dataclass, namedtuple, your_entity], 设置select语句查询结果的数据类型，
                    此参数优先级更高，若Dao和Sql同时设置该参数，则遵循Sql的return_type。

    Return:
        使用select时，将结合return_type自动返回查询结果。
    ...
    def decorator(func):  # noqa
        ...
        def wrapper(self, *args, **kwargs):
            ...
            final_sql = __check_table_name_substitute(sql=sql,
                                                      entity=self.entity,
                                                      enable_table_name_substitute=self.enable_table_name_substitute)
            log.info(f"生成的执行sql:{final_sql}")
...
class DaoFunc:
    ...
    def execute(self, sql: str, args=None, return_type=None):
        """执行sql语句"""
        pass
...
def execute(self, sql: str, args=None, return_type=None):
    ...
    Args:
        ...
        args: 执行该sql需要的参数。
        return_type: [tuple, dict, dataclass, namedtuple, your_entity], 设置select语句查询结果的数据类型，
                    此参数优先级更高，若Dao和execute同时设置该参数，则遵循execute的return_type。

    Returns:
        使用select时，将结合return_type自动返回查询结果。

    """
    sql = __check_table_name_substitute(sql=sql,
                                        entity=self.entity,
                                        enable_table_name_substitute=self.enable_table_name_substitute)
    if args is None:
    ...
    return __fetch_result(self=self, sql=sql, cursor=self.cursor, return_type=return_type)
...
def _generate_sql_create_table(self):
    ...
    log.info(f"生成的建表SQL语句：{sql_create_table}")
...
def __check_table_name_substitute(sql: str, entity, enable_table_name_substitute):
    """检查并替换表名代替符"""
    if enable_table_name_substitute:
...
def __return_single_field(result, return_type, is_list=True):
    """提取出单字段并返回，并处理bool类型"""
    if return_type != bool:
        if is_list:
...
def __is_auto_single_record(sql: str) -> bool:
    """判断sql语句中是否明确仅取一条记录"""
    return any([sql.endswith(flag) for flag in RETURN_FLAG])


def __fetch_result(self, sql, cursor, return_type):
    """确定返回类型"""
    lower_sql = sql.lower()
    ...
    if self.auto_single_record and __is_auto_single_record(lower_sql):
        is_list = False
    ...
    elif _return_type in _ENABLED_TYPES_WITHOUT_NONE:
        return __return_single_field(result=result, return_type=_return_type, is_list=is_list)
