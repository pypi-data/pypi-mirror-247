...
class Trainer():
    ...
    def _instantiate_model(self):
        ...
        if 'module_path' in self._cortex_config.keys() and self._cortex_config['module_path'] is not None:
            model_module = self._load_module(
                '{}/{}.py'.format(self._cortex_config['module_path'], self._cortex_config['model_module']),
                self._cortex_config['model_module']
            )
        else:
            model_module = self._load_module(
                '{}.py'.format(self._cortex_config['model_module']),
                self._cortex_config['model_module']
            )

        model_params = dict(ChainMap(*self._cortex_config['params'])) if self._cortex_config['params'] else {}
        ...
        try:
            ...
            print(f'Failed to get API key from environment variables. Using empty string instead.\n{e}')
        ...
        self.model = getattr(model_module, self._cortex_config['model_class'])(model_params, self._model_id, api_url(), headers)
    ...
    def set_artifacts_dir(self, artifacts_dir: str):
        if artifacts_dir:
