...
from .utils import execute_sql, logger, next_time_occurrence, schema_table
...
class CAgg(MaterializedView):
    ...
    def compress(self, engine: Engine):
        execute_sql(
            f"ALTER MATERIALIZED VIEW {schema_table(self.table)} SET (timescaledb.compress = true);",
            engine,
        )
