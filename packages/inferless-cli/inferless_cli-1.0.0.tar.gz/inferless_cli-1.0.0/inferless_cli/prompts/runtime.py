...
from inferless_cli.utils.services import (
    get_templates_list,
    create_presigned_url_for_runtime_upload,
)
import uuid
...
def list():
    ...
    with Progress(
        ...
        runtimes = get_templates_list(user_id)
...
def upload(
    ...
    with Progress(
        ...
        res = create_presigned_url_for_runtime_upload(
            payload, uid, name, file_name.replace(" ", ""), path, workspace_id
        )
        progress.remove_task(task_id)
