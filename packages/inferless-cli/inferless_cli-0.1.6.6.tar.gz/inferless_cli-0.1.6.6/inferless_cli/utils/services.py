...
from inferless_cli.utils.constants import (
    ...
    PRESIGNED_URL_RUNTIME_UPLOAD_URL,
    SAVE_RUNTIME_URL,
    VALIDATE_TOKEN_URL,
    GET_WORKSPACES,
...
def create_volume(workspace_id: str, name: str):
    payload = {
...
def create_presigned_url_for_runtime_upload(
    payload, uuid, name, file_name, path, workspace_id
):
    response = make_request(
        PRESIGNED_URL_RUNTIME_UPLOAD_URL, method="POST", auth=True, data=payload
    )

    data = response.json()
    if data:
        return upload_runtime(
            data["details"], uuid, name, file_name, path, workspace_id
        )


def upload_runtime(url, uuid, name, file_name, path, workspace_id):
