...
class AmpApi:
    ...
    def _amp_url(self, path: str) -> str:
        path = amp_api_path(path)
        return self.endpoint + path

    def _amp_headers(self) -> str:
        return {
            'user-agent': get_user_agent(self.user_agent)
        }
    ...
    def get_user_model_info(self, model_id: str) -> dict:
        ...
        resp = make_request(self.ak, self.sk, method, url, headers=self._amp_headers())
        amp_raise_for_status(resp)
        return resp.json()

    def list_user_model_files(self, model_id: str, revision: Optional[str]=None) -> list:
        ...
        url = self._amp_url(f'/v1/subscriptions/{subscription_name}/resourceGroups/{resource_group_name}/'
    ...
    def list_public_model_files(self, model_id: str, revision: Optional[str]=None) -> list:
        method = 'GET'
    ...
    def create_public_model(self, model_id: str, display_name: Optional[str]=None) -> dict:
        method = 'POST'
