...
class RioLandsat:
    ...
    def __init__(self, fp):
        ...
        if not os.path.isdir(self.file_path):
            raise Exception("Provide  landsat band folders")
    ...
    def get_product_id(self):
        ...
        if not self.metadata:
            dir_path = os.path.dirname(self.file_path) if os.path.isfile(self.file_path) else self.file_path
    ...
    def get_mtl_metadata(self):
        ...
        """
        mtl_fp = f"{self.file_path}/{self.get_product_id()}_MTL.json"
    ...
    def get_stac_data(self):
        ...
        :return: stac json
        """
        stac_fp = f"{self.file_path}/{self.get_product_id()}_stac.json"
    ...
    def get_asset_list(self) -> list:
        return list(self.stac_data['assets'])
    ...
    def get_band_file_path(self, asset_name: str):
        ...
        return f"{self.file_path}/{asset_info.split('/')[-1]}"
    ...
    def get_band_name(self, asset_name):
        ...
        """
        return self.stac_data['assets'][asset_name]['eo:bands'][0]['name'] if "eo:bands" in self.stac_data['assets'][asset_name] else None
    ...
    def get_band_resolution(self, asset_name) -> float:
        ...
        """
        return self.stac_data['assets'][asset_name]['eo:bands'][0]['gsd'] if 'eo:bands' in self.stac_data['assets'][asset_name] else None
    ...
    def get_bands_meta_data_df(self):
        ...
        df = pd.DataFrame({"asset_name": data_assets, "band_name": band_names,
                           ...
                           "k1_thermal_const": k1_thermal_constant, "k2_thermal_const": k2_thermal_constant, "file_path": band_fps})
        return df
    ...
    def create_QC_map(self, ls_ref_raster: RioRaster) -> np.ndarray:
        ...
        if Landsat_nr in [5, 7]:
            ...
            mask_img = np.where(np.logical_or.reduce((ls_data == 0, ls_data_2 == 0, ls_data_3 == 0, ls_data_4 == 0, ls_data_5 == 0, ls_data_6 == 0, ls_data_7 == 0)), 0, 1)
    ...
    def calculate_TOA_reflectance_radiance(self, band, cos_zn: np.ndarray, ls_ref_raster: RioRaster, QC_Map=None, band_metadata_df=None):
        """
    ...
    def calculate_surface_temperature(self, ls_ref_raster: RioRaster, Temp_inst, b10_emissivity, act_vap_inst, water_mask, cloud_mask=None):
        """
        ...
        if Landsat_nr in [8, 9]:
            if bands_thermal == 1:
                ...
                L_lambda_b10 = (b_10.max_radiance.values[0] - b_10.min_radiance.values[0]) / (65535 - 1) * b_10_raster.get_data_array(1) + b_10.min_radiance.values[0]

                # Get Temperature
                surface_temp = self.get_thermal(L_lambda_b10, Temp_inst, b10_emissivity, b_10.k1_thermal_const.values[0], b_10.k2_thermal_const.values[0])
                ...
                L_lambda_b11 = (b_11.max_radiance.values[0] - b_11.min_radiance.values[0]) / (65535 - 1) * b_11_raster.get_data_array(1) + b_11.min_radiance.values[0]

                # Brightness temperature
                # From Band 10:
                Temp_TOA_10 = (b_10.k2_thermal_const.values[0] / np.log(b_10.k1_thermal_const.values[0] / L_lambda_b10 + 1.0))
                # From Band 11:
                Temp_TOA_11 = (b_11.k2_thermal_const.values[0] / np.log(b_11.k1_thermal_const.values[0] / L_lambda_b11 + 1.0))
                # Combined:
            ...
            L_lambda_b6 = (b_6.max_radiance.values[0] - b_6.min_radiance.values[0]) / (256 - 1) * b_6_raster.get_data_array(1) + b_6.min_radiance.values[0]
