...
from digitalarztools.propcessing.operations.transformation import TransformationOperations
...
class GPDVector(gpd.GeoDataFrame):
    ...
    @staticmethod
    def combine_files(dir_path, output_fp, driver):
        if driver == 'GPKG':
            ext = 'gpkg'
        else:
            ext = 'shp'
        files = FileIO.list_files_in_folder(dir_path, 'gpkg')
