...
import requests
from ML_management.mlmanagement.base_exceptions import ClientException
from ML_management.mlmanagement.singleton_pattern import Singleton
from ML_management.mlmanagement.variables import _get_server_ml_api, _get_server_url
...
class InvalidCredentials(ClientException):
    """Exception for invalid login-password pair."""

    pass


class MissedEnvVar(ClientException):
    """Exception for the case of the absence of the necessary environment variable."""

    pass


class InvalidEnvVar(ClientException):
    """Exception for the case of the invalid value of environment variable."""
...
class AuthSession(metaclass=Singleton):
    ...
    def __init__(self) -> None:
        ...
        if server_url is None:
            raise MissedEnvVar("You must specify environment variable server_url.")
        if not server_url.startswith("http"):
            raise InvalidEnvVar("$server_url must starts with http or https.")
        ...
        if not self._try_authenticate_by_credentials(login, password):
            raise InvalidCredentials(f"User with login {login} and password {password} does not exist.")
    ...
    def sgqlc_request(self, operation: Operation) -> dict:
        ...
        return HTTPEndpoint(posixpath.join(_get_server_url(), "graphql"), base_headers={"Cookie": cookie_header})(operation)
    ...
    def _try_authenticate_by_credentials(self, login, password) -> bool:
        if login is None or password is None:
            return False
        response = requests.post(posixpath.join(_get_server_url(), "oauth", "login"), data={"username": login, "password": password})
