...
class DLIHook(HuaweiBaseHook):
    ...
    def cancel_sql_job(self, job_id) -> DliSdk.CancelSqlJobResponse:
        ...
        """
        try:
            response = self.get_dli_client().cancel_sql_job(self.cancel_sql_job_request(job_id))
            return response
        except Exception as e:
            self.log.error(e)
    ...
    def cancel_batch_job(self, job_id) -> DliSdk.CancelBatchJobResponse:
        ...
        """
        try:
            response = self.get_dli_client().cancel_batch_job(self.cancel_batch_job_request(job_id))
            return response
        except Exception as e:
            self.log.error(e)
            raise AirflowException(f"Errors when cancel sql job result: {e}")
