...
def render_text_blocks(
    ...
    if align_all_blocks:
        ...
        widths = [[max(wcswidth(line) for line in i.lines) if i.width is None else i.width for i in r] for r in blocks]
    ...
    def get_widths(row: list[TextBlock], n: int = 0) -> list[int]:
        return [
            (max(wcswidth(line) for line in block.lines) if block.width is None else block.width) + n + padding
            for block in row
        ]
