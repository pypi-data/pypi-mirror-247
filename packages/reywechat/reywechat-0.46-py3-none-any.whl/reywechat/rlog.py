...
class RLog(object):
    ...
    def log_receive(
        self,
        message: RMessage,
        exc_data_list: Optional[List[ExceptionData]] = None
    ) -> None:
        """
        Log receive message.
        ...
        exc_data_list : Exception data list.
        """
        ...
        if exc_data_list in (None, []):
            level = self.rrlog.INFO
        else:
            level = self.rrlog.ERROR
            exc_report_list = [
                exc_report
                for exc_report, _, _, _ in exc_data_list
            ]
            exc_report = "\n".join(exc_report_list)
