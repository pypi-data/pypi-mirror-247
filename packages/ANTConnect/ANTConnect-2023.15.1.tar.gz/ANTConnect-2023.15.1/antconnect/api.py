...
class API:
    ...
    def _make_api_request(self, path: str, method: str, parameters: dict = None,
                          delete_data: dict = None, isFile: bool = False) -> Any:
    ...
    def tables_query_stream(self, project_id: str, queryBody):
        ...
        downloads_response = self._make_api_request(downloads_path, 'GET')
        for download in downloads_response:
            if query_response['batchId'] in download['name']:
                download_path = 'downloads/{}'.format(download['id'])
                download_response = self._make_api_request(download_path, 'GET', {'hardDelete': True}, isFile=True)
                data[download['name'].split('_')[0]] = download_response
            else:
                break
        return data
