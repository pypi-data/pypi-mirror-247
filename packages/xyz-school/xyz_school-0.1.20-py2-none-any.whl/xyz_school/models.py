...
class Teacher(CodeMixin, models.Model):
    ...
    user = models.OneToOneField(User, verbose_name="网站用户", null=True, blank=True, on_delete=models.PROTECT,
                                related_name="as_school_teacher")
...
class Session(models.Model):
    ...
    number = models.CharField("编号", max_length=16, unique=True)
    ...
    stage = models.PositiveSmallIntegerField("阶段", choices=choices.CHOICES_STAGE, default=choices.STAGE_PENDING)
    year = models.PositiveSmallIntegerField("年份", blank=True, default=2000)

    def save(self, **kwargs):
        if not self.name and self.number:
            self.name = "%s届" % self.number
        if self.name and not self.number:
            from .helper import get_similar_session_values_by_name
            for k, v in get_similar_session_values_by_name(self.name).items():
                setattr(self, k, v)
        self.year = year = int(str(self.number)[:4])
        if not self.begin_date:
            self.begin_date = "%s-08-01" % year
        if not self.end_date:
            self.end_date = "%s-07-31" % (year + 1)
        from .helper import current_stage
        if self.stage != choices.STAGE_INFORMAL:
            self.stage = current_stage(self.begin_date, self.end_date)
...
class Class(CodeMixin, models.Model):
    ...
    major = models.ForeignKey(Major, verbose_name=Major._meta.verbose_name, null=True, blank=True,
                              on_delete=models.PROTECT, related_name='classes', related_query_name='class')
    create_time = models.DateTimeField("创建时间", auto_now_add=True)
    modify_time = models.DateTimeField("修改时间", auto_now=True)
    is_active = models.BooleanField("有效", default=True)
    stage = models.PositiveSmallIntegerField("阶段", choices=choices.CHOICES_STAGE, default=choices.STAGE_PENDING)
    college = models.ForeignKey(College, verbose_name="院系", related_name="classes", related_query_name='class',
...
class ClassCourse(models.Model):
    ...
    begin_date = models.DateField('开始日期', null=True, blank=True)
    ...
    def student_user_ids(self):
        return list(self.clazz.students.values_list('user', flat=True))
...
class Student(CodeMixin, models.Model):
    ...
    stage = models.PositiveSmallIntegerField("阶段", choices=choices.CHOICES_STAGE, blank=True,
                                             default=choices.STAGE_PENDING)
