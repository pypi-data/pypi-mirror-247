...
def _is_1212_watermark(text: bytes):
    return bool(
        re.search(
            rb"Downloaded from https?://(.*) by .+ on \[\d{1,2}/\d{1,2}/\d{4}]", text
        )
    )
...
def _is_7566_watermark(text: bytes):
    return bool(re.search(rb'^.*?\nDownloaded from .*? by University of California on \s*\d+/\d+/\d+\s*$', text))
...
class BasePdfProcessor:
    ...
    def process(self, pdf_reader, pdf_writer, doi):
        ...
        for page_num, page in enumerate(pdf_reader.pages):
            ...
            current_text_content = page.extract_text().strip()
            if (
                (current_text_content and len(current_text_content) > 16)
                and (previous_text_content and len(previous_text_content) > 16)
                and current_text_content == previous_text_content
            ):
                continue
...
class WatermarkEraser4(BaseWatermarkEraser):
    def __init__(self, regexp, inverted=False, separator=""):
        super().__init__()
        self.regexp = regexp
...
pdf_processors = {
    ...
    "10.1212": WatermarkEraser4(doi1212_regexp, separator=""),
