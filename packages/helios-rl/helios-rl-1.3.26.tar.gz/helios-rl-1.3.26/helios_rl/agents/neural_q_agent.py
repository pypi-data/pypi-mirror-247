...
class StateEncNet(nn.Module):
    def __init__(self, input_size: int, output_size: int, 
                 sequence_size: int = 1, seq_hidden_dim: int = 10, 
                 hidden_dim: int = 128, num_hidden: int = 1):
    ...
    def forward(self, x):
        ...
        if (self.seq_size > 1):
            ...
            try:
                ...
                print('Data Type Error: Encoder must be a tensor(float32) \n For example use np.array([0,0,...], dtype="float32")')
...
class DQN:
    def __init__(self, sequence_size: int, input_size: int, output_size: int, action_space_index: Dict[str, int] = None, 
                 seq_hidden_dim: int = 10, hidden_dim: int = 128, num_hidden: int = 1, 
                 learn_step_counter: int = 0, memory_size: int = 2000, 
                 epsilon: float = 0.2, epsilon_step: float = 0.01,
                 target_replace_iter: int = 100, learning_rate: float = 0.001, batch_size: int = 1):
    ...
    def learn(self):
        ...
        try:
            ...
            if self.batch_size_enabled is False:
                print("Sufficient experience to use batch_size=" +str(self.batch_size)+" given by config inputs")
                self.batch_size_enabled = True
        except:
            if self.learn_step_counter == 1:
                print("Not enough experience for batch size, defaulting to batch_size=1 for now...")
...
class NeuralQLearningAgent(QLearningAgent):
    def __init__(self, sequence_size: int, input_size: int, output_size: int, learn_step_counter: int = 0, memory_size: int = 2000, 
                epsilon: float = 0.05, epsilon_step: float = 0.05, 
                seq_hidden_dim: int = 10, hidden_dim: int = 128, num_hidden: int = 1, action_space_index: Dict[str, int] = None, 
                target_replace_iter: int = 100, learning_rate: float = 0.001, batch_size: int = 1):
        self.dqn: DQN = DQN(sequence_size, input_size, output_size, action_space_index, 
                            seq_hidden_dim, hidden_dim, num_hidden, 
                            learn_step_counter, memory_size,
                            epsilon, epsilon_step,
                            target_replace_iter, learning_rate, batch_size)
