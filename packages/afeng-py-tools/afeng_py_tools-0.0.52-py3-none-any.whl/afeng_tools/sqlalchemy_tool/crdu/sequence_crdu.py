...
def update_sequence_value_to(sequence_name: str, to_value: int):
    """
    更新序列值到某个值
    ...
    :return:
    """
    now_value = get_sequence_next_value(sequence_name)
    for i in range(now_value, to_value + 1):
        result = get_sequence_next_value(sequence_name)
        if result == to_value:
            break
