...
class BaseSQLSource(BaseSQLSourceProtocol):
    ...
    def execute_query(
        ...
        if query_execution_parameters is None:
            """
            Sometimes, in tests or in development, we may not have the query_execution_parameters argument
            passed in. However, if this is invoked in engine, this argument should never be None.
            """
            query_execution_parameters = QueryExecutionParameters()
            _logger.warning(
                f"Query execution parameters should be set explicitly, "
                f"instantiating as default {query_execution_parameters}"
            )
        attempt_efficient_execution = query_execution_parameters.attempt_efficient_execution
        _logger.debug(f"execute_query: {attempt_efficient_execution=}")
    ...
    async def async_execute_query_inefficient(
        ...
        async with cnx_ctx as cnx:
            for _, temp_table_values, create_temp_table, temp_table in finalized_query.temp_tables.values():
