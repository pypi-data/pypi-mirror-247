...
class Eesrep:
    ...
    #@profile
    def __init__(self, direction:str = "Minimize", solver:str = "CBC"):
        ...
        solver : str, optional
            Solver/Python module name to use, choose among ["CBC", "Gurobi", "CPLEX", "Pyomo"] or the custom given interfaces. Defaults to "CBC".
        """
    ...
    def create_model_interface(self):
        ...
        """
        if self.__solver.lower() not in ["cbc", "gurobi", "docplex", "cplex", "pyomo"] + list(self.__custom_interfaces.keys()):
            raise ValueError(
                f"Solver name {self.__solver} is not implemented, please use: CBC, Gurobi or CPLEX")

        if self.__solver in self.__custom_interfaces:
            self.__model = self.__custom_interfaces[self.__solver](direction = self.__direction , solver = self.__solver)

        elif self.__solver.lower() == "cbc":
            self.__solver = "CBC"
            self.__model = MIPInterface(direction = self.__direction , solver = self.__solver)

        elif self.__solver.lower() == "gurobi":
            self.__solver = "GBI"
            self.__model = MIPInterface(direction = self.__direction , solver = self.__solver)

        elif self.__solver.lower() == "docplex":
            self.__solver = "DOCPLEX"
            self.__model = DocplexInterface(direction = self.__direction)

        elif self.__solver.lower() == "cplex":
            self.__solver = "CPLEX"
            self.__model = CplexInterface(direction = self.__direction)

        elif self.__solver.lower() == "pyomo":
            self.__model = PyomoInterface(direction = self.__direction, solver="cplex")
