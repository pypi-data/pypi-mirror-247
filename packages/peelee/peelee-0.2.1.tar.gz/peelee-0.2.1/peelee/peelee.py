...
def lighter(base_color, n_color):
    ...
    # to avoid the lighter color is becoming other colors, use the same _gap for r,g,b. _gap is the distance from the max value of r,g,b to 255.
    # when create lighter color, the increasement steps of r,g,b should be the same. if use 255 as the max value, then those smaller values in r,g,b will be becominig bigger than others when it's close to white. And finally, the color is becoming other colors.
...
def _generate_base_colors(min, max, total):
    ...
    # new solution - 2023.12.19(Stockholm.Kungsängen.TibbleTorg)
    diff = max - min
    step = diff // 12
    for index in range(1,12,4):
        random_int = random.randint(min+(index * step), min+((index+2) * step))
        _random_color = _padding(hex(random_int), 2)
        random_hex_color_code = random_hex_color_code + _random_color
...
class Palette:
    ...
    def __init__(
        ...
        Parameters:
            ...
            dark_base_colors - base colors for dark theme. Note: (1) the value of dark_base_colors is a list. (2) the value of dark_base_colors will not be used as color of pelette, only their lighter colors will be used. (3) if this parameter is given, then dark_colors_total will be ignored.
        """
...
def main():
    ...
    for _color in palette.generate_palette():
        # palette format is "C_[base color sequence]_[colormap sequence]":"#8f67ff", so hex code is 2nd element after split by ":"
        _hex = _color.split(":")[1]
