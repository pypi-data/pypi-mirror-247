...
from .helpers import (load_config, 
                      ...
                      Uploader, State)
from .avatars import from_betas, export, from_images, from_measurements, from_scans
...
@click.pass_context
@click.option('--config',
              type=click.Path(exists=True), default=DEFAULT_CONFIG, help="Path to config file")
@click.option(
    "--username",
...
def export_and_download_avatar(
    ...
    out_filename = str(out_file) if out_file is not None else \
        f"{get_timestamp()}_{name}.{format.lower()}" if name is not None else \
...
def user_info(ctx: click.Context) -> None:
    ...
    try:
        api_response_user = api_instance_user.describe_user()
        ...
        log.info(f"Username: {api_response_user.data.attributes.email}")
        log.info(f"Credits: {api_response_user.data.attributes.credits}")
    except Exception as e:
