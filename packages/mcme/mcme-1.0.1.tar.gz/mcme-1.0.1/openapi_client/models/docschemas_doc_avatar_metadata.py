...
class DocschemasDocAvatarMetadata(BaseModel):
    """
    DocschemasDocAvatarMetadata
    """
    body_shape: Optional[DocschemasDocBodyShape] = Field(None, alias="bodyShape")
    __properties = ["bodyShape"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> DocschemasDocAvatarMetadata:
        """Create an instance of DocschemasDocAvatarMetadata from a JSON string"""
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of body_shape
        if self.body_shape:
            _dict['bodyShape'] = self.body_shape.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> DocschemasDocAvatarMetadata:
        """Create an instance of DocschemasDocAvatarMetadata from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return DocschemasDocAvatarMetadata.parse_obj(obj)

        _obj = DocschemasDocAvatarMetadata.parse_obj({
            "body_shape": DocschemasDocBodyShape.from_dict(obj.get("bodyShape")) if obj.get("bodyShape") is not None else None
        })
        return _obj
