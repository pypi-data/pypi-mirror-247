...
def aggregate_metric_scores(metric_name, metric_value, metrics_result, task_type=None):
    ...
    if metric_name in constants.Metric.NON_SCALAR_RAG_EVALUATION_SET and \
...
def check_kwargs(kwargs: Dict, task_type: constants.Tasks, task_type_args: List[str]) -> None:
    ...
    """
    if "openai_params" in kwargs or "llm_params" in kwargs:
        kwargs.pop("openai_params", None)
        kwargs.pop("llm_params", None)
        warning_message = "We have found unused keyword arguments " \
                          "with the keys : openai_params or llm_params"
