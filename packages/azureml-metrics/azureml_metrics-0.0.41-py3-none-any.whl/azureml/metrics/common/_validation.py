...
def validate_classification(y_test: np.ndarray,
    ...
    """
    _validate_metrics_list("classification", metrics, constants.CLASSIFICATION_SET,
...
def validate_translation(y_test: List[Any],
    ...
    _validate_metrics_list("translation", metrics, constants.Metric.TRANSLATION_SET,
                           ReferenceCodes._METRIC_INVALID_TRANSLATION_METRIC)
...
def validate_summarization(y_test: List[Any],
    ...
    _validate_metrics_list("summarization", metrics, constants.Metric.SUMMARIZATION_SET,
                           ReferenceCodes._METRIC_INVALID_SUMMARIZATION_METRIC)
...
def validate_text_generation(y_test: List[Any],
    ...
    """
    _validate_metrics_list("text generation", metrics, constants.Metric.TEXT_GENERATION_SET,
                           ReferenceCodes._METRIC_INVALID_TEXT_GENERATION_METRIC)
...
def validate_code_generation(y_test: List[Any],
    ...
    """
    _validate_metrics_list("code generation", metrics, constants.Metric.CODE_GENERATION_SET,
                           ReferenceCodes._METRIC_INVALID_CODE_GENERATION_METRIC)
...
def log_code_generation_debug(y_test: List[Any],
    ...
    debug_data = {
        ...
        'y_pred_length': len(y_pred),
        'metrics': metrics,
        'test_cases_length': len(test_cases) if test_cases is not None else 0,
...
def validate_qa(y_test: List[Any],
    ...
    """
    _validate_metrics_list("QA", metrics, constants.Metric.QA_SET, ReferenceCodes._METRIC_INVALID_QA_METRIC)
...
def validate_qa_multiple_ground_truth(y_test: List[Any],
                                      y_pred: List[str],
                                      metrics: List[str],
                                      tokenizer: Any,
                                      regexes_to_ignore: List[str],
    ...
    :params ignore_numbers: Boolean to indicate whether to ignore numbers
    """
    _validate_metrics_list("QA Multiple Ground Truth", metrics, constants.Metric.QA_MULTIPLE_GROUND_TRUTH_SET,
                           ReferenceCodes._METRIC_INVALID_QA_METRIC)
    for metric in metrics:
        _check_seq2seq_list_of_list_of_str(y_test, 'y_test', reference_code='validate_qa_multiple_ground_truth')
        Contract.assert_true(
            len(y_test) == len(y_pred), 'Number of samples in y_test and y_pred do not match',
            log_safe=True, reference_code='validate_qa_multiple_ground_truth', target='y_test')

    _check_seq2seq_list_of_str(y_pred, 'y_pred', reference_code='validate_qa_multiple_ground_truth')
    if tokenizer:
        _check_seq2seq_tokenizer(tokenizer, 'tokenizer', reference_code='validate_qa_multiple_ground_truth')
    if regexes_to_ignore:  # if regexes to ignore is provided, it should be a list of string
        _check_seq2seq_list_of_str(regexes_to_ignore, 'regexes_to_ignore',
                                   reference_code='validate_qa_multiple_ground_truth')
    _check_seq2seq_bool(ignore_case, 'ignore_case', reference_code='validate_qa_multiple_ground_truth')
    _check_seq2seq_bool(ignore_punctuation, 'ignore_punctuation', reference_code='validate_qa_multiple_ground_truth')
    _check_seq2seq_bool(ignore_numbers, 'ignore_numbers', reference_code='validate_qa_multiple_ground_truth')


def log_qa_mutliple_ground_truth_debug(y_test: List[Any],
                                       y_pred: List[str],
                                       tokenizer: Any,
                                       regexes_to_ignore: List[str],
                                       ignore_case: bool,
                                       ...
                                       ignore_numbers: bool) -> None:
    """
    Log shapes of QA with multiple ground truth, inputs for debugging.
    ...
    logger.info("QA Mutiplr Ground Truth metrics debug: {}".format(debug_data))
...
def validate_fill_mask(y_test: List[Any],
    ...
    :param add_start_token: (bool) whether to add the start token to the texts,
        so the perplexity can include the probability of the first word. Defaults to True.
    """
    _validate_metrics_list("Fill Masking", metrics, constants.Metric.FILL_MASK_SET,
                           ReferenceCodes._METRIC_INVALID_FILL_MASK_METRIC)
    for metric in metrics:
...
def validate_ner(y_test: Union[List[List[str]], np.ndarray],
    ...
    :param metrics: Metrics to compute.
    """
    _validate_metrics_list("Text NER", metrics, constants.CLASSIFICATION_NLP_NER_SET,
                           ReferenceCodes._METRIC_INVALID_NER_METRIC)
