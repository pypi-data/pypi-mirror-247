...
from heaserver.folderawss3.util import set_file_source
from .util import response_folder_as_zip, move
        ...
        async with mongo.MongoContext(request=request) as mongo_client:
            async with aws.S3ClientContext(request=request, volume_id=volume_id) as s3:
                try:
                    ...
                    for obj in chain(iter(response_.get('Contents', [])), iter(response_.get('CommonPrefixes', []))):
...
async def post_folder_archive_async(request: web.Request) -> web.Response:
    ...
    if not await _is_folder(request, volume_id, bucket_name, id_):
        return response.status_not_found()
    return await awsservicelib.archive_object_async(request, f'{request.url}status', activity_cb=publish_desktop_object)
...
async def post_folder_duplicator_async(request: web.Request) -> web.Response:
    ...
    _status_id += 1
    return await awsservicelib.copy_object_async(request, status_location, activity_cb=publish_desktop_object)
...
async def delete_item(request: web.Request) -> web.Response:
    ...
    if async_requested:
        ...
        response_ = await awsservicelib.delete_object_async(request, status_location, recursive=True, activity_cb=publish_desktop_object, done_cb=done)
    else:
    ...
    async with DesktopObjectActionLifecycle(request=request,
        ...
        async with aws.S3ClientContext(request=request, volume_id=volume_id) as s3_client:
            try:
                ...
                urls = []
                async def objects():
                    async for obj in awsservicelib.list_objects(s3_client, bucket_id, prefix=object_key, loop=loop):
                        # only allow Standard Storage and no duplicate version, and ignore the folder itself as well as subfolders.
                        if not awsservicelib.is_folder(obj['Key']) and obj.get("StorageClass") == S3StorageClass.STANDARD.name:
                            yield obj
                ...
                return awsservicelib.handle_client_error(e)
            if not urls:
                return response.status_bad_request('There are no files in this folder, or none are in Standard storage.')
        ...
        async with mongo.MongoContext(request) as mongo_client:
            ...
            if key != folder.key:
                ...
                async with DesktopObjectActionLifecycle(request,
                    ...
                    async with aws.S3ClientContext(request=request, volume_id=volume_id) as s3_client:
                        try:
                            await move(s3_client=s3_client, source_bucket_id=bucket_id, source_key=key, target_bucket_id=bucket_id, target_key=folder.key)
                            await mongo_client.delete_admin(MONGODB_AWS_S3_FOLDER_METADATA_COLLECTION, {'bucket_id': bucket_id, 'encoded_key': encode_key(awss3key.join(folder.key, awss3key.split(key)[1]))})
                        except BotoClientError as e:
                            activity.status = Status.FAILED
                            return awsservicelib.handle_client_error(e)
...
async def _get_items_in_container(request: web.Request, volume_id: str, bucket_id: str, folder_id: str) -> web.Response:
    ...
    :param bucket_id: the name of the bucket (required).
    :param folder_id: the AWS folder key, base64-encoded (required). The root of a bucket is denoted by 'root'.
    """
    logger = logging.getLogger(__name__)
    sub = request.headers.get(SUB, NONE_USER)
    async def coro():
            ...
            async with DesktopObjectActionLifecycle(request,
                ...
                async with mongo.MongoContext(request=request) as mongo_client:
                    try:
                        ...
                        new_object_uri_prefix = f'volumes/{volume_id}/buckets/{bucket_id}/awss3folders/'
                        item = AWSS3ItemInFolder()
                        async def get_metadata(bid, fid) -> dict[str, DesktopObjectDict]:
                            ...
                            async for item_ in mongo_client.get_all_admin(MONGODB_AWS_S3_FOLDER_METADATA_COLLECTION, {'bucket_id': bid, 'parent_encoded_key': fid}):
                                metadata[item_['encoded_key']] = item_
                            return metadata
                        metadata_task = asyncio.create_task(get_metadata(bucket_id, folder_id))
                        async for obj in awsservicelib.list_objects(s3, bucket_id=bucket_id, prefix=folder_key, loop=loop, delimiter='/', include_restore_status=True):
                        ...
                        await metadata_task
                        activity.new_object_uri = f'{new_object_uri_prefix}'
                        activity.new_object_type_name = AWSS3Folder.get_type_name()
                        activity.new_volume_id = volume_id
