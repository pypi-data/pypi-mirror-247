...
class MonteCarloConfigService:
    ...
    def _init_local_config(self):
        ...
        self.project_config, self.project_config_file_name = self._load_project_config()
        return
            ...
            if response.resource_modifications:
                ...
                for resource_modification in response.resource_modifications:
                    ...
                    self._print_func(
                        ...
                        f"\nchanges detected: "
                        f"\n{'_' * 100} "
                        f"\n{resource_modification.diff_string if resource_modification.diff_string else ''}"
    ...
    def compile(
        ...
        if self.project_config.namespace:
            self._print_func(
                f"namespace: {self.project_config.namespace} found in {self.project_config_file_name} ignoring value passed with --namespace={apply_namespace}"
            )
    ...
    def _load_project_config(self) -> (Optional[ProjectConfig], str):
        contents = None
        project_config_file_name = PROJECT_CONFIG_FILENAME_YML
        try:
            try:
                with open(
                    os.path.join(self.project_dir, PROJECT_CONFIG_FILENAME_YML), "r"
                ) as f:
                    contents = f.read()
            except FileNotFoundError:
                with open(
                    os.path.join(self.project_dir, PROJECT_CONFIG_FILENAME_YAML), "r"
                ) as f:
                    contents = f.read()
        ...
        try:
            ...
            complain_and_abort(
                f"Encountered a validation problem in {project_config_file_name}: "
                f"{e.messages}"
            )
    ...
    def convert_to_ui(self, namespace: str, dry_run: bool = False):
        if not dry_run:
            if not click.confirm(
                ...
                "If you proceed, your monitors will become editable through the UI and applying the monitor configuration "
        ...
        response = self._pycarlo_client(
            convert_to_ui, {"namespace": namespace, "dryRun": dry_run}
        )

        monitors = response.convert_config_template_to_ui_monitors.response.monitors
        if not monitors:
            complain_and_abort(
        ...
        for (
            monitor
        ) in response.convert_config_template_to_ui_monitors.response.monitors:
    ...
    def convert_to_mac(
        ...
        if all_monitors and monitors_file:
            complain_and_abort(
                "You cannot export all monitors and provide a monitors file at "
                "the same time."
            )
        ...
        response = self._pycarlo_client(
            ...
            {
                "namespace": namespace,
                "monitorUuids": [str(m) for m in monitors_uuids]
                if monitors_uuids
                else None,
        ...
        if graphql_errors:
            graphql_errors_str = "\n".join(graphql_errors)
            complain_and_abort(
                f"Errors encountered during export:\n{graphql_errors_str}"
            )
            return

        errors = response.convert_ui_monitors_to_config_template.response.errors
        if errors:
            errors_str = "\n".join(errors)
        ...
        self._export_project(
            project_dir,
            namespace,
            response.convert_ui_monitors_to_config_template.response.config_template_as_dict,
        )
    ...
    def get_template(self, namespace: str, project_dir: str):
        ...
        if not response.get_monte_carlo_config_templates.edges:
        ...
        self._export_project(
            project_dir,
            namespace,
            response.get_monte_carlo_config_templates.edges[0].node.template,
        )
            ...
            for line in f.readlines():
                ...
                if line:
                    try:
                        ...
                        complain_and_abort(
                            f"Invalid UUID in monitors file on line {line_number}"
                        )

        if not monitor_uuids:
            complain_and_abort(
                f"File {monitors_file} does not contain any valid monitors."
            )
