...
from montecarlodata.common.common import read_as_json_string, ConditionalDictionary
...
class AgentService:
    ...
    def __init__(
        ...
        self._ecr_repo = config.mcd_agent_ecr_repo
        self._ecr_registry_id = config.mcd_agent_ecr_registry_id
        self._ecr_image_name = (
            f"{self._ecr_registry_id}.dkr.ecr.*.amazonaws.com/{self._ecr_repo}"
        )
        ...
        if validation_result.errors:
            ...
            for error in validation_result.errors:
                ...
                if settings.MCD_VERBOSE_ERRORS and error.stack_trace:
                    ...
                    error_output[
                        "stack_trace"
                    ] = f"Stack trace written to {stack_trace_filename}"
    ...
    def check_agent_health(self, **kwargs):
        ...
        if agent_result.success:
            if agent_result.additional_data.returned_data:
                click.echo(
                    json.dumps(agent_result.additional_data.returned_data, indent=4)
                )
            return click.echo("Agent health check succeeded!")
    ...
    @manage_errors
    def echo_gcp_logs(self, **kwargs):
        self._get_logs_and_echo("get_gcp_agent_logs", **kwargs)

    @manage_errors
    def echo_aws_logs(self, **kwargs):
        self._get_logs_and_echo("get_aws_agent_logs", **kwargs)
    ...
    def echo_aws_upgrade_logs(self, start_time: Optional[str], **kwargs):
        ...
        parameters["start_time"] = (
            start_time
            or (datetime.now() - timedelta(hours=12))
    ...
    def _get_logs_and_echo(self, method_name: str, **kwargs):
        parameters = ConditionalDictionary(lambda x: x is not None)
        parameters.update(kwargs)  # don't send null values
        logs_query = Query()
        getattr(logs_query, method_name)(**parameters)
        result = self._mc_client(
            logs_query,
            idempotent_request_id=str(uuid.uuid4()),
            timeout_in_seconds=40,  # let Monolith timeout first
        )
        logs_results = getattr(result, method_name)
    ...
    def _choose_image(
        ...
        platform = self._user_service.get_agent(agent_id).get("platform")
        use_aws_ecr = platform == "AWS"

        if not image_tag or not self._validate_tag(image_tag, use_aws_ecr):
            versions = self._get_recent_versions(platform)
            image_tag = (
                versions[0]
                if not image_tag
    ...
    def _validate_tag(self, image_tag: str, use_aws_ecr: bool) -> bool:
        if use_aws_ecr:
            return self._aws_wrapper.ecr_image_exists(
                registry_id=self._ecr_registry_id,
                repository_name=self._ecr_repo,
                image_tag=image_tag,
            )
    ...
    def _get_recent_versions(self, platform: str) -> List[str]:
        if platform == "AWS":
            return self._aws_wrapper.list_ecr_image_tags(
                registry_id=self._ecr_registry_id,
                repository_name=self._ecr_repo,
                limit=DEFAULT_TAGS_PAGE_SIZE,
            )
        image_path = f"{self._image_org}/repositories/{self._image_repo}"
        url = urljoin(
            DOCKER_HUB_TAGS_BASE_URL,
            f"{image_path}/tags?page_size={DEFAULT_TAGS_PAGE_SIZE}&page=1",
        )
