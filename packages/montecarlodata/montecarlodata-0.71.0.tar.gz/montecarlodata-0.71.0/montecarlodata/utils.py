...
from montecarlodata.errors import manage_errors, complain_and_abort, abort_on_gql_errors


class Wrapper:
    def __init__(
        self,
        abort_on_error: Optional[bool] = True,
...
class GqlWrapper(Wrapper):
    ...
    def make_request_v2(
        ...
        """
        Make a GraphQl request to the MCD API and parse out the results into data and errors. Aborts on error if set.
        """
        response = self._make_request(
            query=query,
            variables=self.convert_snakes_to_camels(variables)
            if variables
            else variables,
        )
    ...
    def convert_snakes_to_camels(self, dict_: Dict) -> Dict:
        """
        ...
        for k, v in dict_.items():
            ...
            if isinstance(v, Dict):
                v = self.convert_snakes_to_camels(v)
            converted[k] = v
        return converted
    ...
    def _make_request(
        ...
        if settings.MCD_USER_ID_HEADER:
            headers = {
                "user-id": settings.MCD_USER_ID_HEADER
...
class AwsClientWrapper(Wrapper):
    ...
    def get_stack_details(self, stack_id: str) -> Dict:
        ...
        """
        return self._session.client("cloudformation").describe_stacks(
            StackName=stack_id
        )
    ...
    def create_stack(
        ...
        response = self._session.client("cloudformation").create_stack(
            **deployment_options
        )
    ...
    @manage_errors
    def upgrade_stack(
        self, stack_id: str, template_link: str, parameters: List[Dict]
    ) -> bool:
        """
    ...
    def deploy_gateway(self, gateway_id: str, stage: Optional[str] = "prod") -> None:
        ...
        """
        self._session.client("apigateway").create_deployment(
            restApiId=gateway_id, stageName=stage
        )
    ...
    @manage_errors
    def create_role(
        self, role_name: str, trust_policy: str, tags: Optional[List] = None
    ) -> str:
        """
    ...
    def attach_inline_policy(
        self, role_name: str, policy_name: str, policy_doc: str
    ) -> str:
        """
    ...
    def get_bucket_event_config(
        ...
        """
        return self._session.client("s3").get_bucket_notification_configuration(
            Bucket=name
        )
    ...
    def list_topic_subscriptions(self, arn: str) -> List:
        """
    ...
    def subscribe_to_topic(
        ...
        self._session.client("sns").subscribe(
            TopicArn=arn, Endpoint=endpoint, Protocol=protocol, Attributes=attributes
        )
    ...
    @manage_errors
    def _get_session(
        self, profile_name: str, region_name: str
        ...
        return boto3.session.Session(
            **{k: v for k, v in session_args if v}
    ...
    def describe_routes(
        ...
        filter_key: Optional[str] = "association.subnet-id",
    ) -> Box:
    ...
    def get_athena_workgroup(self, workgroup: str):
        return self._session.client("athena").get_work_group(WorkGroup=workgroup)[
            "WorkGroup"
        ]

    @manage_errors
    def ecr_image_exists(
        self, registry_id: str, repository_name: str, image_tag: str
    ) -> bool:
        response = self._session.client("ecr").batch_get_image(
    ...
    @manage_errors
    def list_ecr_image_tags(
        self, registry_id: str, repository_name: str, limit: int
    ) -> List[str]:
        response = self._session.client("ecr").describe_images(
            registryId=registry_id,
            repositoryName=repository_name,
            maxResults=limit,
            filter={"tagStatus": "TAGGED"},
        )
        results = response["imageDetails"] if "imageDetails" in response else []
        if not results:
            return []

        results.sort(key=lambda t: t["imagePushedAt"].isoformat(), reverse=True)
        return [
            tag for result in results for tag in result["imageTags"] if tag != "latest"
        ]
