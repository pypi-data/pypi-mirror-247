...
@agents.command(
    help="Register a Data Store Agent with remote Azure Blob storage container."
)
@click.pass_obj
@click.option(
    "--connection-string",
    help=f"A connection string to an Azure Storage account. If you prefer a prompt (with hidden input) enter {settings.SHOW_PROMPT_VALUE}.",
    required=True,
    cls=AdvancedOptions,
...
@agents.command(name="gcp-logs", help="Returns the logs for a remote GCP Agent.")
@click.pass_obj
@add_common_options(_AGENT_LOGS_COMMON_OPTIONS)
@click.option(
    "--filter",
    "logs_filter",
    help='Optional filter, for example: \'timestamp >= "2023-12-03T13:45:00Z" SEARCH("text")\', for syntax '
    "see https://cloud.google.com/logging/docs/view/advanced_filters",
    required=False,
)
def get_gcp_logs(ctx, **kwargs):
    """
    Displays the logs for a remote GCP Agent.
    """
    AgentService(
        config=ctx["config"],
        mc_client=create_mc_client(ctx),
    ).echo_gcp_logs(**kwargs)


@agents.command(name="aws-logs", help="Returns the logs for a remote AWS Agent.")
@click.pass_obj
@add_common_options(_AGENT_LOGS_COMMON_OPTIONS)
@click.option(
    "--start-time",
...
@click.option(
    "--pattern",
    help="Optional search pattern, for example: '%text%', for syntax see: "
    "https://docs.aws.amazon.com/AmazonCloudWatch/latest/logs/FilterAndPatternSyntax.html",
    required=False,
)
def get_aws_logs(ctx, **kwargs):
    """
    Displays the logs for a remote AWS Agent.
    """
    AgentService(
        config=ctx["config"],
        mc_client=create_mc_client(ctx),
    ).echo_aws_logs(**kwargs)


@agents.command(
    name="aws-upgrade-logs", help="Returns the upgrade logs for a remote AWS Agent."
)
@click.pass_obj
@click.option("--agent-id", help="UUID of Agent.", required=True)
@click.option(
...
@agents.command(
    name="aws-template-parameters",
    help="Displays the current CloudFormation template parameters in use by an AWS Agent.",
)
@click.pass_obj
@click.option("--agent-id", help="UUID of Agent.", required=True)
