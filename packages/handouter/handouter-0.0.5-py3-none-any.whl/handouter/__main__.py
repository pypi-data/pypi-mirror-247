...
class HandoutGenerator:
    ...
    def make_tikzbox(self, block):
        ...
        fontsize = "\\fontsize{FSpt}{LHpt}\\selectfont ".replace(
            "FS", str(fs)
        ).replace("LH", str(math.floor(fs * 1.2)))
        return (
    ...
    def generate_regular_block(self, block_):
        ...
        if self.args.debug:
            print(f"columns: {columns}, boxwidth: {boxwidth}, total width: {total_width}")
        boxwidthinner = boxwidth - 2 * self.args.tikz_mm
        header = [
