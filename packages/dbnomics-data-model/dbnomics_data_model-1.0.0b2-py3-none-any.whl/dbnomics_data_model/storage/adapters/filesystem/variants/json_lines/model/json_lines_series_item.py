...
class JsonLinesSeriesItem(BaseSeriesJson):
    ...
    def to_domain_model(
        self, *, check_series_dimensions_exist: bool = True, dataset_dimensions: DatasetDimensions | None = None
    ) -> Series:
    ...
    def _parse_attribute_codes(self, header_json: ObservationJson) -> list[str]:
        attribute_codes_json = header_json[2:]
    ...
    def _parse_attributes(self, observation_json: ObservationJson, *, all_attribute_codes: list[str]) -> dict[str, str]:
        attribute_value_codes_json = observation_json[2:]
        ...
        return dict(zip(all_attribute_codes, cast(list[str], attribute_value_codes_json), strict=True))
