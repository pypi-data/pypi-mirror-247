...
from dbnomics_data_model.cli.revision_utils import iter_revision_pairs
...
from dbnomics_data_model.json_utils import load_json_object_from_file, save_json_file
from dbnomics_data_model.json_utils.errors import JsonError, JsonParseError
from dbnomics_data_model.json_utils.loading import load_json_array_from_file
...
from dbnomics_data_model.storage.adapters.filesystem.errors.provider_directory_manager import (
    ProviderJsonCodeMismatch,
)
...
class ProviderDirectoryManager:
    def __init__(
        ...
        *,
        ...
        if git_provider_directory_manager is None:
            ...
            except GitProviderDirectoryManagerError:
                logger.debug("Could not find a Git repository in the provider directory, revisions will be disabled")
    ...
    def iter_dataset_codes(self, *, revision_id: RevisionId | None = None, sort: bool = True) -> Iterator[DatasetCode]:
        for dataset_code, _ in self.iter_dataset_directories(revision_id=revision_id, sort=sort):
            yield dataset_code

    def iter_dataset_directories(
        self, *, revision_id: RevisionId | None = None, sort: bool = True
    ) -> Iterator[tuple[DatasetCode, Path]]:
        ...
        yield from sorted(child_directories_iter) if sort else child_directories_iter
    ...
    def iter_series(
        ...
        *,
        ...
        dataset_directory_manager = self.create_dataset_directory_manager(dataset_code, revision_id=revision_id)
        yield from dataset_directory_manager.iter_series(
    ...
    def iter_series_changes(
        ...
        *,
        ...
        for repo_revision, parent_repo_revision in iter_revision_pairs(
            ...
            logger.debug(
                "Determining if the series %r changed at revision %r",
                str(series_id),
                repo_revision.id,
            )
            ...
            try:
                series = dataset_directory_manager.load_series(
                    series_code, check_series_dimensions_exist=check_series_dimensions_exist
                )
            ...
            if isinstance(parent_repo_revision, Revision):
                ...
                try:
                    previous_series = previous_dataset_directory_manager.load_series(
                        series_code, check_series_dimensions_exist=check_series_dimensions_exist
                    )
                except SeriesNotFound:
                    logger.debug(
    ...
    def load_series(
        ...
        *,
        ...
        dataset_directory_manager = self.create_dataset_directory_manager(dataset_code, revision_id=revision_id)
        return dataset_directory_manager.load_series(
            series_code,
            check_series_dimensions_exist=check_series_dimensions_exist,
            with_observations=with_observations,
        )
    ...
    def save_provider_json(self, provider_json: ProviderJson) -> None:
        ...
        try:
            ...
            raise ProviderJsonSaveError(
                ...
                provider_json_path=provider_json_path,
            ) from exc
    ...
    def _iter_dataset_directories(self) -> Iterator[tuple[DatasetCode, Path]]:
        for child_dir in iter_child_directories(self.provider_dir):
    ...
    def _load_category_tree_json_data_from_file(self) -> list[Json] | None:
        category_tree_json_path = self.category_tree_json_path

        try:
            return load_json_array_from_file(category_tree_json_path)
    ...
    def _load_category_tree_json_data_from_git(self, *, revision_id: RevisionId) -> list[Json] | None:
        if self._git_provider_directory_manager is None:
            raise RevisionsNotAvailable
        ...
        try:
            return self._git_provider_directory_manager.load_file_as_json_array(
                category_tree_json_path, revision_id=revision_id
            )
        except BlobNotFound:
            return None
        except JsonParseError as exc:
    ...
    def _load_provider_json_data_from_file(self) -> JsonObject:
        provider_json_path = self.provider_json_path

        try:
            return load_json_object_from_file(provider_json_path)
        except FileNotFoundError as exc:
            raise ProviderJsonNotFound(provider_code=self.provider_code, provider_json_path=provider_json_path) from exc
        except JsonParseError as exc:
            raise ProviderJsonLoadError(
    ...
    def _load_provider_json_data_from_git(self, *, revision_id: RevisionId) -> JsonObject:
        if self._git_provider_directory_manager is None:
            raise RevisionsNotAvailable
        ...
        try:
            return self._git_provider_directory_manager.load_file_as_json_object(
                provider_json_path, revision_id=revision_id
            )
    ...
    def _load_releases_json_data_from_file(self) -> JsonObject | None:
        releases_json_path = self.releases_json_path

        try:
            return load_json_object_from_file(releases_json_path)
        except FileNotFoundError:
            return None
        except JsonParseError as exc:
            raise ReleasesJsonLoadError(
    ...
    def _load_releases_json_data_from_git(self, *, revision_id: RevisionId) -> JsonObject | None:
        if self._git_provider_directory_manager is None:
            raise RevisionsNotAvailable
        ...
        try:
            return self._git_provider_directory_manager.load_file_as_json_object(
                releases_json_path, revision_id=revision_id
            )
        except BlobNotFound:
            return None
        except JsonParseError as exc:
