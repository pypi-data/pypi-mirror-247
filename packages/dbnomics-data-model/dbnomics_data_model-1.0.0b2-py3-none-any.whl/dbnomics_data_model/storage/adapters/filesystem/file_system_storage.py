...
from .variants.json_lines.offsets.repo import (
    JsonLinesSeriesOffsetRepo,
)
...
class FileSystemStorage(Storage):
    ...
    def iter_dataset_codes(
        self, provider_code: ProviderCode, *, revision_id: RevisionId | None = None, sort: bool = True
    ) -> Iterator[DatasetCode]:
        provider_directory_manager = self.create_provider_directory_manager(provider_code)
        yield from provider_directory_manager.iter_dataset_codes(revision_id=revision_id, sort=sort)
    ...
    def iter_provider_codes(
        self, *, revision_id: RevisionId | None = None, sort: bool = True
    ) -> Iterator[ProviderCode]:
        for provider_dir in self.iter_provider_directories(sort=sort):
            ...
            try:
                ...
                logger.exception(
                    "The child directory %s was not recognized as containing provider data, ignoring",
                    str(provider_dir),
                )
                continue
    ...
    def iter_provider_directories(self, *, sort: bool = True) -> Iterator[Path]:
        child_directories_iter = self._iter_child_directories(ignore_hidden=True)
    ...
    def iter_series(
        ...
        *,
        ...
        provider_directory_manager = self.create_provider_directory_manager(dataset_id.provider_code)
        yield from provider_directory_manager.iter_series(
    ...
    def iter_series_changes(
        ...
        return provider_directory_manager.iter_series_changes(
            series_id.series_code, dataset_code=series_id.dataset_code, start_revision_id=start_revision_id
        )
    ...
    def _iter_child_directories(self, *, ignore_hidden: bool = False) -> Iterator[Path]:
        for child_dir in iter_child_directories(self.storage_dir, ignore_hidden=ignore_hidden):
