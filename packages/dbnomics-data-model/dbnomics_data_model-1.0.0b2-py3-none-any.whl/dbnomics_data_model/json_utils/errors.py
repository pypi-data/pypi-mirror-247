...
class JsonBytesWriteError(JsonError):
    def __init__(self, *, data: Json, file_path: Path, serialized_data: bytes) -> None:
        msg = f"Could not write bytes to {file_path}"
        super().__init__(msg=msg)
        self.data = data
...
class JsonLineParseError(JsonParseError):
    def __init__(self, *, json_file_path: Path, line: bytes) -> None:
        msg = f"Could not parse line of {json_file_path} as JSON data"
        super().__init__(msg=msg)
...
class JsonParseTypeError(JsonParseError):
    def __init__(self, *, data: Json, expected_type: type) -> None:
        msg = f"Expected data type to be {expected_type!r} but got {type(data)!r}"
        super().__init__(msg=msg)
        self.data = data
