...
from dbnomics_data_model.model.periods.periods import (
    BimesterPeriod,
    DayPeriod,
    MonthPeriod,
    Period,
...
def to_month_num(value: str) -> Parser:
    return between(value, min_value=1, max_value=12, name="month num")
...
bimester_num = regex(r"[1-6]").map(int).desc("bimester num")
...
quarter_num = regex(r"[1-4]").map(int).desc("quarter num")
semester_num = regex(r"[1-2]").map(int).desc("semester num")
