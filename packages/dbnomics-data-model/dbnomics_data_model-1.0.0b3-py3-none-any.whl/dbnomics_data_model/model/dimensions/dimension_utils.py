...
from dbnomics_data_model.model.errors.dataset_dimensions import UndefinedDimension, UndefinedDimensionValue
...
__all__ = ["add_missing_dataset_dimensions", "check_series_dimensions_exist"]
    ...
    *,
    ...
    for dimension_code in dimension_codes:
        ...
        if dimension is None:
            ...
            logger.debug("Adding missing dimension %r to dataset", dimension)
        ...
        if dimension_value is None:
            ...
            logger.debug("Adding missing value %r to dimension %r of dataset", dimension_value, dimension)
            dimension.values.append(dimension_value)


def check_series_dimensions_exist(
    dimensions: dict[DimensionCode, DimensionValueCode], *, dataset_dimensions: DatasetDimensions
) -> None:
    for dimension_code, dimension_value_code in dimensions.items():
        dimension = dataset_dimensions.find_dimension_by_code(dimension_code)
        if dimension is None:
            raise UndefinedDimension(dataset_dimensions=dataset_dimensions, dimension_code=dimension_code)

        dimension_value = dimension.find_value_by_code(dimension_value_code)
        if dimension_value is None:
            raise UndefinedDimensionValue(
