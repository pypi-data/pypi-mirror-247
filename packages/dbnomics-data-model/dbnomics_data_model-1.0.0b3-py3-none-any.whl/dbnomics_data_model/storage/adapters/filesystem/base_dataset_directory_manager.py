...
class BaseDatasetDirectoryManager(ABC, Generic[TDatasetJson]):
    ...
    def has_series(self, series_code: SeriesCode) -> bool:
        try:
            self.load_series(series_code, check_series_dimensions_exist=False, with_observations=False)
    ...
    def load_dataset_json(self, *, validate: bool = False) -> TDatasetJson:
        ...
        if validate:
            validation_errors = list(dataset_json.validate())
    ...
    def load_dataset_metadata(self) -> DatasetMetadata:
        ...
        except (DatasetJsonLoadError, DatasetJsonValidationError) as exc:
    ...
    def load_series(
        self, series_code: SeriesCode, *, check_series_dimensions_exist: bool = True, with_observations: bool = True
    ) -> Series:
        series_id = SeriesId.from_dataset_id(self.dataset_id, series_code)
        series_iter = self.iter_series(
    ...
    def _load_dataset_json_data_from_file(self, dataset_json_path: Path) -> JsonObject:
        if self.revision_id is not None:
            raise LatestRevisionOnlyOperation(revision_id=self.revision_id)

        try:
            return load_json_object_from_file(dataset_json_path)
    ...
    def _load_dataset_json_data_from_git(self, dataset_json_path: Path) -> JsonObject:
        ...
        try:
            return self._git_provider_directory_manager.load_file_as_json_object(
                dataset_json_path, revision_id=self.revision_id
            )
...
def create_dataset_directory_manager(
    *,
    ...
    git_provider_directory_manager: GitProviderDirectoryManager,
