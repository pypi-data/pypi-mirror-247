...
from dbnomics_data_model.json_utils import JsonObject, parse_json_bytes_as_object
from dbnomics_data_model.json_utils.errors import JsonParseError
from dbnomics_data_model.json_utils.parsing import parse_json_bytes_as_array
...
class GitProviderDirectoryManager:
    ...
    def iter_dataset_directories(self, *, revision_id: RevisionId | None = None) -> Iterator[tuple[DatasetCode, Path]]:
        ...
        logger.debug(
            "Iterating over dataset directories from Git repository %s at revision %s",
            str(self.repo_dir),
            revision_id,
        )
        ...
        for tree in self._iter_trees(commit.tree):
            ...
            if dir_name is None:
                logger.debug("Ignoring sub-tree without name: %r", tree)
                continue
    ...
    def load_file_as_json_array(self, file: Path | str, *, revision_id: RevisionId | None = None) -> list[Json]:
        if revision_id is None:
            revision_id = DEFAULT_REVISION
        ...
        try:
            data = parse_json_bytes_as_array(blob_bytes)
    ...
    def load_file_as_json_object(self, file: Path | str, *, revision_id: RevisionId | None = None) -> JsonObject:
        if revision_id is None:
            revision_id = DEFAULT_REVISION
        ...
        try:
            data = parse_json_bytes_as_object(blob_bytes)
        except JsonParseError as exc:
            raise BlobParseError(
                blob_bytes=blob_bytes, file_relative_path=file_relative_path, repo=self._repo, revision_id=revision_id
            ) from exc
    ...
    def _iter_commits(self, start_revision_id: RevisionId | None = None) -> Iterator[Commit]:
        ...
        logger.debug(
            "Iterating over commits of Git repository %s starting from commit %s",
            str(self.repo_dir),
            start_revision_id,
        )
