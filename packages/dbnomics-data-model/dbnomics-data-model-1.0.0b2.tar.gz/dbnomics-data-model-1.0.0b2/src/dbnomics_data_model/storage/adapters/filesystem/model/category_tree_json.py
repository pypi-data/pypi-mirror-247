...
from dbnomics_data_model.json_utils import parse_json_data_as
...
class CategoryJson(BaseJsonObjectModel):
    ...
    def to_domain_model(self) -> Category:
        ...
        return Category.create(self.code, children=children, doc_href=self.doc_href, name=self.name)
...
class CategoryTreeJson:
    ...
    nodes: list[CategoryTreeNodeJson]
    ...
    @classmethod
    def from_json_data(cls, data: list[Json]) -> Self:
        try:
            nodes = parse_json_data_as(data, type=list[CategoryTreeNodeJson])
