...
class GPNValidator(BaseValidator):
    ...
    def _slice_plan(data):
        ...
        sliced_plans = []
        current_slice = {"schedule": {"works": []}, "wg": {"nodes": []}}

        for work in data["schedule"]["works"]:
            if work["id"] not in removed_ids:
                current_slice["schedule"]["works"].append(work)
            if work["display_name"] == "Окончание работ по марке":
                sliced_plans.append(current_slice)
                current_slice = {"schedule": {"works": []}, "wg": {"nodes": []}}

        if current_slice["schedule"]["works"]:
            sliced_plans.append(current_slice)

        for slice_plan in sliced_plans:
            slice_plan["wg"]["nodes"] = [
                node
                for node in data["wg"]["nodes"]
                if node["work_unit"]["id"] not in removed_ids
            ]
