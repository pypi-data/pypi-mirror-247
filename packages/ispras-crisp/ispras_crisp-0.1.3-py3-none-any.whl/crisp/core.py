...
def run_crisp(
    ...
    files = list_files(git_repo, mode, config, include)
    if not files:
...
def list_files(
    ...
    return exclude_files([f for f in files if os.path.isfile(f)], patterns)


def exclude_files(files: List[str], patterns: List[ExcludePattern]) -> List[str]:
