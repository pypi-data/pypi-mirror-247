...
class SymbolicEstimator(BaseEstimator, ABC):
    def __init__(self, population_size=10000, generations=20, stopping_criteria=0.95, store_of_fame=50,
                 ...
                 tournament_size=5, device="cpu", sci_template=None, constant_range=None, constants=None,
                 depth=(2, 5), function_set=('add', 'sub', 'mul', 'div', "pow2", "pow3"),
                 n_jobs=1, verbose=0, random_state=None, method_backend='p_numpy', func_p=None, p_sci=0.5,
                 ):
        """
        ...
        self.func_num = len([self.func_names[i] for i in func_index])
        ...
        self.sci_template = sci_template
        self.temp_sci_template = self.filter_sci_perset(sci_template)
        self.xs = None
        self.y = None
        self.x_label = None
    ...
    def run_gp(self):
        ...
        for gen_i in range(1, self.gen + 1):
            ...
            sp = int(self.p_sci * (self.pop_n - self.hall_of_fame))
            ...
            population[sp:(self.pop_n - self.hall_of_fame)] = mutate_sci(self.func_num, xcs_num,
                                                                         ...
                                                                         p=None, func_p=self.func_p, xs_p=xcs_p,
                                                                         sci_template=self.temp_sci_template)
            # if gen_i == 2:
            #     tt.ts12
...
class SymbolicRegressor(SymbolicEstimator):
    ...
    def __init__(self, population_size=10000, generations=20, stopping_criteria=0.95, store_of_fame=50,
        ...
        Args:
            ...
            tournament_size (int): tournament size for selection.
            stopping_criteria (float): criteria of correlation score, max 1.0.
            constant_range (tuple,None): floats. constant_range=(0,1.0)
            constants (tuple,None): floats. constants=(-1,1,2,10), if given, The parameter constant_range would be ignored.
            ...
            function_set (tuple): tuple of str. optional: ('add', 'sub', 'mul', 'div', "max", "min", "ln", "exp", "pow2", "pow3",
                "rec", "sin", "cos").
            n_jobs (int):n jobs to parallel.
            verbose (bool,int):print message.
            p_mutate: mutate probability.
            p_crossover (float): crossover probability.
...
class SymbolicClassifier(SymbolicEstimator):
    ...
    def top_n(self, n=0, scoring="accuracy"):
        ...
        self.logs.record_and_print(f"The top {n+1} result:")
        self.logs.record_and_print(f"Scoring by {scoring}: ( score, expression, coef, intercept )")
