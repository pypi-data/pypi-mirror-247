...
def save_config_to_toml_file(
    ...
    comment = (
        ...
        "please visit https://docs.cerebrium.ai/cerebrium/environments/initial-setup#config-file-example"
    )
...
def req_list_to_dict(requirements: list) -> dict:
...
def req_dict_to_str_list(
    requirements: Union[dict, list], for_display: bool = False
) -> list:
...
def requirements_to_file(requirements: Union[dict, list], file: str) -> None:
...
def update_from_file(file: str, requirements: dict, key: str, confirm=False) -> dict:
    """Update the requirements dictionary from a file"""
    new_requirements = parse_requirements(file)

    if new_requirements != requirements.get(key):
        if confirm:
            if typer.confirm(
                colored(
            ...
            requirements[key] = new_requirements
