...
#
# Copyright (C) 2018-2023 Daniel Scheffler (GFZ Potsdam, daniel.scheffler@gfz-potsdam.de)
#
...
req_setup = ['importlib-metadata', 'packaging']
    ...
    try:
        ...
        if _parse_version(enpt_version) < _parse_version(version['_minimal_enpt_version']):
            warn(f"The EnPT backend package is already installed, however, its version (v{enpt_version}) is too old "
                 f"and not compatible anymore with enpt_enmapboxapp v{version['__version__']}. Please update EnPT, at "
                 f"least to version {version['_minimal_enpt_version']}!")

    except (ImportError, ModuleNotFoundError):
        print(f"NOTE: enpt_enmapboxapp v{version['__version__']} requires at least EnPT "
              f"v{version['_minimal_enpt_version']}. Make sure it is installed before using the EnPT GUI!")
