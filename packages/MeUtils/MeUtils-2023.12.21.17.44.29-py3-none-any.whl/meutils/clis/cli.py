...
@cli.command(help='url编码')
def quote_url(url):
    """url编码"""
    typer.echo(quote(url))


@cli.command(help='url解码')
def unquote_url(url):
    """url解码"""
    typer.echo(unquote(url))


@cli.command(help='url解码')
def shorten_url(url, shortener: str = 'dagd'):
    """短链"""
    url = _shorten_url(url, shortener)
    typer.echo(url)
