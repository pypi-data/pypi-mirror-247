...
class UserService:
    ...
    def check_account(account):
        ...
        user_list = BaseInfo.objects.filter(user_type='ADMIN').filter(
            Q(user_name=account) | Q(phone=account) | Q(email=account)) \
    ...
    def user_list(params=None, allow_user_list=None, ban_user_list=None, need_Pagination=True, is_strict_mode=True):
        ...
        search_params = format_params_handle(
            param_dict=params,
            filter_filed_list=["user_name", "email", "phone", "nickname", "full_name", "is_using", "user_identity"],
            alias_dict={
    ...
    @staticmethod
    def bind_user_identity(user_id, user_identity: int = None):
        print("data", user_id, user_identity)
        if not user_id:
            return None, "用户ID不存在"
            # 更新基础信息
        ...
        try:
            base_query_set.update(**update_fields)
        except Exception as e:
            return None, str(e)
    ...
    def user_edit(params=None, user_id=None):
        ...
        base_info_params = format_params_handle(
            ...
            filter_filed_list=["full_name", "nickname", "email", "user_info", "privacies", "phone", "is_using"],
            is_remove_null=True
        )
        # 更新基础信息
        base_query_set = BaseInfo.objects.filter(id=user_id)
