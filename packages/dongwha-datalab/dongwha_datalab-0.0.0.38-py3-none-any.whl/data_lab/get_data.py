...
def fillna_by_method(df, method, group_columns=None):
    ...
    if method not in ['avg', 'med', 'ffill', 'bfill'] or type(method) != str: 
        return "[ERROR] The 'method' value must be of STRING type and can only take 'mean', 'median', 'previous', or 'next'."
    ...
    if  group_columns==None : 
        ...
        if method == 'avg':
            ...
            df = df.merge(group_means, on=group_columns, suffixes=('', '_avg'), how='left')
            for col in value_columns:
                df[col] = df[col].combine_first(df[col+'_avg'])
            df = df.drop(columns=[col+'_avg' for col in value_columns])


        elif method == 'med':
            group_medians = df.groupby(group_columns)[value_columns].median().reset_index()
            df = df.merge(group_medians, on=group_columns, suffixes=('', '_med'), how='left')
            for col in value_columns:
                df[col] = df[col].combine_first(df[col+'_med'])
            df = df.drop(columns=[col+'_med' for col in value_columns])
...
def to_fillna(database, sql, save_path, method, group_columns=None, AWS_ACCESS_KEY_ID=None, AWS_SECRET_ACCESS_KEY=None, AWS_DEFAULT_REGION="ap-northeast-2", athena_bucket='test-uram-athena' ):
    if AWS_ACCESS_KEY_ID != None or AWS_SECRET_ACCESS_KEY != None : 
