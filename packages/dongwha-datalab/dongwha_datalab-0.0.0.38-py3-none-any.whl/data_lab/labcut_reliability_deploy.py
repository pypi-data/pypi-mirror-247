...
class mlops():
    ...
    def __init__(self, aws_access_key_id = None, aws_secret_access_key = None): 
        ...
        for row in deploy_collection.find({'target' : 'labcut_reliability'}) : 
            self.G_STAT_MACHINE = row['step_function']
        
        #####
        model_key = self.mongodb_connection('dev', 'list', self.G_AWS_ACCESS_KEY, self.G_AWS_SECRET_KEY, self.G_SSM_MONGO_PATH)
        model_keys = []
        for row in model_key.find({'title' : 'reliability'}, {'_id' : 0, 'job_name' : 1}):
            model_keys.append(row['job_name'])
        ...
        match_filter  =  {'$match' : {'train.status' : 'Completed', 'model_name' : {'$in' : model_keys}}}
...
#                 script_ver = self.P_EVALUATE.selected_dataframe.to_dict('recode')[0]['script_ver']
        ...
        def test_event(event):
            pn.state.notifications.success(event.value())
        ...
        self.P_DEPLOY_TARGET.param.watch(test_event, 'value')
