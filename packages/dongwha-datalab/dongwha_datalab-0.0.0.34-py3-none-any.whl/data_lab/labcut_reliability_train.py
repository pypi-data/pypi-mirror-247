...
class mlops():
    ...
    G_STEP_FUNCTION_DIST = {'품질예측신뢰도' :  "sf_qa_mlops_labcut_reliability_train_piplines_step_job"}
    ...
    def __init__(self, environment, file_path = '.', aws_access_key_id = None, aws_secret_access_key = None): 
        pn.extension()
        ...
        if 'PROD' in environment.upper():
            ...
            self.G_MODEL_LIST = []
            for row in model_list.find({'title' : 'reliability'}, {'_id' : 0}):
    ...
    def create_init_maker(self, config, bucket, script_path, sourcedir_train_path,  environment):
        ...
        insert_info['init']['sourcedir_train_path']     = config['train_local_script_path'].split('/')[-1]
    ...
    def upload_s3_ml_script(self, environment, config, endpoint_url = None, aws_access_key_id = '', aws_secret_access_key = '', stepfunction_job = 'mlops_test_step_01',  bucket = 'dwe-mlops-hub-repository',  ssm_path = '/mlops/{0}/database/mongo', region_name = 'ap-northeast-2'   ):
        ...
        file_list  = [config['extract_local_script_path'], config['train_local_script_path'], config['endpoint_local_script_path']]
        model_name = config['model_name']
        model_id   = config['model_id']
...
#             res = s3.upload_file(tmp_path, bucket, script_path, ExtraArgs={'ContentType': 'application/x-gzip'})
            res = s3.upload_file(tmp_path, bucket, script_path)
            print("Fin upload S3")
            ...
            return {'msg' : '[문법에러 추정2] : ' + str(e), 'status' : 'danger' } 
...
#         file_path = r'/Users/dwe_kdy/Untitled Folder 1/MLOPS/script/k121/test_k121_밀도/'
        file_path = self.G_FILE_PATH #r'/Users/dwe_kdy/Untitled Folder 1/MLOPS/script/k121_ibug/ib_avg/'
#         file_path = r'/Users/dwe_kdy/Untitled Folder 1/MLOPS/script/v113_ibug/ib/'
        
        if self.G_FILE_EDITOR == 'PANEL' : 
            self.P_EXTRACT_FILE_PATH   = pn.widgets.FileSelector(file_path)
            self.P_TRAIN_FILE_PATH     = pn.widgets.FileSelector(file_path)
            self.P_ENDPOINT_FILE_PATH  = pn.widgets.FileSelector(file_path)
        elif self.G_FILE_EDITOR == 'FileChooser' : 
