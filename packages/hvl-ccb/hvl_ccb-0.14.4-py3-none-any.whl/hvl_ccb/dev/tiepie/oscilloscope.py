...
class TiePieOscilloscope(SingleCommDevice):
    ...
    def _osc_config_setup(self) -> None:
        ...
        if self.config_osc_channel_dict[1].has_safeground:
            TiePieOscilloscopeChannelConfig.safeground_enabled = (  # type: ignore
                SafeGround()
            )
    ...
    def start_measurement(self) -> None:
        ...
        if self.is_measurement_running():
            raise TiePieError("TiePie measurement is already running")
        self._osc.start()
    ...
    def stop_measurement(self) -> None:
        ...
        if not self.is_measurement_running():
            raise TiePieError("TiePie measurement is not running")
        self._osc.stop()
    ...
    def is_measurement_running(self) -> bool:
        ...
        # `_require_dev_handle` method decorator
        assert self._osc is not None
        _is_running = self._osc.is_running
