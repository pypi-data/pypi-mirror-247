...
from hvl_ccb.dev import Device, DeviceSequenceMixin
...
class ExperimentError(CCBError):
    """
    Error to indicate that the current status of the experiment manager is on
...
class ExperimentManager(DeviceSequenceMixin):
    ...
    def add_device(self, name: str, device: Device) -> None:
        ...
        start the device. If a device with this name already exists, raise an error.
