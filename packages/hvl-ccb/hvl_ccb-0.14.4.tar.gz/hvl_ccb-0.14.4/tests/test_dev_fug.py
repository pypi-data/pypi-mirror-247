...
def com_config():
    return {
        ...
        "baudrate": 9600,
        "parity": FuGSerialCommunicationConfig.Parity.NONE,
        "stopbits": FuGSerialCommunicationConfig.Stopbits.ONE,
        "bytesize": FuGSerialCommunicationConfig.Bytesize.EIGHTBITS,
        "terminator": b"\r\n",
        "timeout": 0.01,
...
def test_setregisters(com_config, dev_config):
    ...
    com.put_text("S0B:0")
    assert fug.voltage.rampmode == FuGRampModes.IMMEDIATELY
    assert com.get_written() == ">S0B ?"
    com.put_text("E0")
    fug.voltage.rampmode = FuGRampModes.FOLLOWRAMP
    ...
    with pytest.raises(FuGError):
        fug.voltage.rampstate = FuGDigitalVal.ON
    assert com.get_written() is None
    com.put_text("S0S:0")
    assert fug.voltage.rampstate == FuGDigitalVal.OFF
    assert com.get_written() == ">S0S ?"

    com.put_text("S0H:1")
    assert fug.voltage.high_resolution == FuGDigitalVal.YES
    assert com.get_written() == ">S0H ?"
    com.put_text("E0")
    fug.voltage.high_resolution = FuGDigitalVal.YES
    assert com.get_written() == ">S0H 1"
    com.put_text("E0")
    fug.voltage.high_resolution = FuGDigitalVal.NO
...
def test_digital_output_registers(com_config, dev_config):
    ...
    com.put_text("E0")
    fug.outX0.out = FuGDigitalVal.ON
    assert com.get_written() == ">B0 1"
    com.put_text("E0")
    fug.outX0.out = FuGDigitalVal.NO
    assert com.get_written() == ">B0 0"
    com.put_text("B0:1")
    assert fug.outX0.out == FuGDigitalVal.ON
    assert com.get_written() == ">B0 ?"

    com.put_text("B0A:0")
    assert fug.outX0.status == FuGDigitalVal.NO
    assert com.get_written() == ">B0A ?"
    with pytest.raises(FuGError):
        fug.outX0.status = FuGDigitalVal.ON
    assert com.get_written() is None

    com.put_text("E0")
    fug.outX1.out = FuGDigitalVal.ON
    assert com.get_written() == ">B1 1"

    com.put_text("E0")
    fug.outX2.out = FuGDigitalVal.ON
    assert com.get_written() == ">B2 1"

    com.put_text("E0")
    fug.outXCMD.out = FuGDigitalVal.ON
    assert com.get_written() == ">BX 1"

    com.put_text("E0")
    fug.on = FuGDigitalVal.ON
...
def test_monitors(com_config, dev_config):
    ...
    com.put_text("M0I:0")
    assert fug.voltage_monitor.adc_mode == FuGMonitorModes.T256US
    assert com.get_written() == ">M0I ?"
    com.put_text("E0")
    fug.voltage_monitor.adc_mode = FuGMonitorModes.T40MS
...
def test_config_registers(com_config, dev_config):
    ...
    com.put_text("KT:2")
    assert fug.config_status.terminator == FuGTerminators.LF
    assert com.get_written() == ">KT ?"
    with pytest.raises(FuGError):
        fug.config_status.terminator = 5
    assert com.get_written() is None
    com.put_text("E0")
    fug.config_status.terminator = FuGTerminators.LF
    ...
    com.put_text("KN:0")
    assert fug.config_status.readback_data == FuGReadbackChannels.VOLTAGE
    assert com.get_written() == ">KN ?"
    com.put_text("E0")
    fug.config_status.readback_data = FuGReadbackChannels.RATEDCURRENT
