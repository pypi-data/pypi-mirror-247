...
class _Switch(ABC):
    ...
    _CLS_ERROR = SwitchOperationError
    _SWITCHABLE_AT_STATES: Optional[tuple[SafetyStatus]] = None

    def __init__(self, handle, device_name: str):
        self._handle: BaseCube = handle
        self._device_name: str = device_name
        self._CMD_STATUS: Optional[str] = None
        self._CMD_OPERATING_STATUS: Optional[str] = None
        self._CMD_MANUAL: Optional[str] = None
    ...
    def operation_conditions_fulfilled(self) -> None:
        ...
        if not self.operating_status == SwitchOperatingStatus.MANUAL:
            msg = (
    ...
    def operate(self, operation: SwitchOperation) -> None:
        ...
        if self._CMD_MANUAL is None:
            ...
            raise self._CLS_ERROR(msg)  # pragma: no cover
