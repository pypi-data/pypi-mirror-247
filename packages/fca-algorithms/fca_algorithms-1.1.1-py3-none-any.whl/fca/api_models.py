...
from .utils.utils import to_subscript, insert_ordered, insert_ordered_unique, is_in, remove_if_exists
...
class Lattice:
    ...
    def _get_maximal_E_concept(self, extent, generator_concept, common_attributes=None):  # O(nm² log m)
        ...
        while children_is_maximal:
            ...
            for child in children:
                if(common_attributes is not None and len(common_attributes[hash(child)]) == len(extent)) or \
                   extent_set.issubset(set(child.O)):
    ...
    def _get_maximal_I_concept(self, intent, generator_concept, common_objects=None):  # O(mn² log n)
        ...
        while parent_is_maximal:  # O(n) or O(max(|m'|))
            ...
            for parent in parents:  # O(m)
                if(common_objects is not None and len(common_objects[hash(parent)]) == len(intent)) or \
                   intent_set.issubset(set(parent.A)):  # O(n log n)
    ...
    def _get_intent(self, g : str, ignore_attr : str = None):
        ...
        return [j for j in range(len(self.ctx.A)) if j != m_idx and self.ctx.I[g_idx][j]]
    

    def _get_extent(self, m : str, ignore_obj : str = None):
        m_idx = self._attribute_idx[m]
        g_idx = self._object_idx.get(ignore_obj)
        return [i for i in range(len(self.ctx.O)) if i != g_idx and self.ctx.I[i][m_idx]]
...
class IncLattice:
    ...
    def __init__(self, ctx_or_attributes : Optional[Union[List[str], Context]]=None):
        ...
        if ctx_or_attributes is None:
            ...
            self.ctx = ContextCpp(ctx_or_attributes.O,
                                  ctx_or_attributes.A,
                                  ctx_or_attributes.I)
    ...
    def plot(self, **kwargs):
        ...
        return plot_from_hasse(
            ...
            [Concept(self.ctx, t.X, t.Y) for t in self._lat_cpp.concepts],
