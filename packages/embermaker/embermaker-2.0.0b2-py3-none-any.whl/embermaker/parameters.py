...
deprecated = {'haz_top_value': (('haz_axis_top', 'haz_valid_top'), None),
...
class ParamDict(dict):
    ...
    def __setitem__(self, key, data):
        ...
        if not isdeprec:
            if key not in self:
                self.logger.addwarn('Unknown parameter: ' + key)
    ...
    def __call__(self, key, default=None):
        ...
        Todo: Experimental. Consider removing get().
        :param key:
        :param default:
        :return:
        """
    ...
    def lst_norm(self, key):
        """
        The list part of the parameter, after "normalisation" = any string becomes lower case
        :param key:
        :return:
        """
    ...
    def update(self, adict=None, **kwargs):
        ...
        if type(adict) is dict:
            ...
            # Todo: check this - why is it done? <==============================================
        ...
        with open(filepath, encoding="utf8") as fi:
            ...
            for iline, line in enumerate(fi):
                ...
                if "<del>" in cells[0]:  # Deprecated parameter - pass
                    continue
                if len(cells) != 3:
                    self.logger.addwarn("Internal problem in the list of parameters (no description?): |{}|".format(line))
                    continue
                key, c1, c2 = cells
