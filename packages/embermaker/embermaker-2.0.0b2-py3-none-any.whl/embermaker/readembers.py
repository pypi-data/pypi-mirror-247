...
def embers_from_xl(wb, gp=None, logger=None):
    ...
    :param wb: The Excel workbook containing the data
    ...
    # Fullflex file format = from Zommers et al 2020 (! deprecated !)
    # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    # - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        for irow, row in enumerate(sht.rows):
            ...
            if dstate == 'reading':
                ...
                if not hlp.isempty(row[2].value):
                    ...
                    if trans is None:
                        return {'error': logger.addfail("Input file format error in line {}. "
                ...
                if (trans is None or hlp.isempty(row[3].value)) and not hlp.isempty(row[4].value):
                    logger.addwarn('Input file may be badly formated: data without transition: ' + str(row[4].value))
                trans_phper = emb.Level.phaserisk(trans_phase)
        ...
        return {'error': logger.addfail("Unknown input file format:" + str(ffmt))}
