...
def nicelevels(vmin: float, vmax: float, nalevels: int = 5, nmticks: int = None, enclose: bool = False) -> tuple:
    """
    ...
    :return: levels = selected levels so that intervals are [ 5., 2.5, 2.0, 1.0]  *  1.E exponent
...
class Logger:
    ...
    def addwarn(self, mes, critical=False):
        """
        Warning message
        """
        if critical:
            logging.critical(mes)
            level = 50  # Following the usual python convention for logging, 50 is "critical"
            fmtmes = 'CRITICAL ISSUE: ' + rembrackets(mes)
        else:
            logging.warning(mes)
            level = 30  # Following the usual python convention for logging, 30 is "warning"
            fmtmes = rembrackets(mes)
        self.logmes.append((fmtmes, level))

    def addfail(self, mes):
        """
        Log message as fatal error and returns it
        Note: this might be removed in the future, as the difference with addwarn(critical=True) is minor (12/2023)
        """
        mes = "FATAL ERROR: " + mes
        logging.critical(mes)
        fmtmes = rembrackets(mes)
        self.logmes.append((fmtmes, 50))
        return mes
    ...
    def getlog(self, levelname, only=False, as_html=False):
        ...
        if only:  # Do not get higher level messages (ignore 'scarier' messages), so as to isolate warnings / critical
            maxlevel = intlevel
        else:
            maxlevel = 100
        ...
        try:
            if as_html:
                # Add html opening and closing html tags, as defined in mtags and required by error level:
                mes = [self.mtags[lm[1]][0] + lm[0] + self.mtags[lm[1]][1]
                       for lm in self.logmes if lm[1] >= intlevel <= maxlevel]
            else:
                mes = [lm[0] for lm in self.logmes if lm[1] >= intlevel <= maxlevel]
...
def rasterize(infilepath, outfilext, fmt='png', width=800, dpi=200, jpegopt=None, cairo=True, logger=None):
    """
