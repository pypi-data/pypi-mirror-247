...
def test_verify_gcp_cloud_resources(
    ...
    with patch.multiple(
        ...
        print(mock_verify_lib.verify_firewall_policy.call_args)
        if host_project_id:
            ...
            mock_verify_lib.verify_firewall_policy.assert_called_once_with(
                ANY, ANY, host_project_id, ANY, True, ANY, strict=ANY
            )
        else:
            mock_verify_lib.verify_gcp_networking.assert_called_once_with(
                ANY,
            ...
            mock_verify_lib.verify_firewall_policy.assert_called_once_with(
                ANY, ANY, mock_project_id, ANY, False, ANY, strict=ANY
            )
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(False, True, True, "AWS", False, False, id="no-cfn-stack"),
        pytest.param(False, False, False, "AWS", False, False, id="no-cloud-resource"),
        pytest.param(True, False, True, "AWS", False, False, id="not-managed-cloud"),
        pytest.param(None, False, True, "AWS", False, False, id="unknown-cloud-type"),
        pytest.param(False, False, True, "AWS", False, True, id="happy-path-iam"),
        pytest.param(
            False, False, True, "GCP", False, False, id="cloud-provider-mismatch"
        ),
        pytest.param(False, False, True, "AWS", True, True, id="happy-path-redis"),
        pytest.param(False, False, True, "GCP", True, True, id="happy-path-redis"),
    ],
)
def test_update_managed_cloud(
    ...
    with patch.multiple(
        ...
        if not expected_result:
            ...
            cloud_controller.update_managed_cloud(None, Mock(), enable_ft, None, True)
...
def test_verify_aws_cloud_quotas_no_errors():
    ...
    # No errors
    with patch(
        "frontend.cli.anyscale.controllers.cloud_controller.boto3.Session"
        ...
        mock_session.client.assert_called_once_with("service-quotas")
    ...
    with patch(
        ...
        def get_service_quota_mock(ServiceCode: str, QuotaCode: str):
            ...
            if QuotaCode == "L-DB2E81BA" or QuotaCode == "L-7212CCBC":
        ...
        assert "All P4, P3 and P2 Spot Instance Requests" in warning_message
        assert "Running On-Demand G and VT instances" in warning_message
