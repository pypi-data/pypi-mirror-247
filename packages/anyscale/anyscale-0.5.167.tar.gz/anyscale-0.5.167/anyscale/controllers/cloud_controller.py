...
class CloudController(BaseController):
    ...
    def setup_managed_cloud(  # noqa: PLR0912
        ...
        *,
        ...
        if provider == "aws":
            ...
            self.verify_aws_cloud_quotas(region=region)
    ...
    def verify_aws_cloud_quotas(
        ...
        quota_client = boto3_session.client("service-quotas")
        ...
        for quota_code, config in QUOTAS_CONFIG.items():
            ...
            if quota["Quota"]["Value"] < config["min"]:
                invalid_quotas.append(quota_code)

        if len(invalid_quotas):
            quota_errors = [
                f"- \"{QUOTAS_CONFIG[quota_code]['description']}\" should be at least {QUOTAS_CONFIG[quota_code]['min']} (curr: {quota['Quota']['Value']})"
                for quota_code in invalid_quotas
            ]
            quota_error_str = "\n".join(quota_errors)
            self.log.warning(
