...
from patched_cli.client.scm import ScmPlatformClientProtocol, Comment
from patched_cli.client.patched import PatchedClient
from patched_cli.models.common import Patch, PatchMessage
from patched_cli.models.records import PrCreationResult
...
def create_branch_and_pr(path: str, repo: Repo, applied_patches: List[Patch], is_create_pr: bool,
                         ...
                         patched_client: PatchedClient | None) -> PrCreationResult:
    ...
    try:
        ...
        # force commiter's config
        repo_config_reader = repo.config_reader()
        repo_config_reader.read()
        original_username = None
        original_email = None
        if repo_config_reader.has_option("user", "name"):
            original_username = repo_config_reader.get_value("user", "name", None)
        if repo_config_reader.has_option("user", "email"):
            original_email = repo_config_reader.get_value("user", "email", None)

        repo_config_writer = repo.config_writer()
        try:
            repo_config_writer.set_value("user", "name", "patched.codes[bot]")
            repo_config_writer.set_value("user", "email", "298395+patched.codes[bot]@users.noreply.github.com")
            ...
            if original_email is None:
                repo_config_writer.remove_option("user", "email")
            else:
                repo_config_writer.set_value("user", "email", original_email)
            if original_username is None:
                repo_config_writer.remove_option("user", "name")
            else:
                repo_config_writer.set_value("user", "name", original_username)
            repo_config_writer.release()

        if not is_create_pr or (scm_client is None and patched_client is None):
            return PrCreationResult(message=f"Branch created at: \"{next_branch.name}\"",
                                    branch_name=next_branch.name,
                                    pr_url=None)

        tracking_branch = branch.tracking_branch()
        if tracking_branch is None:
            msg = ("Unable to create pull request as base branch does not track a remote branch. \n"
                   f"Branch created at: \"{next_branch.name}\"")
            return PrCreationResult(message=msg,
                                    branch_name=next_branch.name,
                                    pr_url=None)
        ...
        return PrCreationResult(message=f"Pull request generated at: \"{url}\"",
                                branch_name=next_branch.name,
                                pr_url=url)
    finally:
        branch.checkout()
...
def create_pr(repo_slug: str, path: str, diff_text: str, original_branch_name: str, next_branch_name: str,
              applied_patches: List[Patch], scm_client: ScmPlatformClientProtocol):
    body = "This is an automated pull request generated by patched."
    pr = scm_client.create_pr(repo_slug, f"Patched results for branch: {original_branch_name}",
    ...
    comment_md_links = []
    for patch_msg, comment in create_comments(applied_patches, diff_text, path):
        gh_comment_url = pr.create_comment(comment)
        if gh_comment_url is None:
            continue
        comment_md_links.append(f"[{patch_msg.title}]({gh_comment_url})")
...
def create_comment(path, patch_message: PatchMessage, hunks: List[List[Line]]) -> Comment | None:
    if len(hunks) < 1:
        return None
...
def create_comments(applied_patches: List[Patch], diff_text: str, path: str
                    ) -> Generator[tuple[PatchMessage, Comment], None, None]:
    ...
    for applied_patch in applied_patches:
        ...
        for patch_message in applied_patch.msgs:
            yv = create_comment(git_file_path, patch_message, hunks)
            if yv is None:
                continue
