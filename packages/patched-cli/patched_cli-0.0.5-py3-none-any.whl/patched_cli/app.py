...
from patched_cli.utils.logging import init_logger
...
def main(
    ...
    try:
        patches = patched_client.get_patches(local_repository.path, local_repository.repo, vuln_files)
...
def _validations(
    ...
    if sonar_client is not None and not sonar_client.test_token():
        click.echo(f'Unable to access Sonar at "{sonar_url}" with provided token.', err=True)
        exit(1)
    ...
    return LocalRepository(pathlib.Path(path), repo)
