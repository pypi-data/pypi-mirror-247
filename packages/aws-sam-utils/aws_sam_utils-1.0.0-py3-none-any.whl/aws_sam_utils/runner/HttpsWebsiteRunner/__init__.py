...
class HttpsWebsiteRunner(TemplateRunner):
...
""")
    ...
    def setup(self):
        ...
        self.run_cmd(f"sam deploy --stack-name {self.config['STACK_NAME']} --parameter-overrides DomainName={self.config['DOMAIN_NAME']} SubDomain={self.config['WEBSITE_SUBDOMAIN']} CreateApex={self.config['CREATE_APEX']} HostedZoneId={self.hosted_zone_id}")

    def teardown(self):
        self.empty_s3_buckets_in_stack(self.config['STACK_NAME'], ["S3BucketLogs", "S3BucketRoot"])
        # delete the stack
