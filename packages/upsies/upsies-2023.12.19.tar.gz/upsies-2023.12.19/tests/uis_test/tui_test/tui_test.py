...
def test__update_jobs_container(tui, mocker):
    mocker.patch.object(tui, '_update_focus')
    tui._jobs = {
    ...
    def assert_jobs_container(*keys, focused):
        ...
        exp_children = [tui._jobs[k].container for k in keys]
        assert tui._jobs_container.children == exp_children
        if focused:
            assert tui._update_focus.call_args_list == [call(tui._jobs[focused])]
        else:
            assert tui._update_focus.call_args_list == []
        tui._update_focus.reset_mock()

    assert_jobs_container('hi', 'gn', 'yn', focused='hi')

    tui._jobs['an'].job.is_finished = True
    assert_jobs_container('hi', 'gn', 'yn', focused='hi')

    tui._jobs['bi'].job.is_finished = True
    assert_jobs_container('hi', 'gn', 'yn', focused='hi')

    tui._jobs['cn'].job.is_finished = True
    assert_jobs_container('hi', 'gn', 'yn', focused='hi')

    tui._jobs['dn'].job.is_finished = True
    assert_jobs_container('hi', 'gn', 'yn', focused='hi')

    tui._jobs['ei'].job.is_finished = True
    assert_jobs_container('hi', 'gn', 'yn', focused='hi')

    tui._jobs['fi'].job.is_finished = True
    assert_jobs_container('hi', 'gn', 'yn', focused='hi')

    tui._jobs['gn'].job.is_finished = True
    assert_jobs_container('hi', 'gn', 'yn', focused='hi')

    tui._jobs['hi'].job.is_finished = True
    assert_jobs_container('hi', 'xi', 'gn', 'yn', focused='xi')

    tui._jobs['xi'].job.errors = ('Error message',)
    assert_jobs_container('dn', 'fi', 'gn', 'hi', 'xi', focused=None)


@pytest.mark.parametrize('raised', (None, ValueError('whatever')))
def test__update_focus(raised, tui, mocker):
    mocker.patch.object(tui._layout, 'focus', side_effect=raised)
    jobinfo = Mock()
    tui._update_focus(jobinfo)
    assert tui._layout.focus.call_args_list == [call(jobinfo.container)]
...
def test__connect_jobs(tui):
    ...
    assert mocks.mock_calls == [
        ...
        call.foo.signal.register('refresh_job_list', tui._refresh_jobs),
        call.bar.signal.register('finished', tui._handle_job_finished),
        call.bar.signal.register('refresh_job_list', tui._refresh_jobs),
        call.baz.signal.register('finished', tui._handle_job_finished),
        call.baz.signal.register('refresh_job_list', tui._refresh_jobs),
    ]
