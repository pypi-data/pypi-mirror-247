...
class ImageHostBase(abc.ABC):
    ...
    def _cache_file(self, image_path):
        ...
        # Make cache id more unique by adding file size
        try:
            image_path += f'.size={os.path.getsize(image_path)}'
        except OSError:
            pass

        # Optionally make cache file more unique based on whatever the subclass
        # specifies (see `cache_id` property)
