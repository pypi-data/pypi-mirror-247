...
def raise_if_already_finished(method):
    """
...
class JobBase(abc.ABC):
    ...
    def signal(self):
        ...
        ``refresh_job_list``
            Emitted when the user interface should refresh the job list,
            ...
            a job to force the refresh immediately. Registered callbacks get
            no arguments.
        """
        return self._signal
    ...
    def _add_task(self, coro, callback=None):
        ...
        # determines if we are using cached output. If we are using cached
        # output, the job is finished immediately by start() and never await any
        # tasks.
        assert self.was_started, 'Job is not started yet'
