...
class TUI:
    ...
    def _update_jobs_container(self, _=None):
        ...
        # If any job has errors, only display that job and finished jobs.
        if any(jobinfo.job.errors for jobinfo in enabled_jobs):
            for jobinfo in enabled_jobs:
                if jobinfo.job.is_finished or jobinfo.job.errors:
                    jobs_container.append(jobinfo.container)

        else:
            # Only display jobs that were started.
            started_jobs = tuple(
                jobinfo
                for jobinfo in enabled_jobs
                if jobinfo.job.was_started
            )

            # List interactive jobs first.
            for jobinfo in started_jobs:
                if jobinfo.widget.is_interactive:
                    ...
                    if not jobinfo.job.is_finished:
                        ...
                        # Don't display more than one interactive job.
                        break
