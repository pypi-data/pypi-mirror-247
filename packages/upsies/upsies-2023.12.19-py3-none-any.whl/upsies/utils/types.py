...
def ImageHost(allowed=None, disallowed=None):
    ...
    :param disallowed: Sequence of disallowed image host names or `None`
        to allow all supported image host names
    """
    existing_imghost_names = list(imghosts.imghost_names())
    if not allowed:
        allowed = existing_imghost_names

    # Include dummy host if we're developing
    from . import is_running_in_development_environment
    if is_running_in_development_environment():
        allowed = list(set(allowed) | {'dummy'})
