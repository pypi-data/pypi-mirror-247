...
@pytest.mark.parametrize(
    argnames='is_running_in_development_environment, exp_dummy_included',
    argvalues=(
        (False, False),
        (True, True),
    ),
...
def test_ImageHost_allowed(is_running_in_development_environment, exp_dummy_included, mocker):
    mocker.patch('upsies.utils.imghosts.imghost_names', return_value=('foo', 'bar', 'baz'))
    mocker.patch('upsies.utils.is_running_in_development_environment', return_value=is_running_in_development_environment)
    cls = types.ImageHost(allowed=('foo', 'baz'))
    name = cls('foo')
    assert name == 'foo'
    assert isinstance(name, cls)
    assert isinstance(name, str)
    if exp_dummy_included:
        exp_allowed = ('baz', 'dummy', 'foo')
    else:
        exp_allowed = ('baz', 'foo')
    with pytest.raises(ValueError, match=rf'^Not one of {", ".join(exp_allowed)}: bar$'):
        cls('bar')
...
def test_ImageHost_disallowed(is_running_in_development_environment, exp_dummy_included, mocker):
    mocker.patch('upsies.utils.imghosts.imghost_names', return_value=('foo', 'bar', 'baz'))
    mocker.patch('upsies.utils.is_running_in_development_environment', return_value=is_running_in_development_environment)
    cls = types.ImageHost(disallowed=('foo', 'asdf'))
    name = cls('bar')
    assert name == 'bar'
    assert isinstance(name, cls)
    assert isinstance(name, str)
    if exp_dummy_included:
        exp_allowed = ('bar', 'baz', 'dummy')
    else:
        exp_allowed = ('bar', 'baz')
    with pytest.raises(ValueError, match=rf'^Not one of {", ".join(exp_allowed)}: foo$'):
        cls('foo')
