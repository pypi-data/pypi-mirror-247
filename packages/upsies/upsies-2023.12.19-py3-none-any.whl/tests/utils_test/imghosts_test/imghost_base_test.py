...
@pytest.mark.parametrize(
    argnames='filesize',
    argvalues=(
        OSError('no such file ignored here'),
        123,
    ),
    ids=lambda v: str(v),
)
@pytest.mark.parametrize('image_dir', (None, 'some/relative/path', '/absolute/path'))
def test_cache_file(image_dir, filesize, cache_id, cache_dir, exp_cache_dir, mocker):
    ...
    exp_cache_name = image_name
    if not isinstance(filesize, OSError):
        exp_cache_name += f'.size={filesize}'
    if cache_id:
        exp_cache_name += f'.{cache_id}'
    exp_cache_name += f'.{ih.name}.url'
    ...
    if isinstance(filesize, OSError):
        patched_getsize = patch('os.path.getsize', side_effect=filesize)
    else:
        patched_getsize = patch('os.path.getsize', return_value=filesize)

    with patched_getsize:
