...
class FooJob(JobBase):
    ...
    def initialize(self):
        ...
        self.signal.register('refresh_job_list', make_emission_recorder('refresh_job_list'))
...
@pytest.mark.asyncio
async def test__add_task_before_job_was_started(job):
    with pytest.raises(AssertionError, match=r'^Job is not started yet$'):
        job._add_task('mock coro')
