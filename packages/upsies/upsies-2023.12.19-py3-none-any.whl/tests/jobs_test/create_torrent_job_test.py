...
@pytest.mark.asyncio
async def test_CreateTorrentJob_run(job, mocker):

    def _start_torrent_process(announce_url):
        job._torrent_process = Mock(join=AsyncMock())

    mocker.patch.object(job, '_get_announce_url', AsyncMock())
    mocker.patch.object(job, '_start_torrent_process', Mock(side_effect=_start_torrent_process))

    await job.run()

    assert job._get_announce_url.call_args_list == [call()]
    assert job._start_torrent_process.call_args_list == [call(job._get_announce_url.return_value)]
