...
class FileAdapter:
    ...
    @asynccontextmanager
    async def async_open(self, mode="r"):
        async with aiofiles.open(self.path, mode=mode) as file:
            yield file
...
class DirectoryAdapter:
    ...
    def file(self, path: Union[str, Path]) -> FileAdapter:
    ...
    def sub(self, sub_dir_path: Path):
        return DirectoryAdapter(self.path / sub_dir_path)

    def search(self, path_regex: Union[str, Path] = "*") -> List[FileAdapter]:
        return [
