...
def steady_state_disturbance_error(G_forward, Gd_ol, H, input_type: InputType, laplace_variable: Symbol = Symbol('s'), h0:float = 1) -> float:  
    if (input_type == InputType.STEP):
        u = h0/laplace_variable

    elif (input_type == InputType.RAMP):
        u = h0/laplace_variable**2
        
    elif (input_type == InputType.PARABOLA):
        u = h0/laplace_variable**3
    
    return limit(
        laplace_variable*(-G_forward*H)/(1+Gd_ol)*u,
        laplace_variable,
        0
    )
