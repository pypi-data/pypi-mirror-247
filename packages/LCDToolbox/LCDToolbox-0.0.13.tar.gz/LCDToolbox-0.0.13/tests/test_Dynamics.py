...
from lcdtoolbox.Dynamics import system_order, system_type, calculate_static_loop_gain, laplace_transform_string_equation, get_transfer_function, gain_margin, phase_margin
...
class Test_get_transfer_function:
    def test_TestCase1(self):
        """
        From exam 2021 Q1
        """
        ...
        out = get_transfer_function(string_equation, 'z', 'x')
    ...
    def test_TestCase2(self):
        ...
        out = get_transfer_function(string_equation, 'x', 'y')
        assert (out - expected_out).simplify() == 0
    ...
    def test_TestCase3(self):
        ...
        out = get_transfer_function(string_equation, 'u', 'y')
        assert (out - expected_out).simplify() == 0
    ...
    def test_TestCase5(self):
        ...
        out = get_transfer_function(string_equation, 'u', 'x')
        assert (out - expected_out).simplify() == 0
    ...
    def test_TestCase6(self):
        ...
        From exam 2022 Q1
        """
        G0xu = get_transfer_function('Derivative(x(t),t) = f0 - a*(x(t) - x0) + b*(u(t)-u0)', 'u', 'x')
        G0yx = get_transfer_function('y(t) = c*x(t)', 'x', 'y')
        G0 = G0yx*G0xu
        G0 = G0\
            .subs(Symbol('f0'), 0)\
            .subs(Symbol('u0'),0)\
