...
class Player(EventManaged):
    ...
    def update(self, player_data):
        ...
        if self.event_manager and self.deaths < player_data.get('deaths', self.deaths):
            self.trigger_event(ListeningEvents.DEATH)
        self.deaths = player_data.get('deaths', 0)

        if self.event_manager and self.kills < player_data.get('kills', self.kills):
            self.trigger_event(ListeningEvents.KILL)
        self.kills = player_data.get('kills', 0)

        if self.event_manager and self.assists < player_data.get('assists', self.assists):
            self.trigger_event(ListeningEvents.ASSIST)
        self.assists = player_data.get('assists', 0)
