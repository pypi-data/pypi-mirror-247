...
class Hero(EventManaged):
    ...
    def update(self, hero_data):
        ...
        if self.event_manager and self.level < hero_data.get('level', self.level):
            self.trigger_event(ListeningEvents.LVL_UP)
        self.level = hero_data.get('level', self.level)
        ...
        if self.event_manager and not self.stunned and hero_data.get('stunned', False):
            self.trigger_event(ListeningEvents.STUNNED)
        self.stunned = hero_data.get('stunned', False)

        if self.event_manager and not self.hexed and hero_data.get('hexed', False):
            self.trigger_event(ListeningEvents.HEXED)
        self.hexed = hero_data.get('hexed', False)

        if self.event_manager and not self.silenced and hero_data.get('silenced', False):
            self.trigger_event(ListeningEvents.SILENCED)
        self.silenced = hero_data.get('silenced', False)

        if self.event_manager and not self.break_ and hero_data.get('break', False):
            self.trigger_event(ListeningEvents.BREAK)
        self.break_ = hero_data.get('break', False)  # 'break' is a keyword, so using 'break_' instead

        if self.event_manager and not self.disarmed and hero_data.get('disarmed', False):
            self.trigger_event(ListeningEvents.DISARMED)
        self.disarmed = hero_data.get('disarmed', False)

        if self.event_manager and not self.muted and hero_data.get('muted', False):
            self.trigger_event(ListeningEvents.MUTED)
        self.muted = hero_data.get('muted', False)