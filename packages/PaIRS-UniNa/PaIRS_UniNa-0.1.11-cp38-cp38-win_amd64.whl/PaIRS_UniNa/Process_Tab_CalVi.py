...
class Process_Tab_CalVi(gPaIRS_Tab):
 ...
 #*************************************************** Calibration parameters callbacks (action)
    def combo_CalibProcType_action(self):
        ...
        if self.PROpar.CalibProcType>0 and self.PROpar.CamMod<3:
            self.PROpar.CamMod=3
        elif self.PROpar.CalibProcType==3:
            self.PROpar.CamMod=4

    def combo_CamMod_action(self):
        if self.PROpar.CamMod!=4 and self.PROpar.CorrMod>2:
...
#*************************************************** From Parameters to UI
    ...
    def setPROpar(self):
        ...
        if self.PROpar.CamMod>=3:
            self.ui.w_CorrMod.setVisible(True)
            flagVisible=False
            self.ui.combo_CorrMod.clear()
            if self.PROpar.CamMod==4:
                self.ui.w_CylPar.setVisible(True)
                for it in CorrMod_items:
                    self.ui.combo_CorrMod.addItem(it)
            else:
                for it in CorrMod_items[:3]:
                    self.ui.combo_CorrMod.addItem(it)
                self.ui.w_CylPar.setVisible(False)
        else:
            self.ui.w_CorrMod.setVisible(False)
            flagVisible=True
            
        if flagVisible:
            self.ui.w_CorrModel.hide()
        else:
            self.ui.w_CorrModel.show()
