...
def data2PIV(data):
    ...
    # % Windowing parameters (Astarita, Exp Flu, 2007) ***************************
    l=0
    for v in     data.PRO.Vect:
      l=len(v) if l< len(v) else l
    PIV.Inp.numInitIt=l
    ...
    flagCalcVelVec=np.array([PRO.FlagCalcVel],dtype=np.intc)
    semiDimCalcVelVec=np.array([abs(PRO.SemiDimCalcVel)],dtype=np.intc)
    ...
    maxDispInCCVec=np.array([-2 ,-4],dtype=np.intc)    
    PIV.SetWindowingVect([maxDispInCCVec,flagCalcVelVec,FlagWindowingVec,semiDimCalcVelVec,FlagDirectCorrVec])
