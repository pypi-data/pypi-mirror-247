...
class Import_Tab_CalVi(gPaIRS_Tab):
    ...
    def __init__(self,*args):
        ...
        myQTableWidget.setup(self.ui.list_images)
        self.ui.list_images.keyPressEvent=lambda e: myQTableWidget.keyPressEvent(self.ui.list_images,e)
        self.ui.list_images.focusInEvent=lambda e: myQTableWidget.focusInEvent(self.ui.list_images,e)
        self.ui.list_images.focusOutEvent=lambda e: myQTableWidget.focusOutEvent(self.ui.list_images,e)
        self.ui.list_images.resizeInfoLabel=lambda: myQTableWidget.resizeInfoLabel(self.ui.list_images)
        self.ui.list_images.mouseReleaseEvent=lambda e: myQTableWidget.mouseReleaseEvent(self.ui.list_images,e)
        self.ui.list_images.resizeEvent=lambda e: myQTableWidget.resizeEvent(self.ui.list_images,e)
                ...
                for f in filenames:
                    ...
                    if len(self.INPpar.cams): 
                        ...
                        if len(fsplitted):
                            ...
                            f='' #redundant
                    if f and not f in self.INPpar.filenames and not f in f_new:
                        f_new.append(f)
                for t in f_new:
...
#*************************************************** Image set controls
    ...
    def adjust_list_images(self):
        ...
        f_unique=[]
        for f in self.INPpar.filenames:
            if not f in f_unique: f_unique.append(f)
    ...
    def set_list_images_items(self):
        ...
        for k,f in enumerate(self.INPpar.filenames):
            ...
            if not all(list_eim_camk):
                cams=",".join([str(self.INPpar.cams[k]) for k,e in enumerate(list_eim_camk) if not e])
                message+="❌"
...
#******************** root
    ...
    def setFlagValidRootOut(self,*args):
        ...
        ext='.cal'
        FlagExistPath=INPpar_prev.FlagValidPath
        if FlagExistPath:
            currpath=myStandardPath(INPpar_prev.path)
        else:
            currpath='./'
    ...
    def setRootOutLabel(self):
        if self.INPpar.FlagValidRoot==-2:
        ...
        elif self.INPpar.FlagValidRoot==0:
...
class myQTableWidget(QtWidgets.QTableWidget):
    ...
    def focusInEvent(self, event):
        super(QtWidgets.QTableWidget,self).focusInEvent(event) 
        #if self.DeleteButton: #and self.currentItem():
        #    self.DeleteButton.setEnabled(True)

    def focusOutEvent(self, event):
        super(QtWidgets.QTableWidget,self).focusOutEvent(event) 
    ...
    def mouseReleaseEvent(self, event):
        super(QtWidgets.QTableWidget,self).mouseReleaseEvent(event) 
        self.resizeInfoLabel()

    def resizeEvent(self, event):
        super(QtWidgets.QTableWidget,self).resizeEvent(event) 
        self.resizeInfoLabel()
