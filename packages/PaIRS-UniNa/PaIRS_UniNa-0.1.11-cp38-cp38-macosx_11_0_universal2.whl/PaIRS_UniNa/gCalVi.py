...
class gCalVi(QMainWindow):
    ...
    def closeEvent(self,event):
        ...
        if self.FlagRun and not self.FlagClosing[0]:
            flagYes=self.questionDialog('CalVi is currently executing the processes in the Future queue. If you close the application now, processing will be stopped. Are you sure you want to quit?')
            if not flagYes: 
                event.ignore()
                return
        self.hide()
...
#********************************************* TAB definitions
    def defineTabs(self):
        ...
        for k,tname in enumerate(self.opwidnames):
            wname='w_'+tname
            setattr(self,wname,wclasses[k](None,False))
    ...
    def actualBridge(self,tname,indTree,indItem,ind):
        ...
        if tname=='Import':
            ...
            if ind:
                flagReset=self.w_Import.TABpar_prev[indTree][indItem][ind-1].isDifferentFrom(self.w_Import.TABpar,[],['filenames','path'],True)
            else:
                flagReset=True
            self.w_Vis.calib2VIS(flagReset,flagSettingNewCalib=True)
            pass
        if tname=='Process':
            self.w_Import.INPpar.FlagOptPlane=self.w_Process.PROpar.FlagPlane and self.w_Process.PROpar.CalibProcType>0
            ...
            if ind:
                flagReset=self.w_Process.TABpar_prev[indTree][indItem][ind-1].isDifferentFrom(self.w_Process.PROpar,[],['DotColor','DotTypeSearch'],True)
            else:
                flagReset=True
            self.w_Vis.calib2VIS(flagReset,flagSettingNewCalib=flagReset)
            pass
        if tname=='Vis':
            pass
        return
    ...
    def initDataAndSetImgFromGui(self,INP,PRO):
        self.w_Vis.calibView.calib.initDataFromGui(INP,PRO)
        inddel=self.w_Vis.calibView.calib.setImgFromGui()
...
#*************************************************** Run
    def run(self):
        self.w_Vis.VISpar.FlagRun=not self.w_Vis.VISpar.FlagRun
        self.setFlagRun()
        indTree,indItem,ind=self.w_Import.INPpar.indexes()
...
#********************* File
    ...
    def load_uicfg(self):
        filename, _ = QFileDialog.getOpenFileName(self,\
            "Select an image file of the sequence", filter=f'*{outExt.cfg_calvi}',\
...
#*************************************************** Graphical interface appearence
    ...
    def setFlagRun(self):
        if not self.w_Vis.VISpar.FlagRun:
            ...
            self.ui.button_Run.setIcon(self.icon_play)
        else:
            self.ui.button_Run.setText('Stop')
            self.ui.button_Run.setIcon(self.icon_pause)
...
def launchCalVi(flagDebug=False,flagInputDebug=False):
    ...
    if not flagDebug or Flag_SHOWSPLASH:
        splash_pix = QPixmap(''+ icons_path +'logo_CalVi_completo.png')
        splash=QLabel()
...
def quitCalVi(app:QApplication):
...
def setAppGuiPalette(self:gCalVi,palette:QPalette):
    ...
    for f in  set([self]+self.floatings+self.floatw+self.findChildren(QDialog)+[self.aboutDialog]):
        if f:
