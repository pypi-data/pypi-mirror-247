...
#********************************************* TAB definitions
    def defineTabs(self):
        ...
        for k,tname in enumerate(self.opwidnames+['Tree']):
            wname='w_'+tname
            setattr(self,wname,wclasses[k](None,tname in ('Log')))
    ...
    def actualBridge(self,tname,indTree,indItem,ind):
        if ind>-1:
            pri.Callback.green(f'    --- gPaIRS bridge_prev {[indTree]}{[indItem]}{[ind]}')
...
#********************************************* Selection of items
    ...
    def setTREpar(self):
        ...
        data:dataTreePar=self.w_Tree.TABpar_prev[idata.indTree][idata.indItem][idata.ind]
...
#*************************************************** Parallel Pool
    ...
    def parPoolInitSetup(self):
        if self.FlagGuiInit:
            ...
            data=self.w_Tree.TABpar_prev[idata.indTree][idata.indItem][idata.ind]
            self.setButtons(data)
            if not self.FlagParPoolInit:
                self.FlagParPoolInit=False
...
#********************* File
    ...
    def reconfigure(self):
        ...
        from .PaIRS_pypacks import basefold
        INPpar_prev.path=basefold
        self.w_Import.FlagAddPrev=False        
        self.w_Import.getPinfoFromPath(INPpar_prev,[basefold,'']) 
    ...
    def load_uicfg(self):
        filename, _ = QFileDialog.getOpenFileName(self,\
            "Select an image file of the sequence", filter=f'*{outExt.cfg}',\
    ...
    def addDebugMenu(self):
        ...
        if Flag_fullDEBUG:
            ...
            def aCheckConnections():
                tree,_=self.w_Tree.pickTree(self.w_Tree.TREpar.indTree)
                d=tree.currentItem().data(0,Qt.UserRole)
                data=self.w_Tree.TABpar_prev[d.indTree][d.indItem][d.ind]
                INP=self.w_Import.TABpar_prev[d.indTree][d.indItem][d.ind]
        ...
        try:
            with open(filename, 'rb') as file:
                ...
                for n,i in enumerate(self.w_Tree.TREpar.past):
                    ...
                    data_old=self.w_Tree.TABpar_prev[ido.indTree][ido.indItem][ido.ind]
...
def launchPaIRS(flagDebug=False,flagInputDebug=False):
    ...
    if not flagDebug or Flag_SHOWSPLASH:
        splash_pix = QPixmap(''+ icons_path +'logo_PaIRS_completo.png')
        splash=QLabel()
...
def quitPaIRS(app:QApplication):
