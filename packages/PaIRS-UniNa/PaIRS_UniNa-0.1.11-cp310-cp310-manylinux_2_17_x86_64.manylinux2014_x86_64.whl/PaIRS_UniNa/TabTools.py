...
#*************************************************** Undo/redo
    ...
    def add_TABpar(self,tip):
        ...
        if not len(TABpar_prev):
            ...
            flagNewPar=self.TABpar.isDifferentFrom(TABpar_prev[-1],self.TABpar.unchecked_fields) #* see below
        if flagNewPar and self.FlagAddPrev: #something changed
            self.add_TABpar_copy(tip,indTree,indItem)
            self.add_TABpar_bridge(tip,indTree,indItem,self.TABpar.ind)  #shoudl create a copy of the other tabs' parameters (see gPaIRS)
        else:
