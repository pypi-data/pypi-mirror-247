...
def TF_expression(TFName,List,Input_dir,Match2,A,outdir):
...
def load_corr_RE_TG(Input_dir,List,Element_name,Element_name_bulk,outdir):
...
def load_motifbinding_chr(chrN,GRNdir,Input_dir,motifWeight,outdir):
...
def load_TFbinding(GRNdir,Input_dir,motifWeight,Match2,TFName,Element_name,outdir):
    from tqdm import tqdm
    motif_binding=pd.DataFrame()
    chrall=['chr'+str(i+1) for i in range(22)]
    chrall.append('chrX')
    for chrN in tqdm(chrall):
        Motif_binding_temp1=load_motifbinding_chr(chrN,GRNdir,Input_dir,motifWeight,outdir)
...
def preprocess(RNA_file,ATAC_file,label_file,Input_dir,GRNdir,genome,method,outdir):
    package_dir = os.path.dirname(os.path.abspath(__file__))
    if method=='LINGER':
        print('Overlap the regions with bulk data ...')
        script_path = os.path.join(package_dir, "scripts", "extract_overlap_regions_LINGER.sh")
        subprocess.run(["sh", script_path, Input_dir, GRNdir, genome,outdir])
        print('Generate psedubulk ...')
        TG_psedubulk,RE_psedubulk=pseudo_bulk.pseudo_bulk(RNA_file,ATAC_file,label_file,Input_dir)
        ...
        List,A=gene_expression(Input_dir,GRNdir,TFName,Match2,TG_psedubulk,outdir)
        print('Generate TF expression...')
        TFName=TF_expression(TFName,List,Input_dir,Match2,A,outdir)
        print('Generate RE chromatin accessibility...')
        RE_psedubulk[RE_psedubulk > 100] = 100
        RE_psedubulk.to_csv(outdir+'Openness.txt',sep='\t',header=None,index=None)
        ...
        load_TFbinding(GRNdir,Input_dir,motifWeight,Match2,TFName,Element_name,outdir)
        print('Generate Index...')
    #Read hg19_Peak_hg19_gene_u.txt
        merged_s,merged_b=load_corr_RE_TG(Input_dir,List,Element_name,Element_name_bulk,outdir)
        from tqdm import tqdm
    ...
    #Create a progress bar for the loop
        ...
        print('Overlap the regions with bulk data ...')
        script_path = os.path.join(package_dir, "scripts", "extract_overlap_regions_baseline.sh")
        subprocess.run(["sh", script_path, Input_dir, GRNdir, genome,outdir])
    else:
