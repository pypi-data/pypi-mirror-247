...
def load_region(Input_dir,GRNdir,genome,chrN,outdir):  
...
def load_TF_RE(GRNdir,chrN,O_overlap,O_overlap_u,O_overlap_hg19_u):      
    ...
    return mat
def TF_RE_LINGER_chr(Input_dir,ATAC_file,chr,outdir):
...
def TF_RE_binding_chr(RNA_file,ATAC_file,Input_dir,GRNdir,chrN,genome,outdir):
    ## the regions
    O_overlap, N_overlap,O_overlap_u,N_overlap_u,O_overlap_hg19_u=load_region(Input_dir,GRNdir,genome,chrN,outdir)
    import numpy as np
    import pandas as pd
## read the count file.
...
## extact the overlapped peaks.
    ...
    return mean_S
def TF_RE_binding(Input_dir,GRNdir,RNA_file,ATAC_file,genome,method,outdir):
    from tqdm import tqdm
    ...
    if method=='baseline':
        ...
        for i in tqdm(range(23)):
            chrN=chrom[i]
            out=TF_RE_binding_chr(RNA_file,ATAC_file,Input_dir,GRNdir,chrN,genome,outdir)
    ...
    if method=='LINGER':
        ...
        for i in tqdm(range(23)):
            chr=chrom[i]
            mat=TF_RE_LINGER_chr(Input_dir,ATAC_file,chr,outdir)
            result = pd.concat([result, mat], join='outer', axis=0)
    result.to_csv(outdir+'cell_population_TF_RE_binding.txt',sep='\t')


def cell_type_specific_TF_RE_binding_chr(RNA_file,ATAC_file,labels,Input_dir,GRNdir,chrN,genome,celltype,outdir):
    ## the regions
        ## the regions
    O_overlap, N_overlap,O_overlap_u,N_overlap_u,O_overlap_hg19_u=load_region(Input_dir,GRNdir,genome,chrN,outdir)
    import numpy as np
...
def cell_type_specific_TF_RE_binding(Input_dir,GRNdir,RNA_file,ATAC_file,labels,genome,celltype,outdir):
    with open(Input_dir+labels, 'r') as file:
    # Read the lines of the file and remove any trailing whitespace
        lines = [line.strip() for line in file]  
    # Create a list from the lines
    ...
    if celltype == 'all':
        for label0 in labelset:
            ...
            for i in tqdm(range(22)):
                chrN='chr'+str(i+1)
                out=cell_type_specific_TF_RE_binding_chr(RNA_file,ATAC_file,labels,Input_dir,GRNdir,chrN,genome,label0,outdir)
        #result=pd.concat([result,out],axis=1).fillna(0)
                result = pd.concat([result, out], join='outer', axis=0)
            chrN='chrX'
            out=cell_type_specific_TF_RE_binding_chr(RNA_file,ATAC_file,labels,Input_dir,GRNdir,chrN,genome,label0,outdir)
            result = pd.concat([result, out], join='outer', axis=0).fillna(0)
            result.to_csv(outdir+'cell_type_specific_TF_RE_binding_'+label0+'.txt', sep='\t')
    else:
        result=pd.DataFrame()
        from tqdm import tqdm
        chrom=['chr'+str(i+1) for i in range(22)]
        ...
        for i in tqdm(range(23)):
            chrN=chrom[i]
            out=cell_type_specific_TF_RE_binding_chr(RNA_file,ATAC_file,labels,Input_dir,GRNdir,chrN,genome,celltype,outdir)
        #result=pd.concat([result,out],axis=1).fillna(0)
            result = pd.concat([result, out], join='outer', axis=0)
        result.to_csv(outdir+'cell_type_specific_TF_RE_binding_'+celltype+'.txt', sep='\t')
    

def load_shap(chr,Input_dir,outdir):
    import torch
    import pandas as pd
    import numpy as np
    import csv
...
def cis_shap(Input_dir,chr,outdir):
    RE_2=[]
    TG_2=[]
    score_2=[]
    data_merge_temp,geneName,REindex,TFindex,shap_all,TFName,REName=load_shap(chr,Input_dir,outdir)
    from tqdm import tqdm
    ...
    return RE_TG
def trans_shap(Input_dir,chr,outdir):
    TG_1=[]
    TF_1=[]
    score_1=[]
    data_merge_temp,geneName,REindex,TFindex,shap_all,TFName,REName=load_shap(chr,Input_dir,outdir)
...
# Create the sparse matrix using coo_matrix
    ...
    return array
def cis_reg_chr(Input_dir,GRNdir,RNA_file,ATAC_file,genome,chrN,outdir):  
    import numpy as np
    import pandas as pd
    from scipy.sparse import csc_matrix
    from scipy.sparse import coo_matrix
...
## extact the overlapped peaks.
    ...
    return combined
def cis_reg(Input_dir,GRNdir,RNA_file,ATAC_file,genome,method,outdir): 
    from tqdm import tqdm
    chrom=['chr'+str(i+1) for i in range(22)]
    chrom.append('chrX')
    if method=='baseline':
        ...
        for i in tqdm(range(23)):
            chrN=chrom[i]
            temp=cis_reg_chr(Input_dir,GRNdir,RNA_file,ATAC_file,genome,chrN,outdir)
    ...
    if method=='LINGER':
        ...
        for i in tqdm(range(23)):
            chrN=chrom[i]
            temp=cis_shap(Input_dir,chrN,outdir)
            result=pd.concat([result,temp],axis=0,join='outer')
    result.to_csv(outdir+'cell_population_cis_regulatory.txt',sep='\t',header=None,index=None)


def cell_type_specific_cis_reg_chr(Input_dir,GRNdir,RNA_file,ATAC_file,genome,chrN,celltype,outdir): 
    import numpy as np
    import pandas as pd
    from scipy.sparse import csc_matrix
    from scipy.sparse import coo_matrix
...
def cell_type_specific_cis_reg(Input_dir,GRNdir,RNA_file,ATAC_file,label_file,genome,celltype,outdir): 
    import pandas as pd
    import numpy as np
    ...
    if celltype=='all':
        for label0 in labelset:
            ...
            for i in tqdm(range(23)):
                chrN=chrom[i]
                temp=cell_type_specific_cis_reg_chr(Input_dir,GRNdir,RNA_file,ATAC_file,genome,chrN,label0,outdir)
                result=pd.concat([result,temp],axis=0,join='outer')
            chrN='chrX'
            temp=cell_type_specific_cis_reg_chr(Input_dir,GRNdir,RNA_file,ATAC_file,genome,chrN,label0,outdir)
            result=pd.concat([result,temp],axis=0,join='outer')
            ...
            for i in tqdm(range(23)):
                chrN=chrom[i]
                temp=cell_type_specific_cis_reg_chr(Input_dir,GRNdir,RNA_file,ATAC_file,genome,chrN,celltype,outdir)
                result=pd.concat([result,temp],axis=0,join='outer')
            chrN='chrX'
            temp=cell_type_specific_cis_reg_chr(Input_dir,GRNdir,RNA_file,ATAC_file,genome,chrN,celltype,outdir)
            result=pd.concat([result,temp],axis=0,join='outer')
...
def load_cis(Input_dir,Binding,celltype,outdir):
    if celltype=='':
...
def trans_reg(Input_dir,GRNdir,RNA_file,ATAC_file,method,outdir):
    import pandas as pd
    from scipy.sparse import coo_matrix
    import numpy as np
    import pandas as pd
    ...
    if method=='baseline':
        ...
        cis=load_cis(outdir,Binding,'',outdir)
        TFset=Binding.columns
        TGset=cis.columns
        ...
        for i in tqdm(range(23)):
            chrN=chrom[i]
            temp=trans_shap(Input_dir,chrN,outdir)
...
def cell_type_specific_trans_reg(Input_dir,GRNdir,RNA_file,labels,ATAC_file,celltype,outdir):
    import pandas as pd
    from scipy.sparse import coo_matrix
    import numpy as np
    import pandas as pd
    ...
    if celltype=='all':
        for label0 in labelset:
            Binding=pd.read_csv(outdir+'cell_type_specific_TF_RE_binding_'+label0+'.txt',sep='\t',index_col=0)
            cis=load_cis(Input_dir,Binding,label0,outdir)
            TFset=Binding.columns
            TGset=cis.columns
            TF_TG=load_TF_TG(GRNdir, TFset,TGset)
            S=np.matmul(Binding.values.T, cis.values).T*(TF_TG.values.T).T
            S=pd.DataFrame(S, index=TGset,columns=TFset)
            S.to_csv(outdir+'cell_type_specific_trans_regulatory_'+label0+'.txt',sep='\t')
    else:
        Binding=pd.read_csv(outdir+'cell_type_specific_TF_RE_binding_'+celltype+'.txt',sep='\t',index_col=0)
        cis=load_cis(Input_dir,Binding,celltype,outdir)
        TFset=Binding.columns
        TGset=cis.columns
        TF_TG=load_TF_TG(GRNdir, TFset,TGset)
        S=np.matmul(Binding.values.T, cis.values).T*(TF_TG.values.T).T
