...
def fetch_infisical_secrets(
    ...
    client = get_client()
    salt = { 'infisical': {} }
    salt["infisical"][environment] = secret_bundles_to_dict(client.get_all_secrets(environment=environment, path=path))
    return salt
...
def fetch_infisical_secret(
    ...
    client = get_client()
    salt = { 'infisical': {} }
    salt["infisical"][environment] = secret_bundle_to_dict(client.get_secret(secret_name=secret_name, environment=environment, path=path))
    return salt
