...
from .parameter import (ParamBoolRequired, ParamEnumeration, ParamFlags,
                        ParamFloatRequired, ParamIntRequired)
...
class ConvertToPostScript(GlyphCommand):
    ...
    def _execute(self, glyph: Glyph):
        if len(glyph) > 0:
            recordingPen = RecordingPen()
            glyph.draw(Qu2CuPen(recordingPen, max_err=self.e, all_cubic=True, reverse_direction=self.r))
            glyph.clearContours()
            recordingPen.replay(glyph.getPen())
