...
class SchedulerBase:
    ...
    def step(self):
        if self.t < self.warmup_steps:
            value = (
                self.init_value
                + (self.max_value - self.init_value)  # noqa: W503
                * self.t  # noqa: W503
                / self.warmup_steps  # noqa: W503
            )
...
class CosineScheduler(SchedulerBase):
    def step_func(self):
        phase = (
            (self.t - self.warmup_steps)
            / (self.total_steps - self.warmup_steps)  # noqa: W503
            * math.pi  # noqa: W503
        )
        value = (
            self.min_value
            + (self.max_value - self.min_value)  # noqa: W503
            * (np.cos(phase) + 1.0)  # noqa: W503
            / 2.0  # noqa: W503
        )
        return value
...
class PolyScheduler(SchedulerBase):
    ...
    def step_func(self):
        value = (
            self.min_value
            + (self.max_value - self.min_value)  # noqa: W503
            * (self.t - self.warmup_steps) ** self.poly_order  # noqa: W503
        )
        return value
