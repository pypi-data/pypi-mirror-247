...
class DbManagerMongo:
    ...
    def update_or_add_many_to_collection(self, data: list[Company]) -> None:
        """
        ...
        """
        if type(data) not in [list] or not all(isinstance(x, Company) for x in data):
            raise TypeError(f"Can't update or add data to mongo. Type {type(data)} is not supported.")
        
        if all(isinstance(x, Company) for x in data):
            data_found = self.find_many_from_collection(data)
        else:
            raise TypeError(f"Can't update or add many data to mongo. Probably a mix of types in the list.")
        if data_found:
            raise NotImplementedError("Update is not implemented yet.")
        else:
            self._add_many_to_collection(data)
    ...
    def find_many_from_collection(self, data: list[Company]) -> list[Company] | None:
        """
        ...
        if all(isinstance(x, Company) for x in data):
            all_ids = [x.inoopa_id for x in data]
    ...
    def delete_many_from_collection(self, data: list[Company]) -> None:
        """
    ...
    def _add_many_to_collection(self, data: list[Company]) -> None:
        """
        Add a list of companies or website contents to the database.
        
        :param data: The list of companies or website contents to add.
        """
        if type(data) not in [list] or not all(isinstance(x, Company) for x in data):
            raise TypeError(f"Can't update or add data to mongo. Type {type(data)} is not supported.")
        
        if all(isinstance(x, Company) for x in data):
            self._company_collection.insert_many(asdict(data))
        else:
            raise TypeError(f"Can't update or add many data to mongo. Probably a mix of types in the list.")