...
class DbManagerMongo:
    ...
    def update_or_add_many_to_collection(self, data_list: list[Company | CompanyWebsiteContent]) -> None:
        ...
        if all(isinstance(x, Company) for x in data_list):
            self._company_collection.update_many(
                filter={"inoopa_id": {"$in": [x.inoopa_id for x in data_list]}},
                update={"$set": {asdict(x) for x in data_list}},
                upsert=True,
            )
            self._loger.info(f"Updated Companies in collection {self._env} with IDs: {[x.inoopa_id for x in data_list]}")
        elif all(isinstance(x, CompanyWebsiteContent) for x in data_list):
            self._website_content_collection.update_many(
                filter={"url": {"$in": [x.url for x in data_list]}},
                update={"$set": {asdict(x) for x in data_list}},
                upsert=True,
            )
            self._loger.info(f"Updated WebsiteContent in collection {self._env} with urls: {[x.url for x in data_list]}")
        else:
    ...
    def find_one_from_collection(self, data: Company | CompanyWebsiteContent) -> Company | CompanyWebsiteContent | None:
        ...
        """
        if type(data) not in [Company, CompanyWebsiteContent]:
            raise TypeError(f"Can't update or add data to mongo. Type {type(data)} is not supported.")
        ...
        if isinstance(data, CompanyWebsiteContent):
    ...
    def find_many_from_collection(self, data_list: list[Company | CompanyWebsiteContent]) -> list[Company | CompanyWebsiteContent] | None:
        ...
        """
        if type(data_list) not in [list] \
            or not all(isinstance(x, Company) for x in data_list) \
            or not all(isinstance(x, CompanyWebsiteContent) for x in data_list):
        ...
        if all(isinstance(x, CompanyWebsiteContent) for x in data_list):
    ...
    def _add_one_to_collection(self, data: Company | CompanyWebsiteContent) -> None:
        """
        ...
        if isinstance(data, CompanyWebsiteContent):
            self._website_content_collection.insert_one(asdict(data))
            self._loger.info(f"Added WebsiteContent to collection {self._env} with ID: {data.url}")

    def _add_many_to_collection(self, data_list: list[Company | CompanyWebsiteContent]) -> None:
        """
        ...
        if all(isinstance(x, CompanyWebsiteContent) for x in data_list):
            self._website_content_collection.insert_many([asdict(data) for data in data_list])
        else:
            raise TypeError(f"Can't update or add many data to mongo. Probably a mix of types in the list.")