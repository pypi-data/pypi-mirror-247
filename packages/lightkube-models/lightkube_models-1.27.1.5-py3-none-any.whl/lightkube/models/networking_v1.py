...
class HTTPIngressPath(DictMixin):
    """HTTPIngressPath associates a path with a backend. Incoming urls matching the
      ...
      * **path** ``Optional[str]`` - *(optional)* path is matched against the path of an incoming request. Currently it can
...
class IPBlock(DictMixin):
    """IPBlock describes a particular CIDR (Ex. "192.168.1.0/24","2001:db8::/64")
      ...
      * **cidr** ``str`` - cidr is a string representing the IPBlock Valid examples are "192.168.1.0/24"
        or "2001:db8::/64"
      * **except_** ``Optional[List[str]]`` - *(optional)* except is a slice of CIDRs that should not be included within an IPBlock Valid
...
class IngressBackend(DictMixin):
    """IngressBackend describes all endpoints for a given service and port.
      ...
      * **resource** ``Optional[core_v1.TypedLocalObjectReference]`` - *(optional)* resource is an ObjectRef to another Kubernetes resource in the namespace of
      ...
      * **service** ``Optional[IngressServiceBackend]`` - *(optional)* service references a service as a backend. This is a mutually exclusive
...
class IngressClass(DictMixin):
    """IngressClass represents the class of the Ingress, referenced by the Ingress
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **spec** ``Optional[IngressClassSpec]`` - *(optional)* spec is the desired state of the IngressClass. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
    """
    apiVersion: 'Optional[str]' = None
    kind: 'Optional[str]' = None
...
class IngressClassParametersReference(DictMixin):
    """IngressClassParametersReference identifies an API object. This can be used to
      ...
      * **namespace** ``Optional[str]`` - *(optional)* namespace is the namespace of the resource being referenced. This field is
        required when scope is set to "Namespace" and must be unset when scope is set
        to "Cluster".
      * **scope** ``Optional[str]`` - *(optional)* scope represents if this refers to a cluster or namespace scoped resource.
...
class IngressClassSpec(DictMixin):
    """IngressClassSpec provides information about the class of an Ingress.
      ...
      * **controller** ``Optional[str]`` - *(optional)* controller refers to the name of the controller that should handle this class.
        ...
        field is immutable.
      * **parameters** ``Optional[IngressClassParametersReference]`` - *(optional)* parameters is a link to a custom resource containing additional configuration
...
class IngressLoadBalancerIngress(DictMixin):
    """IngressLoadBalancerIngress represents the status of a load-balancer ingress
      ...
      * **hostname** ``Optional[str]`` - *(optional)* hostname is set for load-balancer ingress points that are DNS based.
      * **ip** ``Optional[str]`` - *(optional)* ip is set for load-balancer ingress points that are IP based.
      * **ports** ``Optional[List[IngressPortStatus]]`` - *(optional)* ports provides information about the ports exposed by this LoadBalancer.
    """
...
class IngressLoadBalancerStatus(DictMixin):
    """IngressLoadBalancerStatus represents the status of a load-balancer.
      ...
      * **ingress** ``Optional[List[IngressLoadBalancerIngress]]`` - *(optional)* ingress is a list containing ingress points for the load-balancer.
    """
...
class IngressServiceBackend(DictMixin):
    """IngressServiceBackend references a Kubernetes Service as a Backend.
      ...
      * **port** ``Optional[ServiceBackendPort]`` - *(optional)* port of the referenced service. A port name or port number is required for a
...
class IngressSpec(DictMixin):
    """IngressSpec describes the Ingress the user wishes to exist.
      ...
      * **defaultBackend** ``Optional[IngressBackend]`` - *(optional)* defaultBackend is the backend that should handle requests that don't match any
      ...
      * **ingressClassName** ``Optional[str]`` - *(optional)* ingressClassName is the name of an IngressClass cluster resource. Ingress
      ...
      * **rules** ``Optional[List[IngressRule]]`` - *(optional)* rules is a list of host rules used to configure the Ingress. If unspecified,
        or no rule matches, all traffic is sent to the default backend.
      * **tls** ``Optional[List[IngressTLS]]`` - *(optional)* tls represents the TLS configuration. Currently the Ingress only supports a
...
class IngressStatus(DictMixin):
    """IngressStatus describe the current state of the Ingress.
      ...
      * **loadBalancer** ``Optional[IngressLoadBalancerStatus]`` - *(optional)* loadBalancer contains the current status of the load-balancer.
    """
...
class IngressTLS(DictMixin):
    """IngressTLS describes the transport layer security associated with an ingress.
      ...
      * **hosts** ``Optional[List[str]]`` - *(optional)* hosts is a list of hosts included in the TLS certificate. The values in this
        ...
        unspecified.
      * **secretName** ``Optional[str]`` - *(optional)* secretName is the name of the secret used to terminate TLS traffic on port
...
class NetworkPolicy(DictMixin):
    """NetworkPolicy describes what network traffic is allowed for a set of Pods
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **spec** ``Optional[NetworkPolicySpec]`` - *(optional)* spec represents the specification of the desired behavior for this
        NetworkPolicy.
      * **status** ``Optional[NetworkPolicyStatus]`` - *(optional)* status represents the current state of the NetworkPolicy. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
    """
    apiVersion: 'Optional[str]' = None
    kind: 'Optional[str]' = None
...
class NetworkPolicyEgressRule(DictMixin):
    """NetworkPolicyEgressRule describes a particular set of traffic that is allowed
      ...
      * **ports** ``Optional[List[NetworkPolicyPort]]`` - *(optional)* ports is a list of destination ports for outgoing traffic. Each item in this
      ...
      * **to** ``Optional[List[NetworkPolicyPeer]]`` - *(optional)* to is a list of destinations for outgoing traffic of pods selected for this
...
class NetworkPolicyIngressRule(DictMixin):
    """NetworkPolicyIngressRule describes a particular set of traffic that is allowed
      ...
      * **from_** ``Optional[List[NetworkPolicyPeer]]`` - *(optional)* from is a list of sources which should be able to access the pods selected for
        ...
        item, this rule allows traffic only if the traffic matches at least one item
        in the from list.
      * **ports** ``Optional[List[NetworkPolicyPort]]`` - *(optional)* ports is a list of ports which should be made accessible on the pods selected
...
class NetworkPolicyPeer(DictMixin):
    """NetworkPolicyPeer describes a peer to allow traffic to/from. Only certain
      ...
      * **namespaceSelector** ``Optional[meta_v1.LabelSelector]`` - *(optional)* namespaceSelector selects namespaces using cluster-scoped labels. This field
      ...
      * **podSelector** ``Optional[meta_v1.LabelSelector]`` - *(optional)* podSelector is a label selector which selects pods. This field follows
...
class NetworkPolicyPort(DictMixin):
    """NetworkPolicyPort describes a port to allow traffic on
      ...
      * **endPort** ``Optional[int]`` - *(optional)* endPort indicates that the range of ports from port to endPort if set,
      ...
      * **port** ``Optional[util_intstr.IntOrString]`` - *(optional)* port represents the port on the given protocol. This can either be a numerical
        ...
        will be matched.
      * **protocol** ``Optional[str]`` - *(optional)* protocol represents the protocol (TCP, UDP, or SCTP) which traffic must match.
...
class NetworkPolicySpec(DictMixin):
    """NetworkPolicySpec provides the specification of a NetworkPolicy
      ...
      * **podSelector** ``meta_v1.LabelSelector`` - podSelector selects the pods to which this NetworkPolicy object applies. The
        ...
        this namespace.
      * **egress** ``Optional[List[NetworkPolicyEgressRule]]`` - *(optional)* egress is a list of egress rules to be applied to the selected pods. Outgoing
      ...
      * **ingress** ``Optional[List[NetworkPolicyIngressRule]]`` - *(optional)* ingress is a list of ingress rules to be applied to the selected pods. Traffic
      ...
      * **policyTypes** ``Optional[List[str]]`` - *(optional)* policyTypes is a list of rule types that the NetworkPolicy relates to. Valid
...
@dataclass
class NetworkPolicyStatus(DictMixin):
    """NetworkPolicyStatus describes the current state of the NetworkPolicy.

      **parameters**

      * **conditions** ``Optional[List[meta_v1.Condition]]`` - *(optional)* conditions holds an array of metav1.Condition that describe the state of the
...
class ServiceBackendPort(DictMixin):
    """ServiceBackendPort is the service port being referenced.
      ...
      * **name** ``Optional[str]`` - *(optional)* name is the name of the port on the Service. This is a mutually exclusive
        setting with "Number".
      * **number** ``Optional[int]`` - *(optional)* number is the numerical port number (e.g. 80) on the Service. This is a
