...
class NetworkPolicy(DictMixin):
    """NetworkPolicy describes what network traffic is allowed for a set of Pods
      ...
      * **status** ``Optional[NetworkPolicyStatus]`` - *(optional)* Status is the current state of the NetworkPolicy. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
    """
    apiVersion: 'Optional[str]' = None
    kind: 'Optional[str]' = None
...
class NetworkPolicyPort(DictMixin):
    """NetworkPolicyPort describes a port to allow traffic on
      ...
      * **endPort** ``Optional[int]`` - *(optional)* If set, indicates that the range of ports from port to endPort, inclusive,
      ...
      * **port** ``Optional[util_intstr.IntOrString]`` - *(optional)* The port on the given protocol. This can either be a numerical or named port
        ...
        matched.
      * **protocol** ``Optional[str]`` - *(optional)* The protocol (TCP, UDP, or SCTP) which traffic must match. If not specified,
