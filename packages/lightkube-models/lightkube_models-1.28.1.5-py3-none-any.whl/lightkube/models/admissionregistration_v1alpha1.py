...
class MatchResources(DictMixin):
    """MatchResources decides whether to run the admission control policy on an
      ...
      * **excludeResourceRules** ``Optional[List[NamedRuleWithOperations]]`` - *(optional)* ExcludeResourceRules describes what operations on what resources/subresources
        the ValidatingAdmissionPolicy should not care about. The exclude rules take
        precedence over include rules (if a resource matches both, it is excluded)
      * **matchPolicy** ``Optional[str]`` - *(optional)* matchPolicy defines how the "MatchResources" list is used to match incoming
      ...
      * **namespaceSelector** ``Optional[meta_v1.LabelSelector]`` - *(optional)* NamespaceSelector decides whether to run the admission control policy on an
      ...
      * **objectSelector** ``Optional[meta_v1.LabelSelector]`` - *(optional)* ObjectSelector decides whether to run the validation based on if the object
        ...
        Default to the empty LabelSelector, which matches everything.
      * **resourceRules** ``Optional[List[NamedRuleWithOperations]]`` - *(optional)* ResourceRules describes what operations on what resources/subresources the
...
class NamedRuleWithOperations(DictMixin):
    """NamedRuleWithOperations is a tuple of Operations and Resources with
      ...
      * **apiGroups** ``Optional[List[str]]`` - *(optional)* APIGroups is the API groups the resources belong to. '*' is all groups. If '*'
        is present, the length of the slice must be one. Required.
      * **apiVersions** ``Optional[List[str]]`` - *(optional)* APIVersions is the API versions the resources belong to. '*' is all versions.
        If '*' is present, the length of the slice must be one. Required.
      * **operations** ``Optional[List[str]]`` - *(optional)* Operations is the operations the admission hook cares about - CREATE, UPDATE,
        ...
        one. Required.
      * **resourceNames** ``Optional[List[str]]`` - *(optional)* ResourceNames is an optional white list of names that the rule applies to.  An
        empty set means that everything is allowed.
      * **resources** ``Optional[List[str]]`` - *(optional)* Resources is a list of resources this rule applies to.
        ...
        Required.
      * **scope** ``Optional[str]`` - *(optional)* scope specifies the scope of this rule. Valid values are "Cluster",
...
class ParamKind(DictMixin):
    """ParamKind is a tuple of Group Kind and Version.
      ...
      * **apiVersion** ``Optional[str]`` - *(optional)* APIVersion is the API group version the resources belong to. In format of
        "group/version". Required.
      * **kind** ``Optional[str]`` - *(optional)* Kind is the API kind the resources belong to. Required.
    """
...
class ParamRef(DictMixin):
    """ParamRef describes how to locate the params to be used as input to expressions
      ...
      * **name** ``Optional[str]`` - *(optional)* `name` is the name of the resource being referenced.
        `name` and `selector` are mutually exclusive properties. If one is set, the
        other must be unset.
      * **namespace** ``Optional[str]`` - *(optional)* namespace is the namespace of the referenced resource. Allows limiting the
      ...
      * **parameterNotFoundAction** ``Optional[str]`` - *(optional)* `parameterNotFoundAction` controls the behavior of the binding when the
      ...
      * **selector** ``Optional[meta_v1.LabelSelector]`` - *(optional)* selector can be used to match multiple param objects based on their labels.
...
class TypeChecking(DictMixin):
    """TypeChecking contains results of type checking the expressions in the
      ...
      * **expressionWarnings** ``Optional[List[ExpressionWarning]]`` - *(optional)* The type checking warnings for each expression.
    """
...
class ValidatingAdmissionPolicy(DictMixin):
    """ValidatingAdmissionPolicy describes the definition of an admission validation
      ...
      * **spec** ``Optional[ValidatingAdmissionPolicySpec]`` - *(optional)* Specification of the desired behavior of the ValidatingAdmissionPolicy.
      * **status** ``Optional[ValidatingAdmissionPolicyStatus]`` - *(optional)* The status of the ValidatingAdmissionPolicy, including warnings that are
...
class ValidatingAdmissionPolicyBinding(DictMixin):
    """ValidatingAdmissionPolicyBinding binds the ValidatingAdmissionPolicy with
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object metadata; More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata.
      * **spec** ``Optional[ValidatingAdmissionPolicyBindingSpec]`` - *(optional)* Specification of the desired behavior of the ValidatingAdmissionPolicyBinding.
    """
    apiVersion: 'Optional[str]' = None
    kind: 'Optional[str]' = None
    metadata: 'Optional[meta_v1.ObjectMeta]' = None
...
class ValidatingAdmissionPolicyBindingList(DictMixin):
    """ValidatingAdmissionPolicyBindingList is a list of
      ...
      * **apiVersion** ``Optional[str]`` - *(optional)* APIVersion defines the versioned schema of this representation of an object.
        ...
        may reject unrecognized values. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#resources
      * **items** ``Optional[List[ValidatingAdmissionPolicyBinding]]`` - *(optional)* List of PolicyBinding.
      * **kind** ``Optional[str]`` - *(optional)* Kind is a string value representing the REST resource this object represents.
        Servers may infer this from the endpoint the client submits requests to.
        Cannot be updated. In CamelCase. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds
...
class ValidatingAdmissionPolicyBindingSpec(DictMixin):
    """ValidatingAdmissionPolicyBindingSpec is the specification of the
      ...
      * **matchResources** ``Optional[MatchResources]`` - *(optional)* MatchResources declares what resources match this binding and will be
      ...
      * **paramRef** ``Optional[ParamRef]`` - *(optional)* paramRef specifies the parameter resource used to configure the admission
      ...
      * **policyName** ``Optional[str]`` - *(optional)* PolicyName references a ValidatingAdmissionPolicy name which the
      ...
      * **validationActions** ``Optional[List[str]]`` - *(optional)* validationActions declares how Validations of the referenced
...
class ValidatingAdmissionPolicyList(DictMixin):
    """ValidatingAdmissionPolicyList is a list of ValidatingAdmissionPolicy.
      ...
      * **apiVersion** ``Optional[str]`` - *(optional)* APIVersion defines the versioned schema of this representation of an object.
        ...
        may reject unrecognized values. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#resources
      * **items** ``Optional[List[ValidatingAdmissionPolicy]]`` - *(optional)* List of ValidatingAdmissionPolicy.
      * **kind** ``Optional[str]`` - *(optional)* Kind is a string value representing the REST resource this object represents.
        Servers may infer this from the endpoint the client submits requests to.
        Cannot be updated. In CamelCase. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds
...
class ValidatingAdmissionPolicySpec(DictMixin):
    """ValidatingAdmissionPolicySpec is the specification of the desired behavior of
      ...
      * **auditAnnotations** ``Optional[List[AuditAnnotation]]`` - *(optional)* auditAnnotations contains CEL expressions which are used to produce audit
      ...
      * **failurePolicy** ``Optional[str]`` - *(optional)* failurePolicy defines how to handle failures for the admission policy.
      ...
      * **matchConditions** ``Optional[List[MatchCondition]]`` - *(optional)* MatchConditions is a list of conditions that must be met for a request to be
      ...
      * **matchConstraints** ``Optional[MatchResources]`` - *(optional)* MatchConstraints specifies what resources this policy is designed to validate.
        ...
        Required.
      * **paramKind** ``Optional[ParamKind]`` - *(optional)* ParamKind specifies the kind of resources used to parameterize this policy. If
      ...
      * **validations** ``Optional[List[Validation]]`` - *(optional)* Validations contain CEL expressions which is used to apply the validation.
        Validations and AuditAnnotations may not both be empty; a minimum of one
        Validations or AuditAnnotations is required.
      * **variables** ``Optional[List[Variable]]`` - *(optional)* Variables contain definitions of variables that can be used in composition of
...
class ValidatingAdmissionPolicyStatus(DictMixin):
    """ValidatingAdmissionPolicyStatus represents the status of a
      ...
      * **conditions** ``Optional[List[meta_v1.Condition]]`` - *(optional)* The conditions represent the latest available observations of a policy's
        current state.
      * **observedGeneration** ``Optional[int]`` - *(optional)* The generation observed by the controller.
      * **typeChecking** ``Optional[TypeChecking]`` - *(optional)* The results of type checking for each expression. Presence of this field
...
class Validation(DictMixin):
    """Validation specifies the CEL expression which is used to apply the validation.
      ...
      * **reason** ``Optional[str]`` - *(optional)* Reason represents a machine-readable description of why this validation
