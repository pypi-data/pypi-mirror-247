...
class Container(DictMixin):
    """A single application container that you want to run within a pod.
      ...
      * **restartPolicy** ``Optional[str]`` - *(optional)* RestartPolicy defines the restart behavior of individual containers in a pod.
...
class EphemeralContainer(DictMixin):
    """An EphemeralContainer is a temporary container that you may add to an existing
      ...
      * **restartPolicy** ``Optional[str]`` - *(optional)* Restart policy for the container to manage the restart behavior of each
...
class HostIP(DictMixin):
    """HostIP represents a single IP address allocated to the host.
      ...
      * **ip** ``Optional[str]`` - *(optional)* IP is the IP address assigned to the host
    """
    ip: 'Optional[str]' = None
...
class PersistentVolumeClaimStatus(DictMixin):
    """PersistentVolumeClaimStatus is the current status of a persistent volume
      ...
      * **accessModes** ``Optional[List[str]]`` - *(optional)* accessModes contains the actual access modes the volume backing the PVC has.
        More info:
        https://kubernetes.io/docs/concepts/storage/persistent-volumes#access-modes-1
      * **allocatedResourceStatuses** ``Optional[dict]`` - *(optional)* allocatedResourceStatuses stores status of resource being resized for the
        ...
        feature.
      * **allocatedResources** ``Optional[dict]`` - *(optional)* allocatedResources tracks the resources allocated to a PVC including its
...
class PersistentVolumeStatus(DictMixin):
    """PersistentVolumeStatus is the current status of a persistent volume.
      ...
      * **lastPhaseTransitionTime** ``Optional[meta_v1.Time]`` - *(optional)* lastPhaseTransitionTime is the time the phase transitioned from one to another
...
class PodIP(DictMixin):
    """PodIP represents a single IP address allocated to the pod.
      ...
      * **ip** ``Optional[str]`` - *(optional)* IP is the IP address assigned to the pod
    """
    ip: 'Optional[str]' = None
...
class PodResourceClaimStatus(DictMixin):
    """PodResourceClaimStatus is stored in the PodStatus for each PodResourceClaim
      ...
      * **name** ``str`` - Name uniquely identifies this resource claim inside the pod. This must match
        ...
        must be a DNS_LABEL.
      * **resourceClaimName** ``Optional[str]`` - *(optional)* ResourceClaimName is the name of the ResourceClaim that was generated for the
...
class PodStatus(DictMixin):
    """PodStatus represents information about the status of a pod. Status may trail
      ...
      * **hostIP** ``Optional[str]`` - *(optional)* hostIP holds the IP address of the host to which the pod is assigned. Empty if
      ...
      * **hostIPs** ``Optional[List[HostIP]]`` - *(optional)* hostIPs holds the IP addresses allocated to the host. If this field is
      ...
      * **podIP** ``Optional[str]`` - *(optional)* podIP address allocated to the pod. Routable at least within the cluster.
      ...
      * **resourceClaimStatuses** ``Optional[List[PodResourceClaimStatus]]`` - *(optional)* Status of resource claims.
