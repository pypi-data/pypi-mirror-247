...
class CronJob(DictMixin):
    """CronJob represents the configuration of a single cron job.
      ...
      * **spec** ``Optional[CronJobSpec]`` - *(optional)* Specification of the desired behavior of a cron job, including the schedule.
        More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
      * **status** ``Optional[CronJobStatus]`` - *(optional)* Current status of a cron job. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
    """
...
class CronJobSpec(DictMixin):
    """CronJobSpec describes how the job execution will look like and when it will
      ...
      * **concurrencyPolicy** ``Optional[str]`` - *(optional)* Specifies how to treat concurrent executions of a Job. Valid values are:
        - "Allow" (default): allows CronJobs to run concurrently; - "Forbid": forbids
        concurrent runs, skipping next run if previous run hasn't finished yet; -
        "Replace": cancels currently running job and replaces it with a new one
      * **failedJobsHistoryLimit** ``Optional[int]`` - *(optional)* The number of failed finished jobs to retain. Value must be non-negative
        integer. Defaults to 1.
      * **startingDeadlineSeconds** ``Optional[int]`` - *(optional)* Optional deadline in seconds for starting the job if it misses scheduled time
        for any reason.  Missed jobs executions will be counted as failed ones.
      * **successfulJobsHistoryLimit** ``Optional[int]`` - *(optional)* The number of successful finished jobs to retain. Value must be non-negative
        integer. Defaults to 3.
      * **suspend** ``Optional[bool]`` - *(optional)* This flag tells the controller to suspend subsequent executions, it does not
        apply to already started executions.  Defaults to false.
      * **timeZone** ``Optional[str]`` - *(optional)* The time zone name for the given schedule, see
        https://en.wikipedia.org/wiki/List_of_tz_database_time_zones. If not
        specified, this will default to the time zone of the kube-controller-manager
        process. The set of valid time zone names and the time zone offset is loaded
        from the system-wide time zone database by the API server during CronJob
...
class CronJobStatus(DictMixin):
    """CronJobStatus represents the current state of a cron job.
      ...
      * **active** ``Optional[List[core_v1.ObjectReference]]`` - *(optional)* A list of pointers to currently running jobs.
      * **lastScheduleTime** ``Optional[meta_v1.Time]`` - *(optional)* Information when was the last time the job was successfully scheduled.
      * **lastSuccessfulTime** ``Optional[meta_v1.Time]`` - *(optional)* Information when was the last time the job successfully completed.
    """
...
class Job(DictMixin):
    """Job represents the configuration of a single job.
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **spec** ``Optional[JobSpec]`` - *(optional)* Specification of the desired behavior of a job. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
      * **status** ``Optional[JobStatus]`` - *(optional)* Current status of a job. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
    """
    apiVersion: 'Optional[str]' = None
    kind: 'Optional[str]' = None
...
class JobCondition(DictMixin):
    """JobCondition describes current state of a job.
      ...
      * **lastProbeTime** ``Optional[meta_v1.Time]`` - *(optional)* Last time the condition was checked.
...
class JobSpec(DictMixin):
    """JobSpec describes how the job execution will look like.
      ...
      * **activeDeadlineSeconds** ``Optional[int]`` - *(optional)* Specifies the duration in seconds relative to the startTime that the job may
        ...
        again.
      * **backoffLimit** ``Optional[int]`` - *(optional)* Specifies the number of retries before marking this job failed. Defaults to 6
      * **backoffLimitPerIndex** ``Optional[int]`` - *(optional)* Specifies the limit for the number of retries within an index before marking
        ...
        Never. The field is immutable. This field is alpha-level. It can be used when
        the `JobBackoffLimitPerIndex` feature gate is enabled (disabled by default).
      * **completionMode** ``Optional[str]`` - *(optional)* completionMode specifies how Pod completions are tracked. It can be
      ...
      * **completions** ``Optional[int]`` - *(optional)* Specifies the desired number of successfully finished pods the job should be
        ...
        https://kubernetes.io/docs/concepts/workloads/controllers/jobs-run-to-completion/
      * **manualSelector** ``Optional[bool]`` - *(optional)* manualSelector controls generation of pod labels and pod selectors. Leave
      ...
      * **maxFailedIndexes** ``Optional[int]`` - *(optional)* Specifies the maximal number of failed indexes before marking the Job as
        failed, when backoffLimitPerIndex is set. Once the number of failed indexes
        exceeds this number the entire Job is marked as Failed and its execution is
        terminated. When left as null the job continues execution of all of its
        ...
        completions greater than 10^5. This field is alpha-level. It can be used when
        the `JobBackoffLimitPerIndex` feature gate is enabled (disabled by default).
      * **parallelism** ``Optional[int]`` - *(optional)* Specifies the maximum desired number of pods the job should run at any given
        ...
        https://kubernetes.io/docs/concepts/workloads/controllers/jobs-run-to-completion/
      * **podFailurePolicy** ``Optional[PodFailurePolicy]`` - *(optional)* Specifies the policy of handling failed pods. In particular, it allows to
        specify the set of actions and conditions which need to be satisfied to take
        the associated action. If empty, the default behaviour applies - the counter
        of failed pods, represented by the jobs's .status.failed field, is incremented
      ...
      * **podReplacementPolicy** ``Optional[str]`` - *(optional)* podReplacementPolicy specifies when to create replacement Pods. Possible
        values are: - TerminatingOrFailed means that we recreate pods
        ...
        in use. This is an alpha field. Enable JobPodReplacementPolicy to be able to
        use this field.
      * **selector** ``Optional[meta_v1.LabelSelector]`` - *(optional)* A label query over pods that should match the pod count. Normally, the system
        sets this field for you. More info:
        https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/#label-selectors
      * **suspend** ``Optional[bool]`` - *(optional)* suspend specifies whether the Job controller should create Pods or not. If a
      ...
      * **ttlSecondsAfterFinished** ``Optional[int]`` - *(optional)* ttlSecondsAfterFinished limits the lifetime of a Job that has finished
...
class JobStatus(DictMixin):
    """JobStatus represents the current state of a Job.
      ...
      * **active** ``Optional[int]`` - *(optional)* The number of pending and running pods.
      * **completedIndexes** ``Optional[str]`` - *(optional)* completedIndexes holds the completed indexes when .spec.completionMode =
      ...
      * **completionTime** ``Optional[meta_v1.Time]`` - *(optional)* Represents time when the job was completed. It is not guaranteed to be set in
        ...
        successfully.
      * **conditions** ``Optional[List[JobCondition]]`` - *(optional)* The latest available observations of an object's current state. When a Job
        ...
        https://kubernetes.io/docs/concepts/workloads/controllers/jobs-run-to-completion/
      * **failed** ``Optional[int]`` - *(optional)* The number of pods which reached phase Failed.
      * **failedIndexes** ``Optional[str]`` - *(optional)* FailedIndexes holds the failed indexes when backoffLimitPerIndex=true. The
        ...
        5 and 7, they are represented as "1,3-5,7". This field is alpha-level. It can
        be used when the `JobBackoffLimitPerIndex` feature gate is enabled (disabled
        by default).
      * **ready** ``Optional[int]`` - *(optional)* The number of pods which have a Ready condition.
      ...
      * **startTime** ``Optional[meta_v1.Time]`` - *(optional)* Represents time when the job controller started processing a job. When a Job
      ...
      * **succeeded** ``Optional[int]`` - *(optional)* The number of pods which reached phase Succeeded.
      * **terminating** ``Optional[int]`` - *(optional)* The number of pods which are terminating (in phase Pending or Running and have
        a deletionTimestamp).
      ...
      * **uncountedTerminatedPods** ``Optional[UncountedTerminatedPods]`` - *(optional)* uncountedTerminatedPods holds the UIDs of Pods that have terminated but the
...
class JobTemplateSpec(DictMixin):
    """JobTemplateSpec describes the data a Job should have when created from a
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object's metadata of the jobs created from this template. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **spec** ``Optional[JobSpec]`` - *(optional)* Specification of the desired behavior of the job. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
    """
    metadata: 'Optional[meta_v1.ObjectMeta]' = None
    spec: 'Optional[JobSpec]' = None
...
class PodFailurePolicyOnExitCodesRequirement(DictMixin):
    """PodFailurePolicyOnExitCodesRequirement describes the requirement for handling
      ...
      * **values** ``List[int]`` - Specifies the set of values. Each returned container exit code (might be
      ...
      * **containerName** ``Optional[str]`` - *(optional)* Restricts the check for exit codes to the container with the specified name.
...
class PodFailurePolicyOnPodConditionsPattern(DictMixin):
    """PodFailurePolicyOnPodConditionsPattern describes a pattern for matching an
      ...
      * **status** ``str`` - Specifies the required Pod condition status. To match a pod condition it is
        ...
        to True.
      * **type** ``str`` - Specifies the required Pod condition type. To match a pod condition it is
...
class PodFailurePolicyRule(DictMixin):
    """PodFailurePolicyRule describes how a pod failure is handled when the
      ...
      * **onPodConditions** ``List[PodFailurePolicyOnPodConditionsPattern]`` - Represents the requirement on the pod conditions. The requirement is
        represented as a list of pod condition patterns. The requirement is satisfied
        if at least one pattern matches an actual pod condition. At most 20 elements
        are allowed.
      * **onExitCodes** ``Optional[PodFailurePolicyOnExitCodesRequirement]`` - *(optional)* Represents the requirement on the container exit codes.
    """
    action: 'str'
...
class UncountedTerminatedPods(DictMixin):
    """UncountedTerminatedPods holds UIDs of Pods that have terminated but haven't
      ...
      * **failed** ``Optional[List[str]]`` - *(optional)* failed holds UIDs of failed Pods.
      * **succeeded** ``Optional[List[str]]`` - *(optional)* succeeded holds UIDs of succeeded Pods.
    """
