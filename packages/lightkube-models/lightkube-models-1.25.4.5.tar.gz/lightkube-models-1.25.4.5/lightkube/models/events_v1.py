...
class Event(DictMixin):
    """Event is a report of an event somewhere in the cluster. It generally denotes
      ...
      * **action** ``Optional[str]`` - *(optional)* action is what action was taken/failed regarding to the regarding object. It
      ...
      * **deprecatedCount** ``Optional[int]`` - *(optional)* deprecatedCount is the deprecated field assuring backward compatibility with
        core.v1 Event type.
      * **deprecatedFirstTimestamp** ``Optional[meta_v1.Time]`` - *(optional)* deprecatedFirstTimestamp is the deprecated field assuring backward
        compatibility with core.v1 Event type.
      * **deprecatedLastTimestamp** ``Optional[meta_v1.Time]`` - *(optional)* deprecatedLastTimestamp is the deprecated field assuring backward
        compatibility with core.v1 Event type.
      * **deprecatedSource** ``Optional[core_v1.EventSource]`` - *(optional)* deprecatedSource is the deprecated field assuring backward compatibility with
        core.v1 Event type.
      ...
      * **note** ``Optional[str]`` - *(optional)* note is a human-readable description of the status of this operation. Maximal
        length of the note is 1kB, but libraries should be prepared to handle values
        up to 64kB.
      * **reason** ``Optional[str]`` - *(optional)* reason is why the action was taken. It is human-readable. This field cannot be
        empty for new Events and it can have at most 128 characters.
      * **regarding** ``Optional[core_v1.ObjectReference]`` - *(optional)* regarding contains the object this Event is about. In most cases it's an
        ...
        ReplicaSet object.
      * **related** ``Optional[core_v1.ObjectReference]`` - *(optional)* related is the optional secondary object for more complex actions. E.g. when
        regarding object triggers a creation or deletion of related object.
      * **reportingController** ``Optional[str]`` - *(optional)* reportingController is the name of the controller that emitted this Event,
        e.g. `kubernetes.io/kubelet`. This field cannot be empty for new Events.
      * **reportingInstance** ``Optional[str]`` - *(optional)* reportingInstance is the ID of the controller instance, e.g. `kubelet-xyzf`.
        This field cannot be empty for new Events and it can have at most 128
        characters.
      * **series** ``Optional[EventSeries]`` - *(optional)* series is data about the Event series this event represents or nil if it's a
        singleton Event.
      * **type** ``Optional[str]`` - *(optional)* type is the type of this event (Normal, Warning), new types could be added in
