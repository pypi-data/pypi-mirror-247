...
class Endpoint(DictMixin):
    """Endpoint represents a single logical "backend" implementing a service.
      ...
      * **conditions** ``Optional[EndpointConditions]`` - *(optional)* conditions contains information about the current status of the endpoint.
      * **deprecatedTopology** ``Optional[dict]`` - *(optional)* deprecatedTopology contains topology information part of the v1beta1 API. This
        ...
        instead.
      * **hints** ``Optional[EndpointHints]`` - *(optional)* hints contains information associated with how an endpoint should be consumed.
      * **hostname** ``Optional[str]`` - *(optional)* hostname of this endpoint. This field may be used by consumers of endpoints to
      ...
      * **nodeName** ``Optional[str]`` - *(optional)* nodeName represents the name of the Node hosting this endpoint. This can be
        used to determine endpoints local to a Node.
      * **targetRef** ``Optional[core_v1.ObjectReference]`` - *(optional)* targetRef is a reference to a Kubernetes object that represents this endpoint.
      * **zone** ``Optional[str]`` - *(optional)* zone is the name of the Zone this endpoint exists in.
    """
    addresses: 'List[str]'
...
class EndpointConditions(DictMixin):
    """EndpointConditions represents the current condition of an endpoint.
      ...
      * **ready** ``Optional[bool]`` - *(optional)* ready indicates that this endpoint is prepared to receive traffic, according
        ...
        endpoints.
      * **serving** ``Optional[bool]`` - *(optional)* serving is identical to ready except that it is set regardless of the
      ...
      * **terminating** ``Optional[bool]`` - *(optional)* terminating indicates that this endpoint is terminating. A nil value indicates
...
class EndpointHints(DictMixin):
    """EndpointHints provides hints describing how an endpoint should be consumed.
      ...
      * **forZones** ``Optional[List[ForZone]]`` - *(optional)* forZones indicates the zone(s) this endpoint should be consumed by to enable
...
class EndpointPort(DictMixin):
    """EndpointPort represents a Port used by an EndpointSlice
      ...
      * **name** ``Optional[str]`` - *(optional)* The name of this port. All ports in an EndpointSlice must have a unique name.
      ...
      * **port** ``Optional[int]`` - *(optional)* The port number of the endpoint. If this is not specified, ports are not
...
class EndpointSlice(DictMixin):
    """EndpointSlice represents a subset of the endpoints that implement a service.
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object's metadata.
      * **ports** ``Optional[List[EndpointPort]]`` - *(optional)* ports specifies the list of network ports exposed by each endpoint in this
