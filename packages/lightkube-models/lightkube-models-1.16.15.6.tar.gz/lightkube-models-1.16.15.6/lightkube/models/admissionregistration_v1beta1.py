...
class MutatingWebhook(DictMixin):
    """MutatingWebhook describes an admission webhook and the resources and
      ...
      * **admissionReviewVersions** ``Optional[List[str]]`` - AdmissionReviewVersions is an ordered list of preferred `AdmissionReview`
      ...
      * **sideEffects** ``Optional[str]`` - SideEffects states whether this webhookk has side effects. Acceptable values
