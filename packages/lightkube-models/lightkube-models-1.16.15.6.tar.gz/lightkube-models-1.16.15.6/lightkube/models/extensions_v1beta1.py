...
class DaemonSetSpec(DictMixin):
    """DaemonSetSpec is the specification of a daemon set.
      ...
      * **selector** ``Optional[meta_v1.LabelSelector]`` - A label query over pods that are managed by the daemon set. Must match in
        order to be controlled. If empty, defaulted to labels on Pod template. More
        info:
        https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/#label-selectors
      * **templateGeneration** ``Optional[int]`` - DEPRECATED. A sequence number representing a specific generation of the
...
class DeploymentSpec(DictMixin):
    """DeploymentSpec is the specification of the desired behavior of the Deployment.
      ...
      * **paused** ``Optional[bool]`` - Indicates that the deployment is paused and will not be processed by the
...
class NetworkPolicyPort(DictMixin):
    """DEPRECATED 1.9 - This group version of NetworkPolicyPort is deprecated by
      ...
      * **port** ``Optional[util_intstr.IntOrString]`` - If specified, the port on the given protocol.  This can either be a numerical
        ...
        will be matched.
      * **protocol** ``Optional[str]`` - Optional.  The protocol (TCP, UDP, or SCTP) which traffic must match. If not
