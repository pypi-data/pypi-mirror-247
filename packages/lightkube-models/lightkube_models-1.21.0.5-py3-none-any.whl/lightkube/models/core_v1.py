...
class Container(DictMixin):
    """A single application container that you want to run within a pod.
      ...
      * **securityContext** ``Optional[SecurityContext]`` - *(optional)* Security options the pod should run with. More info:
...
class EnvVar(DictMixin):
    """EnvVar represents an environment variable present in a Container.
      ...
      * **value** ``Optional[str]`` - *(optional)* Variable references $(VAR_NAME) are expanded using the previous defined
...
class EphemeralContainer(DictMixin):
    """An EphemeralContainer is a container that may be added temporarily to an
      ...
      * **securityContext** ``Optional[SecurityContext]`` - *(optional)* SecurityContext is not allowed for ephemeral containers.
...
@dataclass
class EphemeralContainers(DictMixin):
    """A list of ephemeral containers used with the Pod ephemeralcontainers
      subresource.

      **parameters**

      * **ephemeralContainers** ``List[EphemeralContainer]`` - A list of ephemeral containers associated with this pod. New ephemeral
...
class Handler(DictMixin):
    """Handler defines a specific action that should be taken
      ...
      * **exec** ``Optional[ExecAction]`` - *(optional)* One and only one of the following should be specified. Exec specifies the
        action to take.
      * **httpGet** ``Optional[HTTPGetAction]`` - *(optional)* HTTPGet specifies the http request to perform.
      * **tcpSocket** ``Optional[TCPSocketAction]`` - *(optional)* TCPSocket specifies an action involving a TCP port. TCP hooks not yet
        supported
    """
...
class Lifecycle(DictMixin):
    """Lifecycle describes actions that the management system should take in response
      ...
      * **postStart** ``Optional[Handler]`` - *(optional)* PostStart is called immediately after a container is created. If the handler
      ...
      * **preStop** ``Optional[Handler]`` - *(optional)* PreStop is called immediately before a container is terminated due to an API
...
class NodeSpec(DictMixin):
    """NodeSpec describes the attributes that a node is created with.
      ...
      * **configSource** ``Optional[NodeConfigSource]`` - *(optional)* If specified, the source to get node configuration from The
...
class PersistentVolumeClaimSpec(DictMixin):
    """PersistentVolumeClaimSpec describes the common attributes of storage devices
      ...
      * **dataSource** ``Optional[TypedLocalObjectReference]`` - *(optional)* This field can be used to specify either: * An existing VolumeSnapshot object
        ...
        source.
      * **resources** ``Optional[ResourceRequirements]`` - *(optional)* Resources represents the minimum resources the volume should have. More info:
...
class PodAffinityTerm(DictMixin):
    """Defines a set of pods (namely those matching the labelSelector relative to the
      ...
      * **namespaceSelector** ``Optional[meta_v1.LabelSelector]`` - *(optional)* A label query over the set of namespaces that the term applies to. The term is
        ...
        namespaces. This field is alpha-level and is only honored when
...
class PodSecurityContext(DictMixin):
    """PodSecurityContext holds pod-level security attributes and common container
      ...
      * **seLinuxOptions** ``Optional[SELinuxOptions]`` - *(optional)* The SELinux context to be applied to all containers. If unspecified, the
        ...
        for that container.
      * **seccompProfile** ``Optional[SeccompProfile]`` - *(optional)* The seccomp options to use by the containers in this pod.
...
class Secret(DictMixin):
    """Secret holds secret data of a certain type. The total bytes of the values in
      ...
      * **type** ``Optional[str]`` - *(optional)* Used to facilitate programmatic handling of secret data.
    """
    apiVersion: 'Optional[str]' = None
    data: 'Optional[dict]' = None
    immutable: 'Optional[bool]' = None
...
class SecurityContext(DictMixin):
    """SecurityContext holds security configuration that will be applied to a
      ...
      * **procMount** ``Optional[str]`` - *(optional)* procMount denotes the type of proc mount to use for the containers. The
        ...
        to be enabled.
      * **readOnlyRootFilesystem** ``Optional[bool]`` - *(optional)* Whether this container has a read-only root filesystem. Default is false.
      * **runAsGroup** ``Optional[int]`` - *(optional)* The GID to run the entrypoint of the container process. Uses runtime default
...
class ServiceSpec(DictMixin):
    """ServiceSpec describes the attributes that a user creates on a service.
      ...
      * **topologyKeys** ``Optional[List[str]]`` - *(optional)* topologyKeys is a preference-order list of topology keys which implementations
        ...
        enable the ServiceTopology feature. This field is deprecated and will be
