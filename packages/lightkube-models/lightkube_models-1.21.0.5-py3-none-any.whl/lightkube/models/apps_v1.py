...
class RollingUpdateDaemonSet(DictMixin):
    """Spec to control the desired behavior of daemon set rolling update.
      ...
      * **maxSurge** ``Optional[util_intstr.IntOrString]`` - *(optional)* The maximum number of nodes with an existing available DaemonSet pod that can
        ...
        is an alpha field and requires enabling DaemonSetUpdateSurge feature gate.
      * **maxUnavailable** ``Optional[util_intstr.IntOrString]`` - *(optional)* The maximum number of DaemonSet pods that can be unavailable during the
        ...
        is calculated from percentage by rounding down to a minimum of one. This
        cannot be 0 if MaxSurge is 0 Default value is 1. Example: when this is set to
        30%, at most 30% of the total number of nodes that should be running the
        daemon pod (i.e. status.desiredNumberScheduled) can have their pods stopped
        for an update at any given time. The update starts by stopping at most 30% of
...
class RollingUpdateStatefulSetStrategy(DictMixin):
    """RollingUpdateStatefulSetStrategy is used to communicate parameter for
      ...
      * **partition** ``Optional[int]`` - *(optional)* Partition indicates the ordinal at which the StatefulSet should be
...
class StatefulSetList(DictMixin):
    """StatefulSetList is a collection of StatefulSets.
      ...
      * **kind** ``Optional[str]`` - *(optional)* Kind is a string value representing the REST resource this object represents.
        ...
        Cannot be updated. In CamelCase. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds
      * **metadata** ``Optional[meta_v1.ListMeta]`` - *(optional)* 
    """
    items: 'List[StatefulSet]'
    apiVersion: 'Optional[str]' = None
    kind: 'Optional[str]' = None
...
class StatefulSetStatus(DictMixin):
    """StatefulSetStatus represents the current state of a StatefulSet.
      ...
      * **readyReplicas** ``Optional[int]`` - *(optional)* readyReplicas is the number of Pods created by the StatefulSet controller that
