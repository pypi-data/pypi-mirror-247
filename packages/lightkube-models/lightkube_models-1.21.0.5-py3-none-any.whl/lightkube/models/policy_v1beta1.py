...
class Eviction(DictMixin):
    """Eviction evicts a pod from its node subject to certain policies and safety
      ...
      * **deleteOptions** ``Optional[meta_v1.DeleteOptions]`` - *(optional)* DeleteOptions may be provided
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* ObjectMeta describes the pod that is being evicted.
    """
...
class PodDisruptionBudget(DictMixin):
    """PodDisruptionBudget is an object to define the max disruption that can be
      ...
      * **spec** ``Optional[PodDisruptionBudgetSpec]`` - *(optional)* Specification of the desired behavior of the PodDisruptionBudget.
      * **status** ``Optional[PodDisruptionBudgetStatus]`` - *(optional)* Most recently observed status of the PodDisruptionBudget.
    """
    apiVersion: 'Optional[str]' = None
    kind: 'Optional[str]' = None
    metadata: 'Optional[meta_v1.ObjectMeta]' = None
...
class PodDisruptionBudgetSpec(DictMixin):
    """PodDisruptionBudgetSpec is a description of a PodDisruptionBudget.
      ...
      * **maxUnavailable** ``Optional[util_intstr.IntOrString]`` - *(optional)* An eviction is allowed if at most "maxUnavailable" pods selected by "selector"
      ...
      * **minAvailable** ``Optional[util_intstr.IntOrString]`` - *(optional)* An eviction is allowed if at least "minAvailable" pods selected by "selector"
        ...
        specifying "100%".
      * **selector** ``Optional[meta_v1.LabelSelector]`` - *(optional)* Label query over pods whose evictions are managed by the disruption budget. A
...
class PodDisruptionBudgetStatus(DictMixin):
    """PodDisruptionBudgetStatus represents information about the status of a
      ...
      * **conditions** ``Optional[List[meta_v1.Condition]]`` - *(optional)* Conditions contain conditions for PDB. The disruption controller sets the
      ...
      * **disruptedPods** ``Optional[dict]`` - *(optional)* DisruptedPods contains information about pods whose eviction was processed by
      ...
      * **observedGeneration** ``Optional[int]`` - *(optional)* Most recent generation observed when updating this PDB status.
...
class PodSecurityPolicySpec(DictMixin):
    """PodSecurityPolicySpec defines the policy enforced.
      ...
      * **allowedCSIDrivers** ``Optional[List[AllowedCSIDriver]]`` - *(optional)* AllowedCSIDrivers is an allowlist of inline CSI drivers that must be
      ...
      * **allowedFlexVolumes** ``Optional[List[AllowedFlexVolume]]`` - *(optional)* allowedFlexVolumes is an allowlist of Flexvolumes.  Empty or nil indicates
      ...
      * **allowedHostPaths** ``Optional[List[AllowedHostPath]]`` - *(optional)* allowedHostPaths is an allowlist of host paths. Empty indicates that all host
        paths may be used.
      * **allowedProcMountTypes** ``Optional[List[str]]`` - *(optional)* AllowedProcMountTypes is an allowlist of allowed ProcMountTypes. Empty or nil
      ...
      * **runtimeClass** ``Optional[RuntimeClassStrategyOptions]`` - *(optional)* runtimeClass is the strategy that will dictate the allowable RuntimeClasses
        ...
        gate being enabled.
      * **volumes** ``Optional[List[str]]`` - *(optional)* volumes is an allowlist of volume plugins. Empty indicates that no volumes may
