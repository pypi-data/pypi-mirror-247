...
class Overhead(DictMixin):
    """Overhead structure represents the resource overhead associated with running a
      ...
      * **podFixed** ``Optional[dict]`` - *(optional)* PodFixed represents the fixed resource overhead associated with running a pod.
    """
...
class RuntimeClassSpec(DictMixin):
    """RuntimeClassSpec is a specification of a RuntimeClass. It contains parameters
      ...
      * **runtimeHandler** ``str`` - RuntimeHandler specifies the underlying runtime and configuration that the CRI
        ...
        a pod. The RuntimeHandler must conform to the DNS Label (RFC 1123)
        requirements and is immutable.
      * **overhead** ``Optional[Overhead]`` - *(optional)* Overhead represents the resource overhead associated with running a pod for a
      ...
      * **scheduling** ``Optional[Scheduling]`` - *(optional)* Scheduling holds the scheduling constraints to ensure that pods running with
...
class Scheduling(DictMixin):
    """Scheduling specifies the scheduling constraints for nodes supporting a
      ...
      * **nodeSelector** ``Optional[dict]`` - *(optional)* nodeSelector lists labels that must be present on nodes that support this
        ...
        admission.
      * **tolerations** ``Optional[List[core_v1.Toleration]]`` - *(optional)* tolerations are appended (excluding duplicates) to pods running with this
