...
class ExemptPriorityLevelConfiguration(DictMixin):
    """ExemptPriorityLevelConfiguration describes the configurable aspects of the
      ...
      * **lendablePercent** ``Optional[int]`` - *(optional)* `lendablePercent` prescribes the fraction of the level's NominalCL that can be
        borrowed by other priority levels.  This value of this field must be between 0
        and 100, inclusive, and it defaults to 0. The number of seats that other
      ...
      * **nominalConcurrencyShares** ``Optional[int]`` - *(optional)* `nominalConcurrencyShares` (NCS) contributes to the computation of the
        ...
        limit the dispatching from this priority level but affects the other priority
        levels through the borrowing mechanism. The server's concurrency limit
        (ServerCL) is divided among all the priority levels in proportion to their NCS
        values:
        NominalCL(i)  = ceil( ServerCL * NCS(i) / sum_ncs ) sum_ncs = sum[priority
        level k] NCS(k)
        Bigger numbers mean a larger nominal concurrency limit, at the expense of
...
class FlowSchema(DictMixin):
    """FlowSchema defines the schema of a group of flows. Note that a flow is made up
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* `metadata` is the standard object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **spec** ``Optional[FlowSchemaSpec]`` - *(optional)* `spec` is the specification of the desired behavior of a FlowSchema. More
        info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
      * **status** ``Optional[FlowSchemaStatus]`` - *(optional)* `status` is the current status of a FlowSchema. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
    """
...
class FlowSchemaCondition(DictMixin):
    """FlowSchemaCondition describes conditions for a FlowSchema.
      ...
      * **lastTransitionTime** ``Optional[meta_v1.Time]`` - *(optional)* `lastTransitionTime` is the last time the condition transitioned from one
        status to another.
      * **message** ``Optional[str]`` - *(optional)* `message` is a human-readable message indicating details about last
        transition.
      * **reason** ``Optional[str]`` - *(optional)* `reason` is a unique, one-word, CamelCase reason for the condition's last
        transition.
      * **status** ``Optional[str]`` - *(optional)* `status` is the status of the condition. Can be True, False, Unknown.
        Required.
      * **type** ``Optional[str]`` - *(optional)* `type` is the type of the condition. Required.
    """
...
class FlowSchemaList(DictMixin):
    """FlowSchemaList is a list of FlowSchema objects.
      ...
      * **kind** ``Optional[str]`` - *(optional)* Kind is a string value representing the REST resource this object represents.
        ...
        Cannot be updated. In CamelCase. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds
      * **metadata** ``Optional[meta_v1.ListMeta]`` - *(optional)* `metadata` is the standard list metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
    """
...
class FlowSchemaSpec(DictMixin):
    """FlowSchemaSpec describes how the FlowSchema's specification looks like.
      ...
      * **distinguisherMethod** ``Optional[FlowDistinguisherMethod]`` - *(optional)* `distinguisherMethod` defines how to compute the flow distinguisher for
        requests that match this schema. `nil` specifies that the distinguisher is
        disabled and thus will always be the empty string.
      * **matchingPrecedence** ``Optional[int]`` - *(optional)* `matchingPrecedence` is used to choose among the FlowSchemas that match a
      ...
      * **rules** ``Optional[List[PolicyRulesWithSubjects]]`` - *(optional)* `rules` describes which requests will match this flow schema. This FlowSchema
...
class FlowSchemaStatus(DictMixin):
    """FlowSchemaStatus represents the current state of a FlowSchema.
      ...
      * **conditions** ``Optional[List[FlowSchemaCondition]]`` - *(optional)* `conditions` is a list of the current states of FlowSchema.
    """
...
class LimitResponse(DictMixin):
    """LimitResponse defines how to handle requests that can not be executed right
      ...
      * **queuing** ``Optional[QueuingConfiguration]`` - *(optional)* `queuing` holds the configuration parameters for queuing. This field may be
...
class LimitedPriorityLevelConfiguration(DictMixin):
    """LimitedPriorityLevelConfiguration specifies how to handle requests that are
      ...
      * **borrowingLimitPercent** ``Optional[int]`` - *(optional)* `borrowingLimitPercent`, if present, configures a limit on how many seats this
        ...
        total number of seats that this level may borrow at any one time. This field
        holds the ratio of that limit to the level's nominal concurrency limit. When
        this field is non-nil, it must hold a non-negative integer and the limit is
        calculated as follows.
        BorrowingCL(i) = round( NominalCL(i) * borrowingLimitPercent(i)/100.0 )
        The value of this field can be more than 100, implying that this priority
        level can borrow a number of seats that is greater than its own nominal
        concurrency limit (NominalCL). When this field is left `nil`, the limit is
        effectively infinite.
      * **lendablePercent** ``Optional[int]`` - *(optional)* `lendablePercent` prescribes the fraction of the level's NominalCL that can be
        borrowed by other priority levels. The value of this field must be between 0
        and 100, inclusive, and it defaults to 0. The number of seats that other
        levels can borrow from this level, known as this level's
        LendableConcurrencyLimit (LendableCL), is defined as follows.
        LendableCL(i) = round( NominalCL(i) * lendablePercent(i)/100.0 )
      * **limitResponse** ``Optional[LimitResponse]`` - *(optional)* `limitResponse` indicates what to do with requests that can not be executed
        right now
      * **nominalConcurrencyShares** ``Optional[int]`` - *(optional)* `nominalConcurrencyShares` (NCS) contributes to the computation of the
        NominalConcurrencyLimit (NominalCL) of this level. This is the number of
        execution seats available at this priority level. This is used both for
        requests dispatched from this priority level as well as requests dispatched
        from other priority levels borrowing seats from this level. The server's
        concurrency limit (ServerCL) is divided among the Limited priority levels in
        proportion to their NCS values:
        NominalCL(i)  = ceil( ServerCL * NCS(i) / sum_ncs ) sum_ncs = sum[priority
        level k] NCS(k)
        Bigger numbers mean a larger nominal concurrency limit, at the expense of
        every other priority level. This field has a default value of 30.
    """
...
class PolicyRulesWithSubjects(DictMixin):
    """PolicyRulesWithSubjects prescribes a test that applies to a request to an
      ...
      * **nonResourceRules** ``Optional[List[NonResourcePolicyRule]]`` - *(optional)* `nonResourceRules` is a list of NonResourcePolicyRules that identify matching
        requests according to their verb and the target non-resource URL.
      * **resourceRules** ``Optional[List[ResourcePolicyRule]]`` - *(optional)* `resourceRules` is a slice of ResourcePolicyRules that identify matching
...
class PriorityLevelConfiguration(DictMixin):
    """PriorityLevelConfiguration represents the configuration of a priority level.
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* `metadata` is the standard object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **spec** ``Optional[PriorityLevelConfigurationSpec]`` - *(optional)* `spec` is the specification of the desired behavior of a "request-priority".
        More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
      * **status** ``Optional[PriorityLevelConfigurationStatus]`` - *(optional)* `status` is the current status of a "request-priority". More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
    """
    apiVersion: 'Optional[str]' = None
    kind: 'Optional[str]' = None
...
class PriorityLevelConfigurationList(DictMixin):
    """PriorityLevelConfigurationList is a list of PriorityLevelConfiguration
      ...
      * **kind** ``Optional[str]`` - *(optional)* Kind is a string value representing the REST resource this object represents.
        ...
        Cannot be updated. In CamelCase. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds
      * **metadata** ``Optional[meta_v1.ListMeta]`` - *(optional)* `metadata` is the standard object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
    """
...
class PriorityLevelConfigurationSpec(DictMixin):
    """PriorityLevelConfigurationSpec specifies the configuration of a priority
      ...
      * **exempt** ``Optional[ExemptPriorityLevelConfiguration]`` - *(optional)* `exempt` specifies how requests are handled for an exempt priority level. This
        field MUST be empty if `type` is `"Limited"`. This field MAY be non-empty if
        `type` is `"Exempt"`. If empty and `type` is `"Exempt"` then the default
        values for `ExemptPriorityLevelConfiguration` apply.
      * **limited** ``Optional[LimitedPriorityLevelConfiguration]`` - *(optional)* `limited` specifies how requests are handled for a Limited priority level.
...
class PriorityLevelConfigurationStatus(DictMixin):
    """PriorityLevelConfigurationStatus represents the current state of a
      ...
      * **conditions** ``Optional[List[PriorityLevelConfigurationCondition]]`` - *(optional)* `conditions` is the current state of "request-priority".
    """
...
class QueuingConfiguration(DictMixin):
    """QueuingConfiguration holds the configuration parameters for queuing
      ...
      * **handSize** ``Optional[int]`` - *(optional)* `handSize` is a small positive number that configures the shuffle sharding of
      ...
      * **queueLengthLimit** ``Optional[int]`` - *(optional)* `queueLengthLimit` is the maximum number of requests allowed to be waiting in
      ...
      * **queues** ``Optional[int]`` - *(optional)* `queues` is the number of queues for this priority level. The queues exist
...
class ResourcePolicyRule(DictMixin):
    """ResourcePolicyRule is a predicate that matches some resource requests, testing
      ...
      * **clusterScope** ``Optional[bool]`` - *(optional)* `clusterScope` indicates whether to match requests that do not specify a
      ...
      * **namespaces** ``Optional[List[str]]`` - *(optional)* `namespaces` is a list of target namespaces that restricts matches.  A request
...
class Subject(DictMixin):
    """Subject matches the originator of a request, as identified by the request
      ...
      * **group** ``Optional[GroupSubject]`` - *(optional)* `group` matches based on user group name.
      * **serviceAccount** ``Optional[ServiceAccountSubject]`` - *(optional)* `serviceAccount` matches ServiceAccounts.
      * **user** ``Optional[UserSubject]`` - *(optional)* `user` matches based on username.
    """
    kind: 'str'
