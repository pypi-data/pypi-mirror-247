...
class BoundObjectReference(DictMixin):
    """BoundObjectReference is a reference to an object that a token is bound to.
      ...
      * **kind** ``Optional[str]`` - *(optional)* Kind of the referent. Valid kinds are 'Pod' and 'Secret'.
      * **name** ``Optional[str]`` - *(optional)* Name of the referent.
      * **uid** ``Optional[str]`` - *(optional)* UID of the referent.
    """
...
class SelfSubjectReview(DictMixin):
    """SelfSubjectReview contains the user information that the kube-apiserver has
      ...
      * **status** ``Optional[SelfSubjectReviewStatus]`` - *(optional)* Status is filled in by the server with the user attributes.
    """
    apiVersion: 'Optional[str]' = None
    kind: 'Optional[str]' = None
...
class SelfSubjectReviewStatus(DictMixin):
    """SelfSubjectReviewStatus is filled by the kube-apiserver and sent back to a
      ...
      * **userInfo** ``Optional[UserInfo]`` - *(optional)* User attributes of the user making this request.
    """
...
class TokenRequest(DictMixin):
    """TokenRequest requests a token for a given service account.
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **status** ``Optional[TokenRequestStatus]`` - *(optional)* Status is filled in by the server and indicates whether the token can be
        authenticated.
    """
...
class TokenRequestSpec(DictMixin):
    """TokenRequestSpec contains client provided parameters of a token request.
      ...
      * **boundObjectRef** ``Optional[BoundObjectReference]`` - *(optional)* BoundObjectRef is a reference to an object that the token will be bound to.
      ...
      * **expirationSeconds** ``Optional[int]`` - *(optional)* ExpirationSeconds is the requested duration of validity of the request. The
