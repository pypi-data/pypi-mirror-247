...
class CSIDriverSpec(DictMixin):
    """CSIDriverSpec is the specification of a CSIDriver.
      ...
      * **fsGroupPolicy** ``Optional[str]`` - *(optional)* fsGroupPolicy defines if the underlying volume supports changing ownership and
      ...
      * **podInfoOnMount** ``Optional[bool]`` - *(optional)* podInfoOnMount indicates this CSI volume driver requires additional pod
      ...
      * **seLinuxMount** ``Optional[bool]`` - *(optional)* seLinuxMount specifies if the CSI driver supports "-o context" mount option.
        ...
        shared filesystem.
        Default is "false".
      * **storageCapacity** ``Optional[bool]`` - *(optional)* storageCapacity indicates that the CSI volume driver wants pod scheduling to
...
class CSINode(DictMixin):
    """CSINode holds information about all CSI drivers installed on a node. CSI
      ...
      * **kind** ``Optional[str]`` - *(optional)* Kind is a string value representing the REST resource this object represents.
        ...
        Cannot be updated. In CamelCase. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object's metadata. metadata.name must be the Kubernetes node name.
    """
    spec: 'CSINodeSpec'
    apiVersion: 'Optional[str]' = None
    kind: 'Optional[str]' = None
...
class CSIStorageCapacity(DictMixin):
    """CSIStorageCapacity stores the result of one CSI GetCapacity call. For a given
      ...
      * **maximumVolumeSize** ``Optional[resource.Quantity]`` - *(optional)* maximumVolumeSize is the value reported by the CSI driver in its
        ...
        claim.
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object's metadata. The name has no particular meaning. It must be a
...
class StorageClass(DictMixin):
    """StorageClass describes the parameters for a class of storage for which
      ...
      * **allowedTopologies** ``Optional[List[core_v1.TopologySelectorTerm]]`` - *(optional)* allowedTopologies restrict the node topologies where volumes can be
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **mountOptions** ``Optional[List[str]]`` - *(optional)* mountOptions controls the mountOptions for dynamically provisioned
      ...
      * **reclaimPolicy** ``Optional[str]`` - *(optional)* reclaimPolicy controls the reclaimPolicy for dynamically provisioned
...
class VolumeAttachment(DictMixin):
    """VolumeAttachment captures the intent to attach or detach the specified volume
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **status** ``Optional[VolumeAttachmentStatus]`` - *(optional)* status represents status of the VolumeAttachment request. Populated by the
...
class VolumeAttachmentSource(DictMixin):
    """VolumeAttachmentSource represents a volume that should be attached. Right now
      ...
      * **persistentVolumeName** ``Optional[str]`` - *(optional)* persistentVolumeName represents the name of the persistent volume to attach.
    """
...
class VolumeAttachmentStatus(DictMixin):
    """VolumeAttachmentStatus is the status of a VolumeAttachment request.
      ...
      * **attached** ``bool`` - attached indicates the volume is successfully attached. This field must only
        ...
        external-attacher.
      * **attachError** ``Optional[VolumeError]`` - *(optional)* attachError represents the last error encountered during attach operation, if
        any. This field must only be set by the entity completing the attach
        operation, i.e. the external-attacher.
      * **attachmentMetadata** ``Optional[dict]`` - *(optional)* attachmentMetadata is populated with any information returned by the attach
      ...
      * **detachError** ``Optional[VolumeError]`` - *(optional)* detachError represents the last error encountered during detach operation, if
...
class VolumeError(DictMixin):
    """VolumeError captures an error encountered during a volume operation.
      ...
      * **message** ``Optional[str]`` - *(optional)* message represents the error encountered during Attach or Detach operation.
        This string may be logged, so it should not contain sensitive information.
      * **time** ``Optional[meta_v1.Time]`` - *(optional)* time represents the time the error was encountered.
    """
...
class VolumeNodeResources(DictMixin):
    """VolumeNodeResources is a set of resource limits for scheduling of volumes.
      ...
      * **count** ``Optional[int]`` - *(optional)* count indicates the maximum number of unique volumes managed by the CSI driver
