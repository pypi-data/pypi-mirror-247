...
class AllocationResult(DictMixin):
    """AllocationResult contains attributes of an allocated resource.
      ...
      * **availableOnNodes** ``Optional[core_v1.NodeSelector]`` - *(optional)* This field will get set by the resource driver after it has allocated the
        resource to inform the scheduler where it can schedule Pods using the
        ResourceClaim.
        Setting this field is optional. If null, the resource is available everywhere.
      * **resourceHandles** ``Optional[List[ResourceHandle]]`` - *(optional)* ResourceHandles contain the state associated with an allocation that should be
      ...
      * **shareable** ``Optional[bool]`` - *(optional)* Shareable determines whether the resource supports more than one consumer at a
        time.
    """
...
class PodSchedulingContext(DictMixin):
    """PodSchedulingContext objects hold information that is needed to schedule a Pod
      ...
      * **status** ``Optional[PodSchedulingContextStatus]`` - *(optional)* Status describes where resources for the Pod can be allocated.
    """
...
class PodSchedulingContextSpec(DictMixin):
    """PodSchedulingContextSpec describes where resources for the Pod are needed.
      ...
      * **potentialNodes** ``Optional[List[str]]`` - *(optional)* PotentialNodes lists nodes where the Pod might be able to run.
      ...
      * **selectedNode** ``Optional[str]`` - *(optional)* SelectedNode is the node for which allocation of ResourceClaims that are
...
class PodSchedulingContextStatus(DictMixin):
    """PodSchedulingContextStatus describes where resources for the Pod can be
      ...
      * **resourceClaims** ``Optional[List[ResourceClaimSchedulingStatus]]`` - *(optional)* ResourceClaims describes resource availability for each pod.spec.resourceClaim
...
class ResourceClaim(DictMixin):
    """ResourceClaim describes which resources are needed by a resource consumer. Its
      ...
      * **kind** ``Optional[str]`` - *(optional)* Kind is a string value representing the REST resource this object represents.
        ...
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object metadata
      * **status** ``Optional[ResourceClaimStatus]`` - *(optional)* Status describes whether the resource is available and with which attributes.
    """
...
class ResourceClaimConsumerReference(DictMixin):
    """ResourceClaimConsumerReference contains enough information to let you locate
      ...
      * **apiGroup** ``Optional[str]`` - *(optional)* APIGroup is the group for the resource being referenced. It is empty for the
...
class ResourceClaimSchedulingStatus(DictMixin):
    """ResourceClaimSchedulingStatus contains information about one particular
      ...
      * **name** ``Optional[str]`` - *(optional)* Name matches the pod.spec.resourceClaims[*].Name field.
      * **unsuitableNodes** ``Optional[List[str]]`` - *(optional)* UnsuitableNodes lists nodes that the ResourceClaim cannot be allocated for.
...
class ResourceClaimSpec(DictMixin):
    """ResourceClaimSpec defines how a resource is to be allocated.
      ...
      * **allocationMode** ``Optional[str]`` - *(optional)* Allocation can start immediately or when a Pod wants to use the resource.
        "WaitForFirstConsumer" is the default.
      * **parametersRef** ``Optional[ResourceClaimParametersReference]`` - *(optional)* ParametersRef references a separate object with arbitrary parameters that will
...
class ResourceClaimStatus(DictMixin):
    """ResourceClaimStatus tracks whether the resource has been allocated and what
      ...
      * **allocation** ``Optional[AllocationResult]`` - *(optional)* Allocation is set by the resource driver once a resource or set of resources
        has been allocated successfully. If this is not specified, the resources have
        not been allocated yet.
      * **deallocationRequested** ``Optional[bool]`` - *(optional)* DeallocationRequested indicates that a ResourceClaim is to be deallocated.
        The driver then must deallocate this claim and reset the field together with
        clearing the Allocation field.
        While DeallocationRequested is set, no new consumers may be added to
        ReservedFor.
      * **driverName** ``Optional[str]`` - *(optional)* DriverName is a copy of the driver name from the ResourceClass at the time
        when allocation started.
      * **reservedFor** ``Optional[List[ResourceClaimConsumerReference]]`` - *(optional)* ReservedFor indicates which entities are currently allowed to use the claim. A
...
class ResourceClaimTemplateSpec(DictMixin):
    """ResourceClaimTemplateSpec contains the metadata and fields for a
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* ObjectMeta may contain labels and annotations that will be copied into the PVC
...
class ResourceClass(DictMixin):
    """ResourceClass is used by administrators to influence how resources are
      ...
      * **kind** ``Optional[str]`` - *(optional)* Kind is a string value representing the REST resource this object represents.
        ...
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object metadata
      * **parametersRef** ``Optional[ResourceClassParametersReference]`` - *(optional)* ParametersRef references an arbitrary separate object that may hold parameters
      ...
      * **suitableNodes** ``Optional[core_v1.NodeSelector]`` - *(optional)* Only nodes matching the selector will be considered by the scheduler when
...
class ResourceClassParametersReference(DictMixin):
    """ResourceClassParametersReference contains enough information to let you locate
      ...
      * **apiGroup** ``Optional[str]`` - *(optional)* APIGroup is the group for the resource being referenced. It is empty for the
        core API. This matches the group in the APIVersion that is used when creating
        the resources.
      * **namespace** ``Optional[str]`` - *(optional)* Namespace that contains the referenced resource. Must be empty for
...
class ResourceHandle(DictMixin):
    """ResourceHandle holds opaque resource data for processing by a specific kubelet
      ...
      * **data** ``Optional[str]`` - *(optional)* Data contains the opaque data associated with this ResourceHandle. It is set
        ...
        but not reduced.
      * **driverName** ``Optional[str]`` - *(optional)* DriverName specifies the name of the resource driver whose kubelet plugin
