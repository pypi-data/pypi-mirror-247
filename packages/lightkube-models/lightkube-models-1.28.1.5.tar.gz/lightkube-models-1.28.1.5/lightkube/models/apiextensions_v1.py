...
class CustomResourceColumnDefinition(DictMixin):
    """CustomResourceColumnDefinition specifies a column for server side printing.
      ...
      * **type** ``str`` - type is an OpenAPI type definition for this column. See
        ...
        for details.
      * **description** ``Optional[str]`` - *(optional)* description is a human readable description of this column.
      * **format** ``Optional[str]`` - *(optional)* format is an optional OpenAPI type definition for this column. The 'name'
        format is applied to the primary identifier column to assist in clients
        identifying column is the resource name. See
        https://github.com/OAI/OpenAPI-Specification/blob/master/versions/2.0.md#data-types
        for details.
      * **priority** ``Optional[int]`` - *(optional)* priority is an integer defining the relative importance of this column
...
class CustomResourceConversion(DictMixin):
    """CustomResourceConversion describes how to convert different versions of a CR.
      ...
      * **webhook** ``Optional[WebhookConversion]`` - *(optional)* webhook describes how to call the conversion webhook. Required when `strategy`
        is set to `"Webhook"`.
    """
    strategy: 'str'
...
class CustomResourceDefinition(DictMixin):
    """CustomResourceDefinition represents a resource that should be exposed on the
      ...
      * **status** ``Optional[CustomResourceDefinitionStatus]`` - *(optional)* status indicates the actual state of the CustomResourceDefinition
    """
...
class CustomResourceDefinitionCondition(DictMixin):
    """CustomResourceDefinitionCondition contains details for the current condition
      ...
      * **type** ``str`` - type is the type of the condition. Types include Established, NamesAccepted
        and Terminating.
      * **lastTransitionTime** ``Optional[meta_v1.Time]`` - *(optional)* lastTransitionTime last time the condition transitioned from one status to
        another.
      * **message** ``Optional[str]`` - *(optional)* message is a human-readable message indicating details about last transition.
      * **reason** ``Optional[str]`` - *(optional)* reason is a unique, one-word, CamelCase reason for the condition's last
        transition.
    """
    status: 'str'
    type: 'str'
...
class CustomResourceDefinitionNames(DictMixin):
    """CustomResourceDefinitionNames indicates the names to serve this
      ...
      * **plural** ``str`` - plural is the plural name of the resource to serve. The custom resources are
        ...
        all lowercase.
      * **categories** ``Optional[List[str]]`` - *(optional)* categories is a list of grouped resources this custom resource belongs to
        (e.g. 'all'). This is published in API discovery documents, and used by
        clients to support invocations like `kubectl get all`.
      * **listKind** ``Optional[str]`` - *(optional)* listKind is the serialized kind of the list for this resource. Defaults to
        "`kind`List".
      * **shortNames** ``Optional[List[str]]`` - *(optional)* shortNames are short names for the resource, exposed in API discovery
        documents, and used by clients to support invocations like `kubectl get
        <shortname>`. It must be all lowercase.
      * **singular** ``Optional[str]`` - *(optional)* singular is the singular name of the resource. It must be all lowercase.
...
class CustomResourceDefinitionSpec(DictMixin):
    """CustomResourceDefinitionSpec describes how a user wants their resource to
      ...
      * **conversion** ``Optional[CustomResourceConversion]`` - *(optional)* conversion defines conversion settings for the CRD.
      * **preserveUnknownFields** ``Optional[bool]`` - *(optional)* preserveUnknownFields indicates that object fields which are not specified in
...
class CustomResourceDefinitionStatus(DictMixin):
    """CustomResourceDefinitionStatus indicates the state of the
      ...
      * **acceptedNames** ``Optional[CustomResourceDefinitionNames]`` - *(optional)* acceptedNames are the names that are actually being used to serve discovery.
        They may be different than the names in spec.
      * **conditions** ``Optional[List[CustomResourceDefinitionCondition]]`` - *(optional)* conditions indicate state for particular aspects of a CustomResourceDefinition
      * **storedVersions** ``Optional[List[str]]`` - *(optional)* storedVersions lists all versions of CustomResources that were ever persisted.
...
class CustomResourceDefinitionVersion(DictMixin):
    """CustomResourceDefinitionVersion describes a version for CRD.
      ...
      * **additionalPrinterColumns** ``Optional[List[CustomResourceColumnDefinition]]`` - *(optional)* additionalPrinterColumns specifies additional columns returned in Table
        output. See
      ...
      * **deprecated** ``Optional[bool]`` - *(optional)* deprecated indicates this version of the custom resource API is deprecated.
        When set to true, API requests to this version receive a warning header in the
        server response. Defaults to false.
      * **deprecationWarning** ``Optional[str]`` - *(optional)* deprecationWarning overrides the default warning returned to API clients. May
      ...
      * **schema** ``Optional[CustomResourceValidation]`` - *(optional)* schema describes the schema used for validation, pruning, and defaulting of
        this version of the custom resource.
      * **subresources** ``Optional[CustomResourceSubresources]`` - *(optional)* subresources specify what subresources this version of the defined custom
        resource have.
    """
    name: 'str'
    served: 'bool'
...
class CustomResourceSubresourceScale(DictMixin):
    """CustomResourceSubresourceScale defines how to serve the scale subresource for
      ...
      * **labelSelectorPath** ``Optional[str]`` - *(optional)* labelSelectorPath defines the JSON path inside of a custom resource that
...
class CustomResourceSubresources(DictMixin):
    """CustomResourceSubresources defines the status and scale subresources for
      ...
      * **scale** ``Optional[CustomResourceSubresourceScale]`` - *(optional)* scale indicates the custom resource should serve a `/scale` subresource that
        returns an `autoscaling/v1` Scale object.
      * **status** ``Optional[CustomResourceSubresourceStatus]`` - *(optional)* status indicates the custom resource should serve a `/status` subresource.
...
class CustomResourceValidation(DictMixin):
    """CustomResourceValidation is a list of validation methods for CustomResources.
      ...
      * **openAPIV3Schema** ``Optional[JSONSchemaProps]`` - *(optional)* openAPIV3Schema is the OpenAPI v3 schema to use for validation and pruning.
    """
...
class JSONSchemaProps(DictMixin):
    """JSONSchemaProps is a JSON-Schema following Specification Draft 4
      ...
      * **d_ref** ``Optional[str]`` - *(optional)* 
      * **d_schema** ``Optional[str]`` - *(optional)* 
      * **additionalItems** ``Optional[JSONSchemaPropsOrBool]`` - *(optional)* 
      * **additionalProperties** ``Optional[JSONSchemaPropsOrBool]`` - *(optional)* 
      * **allOf** ``Optional[List[JSONSchemaProps]]`` - *(optional)* 
      * **anyOf** ``Optional[List[JSONSchemaProps]]`` - *(optional)* 
      * **default** ``Optional[JSON]`` - *(optional)* default is a default value for undefined object fields. Defaulting is a beta
      ...
      * **dependencies** ``Optional[dict]`` - *(optional)* 
      * **description** ``Optional[str]`` - *(optional)* 
      * **enum** ``Optional[List[JSON]]`` - *(optional)* 
      * **example** ``Optional[JSON]`` - *(optional)* 
      * **exclusiveMaximum** ``Optional[bool]`` - *(optional)* 
      * **exclusiveMinimum** ``Optional[bool]`` - *(optional)* 
      * **externalDocs** ``Optional[ExternalDocumentation]`` - *(optional)* 
      * **format** ``Optional[str]`` - *(optional)* format is an OpenAPI v3 format string. Unknown formats are ignored. The
      ...
      * **items** ``Optional[JSONSchemaPropsOrArray]`` - *(optional)* 
      * **maxItems** ``Optional[int]`` - *(optional)* 
      * **maxLength** ``Optional[int]`` - *(optional)* 
      * **maxProperties** ``Optional[int]`` - *(optional)* 
      * **maximum** ``Optional[float]`` - *(optional)* 
      * **minItems** ``Optional[int]`` - *(optional)* 
      * **minLength** ``Optional[int]`` - *(optional)* 
      * **minProperties** ``Optional[int]`` - *(optional)* 
      * **minimum** ``Optional[float]`` - *(optional)* 
      * **multipleOf** ``Optional[float]`` - *(optional)* 
      * **not_** ``Optional[JSONSchemaProps]`` - *(optional)* 
      * **nullable** ``Optional[bool]`` - *(optional)* 
      * **oneOf** ``Optional[List[JSONSchemaProps]]`` - *(optional)* 
      * **pattern** ``Optional[str]`` - *(optional)* 
      * **patternProperties** ``Optional[dict]`` - *(optional)* 
      * **properties** ``Optional[dict]`` - *(optional)* 
      * **required** ``Optional[List[str]]`` - *(optional)* 
      * **title** ``Optional[str]`` - *(optional)* 
      * **type** ``Optional[str]`` - *(optional)* 
      * **uniqueItems** ``Optional[bool]`` - *(optional)* 
      * **x_kubernetes_embedded_resource** ``Optional[bool]`` - *(optional)* x-kubernetes-embedded-resource defines that the value is an embedded
      ...
      * **x_kubernetes_int_or_string** ``Optional[bool]`` - *(optional)* x-kubernetes-int-or-string specifies that this value is either an integer or a
        ...
        2) allOf:
           - anyOf:
             ...
             - type: string
           - ... zero or more
      * **x_kubernetes_list_map_keys** ``Optional[List[str]]`` - *(optional)* x-kubernetes-list-map-keys annotates an array with the x-kubernetes-list-type
      ...
      * **x_kubernetes_list_type** ``Optional[str]`` - *(optional)* x-kubernetes-list-type annotates an array to further describe its topology.
      ...
      * **x_kubernetes_map_type** ``Optional[str]`` - *(optional)* x-kubernetes-map-type annotates an object to further describe its topology.
      ...
      * **x_kubernetes_preserve_unknown_fields** ``Optional[bool]`` - *(optional)* x-kubernetes-preserve-unknown-fields stops the API server decoding step from
      ...
      * **x_kubernetes_validations** ``Optional[List[ValidationRule]]`` - *(optional)* x-kubernetes-validations describes a list of validation rules written in the
        CEL expression language. This field is an alpha-level. Using this field
        requires the feature gate `CustomResourceValidationExpressions` to be enabled.
    """
...
class ServiceReference(DictMixin):
    """ServiceReference holds a reference to Service.legacy.k8s.io
      ...
      * **path** ``Optional[str]`` - *(optional)* path is an optional URL path at which the webhook will be contacted.
      * **port** ``Optional[int]`` - *(optional)* port is an optional service port at which the webhook will be contacted.
...
class ValidationRule(DictMixin):
    """ValidationRule describes a validation rule written in the CEL expression
      ...
      * **fieldPath** ``Optional[str]`` - *(optional)* fieldPath represents the field path returned when the validation fails. It
        ...
        to `.testList['foo.34$']`
      * **message** ``Optional[str]`` - *(optional)* Message represents the message displayed when validation fails. The message is
      ...
      * **messageExpression** ``Optional[str]`` - *(optional)* MessageExpression declares a CEL expression that evaluates to the validation
        ...
        ("+string(self.max)+")"
      * **reason** ``Optional[str]`` - *(optional)* reason provides a machine-readable validation failure reason that is returned
...
class WebhookClientConfig(DictMixin):
    """WebhookClientConfig contains the information to make a TLS connection with the
      ...
      * **caBundle** ``Optional[str]`` - *(optional)* caBundle is a PEM encoded CA bundle which will be used to validate the
        webhook's server certificate. If unspecified, system trust roots on the
        apiserver are used.
      * **service** ``Optional[ServiceReference]`` - *(optional)* service is a reference to the service for this webhook. Either service or url
        must be specified.
        If the webhook is running within the cluster, then you should use `service`.
      * **url** ``Optional[str]`` - *(optional)* url gives the location of the webhook, in standard URL form
...
class WebhookConversion(DictMixin):
    """WebhookConversion describes how to call a conversion webhook
      ...
      * **clientConfig** ``Optional[WebhookClientConfig]`` - *(optional)* clientConfig is the instructions for how to call the webhook if strategy is
        `Webhook`.
    """
