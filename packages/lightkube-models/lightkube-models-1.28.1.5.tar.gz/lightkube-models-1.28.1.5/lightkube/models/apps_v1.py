...
class ControllerRevision(DictMixin):
    """ControllerRevision implements an immutable snapshot of state data. Clients are
      ...
      * **data** ``Optional[runtime.RawExtension]`` - *(optional)* Data is the serialized representation of the state.
...
class DeploymentSpec(DictMixin):
    """DeploymentSpec is the specification of the desired behavior of the Deployment.
      ...
      * **paused** ``Optional[bool]`` - *(optional)* Indicates that the deployment is paused.
...
class DeploymentStatus(DictMixin):
    """DeploymentStatus is the most recently observed status of the Deployment.
      ...
      * **readyReplicas** ``Optional[int]`` - *(optional)* readyReplicas is the number of pods targeted by this Deployment with a Ready
        Condition.
...
class ReplicaSetStatus(DictMixin):
    """ReplicaSetStatus represents the current status of a ReplicaSet.
      ...
      * **observedGeneration** ``Optional[int]`` - *(optional)* ObservedGeneration reflects the generation of the most recently observed
        ReplicaSet.
      * **readyReplicas** ``Optional[int]`` - *(optional)* readyReplicas is the number of pods targeted by this ReplicaSet with a Ready
        Condition.
    """
    replicas: 'int'
    availableReplicas: 'Optional[int]' = None
...
class RollingUpdateDaemonSet(DictMixin):
    """Spec to control the desired behavior of daemon set rolling update.
      ...
      * **maxSurge** ``Optional[util_intstr.IntOrString]`` - *(optional)* The maximum number of nodes with an existing available DaemonSet pod that can
...
class RollingUpdateStatefulSetStrategy(DictMixin):
    """RollingUpdateStatefulSetStrategy is used to communicate parameter for
      ...
      * **partition** ``Optional[int]`` - *(optional)* Partition indicates the ordinal at which the StatefulSet should be partitioned
...
class StatefulSet(DictMixin):
    """StatefulSet represents a set of pods with consistent identities. Identities
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **spec** ``Optional[StatefulSetSpec]`` - *(optional)* Spec defines the desired identities of pods in this set.
      * **status** ``Optional[StatefulSetStatus]`` - *(optional)* Status is the current status of Pods in this StatefulSet. This data may be out
...
class StatefulSetList(DictMixin):
    """StatefulSetList is a collection of StatefulSets.
      ...
      * **kind** ``Optional[str]`` - *(optional)* Kind is a string value representing the REST resource this object represents.
        ...
        Cannot be updated. In CamelCase. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds
      * **metadata** ``Optional[meta_v1.ListMeta]`` - *(optional)* Standard list's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
    """
...
class StatefulSetOrdinals(DictMixin):
    """StatefulSetOrdinals describes the policy used for replica ordinal assignment
      ...
      * **start** ``Optional[int]`` - *(optional)* start is the number representing the first replica's index. It may be used to
...
class StatefulSetPersistentVolumeClaimRetentionPolicy(DictMixin):
    """StatefulSetPersistentVolumeClaimRetentionPolicy describes the policy used for
      ...
      * **whenDeleted** ``Optional[str]`` - *(optional)* WhenDeleted specifies what happens to PVCs created from StatefulSet
      ...
      * **whenScaled** ``Optional[str]`` - *(optional)* WhenScaled specifies what happens to PVCs created from StatefulSet
...
class StatefulSetSpec(DictMixin):
    """A StatefulSetSpec is the specification of a StatefulSet.
      ...
      * **minReadySeconds** ``Optional[int]`` - *(optional)* Minimum number of seconds for which a newly created pod should be ready
        without any of its container crashing for it to be considered available.
        Defaults to 0 (pod will be considered available as soon as it is ready)
      * **ordinals** ``Optional[StatefulSetOrdinals]`` - *(optional)* ordinals controls the numbering of replica indices in a StatefulSet. The
      ...
      * **persistentVolumeClaimRetentionPolicy** ``Optional[StatefulSetPersistentVolumeClaimRetentionPolicy]`` - *(optional)* persistentVolumeClaimRetentionPolicy describes the lifecycle of persistent
      ...
      * **podManagementPolicy** ``Optional[str]`` - *(optional)* podManagementPolicy controls how pods are created during initial scale up,
      ...
      * **replicas** ``Optional[int]`` - *(optional)* replicas is the desired number of replicas of the given Template. These are
        ...
        to 1.
      * **revisionHistoryLimit** ``Optional[int]`` - *(optional)* revisionHistoryLimit is the maximum number of revisions that will be
        maintained in the StatefulSet's revision history. The revision history
        consists of all revisions not represented by a currently applied
        StatefulSetSpec version. The default value is 10.
      * **updateStrategy** ``Optional[StatefulSetUpdateStrategy]`` - *(optional)* updateStrategy indicates the StatefulSetUpdateStrategy that will be employed
        to update Pods in the StatefulSet when a revision is made to Template.
      * **volumeClaimTemplates** ``Optional[List[core_v1.PersistentVolumeClaim]]`` - *(optional)* volumeClaimTemplates is a list of claims that pods are allowed to reference.
...
class StatefulSetStatus(DictMixin):
    """StatefulSetStatus represents the current state of a StatefulSet.
      ...
      * **availableReplicas** ``Optional[int]`` - *(optional)* Total number of available pods (ready for at least minReadySeconds) targeted
        by this statefulset.
      * **collisionCount** ``Optional[int]`` - *(optional)* collisionCount is the count of hash collisions for the StatefulSet. The
      ...
      * **conditions** ``Optional[List[StatefulSetCondition]]`` - *(optional)* Represents the latest available observations of a statefulset's current state.
      * **currentReplicas** ``Optional[int]`` - *(optional)* currentReplicas is the number of Pods created by the StatefulSet controller
        from the StatefulSet version indicated by currentRevision.
      * **currentRevision** ``Optional[str]`` - *(optional)* currentRevision, if not empty, indicates the version of the StatefulSet used
        to generate Pods in the sequence [0,currentReplicas).
      * **observedGeneration** ``Optional[int]`` - *(optional)* observedGeneration is the most recent generation observed for this
        StatefulSet. It corresponds to the StatefulSet's generation, which is updated
        on mutation by the API Server.
      * **readyReplicas** ``Optional[int]`` - *(optional)* readyReplicas is the number of pods created for this StatefulSet with a Ready
        Condition.
      * **updateRevision** ``Optional[str]`` - *(optional)* updateRevision, if not empty, indicates the version of the StatefulSet used to
        generate Pods in the sequence [replicas-updatedReplicas,replicas)
      * **updatedReplicas** ``Optional[int]`` - *(optional)* updatedReplicas is the number of Pods created by the StatefulSet controller
...
class StatefulSetUpdateStrategy(DictMixin):
    """StatefulSetUpdateStrategy indicates the strategy that the StatefulSet
      ...
      * **rollingUpdate** ``Optional[RollingUpdateStatefulSetStrategy]`` - *(optional)* RollingUpdate is used to communicate parameters when Type is
        RollingUpdateStatefulSetStrategyType.
      * **type** ``Optional[str]`` - *(optional)* Type indicates the type of the StatefulSetUpdateStrategy. Default is
        RollingUpdate.
    """
