...
class AWSElasticBlockStoreVolumeSource(DictMixin):
    """Represents a Persistent Disk resource in AWS.
      ...
      * **fsType** ``Optional[str]`` - *(optional)* fsType is the filesystem type of the volume that you want to mount. Tip:
        ...
        https://kubernetes.io/docs/concepts/storage/volumes#awselasticblockstore
      * **partition** ``Optional[int]`` - *(optional)* partition is the partition in the volume that you want to mount. If omitted,
      ...
      * **readOnly** ``Optional[bool]`` - *(optional)* readOnly value true will force the readOnly setting in VolumeMounts. More
...
class Affinity(DictMixin):
    """Affinity is a group of affinity scheduling rules.
      ...
      * **nodeAffinity** ``Optional[NodeAffinity]`` - *(optional)* Describes node affinity scheduling rules for the pod.
      * **podAffinity** ``Optional[PodAffinity]`` - *(optional)* Describes pod affinity scheduling rules (e.g. co-locate this pod in the same
        node, zone, etc. as some other pod(s)).
      * **podAntiAffinity** ``Optional[PodAntiAffinity]`` - *(optional)* Describes pod anti-affinity scheduling rules (e.g. avoid putting this pod in
...
class AzureDiskVolumeSource(DictMixin):
    """AzureDisk represents an Azure Data Disk mount on the host and bind mount to
      ...
      * **cachingMode** ``Optional[str]`` - *(optional)* cachingMode is the Host Caching mode: None, Read Only, Read Write.
      * **fsType** ``Optional[str]`` - *(optional)* fsType is Filesystem type to mount. Must be a filesystem type supported by the
        host operating system. Ex. "ext4", "xfs", "ntfs". Implicitly inferred to be
        "ext4" if unspecified.
      * **kind** ``Optional[str]`` - *(optional)* kind expected values are Shared: multiple blob disks per storage account
        Dedicated: single blob disk per storage account  Managed: azure managed data
        disk (only in managed availability set). defaults to shared
      * **readOnly** ``Optional[bool]`` - *(optional)* readOnly Defaults to false (read/write). ReadOnly here will force the ReadOnly
...
class AzureFilePersistentVolumeSource(DictMixin):
    """AzureFile represents an Azure File Service mount on the host and bind mount to
      ...
      * **readOnly** ``Optional[bool]`` - *(optional)* readOnly defaults to false (read/write). ReadOnly here will force the ReadOnly
        setting in VolumeMounts.
      * **secretNamespace** ``Optional[str]`` - *(optional)* secretNamespace is the namespace of the secret that contains Azure Storage
...
class Binding(DictMixin):
    """Binding ties one object to another; for example, a pod is bound to a node by a
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object's metadata. More info:
...
class CSIPersistentVolumeSource(DictMixin):
    """Represents storage that is managed by an external CSI volume driver (Beta
      ...
      * **controllerExpandSecretRef** ``Optional[SecretReference]`` - *(optional)* controllerExpandSecretRef is a reference to the secret object containing
        sensitive information to pass to the CSI driver to complete the CSI
        ControllerExpandVolume call. This field is optional, and may be empty if no
        secret is required. If the secret object contains more than one secret, all
        secrets are passed.
      * **controllerPublishSecretRef** ``Optional[SecretReference]`` - *(optional)* controllerPublishSecretRef is a reference to the secret object containing
        sensitive information to pass to the CSI driver to complete the CSI
        ControllerPublishVolume and ControllerUnpublishVolume calls. This field is
        optional, and may be empty if no secret is required. If the secret object
        contains more than one secret, all secrets are passed.
      * **fsType** ``Optional[str]`` - *(optional)* fsType to mount. Must be a filesystem type supported by the host operating
        system. Ex. "ext4", "xfs", "ntfs".
      * **nodeExpandSecretRef** ``Optional[SecretReference]`` - *(optional)* nodeExpandSecretRef is a reference to the secret object containing sensitive
      ...
      * **nodePublishSecretRef** ``Optional[SecretReference]`` - *(optional)* nodePublishSecretRef is a reference to the secret object containing sensitive
        ...
        secret is required. If the secret object contains more than one secret, all
        secrets are passed.
      * **nodeStageSecretRef** ``Optional[SecretReference]`` - *(optional)* nodeStageSecretRef is a reference to the secret object containing sensitive
      ...
      * **readOnly** ``Optional[bool]`` - *(optional)* readOnly value to pass to ControllerPublishVolumeRequest. Defaults to false
        (read/write).
      * **volumeAttributes** ``Optional[dict]`` - *(optional)* volumeAttributes of the volume to publish.
    """
    driver: 'str'
    volumeHandle: 'str'
...
class CSIVolumeSource(DictMixin):
    """Represents a source location of a volume to mount, managed by an external CSI
      ...
      * **fsType** ``Optional[str]`` - *(optional)* fsType to mount. Ex. "ext4", "xfs", "ntfs". If not provided, the empty value
        is passed to the associated CSI driver which will determine the default
        filesystem to apply.
      * **nodePublishSecretRef** ``Optional[LocalObjectReference]`` - *(optional)* nodePublishSecretRef is a reference to the secret object containing sensitive
        information to pass to the CSI driver to complete the CSI NodePublishVolume
        and NodeUnpublishVolume calls. This field is optional, and  may be empty if no
        secret is required. If the secret object contains more than one secret, all
        secret references are passed.
      * **readOnly** ``Optional[bool]`` - *(optional)* readOnly specifies a read-only configuration for the volume. Defaults to false
        (read/write).
      * **volumeAttributes** ``Optional[dict]`` - *(optional)* volumeAttributes stores driver-specific properties that are passed to the CSI
...
class Capabilities(DictMixin):
    """Adds and removes POSIX capabilities from running containers.
      ...
      * **add** ``Optional[List[str]]`` - *(optional)* Added capabilities
      * **drop** ``Optional[List[str]]`` - *(optional)* Removed capabilities
    """
...
class CephFSPersistentVolumeSource(DictMixin):
    """Represents a Ceph Filesystem mount that lasts the lifetime of a pod Cephfs
      ...
      * **path** ``Optional[str]`` - *(optional)* path is Optional: Used as the mounted root, rather than the full Ceph tree,
        default is /
      * **readOnly** ``Optional[bool]`` - *(optional)* readOnly is Optional: Defaults to false (read/write). ReadOnly here will force
        the ReadOnly setting in VolumeMounts. More info:
        https://examples.k8s.io/volumes/cephfs/README.md#how-to-use-it
      * **secretFile** ``Optional[str]`` - *(optional)* secretFile is Optional: SecretFile is the path to key ring for User, default
        is /etc/ceph/user.secret More info:
        https://examples.k8s.io/volumes/cephfs/README.md#how-to-use-it
      * **secretRef** ``Optional[SecretReference]`` - *(optional)* secretRef is Optional: SecretRef is reference to the authentication secret for
        User, default is empty. More info:
        https://examples.k8s.io/volumes/cephfs/README.md#how-to-use-it
      * **user** ``Optional[str]`` - *(optional)* user is Optional: User is the rados user name, default is admin More info:
        https://examples.k8s.io/volumes/cephfs/README.md#how-to-use-it
    """
    monitors: 'List[str]'
...
class CephFSVolumeSource(DictMixin):
    """Represents a Ceph Filesystem mount that lasts the lifetime of a pod Cephfs
      ...
      * **secretFile** ``Optional[str]`` - *(optional)* secretFile is Optional: SecretFile is the path to key ring for User, default
        is /etc/ceph/user.secret More info:
        https://examples.k8s.io/volumes/cephfs/README.md#how-to-use-it
      * **secretRef** ``Optional[LocalObjectReference]`` - *(optional)* secretRef is Optional: SecretRef is reference to the authentication secret for
        User, default is empty. More info:
        https://examples.k8s.io/volumes/cephfs/README.md#how-to-use-it
      * **user** ``Optional[str]`` - *(optional)* user is optional: User is the rados user name, default is admin More info:
        https://examples.k8s.io/volumes/cephfs/README.md#how-to-use-it
...
class CinderPersistentVolumeSource(DictMixin):
    """Represents a cinder volume resource in Openstack. A Cinder volume must exist
      ...
      * **fsType** ``Optional[str]`` - *(optional)* fsType Filesystem type to mount. Must be a filesystem type supported by the
      ...
      * **readOnly** ``Optional[bool]`` - *(optional)* readOnly is Optional: Defaults to false (read/write). ReadOnly here will force
        the ReadOnly setting in VolumeMounts. More info:
        https://examples.k8s.io/mysql-cinder-pd/README.md
      * **secretRef** ``Optional[SecretReference]`` - *(optional)* secretRef is Optional: points to a secret object containing parameters used to
...
class CinderVolumeSource(DictMixin):
    """Represents a cinder volume resource in Openstack. A Cinder volume must exist
      ...
      * **volumeID** ``str`` - volumeID used to identify the volume in cinder. More info:
        https://examples.k8s.io/mysql-cinder-pd/README.md
      * **fsType** ``Optional[str]`` - *(optional)* fsType is the filesystem type to mount. Must be a filesystem type supported by
      ...
      * **readOnly** ``Optional[bool]`` - *(optional)* readOnly defaults to false (read/write). ReadOnly here will force the ReadOnly
        ...
        https://examples.k8s.io/mysql-cinder-pd/README.md
      * **secretRef** ``Optional[LocalObjectReference]`` - *(optional)* secretRef is optional: points to a secret object containing parameters used to
        connect to OpenStack.
    """
    volumeID: 'str'
    fsType: 'Optional[str]' = None
...
class ClaimSource(DictMixin):
    """ClaimSource describes a reference to a ResourceClaim.
      ...
      * **resourceClaimName** ``Optional[str]`` - *(optional)* ResourceClaimName is the name of a ResourceClaim object in the same namespace
        as this pod.
      * **resourceClaimTemplateName** ``Optional[str]`` - *(optional)* ResourceClaimTemplateName is the name of a ResourceClaimTemplate object in the
        ...
        The name of the ResourceClaim will be <pod name>-<resource name>, where
        <resource name> is the PodResourceClaim.Name. Pod validation will reject the
        pod if the concatenated name is not valid for a ResourceClaim (e.g. too long).
        An existing ResourceClaim with that name that is not owned by the pod will not
        be used for the pod to avoid using an unrelated resource by mistake.
        Scheduling and pod startup are then blocked until the unrelated ResourceClaim
        is removed.
...
class ClientIPConfig(DictMixin):
    """ClientIPConfig represents the configurations of Client IP based session
      ...
      * **timeoutSeconds** ``Optional[int]`` - *(optional)* timeoutSeconds specifies the seconds of ClientIP type session sticky time. The
...
class ComponentCondition(DictMixin):
    """Information about the condition of a component.
      ...
      * **error** ``Optional[str]`` - *(optional)* Condition error code for a component. For example, a health check error code.
      * **message** ``Optional[str]`` - *(optional)* Message about the condition for a component. For example, information about a
        health check.
    """
    status: 'str'
    type: 'str'
...
class ComponentStatus(DictMixin):
    """ComponentStatus (and ComponentStatusList) holds the cluster validation info.
      ...
      * **apiVersion** ``Optional[str]`` - *(optional)* APIVersion defines the versioned schema of this representation of an object.
        ...
        may reject unrecognized values. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#resources
      * **conditions** ``Optional[List[ComponentCondition]]`` - *(optional)* List of component conditions observed
      * **kind** ``Optional[str]`` - *(optional)* Kind is a string value representing the REST resource this object represents.
        Servers may infer this from the endpoint the client submits requests to.
        Cannot be updated. In CamelCase. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds
...
class ConfigMap(DictMixin):
    """ConfigMap holds configuration data for pods to consume.
      ...
      * **apiVersion** ``Optional[str]`` - *(optional)* APIVersion defines the versioned schema of this representation of an object.
        ...
        may reject unrecognized values. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#resources
      * **binaryData** ``Optional[dict]`` - *(optional)* BinaryData contains the binary data. Each key must consist of alphanumeric
      ...
      * **data** ``Optional[dict]`` - *(optional)* Data contains the configuration data. Each key must consist of alphanumeric
      ...
      * **immutable** ``Optional[bool]`` - *(optional)* Immutable, if set to true, ensures that data stored in the ConfigMap cannot be
...
class ConfigMapEnvSource(DictMixin):
    """ConfigMapEnvSource selects a ConfigMap to populate the environment variables
      ...
      * **name** ``Optional[str]`` - *(optional)* Name of the referent. More info:
        https://kubernetes.io/docs/concepts/overview/working-with-objects/names/#names
      * **optional** ``Optional[bool]`` - *(optional)* Specify whether the ConfigMap must be defined
    """
...
class ConfigMapKeySelector(DictMixin):
    """Selects a key from a ConfigMap.
      ...
      * **name** ``Optional[str]`` - *(optional)* Name of the referent. More info:
        https://kubernetes.io/docs/concepts/overview/working-with-objects/names/#names
      * **optional** ``Optional[bool]`` - *(optional)* Specify whether the ConfigMap or its key must be defined
    """
    key: 'str'
    name: 'Optional[str]' = None
    optional: 'Optional[bool]' = None
...
class ConfigMapList(DictMixin):
    """ConfigMapList is a resource containing a list of ConfigMap objects.
      ...
      * **kind** ``Optional[str]`` - *(optional)* Kind is a string value representing the REST resource this object represents.
        ...
        Cannot be updated. In CamelCase. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds
      * **metadata** ``Optional[meta_v1.ListMeta]`` - *(optional)* More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
    """
...
class ConfigMapNodeConfigSource(DictMixin):
    """ConfigMapNodeConfigSource contains the information to reference a ConfigMap as
      ...
      * **resourceVersion** ``Optional[str]`` - *(optional)* ResourceVersion is the metadata.ResourceVersion of the referenced ConfigMap.
        This field is forbidden in Node.Spec, and required in Node.Status.
      * **uid** ``Optional[str]`` - *(optional)* UID is the metadata.UID of the referenced ConfigMap. This field is forbidden
...
class ConfigMapProjection(DictMixin):
    """Adapts a ConfigMap into a projected volume.
      ...
      * **items** ``Optional[List[KeyToPath]]`` - *(optional)* items if unspecified, each key-value pair in the Data field of the referenced
      ...
      * **name** ``Optional[str]`` - *(optional)* Name of the referent. More info:
        https://kubernetes.io/docs/concepts/overview/working-with-objects/names/#names
      * **optional** ``Optional[bool]`` - *(optional)* optional specify whether the ConfigMap or its keys must be defined
    """
...
class ConfigMapVolumeSource(DictMixin):
    """Adapts a ConfigMap into a volume.
      ...
      * **defaultMode** ``Optional[int]`` - *(optional)* defaultMode is optional: mode bits used to set permissions on created files by
...
class Container(DictMixin):
    """A single application container that you want to run within a pod.
      ...
      * **args** ``Optional[List[str]]`` - *(optional)* Arguments to the entrypoint. The container image's CMD is used if this is not
      ...
      * **command** ``Optional[List[str]]`` - *(optional)* Entrypoint array. Not executed within a shell. The container image's
        ...
        https://kubernetes.io/docs/tasks/inject-data-application/define-command-argument-container/#running-a-command-in-a-shell
      * **env** ``Optional[List[EnvVar]]`` - *(optional)* List of environment variables to set in the container. Cannot be updated.
      * **envFrom** ``Optional[List[EnvFromSource]]`` - *(optional)* List of sources to populate environment variables in the container. The keys
      ...
      * **image** ``Optional[str]`` - *(optional)* Container image name. More info:
      ...
      * **imagePullPolicy** ``Optional[str]`` - *(optional)* Image pull policy. One of Always, Never, IfNotPresent. Defaults to Always if
        :latest tag is specified, or IfNotPresent otherwise. Cannot be updated. More
        info: https://kubernetes.io/docs/concepts/containers/images#updating-images
      * **lifecycle** ``Optional[Lifecycle]`` - *(optional)* Actions that the management system should take in response to container
        lifecycle events. Cannot be updated.
      * **livenessProbe** ``Optional[Probe]`` - *(optional)* Periodic probe of container liveness. Container will be restarted if the probe
        fails. Cannot be updated. More info:
        https://kubernetes.io/docs/concepts/workloads/pods/pod-lifecycle#container-probes
      * **ports** ``Optional[List[ContainerPort]]`` - *(optional)* List of ports to expose from the container. Not specifying a port here DOES
      ...
      * **readinessProbe** ``Optional[Probe]`` - *(optional)* Periodic probe of container service readiness. Container will be removed from
        service endpoints if the probe fails. Cannot be updated. More info:
        https://kubernetes.io/docs/concepts/workloads/pods/pod-lifecycle#container-probes
      * **resizePolicy** ``Optional[List[ContainerResizePolicy]]`` - *(optional)* Resources resize policy for the container.
      * **resources** ``Optional[ResourceRequirements]`` - *(optional)* Compute Resources required by this container. Cannot be updated. More info:
        https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/
      * **securityContext** ``Optional[SecurityContext]`` - *(optional)* SecurityContext defines the security options the container should be run with.
        If set, the fields of SecurityContext override the equivalent fields of
        PodSecurityContext. More info:
        https://kubernetes.io/docs/tasks/configure-pod-container/security-context/
      * **startupProbe** ``Optional[Probe]`` - *(optional)* StartupProbe indicates that the Pod has successfully initialized. If
        ...
        info:
        https://kubernetes.io/docs/concepts/workloads/pods/pod-lifecycle#container-probes
      * **stdin** ``Optional[bool]`` - *(optional)* Whether this container should allocate a buffer for stdin in the container
        runtime. If this is not set, reads from stdin in the container will always
        result in EOF. Default is false.
      * **stdinOnce** ``Optional[bool]`` - *(optional)* Whether the container runtime should close the stdin channel after it has been
      ...
      * **terminationMessagePath** ``Optional[str]`` - *(optional)* Optional: Path at which the file to which the container's termination message
      ...
      * **terminationMessagePolicy** ``Optional[str]`` - *(optional)* Indicate how the termination message should be populated. File will use the
      ...
      * **tty** ``Optional[bool]`` - *(optional)* Whether this container should allocate a TTY for itself, also requires 'stdin'
        to be true. Default is false.
      * **volumeDevices** ``Optional[List[VolumeDevice]]`` - *(optional)* volumeDevices is the list of block devices to be used by the container.
      * **volumeMounts** ``Optional[List[VolumeMount]]`` - *(optional)* Pod volumes to mount into the container's filesystem. Cannot be updated.
      * **workingDir** ``Optional[str]`` - *(optional)* Container's working directory. If not specified, the container runtime's
...
class ContainerImage(DictMixin):
    """Describe a container image
      ...
      * **names** ``Optional[List[str]]`` - *(optional)* Names by which this image is known. e.g.
        ["kubernetes.example/hyperkube:v1.0.7",
        "cloud-vendor.registry.example/cloud-vendor/hyperkube:v1.0.7"]
      * **sizeBytes** ``Optional[int]`` - *(optional)* The size of the image in bytes.
    """
...
class ContainerPort(DictMixin):
    """ContainerPort represents a network port in a single container.
      ...
      * **containerPort** ``int`` - Number of port to expose on the pod's IP address. This must be a valid port
        number, 0 < x < 65536.
      * **hostIP** ``Optional[str]`` - *(optional)* What host IP to bind the external port to.
      * **hostPort** ``Optional[int]`` - *(optional)* Number of port to expose on the host. If specified, this must be a valid port
        number, 0 < x < 65536. If HostNetwork is specified, this must match
        ContainerPort. Most containers do not need this.
      * **name** ``Optional[str]`` - *(optional)* If specified, this must be an IANA_SVC_NAME and unique within the pod. Each
        named port in a pod must have a unique name. Name for the port that can be
        referred to by services.
      * **protocol** ``Optional[str]`` - *(optional)* Protocol for port. Must be UDP, TCP, or SCTP. Defaults to "TCP".
    """
    containerPort: 'int'
...
class ContainerState(DictMixin):
    """ContainerState holds a possible state of container. Only one of its members
      ...
      * **running** ``Optional[ContainerStateRunning]`` - *(optional)* Details about a running container
      * **terminated** ``Optional[ContainerStateTerminated]`` - *(optional)* Details about a terminated container
      * **waiting** ``Optional[ContainerStateWaiting]`` - *(optional)* Details about a waiting container
    """
...
class ContainerStateRunning(DictMixin):
    """ContainerStateRunning is a running state of a container.
      ...
      * **startedAt** ``Optional[meta_v1.Time]`` - *(optional)* Time at which the container was last (re-)started
    """
...
class ContainerStateTerminated(DictMixin):
    """ContainerStateTerminated is a terminated state of a container.
      ...
      * **containerID** ``Optional[str]`` - *(optional)* Container's ID in the format '<type>://<container_id>'
      * **finishedAt** ``Optional[meta_v1.Time]`` - *(optional)* Time at which the container last terminated
      * **message** ``Optional[str]`` - *(optional)* Message regarding the last termination of the container
      * **reason** ``Optional[str]`` - *(optional)* (brief) reason from the last termination of the container
      * **signal** ``Optional[int]`` - *(optional)* Signal from the last termination of the container
      * **startedAt** ``Optional[meta_v1.Time]`` - *(optional)* Time at which previous execution of the container started
    """
    exitCode: 'int'
...
class ContainerStateWaiting(DictMixin):
    """ContainerStateWaiting is a waiting state of a container.
      ...
      * **message** ``Optional[str]`` - *(optional)* Message regarding why the container is not yet running.
      * **reason** ``Optional[str]`` - *(optional)* (brief) reason the container is not yet running.
    """
    message: 'Optional[str]' = None
    reason: 'Optional[str]' = None
...
class ContainerStatus(DictMixin):
    """ContainerStatus contains details for the current status of this container.
      ...
      * **allocatedResources** ``Optional[dict]`` - *(optional)* AllocatedResources represents the compute resources allocated for this
        ...
        resize.
      * **containerID** ``Optional[str]`` - *(optional)* ContainerID is the ID of the container in the format
      ...
      * **lastState** ``Optional[ContainerState]`` - *(optional)* LastTerminationState holds the last termination state of the container to help
        debug container crashes and restarts. This field is not populated if the
        container is still running and RestartCount is 0.
      * **resources** ``Optional[ResourceRequirements]`` - *(optional)* Resources represents the compute resource requests and limits that have been
        successfully enacted on the running container after it has been started or has
        been successfully resized.
      * **started** ``Optional[bool]`` - *(optional)* Started indicates whether the container has finished its postStart lifecycle
      ...
      * **state** ``Optional[ContainerState]`` - *(optional)* State holds details about the container's current condition.
    """
    image: 'str'
    imageID: 'str'
    name: 'str'
...
class DownwardAPIProjection(DictMixin):
    """Represents downward API info for projecting into a projected volume. Note that
      ...
      * **items** ``Optional[List[DownwardAPIVolumeFile]]`` - *(optional)* Items is a list of DownwardAPIVolume file
    """
...
class DownwardAPIVolumeFile(DictMixin):
    """DownwardAPIVolumeFile represents information to create the file containing the
      ...
      * **fieldRef** ``Optional[ObjectFieldSelector]`` - *(optional)* Required: Selects a field of the pod: only annotations, labels, name and
        namespace are supported.
      * **mode** ``Optional[int]`` - *(optional)* Optional: mode bits used to set permissions on this file, must be an octal
      ...
      * **resourceFieldRef** ``Optional[ResourceFieldSelector]`` - *(optional)* Selects a resource of the container: only resources limits and requests
...
class DownwardAPIVolumeSource(DictMixin):
    """DownwardAPIVolumeSource represents a volume containing downward API info.
      ...
      * **defaultMode** ``Optional[int]`` - *(optional)* Optional: mode bits to use on created files by default. Must be a Optional:
      ...
      * **items** ``Optional[List[DownwardAPIVolumeFile]]`` - *(optional)* Items is a list of downward API volume file
    """
    defaultMode: 'Optional[int]' = None
    items: 'Optional[List[DownwardAPIVolumeFile]]' = None
...
class EmptyDirVolumeSource(DictMixin):
    """Represents an empty directory for a pod. Empty directory volumes support
      ...
      * **medium** ``Optional[str]`` - *(optional)* medium represents what type of storage medium should back this directory. The
        default is "" which means to use the node's default medium. Must be an empty
        string (default) or Memory. More info:
        https://kubernetes.io/docs/concepts/storage/volumes#emptydir
      * **sizeLimit** ``Optional[resource.Quantity]`` - *(optional)* sizeLimit is the total amount of local storage required for this EmptyDir
...
class EndpointAddress(DictMixin):
    """EndpointAddress is a tuple that describes single IP address.
      ...
      * **ip** ``str`` - The IP of this endpoint. May not be loopback (127.0.0.0/8 or ::1), link-local
        ...
        ff02::/16).
      * **hostname** ``Optional[str]`` - *(optional)* The Hostname of this endpoint
      * **nodeName** ``Optional[str]`` - *(optional)* Optional: Node hosting this endpoint. This can be used to determine endpoints
        local to a node.
      * **targetRef** ``Optional[ObjectReference]`` - *(optional)* Reference to object providing the endpoint.
    """
    ip: 'str'
...
class EndpointPort(DictMixin):
    """EndpointPort is a tuple that describes a single port.
      ...
      * **appProtocol** ``Optional[str]`` - *(optional)* The application protocol for this port. This is used as a hint for
      ...
      * **name** ``Optional[str]`` - *(optional)* The name of this port.  This must match the 'name' field in the corresponding
        ServicePort. Must be a DNS_LABEL. Optional only if one port is defined.
      * **protocol** ``Optional[str]`` - *(optional)* The IP protocol for this port. Must be UDP, TCP, or SCTP. Default is TCP.
    """
    port: 'int'
...
class EndpointSubset(DictMixin):
    """EndpointSubset is a group of addresses with a common set of ports. The
      ...
      * **addresses** ``Optional[List[EndpointAddress]]`` - *(optional)* IP addresses which offer the related ports that are marked as ready. These
        endpoints should be considered safe for load balancers and clients to utilize.
      * **notReadyAddresses** ``Optional[List[EndpointAddress]]`` - *(optional)* IP addresses which offer the related ports but are not currently marked as
      ...
      * **ports** ``Optional[List[EndpointPort]]`` - *(optional)* Port numbers available on the related IP addresses.
    """
...
class Endpoints(DictMixin):
    """Endpoints is a collection of endpoints that implement the actual service.
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **subsets** ``Optional[List[EndpointSubset]]`` - *(optional)* The set of all endpoints is the union of all subsets. Addresses are placed
...
class EnvFromSource(DictMixin):
    """EnvFromSource represents the source of a set of ConfigMaps
      ...
      * **configMapRef** ``Optional[ConfigMapEnvSource]`` - *(optional)* The ConfigMap to select from
      * **prefix** ``Optional[str]`` - *(optional)* An optional identifier to prepend to each key in the ConfigMap. Must be a
        C_IDENTIFIER.
      * **secretRef** ``Optional[SecretEnvSource]`` - *(optional)* The Secret to select from
    """
...
class EnvVar(DictMixin):
    """EnvVar represents an environment variable present in a Container.
      ...
      * **value** ``Optional[str]`` - *(optional)* Variable references $(VAR_NAME) are expanded using the previously defined
      ...
      * **valueFrom** ``Optional[EnvVarSource]`` - *(optional)* Source for the environment variable's value. Cannot be used if value is not
        empty.
    """
    name: 'str'
...
class EnvVarSource(DictMixin):
    """EnvVarSource represents a source for the value of an EnvVar.
      ...
      * **configMapKeyRef** ``Optional[ConfigMapKeySelector]`` - *(optional)* Selects a key of a ConfigMap.
      * **fieldRef** ``Optional[ObjectFieldSelector]`` - *(optional)* Selects a field of the pod: supports metadata.name, metadata.namespace,
        `metadata.labels['<KEY>']`, `metadata.annotations['<KEY>']`, spec.nodeName,
        spec.serviceAccountName, status.hostIP, status.podIP, status.podIPs.
      * **resourceFieldRef** ``Optional[ResourceFieldSelector]`` - *(optional)* Selects a resource of the container: only resources limits and requests
        (limits.cpu, limits.memory, limits.ephemeral-storage, requests.cpu,
        requests.memory and requests.ephemeral-storage) are currently supported.
      * **secretKeyRef** ``Optional[SecretKeySelector]`` - *(optional)* Selects a key of a secret in the pod's namespace
    """
...
class EphemeralContainer(DictMixin):
    """An EphemeralContainer is a temporary container that you may add to an existing
      ...
      * **args** ``Optional[List[str]]`` - *(optional)* Arguments to the entrypoint. The image's CMD is used if this is not provided.
        ...
        info:
        https://kubernetes.io/docs/tasks/inject-data-application/define-command-argument-container/#running-a-command-in-a-shell
      * **command** ``Optional[List[str]]`` - *(optional)* Entrypoint array. Not executed within a shell. The image's ENTRYPOINT is used
      ...
      * **imagePullPolicy** ``Optional[str]`` - *(optional)* Image pull policy. One of Always, Never, IfNotPresent. Defaults to Always if
        :latest tag is specified, or IfNotPresent otherwise. Cannot be updated. More
        info: https://kubernetes.io/docs/concepts/containers/images#updating-images
      * **lifecycle** ``Optional[Lifecycle]`` - *(optional)* Lifecycle is not allowed for ephemeral containers.
      * **livenessProbe** ``Optional[Probe]`` - *(optional)* Probes are not allowed for ephemeral containers.
      * **ports** ``Optional[List[ContainerPort]]`` - *(optional)* Ports are not allowed for ephemeral containers.
      * **readinessProbe** ``Optional[Probe]`` - *(optional)* Probes are not allowed for ephemeral containers.
      * **resizePolicy** ``Optional[List[ContainerResizePolicy]]`` - *(optional)* Resources resize policy for the container.
      * **resources** ``Optional[ResourceRequirements]`` - *(optional)* Resources are not allowed for ephemeral containers. Ephemeral containers use
        spare resources already allocated to the pod.
      * **securityContext** ``Optional[SecurityContext]`` - *(optional)* Optional: SecurityContext defines the security options the ephemeral container
        should be run with. If set, the fields of SecurityContext override the
        equivalent fields of PodSecurityContext.
      * **startupProbe** ``Optional[Probe]`` - *(optional)* Probes are not allowed for ephemeral containers.
      * **stdin** ``Optional[bool]`` - *(optional)* Whether this container should allocate a buffer for stdin in the container
        runtime. If this is not set, reads from stdin in the container will always
        result in EOF. Default is false.
      * **stdinOnce** ``Optional[bool]`` - *(optional)* Whether the container runtime should close the stdin channel after it has been
        ...
        this flag is false, a container processes that reads from stdin will never
        receive an EOF. Default is false
      * **targetContainerName** ``Optional[str]`` - *(optional)* If set, the name of the container from PodSpec that this ephemeral container
      ...
      * **tty** ``Optional[bool]`` - *(optional)* Whether this container should allocate a TTY for itself, also requires 'stdin'
        to be true. Default is false.
      * **volumeDevices** ``Optional[List[VolumeDevice]]`` - *(optional)* volumeDevices is the list of block devices to be used by the container.
      * **volumeMounts** ``Optional[List[VolumeMount]]`` - *(optional)* Pod volumes to mount into the container's filesystem. Subpath mounts are not
...
class EphemeralVolumeSource(DictMixin):
    """Represents an ephemeral volume that is handled by a normal storage driver.
      ...
      * **volumeClaimTemplate** ``Optional[PersistentVolumeClaimTemplate]`` - *(optional)* Will be used to create a stand-alone PVC to provision the volume. The pod in
...
class Event(DictMixin):
    """Event is a report of an event somewhere in the cluster.  Events have a limited
      ...
      * **metadata** ``meta_v1.ObjectMeta`` - Standard object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **action** ``Optional[str]`` - *(optional)* What action was taken/failed regarding to the Regarding object.
      * **apiVersion** ``Optional[str]`` - *(optional)* APIVersion defines the versioned schema of this representation of an object.
        Servers should convert recognized schemas to the latest internal value, and
        may reject unrecognized values. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#resources
      * **count** ``Optional[int]`` - *(optional)* The number of times this event has occurred.
      * **eventTime** ``Optional[meta_v1.MicroTime]`` - *(optional)* Time when this Event was first observed.
      * **firstTimestamp** ``Optional[meta_v1.Time]`` - *(optional)* The time at which the event was first recorded. (Time of server receipt is in
        TypeMeta.)
      * **kind** ``Optional[str]`` - *(optional)* Kind is a string value representing the REST resource this object represents.
        Servers may infer this from the endpoint the client submits requests to.
        Cannot be updated. In CamelCase. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds
      * **lastTimestamp** ``Optional[meta_v1.Time]`` - *(optional)* The time at which the most recent occurrence of this event was recorded.
      * **message** ``Optional[str]`` - *(optional)* A human-readable description of the status of this operation.
      * **reason** ``Optional[str]`` - *(optional)* This should be a short, machine understandable string that gives the reason
        for the transition into the object's current status.
      * **related** ``Optional[ObjectReference]`` - *(optional)* Optional secondary object for more complex actions.
      * **reportingComponent** ``Optional[str]`` - *(optional)* Name of the controller that emitted this Event, e.g. `kubernetes.io/kubelet`.
      * **reportingInstance** ``Optional[str]`` - *(optional)* ID of the controller instance, e.g. `kubelet-xyzf`.
      * **series** ``Optional[EventSeries]`` - *(optional)* Data about the Event series this event represents or nil if it's a singleton
        Event.
      * **source** ``Optional[EventSource]`` - *(optional)* The component reporting this event. Should be a short machine understandable
        string.
      * **type** ``Optional[str]`` - *(optional)* Type of this event (Normal, Warning), new types could be added in the future
    """
...
class EventSeries(DictMixin):
    """EventSeries contain information on series of events, i.e. thing that was/is
      ...
      * **count** ``Optional[int]`` - *(optional)* Number of occurrences in this series up to the last heartbeat time
      * **lastObservedTime** ``Optional[meta_v1.MicroTime]`` - *(optional)* Time of the last occurrence observed
    """
    count: 'Optional[int]' = None
...
class EventSource(DictMixin):
    """EventSource contains information for an event.
      ...
      * **component** ``Optional[str]`` - *(optional)* Component from which the event is generated.
      * **host** ``Optional[str]`` - *(optional)* Node name on which the event is generated.
    """
...
class ExecAction(DictMixin):
    """ExecAction describes a "run in container" action.
      ...
      * **command** ``Optional[List[str]]`` - *(optional)* Command is the command line to execute inside the container, the working
...
class FCVolumeSource(DictMixin):
    """Represents a Fibre Channel volume. Fibre Channel volumes can only be mounted
      ...
      * **lun** ``Optional[int]`` - *(optional)* lun is Optional: FC target lun number
      * **readOnly** ``Optional[bool]`` - *(optional)* readOnly is Optional: Defaults to false (read/write). ReadOnly here will force
        the ReadOnly setting in VolumeMounts.
      * **targetWWNs** ``Optional[List[str]]`` - *(optional)* targetWWNs is Optional: FC target worldwide names (WWNs)
      * **wwids** ``Optional[List[str]]`` - *(optional)* wwids Optional: FC volume world wide identifiers (wwids) Either wwids or
...
class FlexPersistentVolumeSource(DictMixin):
    """FlexPersistentVolumeSource represents a generic persistent volume resource
      ...
      * **options** ``Optional[dict]`` - *(optional)* options is Optional: this field holds extra command options if any.
      * **readOnly** ``Optional[bool]`` - *(optional)* readOnly is Optional: defaults to false (read/write). ReadOnly here will force
        the ReadOnly setting in VolumeMounts.
      * **secretRef** ``Optional[SecretReference]`` - *(optional)* secretRef is Optional: SecretRef is reference to the secret object containing
...
class FlexVolumeSource(DictMixin):
    """FlexVolume represents a generic volume resource that is provisioned/attached
      ...
      * **readOnly** ``Optional[bool]`` - *(optional)* readOnly is Optional: defaults to false (read/write). ReadOnly here will force
        the ReadOnly setting in VolumeMounts.
      * **secretRef** ``Optional[LocalObjectReference]`` - *(optional)* secretRef is Optional: secretRef is reference to the secret object containing
        sensitive information to pass to the plugin scripts. This may be empty if no
        secret object is specified. If the secret object contains more than one
        secret, all secrets are passed to the plugin scripts.
    """
...
class FlockerVolumeSource(DictMixin):
    """Represents a Flocker volume mounted by the Flocker agent. One and only one of
      ...
      * **datasetName** ``Optional[str]`` - *(optional)* datasetName is Name of the dataset stored as metadata -> name on the dataset
        for Flocker should be considered as deprecated
      * **datasetUUID** ``Optional[str]`` - *(optional)* datasetUUID is the UUID of the dataset. This is unique identifier of a Flocker
        dataset
    """
...
class GCEPersistentDiskVolumeSource(DictMixin):
    """Represents a Persistent Disk resource in Google Compute Engine.
      ...
      * **fsType** ``Optional[str]`` - *(optional)* fsType is filesystem type of the volume that you want to mount. Tip: Ensure
      ...
      * **partition** ``Optional[int]`` - *(optional)* partition is the partition in the volume that you want to mount. If omitted,
        ...
        https://kubernetes.io/docs/concepts/storage/volumes#gcepersistentdisk
      * **readOnly** ``Optional[bool]`` - *(optional)* readOnly here will force the ReadOnly setting in VolumeMounts. Defaults to
        false. More info:
        https://kubernetes.io/docs/concepts/storage/volumes#gcepersistentdisk
    """
    pdName: 'str'
...
class GRPCAction(DictMixin):
    """
      ...
      * **service** ``Optional[str]`` - *(optional)* Service is the name of the service to place in the gRPC HealthCheckRequest
...
class GitRepoVolumeSource(DictMixin):
    """Represents a volume that is populated with the contents of a git repository.
      ...
      * **directory** ``Optional[str]`` - *(optional)* directory is the target directory name. Must not contain or start with '..'.
      ...
      * **revision** ``Optional[str]`` - *(optional)* revision is the commit hash for the specified revision.
    """
    repository: 'str'
...
class GlusterfsPersistentVolumeSource(DictMixin):
    """Represents a Glusterfs mount that lasts the lifetime of a pod. Glusterfs
      ...
      * **path** ``str`` - path is the Glusterfs volume path. More info:
        https://examples.k8s.io/volumes/glusterfs/README.md#create-a-pod
      * **endpointsNamespace** ``Optional[str]`` - *(optional)* endpointsNamespace is the namespace that contains Glusterfs endpoint. If this
        field is empty, the EndpointNamespace defaults to the same namespace as the
        bound PVC. More info:
        https://examples.k8s.io/volumes/glusterfs/README.md#create-a-pod
      * **readOnly** ``Optional[bool]`` - *(optional)* readOnly here will force the Glusterfs volume to be mounted with read-only
...
class HTTPGetAction(DictMixin):
    """HTTPGetAction describes an action based on HTTP Get requests.
      ...
      * **host** ``Optional[str]`` - *(optional)* Host name to connect to, defaults to the pod IP. You probably want to set
        "Host" in httpHeaders instead.
      * **httpHeaders** ``Optional[List[HTTPHeader]]`` - *(optional)* Custom headers to set in the request. HTTP allows repeated headers.
      * **path** ``Optional[str]`` - *(optional)* Path to access on the HTTP server.
      * **scheme** ``Optional[str]`` - *(optional)* Scheme to use for connecting to the host. Defaults to HTTP.
    """
...
class HostAlias(DictMixin):
    """HostAlias holds the mapping between IP and hostnames that will be injected as
      ...
      * **hostnames** ``Optional[List[str]]`` - *(optional)* Hostnames for the above IP address.
      * **ip** ``Optional[str]`` - *(optional)* IP address of the host file entry.
    """
...
class HostPathVolumeSource(DictMixin):
    """Represents a host path mapped into a pod. Host path volumes do not support
      ...
      * **type** ``Optional[str]`` - *(optional)* type for HostPath Volume Defaults to "" More info:
        https://kubernetes.io/docs/concepts/storage/volumes#hostpath
    """
    path: 'str'
    type: 'Optional[str]' = None
...
class ISCSIPersistentVolumeSource(DictMixin):
    """ISCSIPersistentVolumeSource represents an ISCSI disk. ISCSI volumes can only
      ...
      * **targetPortal** ``str`` - targetPortal is iSCSI Target Portal. The Portal is either an IP or
        ...
        3260).
      * **chapAuthDiscovery** ``Optional[bool]`` - *(optional)* chapAuthDiscovery defines whether support iSCSI Discovery CHAP authentication
      * **chapAuthSession** ``Optional[bool]`` - *(optional)* chapAuthSession defines whether support iSCSI Session CHAP authentication
      * **fsType** ``Optional[str]`` - *(optional)* fsType is the filesystem type of the volume that you want to mount. Tip:
        Ensure that the filesystem type is supported by the host operating system.
        Examples: "ext4", "xfs", "ntfs". Implicitly inferred to be "ext4" if
        unspecified. More info:
        https://kubernetes.io/docs/concepts/storage/volumes#iscsi
      * **initiatorName** ``Optional[str]`` - *(optional)* initiatorName is the custom iSCSI Initiator Name. If initiatorName is
      ...
      * **iscsiInterface** ``Optional[str]`` - *(optional)* iscsiInterface is the interface Name that uses an iSCSI transport. Defaults to
        'default' (tcp).
      * **portals** ``Optional[List[str]]`` - *(optional)* portals is the iSCSI Target Portal List. The Portal is either an IP or
        ip_addr:port if the port is other than default (typically TCP ports 860 and
        3260).
      * **readOnly** ``Optional[bool]`` - *(optional)* readOnly here will force the ReadOnly setting in VolumeMounts. Defaults to
        false.
      * **secretRef** ``Optional[SecretReference]`` - *(optional)* secretRef is the CHAP Secret for iSCSI target and initiator authentication
    """
    iqn: 'str'
    lun: 'int'
    targetPortal: 'str'
...
class ISCSIVolumeSource(DictMixin):
    """Represents an ISCSI disk. ISCSI volumes can only be mounted as read/write
      ...
      * **readOnly** ``Optional[bool]`` - *(optional)* readOnly here will force the ReadOnly setting in VolumeMounts. Defaults to
        false.
      * **secretRef** ``Optional[LocalObjectReference]`` - *(optional)* secretRef is the CHAP Secret for iSCSI target and initiator authentication
    """
    iqn: 'str'
    lun: 'int'
    targetPortal: 'str'
...
class KeyToPath(DictMixin):
    """Maps a string key to a path within a volume.
      ...
      * **path** ``str`` - path is the relative path of the file to map the key to. May not be an
        ...
        string '..'.
      * **mode** ``Optional[int]`` - *(optional)* mode is Optional: mode bits used to set permissions on this file. Must be an
        octal value between 0000 and 0777 or a decimal value between 0 and 511. YAML
        accepts both octal and decimal values, JSON requires decimal values for mode
...
class Lifecycle(DictMixin):
    """Lifecycle describes actions that the management system should take in response
      ...
      * **postStart** ``Optional[LifecycleHandler]`` - *(optional)* PostStart is called immediately after a container is created. If the handler
      ...
      * **preStop** ``Optional[LifecycleHandler]`` - *(optional)* PreStop is called immediately before a container is terminated due to an API
...
class LifecycleHandler(DictMixin):
    """LifecycleHandler defines a specific action that should be taken in a lifecycle
      ...
      * **exec** ``Optional[ExecAction]`` - *(optional)* Exec specifies the action to take.
      * **httpGet** ``Optional[HTTPGetAction]`` - *(optional)* HTTPGet specifies the http request to perform.
      * **tcpSocket** ``Optional[TCPSocketAction]`` - *(optional)* Deprecated. TCPSocket is NOT supported as a LifecycleHandler and kept for the
...
class LimitRange(DictMixin):
    """LimitRange sets resource usage limits for each kind of resource in a
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **spec** ``Optional[LimitRangeSpec]`` - *(optional)* Spec defines the limits enforced. More info:
...
class LimitRangeItem(DictMixin):
    """LimitRangeItem defines a min/max usage limit for any resource that matches on
      ...
      * **default** ``Optional[dict]`` - *(optional)* Default resource requirement limit value by resource name if resource limit is
        omitted.
      * **defaultRequest** ``Optional[dict]`` - *(optional)* DefaultRequest is the default resource requirement request value by resource
        name if resource request is omitted.
      * **max** ``Optional[dict]`` - *(optional)* Max usage constraints on this kind by resource name.
      * **maxLimitRequestRatio** ``Optional[dict]`` - *(optional)* MaxLimitRequestRatio if specified, the named resource must have a request and
        ...
        resource.
      * **min** ``Optional[dict]`` - *(optional)* Min usage constraints on this kind by resource name.
    """
    type: 'str'
...
class LoadBalancerIngress(DictMixin):
    """LoadBalancerIngress represents the status of a load-balancer ingress point:
      ...
      * **hostname** ``Optional[str]`` - *(optional)* Hostname is set for load-balancer ingress points that are DNS based (typically
        AWS load-balancers)
      * **ip** ``Optional[str]`` - *(optional)* IP is set for load-balancer ingress points that are IP based (typically GCE or
        OpenStack load-balancers)
      * **ports** ``Optional[List[PortStatus]]`` - *(optional)* Ports is a list of records of service ports If used, every port defined in the
...
class LoadBalancerStatus(DictMixin):
    """LoadBalancerStatus represents the status of a load-balancer.
      ...
      * **ingress** ``Optional[List[LoadBalancerIngress]]`` - *(optional)* Ingress is a list containing ingress points for the load-balancer. Traffic
...
class LocalVolumeSource(DictMixin):
    """Local represents directly-attached storage with node affinity (Beta feature)
      ...
      * **fsType** ``Optional[str]`` - *(optional)* fsType is the filesystem type to mount. It applies only when the Path is a
...
class NFSVolumeSource(DictMixin):
    """Represents an NFS mount that lasts the lifetime of a pod. NFS volumes do not
      ...
      * **server** ``str`` - server is the hostname or IP address of the NFS server. More info:
        https://kubernetes.io/docs/concepts/storage/volumes#nfs
      * **readOnly** ``Optional[bool]`` - *(optional)* readOnly here will force the NFS export to be mounted with read-only
        permissions. Defaults to false. More info:
        https://kubernetes.io/docs/concepts/storage/volumes#nfs
    """
    path: 'str'
...
class Namespace(DictMixin):
    """Namespace provides a scope for Names. Use of multiple namespaces is optional.
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **spec** ``Optional[NamespaceSpec]`` - *(optional)* Spec defines the behavior of the Namespace. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
      * **status** ``Optional[NamespaceStatus]`` - *(optional)* Status describes the current status of a Namespace. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
    """
    apiVersion: 'Optional[str]' = None
    kind: 'Optional[str]' = None
...
class NamespaceCondition(DictMixin):
    """NamespaceCondition contains details about state of namespace.
      ...
      * **lastTransitionTime** ``Optional[meta_v1.Time]`` - *(optional)* 
...
class NamespaceSpec(DictMixin):
    """NamespaceSpec describes the attributes on a Namespace.
      ...
      * **finalizers** ``Optional[List[str]]`` - *(optional)* Finalizers is an opaque list of values that must be empty to permanently
...
class NamespaceStatus(DictMixin):
    """NamespaceStatus is information about the current status of a Namespace.
      ...
      * **conditions** ``Optional[List[NamespaceCondition]]`` - *(optional)* Represents the latest available observations of a namespace's current state.
      * **phase** ``Optional[str]`` - *(optional)* Phase is the current lifecycle phase of the namespace. More info:
        https://kubernetes.io/docs/tasks/administer-cluster/namespaces/
    """
...
class Node(DictMixin):
    """Node is a worker node in Kubernetes. Each node will have a unique identifier
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **spec** ``Optional[NodeSpec]`` - *(optional)* Spec defines the behavior of a node.
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
      * **status** ``Optional[NodeStatus]`` - *(optional)* Most recently observed status of the node. Populated by the system. Read-only.
        More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
    """
    apiVersion: 'Optional[str]' = None
...
class NodeAffinity(DictMixin):
    """Node affinity is a group of node affinity scheduling rules.
      ...
      * **preferredDuringSchedulingIgnoredDuringExecution** ``Optional[List[PreferredSchedulingTerm]]`` - *(optional)* The scheduler will prefer to schedule pods to nodes that satisfy the affinity
      ...
      * **requiredDuringSchedulingIgnoredDuringExecution** ``Optional[NodeSelector]`` - *(optional)* If the affinity requirements specified by this field are not met at scheduling
...
class NodeCondition(DictMixin):
    """NodeCondition contains condition information for a node.
      ...
      * **lastHeartbeatTime** ``Optional[meta_v1.Time]`` - *(optional)* Last time we got an update on a given condition.
      * **lastTransitionTime** ``Optional[meta_v1.Time]`` - *(optional)* Last time the condition transit from one status to another.
      * **message** ``Optional[str]`` - *(optional)* Human readable message indicating details about last transition.
      * **reason** ``Optional[str]`` - *(optional)* (brief) reason for the condition's last transition.
    """
    status: 'str'
    type: 'str'
...
class NodeConfigSource(DictMixin):
    """NodeConfigSource specifies a source of node configuration. Exactly one
      ...
      * **configMap** ``Optional[ConfigMapNodeConfigSource]`` - *(optional)* ConfigMap is a reference to a Node's ConfigMap
    """
...
class NodeConfigStatus(DictMixin):
    """NodeConfigStatus describes the status of the config assigned by
      ...
      * **active** ``Optional[NodeConfigSource]`` - *(optional)* Active reports the checkpointed config the node is actively using. Active will
      ...
      * **assigned** ``Optional[NodeConfigSource]`` - *(optional)* Assigned reports the checkpointed config the node will try to use. When
      ...
      * **error** ``Optional[str]`` - *(optional)* Error describes any problems reconciling the Spec.ConfigSource to the Active
      ...
      * **lastKnownGood** ``Optional[NodeConfigSource]`` - *(optional)* LastKnownGood reports the checkpointed config the node will fall back to when
...
class NodeDaemonEndpoints(DictMixin):
    """NodeDaemonEndpoints lists ports opened by daemons running on the Node.
      ...
      * **kubeletEndpoint** ``Optional[DaemonEndpoint]`` - *(optional)* Endpoint on which Kubelet is listening.
    """
...
class NodeSelectorRequirement(DictMixin):
    """A node selector requirement is a selector that contains values, a key, and an
      ...
      * **values** ``Optional[List[str]]`` - *(optional)* An array of string values. If the operator is In or NotIn, the values array
...
class NodeSelectorTerm(DictMixin):
    """A null or empty node selector term matches no objects. The requirements of
      ...
      * **matchExpressions** ``Optional[List[NodeSelectorRequirement]]`` - *(optional)* A list of node selector requirements by node's labels.
      * **matchFields** ``Optional[List[NodeSelectorRequirement]]`` - *(optional)* A list of node selector requirements by node's fields.
    """
...
class NodeSpec(DictMixin):
    """NodeSpec describes the attributes that a node is created with.
      ...
      * **configSource** ``Optional[NodeConfigSource]`` - *(optional)* Deprecated: Previously used to specify the source of the node's configuration
        for the DynamicKubeletConfig feature. This feature is removed.
      * **externalID** ``Optional[str]`` - *(optional)* Deprecated. Not all kubelets will set this field. Remove field after 1.13.
        see: https://issues.k8s.io/61966
      * **podCIDR** ``Optional[str]`` - *(optional)* PodCIDR represents the pod IP range assigned to the node.
      * **podCIDRs** ``Optional[List[str]]`` - *(optional)* podCIDRs represents the IP ranges assigned to the node for usage by Pods on
      ...
      * **providerID** ``Optional[str]`` - *(optional)* ID of the node assigned by the cloud provider in the format:
        <ProviderName>://<ProviderSpecificNodeID>
      * **taints** ``Optional[List[Taint]]`` - *(optional)* If specified, the node's taints.
      * **unschedulable** ``Optional[bool]`` - *(optional)* Unschedulable controls node schedulability of new pods. By default, node is
...
class NodeStatus(DictMixin):
    """NodeStatus is information about the current status of a node.
      ...
      * **addresses** ``Optional[List[NodeAddress]]`` - *(optional)* List of addresses reachable to the node. Queried from cloud provider, if
        available. More info:
      ...
      * **allocatable** ``Optional[dict]`` - *(optional)* Allocatable represents the resources of a node that are available for
        scheduling. Defaults to Capacity.
      * **capacity** ``Optional[dict]`` - *(optional)* Capacity represents the total resources of a node. More info:
        https://kubernetes.io/docs/concepts/storage/persistent-volumes#capacity
      * **conditions** ``Optional[List[NodeCondition]]`` - *(optional)* Conditions is an array of current observed node conditions. More info:
        https://kubernetes.io/docs/concepts/nodes/node/#condition
      * **config** ``Optional[NodeConfigStatus]`` - *(optional)* Status of the config assigned to the node via the dynamic Kubelet config
        feature.
      * **daemonEndpoints** ``Optional[NodeDaemonEndpoints]`` - *(optional)* Endpoints of daemons running on the Node.
      * **images** ``Optional[List[ContainerImage]]`` - *(optional)* List of container images on this node
      * **nodeInfo** ``Optional[NodeSystemInfo]`` - *(optional)* Set of ids/uuids to uniquely identify the node. More info:
        https://kubernetes.io/docs/concepts/nodes/node/#info
      * **phase** ``Optional[str]`` - *(optional)* NodePhase is the recently observed lifecycle phase of the node. More info:
        https://kubernetes.io/docs/concepts/nodes/node/#phase The field is never
        populated, and now is deprecated.
      * **volumesAttached** ``Optional[List[AttachedVolume]]`` - *(optional)* List of volumes that are attached to the node.
      * **volumesInUse** ``Optional[List[str]]`` - *(optional)* List of attachable volumes in use (mounted) by the node.
    """
...
class ObjectFieldSelector(DictMixin):
    """ObjectFieldSelector selects an APIVersioned field of an object.
      ...
      * **apiVersion** ``Optional[str]`` - *(optional)* Version of the schema the FieldPath is written in terms of, defaults to "v1".
    """
    fieldPath: 'str'
    apiVersion: 'Optional[str]' = None
...
class ObjectReference(DictMixin):
    """ObjectReference contains enough information to let you inspect or modify the
      ...
      * **apiVersion** ``Optional[str]`` - *(optional)* API version of the referent.
      * **fieldPath** ``Optional[str]`` - *(optional)* If referring to a piece of an object instead of an entire object, this string
      ...
      * **kind** ``Optional[str]`` - *(optional)* Kind of the referent. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds
      * **name** ``Optional[str]`` - *(optional)* Name of the referent. More info:
        https://kubernetes.io/docs/concepts/overview/working-with-objects/names/#names
      * **namespace** ``Optional[str]`` - *(optional)* Namespace of the referent. More info:
        https://kubernetes.io/docs/concepts/overview/working-with-objects/namespaces/
      * **resourceVersion** ``Optional[str]`` - *(optional)* Specific resourceVersion to which this reference is made, if any. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#concurrency-control-and-consistency
      * **uid** ``Optional[str]`` - *(optional)* UID of the referent. More info:
...
class PersistentVolume(DictMixin):
    """PersistentVolume (PV) is a storage resource provisioned by an administrator.
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **spec** ``Optional[PersistentVolumeSpec]`` - *(optional)* spec defines a specification of a persistent volume owned by the cluster.
        Provisioned by an administrator. More info:
        https://kubernetes.io/docs/concepts/storage/persistent-volumes#persistent-volumes
      * **status** ``Optional[PersistentVolumeStatus]`` - *(optional)* status represents the current information/status for the persistent volume.
...
class PersistentVolumeClaim(DictMixin):
    """PersistentVolumeClaim is a user's request for and claim to a persistent volume
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **spec** ``Optional[PersistentVolumeClaimSpec]`` - *(optional)* spec defines the desired characteristics of a volume requested by a pod
        author. More info:
        https://kubernetes.io/docs/concepts/storage/persistent-volumes#persistentvolumeclaims
      * **status** ``Optional[PersistentVolumeClaimStatus]`` - *(optional)* status represents the current information/status of a persistent volume claim.
        Read-only. More info:
        https://kubernetes.io/docs/concepts/storage/persistent-volumes#persistentvolumeclaims
    """
    apiVersion: 'Optional[str]' = None
...
class PersistentVolumeClaimCondition(DictMixin):
    """PersistentVolumeClaimCondition contains details about state of pvc
      ...
      * **status** ``str`` - 
      * **type** ``str`` - 
      * **lastProbeTime** ``Optional[meta_v1.Time]`` - *(optional)* lastProbeTime is the time we probed the condition.
      * **lastTransitionTime** ``Optional[meta_v1.Time]`` - *(optional)* lastTransitionTime is the time the condition transitioned from one status to
        another.
      * **message** ``Optional[str]`` - *(optional)* message is the human-readable message indicating details about last
        transition.
      * **reason** ``Optional[str]`` - *(optional)* reason is a unique, this should be a short, machine understandable string that
...
class PersistentVolumeClaimSpec(DictMixin):
    """PersistentVolumeClaimSpec describes the common attributes of storage devices
      ...
      * **accessModes** ``Optional[List[str]]`` - *(optional)* accessModes contains the desired access modes the volume should have. More
        info:
        https://kubernetes.io/docs/concepts/storage/persistent-volumes#access-modes-1
      * **dataSource** ``Optional[TypedLocalObjectReference]`` - *(optional)* dataSource field can be used to specify either: * An existing VolumeSnapshot
      ...
      * **dataSourceRef** ``Optional[TypedObjectReference]`` - *(optional)* dataSourceRef specifies the object from which to populate the volume with
      ...
      * **resources** ``Optional[ResourceRequirements]`` - *(optional)* resources represents the minimum resources the volume should have. If
      ...
      * **selector** ``Optional[meta_v1.LabelSelector]`` - *(optional)* selector is a label query over volumes to consider for binding.
      * **storageClassName** ``Optional[str]`` - *(optional)* storageClassName is the name of the StorageClass required by the claim. More
        info: https://kubernetes.io/docs/concepts/storage/persistent-volumes#class-1
      * **volumeMode** ``Optional[str]`` - *(optional)* volumeMode defines what type of volume is required by the claim. Value of
        Filesystem is implied when not included in claim spec.
      * **volumeName** ``Optional[str]`` - *(optional)* volumeName is the binding reference to the PersistentVolume backing this
        claim.
    """
...
class PersistentVolumeClaimStatus(DictMixin):
    """PersistentVolumeClaimStatus is the current status of a persistent volume
      ...
      * **accessModes** ``Optional[List[str]]`` - *(optional)* accessModes contains the actual access modes the volume backing the PVC has.
        More info:
        https://kubernetes.io/docs/concepts/storage/persistent-volumes#access-modes-1
      * **allocatedResources** ``Optional[dict]`` - *(optional)* allocatedResources is the storage resource within AllocatedResources tracks
        the capacity allocated to a PVC. It may be larger than the actual capacity
        when a volume expansion operation is requested. For storage quota, the larger
        value from allocatedResources and PVC.spec.resources is used. If
        allocatedResources is not set, PVC.spec.resources alone is used for quota
        calculation. If a volume expansion capacity request is lowered,
        allocatedResources is only lowered if there are no expansion operations in
        progress and if the actual volume capacity is equal or lower than the
        requested capacity. This is an alpha field and requires enabling
        RecoverVolumeExpansionFailure feature.
      * **capacity** ``Optional[dict]`` - *(optional)* capacity represents the actual resources of the underlying volume.
      * **conditions** ``Optional[List[PersistentVolumeClaimCondition]]`` - *(optional)* conditions is the current Condition of persistent volume claim. If underlying
        persistent volume is being resized then the Condition will be set to
        'ResizeStarted'.
      * **phase** ``Optional[str]`` - *(optional)* phase represents the current phase of PersistentVolumeClaim.
      * **resizeStatus** ``Optional[str]`` - *(optional)* resizeStatus stores status of resize operation. ResizeStatus is not set by
...
class PersistentVolumeClaimTemplate(DictMixin):
    """PersistentVolumeClaimTemplate is used to produce PersistentVolumeClaim objects
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* May contain labels and annotations that will be copied into the PVC when
...
class PersistentVolumeClaimVolumeSource(DictMixin):
    """PersistentVolumeClaimVolumeSource references the user's PVC in the same
      ...
      * **claimName** ``str`` - claimName is the name of a PersistentVolumeClaim in the same namespace as the
        ...
        https://kubernetes.io/docs/concepts/storage/persistent-volumes#persistentvolumeclaims
      * **readOnly** ``Optional[bool]`` - *(optional)* readOnly Will force the ReadOnly setting in VolumeMounts. Default false.
    """
    claimName: 'str'
    readOnly: 'Optional[bool]' = None
...
class PersistentVolumeSpec(DictMixin):
    """PersistentVolumeSpec is the specification of a persistent volume.
      ...
      * **accessModes** ``Optional[List[str]]`` - *(optional)* accessModes contains all ways the volume can be mounted. More info:
        https://kubernetes.io/docs/concepts/storage/persistent-volumes#access-modes
      * **awsElasticBlockStore** ``Optional[AWSElasticBlockStoreVolumeSource]`` - *(optional)* awsElasticBlockStore represents an AWS Disk resource that is attached to a
        kubelet's host machine and then exposed to the pod. More info:
        https://kubernetes.io/docs/concepts/storage/volumes#awselasticblockstore
      * **azureDisk** ``Optional[AzureDiskVolumeSource]`` - *(optional)* azureDisk represents an Azure Data Disk mount on the host and bind mount to
        the pod.
      * **azureFile** ``Optional[AzureFilePersistentVolumeSource]`` - *(optional)* azureFile represents an Azure File Service mount on the host and bind mount to
        the pod.
      * **capacity** ``Optional[dict]`` - *(optional)* capacity is the description of the persistent volume's resources and capacity.
        More info:
        https://kubernetes.io/docs/concepts/storage/persistent-volumes#capacity
      * **cephfs** ``Optional[CephFSPersistentVolumeSource]`` - *(optional)* cephFS represents a Ceph FS mount on the host that shares a pod's lifetime
      * **cinder** ``Optional[CinderPersistentVolumeSource]`` - *(optional)* cinder represents a cinder volume attached and mounted on kubelets host
        machine. More info: https://examples.k8s.io/mysql-cinder-pd/README.md
      * **claimRef** ``Optional[ObjectReference]`` - *(optional)* claimRef is part of a bi-directional binding between PersistentVolume and
      ...
      * **csi** ``Optional[CSIPersistentVolumeSource]`` - *(optional)* csi represents storage that is handled by an external CSI driver (Beta
        feature).
      * **fc** ``Optional[FCVolumeSource]`` - *(optional)* fc represents a Fibre Channel resource that is attached to a kubelet's host
        machine and then exposed to the pod.
      * **flexVolume** ``Optional[FlexPersistentVolumeSource]`` - *(optional)* flexVolume represents a generic volume resource that is provisioned/attached
        using an exec based plugin.
      * **flocker** ``Optional[FlockerVolumeSource]`` - *(optional)* flocker represents a Flocker volume attached to a kubelet's host machine and
        exposed to the pod for its usage. This depends on the Flocker control service
        being running
      * **gcePersistentDisk** ``Optional[GCEPersistentDiskVolumeSource]`` - *(optional)* gcePersistentDisk represents a GCE Disk resource that is attached to a
        kubelet's host machine and then exposed to the pod. Provisioned by an admin.
        More info:
        https://kubernetes.io/docs/concepts/storage/volumes#gcepersistentdisk
      * **glusterfs** ``Optional[GlusterfsPersistentVolumeSource]`` - *(optional)* glusterfs represents a Glusterfs volume that is attached to a host and exposed
        to the pod. Provisioned by an admin. More info:
        https://examples.k8s.io/volumes/glusterfs/README.md
      * **hostPath** ``Optional[HostPathVolumeSource]`` - *(optional)* hostPath represents a directory on the host. Provisioned by a developer or
      ...
      * **iscsi** ``Optional[ISCSIPersistentVolumeSource]`` - *(optional)* iscsi represents an ISCSI Disk resource that is attached to a kubelet's host
        machine and then exposed to the pod. Provisioned by an admin.
      * **local** ``Optional[LocalVolumeSource]`` - *(optional)* local represents directly-attached storage with node affinity
      * **mountOptions** ``Optional[List[str]]`` - *(optional)* mountOptions is the list of mount options, e.g. ["ro", "soft"]. Not validated
        - mount will simply fail if one is invalid. More info:
        https://kubernetes.io/docs/concepts/storage/persistent-volumes/#mount-options
      * **nfs** ``Optional[NFSVolumeSource]`` - *(optional)* nfs represents an NFS mount on the host. Provisioned by an admin. More info:
        https://kubernetes.io/docs/concepts/storage/volumes#nfs
      * **nodeAffinity** ``Optional[VolumeNodeAffinity]`` - *(optional)* nodeAffinity defines constraints that limit what nodes this volume can be
        accessed from. This field influences the scheduling of pods that use this
        volume.
      * **persistentVolumeReclaimPolicy** ``Optional[str]`` - *(optional)* persistentVolumeReclaimPolicy defines what happens to a persistent volume when
      ...
      * **photonPersistentDisk** ``Optional[PhotonPersistentDiskVolumeSource]`` - *(optional)* photonPersistentDisk represents a PhotonController persistent disk attached
        and mounted on kubelets host machine
      * **portworxVolume** ``Optional[PortworxVolumeSource]`` - *(optional)* portworxVolume represents a portworx volume attached and mounted on kubelets
        host machine
      * **quobyte** ``Optional[QuobyteVolumeSource]`` - *(optional)* quobyte represents a Quobyte mount on the host that shares a pod's lifetime
      * **rbd** ``Optional[RBDPersistentVolumeSource]`` - *(optional)* rbd represents a Rados Block Device mount on the host that shares a pod's
        lifetime. More info: https://examples.k8s.io/volumes/rbd/README.md
      * **scaleIO** ``Optional[ScaleIOPersistentVolumeSource]`` - *(optional)* scaleIO represents a ScaleIO persistent volume attached and mounted on
        Kubernetes nodes.
      * **storageClassName** ``Optional[str]`` - *(optional)* storageClassName is the name of StorageClass to which this persistent volume
        belongs. Empty value means that this volume does not belong to any
        StorageClass.
      * **storageos** ``Optional[StorageOSPersistentVolumeSource]`` - *(optional)* storageOS represents a StorageOS volume that is attached to the kubelet's host
        machine and mounted into the pod More info:
        https://examples.k8s.io/volumes/storageos/README.md
      * **volumeMode** ``Optional[str]`` - *(optional)* volumeMode defines if a volume is intended to be used with a formatted
        filesystem or to remain in raw block state. Value of Filesystem is implied
        when not included in spec.
      * **vsphereVolume** ``Optional[VsphereVirtualDiskVolumeSource]`` - *(optional)* vsphereVolume represents a vSphere volume attached and mounted on kubelets
        host machine
    """
    accessModes: 'Optional[List[str]]' = None
...
class PersistentVolumeStatus(DictMixin):
    """PersistentVolumeStatus is the current status of a persistent volume.
      ...
      * **message** ``Optional[str]`` - *(optional)* message is a human-readable message indicating details about why the volume is
        in this state.
      * **phase** ``Optional[str]`` - *(optional)* phase indicates if a volume is available, bound to a claim, or released by a
        claim. More info:
        https://kubernetes.io/docs/concepts/storage/persistent-volumes#phase
      * **reason** ``Optional[str]`` - *(optional)* reason is a brief CamelCase string that describes any failure and is meant for
...
class Pod(DictMixin):
    """Pod is a collection of containers that can run on a host. This resource is
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **spec** ``Optional[PodSpec]`` - *(optional)* Specification of the desired behavior of the pod. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
      * **status** ``Optional[PodStatus]`` - *(optional)* Most recently observed status of the pod. This data may not be up to date.
        Populated by the system. Read-only. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
    """
    apiVersion: 'Optional[str]' = None
...
class PodAffinity(DictMixin):
    """Pod affinity is a group of inter pod affinity scheduling rules.
      ...
      * **preferredDuringSchedulingIgnoredDuringExecution** ``Optional[List[WeightedPodAffinityTerm]]`` - *(optional)* The scheduler will prefer to schedule pods to nodes that satisfy the affinity
        expressions specified by this field, but it may choose a node that violates
        one or more of the expressions. The node that is most preferred is the one
        with the greatest sum of weights, i.e. for each node that meets all of the
        scheduling requirements (resource request, requiredDuringScheduling affinity
        ...
        preferred.
      * **requiredDuringSchedulingIgnoredDuringExecution** ``Optional[List[PodAffinityTerm]]`` - *(optional)* If the affinity requirements specified by this field are not met at scheduling
        time, the pod will not be scheduled onto the node. If the affinity
        requirements specified by this field cease to be met at some point during pod
...
class PodAffinityTerm(DictMixin):
    """Defines a set of pods (namely those matching the labelSelector relative to the
      ...
      * **labelSelector** ``Optional[meta_v1.LabelSelector]`` - *(optional)* A label query over a set of resources, in this case pods.
      * **namespaceSelector** ``Optional[meta_v1.LabelSelector]`` - *(optional)* A label query over the set of namespaces that the term applies to. The term is
        ...
        namespaces.
      * **namespaces** ``Optional[List[str]]`` - *(optional)* namespaces specifies a static list of namespace names that the term applies
...
class PodAntiAffinity(DictMixin):
    """Pod anti affinity is a group of inter pod anti affinity scheduling rules.
      ...
      * **preferredDuringSchedulingIgnoredDuringExecution** ``Optional[List[WeightedPodAffinityTerm]]`` - *(optional)* The scheduler will prefer to schedule pods to nodes that satisfy the
      ...
      * **requiredDuringSchedulingIgnoredDuringExecution** ``Optional[List[PodAffinityTerm]]`` - *(optional)* If the anti-affinity requirements specified by this field are not met at
...
class PodCondition(DictMixin):
    """PodCondition contains details for the current condition of this pod.
      ...
      * **type** ``str`` - Type is the type of the condition. More info:
        https://kubernetes.io/docs/concepts/workloads/pods/pod-lifecycle#pod-conditions
      * **lastProbeTime** ``Optional[meta_v1.Time]`` - *(optional)* Last time we probed the condition.
      * **lastTransitionTime** ``Optional[meta_v1.Time]`` - *(optional)* Last time the condition transitioned from one status to another.
      * **message** ``Optional[str]`` - *(optional)* Human-readable message indicating details about last transition.
      * **reason** ``Optional[str]`` - *(optional)* Unique, one-word, CamelCase reason for the condition's last transition.
    """
    status: 'str'
    type: 'str'
    lastProbeTime: 'Optional[meta_v1.Time]' = None
...
class PodDNSConfig(DictMixin):
    """PodDNSConfig defines the DNS parameters of a pod in addition to those
      ...
      * **nameservers** ``Optional[List[str]]`` - *(optional)* A list of DNS name server IP addresses. This will be appended to the base
        nameservers generated from DNSPolicy. Duplicated nameservers will be removed.
      * **options** ``Optional[List[PodDNSConfigOption]]`` - *(optional)* A list of DNS resolver options. This will be merged with the base options
        ...
        DNSPolicy.
      * **searches** ``Optional[List[str]]`` - *(optional)* A list of DNS search domains for host-name lookup. This will be appended to
...
class PodDNSConfigOption(DictMixin):
    """PodDNSConfigOption defines DNS resolver options of a pod.
      ...
      * **name** ``Optional[str]`` - *(optional)* Required.
...
class PodIP(DictMixin):
    """IP address information for entries in the (plural) PodIPs field. Each entry
      ...
      * **ip** ``Optional[str]`` - *(optional)* ip is an IP address (IPv4 or IPv6) assigned to the pod
    """
    ip: 'Optional[str]' = None
...
class PodResourceClaim(DictMixin):
    """PodResourceClaim references exactly one ResourceClaim through a ClaimSource.
      ...
      * **name** ``str`` - Name uniquely identifies this resource claim inside the pod. This must be a
        DNS_LABEL.
      * **source** ``Optional[ClaimSource]`` - *(optional)* Source describes where to find the ResourceClaim.
    """
    name: 'str'
...
class PodSecurityContext(DictMixin):
    """PodSecurityContext holds pod-level security attributes and common container
      ...
      * **fsGroup** ``Optional[int]`` - *(optional)* A special supplemental group that applies to all containers in a pod. Some
      ...
      * **fsGroupChangePolicy** ``Optional[str]`` - *(optional)* fsGroupChangePolicy defines behavior of changing ownership and permission of
        ...
        windows.
      * **runAsGroup** ``Optional[int]`` - *(optional)* The GID to run the entrypoint of the container process. Uses runtime default
      ...
      * **runAsNonRoot** ``Optional[bool]`` - *(optional)* Indicates that the container must run as a non-root user. If true, the Kubelet
      ...
      * **runAsUser** ``Optional[int]`` - *(optional)* The UID to run the entrypoint of the container process. Defaults to user
      ...
      * **seLinuxOptions** ``Optional[SELinuxOptions]`` - *(optional)* The SELinux context to be applied to all containers. If unspecified, the
        ...
        windows.
      * **seccompProfile** ``Optional[SeccompProfile]`` - *(optional)* The seccomp options to use by the containers in this pod. Note that this field
        cannot be set when spec.os.name is windows.
      * **supplementalGroups** ``Optional[List[int]]`` - *(optional)* A list of groups applied to the first process run in each container, in
      ...
      * **sysctls** ``Optional[List[Sysctl]]`` - *(optional)* Sysctls hold a list of namespaced sysctls used for the pod. Pods with
        unsupported sysctls (by the container runtime) might fail to launch. Note that
        this field cannot be set when spec.os.name is windows.
      * **windowsOptions** ``Optional[WindowsSecurityContextOptions]`` - *(optional)* The Windows specific settings applied to all containers. If unspecified, the
...
class PodSpec(DictMixin):
    """PodSpec is a description of a pod.
      ...
      * **activeDeadlineSeconds** ``Optional[int]`` - *(optional)* Optional duration in seconds the pod may be active on the node relative to
        StartTime before the system will actively try to mark it failed and kill
        associated containers. Value must be a positive integer.
      * **affinity** ``Optional[Affinity]`` - *(optional)* If specified, the pod's scheduling constraints
      * **automountServiceAccountToken** ``Optional[bool]`` - *(optional)* AutomountServiceAccountToken indicates whether a service account token should
        be automatically mounted.
      * **dnsConfig** ``Optional[PodDNSConfig]`` - *(optional)* Specifies the DNS parameters of a pod. Parameters specified here will be
        merged to the generated DNS configuration based on DNSPolicy.
      * **dnsPolicy** ``Optional[str]`` - *(optional)* Set DNS policy for the pod. Defaults to "ClusterFirst". Valid values are
      ...
      * **enableServiceLinks** ``Optional[bool]`` - *(optional)* EnableServiceLinks indicates whether information about services should be
        injected into pod's environment variables, matching the syntax of Docker
        links. Optional: Defaults to true.
      * **ephemeralContainers** ``Optional[List[EphemeralContainer]]`` - *(optional)* List of ephemeral containers run in this pod. Ephemeral containers may be run
      ...
      * **hostAliases** ``Optional[List[HostAlias]]`` - *(optional)* HostAliases is an optional list of hosts and IPs that will be injected into
        the pod's hosts file if specified. This is only valid for non-hostNetwork
        pods.
      * **hostIPC** ``Optional[bool]`` - *(optional)* Use the host's ipc namespace. Optional: Default to false.
      * **hostNetwork** ``Optional[bool]`` - *(optional)* Host networking requested for this pod. Use the host's network namespace. If
        this option is set, the ports that will be used must be specified. Default to
        false.
      * **hostPID** ``Optional[bool]`` - *(optional)* Use the host's pid namespace. Optional: Default to false.
      * **hostUsers** ``Optional[bool]`` - *(optional)* Use the host's user namespace. Optional: Default to true. If set to true or
      ...
      * **hostname** ``Optional[str]`` - *(optional)* Specifies the hostname of the Pod If not specified, the pod's hostname will be
        set to a system-defined value.
      * **imagePullSecrets** ``Optional[List[LocalObjectReference]]`` - *(optional)* ImagePullSecrets is an optional list of references to secrets in the same
      ...
      * **initContainers** ``Optional[List[Container]]`` - *(optional)* List of initialization containers belonging to the pod. Init containers are
      ...
      * **nodeName** ``Optional[str]`` - *(optional)* NodeName is a request to schedule this pod onto a specific node. If it is
        non-empty, the scheduler simply schedules this pod onto that node, assuming
        that it fits resource requirements.
      * **nodeSelector** ``Optional[dict]`` - *(optional)* NodeSelector is a selector which must be true for the pod to fit on a node.
        Selector which must match a node's labels for the pod to be scheduled on that
        node. More info:
        https://kubernetes.io/docs/concepts/configuration/assign-pod-node/
      * **os** ``Optional[PodOS]`` - *(optional)* Specifies the OS of the containers in the pod. Some pod and container fields
      ...
      * **overhead** ``Optional[dict]`` - *(optional)* Overhead represents the resource overhead associated with running a pod for a
      ...
      * **preemptionPolicy** ``Optional[str]`` - *(optional)* PreemptionPolicy is the Policy for preempting pods with lower priority. One of
        Never, PreemptLowerPriority. Defaults to PreemptLowerPriority if unset.
      * **priority** ``Optional[int]`` - *(optional)* The priority value. Various system components use this field to find the
        priority of the pod. When Priority Admission Controller is enabled, it
        prevents users from setting this field. The admission controller populates
        this field from PriorityClassName. The higher the value, the higher the
        priority.
      * **priorityClassName** ``Optional[str]`` - *(optional)* If specified, indicates the pod's priority. "system-node-critical" and
      ...
      * **readinessGates** ``Optional[List[PodReadinessGate]]`` - *(optional)* If specified, all readiness gates will be evaluated for pod readiness. A pod
      ...
      * **resourceClaims** ``Optional[List[PodResourceClaim]]`` - *(optional)* ResourceClaims defines which ResourceClaims must be allocated and reserved
      ...
      * **restartPolicy** ``Optional[str]`` - *(optional)* Restart policy for all containers within the pod. One of Always, OnFailure,
        Never. In some contexts, only a subset of those values may be permitted.
        Default to Always. More info:
        https://kubernetes.io/docs/concepts/workloads/pods/pod-lifecycle/#restart-policy
      * **runtimeClassName** ``Optional[str]`` - *(optional)* RuntimeClassName refers to a RuntimeClass object in the node.k8s.io group,
      ...
      * **schedulerName** ``Optional[str]`` - *(optional)* If specified, the pod will be dispatched by specified scheduler. If not
        specified, the pod will be dispatched by default scheduler.
      * **schedulingGates** ``Optional[List[PodSchedulingGate]]`` - *(optional)* SchedulingGates is an opaque list of values that if specified will block
      ...
      * **securityContext** ``Optional[PodSecurityContext]`` - *(optional)* SecurityContext holds pod-level security attributes and common container
        settings. Optional: Defaults to empty.  See type description for default
        values of each field.
      * **serviceAccount** ``Optional[str]`` - *(optional)* DeprecatedServiceAccount is a depreciated alias for ServiceAccountName.
        Deprecated: Use serviceAccountName instead.
      * **serviceAccountName** ``Optional[str]`` - *(optional)* ServiceAccountName is the name of the ServiceAccount to use to run this pod.
        More info:
        https://kubernetes.io/docs/tasks/configure-pod-container/configure-service-account/
      * **setHostnameAsFQDN** ``Optional[bool]`` - *(optional)* If true the pod's hostname will be configured as the pod's FQDN, rather than
      ...
      * **shareProcessNamespace** ``Optional[bool]`` - *(optional)* Share a single process namespace between all of the containers in a pod. When
      ...
      * **subdomain** ``Optional[str]`` - *(optional)* If specified, the fully qualified Pod hostname will be
        "<hostname>.<subdomain>.<pod namespace>.svc.<cluster domain>". If not
        specified, the pod will not have a domainname at all.
      * **terminationGracePeriodSeconds** ``Optional[int]`` - *(optional)* Optional duration in seconds the pod needs to terminate gracefully. May be
      ...
      * **tolerations** ``Optional[List[Toleration]]`` - *(optional)* If specified, the pod's tolerations.
      * **topologySpreadConstraints** ``Optional[List[TopologySpreadConstraint]]`` - *(optional)* TopologySpreadConstraints describes how a group of pods ought to spread across
      ...
      * **volumes** ``Optional[List[Volume]]`` - *(optional)* List of volumes that can be mounted by containers belonging to the pod. More
...
class PodStatus(DictMixin):
    """PodStatus represents information about the status of a pod. Status may trail
      ...
      * **conditions** ``Optional[List[PodCondition]]`` - *(optional)* Current service state of pod. More info:
        https://kubernetes.io/docs/concepts/workloads/pods/pod-lifecycle#pod-conditions
      * **containerStatuses** ``Optional[List[ContainerStatus]]`` - *(optional)* The list has one entry per container in the manifest. More info:
        https://kubernetes.io/docs/concepts/workloads/pods/pod-lifecycle#pod-and-container-status
      * **ephemeralContainerStatuses** ``Optional[List[ContainerStatus]]`` - *(optional)* Status for any ephemeral containers that have run in this pod.
      * **hostIP** ``Optional[str]`` - *(optional)* IP address of the host to which the pod is assigned. Empty if not yet
        scheduled.
      * **initContainerStatuses** ``Optional[List[ContainerStatus]]`` - *(optional)* The list has one entry per init container in the manifest. The most recent
        successful init container will have ready = true, the most recently started
        container will have startTime set. More info:
        https://kubernetes.io/docs/concepts/workloads/pods/pod-lifecycle#pod-and-container-status
      * **message** ``Optional[str]`` - *(optional)* A human readable message indicating details about why the pod is in this
        condition.
      * **nominatedNodeName** ``Optional[str]`` - *(optional)* nominatedNodeName is set only when this pod preempts other pods on the node,
      ...
      * **phase** ``Optional[str]`` - *(optional)* The phase of a Pod is a simple, high-level summary of where the Pod is in its
      ...
      * **podIP** ``Optional[str]`` - *(optional)* IP address allocated to the pod. Routable at least within the cluster. Empty
        if not yet allocated.
      * **podIPs** ``Optional[List[PodIP]]`` - *(optional)* podIPs holds the IP addresses allocated to the pod. If this field is
        ...
        allocated yet.
      * **qosClass** ``Optional[str]`` - *(optional)* The Quality of Service (QOS) classification assigned to the pod based on
      ...
      * **reason** ``Optional[str]`` - *(optional)* A brief CamelCase message indicating details about why the pod is in this
        state. e.g. 'Evicted'
      * **resize** ``Optional[str]`` - *(optional)* Status of resources resize desired for pod's containers. It is empty if no
        resources resize is pending. Any changes to container resources will
        automatically set this to "Proposed"
      * **startTime** ``Optional[meta_v1.Time]`` - *(optional)* RFC 3339 date and time at which the object was acknowledged by the Kubelet.
...
class PodTemplate(DictMixin):
    """PodTemplate describes a template for creating copies of a predefined pod.
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **template** ``Optional[PodTemplateSpec]`` - *(optional)* Template defines the pods that will be created from this pod template.
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
    """
    apiVersion: 'Optional[str]' = None
    kind: 'Optional[str]' = None
...
class PortStatus(DictMixin):
    """
      ...
      * **error** ``Optional[str]`` - *(optional)* Error is to record the problem with the service port The format of the error
...
class PortworxVolumeSource(DictMixin):
    """PortworxVolumeSource represents a Portworx volume resource.
      ...
      * **fsType** ``Optional[str]`` - *(optional)* fSType represents the filesystem type to mount Must be a filesystem type
...
class Probe(DictMixin):
    """Probe describes a health check to be performed against a container to
      ...
      * **exec** ``Optional[ExecAction]`` - *(optional)* Exec specifies the action to take.
      * **failureThreshold** ``Optional[int]`` - *(optional)* Minimum consecutive failures for the probe to be considered failed after
        having succeeded. Defaults to 3. Minimum value is 1.
      * **grpc** ``Optional[GRPCAction]`` - *(optional)* GRPC specifies an action involving a GRPC port.
      * **httpGet** ``Optional[HTTPGetAction]`` - *(optional)* HTTPGet specifies the http request to perform.
      * **initialDelaySeconds** ``Optional[int]`` - *(optional)* Number of seconds after the container has started before liveness probes are
        initiated. More info:
        https://kubernetes.io/docs/concepts/workloads/pods/pod-lifecycle#container-probes
      * **periodSeconds** ``Optional[int]`` - *(optional)* How often (in seconds) to perform the probe. Default to 10 seconds. Minimum
        value is 1.
      * **successThreshold** ``Optional[int]`` - *(optional)* Minimum consecutive successes for the probe to be considered successful after
        having failed. Defaults to 1. Must be 1 for liveness and startup. Minimum
        value is 1.
      * **tcpSocket** ``Optional[TCPSocketAction]`` - *(optional)* TCPSocket specifies an action involving a TCP port.
      * **terminationGracePeriodSeconds** ``Optional[int]`` - *(optional)* Optional duration in seconds the pod needs to terminate gracefully upon probe
      ...
      * **timeoutSeconds** ``Optional[int]`` - *(optional)* Number of seconds after which the probe times out. Defaults to 1 second.
...
class ProjectedVolumeSource(DictMixin):
    """Represents a projected volume source
      ...
      * **defaultMode** ``Optional[int]`` - *(optional)* defaultMode are the mode bits used to set permissions on created files by
        default. Must be an octal value between 0000 and 0777 or a decimal value
        between 0 and 511. YAML accepts both octal and decimal values, JSON requires
        decimal values for mode bits. Directories within the path are not affected by
        this setting. This might be in conflict with other options that affect the
        file mode, like fsGroup, and the result can be other mode bits set.
      * **sources** ``Optional[List[VolumeProjection]]`` - *(optional)* sources is the list of volume projections
    """
    defaultMode: 'Optional[int]' = None
...
class QuobyteVolumeSource(DictMixin):
    """Represents a Quobyte mount that lasts the lifetime of a pod. Quobyte volumes
      ...
      * **group** ``Optional[str]`` - *(optional)* group to map volume access to Default is no group
      * **readOnly** ``Optional[bool]`` - *(optional)* readOnly here will force the Quobyte volume to be mounted with read-only
        permissions. Defaults to false.
      * **tenant** ``Optional[str]`` - *(optional)* tenant owning the given Quobyte volume in the Backend Used with dynamically
        provisioned Quobyte volumes, value is set by the plugin
      * **user** ``Optional[str]`` - *(optional)* user to map volume access to Defaults to serivceaccount user
    """
    registry: 'str'
    volume: 'str'
...
class RBDPersistentVolumeSource(DictMixin):
    """Represents a Rados Block Device mount that lasts the lifetime of a pod. RBD
      ...
      * **keyring** ``Optional[str]`` - *(optional)* keyring is the path to key ring for RBDUser. Default is /etc/ceph/keyring.
        More info: https://examples.k8s.io/volumes/rbd/README.md#how-to-use-it
      * **pool** ``Optional[str]`` - *(optional)* pool is the rados pool name. Default is rbd. More info:
        https://examples.k8s.io/volumes/rbd/README.md#how-to-use-it
      * **readOnly** ``Optional[bool]`` - *(optional)* readOnly here will force the ReadOnly setting in VolumeMounts. Defaults to
        false. More info: https://examples.k8s.io/volumes/rbd/README.md#how-to-use-it
      * **secretRef** ``Optional[SecretReference]`` - *(optional)* secretRef is name of the authentication secret for RBDUser. If provided
...
class RBDVolumeSource(DictMixin):
    """Represents a Rados Block Device mount that lasts the lifetime of a pod. RBD
      ...
      * **readOnly** ``Optional[bool]`` - *(optional)* readOnly here will force the ReadOnly setting in VolumeMounts. Defaults to
        false. More info: https://examples.k8s.io/volumes/rbd/README.md#how-to-use-it
      * **secretRef** ``Optional[LocalObjectReference]`` - *(optional)* secretRef is name of the authentication secret for RBDUser. If provided
        overrides keyring. Default is nil. More info:
        https://examples.k8s.io/volumes/rbd/README.md#how-to-use-it
      * **user** ``Optional[str]`` - *(optional)* user is the rados user name. Default is admin. More info:
        https://examples.k8s.io/volumes/rbd/README.md#how-to-use-it
...
class ReplicationController(DictMixin):
    """ReplicationController represents the configuration of a replication
      ...
      * **kind** ``Optional[str]`` - *(optional)* Kind is a string value representing the REST resource this object represents.
        ...
        Cannot be updated. In CamelCase. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* If the Labels of a ReplicationController are empty, they are defaulted to be
        the same as the Pod(s) that the replication controller manages. Standard
        object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **spec** ``Optional[ReplicationControllerSpec]`` - *(optional)* Spec defines the specification of the desired behavior of the replication
        controller. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
      * **status** ``Optional[ReplicationControllerStatus]`` - *(optional)* Status is the most recently observed status of the replication controller.
...
class ReplicationControllerCondition(DictMixin):
    """ReplicationControllerCondition describes the state of a replication controller
      ...
      * **lastTransitionTime** ``Optional[meta_v1.Time]`` - *(optional)* The last time the condition transitioned from one status to another.
      * **message** ``Optional[str]`` - *(optional)* A human readable message indicating details about the transition.
      * **reason** ``Optional[str]`` - *(optional)* The reason for the condition's last transition.
    """
    status: 'str'
    type: 'str'
    lastTransitionTime: 'Optional[meta_v1.Time]' = None
...
class ReplicationControllerSpec(DictMixin):
    """ReplicationControllerSpec is the specification of a replication controller.
      ...
      * **minReadySeconds** ``Optional[int]`` - *(optional)* Minimum number of seconds for which a newly created pod should be ready
        without any of its container crashing, for it to be considered available.
        Defaults to 0 (pod will be considered available as soon as it is ready)
      * **replicas** ``Optional[int]`` - *(optional)* Replicas is the number of desired replicas. This is a pointer to distinguish
        between explicit zero and unspecified. Defaults to 1. More info:
        https://kubernetes.io/docs/concepts/workloads/controllers/replicationcontroller#what-is-a-replicationcontroller
      * **selector** ``Optional[dict]`` - *(optional)* Selector is a label query over pods that should match the Replicas count. If
      ...
      * **template** ``Optional[PodTemplateSpec]`` - *(optional)* Template is the object that describes the pod that will be created if
...
class ReplicationControllerStatus(DictMixin):
    """ReplicationControllerStatus represents the current status of a replication
      ...
      * **replicas** ``int`` - Replicas is the most recently observed number of replicas. More info:
        https://kubernetes.io/docs/concepts/workloads/controllers/replicationcontroller#what-is-a-replicationcontroller
      * **availableReplicas** ``Optional[int]`` - *(optional)* The number of available replicas (ready for at least minReadySeconds) for this
        replication controller.
      * **conditions** ``Optional[List[ReplicationControllerCondition]]`` - *(optional)* Represents the latest available observations of a replication controller's
        current state.
      * **fullyLabeledReplicas** ``Optional[int]`` - *(optional)* The number of pods that have labels matching the labels of the pod template of
        the replication controller.
      * **observedGeneration** ``Optional[int]`` - *(optional)* ObservedGeneration reflects the generation of the most recently observed
        replication controller.
      * **readyReplicas** ``Optional[int]`` - *(optional)* The number of ready replicas for this replication controller.
    """
    replicas: 'int'
...
class ResourceFieldSelector(DictMixin):
    """ResourceFieldSelector represents container resources (cpu, memory) and their
      ...
      * **containerName** ``Optional[str]`` - *(optional)* Container name: required for volumes, optional for env vars
      * **divisor** ``Optional[resource.Quantity]`` - *(optional)* Specifies the output format of the exposed resources, defaults to "1"
    """
    resource: 'str'
...
class ResourceQuota(DictMixin):
    """ResourceQuota sets aggregate quota restrictions enforced per namespace
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **spec** ``Optional[ResourceQuotaSpec]`` - *(optional)* Spec defines the desired quota.
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
      * **status** ``Optional[ResourceQuotaStatus]`` - *(optional)* Status defines the actual enforced quota and its current usage.
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
    """
    apiVersion: 'Optional[str]' = None
    kind: 'Optional[str]' = None
...
class ResourceQuotaSpec(DictMixin):
    """ResourceQuotaSpec defines the desired hard limits to enforce for Quota.
      ...
      * **hard** ``Optional[dict]`` - *(optional)* hard is the set of desired hard limits for each named resource. More info:
        https://kubernetes.io/docs/concepts/policy/resource-quotas/
      * **scopeSelector** ``Optional[ScopeSelector]`` - *(optional)* scopeSelector is also a collection of filters like scopes that must match each
      ...
      * **scopes** ``Optional[List[str]]`` - *(optional)* A collection of filters that must match each object tracked by a quota. If not
...
class ResourceQuotaStatus(DictMixin):
    """ResourceQuotaStatus defines the enforced hard limits and observed use.
      ...
      * **hard** ``Optional[dict]`` - *(optional)* Hard is the set of enforced hard limits for each named resource. More info:
        https://kubernetes.io/docs/concepts/policy/resource-quotas/
      * **used** ``Optional[dict]`` - *(optional)* Used is the current observed total usage of the resource in the namespace.
    """
    hard: 'Optional[dict]' = None
...
class ResourceRequirements(DictMixin):
    """ResourceRequirements describes the compute resource requirements.
      ...
      * **claims** ``Optional[List[ResourceClaim]]`` - *(optional)* Claims lists the names of resources, defined in spec.resourceClaims, that are
        used by this container.
        This is an alpha field and requires enabling the DynamicResourceAllocation
        feature gate.
        This field is immutable. It can only be set for containers.
      * **limits** ``Optional[dict]`` - *(optional)* Limits describes the maximum amount of compute resources allowed. More info:
        https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/
      * **requests** ``Optional[dict]`` - *(optional)* Requests describes the minimum amount of compute resources required. If
...
class SELinuxOptions(DictMixin):
    """SELinuxOptions are the labels to be applied to the container
      ...
      * **level** ``Optional[str]`` - *(optional)* Level is SELinux level label that applies to the container.
      * **role** ``Optional[str]`` - *(optional)* Role is a SELinux role label that applies to the container.
      * **type** ``Optional[str]`` - *(optional)* Type is a SELinux type label that applies to the container.
      * **user** ``Optional[str]`` - *(optional)* User is a SELinux user label that applies to the container.
    """
...
class ScaleIOPersistentVolumeSource(DictMixin):
    """ScaleIOPersistentVolumeSource represents a persistent ScaleIO volume
      ...
      * **protectionDomain** ``Optional[str]`` - *(optional)* protectionDomain is the name of the ScaleIO Protection Domain for the
        configured storage.
      * **readOnly** ``Optional[bool]`` - *(optional)* readOnly defaults to false (read/write). ReadOnly here will force the ReadOnly
        setting in VolumeMounts.
      * **sslEnabled** ``Optional[bool]`` - *(optional)* sslEnabled is the flag to enable/disable SSL communication with Gateway,
        default false
      * **storageMode** ``Optional[str]`` - *(optional)* storageMode indicates whether the storage for a volume should be
        ThickProvisioned or ThinProvisioned. Default is ThinProvisioned.
      * **storagePool** ``Optional[str]`` - *(optional)* storagePool is the ScaleIO Storage Pool associated with the protection domain.
      * **volumeName** ``Optional[str]`` - *(optional)* volumeName is the name of a volume already created in the ScaleIO system that
...
class ScaleIOVolumeSource(DictMixin):
    """ScaleIOVolumeSource represents a persistent ScaleIO volume
      ...
      * **readOnly** ``Optional[bool]`` - *(optional)* readOnly Defaults to false (read/write). ReadOnly here will force the ReadOnly
        setting in VolumeMounts.
      * **sslEnabled** ``Optional[bool]`` - *(optional)* sslEnabled Flag enable/disable SSL communication with Gateway, default false
      * **storageMode** ``Optional[str]`` - *(optional)* storageMode indicates whether the storage for a volume should be
        ThickProvisioned or ThinProvisioned. Default is ThinProvisioned.
      * **storagePool** ``Optional[str]`` - *(optional)* storagePool is the ScaleIO Storage Pool associated with the protection domain.
      * **volumeName** ``Optional[str]`` - *(optional)* volumeName is the name of a volume already created in the ScaleIO system that
...
class ScopeSelector(DictMixin):
    """A scope selector represents the AND of the selectors represented by the
      ...
      * **matchExpressions** ``Optional[List[ScopedResourceSelectorRequirement]]`` - *(optional)* A list of scope selector requirements by scope of the resources.
    """
...
class SeccompProfile(DictMixin):
    """SeccompProfile defines a pod/container's seccomp profile settings. Only one
      ...
      * **localhostProfile** ``Optional[str]`` - *(optional)* localhostProfile indicates a profile defined in a file on the node should be
...
class Secret(DictMixin):
    """Secret holds secret data of a certain type. The total bytes of the values in
      ...
      * **apiVersion** ``Optional[str]`` - *(optional)* APIVersion defines the versioned schema of this representation of an object.
        ...
        may reject unrecognized values. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#resources
      * **data** ``Optional[dict]`` - *(optional)* Data contains the secret data. Each key must consist of alphanumeric
      ...
      * **immutable** ``Optional[bool]`` - *(optional)* Immutable, if set to true, ensures that data stored in the Secret cannot be
        updated (only object metadata can be modified). If not set to true, the field
        can be modified at any time. Defaulted to nil.
      * **kind** ``Optional[str]`` - *(optional)* Kind is a string value representing the REST resource this object represents.
        Servers may infer this from the endpoint the client submits requests to.
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **stringData** ``Optional[dict]`` - *(optional)* stringData allows specifying non-binary secret data in string form. It is
      ...
      * **type** ``Optional[str]`` - *(optional)* Used to facilitate programmatic handling of secret data. More info:
...
class SecretEnvSource(DictMixin):
    """SecretEnvSource selects a Secret to populate the environment variables with.
      ...
      * **name** ``Optional[str]`` - *(optional)* Name of the referent. More info:
        https://kubernetes.io/docs/concepts/overview/working-with-objects/names/#names
      * **optional** ``Optional[bool]`` - *(optional)* Specify whether the Secret must be defined
    """
    name: 'Optional[str]' = None
    optional: 'Optional[bool]' = None
...
class SecretKeySelector(DictMixin):
    """SecretKeySelector selects a key of a Secret.
      ...
      * **name** ``Optional[str]`` - *(optional)* Name of the referent. More info:
        https://kubernetes.io/docs/concepts/overview/working-with-objects/names/#names
      * **optional** ``Optional[bool]`` - *(optional)* Specify whether the Secret or its key must be defined
    """
    key: 'str'
    name: 'Optional[str]' = None
    optional: 'Optional[bool]' = None
...
class SecretProjection(DictMixin):
    """Adapts a secret into a projected volume.
      ...
      * **name** ``Optional[str]`` - *(optional)* Name of the referent. More info:
        https://kubernetes.io/docs/concepts/overview/working-with-objects/names/#names
      * **optional** ``Optional[bool]`` - *(optional)* optional field specify whether the Secret or its key must be defined
    """
    items: 'Optional[List[KeyToPath]]' = None
    name: 'Optional[str]' = None
    optional: 'Optional[bool]' = None
...
class SecretReference(DictMixin):
    """SecretReference represents a Secret Reference. It has enough information to
      ...
      * **name** ``Optional[str]`` - *(optional)* name is unique within a namespace to reference a secret resource.
      * **namespace** ``Optional[str]`` - *(optional)* namespace defines the space within which the secret name must be unique.
    """
    name: 'Optional[str]' = None
    namespace: 'Optional[str]' = None
...
class SecretVolumeSource(DictMixin):
    """Adapts a Secret into a volume.
      ...
      * **items** ``Optional[List[KeyToPath]]`` - *(optional)* items If unspecified, each key-value pair in the Data field of the referenced
        ...
        marked optional. Paths must be relative and may not contain the '..' path or
        start with '..'.
      * **optional** ``Optional[bool]`` - *(optional)* optional field specify whether the Secret or its keys must be defined
      * **secretName** ``Optional[str]`` - *(optional)* secretName is the name of the secret in the pod's namespace to use. More info:
...
class SecurityContext(DictMixin):
    """SecurityContext holds security configuration that will be applied to a
      ...
      * **allowPrivilegeEscalation** ``Optional[bool]`` - *(optional)* AllowPrivilegeEscalation controls whether a process can gain more privileges
        ...
        this field cannot be set when spec.os.name is windows.
      * **capabilities** ``Optional[Capabilities]`` - *(optional)* The capabilities to add/drop when running containers. Defaults to the default
        set of capabilities granted by the container runtime. Note that this field
        cannot be set when spec.os.name is windows.
      * **privileged** ``Optional[bool]`` - *(optional)* Run container in privileged mode. Processes in privileged containers are
        essentially equivalent to root on the host. Defaults to false. Note that this
        field cannot be set when spec.os.name is windows.
      * **procMount** ``Optional[str]`` - *(optional)* procMount denotes the type of proc mount to use for the containers. The
        ...
        windows.
      * **readOnlyRootFilesystem** ``Optional[bool]`` - *(optional)* Whether this container has a read-only root filesystem. Default is false. Note
        that this field cannot be set when spec.os.name is windows.
      * **runAsGroup** ``Optional[int]`` - *(optional)* The GID to run the entrypoint of the container process. Uses runtime default
      ...
      * **runAsUser** ``Optional[int]`` - *(optional)* The UID to run the entrypoint of the container process. Defaults to user
        ...
        cannot be set when spec.os.name is windows.
      * **seLinuxOptions** ``Optional[SELinuxOptions]`` - *(optional)* The SELinux context to be applied to the container. If unspecified, the
        container runtime will allocate a random SELinux context for each container.
        May also be set in PodSecurityContext.  If set in both SecurityContext and
        PodSecurityContext, the value specified in SecurityContext takes precedence.
        Note that this field cannot be set when spec.os.name is windows.
      * **seccompProfile** ``Optional[SeccompProfile]`` - *(optional)* The seccomp options to use by this container. If seccomp options are provided
...
class Service(DictMixin):
    """Service is a named abstraction of software service (for example, mysql)
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **spec** ``Optional[ServiceSpec]`` - *(optional)* Spec defines the behavior of a service.
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
      * **status** ``Optional[ServiceStatus]`` - *(optional)* Most recently observed status of the service. Populated by the system.
        Read-only. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
    """
    apiVersion: 'Optional[str]' = None
...
class ServiceAccount(DictMixin):
    """ServiceAccount binds together: * a name, understood by users, and perhaps by
      ...
      * **apiVersion** ``Optional[str]`` - *(optional)* APIVersion defines the versioned schema of this representation of an object.
        ...
        may reject unrecognized values. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#resources
      * **automountServiceAccountToken** ``Optional[bool]`` - *(optional)* AutomountServiceAccountToken indicates whether pods running as this service
        account should have an API token automatically mounted. Can be overridden at
        the pod level.
      * **imagePullSecrets** ``Optional[List[LocalObjectReference]]`` - *(optional)* ImagePullSecrets is a list of references to secrets in the same namespace to
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **secrets** ``Optional[List[ObjectReference]]`` - *(optional)* Secrets is a list of the secrets in the same namespace that pods running using
...
class ServiceAccountTokenProjection(DictMixin):
    """ServiceAccountTokenProjection represents a projected service account token
      ...
      * **path** ``str`` - path is the path relative to the mount point of the file to project the token
        into.
      * **audience** ``Optional[str]`` - *(optional)* audience is the intended audience of the token. A recipient of a token must
        ...
        the apiserver.
      * **expirationSeconds** ``Optional[int]`` - *(optional)* expirationSeconds is the requested duration of validity of the service account
...
class ServicePort(DictMixin):
    """ServicePort contains information on service's port.
      ...
      * **appProtocol** ``Optional[str]`` - *(optional)* The application protocol for this port. This field follows standard Kubernetes
        ...
        mycompany.com/my-custom-protocol.
      * **name** ``Optional[str]`` - *(optional)* The name of this port within the service. This must be a DNS_LABEL. All ports
      ...
      * **nodePort** ``Optional[int]`` - *(optional)* The port on each node on which this service is exposed when type is NodePort
      ...
      * **protocol** ``Optional[str]`` - *(optional)* The IP protocol for this port. Supports "TCP", "UDP", and "SCTP". Default is
        TCP.
      * **targetPort** ``Optional[util_intstr.IntOrString]`` - *(optional)* Number or name of the port to access on the pods targeted by the service.
...
class ServiceSpec(DictMixin):
    """ServiceSpec describes the attributes that a user creates on a service.
      ...
      * **allocateLoadBalancerNodePorts** ``Optional[bool]`` - *(optional)* allocateLoadBalancerNodePorts defines if NodePorts will be automatically
        ...
        any other type.
      * **clusterIP** ``Optional[str]`` - *(optional)* clusterIP is the IP address of the service and is usually assigned randomly.
      ...
      * **clusterIPs** ``Optional[List[str]]`` - *(optional)* ClusterIPs is a list of IP addresses assigned to this service, and are usually
        ...
        https://kubernetes.io/docs/concepts/services-networking/service/#virtual-ips-and-service-proxies
      * **externalIPs** ``Optional[List[str]]`` - *(optional)* externalIPs is a list of IP addresses for which nodes in the cluster will also
        ...
        Kubernetes system.
      * **externalName** ``Optional[str]`` - *(optional)* externalName is the external reference that discovery mechanisms will return
        ...
        "ExternalName".
      * **externalTrafficPolicy** ``Optional[str]`` - *(optional)* externalTrafficPolicy describes how nodes distribute service traffic they
      ...
      * **healthCheckNodePort** ``Optional[int]`` - *(optional)* healthCheckNodePort specifies the healthcheck nodePort for the service. This
      ...
      * **internalTrafficPolicy** ``Optional[str]`` - *(optional)* InternalTrafficPolicy describes how nodes distribute service traffic they
        receive on the ClusterIP. If set to "Local", the proxy will assume that pods
        only want to talk to endpoints of the service on the same node as the pod,
        dropping the traffic if there are no local endpoints. The default value,
        "Cluster", uses the standard behavior of routing to all endpoints evenly
        (possibly modified by topology and other features).
      * **ipFamilies** ``Optional[List[str]]`` - *(optional)* IPFamilies is a list of IP families (e.g. IPv4, IPv6) assigned to this
      ...
      * **ipFamilyPolicy** ``Optional[str]`` - *(optional)* IPFamilyPolicy represents the dual-stack-ness requested or required by this
      ...
      * **loadBalancerClass** ``Optional[str]`` - *(optional)* loadBalancerClass is the class of the load balancer implementation this
      ...
      * **loadBalancerIP** ``Optional[str]`` - *(optional)* Only applies to Service Type: LoadBalancer. This feature depends on whether
      ...
      * **loadBalancerSourceRanges** ``Optional[List[str]]`` - *(optional)* If specified and supported by the platform, this will restrict traffic through
      ...
      * **ports** ``Optional[List[ServicePort]]`` - *(optional)* The list of ports that are exposed by this service. More info:
        https://kubernetes.io/docs/concepts/services-networking/service/#virtual-ips-and-service-proxies
      * **publishNotReadyAddresses** ``Optional[bool]`` - *(optional)* publishNotReadyAddresses indicates that any agent which deals with endpoints
      ...
      * **selector** ``Optional[dict]`` - *(optional)* Route service traffic to pods with label keys and values matching this
      ...
      * **sessionAffinity** ``Optional[str]`` - *(optional)* Supports "ClientIP" and "None". Used to maintain session affinity. Enable
        client IP based session affinity. Must be ClientIP or None. Defaults to None.
        More info:
        https://kubernetes.io/docs/concepts/services-networking/service/#virtual-ips-and-service-proxies
      * **sessionAffinityConfig** ``Optional[SessionAffinityConfig]`` - *(optional)* sessionAffinityConfig contains the configurations of session affinity.
      * **type** ``Optional[str]`` - *(optional)* type determines how the Service is exposed. Defaults to ClusterIP. Valid
...
class ServiceStatus(DictMixin):
    """ServiceStatus represents the current status of a service.
      ...
      * **conditions** ``Optional[List[meta_v1.Condition]]`` - *(optional)* Current service state
      * **loadBalancer** ``Optional[LoadBalancerStatus]`` - *(optional)* LoadBalancer contains the current status of the load-balancer, if one is
        present.
    """
...
class SessionAffinityConfig(DictMixin):
    """SessionAffinityConfig represents the configurations of session affinity.
      ...
      * **clientIP** ``Optional[ClientIPConfig]`` - *(optional)* clientIP contains the configurations of Client IP based session affinity.
    """
...
class StorageOSPersistentVolumeSource(DictMixin):
    """Represents a StorageOS persistent volume resource.
      ...
      * **readOnly** ``Optional[bool]`` - *(optional)* readOnly defaults to false (read/write). ReadOnly here will force the ReadOnly
        setting in VolumeMounts.
      * **secretRef** ``Optional[ObjectReference]`` - *(optional)* secretRef specifies the secret to use for obtaining the StorageOS API
        credentials.  If not specified, default values will be attempted.
      * **volumeName** ``Optional[str]`` - *(optional)* volumeName is the human-readable name of the StorageOS volume.  Volume names
        are only unique within a namespace.
      * **volumeNamespace** ``Optional[str]`` - *(optional)* volumeNamespace specifies the scope of the volume within StorageOS.  If no
...
class StorageOSVolumeSource(DictMixin):
    """Represents a StorageOS persistent volume resource.
      ...
      * **readOnly** ``Optional[bool]`` - *(optional)* readOnly defaults to false (read/write). ReadOnly here will force the ReadOnly
        setting in VolumeMounts.
      * **secretRef** ``Optional[LocalObjectReference]`` - *(optional)* secretRef specifies the secret to use for obtaining the StorageOS API
        credentials.  If not specified, default values will be attempted.
      * **volumeName** ``Optional[str]`` - *(optional)* volumeName is the human-readable name of the StorageOS volume.  Volume names
        are only unique within a namespace.
      * **volumeNamespace** ``Optional[str]`` - *(optional)* volumeNamespace specifies the scope of the volume within StorageOS.  If no
...
class TCPSocketAction(DictMixin):
    """TCPSocketAction describes an action based on opening a socket
      ...
      * **port** ``util_intstr.IntOrString`` - Number or name of the port to access on the container. Number must be in the
        range 1 to 65535. Name must be an IANA_SVC_NAME.
      * **host** ``Optional[str]`` - *(optional)* Optional: Host name to connect to, defaults to the pod IP.
    """
    port: 'util_intstr.IntOrString'
    host: 'Optional[str]' = None
...
class Taint(DictMixin):
    """The node this Taint is attached to has the "effect" on any pod that does not
      ...
      * **timeAdded** ``Optional[meta_v1.Time]`` - *(optional)* TimeAdded represents the time at which the taint was added. It is only written
        for NoExecute taints.
      * **value** ``Optional[str]`` - *(optional)* The taint value corresponding to the taint key.
    """
    effect: 'str'
    key: 'str'
...
class Toleration(DictMixin):
    """The pod this Toleration is attached to tolerates any taint that matches the
      ...
      * **effect** ``Optional[str]`` - *(optional)* Effect indicates the taint effect to match. Empty means match all taint
        effects. When specified, allowed values are NoSchedule, PreferNoSchedule and
        NoExecute.
      * **key** ``Optional[str]`` - *(optional)* Key is the taint key that the toleration applies to. Empty means match all
        taint keys. If the key is empty, operator must be Exists; this combination
        means to match all values and all keys.
      * **operator** ``Optional[str]`` - *(optional)* Operator represents a key's relationship to the value. Valid operators are
        Exists and Equal. Defaults to Equal. Exists is equivalent to wildcard for
        value, so that a pod can tolerate all taints of a particular category.
      * **tolerationSeconds** ``Optional[int]`` - *(optional)* TolerationSeconds represents the period of time the toleration (which must be
        ...
        system.
      * **value** ``Optional[str]`` - *(optional)* Value is the taint value the toleration matches to. If the operator is Exists,
...
class TopologySelectorTerm(DictMixin):
    """A topology selector term represents the result of label queries. A null or
      ...
      * **matchLabelExpressions** ``Optional[List[TopologySelectorLabelRequirement]]`` - *(optional)* A list of topology selector requirements by labels.
    """
...
class TopologySpreadConstraint(DictMixin):
    """TopologySpreadConstraint specifies how to spread matching pods among the given
      ...
      * **whenUnsatisfiable** ``str`` - WhenUnsatisfiable indicates how to deal with a pod if it doesn't satisfy the
        ...
        field.
      * **labelSelector** ``Optional[meta_v1.LabelSelector]`` - *(optional)* LabelSelector is used to find matching pods. Pods that match this label
        selector are counted to determine the number of pods in their corresponding
        topology domain.
      * **matchLabelKeys** ``Optional[List[str]]`` - *(optional)* MatchLabelKeys is a set of pod label keys to select the pods over which
      ...
      * **minDomains** ``Optional[int]`` - *(optional)* MinDomains indicates a minimum number of eligible domains. When the number of
      ...
      * **nodeAffinityPolicy** ``Optional[str]`` - *(optional)* NodeAffinityPolicy indicates how we will treat Pod's nodeAffinity/nodeSelector
      ...
      * **nodeTaintsPolicy** ``Optional[str]`` - *(optional)* NodeTaintsPolicy indicates how we will treat node taints when calculating pod
...
class TypedLocalObjectReference(DictMixin):
    """TypedLocalObjectReference contains enough information to let you locate the
      ...
      * **apiGroup** ``Optional[str]`` - *(optional)* APIGroup is the group for the resource being referenced. If APIGroup is not
...
class TypedObjectReference(DictMixin):
    """
      ...
      * **apiGroup** ``Optional[str]`` - *(optional)* APIGroup is the group for the resource being referenced. If APIGroup is not
        specified, the specified Kind must be in the core API group. For any other
        third-party types, APIGroup is required.
      * **namespace** ``Optional[str]`` - *(optional)* Namespace is the namespace of resource being referenced Note that when a
...
class Volume(DictMixin):
    """Volume represents a named volume in a pod that may be accessed by any
      ...
      * **azureDisk** ``Optional[AzureDiskVolumeSource]`` - *(optional)* azureDisk represents an Azure Data Disk mount on the host and bind mount to
        the pod.
      * **azureFile** ``Optional[AzureFileVolumeSource]`` - *(optional)* azureFile represents an Azure File Service mount on the host and bind mount to
        the pod.
      * **cephfs** ``Optional[CephFSVolumeSource]`` - *(optional)* cephFS represents a Ceph FS mount on the host that shares a pod's lifetime
      * **cinder** ``Optional[CinderVolumeSource]`` - *(optional)* cinder represents a cinder volume attached and mounted on kubelets host
        machine. More info: https://examples.k8s.io/mysql-cinder-pd/README.md
      * **configMap** ``Optional[ConfigMapVolumeSource]`` - *(optional)* configMap represents a configMap that should populate this volume
      * **csi** ``Optional[CSIVolumeSource]`` - *(optional)* csi (Container Storage Interface) represents ephemeral storage that is handled
        by certain external CSI drivers (Beta feature).
      * **downwardAPI** ``Optional[DownwardAPIVolumeSource]`` - *(optional)* downwardAPI represents downward API about the pod that should populate this
        volume
      * **emptyDir** ``Optional[EmptyDirVolumeSource]`` - *(optional)* emptyDir represents a temporary directory that shares a pod's lifetime. More
        info: https://kubernetes.io/docs/concepts/storage/volumes#emptydir
      * **ephemeral** ``Optional[EphemeralVolumeSource]`` - *(optional)* ephemeral represents a volume that is handled by a cluster storage driver. The
      ...
      * **fc** ``Optional[FCVolumeSource]`` - *(optional)* fc represents a Fibre Channel resource that is attached to a kubelet's host
        machine and then exposed to the pod.
      * **flexVolume** ``Optional[FlexVolumeSource]`` - *(optional)* flexVolume represents a generic volume resource that is provisioned/attached
        using an exec based plugin.
      * **flocker** ``Optional[FlockerVolumeSource]`` - *(optional)* flocker represents a Flocker volume attached to a kubelet's host machine. This
        depends on the Flocker control service being running
      * **gcePersistentDisk** ``Optional[GCEPersistentDiskVolumeSource]`` - *(optional)* gcePersistentDisk represents a GCE Disk resource that is attached to a
        kubelet's host machine and then exposed to the pod. More info:
        https://kubernetes.io/docs/concepts/storage/volumes#gcepersistentdisk
      * **gitRepo** ``Optional[GitRepoVolumeSource]`` - *(optional)* gitRepo represents a git repository at a particular revision. DEPRECATED:
        GitRepo is deprecated. To provision a container with a git repo, mount an
        EmptyDir into an InitContainer that clones the repo using git, then mount the
        EmptyDir into the Pod's container.
      * **glusterfs** ``Optional[GlusterfsVolumeSource]`` - *(optional)* glusterfs represents a Glusterfs mount on the host that shares a pod's
        lifetime. More info: https://examples.k8s.io/volumes/glusterfs/README.md
      * **hostPath** ``Optional[HostPathVolumeSource]`` - *(optional)* hostPath represents a pre-existing file or directory on the host machine that
        ...
        https://kubernetes.io/docs/concepts/storage/volumes#hostpath
      * **iscsi** ``Optional[ISCSIVolumeSource]`` - *(optional)* iscsi represents an ISCSI Disk resource that is attached to a kubelet's host
        machine and then exposed to the pod. More info:
        https://examples.k8s.io/volumes/iscsi/README.md
      * **nfs** ``Optional[NFSVolumeSource]`` - *(optional)* nfs represents an NFS mount on the host that shares a pod's lifetime More
        info: https://kubernetes.io/docs/concepts/storage/volumes#nfs
      * **persistentVolumeClaim** ``Optional[PersistentVolumeClaimVolumeSource]`` - *(optional)* persistentVolumeClaimVolumeSource represents a reference to a
      ...
      * **portworxVolume** ``Optional[PortworxVolumeSource]`` - *(optional)* portworxVolume represents a portworx volume attached and mounted on kubelets
        host machine
      * **projected** ``Optional[ProjectedVolumeSource]`` - *(optional)* projected items for all in one resources secrets, configmaps, and downward API
      * **quobyte** ``Optional[QuobyteVolumeSource]`` - *(optional)* quobyte represents a Quobyte mount on the host that shares a pod's lifetime
      * **rbd** ``Optional[RBDVolumeSource]`` - *(optional)* rbd represents a Rados Block Device mount on the host that shares a pod's
        lifetime. More info: https://examples.k8s.io/volumes/rbd/README.md
      * **scaleIO** ``Optional[ScaleIOVolumeSource]`` - *(optional)* scaleIO represents a ScaleIO persistent volume attached and mounted on
        Kubernetes nodes.
      * **secret** ``Optional[SecretVolumeSource]`` - *(optional)* secret represents a secret that should populate this volume. More info:
        https://kubernetes.io/docs/concepts/storage/volumes#secret
      * **storageos** ``Optional[StorageOSVolumeSource]`` - *(optional)* storageOS represents a StorageOS volume attached and mounted on Kubernetes
        nodes.
      * **vsphereVolume** ``Optional[VsphereVirtualDiskVolumeSource]`` - *(optional)* vsphereVolume represents a vSphere volume attached and mounted on kubelets
        host machine
    """
...
class VolumeMount(DictMixin):
    """VolumeMount describes a mounting of a Volume within a container.
      ...
      * **mountPropagation** ``Optional[str]`` - *(optional)* mountPropagation determines how mounts are propagated from the host to
        container and the other way around. When not set, MountPropagationNone is
        used. This field is beta in 1.10.
      * **readOnly** ``Optional[bool]`` - *(optional)* Mounted read-only if true, read-write otherwise (false or unspecified).
        Defaults to false.
      * **subPath** ``Optional[str]`` - *(optional)* Path within the volume from which the container's volume should be mounted.
        Defaults to "" (volume's root).
      * **subPathExpr** ``Optional[str]`` - *(optional)* Expanded path within the volume from which the container's volume should be
...
class VolumeNodeAffinity(DictMixin):
    """VolumeNodeAffinity defines constraints that limit what nodes this volume can
      ...
      * **required** ``Optional[NodeSelector]`` - *(optional)* required specifies hard node constraints that must be met.
    """
...
class VolumeProjection(DictMixin):
    """Projection that may be projected along with other supported volume types
      ...
      * **configMap** ``Optional[ConfigMapProjection]`` - *(optional)* configMap information about the configMap data to project
      * **downwardAPI** ``Optional[DownwardAPIProjection]`` - *(optional)* downwardAPI information about the downwardAPI data to project
      * **secret** ``Optional[SecretProjection]`` - *(optional)* secret information about the secret data to project
      * **serviceAccountToken** ``Optional[ServiceAccountTokenProjection]`` - *(optional)* serviceAccountToken is information about the serviceAccountToken data to
        project
    """
...
class VsphereVirtualDiskVolumeSource(DictMixin):
    """Represents a vSphere volume resource.
      ...
      * **fsType** ``Optional[str]`` - *(optional)* fsType is filesystem type to mount. Must be a filesystem type supported by the
        host operating system. Ex. "ext4", "xfs", "ntfs". Implicitly inferred to be
        "ext4" if unspecified.
      * **storagePolicyID** ``Optional[str]`` - *(optional)* storagePolicyID is the storage Policy Based Management (SPBM) profile ID
        associated with the StoragePolicyName.
      * **storagePolicyName** ``Optional[str]`` - *(optional)* storagePolicyName is the storage Policy Based Management (SPBM) profile name.
    """
    volumePath: 'str'
    fsType: 'Optional[str]' = None
...
class WindowsSecurityContextOptions(DictMixin):
    """WindowsSecurityContextOptions contain Windows-specific options and
      ...
      * **gmsaCredentialSpec** ``Optional[str]`` - *(optional)* GMSACredentialSpec is where the GMSA admission webhook
        (https://github.com/kubernetes-sigs/windows-gmsa) inlines the contents of the
        GMSA credential spec named by the GMSACredentialSpecName field.
      * **gmsaCredentialSpecName** ``Optional[str]`` - *(optional)* GMSACredentialSpecName is the name of the GMSA credential spec to use.
      * **hostProcess** ``Optional[bool]`` - *(optional)* HostProcess determines if a container should be run as a 'Host Process'
        ...
        be set to true.
      * **runAsUserName** ``Optional[str]`` - *(optional)* The UserName in Windows to run the entrypoint of the container process.
