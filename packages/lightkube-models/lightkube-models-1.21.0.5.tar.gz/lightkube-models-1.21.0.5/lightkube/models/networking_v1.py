...
class HTTPIngressPath(DictMixin):
    """HTTPIngressPath associates a path with a backend. Incoming urls matching the
      ...
      * **pathType** ``Optional[str]`` - *(optional)* PathType determines the interpretation of the Path matching. PathType can be
...
class IngressClassParametersReference(DictMixin):
    """IngressClassParametersReference identifies an API object. This can be used to
      ...
      * **scope** ``Optional[str]`` - *(optional)* Scope represents if this refers to a cluster or namespace scoped resource.
        This may be set to "Cluster" (default) or "Namespace". Field can be enabled
        with IngressClassNamespacedParams feature gate.
    """
    kind: 'str'
    name: 'str'
...
class IngressSpec(DictMixin):
    """IngressSpec describes the Ingress the user wishes to exist.
      ...
      * **ingressClassName** ``Optional[str]`` - *(optional)* IngressClassName is the name of the IngressClass cluster resource. The
...
class NetworkPolicyPort(DictMixin):
    """NetworkPolicyPort describes a port to allow traffic on
      ...
      * **endPort** ``Optional[int]`` - *(optional)* If set, indicates that the range of ports from port to endPort, inclusive,
        ...
        The endPort must be equal or greater than port. This feature is in Alpha state
