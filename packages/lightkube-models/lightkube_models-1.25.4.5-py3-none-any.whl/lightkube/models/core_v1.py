...
class PersistentVolumeClaimSpec(DictMixin):
    """PersistentVolumeClaimSpec describes the common attributes of storage devices
      ...
      * **dataSource** ``Optional[TypedLocalObjectReference]`` - *(optional)* dataSource field can be used to specify either: * An existing VolumeSnapshot
        ...
        field.
      * **dataSourceRef** ``Optional[TypedLocalObjectReference]`` - *(optional)* dataSourceRef specifies the object from which to populate the volume with
        ...
        backwards compatibility, both fields (DataSource and DataSourceRef) will be
        set to the same value automatically if one of them is empty and the other is
        non-empty. There are two important differences between DataSource and
        DataSourceRef: * While DataSource only allows two specific types of objects,
        DataSourceRef
...
class PodSecurityContext(DictMixin):
    """PodSecurityContext holds pod-level security attributes and common container
      ...
      * **supplementalGroups** ``Optional[List[int]]`` - *(optional)* A list of groups applied to the first process run in each container, in
        ...
        added to any container. Note that this field cannot be set when spec.os.name
        is windows.
