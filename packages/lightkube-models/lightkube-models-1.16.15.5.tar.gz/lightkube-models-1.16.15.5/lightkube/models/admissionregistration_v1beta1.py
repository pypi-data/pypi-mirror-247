...
class MutatingWebhook(DictMixin):
    """MutatingWebhook describes an admission webhook and the resources and
      ...
      * **admissionReviewVersions** ``Optional[List[str]]`` - *(optional)* AdmissionReviewVersions is an ordered list of preferred `AdmissionReview`
      ...
      * **failurePolicy** ``Optional[str]`` - *(optional)* FailurePolicy defines how unrecognized errors from the admission endpoint are
        handled - allowed values are Ignore or Fail. Defaults to Ignore.
      * **matchPolicy** ``Optional[str]`` - *(optional)* matchPolicy defines how the "rules" list is used to match incoming requests.
        ...
        Defaults to "Exact"
      * **namespaceSelector** ``Optional[meta_v1.LabelSelector]`` - *(optional)* NamespaceSelector decides whether to run the webhook on an object based on
      ...
      * **objectSelector** ``Optional[meta_v1.LabelSelector]`` - *(optional)* ObjectSelector decides whether to run the webhook based on if the object has
        ...
        Default to the empty LabelSelector, which matches everything.
      * **reinvocationPolicy** ``Optional[str]`` - *(optional)* reinvocationPolicy indicates whether this webhook should be called multiple
        ...
        * to validate an object after all mutations are guaranteed complete, use a
        ...
        Defaults to "Never".
      * **rules** ``Optional[List[RuleWithOperations]]`` - *(optional)* Rules describes what operations on what resources/subresources the webhook
      ...
      * **sideEffects** ``Optional[str]`` - *(optional)* SideEffects states whether this webhookk has side effects. Acceptable values
      ...
      * **timeoutSeconds** ``Optional[int]`` - *(optional)* TimeoutSeconds specifies the timeout for this webhook. After the timeout
...
class MutatingWebhookConfiguration(DictMixin):
    """MutatingWebhookConfiguration describes the configuration of and admission
      ...
      * **webhooks** ``Optional[List[MutatingWebhook]]`` - *(optional)* Webhooks is a list of webhooks and the affected resources and operations.
    """
...
class RuleWithOperations(DictMixin):
    """RuleWithOperations is a tuple of Operations and Resources. It is recommended
      ...
      * **operations** ``Optional[List[str]]`` - *(optional)* Operations is the operations the admission hook cares about - CREATE, UPDATE,
        or * for all operations. If '*' is present, the length of the slice must be
        one. Required.
...
class ValidatingWebhookConfiguration(DictMixin):
    """ValidatingWebhookConfiguration describes the configuration of and admission
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object metadata; More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata.
      * **webhooks** ``Optional[List[ValidatingWebhook]]`` - *(optional)* Webhooks is a list of webhooks and the affected resources and operations.
    """
    apiVersion: 'Optional[str]' = None
    kind: 'Optional[str]' = None
    metadata: 'Optional[meta_v1.ObjectMeta]' = None
