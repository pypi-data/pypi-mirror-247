...
class Initializers(DictMixin):
    """Initializers tracks the progress of initialization.
      ...
      * **result** ``Optional[Status]`` - If result is set with the Failure field, the object will be persisted to
...
class ListMeta(DictMixin):
    """ListMeta describes metadata that synthetic resources must have, including
      ...
      * **selfLink** ``Optional[str]`` - selfLink is a URL representing this object. Populated by the system.
        Read-only.
    """
...
class ManagedFieldsEntry(DictMixin):
    """ManagedFieldsEntry is a workflow-id, a FieldSet and the group version of the
      ...
      * **fields** ``Optional[Fields]`` - Fields identifies a set of fields.
      ...
      * **time** ``Optional[Time]`` - Time is timestamp of when these fields were set. It should always be empty if
        Operation is 'Apply'
    """
    apiVersion: 'Optional[str]' = None
...
class ObjectMeta(DictMixin):
    """ObjectMeta is metadata that all persisted resources must have, which includes
      ...
      * **clusterName** ``Optional[str]`` - The name of the cluster which the object belongs to. This is used to
      ...
      * **initializers** ``Optional[Initializers]`` - An initializer is a controller which enforces some system invariant at object
      ...
      * **namespace** ``Optional[str]`` - Namespace defines the space within each name must be unique. An empty
