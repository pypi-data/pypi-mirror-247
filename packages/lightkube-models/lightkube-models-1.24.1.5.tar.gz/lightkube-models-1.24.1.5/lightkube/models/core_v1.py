...
class Container(DictMixin):
    """A single application container that you want to run within a pod.
      ...
      * **ports** ``Optional[List[ContainerPort]]`` - *(optional)* List of ports to expose from the container. Exposing a port here gives the
...
class ContainerImage(DictMixin):
    """Describe a container image
      ...
      * **names** ``Optional[List[str]]`` - *(optional)* Names by which this image is known. e.g. ["k8s.gcr.io/hyperkube:v1.0.7",
...
class PodSpec(DictMixin):
    """PodSpec is a description of a pod.
      ...
      * **ephemeralContainers** ``Optional[List[EphemeralContainer]]`` - *(optional)* List of ephemeral containers run in this pod. Ephemeral containers may be run
        ...
        pod, use the pod's ephemeralcontainers subresource. This field is beta-level
...
class PodStatus(DictMixin):
    """PodStatus represents information about the status of a pod. Status may trail
      ...
      * **ephemeralContainerStatuses** ``Optional[List[ContainerStatus]]`` - *(optional)* Status for any ephemeral containers that have run in this pod. This field is
...
class ServiceSpec(DictMixin):
    """ServiceSpec describes the attributes that a user creates on a service.
      ...
      * **externalName** ``Optional[str]`` - *(optional)* externalName is the external reference that discovery mechanisms will return
        ...
        "ExternalName".
      * **externalTrafficPolicy** ``Optional[str]`` - *(optional)* externalTrafficPolicy denotes if this Service desires to route external
      ...
      * **internalTrafficPolicy** ``Optional[str]`` - *(optional)* InternalTrafficPolicy specifies if the cluster internal traffic should be
...
class TopologySpreadConstraint(DictMixin):
    """TopologySpreadConstraint specifies how to spread matching pods among the given
      ...
      * **topologyKey** ``str`` - TopologyKey is the key of node labels. Nodes that have a label with this key
        ...
        we define an eligible domain as a domain whose nodes match the node selector.
        e.g. If TopologyKey is "kubernetes.io/hostname", each Node is a domain of that
        topology. And, if TopologyKey is "topology.kubernetes.io/zone", each zone is a
      ...
      * **minDomains** ``Optional[int]`` - *(optional)* MinDomains indicates a minimum number of eligible domains. When the number of
        ...
        This is an alpha field and requires enabling MinDomainsInPodTopologySpread
        feature gate.
    """
    maxSkew: 'int'
    topologyKey: 'str'
