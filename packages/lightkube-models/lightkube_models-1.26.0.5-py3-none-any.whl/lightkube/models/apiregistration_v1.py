...
class APIService(DictMixin):
    """APIService represents a server for a particular GroupVersion. Name must be
      ...
      * **spec** ``Optional[APIServiceSpec]`` - *(optional)* Spec contains information for locating and communicating with a server
      * **status** ``Optional[APIServiceStatus]`` - *(optional)* Status contains derived information about an API server
    """
...
class APIServiceSpec(DictMixin):
    """APIServiceSpec contains information for locating and communicating with a
      ...
      * **caBundle** ``Optional[str]`` - *(optional)* CABundle is a PEM encoded CA bundle which will be used to validate an API
        server's serving certificate. If unspecified, system trust roots on the
        apiserver are used.
      * **group** ``Optional[str]`` - *(optional)* Group is the API group name this server hosts
      * **insecureSkipTLSVerify** ``Optional[bool]`` - *(optional)* InsecureSkipTLSVerify disables TLS certificate verification when communicating
        with this server. This is strongly discouraged.  You should use the CABundle
        instead.
      * **service** ``Optional[ServiceReference]`` - *(optional)* Service is a reference to the service for this API server.  It must
      ...
      * **version** ``Optional[str]`` - *(optional)* Version is the API version this server hosts.  For example, "v1"
    """
...
class APIServiceStatus(DictMixin):
    """APIServiceStatus contains derived information about an API server
      ...
      * **conditions** ``Optional[List[APIServiceCondition]]`` - *(optional)* Current service state of apiService.
    """
...
class ServiceReference(DictMixin):
    """ServiceReference holds a reference to Service.legacy.k8s.io
      ...
      * **name** ``Optional[str]`` - *(optional)* Name is the name of the service
      * **namespace** ``Optional[str]`` - *(optional)* Namespace is the namespace of the service
