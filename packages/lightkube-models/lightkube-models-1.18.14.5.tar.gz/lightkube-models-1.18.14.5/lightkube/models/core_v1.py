...
class ConfigMap(DictMixin):
    """ConfigMap holds configuration data for pods to consume.
      ...
      * **immutable** ``Optional[bool]`` - *(optional)* Immutable, if set to true, ensures that data stored in the ConfigMap cannot be
        ...
        can be modified at any time. Defaulted to nil. This is an alpha field enabled
...
class ConfigMapVolumeSource(DictMixin):
    """Adapts a ConfigMap into a volume.
      ...
      * **defaultMode** ``Optional[int]`` - *(optional)* Optional: mode bits to use on created files by default. Must be a value
...
class DownwardAPIVolumeFile(DictMixin):
    """DownwardAPIVolumeFile represents information to create the file containing the
      ...
      * **mode** ``Optional[int]`` - *(optional)* Optional: mode bits to use on this file, must be a value between 0 and 0777.
...
class EndpointPort(DictMixin):
    """EndpointPort is a tuple that describes a single port.
      ...
      * **appProtocol** ``Optional[str]`` - *(optional)* The application protocol for this port. This field follows standard Kubernetes
        ...
        mycompany.com/my-custom-protocol. Field can be enabled with ServiceAppProtocol
        feature gate.
...
class EventSeries(DictMixin):
    """EventSeries contain information on series of events, i.e. thing that was/is
      ...
      * **state** ``Optional[str]`` - *(optional)* State of this Series: Ongoing or Finished Deprecated. Planned removal for 1.18
    """
    count: 'Optional[int]' = None
...
class ProjectedVolumeSource(DictMixin):
    """Represents a projected volume source
      ...
      * **defaultMode** ``Optional[int]`` - *(optional)* Mode bits to use on created files by default. Must be a value between 0 and
...
class ServiceSpec(DictMixin):
    """ServiceSpec describes the attributes that a user creates on a service.
      ...
      * **ports** ``Optional[List[ServicePort]]`` - *(optional)* The list of ports that are exposed by this service. More info:
        https://kubernetes.io/docs/concepts/services-networking/service/#virtual-ips-and-service-proxies
      * **publishNotReadyAddresses** ``Optional[bool]`` - *(optional)* publishNotReadyAddresses, when set to true, indicates that DNS implementations
...
class Volume(DictMixin):
    """Volume represents a named volume in a pod that may be accessed by any
      ...
      * **csi** ``Optional[CSIVolumeSource]`` - *(optional)* CSI (Container Storage Interface) represents storage that is handled by an
