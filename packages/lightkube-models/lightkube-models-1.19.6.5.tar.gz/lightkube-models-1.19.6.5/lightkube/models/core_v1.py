...
class Container(DictMixin):
    """A single application container that you want to run within a pod.
      ...
      * **startupProbe** ``Optional[Probe]`` - *(optional)* StartupProbe indicates that the Pod has successfully initialized. If
        ...
        is a beta feature enabled by the StartupProbe feature flag. More info:
        https://kubernetes.io/docs/concepts/workloads/pods/pod-lifecycle#container-probes
...
class PersistentVolumeClaimSpec(DictMixin):
    """PersistentVolumeClaimSpec describes the common attributes of storage devices
      ...
      * **dataSource** ``Optional[TypedLocalObjectReference]`` - *(optional)* This field can be used to specify either: * An existing VolumeSnapshot object
        ...
        the specified data source, it will create a new volume based on the contents
        of the specified data source. If the specified data source is not supported,
        the volume will not be created and the failure will be reported as an event.
        In the future, we plan to support more data source types and the behavior of
...
class PodAffinityTerm(DictMixin):
    """Defines a set of pods (namely those matching the labelSelector relative to the
      ...
      * **namespaces** ``Optional[List[str]]`` - *(optional)* namespaces specifies which namespaces the labelSelector applies to (matches
...
class ServicePort(DictMixin):
    """ServicePort contains information on service's port.
      ...
      * **nodePort** ``Optional[int]`` - *(optional)* The port on each node on which this service is exposed when type=NodePort or
...
class ServiceSpec(DictMixin):
    """ServiceSpec describes the attributes that a user creates on a service.
      ...
      * **clusterIP** ``Optional[str]`` - *(optional)* clusterIP is the IP address of the service and is usually assigned randomly by
      ...
      * **externalIPs** ``Optional[List[str]]`` - *(optional)* externalIPs is a list of IP addresses for which nodes in the cluster will also
        ...
        Kubernetes system.
      * **externalName** ``Optional[str]`` - *(optional)* externalName is the external reference that kubedns or equivalent will return
      ...
      * **healthCheckNodePort** ``Optional[int]`` - *(optional)* healthCheckNodePort specifies the healthcheck nodePort for the service. If not
      ...
      * **ipFamily** ``Optional[str]`` - *(optional)* ipFamily specifies whether this Service has a preference for a particular IP
        family (e.g. IPv4 vs. IPv6) when the IPv6DualStack feature gate is enabled. In
        a dual-stack cluster, you can specify ipFamily when creating a ClusterIP
        Service to determine whether the controller will allocate an IPv4 or IPv6 IP
        for it, and you can specify ipFamily when creating a headless Service to
        determine whether it will have IPv4 or IPv6 Endpoints. In either case, if you
        do not specify an ipFamily explicitly, it will default to the cluster's
        primary IP family. This field is part of an alpha feature, and you should not
        make any assumptions about its semantics other than those described above. In
        particular, you should not assume that it can (or cannot) be changed after
        creation time; that it can only have the values "IPv4" and "IPv6"; or that its
        current value on a given Service correctly reflects the current state of that
        Service. (For ClusterIP Services, look at clusterIP to see if the Service is
        IPv4 or IPv6. For headless Services, look at the endpoints, which may be
        dual-stack in the future. For ExternalName Services, ipFamily has no meaning,
        but it may be set to an irrelevant value anyway.)
...
class Volume(DictMixin):
    """Volume represents a named volume in a pod that may be accessed by any
      ...
      * **ephemeral** ``Optional[EphemeralVolumeSource]`` - *(optional)* Ephemeral represents a volume that is handled by a cluster storage driver
