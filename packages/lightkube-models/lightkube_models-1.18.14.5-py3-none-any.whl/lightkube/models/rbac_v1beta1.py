...
class PolicyRule(DictMixin):
    """PolicyRule holds information that describes a policy rule, but does not
      ...
      * **resources** ``Optional[List[str]]`` - *(optional)* Resources is a list of resources this rule applies to.  '*' represents all
...
class Subject(DictMixin):
    """Subject contains a reference to the object or user identities a role binding
      ...
      * **apiGroup** ``Optional[str]`` - *(optional)* APIGroup holds the API group of the referenced subject. Defaults to "" for
