...
class AuditSink(DictMixin):
    """AuditSink represents a cluster level audit sink
      ...
      * **spec** ``Optional[AuditSinkSpec]`` - *(optional)* Spec defines the audit configuration spec
    """
...
class Policy(DictMixin):
    """Policy defines the configuration of how audit events are logged
      ...
      * **stages** ``Optional[List[str]]`` - *(optional)* Stages is a list of stages for which events are created.
    """
    level: 'str'
...
class Webhook(DictMixin):
    """Webhook holds the configuration of the webhook
      ...
      * **throttle** ``Optional[WebhookThrottleConfig]`` - *(optional)* Throttle holds the options for throttling the webhook
    """
...
class WebhookThrottleConfig(DictMixin):
    """WebhookThrottleConfig holds the configuration for throttling events
      ...
      * **burst** ``Optional[int]`` - *(optional)* ThrottleBurst is the maximum number of events sent at the same moment default
        15 QPS
      * **qps** ``Optional[int]`` - *(optional)* ThrottleQPS maximum number of batches per second default 10 QPS
    """
