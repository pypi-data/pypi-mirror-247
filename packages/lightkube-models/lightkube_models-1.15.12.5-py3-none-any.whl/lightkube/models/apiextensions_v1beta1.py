...
class CustomResourceDefinitionNames(DictMixin):
    """CustomResourceDefinitionNames indicates the names to serve this
      ...
      * **plural** ``str`` - Plural is the plural name of the resource to serve.  It must match the name of
        ...
        lowercase.
      * **categories** ``Optional[List[str]]`` - *(optional)* Categories is a list of grouped resources custom resources belong to (e.g.
        'all')
      * **listKind** ``Optional[str]`` - *(optional)* ListKind is the serialized kind of the list for this resource.  Defaults to
        <kind>List.
      * **shortNames** ``Optional[List[str]]`` - *(optional)* ShortNames are short names for the resource.  It must be all lowercase.
...
class CustomResourceDefinitionSpec(DictMixin):
    """CustomResourceDefinitionSpec describes how a user wants their resource to
      ...
      * **scope** ``str`` - Scope indicates whether this resource is cluster or namespace scoped.  Default
        is namespaced
      * **additionalPrinterColumns** ``Optional[List[CustomResourceColumnDefinition]]`` - *(optional)* AdditionalPrinterColumns are additional columns shown e.g. in kubectl next to
      ...
      * **preserveUnknownFields** ``Optional[bool]`` - *(optional)* preserveUnknownFields disables pruning of object fields which are not
      ...
      * **subresources** ``Optional[CustomResourceSubresources]`` - *(optional)* Subresources describes the subresources for CustomResource Optional, the
        global subresources for all versions. Top-level and per-version subresources
        are mutually exclusive.
      * **validation** ``Optional[CustomResourceValidation]`` - *(optional)* Validation describes the validation methods for CustomResources Optional, the
        global validation schema for all versions. Top-level and per-version schemas
        are mutually exclusive.
      * **version** ``Optional[str]`` - *(optional)* Version is the version this resource belongs in Should be always first item in
        Versions field if provided. Optional, but at least one of Version or Versions
        must be set. Deprecated: Please use `Versions`.
      * **versions** ``Optional[List[CustomResourceDefinitionVersion]]`` - *(optional)* Versions is the list of all supported versions for this resource. If Version
...
class CustomResourceDefinitionVersion(DictMixin):
    """CustomResourceDefinitionVersion describes a version for CRD.
      ...
      * **schema** ``Optional[CustomResourceValidation]`` - *(optional)* Schema describes the schema for CustomResource used in validation, pruning,
        ...
        feature.
      * **subresources** ``Optional[CustomResourceSubresources]`` - *(optional)* Subresources describes the subresources for CustomResource Top-level and
...
class CustomResourceSubresourceScale(DictMixin):
    """CustomResourceSubresourceScale defines how to serve the scale subresource for
      ...
      * **labelSelectorPath** ``Optional[str]`` - *(optional)* LabelSelectorPath defines the JSON path inside of a CustomResource that
...
class CustomResourceSubresources(DictMixin):
    """CustomResourceSubresources defines the status and scale subresources for
      ...
      * **scale** ``Optional[CustomResourceSubresourceScale]`` - *(optional)* Scale denotes the scale subresource for CustomResources
      * **status** ``Optional[CustomResourceSubresourceStatus]`` - *(optional)* Status denotes the status subresource for CustomResources
    """
...
class CustomResourceValidation(DictMixin):
    """CustomResourceValidation is a list of validation methods for CustomResources.
      ...
      * **openAPIV3Schema** ``Optional[JSONSchemaProps]`` - *(optional)* OpenAPIV3Schema is the OpenAPI v3 schema to be validated against.
    """
...
class JSONSchemaProps(DictMixin):
    """JSONSchemaProps is a JSON-Schema following Specification Draft 4
      ...
      * **default** ``Optional[JSON]`` - *(optional)* default is a default value for undefined object fields. Defaulting is an alpha
...
class ServiceReference(DictMixin):
    """ServiceReference holds a reference to Service.legacy.k8s.io
      ...
      * **path** ``Optional[str]`` - *(optional)* `path` is an optional URL path which will be sent in any request to this
        service.
      * **port** ``Optional[int]`` - *(optional)* If specified, the port on the service that hosting webhook. Default to 443 for
...
class WebhookClientConfig(DictMixin):
    """WebhookClientConfig contains the information to make a TLS connection with the
      ...
      * **caBundle** ``Optional[str]`` - *(optional)* `caBundle` is a PEM encoded CA bundle which will be used to validate the
        ...
        apiserver are used.
      * **service** ``Optional[ServiceReference]`` - *(optional)* `service` is a reference to the service for this webhook. Either `service` or
