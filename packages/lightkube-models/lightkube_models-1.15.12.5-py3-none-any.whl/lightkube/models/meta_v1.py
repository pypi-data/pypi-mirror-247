...
class Initializers(DictMixin):
    """Initializers tracks the progress of initialization.
      ...
      * **result** ``Optional[Status]`` - *(optional)* If result is set with the Failure field, the object will be persisted to
...
class ManagedFieldsEntry(DictMixin):
    """ManagedFieldsEntry is a workflow-id, a FieldSet and the group version of the
      ...
      * **fields** ``Optional[Fields]`` - *(optional)* Fields identifies a set of fields.
...
class ObjectMeta(DictMixin):
    """ObjectMeta is metadata that all persisted resources must have, which includes
      ...
      * **initializers** ``Optional[Initializers]`` - *(optional)* An initializer is a controller which enforces some system invariant at object
      ...
      * **namespace** ``Optional[str]`` - *(optional)* Namespace defines the space within each name must be unique. An empty
