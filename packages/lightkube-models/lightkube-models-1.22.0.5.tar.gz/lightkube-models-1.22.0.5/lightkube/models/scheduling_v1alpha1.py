...
class PriorityClass(DictMixin):
    """DEPRECATED - This group version of PriorityClass is deprecated by
      ...
      * **description** ``Optional[str]`` - *(optional)* description is an arbitrary string that usually provides guidelines on when
        this priority class should be used.
      * **globalDefault** ``Optional[bool]`` - *(optional)* globalDefault specifies whether this PriorityClass should be considered as the
