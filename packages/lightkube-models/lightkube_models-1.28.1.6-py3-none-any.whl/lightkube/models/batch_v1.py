...
class CronJobSpec(DictMixin):
    """CronJobSpec describes how the job execution will look like and when it will
      ...
      * **timeZone** ``Optional[str]`` - The time zone name for the given schedule, see
...
class JobSpec(DictMixin):
    """JobSpec describes how the job execution will look like.
      ...
      * **maxFailedIndexes** ``Optional[int]`` - Specifies the maximal number of failed indexes before marking the Job as
      ...
      * **parallelism** ``Optional[int]`` - Specifies the maximum desired number of pods the job should run at any given
        ...
        https://kubernetes.io/docs/concepts/workloads/controllers/jobs-run-to-completion/
      * **podFailurePolicy** ``Optional[PodFailurePolicy]`` - Specifies the policy of handling failed pods. In particular, it allows to
      ...
      * **podReplacementPolicy** ``Optional[str]`` - podReplacementPolicy specifies when to create replacement Pods. Possible
...
class PodFailurePolicyOnExitCodesRequirement(DictMixin):
    """PodFailurePolicyOnExitCodesRequirement describes the requirement for handling
      ...
      * **containerName** ``Optional[str]`` - Restricts the check for exit codes to the container with the specified name.
...
class PodFailurePolicyRule(DictMixin):
    """PodFailurePolicyRule describes how a pod failure is handled when the
      ...
      * **onPodConditions** ``List[PodFailurePolicyOnPodConditionsPattern]`` - Represents the requirement on the pod conditions. The requirement is
        ...
        are allowed.
      * **onExitCodes** ``Optional[PodFailurePolicyOnExitCodesRequirement]`` - Represents the requirement on the container exit codes.
    """
    action: 'str'
