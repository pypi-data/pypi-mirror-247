...
class AWSElasticBlockStoreVolumeSource(DictMixin):
    """Represents a Persistent Disk resource in AWS.
      ...
      * **fsType** ``Optional[str]`` - *(optional)* Filesystem type of the volume that you want to mount. Tip: Ensure that the
      ...
      * **partition** ``Optional[int]`` - *(optional)* The partition in the volume that you want to mount. If omitted, the default is
      ...
      * **readOnly** ``Optional[bool]`` - *(optional)* Specify "true" to force and set the ReadOnly property in VolumeMounts to
...
class AzureDiskVolumeSource(DictMixin):
    """AzureDisk represents an Azure Data Disk mount on the host and bind mount to
      ...
      * **cachingMode** ``Optional[str]`` - *(optional)* Host Caching mode: None, Read Only, Read Write.
      * **fsType** ``Optional[str]`` - *(optional)* Filesystem type to mount. Must be a filesystem type supported by the host
        operating system. Ex. "ext4", "xfs", "ntfs". Implicitly inferred to be "ext4"
        if unspecified.
      * **kind** ``Optional[str]`` - *(optional)* Expected values Shared: multiple blob disks per storage account  Dedicated:
        single blob disk per storage account  Managed: azure managed data disk (only
        in managed availability set). defaults to shared
      * **readOnly** ``Optional[bool]`` - *(optional)* Defaults to false (read/write). ReadOnly here will force the ReadOnly setting
        in VolumeMounts.
    """
    diskName: 'str'
    diskURI: 'str'
...
class AzureFilePersistentVolumeSource(DictMixin):
    """AzureFile represents an Azure File Service mount on the host and bind mount to
      ...
      * **readOnly** ``Optional[bool]`` - *(optional)* Defaults to false (read/write). ReadOnly here will force the ReadOnly setting
        in VolumeMounts.
      * **secretNamespace** ``Optional[str]`` - *(optional)* the namespace of the secret that contains Azure Storage Account Name and Key
...
class CSIPersistentVolumeSource(DictMixin):
    """Represents storage that is managed by an external CSI volume driver (Beta
      ...
      * **readOnly** ``Optional[bool]`` - *(optional)* Optional: The value to pass to ControllerPublishVolumeRequest. Defaults to
        false (read/write).
      * **volumeAttributes** ``Optional[dict]`` - *(optional)* Attributes of the volume to publish.
    """
    driver: 'str'
    volumeHandle: 'str'
...
class CSIVolumeSource(DictMixin):
    """Represents a source location of a volume to mount, managed by an external CSI
      ...
      * **fsType** ``Optional[str]`` - *(optional)* Filesystem type to mount. Ex. "ext4", "xfs", "ntfs". If not provided, the
      ...
      * **readOnly** ``Optional[bool]`` - *(optional)* Specifies a read-only configuration for the volume. Defaults to false
        (read/write).
...
class CephFSPersistentVolumeSource(DictMixin):
    """Represents a Ceph Filesystem mount that lasts the lifetime of a pod Cephfs
      ...
      * **path** ``Optional[str]`` - *(optional)* Optional: Used as the mounted root, rather than the full Ceph tree, default is
        /
      * **readOnly** ``Optional[bool]`` - *(optional)* Optional: Defaults to false (read/write). ReadOnly here will force the
        ReadOnly setting in VolumeMounts. More info:
        https://examples.k8s.io/volumes/cephfs/README.md#how-to-use-it
      * **secretFile** ``Optional[str]`` - *(optional)* Optional: SecretFile is the path to key ring for User, default is
        /etc/ceph/user.secret More info:
        https://examples.k8s.io/volumes/cephfs/README.md#how-to-use-it
      * **secretRef** ``Optional[SecretReference]`` - *(optional)* Optional: SecretRef is reference to the authentication secret for User,
        default is empty. More info:
        https://examples.k8s.io/volumes/cephfs/README.md#how-to-use-it
      * **user** ``Optional[str]`` - *(optional)* Optional: User is the rados user name, default is admin More info:
        https://examples.k8s.io/volumes/cephfs/README.md#how-to-use-it
    """
    monitors: 'List[str]'
...
class CephFSVolumeSource(DictMixin):
    """Represents a Ceph Filesystem mount that lasts the lifetime of a pod Cephfs
      ...
      * **secretFile** ``Optional[str]`` - *(optional)* Optional: SecretFile is the path to key ring for User, default is
        /etc/ceph/user.secret More info:
        https://examples.k8s.io/volumes/cephfs/README.md#how-to-use-it
      * **secretRef** ``Optional[LocalObjectReference]`` - *(optional)* Optional: SecretRef is reference to the authentication secret for User,
        default is empty. More info:
        https://examples.k8s.io/volumes/cephfs/README.md#how-to-use-it
      * **user** ``Optional[str]`` - *(optional)* Optional: User is the rados user name, default is admin More info:
        https://examples.k8s.io/volumes/cephfs/README.md#how-to-use-it
...
class CinderPersistentVolumeSource(DictMixin):
    """Represents a cinder volume resource in Openstack. A Cinder volume must exist
      ...
      * **readOnly** ``Optional[bool]`` - *(optional)* Optional: Defaults to false (read/write). ReadOnly here will force the
        ReadOnly setting in VolumeMounts. More info:
        https://examples.k8s.io/mysql-cinder-pd/README.md
      * **secretRef** ``Optional[SecretReference]`` - *(optional)* Optional: points to a secret object containing parameters used to connect to
        OpenStack.
    """
    volumeID: 'str'
    fsType: 'Optional[str]' = None
...
class CinderVolumeSource(DictMixin):
    """Represents a cinder volume resource in Openstack. A Cinder volume must exist
      ...
      * **readOnly** ``Optional[bool]`` - *(optional)* Optional: Defaults to false (read/write). ReadOnly here will force the
        ReadOnly setting in VolumeMounts. More info:
        https://examples.k8s.io/mysql-cinder-pd/README.md
      * **secretRef** ``Optional[LocalObjectReference]`` - *(optional)* Optional: points to a secret object containing parameters used to connect to
        OpenStack.
    """
    volumeID: 'str'
    fsType: 'Optional[str]' = None
...
class ConfigMapProjection(DictMixin):
    """Adapts a ConfigMap into a projected volume.
      ...
      * **items** ``Optional[List[KeyToPath]]`` - *(optional)* If unspecified, each key-value pair in the Data field of the referenced
...
class ConfigMapVolumeSource(DictMixin):
    """Adapts a ConfigMap into a volume.
      ...
      * **defaultMode** ``Optional[int]`` - *(optional)* Optional: mode bits used to set permissions on created files by default. Must
...
class Container(DictMixin):
    """A single application container that you want to run within a pod.
      ...
      * **args** ``Optional[List[str]]`` - *(optional)* Arguments to the entrypoint. The docker image's CMD is used if this is not
      ...
      * **command** ``Optional[List[str]]`` - *(optional)* Entrypoint array. Not executed within a shell. The docker image's ENTRYPOINT
      ...
      * **image** ``Optional[str]`` - *(optional)* Docker image name. More info:
...
class ContainerStateTerminated(DictMixin):
    """ContainerStateTerminated is a terminated state of a container.
      ...
      * **containerID** ``Optional[str]`` - *(optional)* Container's ID in the format 'docker://<container_id>'
...
class EmptyDirVolumeSource(DictMixin):
    """Represents an empty directory for a pod. Empty directory volumes support
      ...
      * **medium** ``Optional[str]`` - *(optional)* What type of storage medium should back this directory. The default is ""
        which means to use the node's default medium. Must be an empty string
        (default) or Memory. More info:
        https://kubernetes.io/docs/concepts/storage/volumes#emptydir
      * **sizeLimit** ``Optional[resource.Quantity]`` - *(optional)* Total amount of local storage required for this EmptyDir volume. The size
...
class FCVolumeSource(DictMixin):
    """Represents a Fibre Channel volume. Fibre Channel volumes can only be mounted
      ...
      * **fsType** ``Optional[str]`` - *(optional)* Filesystem type to mount. Must be a filesystem type supported by the host
        operating system. Ex. "ext4", "xfs", "ntfs". Implicitly inferred to be "ext4"
        if unspecified.
      * **lun** ``Optional[int]`` - *(optional)* Optional: FC target lun number
      * **readOnly** ``Optional[bool]`` - *(optional)* Optional: Defaults to false (read/write). ReadOnly here will force the
        ReadOnly setting in VolumeMounts.
      * **targetWWNs** ``Optional[List[str]]`` - *(optional)* Optional: FC target worldwide names (WWNs)
      * **wwids** ``Optional[List[str]]`` - *(optional)* Optional: FC volume world wide identifiers (wwids) Either wwids or combination
...
class FlexPersistentVolumeSource(DictMixin):
    """FlexPersistentVolumeSource represents a generic persistent volume resource
      ...
      * **fsType** ``Optional[str]`` - *(optional)* Filesystem type to mount. Must be a filesystem type supported by the host
        ...
        FlexVolume script.
      * **options** ``Optional[dict]`` - *(optional)* Optional: Extra command options if any.
      * **readOnly** ``Optional[bool]`` - *(optional)* Optional: Defaults to false (read/write). ReadOnly here will force the
        ReadOnly setting in VolumeMounts.
      * **secretRef** ``Optional[SecretReference]`` - *(optional)* Optional: SecretRef is reference to the secret object containing sensitive
...
class FlexVolumeSource(DictMixin):
    """FlexVolume represents a generic volume resource that is provisioned/attached
      ...
      * **readOnly** ``Optional[bool]`` - *(optional)* Optional: Defaults to false (read/write). ReadOnly here will force the
        ReadOnly setting in VolumeMounts.
      * **secretRef** ``Optional[LocalObjectReference]`` - *(optional)* Optional: SecretRef is reference to the secret object containing sensitive
        information to pass to the plugin scripts. This may be empty if no secret
        object is specified. If the secret object contains more than one secret, all
        secrets are passed to the plugin scripts.
    """
...
class FlockerVolumeSource(DictMixin):
    """Represents a Flocker volume mounted by the Flocker agent. One and only one of
      ...
      * **datasetName** ``Optional[str]`` - *(optional)* Name of the dataset stored as metadata -> name on the dataset for Flocker
        should be considered as deprecated
      * **datasetUUID** ``Optional[str]`` - *(optional)* UUID of the dataset. This is unique identifier of a Flocker dataset
    """
...
class GitRepoVolumeSource(DictMixin):
    """Represents a volume that is populated with the contents of a git repository.
      ...
      * **directory** ``Optional[str]`` - *(optional)* Target directory name. Must not contain or start with '..'.  If '.' is
        ...
        the given name.
      * **revision** ``Optional[str]`` - *(optional)* Commit hash for the specified revision.
    """
    repository: 'str'
...
class ISCSIPersistentVolumeSource(DictMixin):
    """ISCSIPersistentVolumeSource represents an ISCSI disk. ISCSI volumes can only
      ...
      * **chapAuthDiscovery** ``Optional[bool]`` - *(optional)* whether support iSCSI Discovery CHAP authentication
      * **chapAuthSession** ``Optional[bool]`` - *(optional)* whether support iSCSI Session CHAP authentication
      * **fsType** ``Optional[str]`` - *(optional)* Filesystem type of the volume that you want to mount. Tip: Ensure that the
        filesystem type is supported by the host operating system. Examples: "ext4",
        "xfs", "ntfs". Implicitly inferred to be "ext4" if unspecified. More info:
        https://kubernetes.io/docs/concepts/storage/volumes#iscsi
      * **initiatorName** ``Optional[str]`` - *(optional)* Custom iSCSI Initiator Name. If initiatorName is specified with iscsiInterface
        simultaneously, new iSCSI interface <target portal>:<volume name> will be
        created for the connection.
      * **iscsiInterface** ``Optional[str]`` - *(optional)* iSCSI Interface Name that uses an iSCSI transport. Defaults to 'default'
        (tcp).
      * **portals** ``Optional[List[str]]`` - *(optional)* iSCSI Target Portal List. The Portal is either an IP or ip_addr:port if the
        port is other than default (typically TCP ports 860 and 3260).
      * **readOnly** ``Optional[bool]`` - *(optional)* ReadOnly here will force the ReadOnly setting in VolumeMounts. Defaults to
        false.
      * **secretRef** ``Optional[SecretReference]`` - *(optional)* CHAP Secret for iSCSI target and initiator authentication
    """
    iqn: 'str'
    lun: 'int'
    targetPortal: 'str'
...
class ISCSIVolumeSource(DictMixin):
    """Represents an ISCSI disk. ISCSI volumes can only be mounted as read/write
      ...
      * **readOnly** ``Optional[bool]`` - *(optional)* ReadOnly here will force the ReadOnly setting in VolumeMounts. Defaults to
        false.
      * **secretRef** ``Optional[LocalObjectReference]`` - *(optional)* CHAP Secret for iSCSI target and initiator authentication
    """
    iqn: 'str'
    lun: 'int'
    targetPortal: 'str'
...
class LocalVolumeSource(DictMixin):
    """Local represents directly-attached storage with node affinity (Beta feature)
      ...
      * **fsType** ``Optional[str]`` - *(optional)* Filesystem type to mount. It applies only when the Path is a block device.
...
class NodeSpec(DictMixin):
    """NodeSpec describes the attributes that a node is created with.
      ...
      * **configSource** ``Optional[NodeConfigSource]`` - *(optional)* Deprecated. If specified, the source of the node's configuration. The
...
class PersistentVolumeClaimCondition(DictMixin):
    """PersistentVolumeClaimCondition contails details about state of pvc
      ...
      * **message** ``Optional[str]`` - *(optional)* Human-readable message indicating details about last transition.
      * **reason** ``Optional[str]`` - *(optional)* Unique, this should be a short, machine understandable string that gives the
...
class PersistentVolumeClaimSpec(DictMixin):
    """PersistentVolumeClaimSpec describes the common attributes of storage devices
      ...
      * **dataSource** ``Optional[TypedLocalObjectReference]`` - *(optional)* This field can be used to specify either: * An existing VolumeSnapshot object
        ...
        field.
      * **dataSourceRef** ``Optional[TypedLocalObjectReference]`` - *(optional)* Specifies the object from which to populate the volume with data, if a
      ...
      * **selector** ``Optional[meta_v1.LabelSelector]`` - *(optional)* A label query over volumes to consider for binding.
      * **storageClassName** ``Optional[str]`` - *(optional)* Name of the StorageClass required by the claim. More info:
...
class PersistentVolumeClaimStatus(DictMixin):
    """PersistentVolumeClaimStatus is the current status of a persistent volume
      ...
      * **accessModes** ``Optional[List[str]]`` - *(optional)* AccessModes contains the actual access modes the volume backing the PVC has.
        More info:
        https://kubernetes.io/docs/concepts/storage/persistent-volumes#access-modes-1
      * **allocatedResources** ``Optional[dict]`` - *(optional)* The storage resource within AllocatedResources tracks the capacity allocated
        to a PVC. It may be larger than the actual capacity when a volume expansion
        ...
        capacity is equal or lower than the requested capacity. This is an alpha field
        and requires enabling RecoverVolumeExpansionFailure feature.
      * **capacity** ``Optional[dict]`` - *(optional)* Represents the actual resources of the underlying volume.
      * **conditions** ``Optional[List[PersistentVolumeClaimCondition]]`` - *(optional)* Current Condition of persistent volume claim. If underlying persistent volume
...
class PersistentVolumeClaimVolumeSource(DictMixin):
    """PersistentVolumeClaimVolumeSource references the user's PVC in the same
      ...
      * **claimName** ``str`` - ClaimName is the name of a PersistentVolumeClaim in the same namespace as the
        ...
        https://kubernetes.io/docs/concepts/storage/persistent-volumes#persistentvolumeclaims
      * **readOnly** ``Optional[bool]`` - *(optional)* Will force the ReadOnly setting in VolumeMounts. Default false.
    """
    claimName: 'str'
    readOnly: 'Optional[bool]' = None
...
class PersistentVolumeSpec(DictMixin):
    """PersistentVolumeSpec is the specification of a persistent volume.
      ...
      * **azureFile** ``Optional[AzureFilePersistentVolumeSource]`` - *(optional)* AzureFile represents an Azure File Service mount on the host and bind mount to
        the pod.
      * **capacity** ``Optional[dict]`` - *(optional)* A description of the persistent volume's resources and capacity. More info:
        https://kubernetes.io/docs/concepts/storage/persistent-volumes#capacity
      ...
      * **mountOptions** ``Optional[List[str]]`` - *(optional)* A list of mount options, e.g. ["ro", "soft"]. Not validated - mount will
      ...
      * **nodeAffinity** ``Optional[VolumeNodeAffinity]`` - *(optional)* NodeAffinity defines constraints that limit what nodes this volume can be
        ...
        volume.
      * **persistentVolumeReclaimPolicy** ``Optional[str]`` - *(optional)* What happens to a persistent volume when released from its claim. Valid
      ...
      * **scaleIO** ``Optional[ScaleIOPersistentVolumeSource]`` - *(optional)* ScaleIO represents a ScaleIO persistent volume attached and mounted on
        Kubernetes nodes.
      * **storageClassName** ``Optional[str]`` - *(optional)* Name of StorageClass to which this persistent volume belongs. Empty value
...
class PersistentVolumeStatus(DictMixin):
    """PersistentVolumeStatus is the current status of a persistent volume.
      ...
      * **message** ``Optional[str]`` - *(optional)* A human-readable message indicating details about why the volume is in this
        state.
...
class PodStatus(DictMixin):
    """PodStatus represents information about the status of a pod. Status may trail
      ...
      * **conditions** ``Optional[List[PodCondition]]`` - *(optional)* Current service state of pod. More info:
        https://kubernetes.io/docs/concepts/workloads/pods/pod-lifecycle#pod-conditions
      * **containerStatuses** ``Optional[List[ContainerStatus]]`` - *(optional)* The list has one entry per container in the manifest. Each entry is currently
...
class Probe(DictMixin):
    """Probe describes a health check to be performed against a container to
      ...
      * **grpc** ``Optional[GRPCAction]`` - *(optional)* GRPC specifies an action involving a GRPC port. This is an alpha field and
...
class ProjectedVolumeSource(DictMixin):
    """Represents a projected volume source
      ...
      * **defaultMode** ``Optional[int]`` - *(optional)* Mode bits used to set permissions on created files by default. Must be an
        octal value between 0000 and 0777 or a decimal value between 0 and 511. YAML
        accepts both octal and decimal values, JSON requires decimal values for mode
      ...
      * **sources** ``Optional[List[VolumeProjection]]`` - *(optional)* list of volume projections
    """
    defaultMode: 'Optional[int]' = None
...
class RBDPersistentVolumeSource(DictMixin):
    """Represents a Rados Block Device mount that lasts the lifetime of a pod. RBD
      ...
      * **pool** ``Optional[str]`` - *(optional)* The rados pool name. Default is rbd. More info:
        https://examples.k8s.io/volumes/rbd/README.md#how-to-use-it
      * **readOnly** ``Optional[bool]`` - *(optional)* ReadOnly here will force the ReadOnly setting in VolumeMounts. Defaults to
      ...
      * **secretRef** ``Optional[SecretReference]`` - *(optional)* SecretRef is name of the authentication secret for RBDUser. If provided
        ...
        https://examples.k8s.io/volumes/rbd/README.md#how-to-use-it
      * **user** ``Optional[str]`` - *(optional)* The rados user name. Default is admin. More info:
        https://examples.k8s.io/volumes/rbd/README.md#how-to-use-it
    """
    image: 'str'
    monitors: 'List[str]'
...
class ScaleIOPersistentVolumeSource(DictMixin):
    """ScaleIOPersistentVolumeSource represents a persistent ScaleIO volume
      ...
      * **protectionDomain** ``Optional[str]`` - *(optional)* The name of the ScaleIO Protection Domain for the configured storage.
      * **readOnly** ``Optional[bool]`` - *(optional)* Defaults to false (read/write). ReadOnly here will force the ReadOnly setting
        in VolumeMounts.
      * **sslEnabled** ``Optional[bool]`` - *(optional)* Flag to enable/disable SSL communication with Gateway, default false
      * **storageMode** ``Optional[str]`` - *(optional)* Indicates whether the storage for a volume should be ThickProvisioned or
        ThinProvisioned. Default is ThinProvisioned.
      * **storagePool** ``Optional[str]`` - *(optional)* The ScaleIO Storage Pool associated with the protection domain.
      * **volumeName** ``Optional[str]`` - *(optional)* The name of a volume already created in the ScaleIO system that is associated
...
class SecretProjection(DictMixin):
    """Adapts a secret into a projected volume.
      ...
      * **items** ``Optional[List[KeyToPath]]`` - *(optional)* If unspecified, each key-value pair in the Data field of the referenced Secret
...
class SecretVolumeSource(DictMixin):
    """Adapts a Secret into a volume.
      ...
      * **items** ``Optional[List[KeyToPath]]`` - *(optional)* If unspecified, each key-value pair in the Data field of the referenced Secret
        ...
        marked optional. Paths must be relative and may not contain the '..' path or
        start with '..'.
      * **optional** ``Optional[bool]`` - *(optional)* Specify whether the Secret or its keys must be defined
      * **secretName** ``Optional[str]`` - *(optional)* Name of the secret in the pod's namespace to use. More info:
...
class ServiceAccount(DictMixin):
    """ServiceAccount binds together: * a name, understood by users, and perhaps by
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard object's metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **secrets** ``Optional[List[ObjectReference]]`` - *(optional)* Secrets is the list of secrets allowed to be used by pods running using this
...
class ServiceSpec(DictMixin):
    """ServiceSpec describes the attributes that a user creates on a service.
      ...
      * **loadBalancerIP** ``Optional[str]`` - *(optional)* Only applies to Service Type: LoadBalancer LoadBalancer will get created with
...
class Volume(DictMixin):
    """Volume represents a named volume in a pod that may be accessed by any
      ...
      * **portworxVolume** ``Optional[PortworxVolumeSource]`` - *(optional)* PortworxVolume represents a portworx volume attached and mounted on kubelets
        host machine
      * **projected** ``Optional[ProjectedVolumeSource]`` - *(optional)* Items for all in one resources secrets, configmaps, and downward API
      * **quobyte** ``Optional[QuobyteVolumeSource]`` - *(optional)* Quobyte represents a Quobyte mount on the host that shares a pod's lifetime
...
class VolumeProjection(DictMixin):
    """Projection that may be projected along with other supported volume types
      ...
      * **configMap** ``Optional[ConfigMapProjection]`` - *(optional)* information about the configMap data to project
      * **downwardAPI** ``Optional[DownwardAPIProjection]`` - *(optional)* information about the downwardAPI data to project
      * **secret** ``Optional[SecretProjection]`` - *(optional)* information about the secret data to project
      * **serviceAccountToken** ``Optional[ServiceAccountTokenProjection]`` - *(optional)* information about the serviceAccountToken data to project
    """
...
class VsphereVirtualDiskVolumeSource(DictMixin):
    """Represents a vSphere volume resource.
      ...
      * **fsType** ``Optional[str]`` - *(optional)* Filesystem type to mount. Must be a filesystem type supported by the host
        operating system. Ex. "ext4", "xfs", "ntfs". Implicitly inferred to be "ext4"
        if unspecified.
      * **storagePolicyID** ``Optional[str]`` - *(optional)* Storage Policy Based Management (SPBM) profile ID associated with the
        StoragePolicyName.
      * **storagePolicyName** ``Optional[str]`` - *(optional)* Storage Policy Based Management (SPBM) profile name.
    """
    volumePath: 'str'
    fsType: 'Optional[str]' = None
