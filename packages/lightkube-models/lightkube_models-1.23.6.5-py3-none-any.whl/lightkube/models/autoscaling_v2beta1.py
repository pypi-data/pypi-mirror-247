...
class ContainerResourceMetricSource(DictMixin):
    """ContainerResourceMetricSource indicates how to scale on a resource metric
      ...
      * **targetAverageUtilization** ``Optional[int]`` - *(optional)* targetAverageUtilization is the target value of the average of the resource
        metric across all relevant pods, represented as a percentage of the requested
        value of the resource for the pods.
      * **targetAverageValue** ``Optional[resource.Quantity]`` - *(optional)* targetAverageValue is the target value of the average of the resource metric
...
class ContainerResourceMetricStatus(DictMixin):
    """ContainerResourceMetricStatus indicates the current value of a resource metric
      ...
      * **name** ``str`` - name is the name of the resource in question.
      * **currentAverageUtilization** ``Optional[int]`` - *(optional)* currentAverageUtilization is the current value of the average of the resource
        metric across all relevant pods, represented as a percentage of the requested
...
class ExternalMetricSource(DictMixin):
    """ExternalMetricSource indicates how to scale on a metric not associated with
      ...
      * **metricSelector** ``Optional[meta_v1.LabelSelector]`` - *(optional)* metricSelector is used to identify a specific time series within a given
        metric.
      * **targetAverageValue** ``Optional[resource.Quantity]`` - *(optional)* targetAverageValue is the target per-pod value of global metric (as a
        quantity). Mutually exclusive with TargetValue.
      * **targetValue** ``Optional[resource.Quantity]`` - *(optional)* targetValue is the target value of the metric (as a quantity). Mutually
...
class ExternalMetricStatus(DictMixin):
    """ExternalMetricStatus indicates the current value of a global metric not
      ...
      * **currentAverageValue** ``Optional[resource.Quantity]`` - *(optional)* currentAverageValue is the current value of metric averaged over autoscaled
        pods.
      * **metricSelector** ``Optional[meta_v1.LabelSelector]`` - *(optional)* metricSelector is used to identify a specific time series within a given
        metric.
    """
