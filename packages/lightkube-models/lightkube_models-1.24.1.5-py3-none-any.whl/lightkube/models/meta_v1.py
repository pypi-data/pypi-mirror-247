...
class APIGroup(DictMixin):
    """APIGroup contains the name, the supported versions, and the preferred version
      ...
      * **preferredVersion** ``Optional[GroupVersionForDiscovery]`` - *(optional)* preferredVersion is the version preferred by the API server, which probably is
        the storage version.
      * **serverAddressByClientCIDRs** ``Optional[List[ServerAddressByClientCIDR]]`` - *(optional)* a map of client CIDR to server address that is serving this group. This is to
...
class APIResource(DictMixin):
    """APIResource specifies the name of a resource and whether it is namespaced.
      ...
      * **categories** ``Optional[List[str]]`` - *(optional)* categories is a list of the grouped resources this resource belongs to (e.g.
        'all')
      * **group** ``Optional[str]`` - *(optional)* group is the preferred group of the resource.  Empty implies the group of the
        containing resource list. For subresources, this may have a different value,
        for example: Scale".
      * **shortNames** ``Optional[List[str]]`` - *(optional)* shortNames is a list of suggested short names of the resource.
      * **storageVersionHash** ``Optional[str]`` - *(optional)* The hash value of the storage version, the version this resource is converted
      ...
      * **version** ``Optional[str]`` - *(optional)* version is the preferred version of the resource.  Empty implies the version
...
class Condition(DictMixin):
    """Condition contains details for one aspect of the current state of this API
      ...
      * **observedGeneration** ``Optional[int]`` - *(optional)* observedGeneration represents the .metadata.generation that the condition was
...
class DeleteOptions(DictMixin):
    """DeleteOptions may be provided when deleting an API object.
      ...
      * **apiVersion** ``Optional[str]`` - *(optional)* APIVersion defines the versioned schema of this representation of an object.
        ...
        may reject unrecognized values. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#resources
      * **dryRun** ``Optional[List[str]]`` - *(optional)* When present, indicates that modifications should not be persisted. An invalid
        ...
        will be processed
      * **gracePeriodSeconds** ``Optional[int]`` - *(optional)* The duration in seconds before the object should be deleted. Value must be
      ...
      * **kind** ``Optional[str]`` - *(optional)* Kind is a string value representing the REST resource this object represents.
        ...
        Cannot be updated. In CamelCase. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds
      * **orphanDependents** ``Optional[bool]`` - *(optional)* Deprecated: please use the PropagationPolicy, this field will be deprecated in
      ...
      * **preconditions** ``Optional[Preconditions]`` - *(optional)* Must be fulfilled before a deletion is carried out. If not possible, a 409
        Conflict status will be returned.
      * **propagationPolicy** ``Optional[str]`` - *(optional)* Whether and how garbage collection will be performed. Either this field or
...
class LabelSelector(DictMixin):
    """A label selector is a label query over a set of resources. The result of
      ...
      * **matchExpressions** ``Optional[List[LabelSelectorRequirement]]`` - *(optional)* matchExpressions is a list of label selector requirements. The requirements
        are ANDed.
      * **matchLabels** ``Optional[dict]`` - *(optional)* matchLabels is a map of {key,value} pairs. A single {key,value} in the
...
class LabelSelectorRequirement(DictMixin):
    """A label selector requirement is a selector that contains values, a key, and an
      ...
      * **values** ``Optional[List[str]]`` - *(optional)* values is an array of string values. If the operator is In or NotIn, the
...
class ListMeta(DictMixin):
    """ListMeta describes metadata that synthetic resources must have, including
      ...
      * **continue_** ``Optional[str]`` - *(optional)* continue may be set if the user set a limit on the number of items returned,
      ...
      * **remainingItemCount** ``Optional[int]`` - *(optional)* remainingItemCount is the number of subsequent items in the list which are not
      ...
      * **resourceVersion** ``Optional[str]`` - *(optional)* String that identifies the server's internal version of this object that can
      ...
      * **selfLink** ``Optional[str]`` - *(optional)* Deprecated: selfLink is a legacy read-only field that is no longer populated
        by the system.
    """
...
class ManagedFieldsEntry(DictMixin):
    """ManagedFieldsEntry is a workflow-id, a FieldSet and the group version of the
      ...
      * **apiVersion** ``Optional[str]`` - *(optional)* APIVersion defines the version of this resource that this field set applies
      ...
      * **fieldsType** ``Optional[str]`` - *(optional)* FieldsType is the discriminator for the different fields format and version.
        There is currently only one possible value: "FieldsV1"
      * **fieldsV1** ``Optional[FieldsV1]`` - *(optional)* FieldsV1 holds the first JSON version format as described in the "FieldsV1"
        type.
      * **manager** ``Optional[str]`` - *(optional)* Manager is an identifier of the workflow managing these fields.
      * **operation** ``Optional[str]`` - *(optional)* Operation is the type of operation which lead to this ManagedFieldsEntry being
        created. The only valid values for this field are 'Apply' and 'Update'.
      * **subresource** ``Optional[str]`` - *(optional)* Subresource is the name of the subresource used to update that object, or
        ...
        resource.
      * **time** ``Optional[Time]`` - *(optional)* Time is the timestamp of when the ManagedFields entry was added. The timestamp
...
class ObjectMeta(DictMixin):
    """ObjectMeta is metadata that all persisted resources must have, which includes
      ...
      * **annotations** ``Optional[dict]`` - *(optional)* Annotations is an unstructured key value map stored with a resource that may
        be set by external tools to store and retrieve arbitrary metadata. They are
        not queryable and should be preserved when modifying objects. More info:
        http://kubernetes.io/docs/user-guide/annotations
      * **clusterName** ``Optional[str]`` - *(optional)* Deprecated: ClusterName is a legacy field that was always cleared by the
        system and never used; it will be removed completely in 1.25.
        The name in the go struct is changed to help clients detect accidental use.
      * **creationTimestamp** ``Optional[Time]`` - *(optional)* CreationTimestamp is a timestamp representing the server time when this object
      ...
      * **deletionGracePeriodSeconds** ``Optional[int]`` - *(optional)* Number of seconds allowed for this object to gracefully terminate before it
        will be removed from the system. Only set when deletionTimestamp is also set.
        May only be shortened. Read-only.
      * **deletionTimestamp** ``Optional[Time]`` - *(optional)* DeletionTimestamp is RFC 3339 date and time at which this resource will be
        ...
        info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **finalizers** ``Optional[List[str]]`` - *(optional)* Must be empty before the object is deleted from the registry. Each entry is an
      ...
      * **generateName** ``Optional[str]`` - *(optional)* GenerateName is an optional prefix, used by the server, to generate a unique
      ...
      * **generation** ``Optional[int]`` - *(optional)* A sequence number representing a specific generation of the desired state.
        Populated by the system. Read-only.
      * **labels** ``Optional[dict]`` - *(optional)* Map of string keys and values that can be used to organize and categorize
      ...
      * **managedFields** ``Optional[List[ManagedFieldsEntry]]`` - *(optional)* ManagedFields maps workflow-id and version to the set of fields that are
      ...
      * **name** ``Optional[str]`` - *(optional)* Name must be unique within a namespace. Is required when creating resources,
        although some resources may allow a client to request the generation of an
        appropriate name automatically. Name is primarily intended for creation
        idempotence and configuration definition. Cannot be updated. More info:
        http://kubernetes.io/docs/user-guide/identifiers#names
      * **namespace** ``Optional[str]`` - *(optional)* Namespace defines the space within which each name must be unique. An empty
        namespace is equivalent to the "default" namespace, but "default" is the
        canonical representation. Not all objects are required to be scoped to a
        namespace - the value of this field for those objects will be empty.
        Must be a DNS_LABEL. Cannot be updated. More info:
        http://kubernetes.io/docs/user-guide/namespaces
      * **ownerReferences** ``Optional[List[OwnerReference]]`` - *(optional)* List of objects depended by this object. If ALL objects in the list have been
        ...
        controller.
      * **resourceVersion** ``Optional[str]`` - *(optional)* An opaque value that represents the internal version of this object that can
      ...
      * **selfLink** ``Optional[str]`` - *(optional)* Deprecated: selfLink is a legacy read-only field that is no longer populated
        by the system.
      * **uid** ``Optional[str]`` - *(optional)* UID is the unique in time and space value for this object. It is typically
...
class OwnerReference(DictMixin):
    """OwnerReference contains enough information to let you identify an owning
      ...
      * **uid** ``str`` - UID of the referent. More info:
        http://kubernetes.io/docs/user-guide/identifiers#uids
      * **blockOwnerDeletion** ``Optional[bool]`` - *(optional)* If true, AND if the owner has the "foregroundDeletion" finalizer, then the
        ...
        returned.
      * **controller** ``Optional[bool]`` - *(optional)* If true, this reference points to the managing controller.
    """
    apiVersion: 'str'
    kind: 'str'
    name: 'str'
...
class Preconditions(DictMixin):
    """Preconditions must be fulfilled before an operation (update, delete, etc.) is
      ...
      * **resourceVersion** ``Optional[str]`` - *(optional)* Specifies the target ResourceVersion
      * **uid** ``Optional[str]`` - *(optional)* Specifies the target UID.
    """
    resourceVersion: 'Optional[str]' = None
    uid: 'Optional[str]' = None
...
class Status(DictMixin):
    """Status is a return value for calls that don't return other objects.
      ...
      * **apiVersion** ``Optional[str]`` - *(optional)* APIVersion defines the versioned schema of this representation of an object.
        ...
        may reject unrecognized values. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#resources
      * **code** ``Optional[int]`` - *(optional)* Suggested HTTP return code for this status, 0 if not set.
      * **details** ``Optional[StatusDetails]`` - *(optional)* Extended data associated with the reason.  Each reason may define its own
      ...
      * **metadata** ``Optional[ListMeta]`` - *(optional)* Standard list metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds
      * **reason** ``Optional[str]`` - *(optional)* A machine-readable description of why this operation is in the "Failure"
      ...
      * **status** ``Optional[str]`` - *(optional)* Status of the operation. One of: "Success" or "Failure". More info:
...
class StatusCause(DictMixin):
    """StatusCause provides more information about an api.Status failure, including
      ...
      * **field** ``Optional[str]`` - *(optional)* The field of the resource that has caused this error, as named by its JSON
      ...
      * **message** ``Optional[str]`` - *(optional)* A human-readable description of the cause of the error.  This field may be
        presented as-is to a reader.
      * **reason** ``Optional[str]`` - *(optional)* A machine-readable description of the cause of the error. If this value is
...
class StatusDetails(DictMixin):
    """StatusDetails is a set of additional properties that MAY be set by the server
      ...
      * **causes** ``Optional[List[StatusCause]]`` - *(optional)* The Causes array includes more details associated with the StatusReason
        failure. Not all StatusReasons may provide detailed causes.
      * **group** ``Optional[str]`` - *(optional)* The group attribute of the resource associated with the status StatusReason.
      * **kind** ``Optional[str]`` - *(optional)* The kind attribute of the resource associated with the status StatusReason. On
        some operations may differ from the requested resource Kind. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#types-kinds
      * **name** ``Optional[str]`` - *(optional)* The name attribute of the resource associated with the status StatusReason
        (when there is a single name which can be described).
      * **retryAfterSeconds** ``Optional[int]`` - *(optional)* If specified, the time in seconds before the operation should be retried. Some
        ...
        action.
      * **uid** ``Optional[str]`` - *(optional)* UID of the resource. (when there is a single resource which can be described).
