...
class EndpointPort(DictMixin):
    """EndpointPort is a tuple that describes a single port.
      ...
      * **name** ``Optional[str]`` - *(optional)* The name of this port (corresponds to ServicePort.Name). Must be a DNS_LABEL.
...
class Lifecycle(DictMixin):
    """Lifecycle describes actions that the management system should take in response
      ...
      * **preStop** ``Optional[Handler]`` - *(optional)* PreStop is called immediately before a container is terminated due to an API
        request or management event such as liveness probe failure, preemption,
        resource contention, etc. The handler is not called if the container crashes
        or exits. The reason for termination is passed to the handler. The Pod's
        termination grace period countdown begins before the PreStop hooked is
        executed. Regardless of the outcome of the handler, the container will
...
class PodSecurityContext(DictMixin):
    """PodSecurityContext holds pod-level security attributes and common container
      ...
      * **windowsOptions** ``Optional[WindowsSecurityContextOptions]`` - *(optional)* Windows security options.
    """
...
class PodSpec(DictMixin):
    """PodSpec is a description of a pod.
      ...
      * **initContainers** ``Optional[List[Container]]`` - *(optional)* List of initialization containers belonging to the pod. Init containers are
        ...
        request/limit for each resource type, and then using the max of of that value
        or the sum of the normal containers. Limits are applied to init containers in
        a similar fashion. Init containers cannot currently be added or removed.
