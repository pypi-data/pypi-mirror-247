...
class HorizontalPodAutoscaler(DictMixin):
    """configuration of a horizontal pod autoscaler.
      ...
      * **spec** ``Optional[HorizontalPodAutoscalerSpec]`` - *(optional)* behaviour of autoscaler. More info:
        https://git.k8s.io/community/contributors/devel/api-conventions.md#spec-and-status.
      * **status** ``Optional[HorizontalPodAutoscalerStatus]`` - *(optional)* current information about the autoscaler.
    """
    apiVersion: 'Optional[str]' = None
...
class HorizontalPodAutoscalerSpec(DictMixin):
    """specification of a horizontal pod autoscaler.
      ...
      * **scaleTargetRef** ``CrossVersionObjectReference`` - reference to scaled resource; horizontal pod autoscaler will learn the current
        ...
        Scale subresource.
      * **minReplicas** ``Optional[int]`` - *(optional)* lower limit for the number of pods that can be set by the autoscaler, default
        1.
      * **targetCPUUtilizationPercentage** ``Optional[int]`` - *(optional)* target average CPU utilization (represented as a percentage of requested CPU)
...
class HorizontalPodAutoscalerStatus(DictMixin):
    """current status of a horizontal pod autoscaler
      ...
      * **currentCPUUtilizationPercentage** ``Optional[int]`` - *(optional)* current average CPU utilization over all pods, represented as a percentage of
        requested CPU, e.g. 70 means that an average pod is using now 70% of its
        requested CPU.
      * **lastScaleTime** ``Optional[meta_v1.Time]`` - *(optional)* last time the HorizontalPodAutoscaler scaled the number of pods; used by the
        autoscaler to control how often the number of pods is changed.
      * **observedGeneration** ``Optional[int]`` - *(optional)* most recent generation observed by this autoscaler.
    """
    currentReplicas: 'int'
    desiredReplicas: 'int'
...
class ScaleStatus(DictMixin):
    """ScaleStatus represents the current status of a scale subresource.
      ...
      * **selector** ``Optional[str]`` - *(optional)* label query over pods that should match the replicas count. This is same as
