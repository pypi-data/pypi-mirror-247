...
class CertificateSigningRequest(DictMixin):
    """CertificateSigningRequest objects provide a mechanism to obtain x509
      ...
      * **status** ``Optional[CertificateSigningRequestStatus]`` - *(optional)* status contains information about whether the request is approved or denied,
...
class CertificateSigningRequestCondition(DictMixin):
    """CertificateSigningRequestCondition describes a condition of a
      ...
      * **lastUpdateTime** ``Optional[meta_v1.Time]`` - *(optional)* lastUpdateTime is the time of the last update to this condition
      * **message** ``Optional[str]`` - *(optional)* message contains a human readable message with details about the request state
      * **reason** ``Optional[str]`` - *(optional)* reason indicates a brief reason for the request state
    """
    status: 'str'
    type: 'str'
...
class CertificateSigningRequestSpec(DictMixin):
    """CertificateSigningRequestSpec contains the certificate request.
      ...
      * **expirationSeconds** ``Optional[int]`` - *(optional)* expirationSeconds is the requested duration of validity of the issued
        ...
        gate.
      * **extra** ``Optional[dict]`` - *(optional)* extra contains extra attributes of the user that created the
        CertificateSigningRequest. Populated by the API server on creation and
        immutable.
      * **groups** ``Optional[List[str]]`` - *(optional)* groups contains group membership of the user that created the
        CertificateSigningRequest. Populated by the API server on creation and
        immutable.
      * **uid** ``Optional[str]`` - *(optional)* uid contains the uid of the user that created the CertificateSigningRequest.
        Populated by the API server on creation and immutable.
      * **usages** ``Optional[List[str]]`` - *(optional)* usages specifies a set of key usages requested in the issued certificate.
      ...
      * **username** ``Optional[str]`` - *(optional)* username contains the name of the user that created the
        CertificateSigningRequest. Populated by the API server on creation and
        immutable.
    """
    request: 'str'
...
class CertificateSigningRequestStatus(DictMixin):
    """CertificateSigningRequestStatus contains conditions used to indicate
      ...
      * **certificate** ``Optional[str]`` - *(optional)* certificate is populated with an issued certificate by the signer after an
      ...
      * **conditions** ``Optional[List[CertificateSigningRequestCondition]]`` - *(optional)* conditions applied to the request. Known conditions are "Approved", "Denied",
        and "Failed".
    """
