...
class LocalSubjectAccessReview(DictMixin):
    """LocalSubjectAccessReview checks whether or not a user or group can perform an
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard list metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **status** ``Optional[SubjectAccessReviewStatus]`` - *(optional)* Status is filled in by the server and indicates whether the request is allowed
        or not
    """
...
class NonResourceAttributes(DictMixin):
    """NonResourceAttributes includes the authorization attributes available for
      ...
      * **path** ``Optional[str]`` - *(optional)* Path is the URL path of the request
      * **verb** ``Optional[str]`` - *(optional)* Verb is the standard HTTP verb
    """
...
class NonResourceRule(DictMixin):
    """NonResourceRule holds information that describes a rule for the non-resource
      ...
      * **nonResourceURLs** ``Optional[List[str]]`` - *(optional)* NonResourceURLs is a set of partial urls that a user should have access to.
...
class ResourceAttributes(DictMixin):
    """ResourceAttributes includes the authorization attributes available for
      ...
      * **group** ``Optional[str]`` - *(optional)* Group is the API Group of the Resource.  "*" means all.
      * **name** ``Optional[str]`` - *(optional)* Name is the name of the resource being requested for a "get" or deleted for a
        "delete". "" (empty) means all.
      * **namespace** ``Optional[str]`` - *(optional)* Namespace is the namespace of the action being requested.  Currently, there is
      ...
      * **resource** ``Optional[str]`` - *(optional)* Resource is one of the existing resource types.  "*" means all.
      * **subresource** ``Optional[str]`` - *(optional)* Subresource is one of the existing resource types.  "" means none.
      * **verb** ``Optional[str]`` - *(optional)* Verb is a kubernetes resource API verb, like: get, list, watch, create,
        update, delete, proxy.  "*" means all.
      * **version** ``Optional[str]`` - *(optional)* Version is the API Version of the Resource.  "*" means all.
    """
...
class ResourceRule(DictMixin):
    """ResourceRule is the list of actions the subject is allowed to perform on
      ...
      * **apiGroups** ``Optional[List[str]]`` - *(optional)* APIGroups is the name of the APIGroup that contains the resources.  If
      ...
      * **resources** ``Optional[List[str]]`` - *(optional)* Resources is a list of resources this rule applies to.  "*" means all in the
        specified apiGroups.
...
class SelfSubjectAccessReviewSpec(DictMixin):
    """SelfSubjectAccessReviewSpec is a description of the access request.  Exactly
      ...
      * **nonResourceAttributes** ``Optional[NonResourceAttributes]`` - *(optional)* NonResourceAttributes describes information for a non-resource access request
      * **resourceAttributes** ``Optional[ResourceAttributes]`` - *(optional)* ResourceAuthorizationAttributes describes information for a resource access
        request
    """
...
class SelfSubjectRulesReview(DictMixin):
    """SelfSubjectRulesReview enumerates the set of actions the current user can
      ...
      * **metadata** ``Optional[meta_v1.ObjectMeta]`` - *(optional)* Standard list metadata. More info:
        https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
      * **status** ``Optional[SubjectRulesReviewStatus]`` - *(optional)* Status is filled in by the server and indicates the set of actions a user can
        perform.
    """
...
class SelfSubjectRulesReviewSpec(DictMixin):
    """SelfSubjectRulesReviewSpec defines the specification for
      ...
      * **namespace** ``Optional[str]`` - *(optional)* Namespace to evaluate rules for. Required.
    """
    namespace: 'Optional[str]' = None
...
class SubjectAccessReviewSpec(DictMixin):
    """SubjectAccessReviewSpec is a description of the access request.  Exactly one
      ...
      * **extra** ``Optional[dict]`` - *(optional)* Extra corresponds to the user.Info.GetExtra() method from the authenticator.
        Since that is input to the authorizer it needs a reflection here.
      * **groups** ``Optional[List[str]]`` - *(optional)* Groups is the groups you're testing for.
      * **nonResourceAttributes** ``Optional[NonResourceAttributes]`` - *(optional)* NonResourceAttributes describes information for a non-resource access request
      * **resourceAttributes** ``Optional[ResourceAttributes]`` - *(optional)* ResourceAuthorizationAttributes describes information for a resource access
        request
      * **uid** ``Optional[str]`` - *(optional)* UID information about the requesting user.
      * **user** ``Optional[str]`` - *(optional)* User is the user you're testing for. If you specify "User" but not "Groups",
...
class SubjectAccessReviewStatus(DictMixin):
    """SubjectAccessReviewStatus
      ...
      * **denied** ``Optional[bool]`` - *(optional)* Denied is optional. True if the action would be denied, otherwise false. If
      ...
      * **evaluationError** ``Optional[str]`` - *(optional)* EvaluationError is an indication that some error occurred during the
        ...
        about the request.
      * **reason** ``Optional[str]`` - *(optional)* Reason is optional.  It indicates why a request was allowed or denied.
    """
    allowed: 'bool'
...
class SubjectRulesReviewStatus(DictMixin):
    """SubjectRulesReviewStatus contains the result of a rules check. This check can
      ...
      * **resourceRules** ``List[ResourceRule]`` - ResourceRules is the list of actions the subject is allowed to perform on
        resources. The list ordering isn't significant, may contain duplicates, and
        possibly be incomplete.
      * **evaluationError** ``Optional[str]`` - *(optional)* EvaluationError can appear in combination with Rules. It indicates an error
