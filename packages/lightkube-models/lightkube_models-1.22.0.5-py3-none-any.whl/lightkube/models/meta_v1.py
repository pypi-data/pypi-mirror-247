...
class ListMeta(DictMixin):
    """ListMeta describes metadata that synthetic resources must have, including
      ...
      * **selfLink** ``Optional[str]`` - *(optional)* selfLink is a URL representing this object. Populated by the system.
        Read-only.
...
class ManagedFieldsEntry(DictMixin):
    """ManagedFieldsEntry is a workflow-id, a FieldSet and the group version of the
      ...
      * **subresource** ``Optional[str]`` - *(optional)* Subresource is the name of the subresource used to update that object, or
        ...
        resource.
      * **time** ``Optional[Time]`` - *(optional)* Time is timestamp of when these fields were set. It should always be empty if
        Operation is 'Apply'
    """
    apiVersion: 'Optional[str]' = None
...
class ObjectMeta(DictMixin):
    """ObjectMeta is metadata that all persisted resources must have, which includes
      ...
      * **clusterName** ``Optional[str]`` - *(optional)* The name of the cluster which the object belongs to. This is used to
