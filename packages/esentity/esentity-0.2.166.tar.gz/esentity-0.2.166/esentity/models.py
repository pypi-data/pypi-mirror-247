...
class Page(ElasticEntity):
    ...
    @classmethod
    def query(cls, query, is_full=False, locale='en', category=[], in_locale=None, count=10, match_type='bool_prefix', page=1):
        if not query:
            return []
    ...
    @classmethod
    def provider_by_context(cls, country, _exclude=[], _exclude_params={}, _count=12, _page=1, _offset=None, _source=["title", "alias", "logo"], _aggs={}, _sorting=None, _locale='en', **params):
        filter_list =  [ 
            {"term": {"is_active": True}},
            {"term": {"is_draft": False}},
            {"term": {"is_sandbox": False}},
        ...
        def process_aggs(_a):
            ...
            for k, v in _a.items():
                ...
                if isinstance(buckets, dict):
                    _res[k] = [{'item': r, 'count': dia['doc_count']} for r, dia in v['buckets'].items()]
                else:
...
class ProductAggs(ElasticEntity):
    ...
    @classmethod
    def aggs_product(cls, _cnt=5000, _sort={'_key': 'asc'}, _ranges={}, **kwargs):
        aggs = {
            "brand": {
                "terms": {
