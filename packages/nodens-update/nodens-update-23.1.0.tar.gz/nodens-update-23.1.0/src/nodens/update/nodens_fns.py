...
class radar_config_params:
    ...
    def receive_config(self, raw):      # NodeNs KZR FIX - change this to update __init__.cp
        if self.cfg_idx == 0:
            self.config_radar = []
...
def parse_config(EntryWays,cp):
    ...
    sendCMDtoSensor.request_version(cp,sv)
    ...
    config_pub_rate = "CMD: PUBLISH RATE: " + str(round(cp.SCAN_TIME/rate_unit))
    payload_msg = [{ "addr" : [cp.SENSOR_TARGET],
                        "type" : "json",
                        "data" : config_pub_rate + "\n"}]

    if cp.FULL_DATA_FLAG:
        config_full_data = "CMD: FULL DATA ON. RATE: " + str(max(1,cp.FULL_DATA_TIME/cp.SCAN_TIME))
    else:
        config_full_data = "CMD: FULL DATA OFF."

    payload_msg.append({ "addr" : [cp.SENSOR_TARGET],
                    "type" : "json",
                    "data" : config_full_data + "\n"})
        
    # Send radar config #
    if cp.RADAR_SEND_FLAG:
        # Occupant tracker sensitivity #
        # NOTE: only implemented for 2D so far
        if sv.radar_dim == 2:
            param_temp = rcp.config_radar[12].split(" ")
            param_temp[1] = str(round(np.exp(1.8/float(cp.OCC_SENSITIVITY)+2.1)))
            param_temp[2] = str(round(np.exp(0.7/float(cp.OCC_SENSITIVITY)+5.3)))
            param_temp[4] = str(round(np.exp(0.7/float(cp.OCC_SENSITIVITY)+2)))
        ...
        if sv.radar_dim == 2:
            ...
            param_temp[10] = str(round(np.exp(0.7/float(cp.SENSITIVITY)+2.3)))
            param_temp[11] = str(round(np.exp(0.5/float(cp.SENSITIVITY)+3)))
        ...
        if sv.radar_dim == 2:
            ...
            param_temp[1:5] = [str(cp.ROOM_X_MIN), str(cp.ROOM_X_MAX), str(cp.ROOM_Y_MIN), str(cp.ROOM_Y_MAX)]
            ...
            while True:
                if i == len(rcp.config_radar):
                    ...
                    temp_x = cp.MONITOR_X.split(',')
                    temp_y = cp.MONITOR_Y.split(',')
                    temp_z = cp.MONITOR_Z.split(',')
            ...
            while True:
                if i == len(rcp.config_radar):
                    ...
                    param_temp = rcp.config_radar[i].split(" ")
                    param_temp[1:7] = [str(cp.ROOM_X_MIN), str(cp.ROOM_X_MAX), str(cp.ROOM_Y_MIN), str(cp.ROOM_Y_MAX), str(cp.ROOM_Z_MIN), str(cp.ROOM_Z_MAX)]
                    rcp.config_radar[i] = " ".join(param_temp)
                    logging.debug(rcp.config_radar[i])
                    break
                else:
    ...
    if cp.ENTRY_FLAG:
        # Check for sensor id
        if (cp.SENSOR_TARGET not in EntryWays.id):
            EntryWays.id.append(cp.SENSOR_TARGET)
            EntryWays.x.append([])
            EntryWays.y.append([])
            EntryWays.count.append(0)
        sen_idx = EntryWays.id.index(cp.SENSOR_TARGET)
                    ...
                    for i in range(int(len(ew.x[ind_e])/2)):
                        ...
                        if (0<=t<=1) and (0<=u<=1):
                            if (sx0**2 + sy0**2) > (sx1**2 + sy1**2):
                                ew.count[ind_e] += 1
                                logging.info('Entered room at entry {} with (t,u)=({},{}). Occupancy = {}.'.format(i,t,u,ew.count[ind_e]))
                            else:
                                ew.count[ind_e] -= 1
                                if ew.count[ind_e] < 0:
                                    ew.count[ind_e] = 0
                                    logging.warning('Warning! Count dropped below 0 and was reset.')
                                logging.info('Leaving room at entry {} with (t,u)=({},{}). Occupancy = {}.'.format(i,t,u,ew.count[ind_e]))
            except:
                logging.warning('Entryway update issue. (sx0,sx1)=({},{}). (sy0,sy1)=({},{})'.format(sx0,sx1,sy0,sy1))
...
class sendCMDtoSensor(object):
    ...
    def full_data_off(cp):
        ...
        cp.SENSOR_TOPIC = 'mesh/' + cp.SENSOR_ROOT + '/toDevice'

        ndns_mesh.MESH.multiline_payload(cp.SENSOR_IP,cp.SENSOR_PORT,60, cp.SENSOR_TOPIC,"", payload_msg)
    ...
    def request_version(cp,sv):
        ...
        print("Published sensor version request to {}. ip: {}. target: {}".format(cp.SENSOR_TOPIC, cp.SENSOR_IP, cp.SENSOR_TARGET))
        temp = 0
        while (1):
            if sv.string != []:
