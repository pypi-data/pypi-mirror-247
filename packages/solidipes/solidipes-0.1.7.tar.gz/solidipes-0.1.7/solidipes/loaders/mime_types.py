...
mime_types2extensions = {}
for line in _mime_types.split("\n"):
    if line == "":
        continue
    s = line.split()
    _type = s[0].strip()
    _exts = [e.strip() for e in s[1:]]
    mime_types2extensions[_type] = _exts
...
def get_possible_extensions(mime):
    mime = mime.split(";")[0]
    try:
        extensions = mime_types2extensions[mime]
    except KeyError:
        return []
    return extensions
...
def get_mime_type(path):
    ...
    if ext in extension2mime_type:
        return extension2mime_type[ext], ""
