...
hicCorrectMatrix = os.path.dirname(current_file_path) + '/' + 'hicCorrectMatrix.py'
...
def pp(opt, fastq,log_out):
    ...
    if qc == 0:
        ...
        cmd = 'samtools view -h -@ %s -b -q %s %s.bam > %s.qc.bam' % (opt['thread'], qc, fastq,fastq)
        # os.system(cmd)


    cmd = 'samtools view -h -@ ' + str(opt['thread'])
    if qc == 0 :
        cmd += ' -b -q 30 ' + fastq + '.bam > ' + fastq + '.filtered.bam'
    else:
        cmd += ' -b -q 30 ' + fastq + '.qc.bam > ' + fastq + '.filtered.bam'
    t = time.time()
    logging.info(cmd)
    # os.system(cmd)
