...
class SmbClient(SMBConnection):
    ...
    def upload_file(self, path: AnyStr, data: AnyStr | SupportsBytes | IO):
        ...
        while True:
            try:
                ...
                elapse: int = int((time.time() - t1) * 1000)
                logger.success(
                    f"文件上传成功 PID={os.getpid()} " f"TIME={elapse:3d}ms SIZE={size / 1024 ** 2:4.1f}MB PATH={path}"
                )
                return
            except NotConnectedError:
                self.reload()
                ...
                logger.error(f"[{e.__class__.__name__}]{e}")
                logger.error(
                    f"文件上传失败 PID={os.getpid()} " f"TIME={elapse:3d}ms SIZE={size / 1024 ** 2:4.1f}MB PATH={path}"
                )
                raise e
            finally:
    ...
    def upload_dir(self, path: AnyStr, source: AnyStr):
        ...
        try:
            ...
            elapse: int = int((time.time() - t1) * 1000)
            logger.error(f"[{e.__class__.__name__}]{e}")
            logger.error(f"上传目录失败 PID={os.getpid()} TIME={elapse:3d}ms PATH={path}")
            raise e
    ...
    def download_file(self, path: AnyStr, target: AnyStr | io.IOBase | None = None) -> io.BytesIO | None:
        ...
        while True:
            try:
                ...
                size: int = bio.tell()
                logger.success(
                    f"文件下载成功 PID={os.getpid()} TIME={elapse:3d}ms SIZE={size / 1024 ** 2:4.1f}MB PATH={path}"
                )
                if target is None:
                    return bio
                else:
                ...
                elapse: int = int((time.time() - t1) * 1000)
                logger.error(f"[{e.__class__.__name__}]{e}")
                logger.error(f"文件下载失败 PID={os.getpid()} TIME={elapse:3d}ms PATH={path}")
                raise e
            finally:
                bio.seek(0)
    ...
    def download_dir(self, path: AnyStr, target: AnyStr):
        ...
        try:
            ...
            elapse: int = int((time.time() - t1) * 1000)
            logger.error(f"[{e.__class__.__name__}]{e}")
            logger.error(f"下载目录失败 PID={os.getpid()} TIME={elapse:3d}ms PATH={path}")
            raise e
        ...
        while True:
            try:
                if self.isfile(path=path):
                    ...
                    elapse: int = int((time.time() - t1) * 1000)
                    logger.trace(f"文件删除成功 PID={os.getpid()} TIME={elapse:3d}ms PATH={path}")
                    ...
                    elapse: int = int((time.time() - t1) * 1000)
                    logger.trace(f"目录删除成功 PID={os.getpid()} TIME={elapse:3d}ms PATH={path}")
                return
            except NotConnectedError:
                self.reload()
                logger.debug(f"SMB连接成功 PID={os.getpid()}")
                ...
                elapse: int = int((time.time() - t1) * 1000)
                logger.error(f"[{e.__class__.__name__}]{e}")
                logger.error(f"文件删除失败 PID={os.getpid()} TIME={elapse:3d}ms PATH={path}")
                raise e
