...
class _WallarooAuth(requests.auth.AuthBase):
...
def create(keycloak_addr: str, auth_type: Union[AuthType, str, None]) -> _WallarooAuth:
    ...
    :return: Auth object that can be passed to all `requests` calls
    :rtype: AuthBase
    ...
    elif auth_type == None or auth_type == AuthType.NONE:
        return _NoAuth()
    else:
        # TODO: Error?
...
class _KeycloakTokenFetcher(_TokenFetcher):
    ...
    def Fetch(self) -> _AccessToken:
        ...
        external_url = os.environ.get("WALLAROO_SDK_AUTH_ENDPOINT", self.address)
        verification_uri_complete = res_data["verification_uri_complete"].replace(
            self.address, external_url, 1
        )
...
class _PlatformAuth(_WallarooAuth):
    ...
    This object should be constructed once and then passed to every `requests`
    ...
    def __call__(self, req: requests.PreparedRequest) -> requests.PreparedRequest:
        ...
        req.headers["Authorization"] = self._bearer_token_str()
        return req
