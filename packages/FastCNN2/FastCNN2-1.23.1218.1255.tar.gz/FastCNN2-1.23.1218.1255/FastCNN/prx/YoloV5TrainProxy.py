...
def doTrain(projectid,modelid):
    ...
    weightspath = os.path.join(weightspath,config["Type"]+".pt")
    
    validweight = PathProxy.getValidCKPT(projectid,modelid)
    if os.path.exists(validweight):
        weightspath = validweight
    ...
    parser.add_argument('--weights', type=str, default=weightspath, help='initial weights path')
    parser.add_argument('--resume', nargs='?', const=True, default=False, help='resume most recent training')
