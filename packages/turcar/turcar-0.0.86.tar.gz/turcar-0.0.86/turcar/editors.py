...
class EditorNotebook(CustomNotebook):
    ...
    def _cmd_course_update(self):
                ...
                "childer": [
                    {
                        ...
                        "childer": [
                            {
                                "name": "变量.mp4",
                                "file_url": "https://snowyp.oss-cn-chengdu.aliyuncs.com/oceans.mp4"
                            },
                            {
                                "name": "分支.mp4",
                                "file_url": "https://snowyp.oss-cn-chengdu.aliyuncs.com/oceans.mp4"
        ...
        if not files:
            # 第一次，全量更新
            print('全量更新')
            self.create_folder(json_data, file_path)
        else:
            # 差异更新
            print('差异更新')

    def create_folder(self, data, parent_path):
        count = self.count_files(data)
        current = 1
        for item in data:
            ...
            if 'file_url' in item:
                ...
                with open(folder_path, 'wb') as f:
                    ...
                    if total_length is None:  # 没有内容长度头
                        ...
                        self.create_download_progress_view(total_length, f, item, response, str(str(current) + '/' + str(count)))
                current += 1
            else:
                # 否则就是文件夹，先创建文件夹，再递归调用 create_folder
                os.makedirs(folder_path, exist_ok=True)
                self.create_folder(item['childer'], folder_path)
    ...
    def create_download_progress_view(self, total_length, f, item, response, count):
        ...
        for data in response.iter_content(chunk_size=4096):
            ...
            if total_length != 0:
                ...
                pro = "0.00"
            self.show(pro, '当前下载视频为--' + item['name'] + '--,' + "进度为:--" + str(pro) + '%--')
