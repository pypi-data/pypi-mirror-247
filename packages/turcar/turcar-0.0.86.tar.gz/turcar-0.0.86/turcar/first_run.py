...
from turcar import is_portable, languages, ui_utils
...
class FirstRunWindow(tk.Tk):
    def __init__(self, configuration_manager):
        super().__init__(className="turcar")
        ttk.Style().theme_use(ui_utils.get_default_basic_theme())

        self.title("Welcome to turcar!" + "   [portable]" if is_portable() else "")
        ...
        logo_file = os.path.join(os.path.dirname(__file__), "res", "turcar.png")
