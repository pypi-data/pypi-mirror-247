...
from turcar.common import Record, UserError, normpath_with_actual_case
from turcar.config import try_load_configuration
from turcar.config_ui import ConfigurationDialog
from turcar.custom_notebook import CustomNotebookPage
from turcar.editors import EditorNotebook, is_local_path
from turcar.languages import tr
from turcar.login import UserLogin
from turcar.check_for_updates import CheackUpdates
from turcar.misc_utils import (
    copy_to_clipboard,
    get_menu_char,
    running_on_linux,
    running_on_mac_os,
...
from turcar.running import BackendProxy, Runner
...
OBSOLETE_PLUGINS = [
    "turcarcontrib.pi",
    "turcarcontrib.micropython",
    "turcarcontrib.circuitpython",
    "turcarcontrib.microbit",
    "turcarcontrib.esp",
    "turcarcontrib.rpi_pico",
]


class Workbench(tk.Tk):
    """
    turcar's main window and communication hub.
    ...
    def __init__(self) -> None:
        ...
        # cheack.cheack_version()
        logger.info("Starting Workbench")
        turcar._workbench = self
        self.ready = False
        self._closing = False
        ...
        tk.Tk.__init__(self, className="turcar")
        ...
        self._active_ui_mode = os.environ.get("turcar_MODE", self.get_option("general.ui_mode"))
        ...
        self._try_action(self._restore_active_views)
        ...
        for name in sorted(sys.modules):
            if (
                ...
                and not name.startswith("turcar")
    ...
    def _make_sanity_checks(self):
        ...
        if bad_home_msg:
            messagebox.showwarning(
                ...
                bad_home_msg + "\nThis may cause problems for turcar.",
                master=self,
            )
    ...
    def _init_configuration(self) -> None:
        self._configuration_manager = try_load_configuration(turcar.CONFIGURATION_FILE)
        self._configuration_pages = []  # type: List[Tuple[str, str, Type[tk.Widget], int]]

        self.set_default("general.single_instance", turcar.SINGLE_INSTANCE_DEFAULT)
    ...
    def update_debug_mode(self):
        os.environ["turcar_DEBUG"] = str(self.get_option("general.debug_mode", False))
        turcar.set_logging_level()
    ...
    def _init_window(self) -> None:
        ...
        # even if they were True when turcar was last run
    ...
    def _init_icon(self) -> None:
        ...
        if running_on_linux() and ui_utils.get_tk_version_info() >= (8, 6):
            self.iconphoto(True, self.get_image("turcar.png"))
        else:
            icon_file = os.path.join(self.get_package_dir(), "res", "turcar.ico")
            try:
    ...
    def _load_plugins(self) -> None:
        # built-in plugins
        import turcar.plugins  # pylint: disable=redefined-outer-name

        self._load_plugins_from_path(turcar.plugins.__path__, "turcar.plugins.")  # type: ignore

        # 3rd party plugins from namespace package
        # Now it's time to add plugins dir to sys path
        sys.path.append(turcar.get_sys_path_directory_containg_plugins())
        try:
            import turcarcontrib  # @UnresolvedImport
        except ImportError:
            # No 3rd party plugins installed
            pass
        else:
            self._load_plugins_from_path(turcarcontrib.__path__, "turcarcontrib.")
    ...
    '''
    这段代码实现了一个用于处理来自其他 turcar 实例的文件打开请求的 IPC 服务器。
    新请求会通过套接字传递，然后通过队列存储，以便主线程可以异步处理这些请求
    '''

    def _check_init_server_loop(self) -> None:
        """Socket will listen requests from newer turcar instances,
        which try to delegate opening files to older instance"""

        if not self.get_option("general.single_instance") or os.path.exists(
                turcar.get_ipc_file_path()
        ):
            print("1111 the general.single_instance is null or turcar.get_ipc_file_path() is true")
    ...
    '''
    这段代码负责在 turcar 启动时创建 IPC 服务器套接字，以便其他 turcar 实例可以通过此套接字发送请求。
    '''
    ...
    def _create_server_socket(self):
        if running_on_windows():
            ...
            print("win10 _create_server_socket succee")
            with open(turcar.get_ipc_file_path(), "w") as fp:
                fp.write(str(port) + "\n")
                fp.write(secret + "\n")

        else:
            server_socket = socket.socket(socket.AF_UNIX)  # @UndefinedVariable
            server_socket.bind(turcar.get_ipc_file_path())
            secret = ""
        print("linux _create_server_socket succee")
        os.chmod(turcar.get_ipc_file_path(), 0o600)
    ...
    #     webbrowser.open("https://github.com/turcar/turcar/wiki/Support-Ukraine")
    ...
    def _post_backend_menu(self):
        from turcar.plugins.micropython.uf2dialog import (
            show_uf2_installer,
        ...
        if self.winfo_screenwidth() / self.winfo_screenheight() > 2:
            ...
            # Tk will adjust x properly with single monitor, but when turcar is maximized
    ...
    def _publish_command(
        ...
        if sequence:
            if not skip_sequence_binding:
                ...
                # https://github.com/turcar/turcar/issues/1347
        ...
        def on_click(event):
            ...
            tk.messagebox.showinfo(
                ...
                tr(
                    ...
                    + "Restart turcar to start working in regular mode.\n\n"
    ...
    def update_image_mapping(self, mapping: Dict[str, str]) -> None:
        """Was used by turcar-pi. Not recommended anymore"""
    ...
    def get_package_dir(self):
        """Returns turcar package directory"""
        return os.path.dirname(sys.modules["turcar"].__file__)
    ...
    def show_view(self, view_id: str, set_focus: bool = True) -> Union[bool, tk.Widget]:
        ...
        if containing_notebook is None:
            ...
            logger.info("Adding view %r to notebook %s", view, location)
            notebook.insert("auto", view, text=label)  # type: ignore
    ...
    def in_debug_mode(self) -> bool:
        return os.environ.get("turcar_DEBUG", False) in [
            "1",
            1,
            "True",
            True,
        ...
        if self._toolbar.winfo_ismapped():
            for group_frame in self._toolbar.grid_slaves(0):
                for button in group_frame.pack_slaves():
                    if turcar._runner is None or button.tester and not button.tester():
    ...
    def _guard_font_size(self, size: int) -> int:
        # https://bitbucket.org/plas/turcar/issues/164/negative-font-size-crashes-turcar
        MIN_SIZE = 4
        MAX_SIZE = 200
        if size < MIN_SIZE:
            return MIN_SIZE
    ...
    def destroy(self) -> None:
        try:
            ...
            if self._is_server() and os.path.exists(turcar.get_ipc_file_path()):
                os.remove(turcar.get_ipc_file_path())
    ...
    def _restore_active_views(self) -> None:
        for nb_name in self._view_notebooks:
    ...
    def open_url(self, url):
        m = re.match(r"^turcar-editor://(.*?)(#(\d+)(:(\d+))?)?$", url)
        if m is not None:
    ...
    def get_temp_dir(self, create_if_doesnt_exist=True):
        path = os.path.join(turcar_USER_DIR, "temp")
    ...
    def show_notebook_drop_targets(self):
        from turcar.custom_notebook import NotebookTabDropTarget
