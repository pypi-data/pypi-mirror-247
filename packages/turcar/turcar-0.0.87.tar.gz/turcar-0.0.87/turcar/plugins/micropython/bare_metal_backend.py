...
from turcar.backend import UploadDownloadMixin, convert_newlines_if_has_shebang
...
from turcar.misc_utils import find_volumes_by_name
from turcar.plugins.micropython.connection import MicroPythonConnection
from turcar.plugins.micropython.mp_back import (
    EOT,
    NORMAL_MODE_CMD,
    PASTE_MODE_CMD,
...
from turcar.plugins.micropython.mp_common import (
    PASTE_SUBMIT_MODE,
    RAW_PASTE_SUBMIT_MODE,
    RAW_SUBMIT_MODE,
)
from turcar.plugins.micropython.webrepl_connection import WebReplConnection
...
logger = getLogger("turcar.plugins.micropython.bare_metal_backend")
...
class BareMetalMicroPythonBackend(MicroPythonBackend, UploadDownloadMixin):
    ...
    def _check_prepare(self):
        out, err = self._execute(
            "__turcar_helper.builtins.print('__turcar_helper' in __turcar_helper.builtins.dir())",
            capture_output=True,
        )
        if out.strip() == "True":
            return
    ...
    def _get_helper_code(self):
        ...
        if self._read_block_size and self._read_block_size > 0:
            ...
            from turcar.plugins.micropython.serial_connection import OUTPUT_ENQ
            ...
            result += indent(
                ...
                def controlled_print(cls, *args, sep=" ", end="\\n"):
                    for arg_i, arg in enumerate(args):
                        ...
                        data = __turcar_helper.builtins.str(arg)
                        
                        for i in __turcar_helper.builtins.range(0, __turcar_helper.builtins.len(data), cls._print_block_size):
    ...
    def _infer_write_block_size(self):
        ...
        # https://github.com/turcar/turcar/issues/2143
    ...
    def _fetch_builtin_modules(self):
        script = "__turcar_helper.builtins.help('modules')"
    ...
    def _sync_time(self):
        ...
        if self._using_microbit_micropython():
            ...
            specific_script = dedent(
                """
                from rtc import RTC as __turcar_RTC
                __turcar_RTC().datetime = {ts}
                del __turcar_RTC
            """
            ).format(ts=tuple(now))
        else:
            ...
            specific_script = dedent(
                """
                from machine import RTC as __turcar_RTC
                try:
                    __turcar_RTC().datetime({datetime_ts})
                except:
                    __turcar_RTC().init({init_ts})
                finally:
                    del __turcar_RTC
        ...
        script = (
            ...
            try:
            %s
                __turcar_helper.print_mgmt_value(True)
            except __turcar_helper.builtins.Exception as e:
                __turcar_helper.print_mgmt_value(__turcar_helper.builtins.str(e))
        """
            )
    ...
    def _get_utc_timetuple_from_device(self) -> Union[tuple, str]:
        if self._using_microbit_micropython():
            ...
            specific_script = dedent(
                """
                from rtc import RTC as __turcar_RTC
                __turcar_helper.print_mgmt_value(__turcar_helper.builtins.tuple(__turcar_RTC().datetime)[:6])
                del __turcar_RTC
                """
            )
        else:
            specific_script = dedent(
                ...
                try:
                    ...
                    __turcar_temp = __turcar_helper.builtins.tuple(__turcar_RTC().datetime())
                    # remove weekday from index 3
                    __turcar_helper.print_mgmt_value(__turcar_temp[0:3] + __turcar_temp[4:7])
                    del __turcar_temp
                except:
                    __turcar_helper.print_mgmt_value(__turcar_helper.builtins.tuple(__turcar_RTC().now())[:6])
                del __turcar_RTC
                """
            )
    ...
    def _get_actual_time_tuple_on_device(self):
        script = dedent(
            ...
            try:
                try:
                    from time import localtime as __turcar_localtime
                    __turcar_helper.print_mgmt_value(__turcar_helper.builtins.tuple(__turcar_localtime()))
                    del __turcar_localtime
                except:
                    # some CP boards
                    from rtc import RTC as __turcar_RTC
                    __turcar_helper.print_mgmt_value(__turcar_helper.builtins.tuple(__turcar_RTC().datetime))
                    del __turcar_RTC
            except __turcar_helper.builtins.Exception as e:
                __turcar_helper.print_mgmt_value(__turcar_helper.builtins.str(e))
        """
    ...
    def _clear_repl(self):
        """NB! assumes prompt and may be called without __turcar_helper"""
    ...
    def _submit_code_via_raw_paste_mode(self, script_bytes: bytes) -> None:
        ...
        # https://github.com/turcar/turcar/issues/1545
    ...
    def _process_output_until_active_prompt(
        ...
        Note that this Ctrl+A may have been issued even before turcar connected to
        the device.
        ...
        output or the starting EOT from turcar expression (or, in principle, even
        the second raw-REPL EOT or terminating turcar expression EOT)
        ...
        if cmd.get("source"):
            ...
            if cmd.get("populate_argv", False):
                ...
                argv_updater = textwrap.dedent(
                    f"""
                try:
                    import sys as _turcar_sys
                    _turcar_sys.argv[:] = {argv}
                    del __turcar_sys
                except:
                    pass
                """
    ...
    def _cmd_get_fs_info(self, cmd):
        if self._using_microbit_micropython():
            used = self._evaluate(
                dedent(
                    """
                    __turcar_helper.print_mgmt_value(
                        __turcar_helper.builtins.sum([__turcar_helper.os.size(name) for name in __turcar_helper.os.listdir()])
                    )  
                    """
                )
            )
            ...
            result = self._evaluate(
                dedent(
                    """
                __turcar_stat = __turcar_helper.os.statvfs(%r)
                __turcar_total = __turcar_stat[2] * __turcar_stat[0]
                __turcar_free = __turcar_stat[3] * __turcar_stat[0]

                __turcar_helper.print_mgmt_value({
                    "total" : __turcar_total,
                    "used" : __turcar_total - __turcar_free,
                    "free": __turcar_free,
                })  
    ...
    def _delete_sorted_paths(self, paths):
        if not self._supports_directories():
            ...
            self._execute_without_output(
                dedent(
                    """
                for __turcar_path in %r: 
                    __turcar_helper.os.remove(__turcar_path)
    ...
    def _read_file_via_serial(
        ...
        self._execute_without_output(
            "__turcar_fp = __turcar_helper.builtins.open(%r, 'rb')" % source_path
        )
        if hex_mode:
        ...
        while True:
            ...
            if hex_mode:
                block = binascii.unhexlify(
                    self._evaluate("__temp_hexlify(__turcar_fp.read(%s))" % block_size)
                )
            else:
                block = self._evaluate("__turcar_fp.read(%s)" % block_size)
    ...
    def _write_file_via_serial(
        ...
        out, err = self._execute(
            dedent(
                ...
                __turcar_path = '{path}'
                __turcar_written = 0
                __turcar_fp = __turcar_helper.builtins.open(__turcar_path, 'wb')
            """
        ...
        if hex_mode:
            self._execute_without_output(
                dedent(
                    """
                from binascii import unhexlify as __turcar_unhex
                def __W(x):
                    global __turcar_written
                    __turcar_written += __turcar_fp.write(__turcar_unhex(x))
                    __turcar_fp.flush()
                    if __turcar_helper.builtins.hasattr(__turcar_helper.os, "sync"):
                        __turcar_helper.os.sync()
            """
                )
            )
            ...
            self._execute_without_output(
                ...
                def __W(x):
                    global __turcar_written
                    __turcar_written += __turcar_fp.write(x)
            """
                )
            )
        else:
        ...
        bytes_received = self._evaluate("__turcar_written")
    ...
    def _get_fs_mount_label(self):
        ...
        result = self._evaluate(
            dedent(
                """
            try:
                from storage import getmount as __turcar_getmount
                try:
                    __turcar_result = __turcar_getmount("/").label
                finally:
                    del __turcar_getmount
            except __turcar_helper.builtins.ImportError:
                __turcar_result = None 
            except __turcar_helper.builtins.OSError:
                __turcar_result = None 
            
            __turcar_helper.print_mgmt_value(__turcar_result)
...
def launch_bare_metal_backend(backend_class: Callable[..., BareMetalMicroPythonBackend]) -> None:
    ...
    try:
        if args["port"] is None:
            ...
            from turcar.plugins.micropython.serial_connection import (
