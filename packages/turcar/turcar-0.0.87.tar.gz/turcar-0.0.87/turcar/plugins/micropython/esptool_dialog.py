...
from turcar.common import normpath_with_actual_case
from turcar.misc_utils import get_menu_char, running_on_windows
from turcar.plugins.micropython import BareMetalMicroPythonProxy, list_serial_ports
from turcar.plugins.micropython.base_flashing_dialog import (
    BaseFlashingDialog,
    TargetInfo,
    family_code_to_name,
)
from turcar.plugins.micropython.mp_front import get_serial_port_label
from turcar.running import get_front_interpreter_for_subprocess
from turcar.ui_utils import EnhancedBooleanVar, MappingCombobox
...
class ESPFlashingDialog(BaseFlashingDialog):
    ...
    def get_variants_url(self) -> str:
        return f"https://raw.githubusercontent.com/turcar/turcar/master/data/{self.firmware_name.lower()}-variants-esptool.json"
    ...
    def prepare_work_get_options(self) -> Dict[str, Any]:
        ...
        port_was_used_in_turcar = (
            isinstance(proxy, BareMetalMicroPythonProxy) and proxy._port == target.port.device
        )
        if port_was_used_in_turcar and self._work_needs_disconnect():
        ...
        return {
            ...
            "port_was_used_in_turcar": port_was_used_in_turcar,
        }
    ...
    def perform_core_operation(
        ...
        if self._work_needs_disconnect() and work_options["port_was_used_in_turcar"]:
