...
from turcar import get_workbench, languages, tktextext
from turcar.config_ui import ConfigurationPage
from turcar.languages import tr
from turcar.misc_utils import running_on_linux
...
class GeneralConfigurationPage(ConfigurationPage):
    def __init__(self, master):
        ...
        self.add_checkbox(
            "general.single_instance", tr("Allow only single turcar instance"), row=1, columnspan=2
        )
        self.add_checkbox(
        ...
        reopen_label = ttk.Label(
            self,
            text=tr("NB! Restart turcar after changing these options!"),
