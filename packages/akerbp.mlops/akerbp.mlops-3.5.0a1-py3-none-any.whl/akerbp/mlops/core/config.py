...
class CdfClientSecrets:
    ...
    osdu_data_partition_id: Optional[str] = None
    # TODO: separate osdu secrets from cdf secrets in a later release. For now, we keep them together
...
def read_cdf_client_secrets() -> Dict[str, str]:
    secrets = asdict(
        CdfClientSecrets(
            ...
            osdu_id=os.getenv("OSDU_CLIENT_ID", "not found"),
            osdu_secret=os.getenv("OSDU_CLIENT_SECRET", "not found"),
            osdu_base_url=os.getenv("OSDU_API_BASE_URL", "not found"),
            osdu_data_partition_id=os.getenv("OSDU_DATA_PARTITION_ID", "not found"),
        )
    )
...
class ServiceSettings:
    ...
    helper_models: Optional[List[str]] = None
...
def validate_python_runtime(python_runtime: str) -> None:
    ...
    allowed_runtimes = ["py37", "py38", "py39", "py310"]
