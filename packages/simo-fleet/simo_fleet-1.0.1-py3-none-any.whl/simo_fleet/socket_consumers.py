...
class FleetConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if not self.colonel.name and headers.get('colonel-name'):
            def save_colonel_name(name):
                self.colonel.name = headers['colonel-name']
                self.colonel.save()
    ...
    async def watch_connection(self):
        while self.connected:
            ...
            # Default system only sends pings to colonels if nothing
            # is received from connected socket consumer,
            # therefore we must send this ping periodically
            # so that our colonel knows that the socket is still connected.
