...
class DsetSimMat(object):
        ...
        if ignore_overlaps:
            ...
            while i < len(_ends):
                ...
                for comppos, compend, compsim in _ends[i+1:]:
                    if comppos[direction] >= end[direction]:
                        break
                    if not (comppos[~direction+2] < pos[~direction+2] < compend[~direction+2]):
                        continue
                    if compsim > sim:
    ...
    def filter(self, similarity, ignore_overlaps=False, direction=0) -> TargetList:
        ...
        return TargetList._new_((xlist, ylist, lambda x,y: Target._DotsetTarget((x,y), *self.get(x,y))))
    ...
    def separate(self, similarity, ignore_overlaps=False, direction=0) -> list[TargetList]:
        ...
        for pos in zip(xlist, ylist):
            ...
            if pos[not direction] > groupBorderEnd:
                groups.append([])
            groups[-1].append(Target._DotsetTarget(pos, dotset, sim))
