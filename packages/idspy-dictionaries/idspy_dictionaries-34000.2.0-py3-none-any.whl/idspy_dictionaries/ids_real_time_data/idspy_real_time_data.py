...
class RtdAllocatableSignals(IdsBaseClass):
    ...
    :ivar data_type: Signal data type
    :ivar allocated_position: Allocation of signal to a position in the
        SDN (1..N), or several positions in case of signal reshaping;
        this will be implementation specific
    :ivar data_str: Signal data (e.g. diagnostic signal or PCS command),
    ...
    allocated_position: ndarray[(int,), int] = field(
        default_factory=list,
        metadata={
            "max_occurs": 999,
        }
    )
    data_str: Optional[list[str]] = field(
        default=None
    )
    quality: ndarray[(int,), int] = field(
        default_factory=list,
        metadata={
            "max_occurs": 999,
        }
...
class RtdTopic(IdsBaseClass):
    ...
    :ivar signal: List of signals that are allocated to the PCS
        interface
    :ivar time_stamp: Time of creation for all signals belonging to this
        topic. A set of time stamps can be recorded if needed
    :ivar sample: Index of sample (time stamp counter)
    """
    class Meta:
        name = "rtd_topic"
    ...
    time_stamp: ndarray[(int,), float] = field(
        default_factory=list,
        metadata={
            "max_occurs": 999,
        }
    )
    sample: ndarray[(int,), int] = field(
        default_factory=list,
        metadata={
            "max_occurs": 999,
        }
