...
class Profiles2DGrid(IdsBaseClass):
    """
    Definition of the 2D grid with dim1, dim2, constant.

    :ivar dim1: First dimension values
    :ivar dim2: Second dimension values
    :ivar volume_element: Volume of plasma enclosed in the cell formed
        by the nodes [dim1(i) dim2(j)], [dim1(i+1) dim2(j)], [dim1(i)
        dim2(j+1)] and [dim1(i+1) dim2(j+1)]
    """
    class Meta:
...
class BolometerChannel(IdsBaseClass):
    ...
    :ivar line_of_sight: Description of the reference line of sight of
        the channel, defined by two points when the beam is not
...
class Bolometer(IdsBaseClass):
    ...
    :ivar power_radiated_total: Total radiated power reconstructed from
        bolometry data
    :ivar power_radiated_inside_lcfs: Radiated power from the plasma
        inside the Last Closed Flux Surface, reconstructed from
        bolometry data
    :ivar power_radiated_validity: Validity flag related to the radiated
        power reconstructions
    :ivar grid_type: Selection of one of a set of grid types for the 2D
        power density map
    :ivar grid: Definition of the 2D grid (the content of dim1 and dim2
        is defined by the selected grid_type)
    :ivar power_density: Power density map in the poloidal cross-
        section, obtained from tomographic inversion of the bolometer
        data
    ...
    channel: list[BolometerChannel] = field(
        default_factory=list,
        metadata={
            "max_occurs": 500,
        }
    )
    power_radiated_total: ndarray[(int,), float] = field(
        default_factory=list,
        metadata={
            "max_occurs": 999,
        }
    )
    power_radiated_inside_lcfs: ndarray[(int,), float] = field(
        default_factory=list,
        metadata={
            "max_occurs": 999,
        }
    )
    power_radiated_validity: ndarray[(int,), int] = field(
        default_factory=list,
        metadata={
            "max_occurs": 999,
        }
    )
    grid_type: Optional[Identifier] = field(
        default=None
    )
