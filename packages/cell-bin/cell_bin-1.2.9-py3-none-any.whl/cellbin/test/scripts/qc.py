import os
vipsbin = r'C:\Users\wuyiwen\Downloads\vips-dev-w64-web-8.10.5\vips-dev-8.10\bin'
...
if __name__ == '__main__':
    ...
    # main()
    for ipr_file in glob.glob(r'D:\data_copy\nan_chip\**\*.ipr'):
        # chipname = os.path.basename(stitch_transform_img).split('.')[0]
        # print(chipname, stitch_transform_img)
        # main_qc(
        #     stain_type='DAPI',
        #     src_data=stitch_transform_img,
        #     chip_name=chipname,
        #     fov_size=tuple(DEFAULT_FOV_SIZE),
        #     magnification=DEFAULT_MAGNIFICATION,
        #     save_dir=fr'D:\data_copy\nan_chip\{chipname}',
        #     debug_mode=False,
        #     zoo_dir=r'D:\Cell_Bin\weights',
        #     config_path=r'D:\git_libs\cellbin\cellbin\cellbin\test\scripts\qc_config.json'
        # )
        f = h5py.File(ipr_file, 'r')
        a = np.array(f['Research']['Stitch']['GlobalTemplate'])
        np.savetxt(os.path.join(os.path.dirname(ipr_file), 'matrix_template.txt'), a)