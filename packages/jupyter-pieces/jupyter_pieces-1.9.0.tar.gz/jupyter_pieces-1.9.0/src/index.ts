...
import CopilotLLMConfigModal from 'copilot/dist/modals/LLMConfigModal';
import { initCopilot } from 'copilot/dist/view';
import { copilotParams, copilotState } from './ui/views/copilot';
  ...
  activate: async (
    ...
    app.restored
      .then(async () => {
        ...
        if (data) {
          CopilotLLMConfigModal.selectedModel = data.selectedModel ?? '';
          copilotState.selectedModel = data.selectedModel ?? '';

          copilotState.directives = data.directives;
        }
      })
      .finally(async () => {
    ...
    // TODO reimplement intention and suggestion
    // document.addEventListener('copy', (e) => {
    //   const selection = window.getSelection()?.toString();
    //   if (selection) {
    //     const intention: IntentionRequest = {
    ...
    //             },
    //             metadata: {
    //               ext: ClassificationSpecificEnum.Py,
    //             },
    //           },
    //         },
    //       },
    //     };
    //     try {
    //       config.api.intention(intention);
    //     } catch (e) {
    //       console.log('Pieces: Intent error', e);
    //     }
    //   }
    // });
