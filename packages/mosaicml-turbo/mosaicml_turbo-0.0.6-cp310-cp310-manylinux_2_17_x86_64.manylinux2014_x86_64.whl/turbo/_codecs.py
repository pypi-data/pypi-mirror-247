...
class LambdaCodec(Codec):
    ...
    def __init__(self,
                 f_encode: Optional[Callable[[Tensor], Tensor]] = None,
                 f_decode: Optional[Callable[[Tensor], Tensor]] = None):
        self.f_encode = f_encode or (lambda x, out: x)
        self.f_decode = f_decode or (lambda x, out: x)
...
class SignedIntQuantizer(Codec):
    ...
    def _compressed_offsets(self, numel: int) -> Tuple[int, int, int]:
