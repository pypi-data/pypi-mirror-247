...
from typing import Any, Optional, Callable, AsyncIterator, Tuple, NamedTuple
...
from .aws import S3ClientContext
            ...
            try:
                ...
                async def _restoring_archive():
                    async for obj_sum in list_objects(s3_client, bucket_id=bucket_name, prefix=source_key_name):
                        ...
                        if obj.storage_class in (S3StorageClass.GLACIER.name, S3StorageClass.DEEP_ARCHIVE.name):
                ...
                await request.app[HEA_BACKGROUND_TASKS].add(_gather_restored_obj_tasks(request, s3_client, watch_restore_tasks, bucket_name,
                                                                                       source_key_name, activity_cb, activity), name=task_name)
...
async def watch_restore_status(get_resource: Callable[[], Awaitable[ServiceResource]],
...
async def delete_object_async(request: Request, status_location: URL | str, recursive=False,
                              ...
                              done_cb: Callable[[Response], Awaitable[None]] = None,
                              delete_completed_cb: Callable[[str, str], Awaitable[None]] | None = None) -> Response:
    """
    Deletes a single object. The volume id must be in the volume_id entry of the request's match_info dictionary. The
    bucket id must be in the bucket_id entry of the request's match_info dictionary. The item id must be in the id
...
def handle_client_error(e: BotoClientError) -> Response:
    logger = logging.getLogger(__name__)
...
async def list_objects(s3: S3Client,
                       ...
                       include_restore_status: bool | None = None) -> AsyncIterator[dict[str, Any]]:
    if not loop:
    ...
    while first_time or continuation_token:
        ...
        response_ = await loop_.run_in_executor(None, list_partial)
        continuation_token = response_['NextContinuationToken'] if response_['IsTruncated'] else None
        for obj in response_.get('CommonPrefixes', []):
            yield obj
...
async def list_object_versions(s3: S3Client, bucket_id: str, prefix: str | None = None,
                               ...
                               filter_folder: bool = False) -> AsyncIterator[dict[str, Any]]:
    """
    ...
    while first_time or continuation_token:
        ...
        if filter_deleted:
            deleted_markers: list[dict[str, str]] = response_.get('DeleteMarkers', [])
...
async def _gather_restored_obj_tasks(request: web.Request, s3_client: S3Client, tasks: list[Task],
                                     ...
                                     activity_cb, activity: DesktopObjectAction | None = None,
                                     restore_object_completed_cb: Callable[[str, str], Awaitable[None]] | None = None) -> None:
    """
    ...
    :param restore_object_completed_cb: called after successfully restoring each object (optional). The
    callable must be reentrant. Its two parameters are bucket name and key.
    key.
    :return: None
    """
    logger = logging.getLogger(__name__)
    if activity:
        ...
        activity.description = f'Restoring {_activity_object_display_name(bucket_name, source_key)}'
        if activity_cb:
            await activity_cb(request.app, activity)
    try:
        before = datetime.now()
        if logger.getEffectiveLevel() == logging.DEBUG:
            logger.debug("number of tasks to gather: %s", len(tasks))
        objects = {'Objects': [{'Key': k, 'VersionId': v} for k, v in await asyncio.gather(*tasks)]}
        after = datetime.now()
        diff = after - before
        if logger.getEffectiveLevel() == logging.DEBUG:
            logger.debug("Time to restore archive %s in minutes: %s", source_key, diff.total_seconds() / 60)
        if restore_object_completed_cb:
            async def copy_object_completed_cb(source_bucket, source_key, target_bucket, target_key):
                return await restore_object_completed_cb(source_bucket, source_key)
        else:
            copy_object_completed_cb = None
        cp_resp = await _copy_object(s3_client, bucket_name, source_key, bucket_name,
                                     source_key, copy_completed_cb=copy_object_completed_cb)
        ...
        if activity:
            ...
            if activity_cb:
                await activity_cb(request.app, activity)
        logger.exception('Failed to restore object')
        raise ex
...
async def _copy_object_extract_target(body: dict[str, Any]) -> TargetInfo:
    ...
    try:
        ...
        return TargetInfo(url=target_url, bucket_name=target_bucket_name, key=target_folder_name, volume_id=vars_['volume_id'])
