...
from aligo.core.Config import UNI_HEADERS
...
def openAutoBackup_uploadFile(aligo=None, access_token=None, time_sleep=0, file_num=0):
    if access_token is None:
        return False, 'access_token为空'
    if aligo is None:
        return False, 'aligo为空'
    ...
    for i in range(0, len(device_info['items'])):
        ...
        if autoStatus is True:
            break
    # # 如果相册自动备份为FALSE，则开启自动备份
    # if autoStatus is False:
    #     update_device_extras = requests.post(url='https://api.alipan.com/users/v1/users/update_device_extras',
    #                                          json={
    ...
    #     autoStatus = device_summary['albumBackupSetting']['autoStatus']
    #
    #     if update_device_extras['result'] is False or update_device_extras['success'] is False or autoStatus is False:
    #         return False, "开启自动备份失败"

    if device_id is None:
        return False, "所有手机设备均未开启自动备份，请开启自动备份"
    if file_num > 0:
        # 上传固定数量的文件
        upload_file(aligo=aligo, path='../aliyunpan_daily_task/file', time_sleep=2, file_num=file_num)
        openAutoBackup_oneHour(aligo=aligo, access_token=access_token, time_sleep=time_sleep)
        return True, "已开启自动备份并备份满10个文件"
    else:
        return True, "已开启自动备份"
...
def device_room_task(aligo=None, access_token=None, time_sleep=0):
    if aligo is None:
    ...
    #     2. 针对每个设备上传一张照片到阿里云盘备份任务文件夹，并删除本地文件，需要修改header中的x-device参数
    for i in range(0, len(device_info['items'])):
        # 使用不同的device上传一张照片
        aligo._auth.session.headers["x-device-id"] = device_info['items'][i]['id']
        # 上传一个文件
        message, is_success = upload_file(aligo=aligo, path='../aliyunpan_daily_task/file', file_num=1)
        if is_success is True:
            time.sleep(time_sleep)
        else:
            return False, '备份文件失败'

    # 3. 所有的设备上传了文件之后得到可以领取的设备，然后领取
    device_info = requests.post(url='https://user.aliyundrive.com/v1/deviceRoom/listDevice', headers={
        'Authorization': f'Bearer {access_token}'}).json()
    size_count = 0
    for i in range(0, len(device_info['items'])):
        # 是否可以领取奖励
        canCollectEnergy = device_info['items'][i]['canCollectEnergy']
        if canCollectEnergy is True:
            # 成功后领取奖励
            reward_get_info = requests.post(url="https://member.aliyundrive.com/v1/deviceRoom/rewardEnergy", headers={
                'Authorization': f'Bearer {access_token}'}, json={
                "deviceId": device_info['items'][i]['id']
            }).json()
    ...
    return True, f"获取永久空间成功，今日已获得{size_count}MB"
