...
from yaoys_checkin.aliyunpan_checkin.aliyunpan_task.aliyunpan_task import create_album, create_quick_pass, delete_file, delete_lastTime_album, delete_photo_from_album, device_room_task, fish_save, follow_user, get_device_room_info, get_lucky_bittle, openAutoBackup_oneHour, \
                ...
                try:
                    ...
                    for i in range(len(self.daily_task_json)):
                        ...
                        # 开启自动备份并备份满10个文件
                            ...
                            is_success, message = openAutoBackup_uploadFile(aligo=self.aligo, access_token=self.access_token, time_sleep=self.time_sleep, file_num=10)
                            break
        ...
        try:
            ...
            if is_checkin is True and 'result' in data and isinstance(data['result'], dict) is True:
                ...
                reward = (
                    ...
                    else f'获得{reward_data["result"]["name"]},{reward_data["result"]["notice"]}'
                )
            ...
            if "deviceRoom_task" in self.config_file['cookieOrUser']["aliyunpan"] and self.config_file['cookieOrUser']["aliyunpan"]['deviceRoom_task'] is True:
                ...
                if has_device_room:
                    is_success, device_room_task_info = device_room_task(aligo=self.aligo, access_token=self.access_token, time_sleep=self.time_sleep)
                    if is_success:
