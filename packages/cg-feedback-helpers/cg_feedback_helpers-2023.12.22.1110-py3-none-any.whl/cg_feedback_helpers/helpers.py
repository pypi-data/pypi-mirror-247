"""Helpers functions that implement boiler plate code used together with the
:class:`cg_feedback_helpers.assertions.Asserter`. It provides support for
...
# pascal case here.
class capture_output(redirect_stdout):  # pylint: disable=invalid-name,too-few-public-methods
    """This context manager allows you to capture the output printed to `stdout`
    in a :class:`io.StringIO` buffer.
...
# For ease of use, and in style with `contextlib`, we do not conform to
# pascal case here.
class as_stdin:  # pylint: disable=invalid-name
    """This context manager allows you to provide input to a program by passing
    in the input lines. It temporarily replaces the standard `stdin` buffer with
    one containing the specified input lines.
...
def get_lines_from_buffer(
    buffer: io.StringIO,
    *,
    extra_splits: t.List[str] | None = None,
) -> t.List[str]:
    ...
    .. code-block:: python
        ...
        ['something', 'goes', 'out']
    """
    if extra_splits is None or len(extra_splits) == 0:
