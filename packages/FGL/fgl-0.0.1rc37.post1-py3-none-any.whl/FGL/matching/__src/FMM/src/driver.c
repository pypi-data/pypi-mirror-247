...
void match_track(PyObject *edge_list, PyObject *paths, PyObject *trees, PyObject *dead, PyObject *matching, PyObject *result)
{
    ...
    // Record the starting time
    start_time_match = getTimeOfDay();
    M = MaximumCardinalityMatchingTrack(G, paths, trees, dead);
    end_time_match = getTimeOfDay();
    end_time_wall = getTimeOfDay();
