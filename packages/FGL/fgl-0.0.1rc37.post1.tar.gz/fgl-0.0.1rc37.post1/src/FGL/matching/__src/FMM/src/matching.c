...
#ifdef Ansi
    (Graph *G, PyObject *paths, PyObject *trees, PyObject *dead)
#else
    (G)
Graph *G;
#endif

{
   ...
   while ((V = ListGet(Roots)))
   {
      if (Search_Track(V, &P, &T))
      {
         ...
         if (PyList_Append(paths, path_length) == -1)
         {
         ...
         if (PyList_Append(trees, tree_size) == -1)
         {
            printf("Error appending py tuple object\n");
         }
         ...
         if (PyList_Append(dead, dead_verts) == -1)
         {
            printf("Error appending py tuple object\n");
         }
         Augment(P, T);
