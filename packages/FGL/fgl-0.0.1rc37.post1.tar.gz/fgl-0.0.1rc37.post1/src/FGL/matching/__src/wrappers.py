...
def max_cardinality_matching_track(G: networkx.Graph, paths: list, trees: list, dead: list, init_list: Optional[list] = []):
    edge_list = list(G.edges)
    result_matching = []
    __lib.match_track_wrapper(edge_list, paths, trees, dead, init_list, result_matching)
    return copy.deepcopy(result_matching)