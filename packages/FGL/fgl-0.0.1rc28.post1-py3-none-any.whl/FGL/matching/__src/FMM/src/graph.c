...
#ifdef Ansi
    (PyObject *rows, PyObject *cols, int *matching, int nr, int nc, int nn, int just_read_file)
#else
    (stream)
FILE *stream;
#endif

{
   ...
   */
   ...
   {
      PyObject *item1 = PyList_GetItem(rows, edge);
      if (!PyLong_Check(item1))
      {
         PyErr_SetString(PyExc_TypeError, "List elements must be integers");
         return NULL;
      }
      PyObject *item2 = PyList_GetItem(cols, edge);
      if (!PyLong_Check(item2))
      {
         PyErr_SetString(PyExc_TypeError, "List elements must be integers");
         return NULL;
