...
#ifdef Ansi
    (int *edge_list, int *matching, int nr, int nc, int nn, int just_read_file)
#else
    (stream)
FILE *stream;
#endif

{
   ...
   for (int edge = 0; edge < nn; ++edge)
   {
      ...
      long value1 = edge_list[2*edge];
      long value2 = edge_list[2*edge+1];
#ifndef NDEBUG
...
#ifdef Ansi
    (int *edge_list, int *matching, int nn)
#else
    (stream)
FILE *stream;
#endif
