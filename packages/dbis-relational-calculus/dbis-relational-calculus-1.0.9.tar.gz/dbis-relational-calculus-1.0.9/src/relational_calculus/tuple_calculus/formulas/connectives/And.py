...
class And(tc.Formula):
    """
    A class representing the logical connective 'and'.
    """
    ...
    def __repr__(self) -> str:
        ...
        if isinstance(self.children[0], And):
            ...
            if not isinstance(left_grandchild, And | tc.ATOM_TYPES | tc.Variable):
                left_repr = f"({left_repr})"
            if not isinstance(right_grandchild, And | tc.ATOM_TYPES | tc.Variable):
    ...
    def _rename_variable(
        ...
        if self.children[0] == old_variable and self.children[1] == old_variable:
            return And(new_variable, new_variable)
    ...
    def _check_variable_legality(self, variable: tc.Variable) -> bool:
        ...
        if self.children[0].contains_variable_typing(variable) and self.children[
            ...
            raise Exception("Each variable must be declared before it is used")
