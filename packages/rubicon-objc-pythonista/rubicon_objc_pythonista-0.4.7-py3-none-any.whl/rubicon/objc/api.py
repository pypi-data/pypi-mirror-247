...
class ObjCInstance:
    ...
    def __dir__(self):
        ...
        py_attrs = []
        while objc_class is not None:
            properties_ptr_count = c_uint(0)
            methods_ptr_count = c_uint(0)
            ivars_ptr_count = c_uint(0)

            properties_ptr = libobjc.class_copyPropertyList(objc_class, byref(properties_ptr_count))
            methods_ptr = libobjc.class_copyMethodList(objc_class, byref(methods_ptr_count))
            ivars_ptr = libobjc.class_copyIvarList(objc_class, byref(ivars_ptr_count))

            for i in range(properties_ptr_count.value):
                property_ptr = properties_ptr[i]
                name = libobjc.property_getName(property_ptr).decode("utf-8")
                if "." not in name:
                    py_attrs.append(name)

            for j in range(methods_ptr_count.value):
                method_ptr = methods_ptr[j]
                name = libobjc.method_getName(method_ptr).name.decode("utf-8")
                name = name.replace(":", "_")
                if "." not in name:
                    py_attrs.append(name)

            for k in range(ivars_ptr_count.value):
                ivar_ptr = ivars_ptr[k]
                name = libobjc.ivar_getName(ivar_ptr).decode("utf-8")
                if "." not in name:
                    py_attrs.append(name)

            libc.free(properties_ptr)
            libc.free(methods_ptr)
            libc.free(ivars_ptr)

            objc_class = objc_class.superclass

        return sorted(set(py_attrs))
...
class ObjCClass(ObjCInstance, type):
    ...
    def _new_from_class_statement(cls, name, bases, attrs, *, protocols):
        ...
        suffix = 1
        while True:
            if get_class(ensure_bytes(name)).value:
                suffix += 1
...
class ObjCProtocol(ObjCInstance):
    ...
    def __dir__(self):
        ...
        method_descriptions = libobjc.protocol_copyMethodDescriptionList(self, False, True, count)
        for i in range(count.value):
            method_description = method_descriptions[i]
            name = libobjc.sel_getName(method_description.name).decode("utf-8")
            py_attrs.append(name)

        libc.free(method_descriptions)
