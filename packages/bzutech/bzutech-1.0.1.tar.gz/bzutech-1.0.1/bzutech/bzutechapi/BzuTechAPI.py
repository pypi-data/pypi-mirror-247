...
class BzuTech:
    ...
    def start(self) -> bool:
        authenticated = asyncio.run(self._async_auth())
        self.httpheaders["Authorization"] = self._token
        contract = asyncio.run(self._async_set_contrato())
        got_devices = asyncio.run(self._async_set_dispositivos())

        return authenticated and contract and got_devices
    ...
    async def _async_auth(self) -> bool:
        ...
        async with client.post(url=url, data=data) as resp:
            ...
            await client.close()
            self._token = resposta["tokens"]["refresh"]
    ...
    async def _async_set_contrato(self) -> bool:
        ...
        async with client.get(url=url) as resp:
            resposta = await resp.json()
            await client.close()
            self._contratoid = resposta["empresas"][1]["contratos_id"]
            return True
        ...
        async with client.get(url=url, headers = self.httpheaders) as resp:
            ...
            for disp in resposta:
                if disp["status_dispositivo"] == 1:
                    ...
                    dispositivo = Device(chipid, dispname)
