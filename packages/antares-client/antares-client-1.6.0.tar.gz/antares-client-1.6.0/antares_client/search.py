...
from ._api.api import _get_raw_resource, _get_resource, _list_all_resources
from ._api.schemas import _GravWaveNoticeSchema, _LocusListingSchema, _LocusSchema
...
def get_latest_grav_wave_notices(gracedb_id: str) -> List[str]:
    return _get_resource(
        urljoin(
            config["ANTARES_API_BASE_URL"],
            "grav_wave_notices/{}/latest".format(gracedb_id),
        ),
...
def get_grav_wave_notices(
    ...
    return _get_resource(
        urljoin(
            config["ANTARES_API_BASE_URL"],
            "grav_wave_notices/{}/{}".format(gracedb_id, notice_datetime.isoformat()),
        ),
        _GravWaveNoticeSchema,
    )
