...
class DataRequestsClient:
    ...
    def download_data_request(self, data_request_id: UUID, index: int) -> dict:
        """
        Download the data for a specific data request.

        Parameters:
            data_request_id (str): Identifier of the data request to be downloaded.
        ...
        """
        self._ensure_client_open()
        response = self.client.get(f"/data-requests/download/{data_request_id}/{index}")
    ...
    def download_output(
        ...
        if index is None:
            ...
            output = [
                i
                for i, output in enumerate(data_request.analysis["outputs"])
            ...
            index = output[0]
        data = self.download_data_request(data_request_id, index)
        return data
