                ...
                if schemaName_Updated in schemaClasslist:
                    for key in queryJson:
                        if len(queryJson[key]) > 0 and queryJson[key].strip() != "":
                            ...
                            logger.log(f"Input ::: {queryJson[key]} \n Responsee:::: {response}")

                            if response != {}:

                                response_List = response['data']['Get'][schemaName_Updated]
                                finalResultJson[key]= {"material_description": response_List[0]['description'] , "id": response_List[0]['answer'] } if not "errors" in list(response.keys()) else response

                                for index in range(len(response_List)):
                                    descr   =  response_List[index]['description']
                                    id      =  response_List[index]['answer']
                                    
                                    if descr == queryJson[key]:
                                        logger.log(f"\n Input::: '{queryJson[key]}' MATCHEDD with description ::: '{descr}' \n")
                                        finalResultJson[key]    =  {"material_description": descr, "id": id }
                                        break
                                    else:
                                        logger.log(f"\n Input '{queryJson[key]}' not matched with returned response description '{descr} '\n ")    
        ...
        try:
            ...
            for key in queryJson:
                ...
                if len(key) > 0 and key.strip() != "":
                    ...
                    llm = ChatOpenAI(openai_api_key=self.openAI_apiKey, model_name='gpt-3.5-turbo', temperature=0)
