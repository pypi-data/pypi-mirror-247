...
from multiformats_config.multicodec import build_multicodec_tables
...
multicodecs = (multicodec.Multicodec(**{k.strip(): v.strip() for k, v in _row.items()})
               for _row in reader)
new_table, _ = build_multicodec_tables(multicodecs)

# Loads and validates the current multicodec table:
print("Building current multicodec table...")
with open("multiformats_config/multicodec-table.csv", "r") as f:
    current_text = f.read()
reader = csv.DictReader(io.StringIO(current_text))
multicodecs = (multicodec.Multicodec(**{k.strip(): v.strip() for k, v in _row.items()})
               for _row in reader)
current_table, _ = build_multicodec_tables(multicodecs)
...
if added or removed or changed:
    ...
    if answer.lower().startswith("y"):
        with open("multiformats_config/multicodec-table.csv", "w") as f:
            f.write(new_text)
        with open("multiformats_config/multicodec-table.json", "w") as f:
