...
def error_translate(err: ferr.Error, schema: Optional[frictionless.Schema] = None):
    ...
    # blank header
    elif isinstance(err, ferr.BlankHeaderError):
        err["name"] = "En-tête manquant"
        if len(err.row_positions) == 1:  # pyright: ignore
            err[
                "message"
            ] = f"La colonne n°{err.row_positions[0]} n'a pas d'entête."  # pyright: ignore
        else:
            pos_list = ", ".join(err.row_positions)  # pyright: ignore
            err["message"] = f"Les colonnes n°{pos_list} n'ont pas d'entête."
        return err

    # blank row
    elif isinstance(err, ferr.BlankRowError):
        err["name"] = "Ligne vide"
        err["message"] = "Les lignes vides doivent être retirées de la table."
        return err

    # extra cell
    elif isinstance(err, ferr.ExtraCellError):
    ...
    # type error
    elif isinstance(err, ferr.TypeError):
