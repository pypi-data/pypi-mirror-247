...
class JobApplicationForm(forms.ModelForm):
    ...
    position_applied = forms.CharField(
        max_length=100,
        label=_("Position Applied For"),
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': _('Position Applied For')}),
        required=True
    )
    salary_expectations = forms.DecimalField(
        max_digits=10,
        decimal_places=2,
        label=_("Salary Expectations"),
        widget=forms.NumberInput(attrs={'class': 'form-control', 'placeholder': _('Salary Expectations')}),
        required=True
    )
    available_start_date = forms.DateField(
        label=_("Available Start Date"),
        widget=forms.DateInput(attrs={'class': 'form-control datepicker', 'placeholder': _('Available Start Date')}),
        required=True
    )
    comments = forms.CharField(
        label=_("Additional Comments"),
        widget=forms.Textarea(attrs={'class': 'form-control', 'placeholder': _('Additional Comments')}),
        required=False
    )

    class Meta:
        model = JobApplication
        fields = ['first_name', 'last_name', 'email', 'phone', 'address', 'position_applied', 'salary_expectations', 'available_start_date', 'comments']

        # Esta es la manera predeterminada de incluir widgets para cada campo sin definirlos uno por uno.
        # widgets = {
