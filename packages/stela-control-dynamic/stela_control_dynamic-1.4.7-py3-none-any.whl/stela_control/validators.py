...
from .models import (
    ...
    Booking, BookingServices, City, Team
...
from .forms import (
    ...
    ConsultingForm, BookingConsultingForm, RegistrationForm, UserEditForm, UserPortalForm, UserLoginForm, PwdResetForm, PwdResetConfirmForm, ValuesForm
)
form_mapping = {
...
def accountsData(request):
    if request.method == 'POST':
        ...
        print(action, userpk, form_name, form_id, pk)
        ...
        if form_id == "login":
            ...
            if form.is_valid():
                ...
                if user is not None:
                    login(request,user)
                    return JsonResponse({'success': settings.LOGIN_REDIRECT_URL})
                else:
                    return JsonResponse({'alert': _('The data entered is not in our records')})

            else: 
                print(form.errors)
                obj_data = render_to_string('stela_control/load-data/auth/login/login_errors_v1.html', { 
                    'form': form,
                })
