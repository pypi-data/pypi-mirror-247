...
class OtterHandler(HubOAuthenticated, tornado.web.RequestHandler):
    ...
    async def get(self):
        log_tornado_issues('Authenticated?', "info")
        self.write("This is a post only page. You probably shouldn't be here!")
        user = self.get_current_user()
        log_tornado_issues('Got User', "info")
        log_tornado_issues(json.dumps(user, indent=1, sort_keys=True), "info")
...
def start_server():
    ...
    log_path = "/opt/logs/tornado.access.log"
