...
class FrogModel:
    ...
    def __generate_index_sql(
        self, index_name, table_name, key_column_list, placeholder_value
    ) -> str:
    ...
    def upsert_csv(self, table_name: str, filename: str) -> (int, int):
        total_updated = 0
        total_inserted = 0

        for chunk in pd.read_csv(
            filename, chunksize=UPSERT_CHUNK_SIZE, dtype=str, skipinitialspace=True
        ):
            chunk.replace("", np.nan, inplace=True)
            updated, inserted = self.upsert(table_name, chunk)
    ...
    def upsert_excel(self, filename: str, activity: ModelActivity = None) -> (int, int):
        ...
        with pd.ExcelFile(filename) as xls:
            ...
            for count, sheet_name in enumerate(xls.sheet_names):
                ...
                for i in range(0, len(df), UPSERT_CHUNK_SIZE):
                    ...
                    chunk.replace("", np.nan, inplace=True)
                    updated, inserted = self.upsert(table_to_upload, chunk)
    ...
    def upsert(self, table_name: str, data: pd.DataFrame) -> (int, int):
        """
        Upsert a pandas dataframe to a Cosmic Frog model
        """
        ...
        with self.begin() as connection:
            ...
            placeholder_value = (
                ""  # replace with a value that does not appear in your data
            )
            ...
            index_sql = self.__generate_index_sql(
                upsert_index_name, table_name, combined_key_columns, placeholder_value
            )
