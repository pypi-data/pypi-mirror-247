...
class BigDFTool(object):
    """
    This module defines a number of post-processing options, including
    those that can be driven by utilities like ``bigdft-tool``, ``memguess``,
    or ``utilities``.
    ...
    def __init__(self, omp=1, mpi_run=None):
        ...
        if not omp:
            omp = environ.get('OMP_NUM_THREADS', '1'),
        if not mpi_run:
            mpi_run = environ.get("BIGDFT_MPIRUN", '')

        # Load the dictionary that defines all the operations.
        try:
            with open(join(dirname(__file__), "Database",
                      "postprocess.yaml")) as ifile:
                db = YamlIO.load(stream=ifile)[0]
        except Exception as e:
            print(e)
            raise IOError("Couldn't find or load postprocess.yaml. " +
                          "Check your python path." +
                          f"The exception raised was {e}")
    ...
    def fragment_small_molecule(self, sys, view, cutoff=0.05,
                                maxiter=100, verbose=False, seed=0):
        """
        ...
        Returns:
            (dict): a mapping from the original fragments to the new fragments.
            (BigDFT.Systems.FragmentView): the new fragment view.
        """
        ...
        # Original
        nfrag = 0
        mapping = {x: x for x in sys}

        if verbose:
            print("Iteration", "Number of Fragments")

        # Loop
        for i in range(maxiter):
            new_mapping = self.auto_fragment(sys, view, cutoff, rand=True)
            if nfrag < len(new_mapping):
                mapping = new_mapping
                nfrag = len(mapping)
                if verbose:
                    print(i, nfrag)
...
def dict_distplot(system_dict, ax=None, reuse_ticks=False, kind='violin',
                  **kwargs):
    ...
    Args:
        ...
        reuse_ticks (bool): Employ the existing xticks of the axes
             to plot the data
    ...
    def get_keys_and_values(ax, dd, from_scratch):
        keys = [lb.get_text() for lb in ax.get_xticklabels()]
        if sum(len(k) for k in keys) == 0 or from_scratch:
            keys = dd.keys()
        else:
            keys += [k for k in sorted(dd.keys()) if k not in keys]
        values = [dd.get(k, len(dd[list(dd.keys())[0]])*[nan]) for k in keys]
        return keys, values
