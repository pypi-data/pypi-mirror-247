...
def distance(i, j, cell=None):
    ...
    Returns:
        (float): the distance between centroids.
    """
    from scipy.spatial.distance import cdist

    if not cell:
        dmat = cdist([i.centroid], [j.centroid])
    else:
        centi = cell.mirror_point(i.get_centroid(cell=cell))
        centj = cell.mirror_point(j.get_centroid(cell=cell))
        dmat = cdist(centi, centj)
...
def pairwise_distance(i, j, cell=None):
    ...
    if not cell:
        dmat = cdist([x.get_position() for x in i],
                     [x.get_position() for x in j])
    else:
        pi = sum([cell.mirror_point(x.get_position(cell=cell))
                  for x in i], [])
        pj = sum([cell.mirror_point(x.get_position(cell=cell))
                  for x in j], [])
        dmat = cdist(pi, pj)
...
class Fragment(MutableSequence):
    ...
    and also geometrical information (centroids, principla axis etc.). A
    ...
    def __setitem__(self, index, value):
        from BigDFT.Atoms import Atom
        if isinstance(value, Atom):
            self.atoms.__setitem__(index, value)
        else:
    ...
    def __add__(self, other):
        atlist = [x for x in self] + [x for x in other]
        return Fragment(atlist)
    ...
    def centroid(self):
        """
        The center of a fragment. Returned in bohr and without regard
        to unit cell.
        """
        return self.get_centroid()

    def get_centroid(self, cell=None, units="bohr"):
        """
    ...
    def rotate_on_axis(self, angle, axis, centroid=None, units="radians"):
        """
        ...
        Args:
          ...
          centroid (list): a list of floats defining the center from which
            the axis comes out. If not specified, we pick the centroid of the
            fragment.
        ...
        for i in range(0, len(self)):
            self[i].set_position(rself[i].get_position())
    ...
    def translate(self, vec):
        ...
        trans = rt.dot(self)
        for i in range(0, len(self)):
            self[i].set_position(trans[i].get_position())
