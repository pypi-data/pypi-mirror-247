...
class Runner():
    ...
    def __init__(self, **kwargs):
        ...
        self._logger.info('Runner created')
        import copy
        mro = type(self).mro()
        imro = mro.index(Runner)
        if not issubclass(object, type(self).mro()[imro + 1]):
            self._logger.info(f'Forwarding args to parent {super()} args:')
    ...
    def get_global_option(self, key):
        ...
        """
        self._logger.debug(f'requesting global option {key}')
        self._logger.debug(f'{self._global_options[key]}')
    ...
    def update_global_options(self, **kwargs):
        ...
        """
        self._logger.debug('updating global options')
    ...
    def pop_global_option(self, key):
        ...
        """
        self._logger.debug(f'popping global option at key "{key}"')
    ...
    def run(self, **kwargs):
        ...
        """
        self._logger.info('### New Run with initial args:')
        self._logger.info(f'{format_iterable(kwargs)}')
        from futile.Utils import dict_merge
        self._run_options(**kwargs)
        run_args = self.pre_processing()
        self._logger.debug('initial run_args:')
        self._logger.debug(f'{format_iterable(run_args)}')
        # safe_print('\ninitial run args',run_args)
        # required to pass arguments into aiida constructor for metadata
        # TODO(lbeal): check that this doesn't break functionality in json/dill
        inp = kwargs.get('arguments', None)
        if inp is not None:
            self.local_options.update(inp)
        self._logger.debug('calling process_run')
        run_results = self.process_run(**run_args)
        self._logger.debug('run results:')
        self._logger.debug(f'{format_iterable(run_results)}')
...
class SystemCalculator(Runner):
    ...
    Methods:
         ...
         run(name='',run_dir='.',outdir='',run_names='',input=None,posinp='posinp.xyz'):
                ...
                Args:
                   ...
                   sys (BigDFT.Systems.System): a system class for providing
                           the atomic data.
    ...
    def __init__(self,
        ...
        executable = os.path.join(os.environ.get('BIGDFT_ROOT', ''), 'bigdft')
    ...
    def pre_processing(self):
        ...
        # from here onwards the local input is a dict and not anymore another
        # class
        ...
        if posinp is None:
            sys = self.run_options.get('sys', None)
            if sys is not None:
    ...
    def process_run(self, command):
        ...
        if not verbose:
            ...
            redirect = ""
        os.system("cd " + self.run_dir + "; " + command + redirect)
    ...
    def post_processing(self, timedbg, logname, command):
        ...
        if os.path.exists(logname):
            ...
            # there can be an error file within this folder
            # if so, print the contents of that
            # it will be more useful than the fallback error
            #
            # logpath, sans any spurious '.' from ./ path entities
            logpath = os.path.split(logname)[0].rstrip('.')

            errfile_name = '_scheduler-stderr.txt'
            errfile = logpath + errfile_name
            if os.path.exists(errfile):
                safe_print(f'Logfile not found.\n'
                           f'Contents of error file {errfile_name}:')
                with open(errfile) as o:
                    i = 1
    ...
    def _get_command(self):
        ...
        dry_mpi = self.run_options.get('dry_mpi')
        ...
        if dry_run:
            ...
            if name != '':
                command += ' --name=' + name
            if dry_mpi is not None:
                command += ' -n '+str(dry_mpi)
        else:
    ...
    def _posinp_dictionary_value(self, posinp):
        ...
        Args:
          ...
          For extensions which are supported externally of the bigdft
          executable, the system is read by the Io module and converted 
          into a posinp class.
        ...
        from BigDFT.IO import read_pdb, read_mol2
        ...
        if not os.path.isfile(posinp):
            raise ValueError(
                ...
                % posinp)
        if posinp.endswith('.pdb'):
            with open(posinp) as ifile:
                sys = read_pdb(ifile)
            return make_dict(sys.get_posinp())
        elif posinp.endswith('.mol2'):
            with open(posinp) as ifile:
                sys = read_mol2(ifile)
            return make_dict(sys.get_posinp())
        posinpdict = posinp
...
class DummyCalculator(Runner):
    """!skip

    False calculator for testing RemoteRunner functionality.

    To be used on systems where BigDFT is either not installed, or it is
    inconvenient to do so. Purely for development and/or testing purposes
    """
    import os
    import shutil
    ...
    def __init__(self,
        ...
        # Use the initialization from the Runner class (so all options inside
        # _global_options)
        # TODO(lbeal): separate from Runner?
        Runner.__init__(self, omp=str(omp), mpi_run=mpi_run,
                        dry_run=dry_run, skip=skip, verbose=verbose)
        self.command = ('python dummy.py')

    def pre_processing(self):
        """!skip

        generates run_args not needed for a dummy run
        """
        # anything returned from here is passed to process_run as args
        outargs = self._global_options
        outargs['test'] = True
        outargs.update(self.run_options)
        return outargs
    ...
    def process_run(self, **kwargs):
        ...
        dummy process_run function, simply have to update the 'run' flag
        """
        # print('process_run')
        # print(kwargs)
        kwargs['run'] = True
        # again, returned object is passed to post_processing as args
        return kwargs

    def post_processing(self, **kwargs):
        """!skip

        this is what actually gets returned from Run()
        """
