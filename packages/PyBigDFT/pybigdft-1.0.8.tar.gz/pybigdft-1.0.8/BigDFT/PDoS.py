...
def compute_weight(evec, orb_idx, proj_idx, s=None, k=None):
    """
    ...
    Args:
        ...
        s (numpy.array, optional): Overlap matrix, used in the calculation of
            the weight. Defaults to the identity matrix if not provided.
        k (numpy.array, optional): Density matrix, used in the calculation of
            the weight. Defaults to the identity matrix if not provided.
    ...
    # Project
    submat = s @ k @ s
    seig = evec[:, orb_idx:orb_idx+1]
    return trace(seig[proj_idx, :].T @ (submat @ seig)[proj_idx, :])
...
def linear_projection_info(log, thresh=PROJECTION_THRESHOLD):
    ...
    def guess_proj_type(atm, thresh):
        ...
        return ptype
    from BigDFT.Systems import System, system_from_log
    ...
    # Make the system where each atom is its own fragment to get the indices
    # associated with an atom
    atsys = System()
    for i, at in enumerate(sys.get_atoms()):
        atsys["ATM:" + str(i)] = Fragment([at])
...
def linear_projection_weights(evec, proj_info, s, k):
    ...
    for i in range(evec.shape[1]):
        ...
        for atm, proj in proj_info.items():
            for p in proj:
                wdict[atm][p] += compute_weight(evec, i,
                                                proj_info[atm][p], s=s, k=k)
        weights.append(wdict)
    return _defaultdict_to_dict(weights)


def compute_fragment_weights(sys, log, evec, evals, s, k):
    """
