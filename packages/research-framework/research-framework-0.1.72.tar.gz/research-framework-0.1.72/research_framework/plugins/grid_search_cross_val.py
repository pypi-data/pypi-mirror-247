...
class WandbSeepsAgent(BaseFilterPlugin):
    ...
    def __init__(self, scorer=MetricModel(clazz='F1'), refit=True, sweep_id=None, filters=[]):
        self.filters = filters
        self.scorer = scorer
        self.refit = refit
...
class WandbSeepsRun(BaseFilterPlugin):
    def __init__(self, dataset, run_id, sweep_id, scorer, n_splits, test_size, random_state, count):
        ...
        with wandb.init(config=config, project=Container.global_config.project_name):
            ...
            for train, test in cv.split(x):
                if type(x) == pd.DataFrame:
                    ...
                    x_train = torch.utils.data.Subset(x, train)
                    x_test = torch.utils.data.Subset(x, test)
                else:
                    x_train = x[train]
                    x_test = x[test]
