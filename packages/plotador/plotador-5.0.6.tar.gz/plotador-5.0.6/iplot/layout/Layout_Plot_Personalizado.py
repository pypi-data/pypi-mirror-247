    ...
    def __init__(self, figura, tituloGrafico):
        ...
        for frame in figura.listaFrames:
            for go in frame.getListaGO():
                ...
                #print(go)
                #print(go.returnGoObject(show))
                #fig0.add_trace(go.returnGoObject(show), row=frame.linha,col=frame.coluna)
                fig0.add_trace(go.returnGoObject(), row=frame.linha,col=frame.coluna)
                fig0.update_xaxes(title_text=go.xaxis, row = frame.linha, col = frame.coluna)
                fig0.update_yaxes(title_text=go.yaxis, row = frame.linha, col = frame.coluna)
