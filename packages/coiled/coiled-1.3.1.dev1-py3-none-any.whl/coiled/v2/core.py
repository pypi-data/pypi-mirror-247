...
class CloudV2(OldCloud, Generic[IsAsynchronous]):
    ...
    def list_events(self, cluster_id: int, account: Optional[str] = None) -> Union[list, Awaitable[list]]:
        return self._sync(self._list_events, cluster_id, account)

    async def _send_state(self, cluster_id: int, desired_status: str, account: Optional[str] = None):
        account = account or self.default_account
        response = await self._do_request(
            "POST",
    ...
    @track_context
    async def _list_clusters(self, account: Optional[str] = None, max_pages: Optional[int] = None):
    ...
    async def _list_clusters_page(self, page: int, account: Optional[str] = None) -> Tuple[list, bool]:
        page_size = 100
        account = account or self.default_account
        response = await self._do_request(
            "GET",
    ...
    @overload
    def delete_cluster(self: Cloud[Sync], cluster_id: int, account: Optional[str] = None) -> None: ...

    @overload
    def delete_cluster(self: Cloud[Async], cluster_id: int, account: Optional[str] = None) -> Awaitable[None]: ...

    @delete_docstring  # TODO: this docstring erroneously says "Name of cluster" when it really accepts an ID
    def delete_cluster(self, cluster_id: int, account: Optional[str] = None) -> Optional[Awaitable[None]]:
        return self._sync(self._delete_cluster, cluster_id, account)

    async def _get_cluster_state(self, cluster_id: int, account: Optional[str] = None) -> dict:
        account = account or self.default_account
        response = await self._do_request(
    ...
    async def _get_cluster_details(self, cluster_id: int, account: Optional[str] = None):
        account = account or self.default_account
    ...
    def _get_cluster_details_synced(self, cluster_id: int, account: Optional[str] = None):
        return self._sync(
    ...
    async def _get_workers_page(self, cluster_id: int, page: int, account: Optional[str] = None) -> Tuple[list, bool]:
        page_size = 100
        account = account or self.default_account
    ...
    @track_context
    async def _get_cluster_by_name(self, name: str, account: Optional[str] = None) -> int:
        account, name = self._normalize_name(
    ...
    @track_context
    async def _get_clusters_by_name(self, name: str, account: Optional[str] = None) -> List[dict]:
    ...
    @overload
    def scale_up(self: Cloud[Sync], cluster_id: int, n: int, account: Optional[str] = None) -> Optional[Dict]: ...
    ...
    @track_context
    async def _fetch_package_levels(self, account: Optional[str] = None) -> List[PackageLevel]:
        account = account or self.default_account
        response = await self._do_request("GET", f"{self.server}/api/v2/packages/", params={"account": account})
        if response.status >= 400:
            await handle_api_exception(response)
        return await response.json()
    ...
    @track_context
    async def _get_ssh_key(self, cluster_id: int, account: str, worker: Optional[str]) -> dict:
        account = account or self.default_account
    ...
    @track_context
    async def _add_cluster_span(self, cluster_id: int, account: Optional[str], span_identifier: str, data: dict):
        account = account or self.default_account
