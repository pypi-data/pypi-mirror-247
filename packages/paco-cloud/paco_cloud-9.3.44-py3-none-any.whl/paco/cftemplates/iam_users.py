...
class IAMUsers(StackTemplate):
    def __init__(self, stack, paco_ctx):
        ...
        # IAM Users
        is_enabled = False
        if schemas.IIAMUsers.providedBy(self.resource):
            for iam_user in self.resource.values():
                ...
                is_enabled = True
        elif schemas.IIAMUser.providedBy(self.resource):
            if self.resource.is_enabled():
                is_enabled = True
                stack_username = self.create_resource_name(
                    self.resource.username,
                    camel_case=False
                )
                self.set_aws_name(f'Account-{stack_username}')
                self.add_iam_user(self.resource)
    ...
    def add_iam_user(self, iam_user):
        ...
        if iam_user.console_access_enabled == True:
            user_password = iam_user.default_password
            ...
            password_param = self.create_cfn_parameter(
                ...
                param_type='String',
                description=f'The password to assign of the user {iam_user.name}',
                noecho=True,
                value=user_password
            )
        ...
        if len(iam_user.managed_policies) > 0:
            self.add_managed_policies(iam_user, iam_user_dict)
        ...
        if iam_user.disable_account_delegates == False:
            self.add_account_delegate_assume_rule(iam_user, iam_user_res)

    def add_managed_policies(self, iam_user, iam_user_dict):
        for policy_ref in iam_user.managed_policies:
            policy_arn = self.paco_ctx.get_ref(policy_ref+'.arn')
            if 'ManagedPolicyArns' not in iam_user_dict:
                iam_user_dict['ManagedPolicyArns'] = []
            iam_user_dict['ManagedPolicyArns'].append(policy_arn)

    def add_account_delegate_assume_rule(self, iam_user, iam_user_res):
