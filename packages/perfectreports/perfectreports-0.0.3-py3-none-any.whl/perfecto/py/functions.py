...
def topFailedTC(total, failed_df):
    # top failed TCs
    failed = failed_df[~failed_df.name.isin(["Interactive session"])]
    topfailedTCNames = (
        failed.groupby(["name"])
        .size()
    ...
    topfailedTCNames = hyperlink(
        topfailedTCNames, "Result", "reportURL", "name", failed, first_item=True)
    topfailedTCNames.columns = ["Failed Tests", "Σ", "%"]
    topfailedtable = {}
...
def topFailedReasons(total, failed_df):
    # top failed reasons
    failed = failed_df[~failed_df.name.isin(["Interactive session"])]
    topfailedTCNames = (
        failed.groupby(["Failure Message"])
        .size()
        .reset_index(name="#Failed")
        .sort_values("#Failed", ascending=False)
        .head(10)
    ...
    topfailedTCNames['%'] = round(topfailedTCNames["#Failed"].div(
        total).mul(100).astype(float), 1).astype(str) + '%'
    topfailedTCNames = hyperlink(
        topfailedTCNames, "Result", "reportURL", "Failure Message", failed, first_item=True)
    topfailedTCNames.columns = ["Failure Messages", "Σ", "%"]
    topfailedtable = {}
    topfailedtable = topfailedTCNames.to_html(
...
def create_summary_pie(df, title, column, value_column):
...
def retrieve_tests_executions(daysOlder, jobName, jobNumber, page, startDate, endDate, reportTag):
...
def prepare_device_coverage(df):
    ...
    if (len(df['osCoverage'].value_counts()) > 0):
        version_items = create_top_pie(
            df, "", "osCoverage", "Σ", 5, "height: 200px;width: 190px;")
...
def get_resources(truncated, reportTag, page, daysOlder, resources, jobName, jobNumber, startDate, endDate):
    while truncated == True:
        executions = retrieve_tests_executions(
            daysOlder, jobName, jobNumber, page, startDate, endDate, reportTag)
...
def prepare_custom_failure_graph(failed_df):
    failed_df["Custom Failure Reason"] = failed_df["Custom Failure Reason"].fillna(
        'Uncategorized')
    return create_top_pie(
        failed_df, "", "Custom Failure Reason", "count", 5, "height: 200px;width: 300px;")


def prepare_module_graph(df):
    # https://towardsdatascience.com/100-stacked-charts-in-python-6ca3e1962d2b
    cross_tab = pandas.crosstab(index=df['Module'],
                                columns=df['Test Status'])
...
def prepare_tags_table(total, failed_df):
    tags_df = failed_df.pivot_table(
        index=['Owner', 'Module', 'Failed Step'], aggfunc='size')
...
def prepare_failed_table(failed_df):
    failed_df["Video"] = '<img id="vidlink"/>'
    # hyperlinking video link to Failed Step
    failed_df = hyperlink(failed_df, "vidResult",
                          "videos/0/downloadUrl", "Video", "", False)
    failed_df = failed_df[["Owner", "Module", "name", "Custom Failure Reason",
                           "Failure Message", "Failed Step", "Video"]]
...
def format_df(df, jobName, jobNumber, automation_owners):
