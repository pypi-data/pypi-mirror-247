...
class DataField(DataBaseObject):
    ...
    _save_in_db_properties = ['name', 'full_name', 'origin_desc',
                              'curr_desc', 'curr_desc_stat', 'sample_data']
    _in_dict_properties = (
    ...
    # def to_dict_for_llm(self):
    #     return self.to_dict(
    #         excludes=('full_name', 'origin_desc', 'curr_desc_stat')
    #     )
...
class DataTable(DataBaseObject):
    ...
    custom_fields: 自定义字段，用于存储一些额外的信息, dict类型，用json.dumps()转换为字符串存储
        对于Excel数据源的表格，custom_fields中存储的是：data_begin_row
    ...
    def to_dict(self, level='field', includes=(), excludes=()):
        ...
        if level == 'field':
            _d['fields'] = [field.to_dict(includes=includes, excludes=excludes)
    ...
    def to_dict_for_llm(self):
        ...
        if need_infer:
            return self.to_dict(includes=('name', 'curr_desc', 'sample_data'),)
        else:
            return {}
...
class DataSchema(DataBaseObject):
    ...
    def custom_configs_dict(self):
        return json.loads(self.custom_configs) if self.custom_configs else {}
    ...
    def to_dict(self, level='field', includes=(), excludes=()):
        ...
        if level in ('table', 'field'):
            _d['tables'] = [table.to_dict(level, includes=includes, excludes=excludes)
