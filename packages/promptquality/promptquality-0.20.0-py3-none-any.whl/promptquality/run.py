...
def run(
    ...
    run_id = create_run(
        project.id,
        run_name=run_name or ts_run_name(template_response.name, template_response.selected_version.version),
        config=config,
    )
    ...
    return get_metrics(project_id=project.id, run_id=run_id, job_id=job_id, config=config)
