...
def create_project(project_name: Optional[str] = None, config: Optional[Config] = None) -> CreateProjectResponse:
    config = config or set_config()
    project_request = CreateProjectRequest(name=project_name)
    existing_projects: Dict[str, CreateProjectResponse] = {proj.name: proj for proj in get_all_projects(config)}
...
def get_project(project_id: UUID4, config: Optional[Config] = None) -> CreateProjectResponse:
    config = config or set_config()
    response_dict = config.api_client.get_project(project_id)
    project = CreateProjectResponse.model_validate(response_dict)
...
def get_all_projects(config: Optional[Config] = None) -> List[CreateProjectResponse]:
    ...
    return [CreateProjectResponse.model_validate(proj) for proj in config.api_client.get_projects()]
...
def create_run(
    ...
    config = config or set_config()
    run_request = CreateRunRequest(name=run_name, project_id=project_id, task_type=task_type)
...
def create_job(
    ...
    if not dataset_id:
        raise ValueError("Dataset ID must be provided or set in config before creating job.")
    ...
    job_request = CreateJobRequest(
        ...
        prompt_scorers_configuration=ScorersConfiguration.from_scorers(scorers) if scorers else None,
...
def upload_custom_metrics(
    ...
    prompt_rows = get_rows(project_id, run_id, config=config)
