...
from typing import no_type_check, Any, Dict, List, NamedTuple, Tuple, Type
...
class ValidationChainSettings(NamedTuple):
    ...
    @classmethod
    def from_dict(cls, settings: dict) -> ValidationChainSettings:
...
class ValidationModule:
    ...
    @no_type_check
    def get_package_module(self) -> Any:
        try:
    ...
    def create_mock_df(self) -> pd.DataFrame:
        df_dict = {}
        for column in self.model.inputs:
            ...
            if column.data_type is DataType.INTEGER:
                ...
                # DataType.CATEGORY
                data = column.possible_values[0]  # type: ignore
...
class PackageValidator:
    def __init__(
        ...
        self.validation_chain: List[ValidationModule] = []
    ...
    def from_validation_chain(
        ...
        with open(str_loc, 'rb') as pkl:
            ...
            validator.validation_chain = [
                module(model, dataset, assets_dir) for module in class_defns_chain
            ]
            return validator
    ...
    def from_hosted_validation_chain(
        ...
        """
        try:
            # TODO: this is exceptionally dangerous! Let's get some clarity
            # about how we are mitigating the risks associated with unpickling
            # an object from the internet.
            chain = pickle.load(urllib.request.urlopen(chain_url))
            validator = cls(model, dataset, assets_dir, settings)
            validator.validation_chain = [
                module(model, dataset, assets_dir) for module in chain
            ]
            return validator
        except Exception as e:  # TODO: don't catch all exceptions
            raise e

    def _present_errors(self, errors: dict) -> None:
    ...
    def _push_default_modules_to_chain(self) -> None:
    ...
    def run_chain(self) -> Tuple[bool, dict]:
        ...
        def fail(errors: Any) -> Tuple[bool, Any]:
            print('Validation Result:', end=' ')
            self.logger.error('FAIL')
            return False, errors

        def success() -> Tuple[bool, Dict[str, List[ValidationError]]]:
            print('Validation Result:', end=' ')
            self.logger.success('PASS')
            return True, errors

        errors: Dict[str, List[ValidationError]] = {'all_errors': []}
    ...
    def push_module_to_chain(
        self, module: Type[ValidationModule], position: int = -1
    ) -> None:
    ...
    def export_validation_chain(self, filename: str = 'chain.pkl') -> None:
...
class DatasetValidator(ValidationModule):
    ...
    def _check_reserved_words(self) -> None:
    ...
    def _check_columns_name_length(self) -> None:
    ...
    def _check_dataset_name_case(self) -> None:
        """Validates that a dataset is named with strictly lowercase letters"""
        if (self.dataset.dataset_id is not None) and (
            not self.dataset.dataset_id.islower()
        ):
            self.append_error(
                f"Dataset name '{self.dataset.dataset_id}' may not contain uppercase letters"
            )

    def _check_dataset_name_spaces(self) -> None:
...
class ModelValidator(ValidationModule):
    ...
    def _check_package_module_valid(self) -> None:
        package_module = self.get_package_module()
    ...
    def _check_outputs_match(self) -> None:
        """Validates that outputs listed in ModelInfo matches outputs in package.py"""
        prediction = self._run_model_prediction()
        prediction_cols = [str(col) for col in prediction.columns]
        model_output_cols = [col.name for col in self.model.outputs]
        if not set(prediction_cols) == set(model_output_cols):
            self.append_error(
                f'Expected prediction columns {model_output_cols}. Got {prediction_cols} in package.py instead.'
            )

    def _check_model_dataset_case(self) -> None:
        """Validates that all datasets associated with the ModelInfo only have lowercase letters in their name"""
        for dataset in self.model.datasets:  # type: ignore
            if not dataset.islower():
                self.append_error(
                    f"Associated dataset '{dataset}' may not contain uppercase letters"
                )

    def _check_classification_type_output_count(self) -> None:
