...
class TestUtils(BaseTestCase):
    ...
    def test_cleaning_df_with_all_datetime_string_col_should_convert_to_string_col(self):
        all_data = pd.DataFrame(
            {
                'a': [np.int64(1), np.int64(2)],
                'b': [np.uint8(1), np.uint8(2)],
    ...
    def test_cleaning_df_with_non_datetime_string_col_should_not_convert_to_string_col(self):
        all_data = pd.DataFrame(
            {
                'a': [np.int64(1), np.int64(2)],
                'b': [np.uint8(1), np.uint8(2)],
    ...
    def test_cleaning_df_with_datetime_col_df_w_tz_should_convert_to_string_col(self):
        ...
        all_data['dates'] = pd.to_datetime(all_data['dates'], utc=True).dt.tz_convert('UTC')
        original_col_dtypes = all_data.dtypes.to_dict()
    ...
    def test_cleaning_df_with_datetime_col_string_w_tz_should_convert_to_string_col(self):
        all_data = pd.DataFrame(
            {
                'a': [np.int64(1), np.int64(2)],
                'b': [np.uint8(1), np.uint8(2)],
    ...
    def test_try_series_retype_timestamp(self):
        series = pd.Series(['2023-11-12 09:15:32.23', '2023-12-11 09:15:32.45'],
                           dtype='str')
    ...
    def test_try_series_retype_timestamp_error(self):
        series = pd.Series(['2023-11-12 09:15:32.23', None],
                           dtype='str')
