...
def process_baremetal_rules(config, baremetal_hosts, vmrules=[], overrides={}):
    ...
    with open('macs.txt', 'w') as f:
        for name in baremetal_rules:
            ...
            if mac is None or ip is None or netmask is None or gateway is None:
                ...
                entry = f"{mac};{hostname};{ip};{netmask};{gateway};{nameserver}"
                f.write(entry)
...
def create(config, plandir, cluster, overrides, dnsconfig=None):
    ...
    if sno:
        ...
        if run != 0:
            ...
            return {'result': 'failure', 'reason': msg}
        process_baremetal_rules(config, baremetal_hosts, vmrules=vmrules, overrides=overrides)
