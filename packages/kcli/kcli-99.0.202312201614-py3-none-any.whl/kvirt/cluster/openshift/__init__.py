...
def create(config, plandir, cluster, overrides, dnsconfig=None):
    ...
    if sno:
        ...
        with open("iso.ign", 'w') as f:
            ...
            if os.path.exists('{clusterdir}/macs.txt'):
                _files.append({"path": "/root/macs.txt", "origin": f"{clusterdir}/macs.txt"})
        ...
        iso_installer = len(baremetal_hosts) < 2
        if provider == 'fake':
            pprint("Storing generated iso in current dir")
            generate_rhcos_iso(k, f"{cluster}-sno", 'default', installer=iso_installer, extra_args=extra_args)
        else:
            iso_pool = data['pool'] or config.pool
            pprint(f"Storing generated iso in pool {iso_pool}")
            generate_rhcos_iso(k, f"{cluster}-sno", iso_pool, installer=iso_installer, extra_args=extra_args)
        if sno_ctlplanes:
            if api_ip is None:
