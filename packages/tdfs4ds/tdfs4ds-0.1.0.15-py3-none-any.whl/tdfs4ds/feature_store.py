...
import pandas as pd
from tdfs4ds.utils import execute_query, display_table
...
def get_feature_store_table_name(entity_id, feature_type):
    ...
    table_name = ['FS','T']+[data_domain]+list(entity_id.keys())+[feature_type]
    ...
    view_name  = ['FS','V']+[data_domain]+list(entity_id.keys())+[feature_type]
...
def prepare_feature_ingestion(df, entity_id, feature_names, feature_versions = None, **kwargs):
    ...
    primary_index = ','.join(list(entity_id.keys()))
...
def store_feature(entity_id, prepared_features, date_in_the_past_= date_in_the_past, **kwargs):
    """
    ...
    if date_in_the_past_ == None:
        validtime_statement = 'CURRENT VALIDTIME'
        validtime_statement2 = validtime_statement
    else:
        validtime_statement = f"VALIDTIME PERIOD '({date_in_the_past_},{end_period})'"
        validtime_statement2 = f"VALIDTIME AS OF TIMESTAMP '{date_in_the_past_}'"
...
def build_dataset(entity_id, selected_features, view_name,
    ...
    if type(entity_id) == dict:
        ...
        if len(selected_features) > 1:
            ENTITY_ID_ = ','.join([','.join(['COALESCE('+','.join(['AA'+str(i+1)+'.'+k for i,c in enumerate(selected_features)])+') as '+k]) for k in entity_id.keys()])
        else:
            ENTITY_ID_ = ','.join([','.join(['' + ','.join(['AA' + str(i + 1) + '.' + k for i, c in enumerate(selected_features)]) + ' as ' + k]) for k in entity_id.keys()])
    elif type(entity_id) == list:
        ENTITY_NAMES = ','.join([k for k in entity_id])
        ENTITY_ID = ', \n'.join([k for k in entity_id])
        if len(entity_id) > 1:
            ENTITY_ID_ = ','.join([','.join(['COALESCE('+','.join(['AA'+str(i+1)+'.'+k for i,c in enumerate(selected_features)])+') as '+k ]+'NULL')for k in entity_id])
        else:
            ENTITY_ID_ = ','.join([','.join(['' + ','.join(
                ['AA' + str(i + 1) + '.' + k for i, c in enumerate(selected_features)]) + ' as ' + k] + 'NULL') for k
                                   in entity_id])
    elif type(entity_id) == str:
        ENTITY_NAMES = entity_id
        ENTITY_ID = entity_id
        ENTITY_ID_ = 'COALESCE('+','.join(['AA'+str(i+1)+'.'+entity_id for i,c in enumerate(selected_features)])+',NULL) as '+entity_id
    ...
    query_from    = query_from + [' FULL OUTER JOIN ('+q+') AA'+str(i+1)+' \n ON '+' \n AND '.join([f'AA1.{c}=AA{i+1}.{c}' for c in entity_id.keys()]) for i,q in enumerate(query) if i>0]
    query_from    = '\n'.join(query_from)
