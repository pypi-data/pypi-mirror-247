    ...
    if feature_store.date_in_the_past == None:
        ...
        # Constructing the query for updating existing views with a past date
            ...
            process_id = tdml.DataFrame.from_query(
                f"VALIDTIME AS OF TIMESTAMP '{date_in_the_past_}' SEL PROCESS_ID FROM {feature_store.schema}.{process_catalog_name} WHERE VIEW_NAME = '{view_name}'").to_pandas().PROCESS_ID.values[
                0]
...
def run(process_id, as_date_of=None, return_dataset = False):
    """
