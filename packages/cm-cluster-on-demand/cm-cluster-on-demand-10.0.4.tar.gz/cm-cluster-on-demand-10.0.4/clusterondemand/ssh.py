...
def private_key_for_public(public_key_path: str) -> list[str] | None:
...
def validate_ssh_access(floating_ip: str) -> None:
    try:
...
class SSHExecutor:
    ...
    def __init__(self, host: str, user: str | None = None, identity_files: str | list[str] | None = None,
                 password: str | None = None, raise_exceptions: bool = True,
                 connect_timeout: float | int | None = None, wait_between_connect_retries: float = 1) -> None:
    ...
    def _connect(self) -> None:
        ...
        def connect_wait() -> None:
            self._ssh_client.connect(  # type: ignore
                self._host,
                username=self._user,
                key_filename=self._identity_files,  # type: ignore
