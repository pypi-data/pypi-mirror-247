...
def edit(ctx: click.Context, **kwargs):
    ...
    Edit(ctx.obj["config"], TaskManager(ctx.obj["config"])).run()
...
@MainUtils.common_options
def recur(ctx: click.Context, **kwargs):
    """
    ...
    Recur(ctx.obj["config"], TaskManager(ctx.obj["config"])).run()
...
def addons_command(ctx: click.Context, command: str, **kwargs):
    ...
    MainUtils.initialize(ctx.obj["config"], **kwargs)
    result = Addons(ctx.obj["config"]).run(command)
    click.echo(result.stdout, nl=False)
    if result.returncode != 0:
        click.echo(result.stderr)
    ctx.obj["config"].git.commit_and_push(f"addons {command}")
