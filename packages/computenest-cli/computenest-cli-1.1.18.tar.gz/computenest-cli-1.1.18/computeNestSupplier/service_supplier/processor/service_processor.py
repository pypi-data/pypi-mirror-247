...
class ServiceProcessor:
    ...
    def import_command(self, data_config, file_path, update_artifact, service_name='', version_name='', icon='',
        ...
        if service_config.get(constant.OPERATION_METADATA):
            ...
            if self.CUSTOM_OPERATIONS in service_config[constant.OPERATION_METADATA]:
                for operation_template in service_config[constant.OPERATION_METADATA][self.CUSTOM_OPERATIONS][
                    self.ACTIONS]:
                    # 将相对路径替换成绝对路径
        ...
        if len(data_list.body.services) == 0:
            ...
            create_service_instance_result = self.service.create_service(service_config)
