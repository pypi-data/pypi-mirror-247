...
def upsert(
        ...
        timeout=60, disable_rollback=False, block=True,
    ...
    :param bool block:  by default, an upsert will wait for the update
      to complete and print the results of the upsert as they are happening;
      set this flag to false to return control to the command line once the
      upsert has been initiated
    :return:
    """
...
def test(
    ...
    try:
        ...
        error = f'{ex}'
        if error.endswith('"FAILED"'):
            notice_end('\N{wilted flower} no changes')
        else:
            notice_end(error)
        return
