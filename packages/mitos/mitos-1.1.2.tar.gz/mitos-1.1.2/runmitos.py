...
#         print f.name, f.strand, f.start, f.stop
    ...
    if prot or cr:
        cmd = ["plotprot.R",
               ...
               "%d" % (len(sequence))]
        util.execute(cmd, supress_warnings=True)
...
if __name__ == '__main__':
    ...
    ingroup.add_argument(
        '--fasta', dest="fastacontent", action="store", help="input fasta sequence")
    ...
    if args.file is None and args.fastacontent is None:
        parser.error("no input file/fasta given")
    elif args.file is not None and args.fastacontent is not None:
    ...
    gfffile.gffwriter(featurelist, acc, len(sequences[0]["sequence"]), outfile="%s/result.gff" % (args.outdir), mode="w")
    ...
    cmd = ["drawmitos.py",
        "--input", f"{args.outdir}/result.bed",
        "--output", f"{args.outdir}/result.png",
    ...
    if args.zip:
        shutil.make_archive(os.path.normpath(args.outdir), 'zip', args.outdir)
