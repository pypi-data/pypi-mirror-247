...
class gb:
    ...
    def _add_feature(self, name, tpe, start, stop, strand, translation):
        ...
        add a feature ..
        """
        nf = feature.feature(name, tpe, start, stop, strand, "genBank", translation)
    ...
    def is_allowed(self, atax=None, ftax=None):
        ...
        if atax is not None and len(filter(lambda x: x in self.taxonomy, atax)) == 0:
            return False
        elif ftax is not None and len(filter(lambda x: x in self.taxonomy, ftax)) > 0:
            return False
...
#        self._parse_circular(record.residue_type)
        ...
        self.sequence = sequence.sequence(str(record.seq), self.circular)
...
#            sys.exit()
    ...
    def _parse_anticodon(self, v, strand):
        ...
        m = re.search(r"POS:([\d]+)\.\.([\d]+),AA:(\w{3})", v)
        if m is not None:
            s = int(m.group(1)) - 1
            e = int(m.group(2))
            try:
        ...
        m = re.match(r"ANTICODON: ([U%s]{3})" % Data.IUPACData.ambiguous_dna_letters, v)
        if m is not None:
            try:
        ...
        m = re.match(r"^ANTICODON[ \s]{1,2}([U%s]{3}).*" % Data.IUPACData.ambiguous_dna_letters, v)
        if m is not None:
            try:
                cdn = codon(m.group(1), "anticodon")
            except CodonError:
        ...
        if nam is not None:
            m = re.search(r"\(([U%s]{3})\)" % Data.IUPACData.ambiguous_dna_letters, v)
            if m is not None:
                try:
                    cdn = codon(m.group(1), "anticodon")
                except CodonError:
    ...
    def _parse_codon_recognized(self, v, strand):
        ...
        m = re.match(r"^([U%s]{3})$" % Data.IUPACData.ambiguous_dna_letters, v)
        if m is not None:
            try:
                cdn = codon(v, "codon")
            except CodonError:
        ...
        m = re.match(r"^CODONS? RECOGN[IZ]{2}ED[\s:]+([U%s]{3}$)" % Data.IUPACData.ambiguous_dna_letters, v)
        if m is not None:
            # s =
            # print "CR %s" %(s)
            try:
    ...
    def _parse_names(self, annotations):
        ...
        if "source" in annotations:
            m = re.match(r'mitochondrion %s(?P<commonname> \(.*\)){0,1}$' % n, annotations["source"])
    ...
    def _parse_feature_codon(self, f, strand):
        ...
        return [x for x in codons if x is not None]
...
#        print "location", feature.location
    ...
    def _parse_translation(self, feature):
        ...
        try:
            return sequence(feature.qualifiers['translation'][0], False)
        except Exception:
            return None
