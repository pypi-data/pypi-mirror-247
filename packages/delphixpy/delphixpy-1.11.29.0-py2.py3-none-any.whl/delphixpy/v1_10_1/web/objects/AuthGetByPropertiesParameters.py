...
class AuthGetByPropertiesParameters(TypedObject):
    ...
    *(extends* :py:class:`v1_10_1.web.vo.TypedObject` *)* The parameters to use
    as input to get an authorization with given user, target, and role.
    """
    ...
    def from_dict(cls, data, dirty=False, undef_enabled=True):
        ...
        if obj._role[0] is not None and obj._role[0] is not obj.__undef__:
            assert isinstance(obj._role[0], str), ("Expected one of ['string'], but got %s of type %s" % (obj._role[0], type(obj._role[0])))
    ...
    def to_dict(self, dirty=False, belongs_to_parent=False):
        ...
        if "target" == "type" or (self.target is not self.__undef__ and (not (dirty and not self._target[1]) or self.is_dirty_list(self.target, self._target) or belongs_to_parent)):
            dct["target"] = dictify(self.target)
        if "role" == "type" or (self.role is not self.__undef__ and (not (dirty and not self._role[1]) or self.is_dirty_list(self.role, self._role) or belongs_to_parent)):
    ...
    def is_dirty(self):
        return any([self._user[1], self._target[1], self._role[1]])
