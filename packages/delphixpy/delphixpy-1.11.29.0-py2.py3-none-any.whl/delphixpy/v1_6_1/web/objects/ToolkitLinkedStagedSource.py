...
from delphixpy.v1_6_1.web.objects.ToolkitLinkedSource import ToolkitLinkedSource
...
class ToolkitLinkedStagedSource(ToolkitLinkedSource):
    """
    *(extends* :py:class:`v1_6_1.web.vo.ToolkitLinkedSource` *)* A linked
    ...
    def is_dirty(self):
        return any([self._post_snapshot[1], self._pre_snapshot[1], self._resync[1], self._start_staging[1], self._status[1], self._stop_staging[1]])
    ...
    def status(self):
        """
        A workflow script run on the staging host to determine if the staging
        instance is configured to receive data from the dSource. The script
        should exit with an exit status of ACTIVE if data reception is
        correctly configured and available. The script should exit with an exit
        status of INACTIVE if the staging instance is not configured to receive
        data. An exit status of UNKNOWN implies the script encountered an
        unexpected state or error. If no status script is supplied, the dSource
        will always be in an active state while enabled.
