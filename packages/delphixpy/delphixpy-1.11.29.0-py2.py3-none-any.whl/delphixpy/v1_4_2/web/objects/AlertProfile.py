...
class AlertProfile(PersistentObject):
    """
    *(extends* :py:class:`v1_4_2.web.vo.PersistentObject` *)* A profile that
    ...
    def from_dict(cls, data, dirty=False, undef_enabled=True):
        ...
        for item in data.get("severityFilter") or []:
            assert isinstance(item, str), ("Expected one of ['string'], but got %s of type %s" % (item, type(item)))
            assert item in ['INFORMATIONAL', 'WARNING', 'CRITICAL'], "Expected enum ['INFORMATIONAL', 'WARNING', 'CRITICAL'] but got %s" % item
            common.validate_format(item, "None", None, None)
    ...
    def to_dict(self, dirty=False, belongs_to_parent=False):
        ...
        if "event_filter" == "type" or (self.event_filter is not self.__undef__ and (not (dirty and not self._event_filter[1]) or self.is_dirty_list(self.event_filter, self._event_filter) or belongs_to_parent)):
            dct["eventFilter"] = dictify(self.event_filter, prop_is_list_or_vo=True)
        if "severity_filter" == "type" or (self.severity_filter is not self.__undef__ and (not (dirty and not self._severity_filter[1]) or self.is_dirty_list(self.severity_filter, self._severity_filter) or belongs_to_parent)):
            dct["severityFilter"] = dictify(self.severity_filter, prop_is_list_or_vo=True)
        if "target_filter" == "type" or (self.target_filter is not self.__undef__ and (not (dirty and not self._target_filter[1]) or self.is_dirty_list(self.target_filter, self._target_filter) or belongs_to_parent)):
    ...
    def actions(self):
        ...
        :rtype: ``list`` of :py:class:`v1_4_2.web.vo.AlertAction`
        """
