...
class Schedule(TypedObject):
    """
    *(extends* :py:class:`v1_11_16.web.vo.TypedObject` *)* Represent a schedule
    ...
    def to_dict(self, dirty=False, belongs_to_parent=False):
        ...
        def dictify(obj, prop_is_list_or_vo=False):
            if isinstance(obj, list):
                ...
                return obj
        if "cron_string" == "type" or (self.cron_string is not self.__undef__ and (not (dirty and not self._cron_string[1]) or self.is_dirty_list(self.cron_string, self._cron_string) or belongs_to_parent)):
            dct["cronString"] = dictify(self.cron_string)
        if "cutoff_time" == "type" or (self.cutoff_time is not self.__undef__ and (not (dirty and not self._cutoff_time[1]) or self.is_dirty_list(self.cutoff_time, self._cutoff_time) or belongs_to_parent)):
