...
from delphixpy.v1_9_2.web.objects.KeyPair import KeyPair
...
class EcdsaKeyPair(KeyPair):
    """
    *(extends* :py:class:`v1_9_2.web.vo.KeyPair` *)* A key pair generated using
    the ECDSA algorithm.
    """
    ...
    def from_dict(cls, data, dirty=False, undef_enabled=True):
        ...
        if obj._signature_algorithm[0] is not None and obj._signature_algorithm[0] is not obj.__undef__:
            assert isinstance(obj._signature_algorithm[0], str), ("Expected one of ['string'], but got %s of type %s" % (obj._signature_algorithm[0], type(obj._signature_algorithm[0])))
    ...
    def to_dict(self, dirty=False, belongs_to_parent=False):
        ...
        def dictify(obj, prop_is_list_or_vo=False):
            if isinstance(obj, list):
                ...
                return obj
        if "key_size" == "type" or (self.key_size is not self.__undef__ and (not (dirty and not self._key_size[1]) or self.is_dirty_list(self.key_size, self._key_size) or belongs_to_parent)):
            dct["keySize"] = dictify(self.key_size)
        elif belongs_to_parent and self.key_size is self.__undef__:
            dct["keySize"] = 256
        if "signature_algorithm" == "type" or (self.signature_algorithm is not self.__undef__ and (not (dirty and not self._signature_algorithm[1]) or self.is_dirty_list(self.signature_algorithm, self._signature_algorithm) or belongs_to_parent)):
