...
class VirtualSourceOperations(TypedObject):
    """
    *(extends* :py:class:`v1_8_2.web.vo.TypedObject` *)* Describes operations
    ...
    def to_dict(self, dirty=False, belongs_to_parent=False):
        ...
        def dictify(obj, prop_is_list_or_vo=False):
            if isinstance(obj, list):
                ...
                return obj
        if "configure_clone" == "type" or (self.configure_clone is not self.__undef__ and (not (dirty and not self._configure_clone[1]) or self.is_dirty_list(self.configure_clone, self._configure_clone) or belongs_to_parent)):
            dct["configureClone"] = dictify(self.configure_clone, prop_is_list_or_vo=True)
        if "post_refresh" == "type" or (self.post_refresh is not self.__undef__ and (not (dirty and not self._post_refresh[1]) or self.is_dirty_list(self.post_refresh, self._post_refresh) or belongs_to_parent)):
            dct["postRefresh"] = dictify(self.post_refresh, prop_is_list_or_vo=True)
        if "post_rollback" == "type" or (self.post_rollback is not self.__undef__ and (not (dirty and not self._post_rollback[1]) or self.is_dirty_list(self.post_rollback, self._post_rollback) or belongs_to_parent)):
        ...
        if "post_start" == "type" or (self.post_start is not self.__undef__ and (not (dirty and not self._post_start[1]) or self.is_dirty_list(self.post_start, self._post_start) or belongs_to_parent)):
            dct["postStart"] = dictify(self.post_start, prop_is_list_or_vo=True)
        if "post_stop" == "type" or (self.post_stop is not self.__undef__ and (not (dirty and not self._post_stop[1]) or self.is_dirty_list(self.post_stop, self._post_stop) or belongs_to_parent)):
            dct["postStop"] = dictify(self.post_stop, prop_is_list_or_vo=True)
        if "pre_refresh" == "type" or (self.pre_refresh is not self.__undef__ and (not (dirty and not self._pre_refresh[1]) or self.is_dirty_list(self.pre_refresh, self._pre_refresh) or belongs_to_parent)):
            dct["preRefresh"] = dictify(self.pre_refresh, prop_is_list_or_vo=True)
        if "pre_rollback" == "type" or (self.pre_rollback is not self.__undef__ and (not (dirty and not self._pre_rollback[1]) or self.is_dirty_list(self.pre_rollback, self._pre_rollback) or belongs_to_parent)):
        ...
        if "pre_start" == "type" or (self.pre_start is not self.__undef__ and (not (dirty and not self._pre_start[1]) or self.is_dirty_list(self.pre_start, self._pre_start) or belongs_to_parent)):
            dct["preStart"] = dictify(self.pre_start, prop_is_list_or_vo=True)
        if "pre_stop" == "type" or (self.pre_stop is not self.__undef__ and (not (dirty and not self._pre_stop[1]) or self.is_dirty_list(self.pre_stop, self._pre_stop) or belongs_to_parent)):
    ...
    def is_dirty(self):
        return any([self._configure_clone[1], self._post_refresh[1], self._post_rollback[1], self._post_snapshot[1], self._post_start[1], self._post_stop[1], self._pre_refresh[1], self._pre_rollback[1], self._pre_snapshot[1], self._pre_start[1], self._pre_stop[1]])
    ...
    def configure_clone(self):
        ...
        :rtype: ``list`` of :py:class:`v1_8_2.web.vo.SourceOperation`
        """
