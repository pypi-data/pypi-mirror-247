...
def _aigc_post_product(country, request, function):
...
def TaskNotify(country, taskUUID, status, msg, dataStr):
    ...
    if r1 != 0:
        ...
        return False
    rsp.ParseFromString(r3)
    taskUtils.taskPrint(taskUUID, f"receive server ok={rsp.ok} notify_id = {rsp.notify_id}")
