...
from core_base.utils.models import get_all_models_objects
...
class InitSettingsViewSet(APIView):
    ...
    def post(self, request):
        is_init = SystemConfig.objects.count()
        if is_init > 0:
            return DetailResponse(msg="初始化失败")
        os.system("python3 manage.py init -y")
        dispatch.refresh_system_config()
        return DetailResponse(msg="初始化成功")