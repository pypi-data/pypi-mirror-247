...
class UserViewSet(CustomModelViewSet):
    ...
    def sso_token(self, request, *args, **kwargs):
        ...
        cache.set(token, request.user.id, timeout=300)
        return DetailResponse(data=token)
    ...
    def sso_user_info(self, request, *args, **kwargs):
        ...
        uid = cache.get(token, {})
        user_info = Users.objects.filter(id=uid).values("username", "name")
        cache.delete(token)
        return DetailResponse(data=user_info)
