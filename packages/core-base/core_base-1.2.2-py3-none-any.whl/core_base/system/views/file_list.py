import base64, uuid, datetime, hashlib, json, os, random
...
from core_base.system.models import FileList, media_file_name
from core_base.system.views.ueditor_settings import ueditor_upload_settings, ueditor_settings
from core_base.utils.json_response import DetailResponse, ErrorResponse
from core_base.utils.serializers import CustomModelSerializer
from core_base.utils.string_util import format_bytes
...
class FileViewSet(CustomModelViewSet):
    ...
    def get_ueditor_settings(self, request):
        return HttpResponse(json.dumps(ueditor_upload_settings, ensure_ascii=False),
                            content_type="sobase/javascript")
    ...
    def upload_file(self, request):
        ...
        if action == "uploadscrawl":
            ...
            if file is None:
                return HttpResponse(json.dumps(u"{'state:'ERROR'}"), content_type="sobase/javascript")
        ...
        if action in upload_allow_type:
            ...
            if not upload_original_ext.lower() in allow_type:
                ...
                return HttpResponse({"state": state}, content_type="sobase/javascript")
        ...
        return HttpResponse(json.dumps(return_info, ensure_ascii=False), content_type="sobase/javascript")

    @csrf_exempt
    @action(methods=['post'], detail=False, url_path='upload_allow_file', permission_classes=[AllowAny],
    ...
    def upload_allow_file(self, request, *args, **kwargs):
        ...
        for item in files:
            ext = [".png", ".gif", ".jpg", ".mp3", ".mp4", ".zip", ".xlsx", ".xls", ".doc", ".docx", ".pptx", ".pdf",
                   ".txt", ".tar"]
            fn, t = os.path.splitext(item.name)
            if t.lower() not in ext:
                t = ".png"
            directory_path = os.path.join(BASE_DIR, fid)
            if not os.path.exists(directory_path):
                os.makedirs(directory_path)
            filename = f"{uuid.uuid4().hex}_{fn}{t.lower()}"
            filePath = directory_path + "/" + filename
            with open(filePath, 'wb') as f:
                for c in item.chunks():
                    f.write(c)
                f.close()
            fileNames.append(f"{settings.MEDIA_URL}{filename}")
        return DetailResponse(fileNames)
