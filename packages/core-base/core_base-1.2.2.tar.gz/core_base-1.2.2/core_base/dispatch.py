...
def get_dictionary_choices(dictionarg_key="") -> tuple:
    try:
        dictionary_children = get_dictionary_values(dictionarg_key).get('children', [])
        return tuple(((int(item.get('value', 0)), item.get('label', '未知')) for item in dictionary_children))
    except Exception as e:
        print("get_dictionary_choices exception", e.args)
        return ()
...
def get_import_dictionary_choices(dictionarg_key="") -> dict:
    try:
        ...
        for item in dictionary_children:
            choices_infos.update({item.get('label', '未知'): int(item.get('value', 0))})
        return choices_infos
    except Exception as e:
        print("get_import_dictionary_choices exception", e.args)
        return {}
