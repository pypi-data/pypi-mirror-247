...
class Quantization:
    ...
    def hybrid_convert(self, ):
        ...
        ret = model.hybrid_quantization_step1(dataset=self.dataset_file, proposal=False)
        if ret != 0:
    ...
    def convert(self, quantize=False):
        ...
        ret = model.build(do_quantization=quantize, dataset=self.dataset_file, rknn_batch_size=1)
        if ret != 0:
