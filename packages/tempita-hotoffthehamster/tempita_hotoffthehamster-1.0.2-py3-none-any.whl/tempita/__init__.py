...
from tempita.compat3 import basestring_, coerce_text, is_unicode
...
class Template:
    ...
    def __repr__(self):
        return "<{} {} name={!r}>".format(
            self.__class__.__name__, hex(id(self))[2:], self.name
        )
...
class bunch(dict):
    ...
    def __repr__(self):
        ...
        items.sort()
        return "<{} {}>".format(
            self.__class__.__name__, " ".join([f"{k}={v!r}" for k, v in items])
        )
...
def html_quote(value, force=True):
    ...
    if sys.version >= "3" and isinstance(value, bytes):
        value = escape(value.decode("latin1"))
...
def attr(**kw):
    ...
    for name, value in kw:
        ...
        if name.endswith("_"):
            name = name[:-1]
        parts.append(f'{html_quote(name)}="{html_quote(value)}"')
...
class TemplateDef:
    ...
    def __repr__(self):
        return "<tempita function {}({}) at {}:{}>".format(
            self._func_name, self._func_signature, self._template.name, self._pos
        )
...
def lex(s, name=None, trim_whitespace=True, line_offset=0, delimiters=None):
    ...
    token_re = re.compile(
        r"{}|{}".format(re.escape(delimiters[0]), re.escape(delimiters[1]))
    )
