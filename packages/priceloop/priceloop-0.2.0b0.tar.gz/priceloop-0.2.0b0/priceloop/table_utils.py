...
from typing import Any, Callable, List, Literal, Optional, TypeAlias
...
ImportMode: TypeAlias = TableImportMode
...
class TableUtils:
    ...
    def import_data(
        ...
        client: AuthenticatedClient,
        import_mode: ImportMode = ImportMode.REPLACE_DATA,
    ) -> Any:
        """
    ...
    def patch_table(
        ...
        *,
        ...
        """
        batch_num = (len(table_df) // batch_size) + 1
        ...
        for i in range(batch_num):
            ...
            if mode == PatchMode.Append:
                json_body.append(TableUtils.get_patch_append(data_columns, table_df.iloc[start_index:end_index]))

            elif mode == PatchMode.Delete:
                json_body.append(
                    TableUtils.get_patch_delete(
                        match_on_columns,
                        "delete_matching",
                        table_df.iloc[start_index:end_index],
                    )
                )
            elif mode == PatchMode.InsertOnConflictFail:
                json_body.append(
                    TableUtils.get_patch_insert(
                        match_on_columns,
                        data_columns,
                        table_df.iloc[start_index:end_index],
                        "fail",
