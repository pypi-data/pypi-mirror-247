...
def cli(
    ...
    if authentication_document:
        ...
        token = tokens.create_service_token(
            auth_doc=authentication_document,
            ctx=ctx,
            scope=scope_list,
        )
...
@click.argument("org-id", default=None)
@click.argument("parent-issuer", default=None)
@click.pass_context
...
def list_connectors(ctx, **kwargs):
    ...
    table = connectors.format_connectors_as_text(ctx, results, **kwargs)
    print(table)
...
def list_billing_accounts(ctx, **kwargs):
    ...
    table = billing.format_accounts(ctx, accounts.billing_accounts, **kwargs)
    print(table)
...
@click.option("--get-subscription-data", is_flag=True, default=False)
@click.pass_context
def list_billing_subscriptions(ctx, **kwargs):
    result = billing.list_subscriptions(ctx, **kwargs)
    table = billing.format_subscriptions(ctx, result.billing_subscriptions)
    print(table)
...
@click.option("--billing-account-id", default=None)
@click.option("--push-to-prometheus", type=bool, default=True)
@click.pass_context
...
def list_feature_tags(ctx, **kwargs):
    ids = features.list_feature_tags(ctx, **kwargs)
    table = features.format_feature_tags_as_text(ctx, ids)
    print(table)
...
def add_feature_tag(ctx, **kwargs):
    result = features.add_feature_tag(ctx, **kwargs)
    output_entry(ctx, result.to_dict())
...
def show_feature_tag(ctx, name):
    result = features.show_feature_tag(ctx, name=name)
    output_entry(ctx, result.to_dict())
...
def delete_feature_tag(ctx, name):
    features.delete_feature_tag(ctx, name=name)
