...
class Telnet:
    ...
    def __get_mode_switch_cli(self, mode_config: dict) -> List[str]:
        ...
        for key, value in mode_config.items():
            if re.search(key, self.model, re.I):
                cli = value
                break
        else:
        ...
        if switch_mode:
            self.send_cli(self.__get_mode_switch_cli(self.setting.get(switch_mode).get('super_mode')),
                          key_replace={'super_user': self.super_user, 'super_password': self.super_password,
                                       'password': self.password, 'factory_user': self.factory_username,
    ...
    def connect_aws(self, platform='star.inhandcloud.cn'):
        ...
        awscloud_path = {'er605': '/var/backups/awscloud.json', 'er805': '/tmp/backups/awscloud.json',
                         'odu2002': '/var/backups/awscloud.json', 'ir624': '/var/backups/awscloud.json'}.get(
            self.model.lower())
        self.super_mode()
        # 使用echo命令直接覆盖原文件内容
        command = 'echo -e \'{\n"server": "' + platform + '",\n"keepalive": 60\n}\'  > ' + awscloud_path
        # self.send_cli({"cd /tmp/backups": "/tmp/backups #"})
        self.send_cli({command: "/www #"})
        # self.send_cli({"more /tmp/backups/awscloud.cfg": "amazonaws.com.cn"})
...
if __name__ == '__main__':
    ...
    a = Telnet('ER605', '192.168.2.1', 'inhand', '64391099@inhand', connect_timeout=20, connect_interval=5)
