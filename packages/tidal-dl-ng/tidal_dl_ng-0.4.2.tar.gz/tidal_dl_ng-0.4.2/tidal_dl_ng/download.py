...
from helper.decryption import decrypt_file, decrypt_security_token
from helper.exceptions import MediaUnknown
from helper.path import check_file_exists, format_path_media, path_validate
from helper.wrapper import WrapperLogger
from logger import logger_gui
from metadata import Metadata
from model.gui_data import ProgressBars
from pathvalidate import sanitize_filepath
from rich.progress import Progress
from tidalapi import Album, Playlist, Session, Track, UserPlaylist, Video
...
class Download:
    ...
    def metadata_write(self, track: Track, path_file: str):
        result: bool = False
        release_date: str = track.album.release_date.strftime("%Y-%m-%d") if track.album.release_date else "N/A"
        m: Metadata = Metadata(
            path_file=path_file,
            lyrics=track.lyrics().text if hasattr(track, "lyrics") else "",
            copy_right=track.copyright,
            title=track.name,
    ...
    def list(
        ...
        media_type: MediaType = None,
        file_template: str = None,
        list_media: Album | Playlist | UserPlaylist = None,
        video_download: bool = False,
        progress_gui: ProgressBars = None,
        progress: Progress = None,
        download_delay: bool = True,
        fn_logger: Logger | WrapperLogger = logger_gui,
    ):
        if not list_media:
        ...
        items = list_media.tracks(limit=999) if video_download else list_media.tracks(limit=999)
        ...
        p_task1 = progress.add_task(f"[green]List '{list_media.name[:30]}'", total=len(items), visible=progress_stdout)
