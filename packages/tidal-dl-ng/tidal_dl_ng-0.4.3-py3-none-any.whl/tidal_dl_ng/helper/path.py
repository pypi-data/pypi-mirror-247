...
from tidalapi import Album, Playlist, Track, UserPlaylist, Video
...
def format_path_media(fmt_template: str, media: Track | Album | Playlist | UserPlaylist | Video) -> str:
    result = fmt_template
...
def get_fn_format(media: Track | Album | Playlist | UserPlaylist | Video) -> Callable:
    result = None
...
def path_validate(path_file: str, adapt: bool = False) -> (bool, str):
    result: bool = False
    length_max_path: int = os.pathconf("/", "PC_PATH_MAX")
    length_max_file: int = os.pathconf("/", "PC_NAME_MAX")
