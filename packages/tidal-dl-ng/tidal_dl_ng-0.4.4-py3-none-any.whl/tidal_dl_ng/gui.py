...
import qdarktheme
from config import Settings, Tidal
from constants import QualityVideo
from model.gui_data import ProgressBars, ResultSearch
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def menu_context_tree_lists(self, point):
        ...
        action = menu.addAction("Dowloading Playlist", lambda: self.download_list_media_thread(point))

        menu.exec(self.tr_lists_user.mapToGlobal(point))

    def download_list_media_thread(self, point):
        self.thread_it(self.download_list_media, point)
        self.thread_it(self.list_items_show, point=point)

    def download_list_media(self, point: QtCore.QPoint):
    ...
    def _init_signals(self):
        self.b_download.clicked.connect(lambda: self.thread_it(self.download_results))
    ...
    def download_item(self, media: Track | Album | Playlist | Video, dl: Download) -> bool:
        ...
        if isinstance(media, Track) or isinstance(media, Video):
        ...
        elif isinstance(media, Album) or isinstance(media, Playlist):
            ...
            self.progress_list_name(media.name)
