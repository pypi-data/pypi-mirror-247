...
async function activate(app: JupyterFrontEnd, mainMenu: IMainMenu, settings: ISettingRegistry, launcher: ILauncher): Promise<void> {
    ...
    app.commands.addCommand(getSupport, {
        ...
        execute: async () => {
            console.log(`TreeURL => ${JSON.stringify(PageConfig.getOption('treeUrl'))}`)
            console.log(`BaseURL => ${JSON.stringify(PageConfig.getOption('baseUrl'))}`)
            console.log(`AppURL => ${JSON.stringify(PageConfig.getOption('appUrl'))}`)
            const content = new GetSupportFormWidget({ username: "frivas" })
