...
class ImageInferenceV2:
    def __init__(self, img_inference_client: ImageInference, stream: bool):
        self._img_inference_client = img_inference_client
        self._stream = stream

    # Generate is OpenAI's equivalent of text to image
    def generate(
        self,
        prompt: str,
        model: str,
        ...
        response_format: Literal["PNG", "b64_json"] = "PNG",
        # fireworks specific
        negative_prompt: Optional[str] = None,
        cfg_scale: int = 7,
        clip_guidance_preset: str = "NONE",
        ...
        (height, width) = tuple(int(x) for x in size.split("x"))
        # TODO Make style preset determined by style?
        to_call = (
            self._img_inference_client.text_to_image
            if not self._stream
            else self._img_inference_client.text_to_image_async
        )
        return to_call(
            prompt=prompt,
            height=height,
            width=width,
            samples=n,
            output_image_format=response_format,
            negative_prompt=negative_prompt,
            cfg_scale=cfg_scale,
            clip_guidance_preset=clip_guidance_preset,
            steps=steps,
    ...
    def edit(
        ...
        **kwargs,
        ...
        to_call = (
            self._img_inference_client.image_to_image
            if not self._stream
            else self._img_inference_client.image_to_image_async
        )
        return to_call(
            init_image=image,
            prompt=prompt,
    ...
    def create_variation(
        ...
        **kwargs,
        ...
        raise NotImplementedError(
            "create_variation currently not supported. Stay tuned!"
        )
