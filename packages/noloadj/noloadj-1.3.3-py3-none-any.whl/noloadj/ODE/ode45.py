...
def compute_new_point(x_now,x_prev,t_prev,t_now,y_prev,y_now,
    ...
    :param y_prev: output vector at the previous iteration
    :param y_now: output vector at the present iteration
    :param diff_var_threshold_value: differences between the state variable and
        the threshold value at the present iteration
    :param diff_var_threshold_value_prev: differences between the state variable
...
def interp_state_chgt(x_prev,y_prev,y_target,t_prev,x_now,y_now,t_now,f,i_prev,
    ...
    condition = np.bitwise_and(np.sign(x_now[ind]-threshold_value)!=np.sign(
            x_prev[ind]-threshold_value),np.not_equal(ind,-1))
    condition=np.bitwise_and(condition,np.bitwise_not(np.allclose(
            x_prev[ind]-threshold_value,0.)))
    x_now,h_now,t_now,y_now=cond(condition,lambda state:compute_new_point(
        x_now,x_prev,t_prev,t_now,y_prev,y_now,x_now[ind]-threshold_value,
    x_prev[ind]-threshold_value),lambda state:state,(x_now,h_prev,t_now,y_now))
...
def next_step_simulation(x_prev,t_prev,y_prev,i_prev,c_prev,h_prev,f,tol,T,*P):
    ...
    if hasattr(f,'event'):
        for e in f.event:
            ...
            threshold_value,threshold_value_prev=get_indice(f.xnames,x_now,
                    [name]),get_indice(f.xnames,x_prev,[name])
            signe=np.where(signe_str=='<',-1,1)
            condition = np.bitwise_and(np.sign(threshold_value-seuil)==signe,
                    np.bitwise_not(np.allclose(threshold_value_prev-seuil,0.)))
            h_now = optimal_step(x_now, x_now_est, h_prev, tol,x_prev)
            x_now,h_prev,t_now,y_now=cond(condition,lambda state:
                compute_new_point(x_now,x_prev,t_prev,t_now,y_prev,y_now,
                threshold_value-seuil, threshold_value_prev-seuil),
                            lambda state:state,(x_now,h_prev,t_now,y_now))
