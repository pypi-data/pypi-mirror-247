...
from volstreet.dealingroom import SharedData, get_strangle_indices_to_trade
...
def run_client(
    client: Client, special_error_url: bool = False, notifier_level="INFO"
) -> None:
    client.set_webhook_urls()

    client.login()

    error_url_env_var = f"{client.name}_ERROR_URL" if special_error_url else "ERROR_URL"

    try:
        error_url = os.getenv(error_url_env_var)
        set_error_notification_settings("url", error_url)
    except KeyError:
        logger.warnning(f"Error URL not found for {client.name}. Using None.")
