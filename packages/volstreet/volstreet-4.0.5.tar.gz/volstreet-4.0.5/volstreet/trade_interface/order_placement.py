...
from volstreet.angel_interface.orders import handle_open_orders
from volstreet.trade_interface.blocks import (
    Option,
    Strangle,
    Straddle,
    SyntheticFuture,
...
def place_option_order_and_notify(
    ...
    **kwargs,
    ...
    if LiveFeeds.order_feed_connected():
        if not all([order in LiveFeeds.order_feed.data_bank for order in order_ids]):
            sleep(0.5)
    else:
        sleep(0.5)

    try:  # Remove this try-except block after testing for a few days
        handle_open_orders(order_ids)
...
def check_and_notify_order_placement_statuses(
    ...
    elif any(["open" in status for status in statuses]) or any(
        ["modi" in status for status in statuses]
    ):
        notifier(
