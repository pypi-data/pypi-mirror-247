...
import websocket
import ssl
from volstreet.config import logger, ERROR_NOTIFICATION_SETTINGS
...
class BaseWebsocket(ABC):
    ...
    def __init__(self, auth_token, api_key, client_code, feed_token, **kwargs):
    ...
    @classmethod
    def from_active_session(cls, **kwargs):
        ...
        return cls(auth_token, api_key, client_code, feed_token, **kwargs)
    ...
    def on_open(self, wsapp):
        ...
        for task in parallel_tasks:
            logger.debug(f"Starting parallel task: {task.__name__}")
    ...
    def retry_connect(self):
        ...
        if self.current_retry_attempt < self.MAX_RETRY_ATTEMPT:
            self.current_retry_attempt += 1
            logger.info(f"{self.websocket_type} reconnecting")
            time.sleep(self.RETRY_DELAY)
            threading.Thread(target=self.connect).start()
        else:
            logger.warning(
