...
class TestBuilderModel(unittest.TestCase):
    ...
    def test_BuilderModel_initialization(self):
        ...
        """
        BM = BuilderModel(file_model_data=None, software=None, flag_build=False, verbose=True)
    ...
    def test_BuilderModel_readYaml_REFERENCE(self):
        ...
        # act
        BM = BuilderModel(file_model_data=reference_file, case_model='conductor', software=None, flag_build=True, verbose=True)
    ...
    def compare_to_reference_PyBBQ(self, conductor_name, verbose=False, flag_plot_all=False):
        ...
        input_file_REFERENCE = os.path.join('references', 'conductors', conductor_name, conductor_name + '_REFERENCE.yaml')
        input_file_GENERATED = os.path.join(output_path, conductor_name + '.yaml')

        # act
        BM = BuilderModel(file_model_data=file_model_data, case_model='conductor', software=['PyBBQ'], flag_build=True, flag_dump_all=False,
    ...
    def compare_to_reference_LEDET(self, conductor_name, verbose=False, flag_plot_all=False):
        ...
        file_model_data = os.path.join('model_library', 'conductors', conductor_name, 'input', 'modelData_' + conductor_name + '.yaml')
        output_path = os.path.join('model_library', 'conductors', conductor_name, 'output')
        input_file_REFERENCE = os.path.join('references',    'conductors', conductor_name, conductor_name + '_REFERENCE.xlsx')
        input_file_GENERATED = os.path.join('model_library', 'conductors', conductor_name, 'output', conductor_name + '.xlsx')

        # Check whether csv and map2d files to be copied are also present
        flag_check_csv   = True if os.path.isfile(os.path.join('model_library', 'conductors', conductor_name, 'input', conductor_name + '_1.csv')) else False
        flag_check_map2d = True if os.path.isfile(os.path.join('model_library', 'conductors', conductor_name, 'input', conductor_name + '.map2d')) else False

        csv_file_GENERATED   = os.path.join('model_library', 'conductors', conductor_name, 'output', conductor_name + '_1.csv')
        map2d_file_GENERATED = os.path.join('model_library', 'conductors', conductor_name, 'output', conductor_name + '_All_WithIron_WithSelfField.map2d')
