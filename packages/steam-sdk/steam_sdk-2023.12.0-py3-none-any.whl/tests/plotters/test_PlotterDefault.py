...
class TestPlotterDefault(unittest.TestCase):
    ...
    def test_plotterModel(self, magnet_name: str = 'MBRD', flag_save_png: int = 0):
        ...
        BL = BuilderLEDET(path_magnet=file_model_data, input_model_data=BM.model_data,
                                     input_roxie_data=BM.roxie_data, input_map2d=BM.map2d_file_modified,
                                     smic_write_path=nameFileSMIC, flag_build=BM.flag_build, flag_plot_all=False,
                                     verbose=BM.verbose)

        #Data
        if BM.model_data.Options_LEDET.field_map_files.flag_modify_map2d_ribbon_cable == 1:  #Ribbon

            _, _, _, strandToHalfTurn, strandToGroup, x_strand, y_strand, _ = getParametersFromMap2d(BM.map2d_file_modified)
            ...
            _, _, _, strandToHalfTurn, strandToGroup, x_strand, y_strand, _ = getParametersFromMap2d(BM.path_map2d)
            nGroups = int(np.max(strandToGroup))
            nHalfTurns = int(np.max(strandToHalfTurn))
