...
class TestDriverPSPICE(unittest.TestCase):
    ...
    def test_runPSPICE_CSD_Plots(self, circuit_name = 'RU'):
        ...
        BM = BuilderModel(file_model_data=file_model_data, software=['PSPICE'], case_model='circuit',
                          flag_build=True, flag_dump_all=False, flag_plot_all=False,
                          output_path=path_folder_PSPICE,
                          verbose=False)
        BM.circuit_data.PostProcess.probe.probe_type = 'CSDF'
        BM.buildPSPICE()  # re-write the netlist with different probe settings
        ...
        plt.grid(True)
        plt.legend(loc='best')
        displayWaitAndClose(waitTimeBeforeMessage=.1, waitTimeAfterMessage=5)