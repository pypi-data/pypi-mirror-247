...
class PostprocsMetrics:
    ...
    def run_metrics(self):
        ...
        list_metrics_that_need_interpolation_ref = ['maximum_abs_error', 'RMSE', 'RELATIVE_RMSE','RMSE_ratio']

        #TODO: Handle that sometimes the interpolation does not make sense, because the timevectors are of different size but are interpolated to the same size
    ...
    def _RELATIVE_RMSE(y, y_ref):
        ...
        return np.sqrt((((y - y_ref)/(y_ref+avoid_zero_division)) ** 2).mean()) # np.sqrt(mean_squared_error(y, y_ref))
