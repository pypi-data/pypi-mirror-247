...
from steam_sdk.analyses.AnalysisEvent import find_IPQ_circuit_type_from_IPQ_parameters_table, \
    get_circuit_name_from_eventfile, get_circuit_type, generate_unique_event_identifier_from_eventfile, \
    get_signal_keys_from_configurations_file
...
def steam_analyze_lhc_event(settings_file_path:str, input_csv_file: str, flag_run_software: bool, software: str, overwrite_settings: dict, file_counter: int):
    ...
    if circuit_name.startswith("RCD"):
        ...
        circuit_type = get_circuit_type(circuit_name,  settings_dictionary['library_path'])
        analyze_circuit_event(settings_dictionary,os.path.join(os.getcwd(), input_csv_file), circuit_type, flag_run_software, software, file_counter)
...
def analyze_circuit_event(settings_dictionary: dict, input_csv_file: str,circuit_type: str, flag_run_software: bool, software: str, file_counter: int):
    ...
    if settings_dictionary['directory_config_files'] != None:
        ...
        # We have a new commulative config file for IPQs --> TODO: its not very elegant that the config file has the same name as the models except for IPQ...maybe we van include all circuits into onw single config file in the future

        config_file_name = f"config_{circuit_type}.yaml"
        if circuit_type.startswith("IPQ"): config_file_name = f"config_IPQ.yaml"

        full_path_to_config_file = os.path.join(settings_dictionary['directory_config_files'], config_file_name)
        signals_to_include = get_signal_keys_from_configurations_file(full_path_to_config_file, circuit_type)
    else:
        print("< no signals from the config file will be included in the model, variables entry will be empty in the cir"
              " file, causing all signals to be calculated")
    ...
    if flag_run_software== False:
        aSTEAM.data_analysis.AnalysisStepSequence = ['setup_folder_PSPICE', 'makeModel_ref', 'modifyModel_probe', 'runParsimEvent']
    else:
        aSTEAM.data_analysis.AnalysisStepSequence = ['setup_folder_PSPICE', 'makeModel_ref', 'modifyModel_probe', 'runParsimEvent', 'run_simulation']
...
def analyze_RCD_RCO_event(settings_dictionary: dict, input_csv_file: str, flag_run_software: bool, software: str, file_counter: int):
    ...
    signals_to_include = ['']
    if settings_dictionary['directory_config_files'] != None:
        full_path_to_config_file = os.path.join(settings_dictionary['directory_config_files'],
                                                f"config_RCO.yaml")
        signals_to_include = get_signal_keys_from_configurations_file(full_path_to_config_file, "RCO")
    else:
        print(
            "< no signals from the config file will be included in the model, variables entry will be empty in the cir"
            " file, causing all signals to be calculated")
    ...
    aSTEAM_1.data_analysis.AnalysisStepDefinition = {'setup_folder_PSPICE': SetUpFolder(type='SetUpFolder', simulation_name='RCO', software=[software]),
                                                     'makeModel_ref': MakeModel(type='MakeModel', model_name='BM', file_model_data='RCO',
                                                                                case_model='circuit', software=[software], simulation_name=None,
                                                                                simulation_number=None, flag_build=True, flag_dump_all=False, verbose=False,
                                                                                flag_plot_all=False, flag_json=False),
                                                     'modifyModel_probe': ModifyModelMultipleVariables(
    ...
    if settings_dictionary['directory_config_files'] != None:
        full_path_to_config_file = os.path.join(settings_dictionary['directory_config_files'],
                                                f"config_RCD.yaml")
        signals_to_include = get_signal_keys_from_configurations_file(full_path_to_config_file, "RCD")
    else:
        print(
            "< no signals from the config file will be included in the model, variables entry will be empty in the cir"
            " file, causing all signals to be calculated")
    ...
    aSTEAM_2.data_analysis.AnalysisStepDefinition = {'setup_folder_PSPICE': SetUpFolder(type='SetUpFolder', simulation_name='RCD', software=[software]),
                                                     'makeModel_ref': MakeModel(type='MakeModel', model_name='BM', file_model_data='RCD',
                                                                                ...
                                                                                flag_build=True, flag_dump_all=False, verbose=False, flag_plot_all=False, flag_json=False),
                                                     'modifyModel_probe': ModifyModelMultipleVariables(
                                                         type='ModifyModelMultipleVariables', model_name='BM',
                                                         variables_to_change=['PostProcess.probe.probe_type'],
                                                         variables_value=[['CSDF']], software=[software],
