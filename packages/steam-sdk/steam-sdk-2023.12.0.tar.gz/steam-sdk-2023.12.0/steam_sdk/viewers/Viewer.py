            ...
            for _, signal in configuration.SignalList.items():  # a signal can include multiple meas, sim, or both signals/channels
                ...
                for s, _ in enumerate(meas_signals_to_add):
                    ...
                    if group_name not in temp_dict_meas:
                        ...
                        if 'measurement_signal_time_shift' in dict_events:
                                print("Shifting measurement time") # e.g. to shift measurement signals in time by t_PC_off
                                df_meas_data['Time [s]'] = df_meas_data['Time [s]'] + dict_events['measurement_signal_time_shift']
