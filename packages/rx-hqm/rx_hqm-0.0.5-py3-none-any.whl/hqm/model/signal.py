...
def get_signal_ee(name, preffix, year, trigger):
    ...
    r0_value = all_br.loc[f"{trigger}_{year}", f"v_0"]
    r0_error = all_br.loc[f"{trigger}_{year}", f"e_0"]
    r1_value = all_br.loc[f"{trigger}_{year}", f"v_1"]
    r1_error = all_br.loc[f"{trigger}_{year}", f"e_1"]
    r2_value = all_br.loc[f"{trigger}_{year}", f"v_2"]
    r2_error = all_br.loc[f"{trigger}_{year}", f"e_2"]

    dmu = zfit.Parameter(f"dmu_ee_{preffix}", dmu_value, -15, 15)
...
def get_signal_mm(name, preffix, year, trigger):
    ...
    dmu = zfit.Parameter(f"dmu_mm_{preffix}", dmu_value, -15, 15)
