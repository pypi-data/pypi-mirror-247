...
class ZCAN_ReceiveData1(Structure):
    _fields_ = [("frame", ZCANDataObj), ("timestamp", c_ulonglong)]
...
class ZCAN(object):
    ...
    def ReceiveData(self, chn_handle, rcv_num, wait_time=c_int(-1)):
        try:
            rcv_can_msgs = (ZCANDataObj * rcv_num)()
            ret = self.__dll.ZCAN_ReceiveData(chn_handle, byref(rcv_can_msgs), rcv_num, wait_time)
