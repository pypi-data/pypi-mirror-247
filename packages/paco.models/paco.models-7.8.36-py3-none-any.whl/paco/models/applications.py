...
@implementer(schemas.IPacoServiceHook)
class PacoServiceHook():
    paco_service_hook = FieldProperty(schemas.IPacoServiceHook['paco_service_hook'])

@implementer(schemas.IPacoServiceHookItem)
class PacoServiceHookItem(Named, Deployable):
    service = FieldProperty(schemas.IPacoServiceHookItem['service'])
    config = FieldProperty(schemas.IPacoServiceHookItem['config'])

@implementer(schemas.IPacoServiceHookConfig)
class PacoServiceHookConfig(Named, dict):
    """Paco Service Configuration"""
    pass

@implementer(schemas.IFilePermissions)
class FilePermissions():
    group = FieldProperty(schemas.IFilePermissions['group'])
    owner = FieldProperty(schemas.IFilePermissions['owner'])
    mode = FieldProperty(schemas.IFilePermissions['mode'])
...
@implementer(schemas.IS3BucketLifecycleRule)
class S3BucketLifecycleRule(Parent, Deployable):
   expiration_in_days = FieldProperty(schemas.IS3BucketLifecycleRule['expiration_in_days'])
   id = FieldProperty(schemas.IS3BucketLifecycleRule['id'])
...
class S3Bucket(Resource, Deployable, PacoServiceHook):
    ...
    lifecycle_rules = FieldProperty(schemas.IS3Bucket['lifecycle_rules'])
...
class EBS(ApplicationResource):
    ...
    tags = FieldProperty(schemas.IEBS['tags'])

    def resolve_ref(self, ref):
        return self.resolve_ref_obj.resolve_ref(ref)

@implementer(schemas.IAmazonLinuxLivePatching)
class AmazonLinuxLivePatching(Named, Enablable):
    release_version = FieldProperty(schemas.IAmazonLinuxLivePatching['release_version'])
    kernel_version = FieldProperty(schemas.IAmazonLinuxLivePatching['kernel_version'])
    s3_bucket = FieldProperty(schemas.IAmazonLinuxLivePatching['s3_bucket'])
...
class EC2LaunchOptions(Named):
    ...
    amazon_linux_live_patching = FieldProperty(schemas.IEC2LaunchOptions['amazon_linux_live_patching'])
...
class ECSCapacityProvider(Named, Deployable):
    ...
    def get_aws_name(self):
        ...
        aws_name = f"{asg.netenv_name}-{asg.env_name}-{asg.app_name}-{asg.group_name}-{asg.name}"
        if aws_name.startswith('aws') or aws_name.startswith('ecs') or aws_name.startswith('fargate'):
            aws_name = f'cp-{aws_name}'
        return aws_name
...
class ECSASGConfiguration(Named, Deployable):
...
@implementer(schemas.IScriptManagerCustomScripts)
class ScriptManagerCustomScripts(Named, dict):
    pass

@implementer(schemas.IScriptManagerCustomScript)
class ScriptManagerCustomScript(Named, FilePermissions):
    content = FieldProperty(schemas.IScriptManagerCustomScript['content'])
    location = FieldProperty(schemas.IScriptManagerCustomScript['location'])
...
class ScriptManager(Named):
    ...
    custom = FieldProperty(schemas.IScriptManager['custom'])
...
class ECSLogging(Named, CloudWatchLogRetention):
    ...
    awslogs_one_cluster_group = FieldProperty(schemas.IECSLogging['awslogs_one_cluster_group'])
    awslogs_resource_app_name = FieldProperty(schemas.IECSLogging['awslogs_resource_app_name'])
    awslogs_create_log_group = FieldProperty(schemas.IECSLogging['awslogs_create_log_group'])
...
class ECSContainerDefinition(Named):
    ...
    ignore_image_changes =FieldProperty(schemas.IECSContainerDefinition['ignore_image_changes'])
...
class ECSTaskDefinition(Named):
    ...
    task_role = FieldProperty(schemas.IECSTaskDefinition['task_role'])
...
class ECSService(Named, Monitorable):
    ...
    ignore_capacity_changes = FieldProperty(schemas.IECSService['ignore_capacity_changes'])
...
class ECSServices(Resource, Monitorable):
    ...
    ignore_image_changes = FieldProperty(schemas.IECSServices['ignore_image_changes'])
    ignore_capacity_changes = FieldProperty(schemas.IECSServices['ignore_capacity_changes'])
...
@implementer(schemas.IECRRepositoryImageScanning)
class ECRRepositoryImageScanning(Named):
    scan_on_push = FieldProperty(schemas.IECRRepositoryImageScanning['scan_on_push'])

@implementer(schemas.IECRRepositoryLifecyclePolicies)
class ECRRepositoryLifecyclePolicies(Named, dict):
    pass

@implementer(schemas.IECRRepositoryLifecyclePolicy)
class ECRRepositoryLifecyclePolicy(Named):
    rule_priority = FieldProperty(schemas.IECRRepositoryLifecyclePolicy['rule_priority'])
    description = FieldProperty(schemas.IECRRepositoryLifecyclePolicy['description'])
    tag_status = FieldProperty(schemas.IECRRepositoryLifecyclePolicy['tag_status'])
    tag_prefix_list = FieldProperty(schemas.IECRRepositoryLifecyclePolicy['tag_prefix_list'])
    count_type = FieldProperty(schemas.IECRRepositoryLifecyclePolicy['count_type'])
    count_number = FieldProperty(schemas.IECRRepositoryLifecyclePolicy['count_number'])


@implementer(schemas.IIAMIdentityProviderRole)
class IAMIdentityProviderRole(Named, Deployable, dict):
    policy_actions = FieldProperty(schemas.IIAMIdentityProviderRole['policy_actions'])
    provider = FieldProperty(schemas.IIAMIdentityProviderRole['provider'])
    repository_filters = FieldProperty(schemas.IIAMIdentityProviderRole['repository_filters'])

@implementer(schemas.IIAMIdentityProviderRoles)
class IAMIdentityProviderRoles(Named, dict):
    pass

# ECR: Elastic Container Repository
@implementer(schemas.IECRReplicationRule)
class ECRReplicationRule(Named, Enablable):
    regions = FieldProperty(schemas.IECRReplicationRule['regions'])
    repositories = FieldProperty(schemas.IECRReplicationRule['repositories'])

@implementer(schemas.IECRReplicationRules)
class ECRReplicationRules(Named, dict):
    pass

@implementer(schemas.IECRReplicationConfiguration)
class ECRReplicationConfiguration(Resource):
    rules = FieldProperty(schemas.IECRReplicationConfiguration['rules'])
...
class ECRRepository(Resource):
    ...
    external_repository_arn = FieldProperty(schemas.IECRRepository['external_repository_arn'])
    image_scanning = FieldProperty(schemas.IECRRepository['image_scanning'])
    lifecycle_policies = FieldProperty(schemas.IECRRepository['lifecycle_policies'])
    ...
    prefix_environment_name = FieldProperty(schemas.IECRRepository['prefix_environment_name'])
    ...
    identity_provider_roles = FieldProperty(schemas.IECRRepository['identity_provider_roles'])
    ...
    def full_repository_name(self):
        if hasattr(self, '_external_repository_name') and self._external_repository_name != None:
            return self._external_repository_name
        if self.repository_name != None:
            repository_name = self.repository_name
        if self.prefix_environment_name == True or self.repository_name == None:
            if self.repository_name == None:
                repository_name = self.name
            repo_parts = self.paco_ref_parts.split('.')
            netenv_name = repo_parts[1]
            env_name = repo_parts[2]
            app_name = repo_parts[5]
            res_grp_name = repo_parts[7]
            repository_name = f'{netenv_name}-{env_name}-{app_name}-{res_grp_name}-{repository_name}'
        return repository_name

    def repository_arn(self, region, account_id):
        if self.external_repository_arn != None:
            return self.external_repository_arn
        return f'arn:aws:ecr:{region}:{account_id}:repository/{self.full_repository_name}'

    def resolve_ref(self, ref):
        if ref.last_part == 'arn':
            if self.external_repository_arn:
                return self.external_repository_arn
        elif ref.last_part == 'name':
            return self.full_repository_name
        return self.stack
...
class TargetGroup(Resource, PortProtocol, Monitorable, Deployable):
    type = 'TargetGroup'
...
class ListenerRule(Named, Deployable):
    ...
    def resolve_ref(self, ref):
        if ref.last_part == 'host':
            return self.host

@implementer(schemas.IListenerDefaultActionFixedResponse)
class ListenerDefaultActionFixedResponse(Named):
    content_type = FieldProperty(schemas.IListenerDefaultActionFixedResponse['content_type'])
    message_body = FieldProperty(schemas.IListenerDefaultActionFixedResponse['message_body'])
    status_code = FieldProperty(schemas.IListenerDefaultActionFixedResponse['status_code'])

@implementer(schemas.IListenerDefaultAction)
class ListenerDefaultAction(Named):
    redirect = FieldProperty(schemas.IListenerDefaultAction['redirect'])
    target_group = FieldProperty(schemas.IListenerDefaultAction['target_group'])
    fixed_response = FieldProperty(schemas.IListenerDefaultAction['fixed_response'])
...
class Listener(Named, PortProtocol):
    default_action = FieldProperty(schemas.IListener['default_action'])
    ...
    webacl_id = FieldProperty(schemas.IListener['webacl_id'])
...
class LoadBalancer(ApplicationResource):
    cost_disabled = FieldProperty(schemas.ILoadBalancer['cost_disabled'])
    target_groups = FieldProperty(schemas.ILoadBalancer['target_groups'])
    listeners = FieldProperty(schemas.ILoadBalancer['listeners'])
    ignore_listener_rule_target_group_changes = FieldProperty(schemas.ILoadBalancer['ignore_listener_rule_target_group_changes'])
    ...
    cross_zone_balancing = FieldProperty(schemas.ILoadBalancer['cross_zone_balancing'])

    def resolve_ref(self, ref):
        return self.resolve_ref_obj.resolve_ref(ref)

    def get_aws_name(self, hash_long_names=False):
        "LoadBalancer Name for AWS"
        netenv = get_parent_by_interface(self, schemas.INetworkEnvironment)
        env = get_parent_by_interface(self, schemas.IEnvironment)
        app = get_parent_by_interface(self, schemas.IApplication)
        resource_group = get_parent_by_interface(self, schemas.IResourceGroup)
        return self.create_resource_name_join(
            name_list=[netenv.name, env.name, app.name, resource_group.name, self.name],
            separator='',
...
class LambdaFunctionCode(Parent):
    ...
    layer_zipfile = FieldProperty(schemas.ILambdaFunctionCode['layer_zipfile'])
...
@implementer(schemas.ILambdaFunctionUrl)
class LambdaFunctionUrl(Named, Enablable):
    auth_type = FieldProperty(schemas.ILambdaFunctionUrl['auth_type'])
    qualifier = FieldProperty(schemas.ILambdaFunctionUrl['qualifier'])

@implementer(schemas.ILambda)
class Lambda(ApplicationResource, Monitorable, CloudWatchLogRetention):
    """
    Lambda Function resource
    """
    account = FieldProperty(schemas.ILambda['account'])
    ...
    identity_provider_roles = FieldProperty(schemas.ILambda['identity_provider_roles'])
    ...
    s3_buckets = FieldProperty(schemas.ILambda['s3_buckets'])
...
class SNSTopic(Enablable, Resource):
    ...
    events_bridge_notification_access = FieldProperty(schemas.ISNSTopic['events_bridge_notification_access'])
...
class CloudFrontLambdaFunctionAssocation(Named):
    ...
    lambda_version = FieldProperty(schemas.ICloudFrontLambdaFunctionAssocation['lambda_version'])
...
class CloudFrontDefaultCacheBehavior(Named):
    ...
    response_headers_policy_id = FieldProperty(schemas.ICloudFrontDefaultCacheBehavior['response_headers_policy_id'])
...
@implementer(schemas.ICloudFrontSecurityHeadersConfig)
class CloudFrontSecurityHeadersConfig(Named, Deployable):
    content_security_policy = FieldProperty(schemas.ICloudFrontSecurityHeadersConfig['content_security_policy'])
    referrer_policy = FieldProperty(schemas.ICloudFrontSecurityHeadersConfig['referrer_policy'])
    frame_options_policy = FieldProperty(schemas.ICloudFrontSecurityHeadersConfig['frame_options_policy'])
    content_type_options = FieldProperty(schemas.ICloudFrontSecurityHeadersConfig['content_type_options'])
    strict_transport_security_policy = FieldProperty(schemas.ICloudFrontSecurityHeadersConfig['strict_transport_security_policy'])
    xss_protection_policy = FieldProperty(schemas.ICloudFrontSecurityHeadersConfig['xss_protection_policy'])


@implementer(schemas.ICloudFrontResponseHeaderPolicies)
class CloudFrontResponseHeaderPolicies(Named, dict):
    pass

@implementer(schemas.ICloudFrontResponseHeaderPolicy)
class CloudFrontResponseHeaderPolicy(Named, Deployable):
    security_headers_config = FieldProperty(schemas.ICloudFrontResponseHeaderPolicy['security_headers_config'])

@implementer(schemas.ICloudFrontGenericResponseHeaderPolicy)
class CloudFrontGenericResponseHeaderPolicy(Named):
    response_header_value = FieldProperty(schemas.ICloudFrontGenericResponseHeaderPolicy['response_header_value'])
    override = FieldProperty(schemas.ICloudFrontGenericResponseHeaderPolicy['override'])

@implementer(schemas.ICloudFrontStrictTransportHeaderPolicy)
class CloudFrontStrictTransportHeaderPolicy(Named):
    access_control_max_age = FieldProperty(schemas.ICloudFrontStrictTransportHeaderPolicy['access_control_max_age'])
    include_subdomains = FieldProperty(schemas.ICloudFrontStrictTransportHeaderPolicy['include_subdomains'])
    override = FieldProperty(schemas.ICloudFrontStrictTransportHeaderPolicy['override'])
    preload = FieldProperty(schemas.ICloudFrontStrictTransportHeaderPolicy['preload'])

@implementer(schemas.ICloudFrontXSSProtectionHeaderPolicy)
class CloudFrontXSSProtectionHeaderPolicy(Named):
    mode_block = FieldProperty(schemas.ICloudFrontXSSProtectionHeaderPolicy['mode_block'])
    override = FieldProperty(schemas.ICloudFrontXSSProtectionHeaderPolicy['override'])
    protection = FieldProperty(schemas.ICloudFrontXSSProtectionHeaderPolicy['protection'])
    report_uri = FieldProperty(schemas.ICloudFrontXSSProtectionHeaderPolicy['report_uri'])

@implementer(schemas.ICloudFrontLogging)
class CloudFrontLogging(Named, Deployable):
    s3_bucket = FieldProperty(schemas.ICloudFrontLogging['s3_bucket'])
    include_cookies = FieldProperty(schemas.ICloudFrontLogging['include_cookies'])
    prefix = FieldProperty(schemas.ICloudFrontLogging['prefix'])
...
class CloudFront(ApplicationResource, Deployable, Monitorable):
    ...
    response_header_policies = FieldProperty(schemas.ICloudFront['response_header_policies'])
    ...
    logging = FieldProperty(schemas.ICloudFront['logging'])
...
@implementer(schemas.IRDSAuroraGlobalCluster)
class RDSAuroraGlobalCluster(Named, Enablable):
    pass
...
class RDSAurora(RDS):
    ...
    global_cluster = FieldProperty(schemas.IRDSAurora['global_cluster'])
    global_cluster_identifier = FieldProperty(schemas.IRDSAurora['global_cluster_identifier'])
...
@implementer(schemas.IDeploymentPipelineCodeStarConnectionSourceAction)
class DeploymentPipelineCodeStarConnectionSourceAction():
    detect_changes = FieldProperty(schemas.IDeploymentPipelineCodeStarConnectionSourceAction['detect_changes'])
    owner = FieldProperty(schemas.IDeploymentPipelineCodeStarConnectionSourceAction['owner'])
    repository = FieldProperty(schemas.IDeploymentPipelineCodeStarConnectionSourceAction['repository'])
    codestar_connection_arn = FieldProperty(schemas.IDeploymentPipelineCodeStarConnectionSourceAction['codestar_connection_arn'])

@implementer(schemas.IDeploymentPipelineSourceGitHub)
class DeploymentPipelineSourceGitHub(DeploymentPipelineStageAction, DeploymentPipelineCodeStarConnectionSourceAction):
...
class DeploymentPipelineSourceBitBucket(DeploymentPipelineStageAction, DeploymentPipelineCodeStarConnectionSourceAction):
...
@implementer(schemas.IEnvironmentVariable)
class EnvironmentVariable(Parent):
    key = FieldProperty(schemas.IEnvironmentVariable['key'])
    value = FieldProperty(schemas.IEnvironmentVariable['value'])
...
class DeploymentPipelineBuildCodeBuild(DeploymentPipelineStageAction):
    ...
    environment_variables = FieldProperty(schemas.IDeploymentPipelineBuildCodeBuild['environment_variables'])
...
class DeploymentPipelineDeployCodeDeploy(DeploymentPipelineStageAction, PacoServiceHook):
    ...
    input_artifact_action = FieldProperty(schemas.IDeploymentPipelineDeployCodeDeploy['input_artifact_action'])
...
class DeploymentPipeline(ApplicationResource, Monitorable):
    ...
    def get_account_ref(self):
        if self.configuration.account != None:
            return Reference(self.configuration.account)
        else:
            return Reference(super().get_account().paco_ref)
...
class EFSMount(Resource):
    ...
    read_only_mode = FieldProperty(schemas.IEFSMount['read_only_mode'])
...
@implementer(schemas.IWAFWebACLRuleOverrideAction)
class WAFWebACLRuleOverrideAction(Named):
    count = FieldProperty(schemas.IWAFWebACLRuleOverrideAction['count'])
    none = FieldProperty(schemas.IWAFWebACLRuleOverrideAction['none'])

@implementer(schemas.IWAFWebACLRuleActionOverride)
class WAFWebACLRuleActionOverride(Named):
    action_to_use = FieldProperty(schemas.IWAFWebACLRuleActionOverride['action_to_use'])
    rule_name = FieldProperty(schemas.IWAFWebACLRuleActionOverride['rule_name'])
...
class WAFWebACLRuleManagedRuleGroup(Named, Deployable):
    ...
    excluded_rules = FieldProperty(schemas.IWAFWebACLRuleManagedRuleGroup['excluded_rules'])
    rule_action_overrides = FieldProperty(schemas.IWAFWebACLRuleManagedRuleGroup['rule_action_overrides'])
...
class WAFWebACLRule(Named, Deployable):
    ...
    override_action = FieldProperty(schemas.IWAFWebACLRule['override_action'])

@implementer(schemas.IWAFWebACLRules)
class WAFWebACLRules(Named, dict):
    pass

@implementer(schemas.IWAFWebACLLogging)
class WAFWebACLLogging(Named, Enablable):
    log_retention_days = FieldProperty(schemas.IWAFWebACLLogging['log_retention_days'])
...
class WAFWebACL(Resource):
    ...
    logging = FieldProperty(schemas.IWAFWebACL['logging'])
...
@implementer(schemas.IPacoService)
class PacoService(Resource, PacoServiceHookItem):
    pass