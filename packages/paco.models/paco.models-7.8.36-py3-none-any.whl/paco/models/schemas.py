...
class InvalidListenerDefaultActionContentType(zope.schema.ValidationError):
    __doc__ = 'Scope must be one of: text/plain | text/css | text/html | application/javascript | application/json'

def isValidListenerDefaultActionContentType(value):
    if value not in ('text/plain', 'text/css', 'text/html', 'application/javascript', 'application/json'):
        raise InvalidListenerDefaultActionContentType
    return True
...
class InvalidVpcPeerAssocatePrivateHostedZone(zope.schema.ValidationError):
    __doc__ = 'Scope of associate_private_hosted_zone must be one of: associate | disassociate'

def isValidVpcPeerAssocatePrivateHostedZone(value):
    if value not in ('associate', 'disassociate'):
        raise InvalidVpcPeerAssocatePrivateHostedZone
    return True
...
valid_legacy_flags = (
        ...
        'aim_name_2019_11_28',
        'codedeploy_stack_name_2022_07_07',
        'secrets_manager_default_2023_03_09',
        'ecs_tracking_policy_name_2023_08_29'
    )
...
class InvalidFeatureFlag(zope.schema.ValidationError):
    __doc__ = 'Not a valid feature flag. Must be one of: '
    first = True
    for flag in valid_feature_flags:
        if first == False:
            __doc__ += ' | '
        __doc__ += flag
        first = False

def isValidFeatureFlag(value):
    if value not in valid_feature_flags:
        raise InvalidFeatureFlag
    return True

def isValidFeatureFlagList(value):
    for flag in value:
...
class InvalidLambdaFunctionUrlAuthType(zope.schema.ValidationError):
    __doc__ = 'Lamdba function_url.auth_type must be one of: NONE | AWS_IAM'

def isValidLambdaFunctionUrlAuthType(value):
    if value not in ('NONE', 'AWS_IAM'):
        raise InvalidLambdaFunctionUrlAuthType
    return True
...
def isValidEBSVolumeType(value):
    if value not in ('gp3', 'gp2', 'io1', 'sc1', 'st1', 'standard'):
...
# ECR
class InvalidECRLifeCyclePolicySelectionCount(zope.schema.ValidationError):
    __doc__ = 'NATGateway type must be one of: Managed | EC2'

def isValidECRLifeCyclePolicySelectionCount(value):
    if value not in ['imageCountMoreThan', 'sinceImagePushed']:
        raise InvalidECRLifeCyclePolicySelectionCount
    return True

# Flow Logs
class InvalidFlowLogDestinationType(zope.schema.ValidationError):
    __doc__ = 'EC2 flow log destination_type must be one of: s3 | cloud-watch-logs | kinesis-data-firehose'

# s3 | cloud-watch-logs | kinesis-data-firehose
def isValidFlowLogDestinationType(value):
    if value not in ('s3', 'cloud-watch-logs', 'kinesis-data-firehose'):
        raise InvalidFlowLogDestinationType
    return True

class InvalidFlowLogTrafficType(zope.schema.ValidationError):
    __doc__ = 'EC2 flow log traffic_type must be one of: ACCEPT | ALL | REJECT'

# s3 | cloud-watch-logs | kinesis-data-firehose
def isValidFlowLogTrafficType(value):
    if value not in ('ACCEPT', 'ALL', 'REJECT'):
        raise InvalidFlowLogTrafficType
    return True

class InvalidFlowLogFileFormat(zope.schema.ValidationError):
    __doc__ = 'EC2 flow log destination_options.file_format must be one of: plain-text | parquet'

# s3 | cloud-watch-logs | kinesis-data-firehose
def isValidFlowLogFileFormat(value):
    if value not in ('plain-text', 'parquet'):
        raise InvalidFlowLogFileFormat
    return True

class InvalidFlowLogResourceType(zope.schema.ValidationError):
    __doc__ = 'EC2 flow log resource_typoe must be one of: NetworkInterface | Subnet | TransitGateway | TransitGatewayAttachment | VPC'

# s3 | cloud-watch-logs | kinesis-data-firehose
def isValidFlowLogResourceType(value):
    if value not in ('NetworkInterface', 'Subnet', 'TransitGateway', 'TransitGatewayAttachment', 'VPC'):
        raise InvalidFlowLogResourceType
    return True

# Amazon Inspector v2
class InvalidInspectorResourceType(zope.schema.ValidationError):
    __doc__ = 'Amazon Inspector resource_types must match one or more of: EC2 | ECR | LAMBDA'

# s3 | cloud-watch-logs | kinesis-data-firehose
def isValidInspectorResourceType(value):
    valid_types = [ 'EC2', 'ECR', 'LAMBDA']
    for resource_type in value:
        if resource_type not in valid_types:
            raise InvalidInspectorResourceType
    return True

# CloudWatch Monitoring Accounts
class InvalidMonitoringAccountDataType(zope.schema.ValidationError):
    __doc__ = 'Amazon Inspector resource_types must match one or more of: EC2 | ECR | LAMBDA'

# s3 | cloud-watch-logs | kinesis-data-firehose
def isValidMonitoringAccountDataType(value):
    valid_types = [ 'logs', 'metrics', 'traces']
    for data_type in value:
        if data_type not in valid_types:
            raise InvalidMonitoringAccountDataType
    return True

# VPC VPN Client Endpoint
class InvalidVPNClientEndpointSessionTimeoutHours(zope.schema.ValidationError):
    __doc__ = 'VPN Client Endpoint session_timeout_hours must be one of: 8 | 10 | 12 | 24'

# s3 | cloud-watch-logs | kinesis-data-firehose
def isValidVPNClientEndpointSessionTimeoutHours(value):
    valid_types = [8, 10, 12, 24]
    if value not in valid_types:
        raise InvalidVPNClientEndpointSessionTimeoutHours
    return True


class InvalidVPNClientEndpointSelfServicePortal(zope.schema.ValidationError):
    __doc__ = 'VPN Client Endpoint self_service_portal must be one of: enabled | disabled'

# s3 | cloud-watch-logs | kinesis-data-firehose
def isValidVPNClientEndpointSelfServicePortal(value):
    valid_types = ['enabled', 'disabled']
    if value not in valid_types:
        raise InvalidVPNClientEndpointSelfServicePortal
    return True
...
class ISecurityGroupRule(IName):
    ...
    port = zope.schema.Int(
        title="Port",
        description="The port to allow access to/from.",
        default=-1,
        required=False
    )
...
# Paco Service Hook Config
class IPacoServiceHookConfig(INamed, IMapping):
    "A dict of service configuration"

class IPacoServiceHookItem(INamed, IDeployable):
    """
    Paco Service Hook and Configuration
    """
    service = PacoReference(
        title="Reference to a Paco Service",
        description="",
        required=True
    )
    config = zope.schema.Object(
        title="Configuration Dictionary",
        schema=IPacoServiceHookConfig,
        default=None,
        required=False
    )

class IPacoServiceHook(Interface):
    """
    Paco Service Hook and Configuration
    """
    paco_service_hook = zope.schema.Object(
        title="Paco Service Hook",
        schema=IPacoServiceHookItem,
        required=False
    )

class IPacoService(IResource, IPacoServiceHookItem):
    """
    Paco Service
    """
...
class ICloudWatchDashboard(IResource):
    ...
    dashboard_name = zope.schema.TextLine(
...
class ICloudWatchLogRetention(Interface):
    expire_events_after_days = zope.schema.TextLine(
        ...
        description="",
        # Default not support for LogRetention
...
class ICloudWatchLogGroup(INamed, ICloudWatchLogRetention, IEnablable):
    """
A CloudWatchLogGroup is responsible for retention, access control and metric filters
    ...
    log_group_name_prefix = zope.schema.TextLine(
        title="A name to prefix to the log group name.",
        description="",
        default="",
        required=False,
    )

class ILogGroup(IResource, ICloudWatchLogGroup):
    """
A CloudWatchLogGroup Resource
    """
...
A set of Log Group objects
    ...
    log_group_app_name =  zope.schema.TextLine(
        title="Override the LogGroup Application name",
        description="",
        required=False,
        default=None
    )
    create_log_group = zope.schema.Bool(
        title="Whether the LogGroup should be created or not",
        default=True,
        required=False
    )
...
.. code-block:: yaml
    ...
    event_bus = zope.schema.TextLine(
        title="The name or ARN of the event bus associated with the rule.",
        required=False
    )
...
class IS3BucketLifecycleRule(IDeployable):
    """
S3 Bucket Lifecycle Rule
    """
    expiration_in_days = zope.schema.TextLine(
...
class IS3PublicAccessBlockConfig(INamed):
    """S3 Bucket PublicAccessBlock Configuration"""

    block_public_acls = zope.schema.Bool(
        title="Specifies whether Amazon S3 should block public access control lists (ACLs) for this bucket and objects in this bucket",
        required=False,
        default=True
    )
    block_public_policy = zope.schema.Bool(
        title="Specifies whether Amazon S3 should block public bucket policies for this bucket",
        required=False,
        default=True
    )
    ignore_public_acls = zope.schema.Bool(
        title="Specifies whether Amazon S3 should ignore public ACLs for this bucket and objects in this bucket",
        required=False,
        default=True
    )
    restrict_public_buckets = zope.schema.Bool(
        title="Specifies whether Amazon S3 should restrict public bucket policies for this bucket",
        required=False,
        default=True
    )
...
class IS3Bucket(IResource, IDeployable, IPacoServiceHook):
...
"""
    ...
    lifecycle_rules = zope.schema.List(
        title="List of S3 Bucket Lifecycle Rules",
        description="",
        value_type=zope.schema.Object(IS3BucketLifecycleRule),
        required=False,
    )
    notifications = zope.schema.Object(
    ...
    # public_access_block_config = zope.schema.Object(
    #     title="Bucekt PublicAccessBlock configuration.",
    #     required=False,
    #     default=None,
    #     schema=IS3PublicAccessBlockConfig
    # )
...
class IEC2FlowLogDestinationOptions(INamed):
    file_format = zope.schema.TextLine(
        title="Flow Log File Format",
        default="plain-text",
        required=False,
        constraint=isValidFlowLogFileFormat # plain-text | parquet
    )

    hive_compatible_partitions = zope.schema.Bool(
        title="Flow Log Hive-compatible Partitions",
        description="",
        default=False,
        required=False
    )

    per_hour_partition = zope.schema.Bool(
        title="Flow Log Partition by the hour",
        description="",
        default=False,
        required=False
    )
...
class IEC2FlowLog(IResource):
    # NOTE: DesitationOptions are not yet supported. Need to upgrade Troposphere
    # or add support manually.
    destination_options = zope.schema.Object(
        title="Flow Log Destination Options",
        description="",
        schema=IEC2FlowLogDestinationOptions,
        required=False
    )

    log_destination_type = zope.schema.TextLine(
        title="Flow Log Destination Type",
        required=True,
        constraint=isValidFlowLogDestinationType # s3 | cloud-watch-logs | kinesis-data-firehose
    )

    log_destination = PacoReference(
        title='Flow Log Destination',
        required=True
    )

    # https://docs.aws.amazon.com/vpc/latest/userguide/flow-logs.html#flow-log-records
    log_format = zope.schema.TextLine(
        title="Flow Log File Format",
        required=False
    )

    max_aggregation_interval = zope.schema.Int(
        title="Flow Log Maximum Aggregation Interval",
        description="",
        default=600,
        min=60,
        max=600,
    ...
    resource_type = zope.schema.TextLine(
        ...
        description="Must be a valid AWS resource type",
        required=True,
        constraint=isValidFlowLogResourceType # NetworkInterface | Subnet | TransitGateway | TransitGatewayAttachment | VPC
    )

    traffic_type = zope.schema.TextLine(
        title="Flow Log File Format",
        required=False,
        default='ALL', # ACCEPT | ALL | REJECT
        constraint=isValidFlowLogTrafficType
    )
...
class IVPCEIP(INamed, IDeployable):
    """VPC EIP"""
    allocation_id = zope.schema.TextLine(
        title='External EIP Allocation id',
        required=False
    )
    ip_address = zope.schema.TextLine(
        title='External EIP Address',
        required=False
    )

class IVPCEIPs(INamed, IMapping):
    """Container for `IVPCEIP`_ objects."""
    taggedValue('contains', 'IVPCEIP')

class IVPCEIPRegions(INamed, IMapping):
    """Container for `IVPCEIP`_ objects."""
    taggedValue('contains', 'IVPCEIP')

class IVPCEIPAccounts(INamed, IMapping):
    """Container for `IVPCEIPRegions`_ objects."""
    taggedValue('contains', 'IVPCEIPRegions')


class IGuardDutyMalwareProtectionScanEC2(INamed, IMapping):
    """Container for `IVPCEIP`_ objects."""
    ebs_volumes = zope.schema.Bool(
        title="GuardDuty Malware Protection Scan EBS Volumes",
        description="",
        required=False,
        default=False
    )
class IGuardDutyDataSourceMalwareProtection(INamed, IMapping):
    """Container for `IVPCEIP`_ objects."""
    scan_ec2_instance_with_findings = zope.schema.Object(
        title="GuardDuty Malware Protection scan EC2 instance with findings",
        description="",
        schema=IGuardDutyMalwareProtectionScanEC2,
        required=False
    )

class IGuardDutyDataSourceS3Logs(INamed, IDeployable):
    """Guard Duty Detector S3 Logs Data Source"""

class IGuardDutyDetectorDataSources(INamed, IMapping):
    """Container for `IVPCEIP`_ objects."""
    malware_protection = zope.schema.Object(
        title="GuardDuty Detector Data Sources",
        description="",
        schema=IGuardDutyDataSourceMalwareProtection,
        required=False,
    )
    s3_logs = zope.schema.Object(
        title="GuardDuty Detector Data Sources",
        description="",
        schema=IGuardDutyDataSourceS3Logs,
        required=False,
    )

class IGuardDutyDetectorRegion(INamed, IDeployable, IMonitorable):
    """GuardDuty Detector Region"""
    data_sources = zope.schema.Object(
        title="GuardDuty Detector Data Sources",
        description="",
        schema=IGuardDutyDetectorDataSources,
        required=False,
    )
    external_resource = zope.schema.Bool(
...
class IGuardDutyDetectorAccounts(INamed, IMapping):
    """Container for `IGuardDutyDetectorRegion`_ objects."""
    taggedValue('contains', 'IGuardDutyDetectorRegion')

class IGuardDutyDetectorAccountRegionSets(INamed, IMapping):
    """Container for `GuardDutyDectorAccounts`_ objects."""
    taggedValue('contains', 'IGuardDutyDetectorAccounts')

class IGuardDutyResource(INamed):
    """
GuardDuty Resource Configuration
    """
    detectors = zope.schema.Object(
        title="GuardDuty Detectors",
        description="",
        schema=IGuardDutyDetectorAccountRegionSets,
        required=False
    )

class IInspectorRegion(INamed, IDeployable, IMonitorable):
    """Inspector Region"""
    resource_types = zope.schema.List(
        title="GuardDuty Detector Data Sources",
        description="",
        required=True,
        value_type=zope.schema.TextLine(),
        constraint=isValidInspectorResourceType,
    )

class IInspectorAccounts(INamed, IMapping):
    """Container for `IInspectorRegion`_ objects."""
    taggedValue('contains', 'IInspectorRegion')

class IInspectorAccountRegionSets(INamed, IMapping):
    """Container for `IInspectorAccounts`_ objects."""
    taggedValue('contains', 'IInspectorAccounts')

class IInspectorResource(INamed):
    """
Amazon Inspector Resource Configuration
    """
    inspectors = zope.schema.Object(
        title="GuardDuty Detectors",
        description="",
        schema=IInspectorAccountRegionSets,
        required=False
    )

class IMonitoringAccountRegion(INamed, IDeployable):
    """CloudWatch Monitoring Account Region"""
    data_types = zope.schema.List(
        title="Monitoring Account Data Types",
        description="",
        required=True,
        value_type=zope.schema.TextLine(),
        constraint=isValidMonitoringAccountDataType
    )
    source_accounts = zope.schema.List(
        title="Source AWS accounts",
        description="",
        value_type=PacoReference(
...
class IMonitoringAccountAccounts(INamed, IMapping):
    """Container for `IMonitoringAccountRegion`_ objects."""
    taggedValue('contains', 'IMonitoringAccountRegion')

class IMonitoringAccountAccountRegionSets(INamed, IMapping):
    """Container for `IMonitoringAccountAccounts`_ objects."""
    taggedValue('contains', 'IMonitoringAccountAccounts')


class ICloudWatchLogsRegion(INamed, IDeployable):
    """CloudWatch Monitoring Account Region"""
    log_groups = zope.schema.Object(
        title="A CloudWatchLogGroups container",
        schema=ICloudWatchLogGroups,
...
class ICloudWatchLogsAccounts(INamed, IMapping):
    """Container for `IMonitoringAccountRegion`_ objects."""
    taggedValue('contains', 'IMonitoringAccountRegion')

class ICloudWatchLogsAccountRegionSets(INamed, IMapping):
    """Container for `ICloudWatchLogsAccounts`_ objects."""
    taggedValue('contains', 'ICloudWatchLogsAccounts')
class ICloudWatchResource(INamed):
    """
Amazon CloudWatch Global Resource Configuration
    """
    monitoring_accounts = zope.schema.Object(
        title="Monitoring Accounts",
        description="",
        schema=IMonitoringAccountAccountRegionSets,
        required=False
    )

    logs = zope.schema.Object(
        title="CloudWatch Logs",
        description="",
        schema=ICloudWatchLogsAccountRegionSets,
        required=False
    )

class IEventBridgeBusesRule(IEventsRule):
    """EventBridge Buses Rule."""

class IEventBridgeBusesRules(INamed, IMapping):
    """Container for `IEventBridgeBusesRule`_ objects."""
    taggedValue('contains', 'IEventBridgeBusesRule')

class IEventBridgeBuses(INamed, IDeployable):
    """EventBridge Bus Services"""
    # event_buses = zope.schema.Object(
    #     title="EventBridge Buses",
    #     description="",
    #     schema=IEventBridgeBusesEventBuses,
    #     required=False
    # )
    rules = zope.schema.Object(
        title="EventBridge Bus Rules",
        description="",
        schema=IEventBridgeBusesRules,
        required=False
    )

class IEventBridgeRegion(INamed, IDeployable):
    """EventBridge Region"""
    buses = zope.schema.Object(
        title="EventBridge Bus Services",
        description="",
        schema=IEventBridgeBuses,
        required=False
    )

class IEventBridgeAccounts(INamed, IMapping):
    """Container for `IEventBridgeRegion`_ objects."""
    taggedValue('contains', 'IEventBridgeRegion')

class IEventBridgeAccountRegionSets(INamed, IMapping):
    """Container for `IEventBridgeAccounts`_ objects."""
    taggedValue('contains', 'IEventBridgeAccounts')

class IEventBridgeResource(INamed):
    """
Amazon EventBridge Global Resource Configuration
    """
    services = zope.schema.Object(
        title="Monitoring Accounts",
        description="",
        schema=IEventBridgeAccountRegionSets,
        required=False
    )

class IAppConfigApplicationEnvironment(INamed):
    """AppConfig Application Environment"""

class IAppConfigApplicationProfile(INamed):
    """AppConfig Application Profile"""

class IAppConfigApplicationEnvironments(INamed, IMapping):
    """Container for `IAppConfigApplicationEnvironment`_ objects."""
    taggedValue('contains', 'IAppConfigApplicationEnvironment')

class IAppConfigApplicationProfiles(INamed, IMapping):
    """Container for `IAppConfigApplicationProfile`_ objects."""
    taggedValue('contains', 'IAppConfigApplicationProfile')

class IAppConfigApplication(INamed):
    profiles = zope.schema.Object(
        title="AppConfig ApplicationsProfiles",
        description="",
        schema=IAppConfigApplicationProfiles,
        required=False
    )
    environments = zope.schema.Object(
        title="AppConfig Application Environments",
        description="",
        schema=IAppConfigApplicationEnvironments,
        required=False
    )

class IAppConfigApplications(INamed, IMapping):
    """Container for `IAppConfigApplication`_ objects."""
    taggedValue('contains', 'IAppConfigApplication')

class IAppConfigRegion(INamed, IDeployable, IMonitorable):
    """AppConfig Region"""

    applications = zope.schema.Object(
        title="AppConfig Applications",
        description="",
        schema=IAppConfigApplications,
        required=False
    )

class IAppConfigAccounts(INamed, IMapping):
    """Container for `IAppConfigRegion`_ objects."""
    taggedValue('contains', 'IAppConfigRegion')

class IAppConfigAccountRegionSets(INamed, IMapping):
    """Container for `IAppConfigAccounts`_ objects."""
    taggedValue('contains', 'IAppConfigAccounts')

class IAppConfigResource(INamed, IMapping):
    """
Container for IAppConfigAccounts Resource Configuration
    """
    taggedValue('contains', 'IAppConfigAccounts')
...
EC2 Resource Configuration
    """
    eips = zope.schema.Object(
        title="VPC EIPs",
        description="",
        schema=IVPCEIPAccounts,
        required=False,
    )
...
class IProject(INamed, IMapping):
    ...
    feature_flags = zope.schema.List(
        title='List of Feature Flags',
        value_type=zope.schema.TextLine(),
        constraint=isValidFeatureFlagList,
        required=False,
    )
...
Route53 Hosted Zone
    """
    @invariant
    def private_zone_check(obj):
        """
        If private hosted zone is enabled, vpc_associations must exist
        """
        if obj.private_hosted_zone == True and obj.vpc_associations == None:
            if obj.external_resource == None or obj.external_resource.enabled == False:
                raise Invalid('Private Route53 Hosted Zone must have a VPC association configured.')
...
class INATGateway(INamed, IDeployable):
    ...
    ec2_instance_ami = PacoReference(
        title="Instance AMI",
        description="",
        str_ok=True,
        required=False,
    ...
    eips = zope.schema.List(
        title="List of EIP names",
        required=False,
        value_type=zope.schema.TextLine(
            title="VPC EIP Name"
        ),
...
class IVPNClientEndpoint(INamed, IDeployable):
    """VPN Client Endpoint"""
    availability_zone = zope.schema.TextLine(
        # Can be: all | 1 | 2 | 3 | 4 | ...
        title='Availability Zones to launch instances in.',
    ...
    segment = zope.schema.TextLine(
        title="VPC Segment to assocaite VPN with",
        required=True,
        default=''
    )
    self_service_portal = zope.schema.TextLine(
        title="Enable the AWS VPN Self Service Portal for downloading the client and configuration: enabled | disabled",
        default='disabled',
        required=False,
        constraint = isValidVPNClientEndpointSelfServicePortal
    )
    session_timeout_hours = zope.schema.Int(
        title="The maximum VPN session duration time in hours: 8 | 10 | 12 | 24",
        default=24,
        required=False,
        min=8,
        max=24,
        constraint = isValidVPNClientEndpointSessionTimeoutHours
    )
    split_tunnel = zope.schema.Bool(
        title="Indicates whether split-tunnel is enabled on the AWS Client VPN endpoint.",
        default=False,
        required=False
    )
    transport_protocol = zope.schema.Choice(
        title="The transport protocol to be used by the VPN session: tcp | udp",
        vocabulary=vocabulary.network_protocols,
        default='udp'
    )
    vpn_port = zope.schema.Choice(
        title="The port number to assign to the Client VPN endpoint for TCP and UDP traffic: tcp | udp",
        vocabulary=vocabulary.vpn_client_endpoint_ports,
        default=443
    )
    client_cidr_block = zope.schema.TextLine(
        title="The IPv4 address range, in CIDR notation, from which to assign client IP addresses",
        default="",
        required=False,
        constraint = isValidCidrIpv4orBlank
    )
    security_groups = zope.schema.List(
        title="List of VPC Security Group References",
        value_type=PacoReference(
            schema_constraint='ISecurityGroup'
        ),
        required=True
    )
    server_certificate_arn = zope.schema.TextLine(
        title='Server Certificate ARN',
        required=True,
        default=''
    )
    #   segment: application
    ...
    #   session_timeout_hours: 24 # 8 | 10 | 12 | 24
    #   #split_tunnel: false
    #   transport_protocol: udp # tcp | udp
    #   vpn_port: 443 # 443 | 1194
    #   authentication:
    #     type: certificate-authentication # certificate-authentication | directory-service-authentication | federated-authentication
    #     #mutual_authentication:
    #     #  client_root_certificate_chain_arn:
    #   client_cidr_block: 10.99.0.0/22
    #   client_connection_logging:
    #     enabled: true
    #   security_groups:
    #     - vpn.testing
...
class ISegmentNetworkACLInboundRule(INamed):
    """Segment Network ACL Inbound"""
    source_cidr = zope.schema.TextLine(
        title="CIDR",
        required=True,
        constraint = isValidCidrIpv4orBlank
    )
...
class ISegmentNetworkACL(INamed, IEnablable):
    """Segment Network ACL"""
    inbound_rules = zope.schema.List(
        title="Inbound ACLs",
        required=False,
        value_type=zope.schema.Object(
            title="Inbound ACL",
            schema=ISegmentNetworkACLInboundRule,
        )
    )
...
Segment
    ...
    nacl = zope.schema.Object(
        title="Segment Network ACLs",
        schema=ISegmentNetworkACL,
        required=False,
    )
...
VPC Peering
    ...
    associate_private_hosted_zone = zope.schema.TextLine(
        title="Type of Rule",
        required=False,
        constraint = isValidVpcPeerAssocatePrivateHostedZone
    )
...
# class IVPCDHCPOptions(IDeployable, IMapping):
#     domain_name = zope.schema.TextLine(
#         title="DHCP Options Domain Name",
#         description="",
#         default="",
#         required=False,
#     )
...
class IVPC(INamed, IDeployable):
    ...
    # dhcp_options = zope.schema.Object(
    #     title="VPC DHCP Options",
    #     description="",
    #     schema=IVPCDHCPOptions,
    #     required=False,
    # )
    ...
    vpn_client_endpoint = zope.schema.Object(
        title="VPN Client Endpoint",
        description="",
        schema=IVPNClientEndpoint,
        required=False,
    )
...
class ITargetGroup(IPortProtocol, IResource, IMonitorable, IDeployable):
    """Target Group"""
    ...
    def health_check_check(obj):
        ...
        """
        if obj.health_check_interval != None and obj.health_check_timeout != None:
...
class IListenerDefaultActionFixedResponse(INamed):
    content_type = zope.schema.TextLine(
        title="Content Type: text/plain | text/css | text/html | application/javascript | application/json",
        default="text/html",
        required=False,
        constraint=isValidListenerDefaultActionContentType,
    )
    message_body = zope.schema.TextLine(
        title="Message Body",
        default="<html><body>Path not found.</body></html>",
        required=False
    )
...
class IListenerDefaultAction(INamed):
    ...
    fixed_response = zope.schema.Object(
        title="Fixed Response",
        schema=IListenerDefaultActionFixedResponse,
        required=False,
    )
...
class IListener(IParent, IPortProtocol):
    ...
    def redirect_or_target_group(obj):
        ...
        if obj.redirect == None and obj.target_group == '' and obj.default_action == None:
            raise Invalid("Either a redirect, target_group, or default_action (recommended) must be set for a listener.")
        if obj.redirect != None and obj.target_group != '':
            raise Invalid("Can not set both a redirect and a target_group for a listener.")

    default_action = zope.schema.Object(
        title="Default Action",
        schema=IListenerDefaultAction,
        required=False,
    )
    redirect = zope.schema.Object(
        title="Redirect",
...
class ILoadBalancer(IResource, IMonitorable):
    ...
    cost_disabled = zope.schema.Bool(
        title="Boolean indicating whether LB is disabled for cost savings.",
        required=False,
        default=False
    )
    ...
    ignore_listener_rule_target_group_changes = zope.schema.Bool(
        title="Boolean that will ignore changes to Listener Rule Target Group Parameters.",
        required=False,
        default=False
    )
    ...
    cross_zone_balancing = zope.schema.Bool(
        title="Enabled Cross Zone Load Balancing",
        default=True,
        required=False
    )
...
class IPrincipal(INamed):
    ...
    federated = zope.schema.TextLine(
        title="Federated Principal",
        default="",
        required=False,
    )
...
class IAssumeRolePolicies(IParent):
    statement = zope.schema.List(
        title="Statements",
        value_type=zope.schema.Object(
            title="Statement",
...
class IIAMIdentityProviderRole(INamed, IDeployable, IMapping):
    policy_actions = zope.schema.List(
        title="Lambda Policy Actions",
        value_type=zope.schema.TextLine(
            title="Lambda IAM policy Actions"
        ),
        required=False
    )
    provider = PacoReference(
        title="The Identity Provider to associate with.",
        required=True,
    )
    repository_filters = zope.schema.List(
        title="List of Repository Filters",
        value_type=zope.schema.TextLine(
...
class IIAMIdentityProviderRoles(INamed, IMapping):
    """
Container for IIAMIdentityProviderRole objects.
    """
    taggedValue('contains', 'IIAMIdentityProviderRole')
...
class IBaseRole(INamed):
    ...
    assume_role_policies = zope.schema.Object(
        title="Multiple assume role policies",
        schema=IAssumeRolePolicies,
        required=False
    )
    identity_provider = zope.schema.Object(
        title="Identity Provider Configuration",
        schema=IIAMIdentityProviderRole,
        default=None,
        required=False
    )
...
EFS Mount Folder and Target Configuration
    ...
    read_only_mode = zope.schema.Bool(
        title="Actions Enabled",
        required=False,
        default=False
    )
...
class IFilePermissions(Interface):
    """File ownership, group, and mode"""
...
class ICloudFormationInitFile(INamed, IFilePermissions):
    @invariant
...
.. code-block:: yaml
    ...
    volume_type = zope.schema.TextLine(
        title="Volume Type",
        description="Must be one of: gp3 | gp2 | io1 | sc1 | st1 | standard",
        default='gp3',
        constraint = isValidEBSVolumeType,
        required=False
    )
    tags = zope.schema.List(
        title='List of tags to attach to EBS volume.',
        value_type=zope.schema.Object(INameValuePair),
        required=False
    )

class IAmazonLinuxLivePatching(INamed, IEnablable):
    """
Amazon Linux Live Patching Configuration
    """
    release_version = zope.schema.TextLine(
        title='Amazon Linux Release Version',
        required=True,
        default=''
    )
    kernel_version = zope.schema.TextLine(
        title='Amazon Linux Kernel Version',
        required=False,
        default=''
    )
...
EC2 Launch Options
    ...
    amazon_linux_live_patching = zope.schema.Object(
        title='Amazon Linux Live Patching',
        schema=IAmazonLinuxLivePatching,
        required=False
    )
...
class IECSASGConfiguration(INamed, IDeployable):
...
class IScriptManagerCustomScript(INamed, IFilePermissions):
    """
Script Manager ECS Tool
    """
    content = zope.schema.Text(
    ...
    location = zope.schema.TextLine(
        title="The location to install the script",
        description="",
        default="",
        required=False,
    )
...
class IScriptManagerCustomScripts(INamed, IMapping):
    """
Script Manager ECS Group
    """
    taggedValue('contains', 'IScriptManagerCustomScript')
...
EC2 Script Manager
    ...
    custom = zope.schema.Object(
        title="Custom Scripts",
        required=False,
        schema=IScriptManagerCustomScripts
    )
...
class IECSLogging(INamed, ICloudWatchLogRetention):
    ...
    awslogs_one_cluster_group =  zope.schema.Bool(
        title="Consolidate task logs into one Log Group",
        description="",
        required=False,
        default=False,
    )
    awslogs_resource_app_name =  zope.schema.TextLine(
        title="Consolidate task logs into one Log Group",
        description="",
        required=False,
        default=None
    )
    awslogs_create_log_group =  zope.schema.Bool(
        title="Create the LogGroup for the service",
        description="",
        required=True,
        default=True,
    )
...
class IECSContainerDefinition(INamed):
    ...
    ignore_image_changes = zope.schema.Bool(
        title="Do not update the instance_ami after creation.",
        description="",
        default=False,
        required=False,
...
class IECSTaskDefinition(INamed):
    ...
    task_role = zope.schema.Object(
        title="The Task Role used by the applications in the container to access AWS APIs and services.",
        required=False,
        schema=IRole,
    )
...
class IECSService(INamed, IMonitorable):
    ...
    ignore_capacity_changes = zope.schema.Bool(
        title="Do not update the desired_capacity parameters after creation.",
        description="",
        default=False,
        required=False,
    )
...
class IECRRepositoryLifecyclePolicy(INamed):
    rule_priority = zope.schema.TextLine(
    ...
    tag_status = zope.schema.TextLine(
        title="Policy Selection Tag Statu",
        description="",
        required=False,
        default="any"
    )
    tag_prefix_list = zope.schema.List(
        title="Policy Selection Tag Prefix List",
        value_type=zope.schema.TextLine(),
        required=False,
        description='List of tag prefixes to match.'
    )
    count_type = zope.schema.TextLine(
        title="Policy Selection Count Type",
        description="Count Type values: imageCountMoreThan | sinceImagePushed",
        required=False,
        constraint=isValidECRLifeCyclePolicySelectionCount
    )
    count_number = zope.schema.TextLine(
        title="Policy Selection Count Number", # Count number is in Days
        description="",
        required=False
    )

# ECR: Repository Lifecycle Policy
class IECRRepositoryLifecyclePolicies(INamed, IMapping):
    """
Container of Repository LifeCycle Policies
"""
    taggedValue('contains', 'IECRRepositoryLifecyclePolicy')


class IECRRepositoryImageScanning(INamed):
    scan_on_push = zope.schema.Bool(
...
class IECRReplicationRule(INamed, IEnablable):
...
class IECRReplicationRules(INamed, IMapping):
    """ECR Replication Rules"""
    taggedValue('contains', 'IECRReplicationRule')

class IECRReplicationConfiguration(IResource):

    rules = zope.schema.Object(
        title="Replication Rules",
        description="",
        schema=IECRReplicationRules,
        required=False,
    )
...
"""
    ...
    external_repository_arn = zope.schema.TextLine(
        title="Repository ARN",
        required=False,
        default=None
    )
    image_scanning = zope.schema.Object(
        title="ECR Image Scanning",
        schema=IECRRepositoryImageScanning,
        required=False
    )
    ...
    lifecycle_policies = zope.schema.Object(
        title="LifeCycle Policies",
        required=False,
        schema=IECRRepositoryLifecyclePolicies,
    )
    ...
    prefix_environment_name = zope.schema.Bool(
        title="Prefix the environment to the repository name",
        default=False,
        required=False,
    )
    identity_provider_roles = zope.schema.Object(
        title='IAM Identity Provider Roles',
        schema=IIAMIdentityProviderRoles,
        required=False
    )
...
class ILambdaFunctionCode(IParent):
    ...
    layer_zipfile = LocalPath(
        title="The layer code as a local file or directory.",
        description="Maximum of 4096 characters.",
        required=False,
    )
    s3_bucket = PacoReference(
...
class ILambdaFunctionUrl(INamed, IEnablable):
    auth_type = zope.schema.TextLine(
        title="The type of authentication that your function URL use. AWS_IAM | NONE",
        required=False,
        default="NONE",
        constraint = isValidLambdaFunctionUrlAuthType
    )
    qualifier = zope.schema.TextLine(
        title="The function url alias name",
        required=False,
    )
...
"""
    account = PacoReference(
        title="Account where the function will be created.",
        required=False,
        schema_constraint='IAccount',
        default=None
    )
    ...
    # function_url = zope.schema.Object(
    #     title="Lamdba Function URL.",
    #     required=False,
    #     schema=ILambdaFunctionUrl
    # )
    ...
    s3_buckets = zope.schema.List(
        title="List of S3 Bucket Paco References that can invoke this function.",
        value_type=PacoReference(
            str_ok=True,
            schema_constraint='IS3Bucket'
        ),
...
# class IWAFWebACLRuleActionCount(INamed):
#     """
# WAF Web ACL Rule Action Count Configuration
#     """
#     custom_request_handling = zope.schema.Object(
#         title="WAF Web ACL Custom Response.",
#         required=True,
#         default=None,
#         schema=IWAFWebACLCustomResponse
...
WAF Web ACL Rule Action Configuration
    ...
    count = zope.schema.Bool(
        title="WAF Web ACL Rule Count Action.",
        default=False,
        required=False
    )

class IWAFWebACLRuleOverrideAction(INamed):
    """
WAF Web ACL Rule Override Action Configuration
    """
    # allow = zope.schema.Object(
    #     title="WAF Web ACL Rule Allow Action.",
    #     required=False,
    ...
    none = zope.schema.Bool(
        title="WAF Web ACL Rule None Action.",
        default=False,
        required=False
    )

class IWAFWebACLRuleActionOverride(INamed):
    """
WAF Web ACL Managed Rule Action Overrides
    """
    action_to_use = zope.schema.Object(
        title="Action to use",
...
WAF Web ACL Rule ManagedRuleGroup Configuration
    ...
    excluded_rules = zope.schema.List(
        title="Managed Rule Excluded Rules",
        value_type=zope.schema.TextLine(),
        required=False
    )
    rule_action_overrides = zope.schema.List(
        title="Managed Rule Action Overrides",
        value_type=zope.schema.Object(IWAFWebACLRuleActionOverride),
        required=False
    )
...
class IWAFWebACLRule(INamed, IDeployable):
    ...
    override_action = zope.schema.Object(
        title="WAF Web ACL Rule Action.",
        required=False,
        schema=IWAFWebACLRuleOverrideAction
    )
...
class IWAFWebACLLogging(INamed, IEnablable):
    """Web Application Firewall Web ACL Logging Configuration"""
    log_retention_days = zope.schema.Int(
...
class IWAFWebACL(IResource):
    ...
    logging = zope.schema.Object(
        title="WAF Web ACL Logging Configuration.",
        required=False,
        default=None,
        schema=IWAFWebACLLogging
    )
...
class ICloudFrontLambdaFunctionAssocation(INamed):
    ...
    lambda_version = zope.schema.TextLine(
        title="Lambda@Edge Function Version",
        required=False,
        default='1'
    )

class ICloudFrontGenericResponseHeaderPolicy(INamed):
    """CloudFront Response Header Policy Content Security Policy"""
    response_header_value = zope.schema.TextLine(
        title="Header value for the Content-Security-Policy response header",
        required=False
    )
    override = zope.schema.Bool(
        title="Override origin headers if they exist",
        required=False,
        default=False
    )

class ICloudFrontStrictTransportHeaderPolicy(INamed):
    """CloudFront Referrer Response Header Policy"""
    access_control_max_age = zope.schema.Int(
        title="Acces Control Max Age",
        default=31536000
    )
    include_subdomains = zope.schema.Bool(
        title="Boolean indicating whether includeSubDomains directive is added to the Strict-Transport-Security HTTP response header",
        required=False,
        default=False
    )
    override = zope.schema.Bool(
        title="A Boolean indicating whether the Strict-Transport-Security overrides origin response headers.",
        required=False,
        default=False
    )
    preload = zope.schema.Bool(
        title="A Boolean that determines whether CloudFront includes the preload directive in the Strict-Transport-Security HTTP response header.",
        required=False,
        default=False
    )

class ICloudFrontXSSProtectionHeaderPolicy(INamed):
    """CloudFront Referrer Response Header Policy"""
    mode_block = zope.schema.Bool(
        title="A Boolean that determines whether CloudFront includes the mode=block directive in the X-XSS-Protection header",
        required=False,
        default=False
    )
    override = zope.schema.Bool(
        title="A Boolean that determines whether CloudFront overrides the X-XSS-Protection HTTP response header received from the origin with the one specified in this response headers policy",
        required=False,
        default=False
    )
    protection = zope.schema.Bool(
        title=" Boolean that determines the value of the X-XSS-Protection HTTP response header. When this setting is true, the value of the X-XSS-Protection header is 1. When this setting is false, the value of the X-XSS-Protection header is 0.",
        required=False,
        default=False
    )
    report_uri = zope.schema.TextLine(
        title="A reporting URI, which CloudFront uses as the value of the report directive in the X-XSS-Protection header.",
        required=False,
        default=None
    )

class ICloudFrontSecurityHeadersConfig(INamed, IDeployable):
    """CloudFront Response Header Policy"""
    content_security_policy = zope.schema.Object(
        title="Content Security Policy header",
        schema=ICloudFrontGenericResponseHeaderPolicy,
        required=False,
    )
    referrer_policy = zope.schema.Object(
        title="Referrer Policy header",
        schema=ICloudFrontGenericResponseHeaderPolicy,
        required=False,
    )
    frame_options_policy = zope.schema.Object(
        title="Referrer Policy header",
        schema=ICloudFrontGenericResponseHeaderPolicy,
        required=False,
    )
    content_type_options = zope.schema.Object(
        title="Referrer Policy header",
        schema=ICloudFrontGenericResponseHeaderPolicy,
        required=False,
    )
    strict_transport_security_policy = zope.schema.Object(
        title="Strict Transport Security Policy header",
        schema=ICloudFrontStrictTransportHeaderPolicy,
        required=False,
    )
    xss_protection_policy = zope.schema.Object(
        title="XSS Protection Security Policy header",
        schema=ICloudFrontXSSProtectionHeaderPolicy,
        required=False,
    )


class ICloudFrontResponseHeaderPolicy(INamed, IDeployable):
    """CloudFront Response Header Policy"""
    security_headers_config = zope.schema.Object(
        title="Content Security Policy header",
        schema=ICloudFrontSecurityHeadersConfig,
        required=False,
    )


class ICloudFrontResponseHeaderPolicies(INamed, IMapping):
    """Container for `ICloudFrontResponseHeaderPolicy`_ objects."""
    taggedValue('contains', 'ICloudFrontResponseHeaderPolicy')
...
class ICloudFrontDefaultCacheBehavior(INamed):
    ...
    cache_policy_id = zope.schema.TextLine(
        title="CloudFront Cache Policy ID",
        required=False
    )
    response_headers_policy_id = PacoReference(
        title="CloudFront Response Headers Policy ID",
        str_ok=True,
        required=False
    )
...
class ICloudFrontCustomOriginConfig(INamed):
    ...
    protocol_policy = zope.schema.TextLine(
        title="Protocol Policy", # http-only | https-only | match-viewer
...
class ICloudFrontLogging(INamed, IDeployable):
    """
CloudFront Origin Configuration
    """
    s3_bucket = PacoReference(
        title="Log storage S3 Bucket",
        required=False,
        schema_constraint='IS3Bucket'
    )
    include_cookies = zope.schema.Bool(
        title="Include cookies in access logs",
        required=False,
        default=False
    )
    prefix = zope.schema.TextLine(
        title="Prefix to the access log location",
        required=False,
    )
...
CloudFront CDN Configuration
    ...
    response_header_policies = zope.schema.Object(
        title="Response Header Policies",
        description="",
        schema=ICloudFrontResponseHeaderPolicies,
        required=False,
    )
...
RDS common fields shared by both DBInstance and DBCluster
    ...
    def password_or_snapshot_or_secret(obj):
        ...
        if obj.secrets_password:
            count += 1
        if obj.global_cluster_identifier:
            count += 1
        if count > 1:
    ...
    def valid_engine_version(obj):
        ...
        if obj.engine_version not in gen_vocabulary.rds_engine_versions[obj.engine]:
            raise Invalid(f"Engine Version is not support by AWS RDS: {obj.engine_version}")
        return True
...
class IRDSAuroraGlobalCluster(INamed, IEnablable):
    """
...
RDS Aurora DB Cluster
    ...
    global_cluster = zope.schema.Object(
        title="Global Cluster",
        required=False,
        schema=IRDSAuroraGlobalCluster,
    )
    global_cluster_identifier = PacoReference(
        title="Global Cluster Identifier",
        required=False,
        schema_constraint='IRDSAurora',
        str_ok=True
    )
...
IAM User Programmatic Access Configuration
    """
    externally_managed = zope.schema.Bool(
        title='Disables paco automation from managing keys.',
        default=False,
        required=False,
    )
...
class IIAMUserPermissionAccessKeys(IIAMUserPermission):
    """
Session Manager Session Permissions
    """
...
.. code-block:: yaml
    ...
    default_password = zope.schema.TextLine(
        title='A default password to override the Paco generated one',
        required=False
    )
    description=zope.schema.TextLine(
    ...
    disable_account_delegates = zope.schema.Bool(
        title='Disable Account Delegates',
        default=False
    )
    console_access_enabled = zope.schema.Bool(
        title='Console Access Boolean'
...
class IIAMIdentityProvider(INamed, IMapping):
    """
IdentityProvider object
    """
    provider_type = zope.schema.TextLine(
        title='Identity Provider Type: openid | saml',
        required=True
    )
    provider = zope.schema.TextLine(
        title='Identity Provider Type: openid | saml',
        required=True
    )
    accounts = zope.schema.List(
        title="Accounts to add this identity provider.",
        description="",
        value_type=PacoReference(
            title="Account Reference",
            schema_constraint='IAccount'
    ...
    audiences = zope.schema.List(
        title="List of audiences for the provider.",
        description="",
        value_type=zope.schema.TextLine(
            title="Audience"
        ),
...
class IIAMIdentityProviders(INamed, IMapping):
    """
Container for IdentityProvider objects.
    """
    taggedValue('contains', 'IIAMIdentityProvider')


class IIAMPolicy(IResource, IManagedPolicy):
    """
    IAM Policies
    """
    accounts = zope.schema.List(
        ...
        required=True,
        value_type=PacoReference(
            title="The account where the Policy will be provisioned.",
            required=False,
            schema_constraint=IAccount
        )
    )

class IIAMPolicies(INamed, IMapping):
    """
Container for IIAMPolicy objects.
    """
    taggedValue('contains', 'IIAMPolicy')
...
class IIAMRoles(INamed, IMapping):
    """
Container for IRole objects.
    """
    taggedValue('contains', 'IIAMRole')

class IIAMGroup(INamed, IMapping):
    """IAM Group"""

class IIAMGroups(INamed, IMapping):
    taggedValue('contains', 'IIAMGroup')
...
IAM Resource contains IAM Users who can login and have different levels of access to the AWS Console and API.
    ...
    identity_providers = zope.schema.Object(
        title='IAM Identity Providers',
        schema=IIAMIdentityProviders,
        required=False,
    )
    ...
    policies = zope.schema.Object(
        title='IAM Managed Policies',
        schema=IIAMPolicies,
        required=False,
    )

    roles = zope.schema.Object(
        title='IAM Roles',
        schema=IIAMRoles,
        required=False,
    )
...
class IEnvironmentVariable(IParent):
    """
    ...
    value = PacoReference(
        title='String Value or a Paco Reference',
        required=True,
        str_ok=True,
        schema_constraint='Interface'
    )
...
class IDeploymentPipelineCodeStarConnectionSourceAction(Interface):
    """Common configuration for CodeStar Connection Source Actions: BitBucket & GitHub"""
    codestar_connection_arn = zope.schema.TextLine(
        title='The CodeStar Connection ARN to BitBucket.',
        required=False
    )
    detect_changes = zope.schema.Bool(
        title='Trigger on git commits',
        required=False,
        default=True,
    )
    owner = zope.schema.TextLine(
        title='The name of the organization who owns the repository.',
        required=False
    )
    repository = zope.schema.TextLine(
        title='The name of the repository where source changes are to be detected.',
        required=False
    )

class IDeploymentPipelineSourceGitHub(IDeploymentPipelineStageAction, IDeploymentPipelineCodeStarConnectionSourceAction):
...
class IDeploymentPipelineSourceBitBucket(IDeploymentPipelineStageAction, IDeploymentPipelineCodeStarConnectionSourceAction):
    """BitBuket DeploymentPipeline Source Stage

To configure a BitBucket source, first provision the deployment pipeline to create a CodeStar
...
CodeBuild DeploymentPipeline Build Stage
    ...
    environment_variables = zope.schema.List(
        title="Environment Variables",
        value_type=zope.schema.Object(IEnvironmentVariable),
        required=False,
    )
...
class IDeploymentPipelineDeployCodeDeploy(IDeploymentPipelineStageAction, IPacoServiceHook):
    """
CodeDeploy DeploymentPipeline Deploy Stage
    ...
    deploy_style_type = zope.schema.TextLine(
    ...
    input_artifact_action = zope.schema.TextLine(
        title="Name of the Action to receive input from.",
        description="",
        default="",
        required=False,
    )
