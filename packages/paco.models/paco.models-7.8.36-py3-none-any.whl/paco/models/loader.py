...
from paco.models.networks import NetworkEnvironment, Environment, EnvironmentDefault, \
    EnvironmentRegion, Segment, SegmentNetworkACL, SegmentNetworkACLInbound, Network, VPC, VPCPeering, VPCPeeringRoute, VPCEIPAccounts, VPCEIPRegions, VPCEIP, VPCEIPs, NATGateway, VPNGateway, \
    VPNClientEndpoint, PrivateHostedZone, SecurityGroup, IngressRule, EgressRule, NATGateways, VPNGateways, Segments, \
...
from paco.models.applications import Application, PinpointApplication, ResourceGroups, ResourceGroup, \
    ...
    S3StaticWebsiteHosting, S3StaticWebsiteHostingRedirectRequests, S3BucketLifecycleRule, S3BucketPolicy, \
    ACM, ListenerRule, ListenerRules, ListenerDefaultAction, ListenerDefaultActionFixedResponse, \
    Lambda, LambdaEnvironment, LambdaVpcConfig, \
    ...
    CloudFrontOrigin, CloudFrontOrigins, CloudFrontCustomOriginConfig, CloudFrontLogging, \
    ...
    CloudFrontLambdaFunctionAssocation, CloudFrontResponseHeaderPolicies, CloudFrontResponseHeaderPolicy, CloudFrontGenericResponseHeaderPolicy, \
    CloudFrontStrictTransportHeaderPolicy, CloudFrontXSSProtectionHeaderPolicy, CloudFrontSecurityHeadersConfig, \
    ...
    EnvironmentVariable, RDSAuroraGlobalCluster, \
    ...
    GenerateSecretString, EC2LaunchOptions, BlockDeviceMapping, BlockDevice, EC2LaunchOptions, AmazonLinuxLivePatching, \
    ...
    ECSVolumesFrom, ECSVolume, ECSLogging, ECRRepository, ECRRepositoryLifecyclePolicies, ECRRepositoryLifecyclePolicy, \
    ECRRepositoryImageScanning, ECRReplicationConfiguration, ECRReplicationConfiguration, ECRReplicationRules, ECRReplicationRule, \
    ECSTaskDefinitionSecret, ECSContainerDependency, IAMIdentityProviderRoles, IAMIdentityProviderRole, \
    ...
    ScriptManagerEcs, ASGPatchManager, S3ReplicationConfigurations, S3ReplicationConfiguration, ScriptManagerCustomScripts, \
    ScriptManagerCustomScript, PacoServiceHook, PacoServiceHookItem, PacoServiceHookConfig, \
    ...
    WAFWebACLRuleAction, WAFWebACLRuleActionBlock, WAFWebACLCustomResponse, WAFWebACLLogging, WAFWebACLRuleActionOverride, \
    WAFWebACLRuleOverrideAction, VPCConfiguration, PacoService
...
from paco.models.resources import S3Resource, S3Buckets, \
    EC2Resource, EC2KeyPairs, EC2KeyPair, EC2Users, EC2User, EC2Group, EC2Groups, EC2FlowLog, EC2FlowLogDestinationOptions, \
    ...
    IAMResource, IAMUser, IAMUsers, IAMRole, IAMRoles, IAMIdentityProviders, IAMIdentityProvider, IAMUserPermissions, \
    IAMUserProgrammaticAccess, IAMUserPermissionCodeCommitRepository, IAMUserPermissionCodeCommit, IAMUserPermissionAdministrator, \
    ...
    IAMUserPermissionAccessKeys, IAMPolicies, IAMPolicy, \
    SSMResource, SSMDocuments, SSMDocument, ConfigResource, Config, \
    SNS, Topics, \
    GuardDutyResource, GuardDutyDetectorAccountRegionSets, GuardDutyDetectorAccounts, GuardDutyDetectorRegion, GuardDutyDetectorDataSources,\
    GuardDutyDataSourceS3Logs, GuardDutyDataSourceMalwareProtection, GuardDutyMalwareProtectionScanEC2, \
    InspectorResource, InspectorAccountRegionSets, InspectorAccounts, InspectorRegion, \
    CloudWatchResource, MonitoringAccountAccountRegionSets, MonitoringAccountAccounts, MonitoringAccountRegion, \
    AppConfigResource, AppConfigAccountRegionSets, AppConfigAccounts, AppConfigRegion, AppConfigApplications, AppConfigApplication, \
    AppConfigApplicationProfiles, AppConfigApplicationEnvironments, AppConfigApplicationProfile, AppConfigApplicationEnvironment, \
    EventBridgeResource, EventBridgeAccountRegionSets, EventBridgeAccounts, EventBridgeRegion, EventBridgeBuses, EventBridgeBusesRules, \
    EventBridgeBusesRule, CloudWatchLogsAccountRegionSets, CloudWatchLogsAccounts, CloudWatchLogsRegion
...
from paco.models.iam import IAM, ManagedPolicy, Role, RoleDefaultEnabled, Policy, AssumeRolePolicy, AssumeRolePolicies, Statement, Principal
...
IAM_USER_PERMISSIONS_CLASS_MAP = {
    ...
    'AccessKeys': IAMUserPermissionAccessKeys
}
...
SUB_TYPES_CLASS_MAP = {
    ...
    ECRRepository: {
        'image_scanning': ('direct_obj', ECRRepositoryImageScanning),
        'repository_policy': ('direct_obj', Policy),
        'lifecycle_policies': ('container', (ECRRepositoryLifecyclePolicies, ECRRepositoryLifecyclePolicy)),
        'identity_provider_roles': ('container', (IAMIdentityProviderRoles, IAMIdentityProviderRole))
    },
    ECRReplicationConfiguration: {
        'rules': ('container', (ECRReplicationRules, ECRReplicationRule)),
    },
    ECRReplicationRule: {
        'regions': ('str_list', zope.schema.TextLine),
        'repositories': ('str_list', zope.schema.TextLine)
    },
    IAMIdentityProviderRole: {
        'policy_actions': ('str_list', zope.schema.TextLine),
        'repository_filters': ('str_list', zope.schema.TextLine)
    },
    DynamoDB: {
    ...
    EC2Resource: {
        'eips': ('threelevel_container', (VPCEIPAccounts, VPCEIPRegions, VPCEIPs, VPCEIP)),
        'keypairs': ('container', (EC2KeyPairs, EC2KeyPair)),
        'users': ('container', (EC2Users, EC2User)),
        'groups': ('container', (EC2Groups, EC2Group)),
    },
    EC2FlowLog: {
        'destination_options': ('direct_obj', EC2FlowLogDestinationOptions)
    },
    GuardDutyResource: {
        'detectors': ('twolevel_container', (GuardDutyDetectorAccountRegionSets, GuardDutyDetectorAccounts, GuardDutyDetectorRegion))
    },
    GuardDutyDetectorRegion: {
        'data_sources': ('direct_obj', GuardDutyDetectorDataSources),
        'monitoring': ('direct_obj', MonitorConfig),
    },
    GuardDutyDetectorDataSources: {
        'malware_protection': ('direct_obj', GuardDutyDataSourceMalwareProtection),
        's3_logs': ('direct_obj', GuardDutyDataSourceS3Logs)
    },
    GuardDutyDataSourceMalwareProtection: {
        'scan_ec2_instance_with_findings': ('direct_obj', GuardDutyMalwareProtectionScanEC2)
    },
    InspectorResource: {
        'inspectors': ('twolevel_container', (InspectorAccountRegionSets, InspectorAccounts, InspectorRegion))
    },
    InspectorRegion: {
        'resource_types': ('str_list', zope.schema.TextLine),
        'monitoring': ('direct_obj', MonitorConfig),
    },
    CloudWatchResource: {
        'monitoring_accounts': ('twolevel_container', (MonitoringAccountAccountRegionSets, MonitoringAccountAccounts, MonitoringAccountRegion)),
        'logs': ('twolevel_container', (CloudWatchLogsAccountRegionSets, CloudWatchLogsAccounts, CloudWatchLogsRegion))
    },
    CloudWatchLogsRegion: {
        'log_groups': ('container', (CloudWatchLogGroups, CloudWatchLogGroup)),
    },
    MonitoringAccountRegion: {
        'data_types': ('str_list', zope.schema.TextLine),
        'source_accounts': ('str_list', PacoReference),
    },
    AppConfigRegion: {
        'applications': ('container', (AppConfigApplications, AppConfigApplication))
    },
    AppConfigApplication: {
        'profiles': ('container', (AppConfigApplicationProfiles, AppConfigApplicationProfile)),
        'environments': ('container', (AppConfigApplicationEnvironments, AppConfigApplicationEnvironment))
    },
    CloudWatchDashboard: {
    ...
    EventBridgeResource: {
        ...
        'services': ('twolevel_container', (EventBridgeAccountRegionSets, EventBridgeAccounts, EventBridgeRegion))
    },
    EventBridgeRegion: {
        'buses': ('direct_obj', EventBridgeBuses)
    },
    EventBridgeBuses: {
        'rules': ('container', (EventBridgeBusesRules, EventBridgeBusesRule))
    },
    EventBridgeBusesRule: {
    ...
    DeploymentPipelineBuildCodeBuild: {
        ...
        'environment_variables': ('obj_list', EnvironmentVariable),
        'role_policies': ('obj_list', Policy),
    ...
    DeploymentPipelineDeployCodeDeploy: {
        ...
        'paco_service_hook': ('direct_obj', PacoServiceHookItem)
    },
    DeploymentPipeline: {
    ...
    RDSPostgresqlAurora: {
        ...
        'dns': ('obj_list', DNS),
        'read_dns': ('obj_list', DNS),
        'global_cluster': ('direct_obj', RDSAuroraGlobalCluster),
    },
    RDSClusterInstance: {
    ...
    CloudFront: {
        ...
        'response_header_policies': ('container', (CloudFrontResponseHeaderPolicies, CloudFrontResponseHeaderPolicy)),
        'factory': ('container', (CloudFrontFactories, CloudFrontFactory)),
        'monitoring': ('direct_obj', MonitorConfig),
        'logging': ('direct_obj', CloudFrontLogging),
    },
    CloudFrontResponseHeaderPolicy: {
        'security_headers_config': ('direct_obj', CloudFrontSecurityHeadersConfig),
    },
    CloudFrontSecurityHeadersConfig: {
        'content_security_policy': ('direct_obj', CloudFrontGenericResponseHeaderPolicy),
        'referrer_policy': ('direct_obj', CloudFrontGenericResponseHeaderPolicy),
        'frame_options_policy': ('direct_obj', CloudFrontGenericResponseHeaderPolicy),
        'content_type_options': ('direct_obj', CloudFrontGenericResponseHeaderPolicy),
        'strict_transport_security_policy': ('direct_obj', CloudFrontStrictTransportHeaderPolicy),
        'xss_protection_policy': ('direct_obj', CloudFrontXSSProtectionHeaderPolicy)
    ...
    ScriptManager: {
        ...
        'custom': ('container', (ScriptManagerCustomScripts, ScriptManagerCustomScript))
    },
    ...
    EC2LaunchOptions: {
        ...
        'amazon_linux_live_patching': ('direct_obj', AmazonLinuxLivePatching),
    },
    Listener: {
        'default_action': ('direct_obj', ListenerDefaultAction),
        'redirect': ('direct_obj', PortProtocol),
        'rules': ('container', (ListenerRules, ListenerRule))
    },
    ListenerRule: {
        'host': ('str_list', zope.schema.TextLine),
        'path_pattern': ('str_list', zope.schema.TextLine)
    },
    ListenerDefaultAction: {
        'redirect': ('direct_obj', PortProtocol),
        'fixed_response': ('direct_obj', ListenerDefaultActionFixedResponse),
    },
    EC2: {
        'security_groups': ('str_list', PacoReference)
    },
    ...
    S3Bucket: {
        'lifecycle_rules': ('obj_list', S3BucketLifecycleRule),
    ...
    NATGateway: {
        ...
        'eips': ('str_list', zope.schema.TextLine)
    },
    VPC: {
        'eip': ('container', (VPCEIPs, VPCEIP)),
        'nat_gateway': ('container', (NATGateways, NATGateway)),
        'vpn_gateway': ('container', (VPNGateways, VPNGateway)),
        'vpn_client_endpoint': ('direct_obj', VPNClientEndpoint),
    ...
    Segment: {
        'nacl': ('direct_obj', SegmentNetworkACL),
    },
    SegmentNetworkACL: {
        'inbound_rules': ('obj_list', SegmentNetworkACLInbound),
    },
    VPNClientEndpoint: {
        'security_groups': ('str_list', PacoReference)
    },
    ...
    Role: {
        ...
        'assume_role_policies': ('direct_obj', AssumeRolePolicies),
        'identity_provider': ('direct_obj', IAMIdentityProviderRole)
    },
    AssumeRolePolicies: {
    ...
    IAMResource: {
        ...
        'policies': ('container', (IAMPolicies, IAMPolicy)),
        'roles': ('container', (IAMRoles, IAMRole)),
        'identity_providers': ('container', (IAMIdentityProviders, IAMIdentityProvider))
    },
    IAMIdentityProvider: {
        'accounts': ('str_list', PacoReference),
        'audiences': ('str_list', zope.schema.TextLine)

    },
    IAMUser: {
        'programmatic_access': ('direct_obj', IAMUserProgrammaticAccess),
        'permissions': ('iam_user_permissions', IAMUserPermissions),
        'managed_policies': ('str_list', PacoReference)
    },
    IAMPolicy: {
        'accounts': ('str_list', PacoReference),
        'roles': ('str_list', zope.schema.TextLine),
    ...
    PacoService: {
        'config': ('dynamic_dict', PacoServiceHookConfig)
    },
    PacoServiceHookItem: {
        'config': ('dynamic_dict', PacoServiceHookConfig)
    },
    ...
    WAFWebACL: {
        ...
        'logging': ('direct_obj', WAFWebACLLogging)
    },
    WAFWebACLRule: {
        'statement': ('direct_obj', WAFWebACLRuleStatement),
        'visibility_config': ('direct_obj', WAFWebACLVisibilityConfig),
        'action': ('direct_obj', WAFWebACLRuleAction),
        'override_action': ('direct_obj', WAFWebACLRuleOverrideAction)
    },
    ...
    WAFWebACLRuleManagedRuleGroup: {
        'excluded_rules': ('str_list', zope.schema.TextLine),
        'rule_action_overrides': ('obj_list', WAFWebACLRuleActionOverride)
    },
    WAFWebACLRuleActionOverride: {
        'action_to_use': ('direct_obj', WAFWebACLRuleAction)
    },
    WAFIPSet: {
...
def merge(base, override):
    ...
    if isinstance(base, list) and isinstance(override, list):
        return deepcopy_except_parent(override)
            ...
            if name != 'name' or not schemas.INamed.providedBy(obj):
                ...
                if schemas.IFileReference.providedBy(field) and value:
                    ...
                    if isinstance(value, list) and None in value:
                        if field.__name__ != 'path_pattern':
                            print('debug msg: might need to investigate this.')
...
{}""".format(read_file_path, obj.paco_ref_parts, name, type(config), config))
            try:
...
""".format(read_file_path, config['type'], name, config))
        ...
        container = container_class(name, obj)
        # if name == 'security_groups' and obj.paco_ref == 'paco.ref netenv.koxa.dev.us-west-2.network.vpc':
        #     breakpoint()
        ...
        first_object_class = sub_class[1]
        second_object_class = sub_class[2]
        third_object_class = sub_class[3]
        container = container_class(name, obj)
        for first_key, first_value in value.items():
            first_obj = first_object_class(first_key, container)
            container[first_key] = first_obj
            for second_key, second_value in first_value.items():
                ...
                for third_key, third_value in second_value.items():
                    if schemas.IVPCEIP.implementedBy(third_object_class):
                        third_obj = third_object_class(third_key, container[first_key][second_key])
                    else:
        ...
        str_ok = False
        if PacoReference == sub_class:
            fields = get_all_fields(obj)
            str_ok = fields[name].value_type.str_ok

        for sub_value in value:
            if PacoReference == sub_class:
                sub_obj = sub_class(str_ok=str_ok).fromUnicode(sub_value)
            else:
...
def iterate_nested_list(nested_list, parent_obj, user_callback, callback_data, paco_ref_list=[]):
    idx = 0
    orig_paco_ref_list = paco_ref_list
    for list_item in nested_list:
        paco_ref_list = deepcopy(orig_paco_ref_list)
        paco_ref_list = orig_paco_ref_list
        if isinstance(list_item, dict):
            nested_list[idx] = iterate_nested_dict(nested_list[idx], nested_list, user_callback, callback_data, paco_ref_list)
        elif isinstance(list_item, list):
            nested_list[idx] = iterate_nested_list(nested_list[idx], nested_list, user_callback, callback_data, paco_ref_list)
        else:
            nested_list[idx] = user_callback(nested_list, idx, parent_obj, callback_data, paco_ref_list)
        idx += 1

    return nested_list

def iterate_nested_dict(nested_dict, parent_obj, user_callback, callback_data, paco_ref_list=[], user_callback_post_dict=None):
    orig_paco_ref_list = paco_ref_list
    for key, value in nested_dict.items():
        paco_ref_list = deepcopy(orig_paco_ref_list)
        if isinstance(value, dict):
            paco_ref_list.append(key)
            nested_dict[key] = iterate_nested_dict(nested_dict[key], nested_dict, user_callback, callback_data, paco_ref_list, user_callback_post_dict)
        elif isinstance(value, list):
            paco_ref_list.append(key)
            nested_dict[key] = iterate_nested_list(nested_dict[key], nested_dict, user_callback, callback_data, paco_ref_list)
        else:
            nested_dict[key] = user_callback(nested_dict, key, parent_obj, callback_data, paco_ref_list)
    if user_callback_post_dict != None:
        nested_dict = user_callback_post_dict(nested_dict, parent_obj, callback_data)
    return nested_dict
...
""".format(self.read_file_path, duplicate_key, exc.context_mark.line, exc.context_mark.column))
    ...
    def load_all(self, config_scope=None):
...
Caveat: You can not have an environment named 'applications'.
    ...
    def instantiate_services(self):
        ...
        for service_info in service_plugins.values():
            ...
            if service_info['name'] in self.generated_service_config.keys():
                config['generated_config'] = self.generated_service_config[service_info['name']]
    ...
    def instantiate_guardduty(self, config):
        obj = GuardDutyResource('guardduty', self.project.resource)
        if config != None:
            apply_attributes_from_config(obj, config, self.config_folder)
        return obj

    def instantiate_inspector(self, config):
        obj = InspectorResource('inspector', self.project.resource)
        if config != None:
            apply_attributes_from_config(obj, config, self.config_folder)
        return obj

    def instantiate_cloudwatch(self, config):
        obj = CloudWatchResource('cloudwatch', self.project.resource)
        if config != None:
            apply_attributes_from_config(obj, config, self.config_folder)
        return obj

    def instantiate_eventbridge(self, config):
        obj = EventBridgeResource('eventbridge', self.project.resource)
        if config != None:
            apply_attributes_from_config(obj, config, self.config_folder)
        return obj

    def instantiate_appconfig(self, config):
        obj = sub_types_loader(
            ...
            sub_class=(AppConfigResource, AppConfigAccounts, AppConfigRegion),
        )
        return obj
    ...
    def instantiate_resources(self, name, config, read_file_path=None):
        ...
        if instantiate_method == None:
            ...
            config = self.process_aliases(None, config)
    ...
    def generated_service_config_duplication_callback(self, config_obj, key, parent_obj, callback_data, paco_ref_list):
        (app_name, dup_app_name) = callback_data
        if isinstance(config_obj[key], str):
            if is_ref(config_obj[key]):
                ref = Reference(config_obj[key])
                ref_app_name = ref.get_netenv_app_name()
                if ref_app_name != None and ref_app_name == app_name:
                    ref.set_netenv_app_name(dup_app_name)
                    config_obj[key] = ref.raw
    ...
    def generated_service_config_duplication(self, env_name, app_name, dup_app_name):
        dup_config = {}
        for service in self.generated_service_config.keys():
            for resource_ref in self.generated_service_config[service].keys():
                ref = Reference('paco.ref '+resource_ref)
                config_env_name = ref.get_netenv_env_name()
                config_app_name = ref.get_netenv_app_name()
                if config_env_name == env_name and config_app_name == app_name:
                    ref.set_netenv_app_name(dup_app_name)
                    # Copy the duplicate config
                    if service not in dup_config.keys():
                        dup_config[service] = {}
                    dup_config[service][ref.ref] = deepcopy(self.generated_service_config[
                        service][resource_ref])
                    dup_config[service][ref.ref] = iterate_nested_dict(
                        dup_config[service][ref.ref],
                        None,
                        self.generated_service_config_duplication_callback,
                        callback_data=(app_name, dup_app_name)
                    )
        self.generated_service_config = merge(self.generated_service_config, dup_config)
    ...
    def instantiate_applications(
        ...
        """
        if 'applications' not in global_config:
            global_config['applications'] = None
        ...
        if env_region_config['applications'] == None: return global_config
        ...
        for item_name, item_config in env_region_config['applications'].items():
            ...
            if match:
                ...
                # Copy Generated Service Configs
                self.generated_service_config_duplication(env_region.__parent__.name, app_name, unique_app_name)
            else:
    ...
    def instantiate_secrets_manager(
        ...
        if 'secrets_manager' not in env_region_config:
            return
        if env_region_config['secrets_manager'] == None:
            return
    ...
    def process_import_from_env_region(self, config):
        ...
        for env_name in config['environments'].keys():
            ...
            for region_name in env_config.keys():
                ...
                if region_config == None or 'import_from' not in region_config.keys():
                    continue
                # Import from file or paco.ref
    ...
    def process_aliases_replace(self, config_obj, key, netenv_name):
        """Iterator User Callback Replace method"""
        # Check if the alias is to a netenv
        alias_ref = Reference(config_obj[key])
        if alias_ref.parts[1] == 'netenv' and alias_ref.parts[2] == netenv_name:
            del alias_ref.parts[0]
            del alias_ref.parts[0]
            del alias_ref.parts[0]
            alias_config = self.before_process_alias_config
            for part in alias_ref.parts:
                alias_config = alias_config[part]
            config_obj[key] = copy.deepcopy(alias_config)
        else:
            ...
            config_obj[key] = copy.deepcopy(self.aliases[alias_name][alias_key])

        return config_obj[key]

    def process_aliases_get_alias(self, config_obj, key, netenv_name):
        """Iterator User Callback Replace method"""
        # Check if the alias is to a netenv
        alias_ref = Reference(key)
        if alias_ref.parts[1] == 'netenv' and alias_ref.parts[2] == netenv_name:
            del alias_ref.parts[0]
            del alias_ref.parts[0]
            del alias_ref.parts[0]
            alias_config = self.before_process_alias_config
            for part in alias_ref.parts:
                if part not in alias_config.keys():
                    breakpoint()
                alias_config = alias_config[part]
            alias_dict = copy.deepcopy(alias_config)
        else:
            alias_name = alias_ref.parts[1]
            if alias_name not in self.aliases.keys():
                raise InvalidPacoAliasReference(f"Invalid alias reference: {alias_ref.raw}: Unable to find alias: {alias_name}")
            alias_key = alias_ref.parts[2]
            if alias_key not in self.aliases[alias_name].keys():
                raise InvalidPacoAliasReference(f"Invalid alias reference: {alias_ref.raw}: unable to find {alias_name} key: {alias_key}")
            alias_dict = copy.deepcopy(self.aliases[alias_name][alias_key])

        return alias_dict

    def process_aliases_user_callback(self, config_obj, key, parent_obj, netenv_name, paco_ref_list):
        alias_sub_str = 'paco.ref alias.'

        if isinstance(key, str) and key.startswith(alias_sub_str):
            config_obj[key] = self.process_aliases_get_alias(config_obj, key, netenv_name)
        if isinstance(config_obj[key], str):
            if config_obj[key].startswith(alias_sub_str):
                config_obj[key] = self.process_aliases_replace(config_obj, key, netenv_name)
        elif config_obj[key] == None or isinstance(config_obj[key], int):
            pass
        else:
            #breakpoint()
    ...
    # Merges in paco.ref alias...: keys into existing dict
    def process_aliases_user_callback_post_dict(self, config_obj, parent_obj, netenv_name):
        alias_sub_str = 'paco.ref alias.'
        for key in config_obj.keys():
            if key.startswith(alias_sub_str):
                config_obj = merge(config_obj[key], config_obj)
                del config_obj[key]
        return config_obj

    def process_aliases(self, netenv_name, config):
        self.before_process_alias_config = config
        return iterate_nested_dict(config, None, self.process_aliases_user_callback, callback_data=netenv_name, user_callback_post_dict=self.process_aliases_user_callback_post_dict)

    def process_generated_service_config(self, service_name, service_config, key, callback_data, paco_ref_list):
        (netenv_ref, env_region_config) = callback_data
        paco_ref_path = f"{netenv_ref}.{'.'.join(paco_ref_list)}"
        if service_name not in self.generated_service_config.keys():
            self.generated_service_config[service_name] = {}
        if 'config' not in service_config.keys():
            self.generated_service_config[service_name][paco_ref_path] = {}
            return
        # Add NetEnv Environment and Region to paco.ref strings
        service_ref_parts = paco_ref_path.split('.')
        service_hook_config = service_config['config']
        if service_ref_parts[0] == 'netenv':
            netenv_name = service_ref_parts[1]
            env_name = service_ref_parts[2]
            region_name = service_ref_parts[3]
            section_name = service_ref_parts[4]
            if section_name in ('applications', 'network', 'secrets_manager', 'backup_vaults'):
                match_str = f'netenv.{netenv_name}.{section_name}'
                replace_str = f'netenv.{netenv_name}.{env_name}.{region_name}.{section_name}'
                service_hook_config = json.loads(json.dumps(service_config['config']).replace(match_str, replace_str))
                config_iter = env_region_config['applications']
                # Merge in global Service config into Environment Region config
                for ref_item in paco_ref_list:
                    if ref_item not in config_iter.keys():
                        config_iter[ref_item] = {}
                    if ref_item == 'paco_service_hook':
                        if config_iter[ref_item] == None:
                            config_iter[ref_item] = {}
                        if 'config' not in config_iter[ref_item].keys():
                            config_iter[ref_item]['config'] = {}
                        config_iter[ref_item]['config'] = merge(service_hook_config, config_iter[ref_item]['config'])
                        break
                    if config_iter[ref_item] == None:
                        config_iter[ref_item] = {}
                    config_iter = config_iter[ref_item]


        self.generated_service_config[service_name][paco_ref_path] = service_hook_config

    def process_service_config_callback(self, config_obj, key, parent_obj, callback_data, paco_ref_list):
        service_sub_str = 'paco.ref service.'
        if isinstance(config_obj[key], str):
            if config_obj[key].startswith(service_sub_str):
                service_name = config_obj[key].split('.')[2]
                self.process_generated_service_config(service_name, config_obj, key, callback_data, paco_ref_list)
        elif config_obj[key] == None or isinstance(config_obj[key], int):
            pass
        else:
            pass
    ...
    def process_service_config(self, netenv_name, env_name, env_region_name, global_config, env_region_config):
        if 'applications' in global_config.keys() and 'applications' in env_region_config.keys():
            netenv_ref = f'netenv.{netenv_name}.{env_name}.{env_region_name}.applications'
            merged_app_config = merge(global_config['applications'], env_region_config['applications'])
            return iterate_nested_dict(merged_app_config, None, self.process_service_config_callback, callback_data=(netenv_ref, env_region_config))
        else:
            return global_config
    ...
    def instantiate_network_environments(self, name, config, read_file_path):
        ...
        config = self.process_aliases(name, config)
        ...
        if 'environments' in config:
            for env_name, env_config in config['environments'].items():
                ...
                for env_reg_name, env_reg_config in config['environments'][env_name].items():
                    ...
                    if config != None:
                        if env_reg_name != 'default':
                            self.process_service_config(name, env_region.__parent__.name, env_reg_name, config, env_region_config)
                    before_config = config
                    config = self.instantiate_applications(
                        env_region_config,
                        env_region,
                        config
                    )
