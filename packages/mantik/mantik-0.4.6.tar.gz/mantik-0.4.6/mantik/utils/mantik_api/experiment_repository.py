...
def get_one(
    ...
    endpoint = f"/mantik-api/projects/{str(project_id)}/experiments/{str(experiment_repository_id)}"  # noqa
...
def get_unique_run_name(
    ...
    endpoint = f"/mantik-api/projects/{str(project_id)}/experiments/{str(experiment_repository_id)}/unique-mlflow-run-name"  # noqa
    response = client.send_request_to_mantik_api(
        method="GET",
        data={},
