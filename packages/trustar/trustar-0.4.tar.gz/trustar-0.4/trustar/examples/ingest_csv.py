"""
TS SDK v1 Ingest CSV example for converting CSV into reports
...
from trustar import log, TruStar, Report
...
def main():
    ...
    parser.add_argument('-t', '--title', required=True, dest='title_col',
    ...
    parser.add_argument('-n', '--num-reports', required=False, dest='num_reports',
                        type=int, default=1000,
                        help='Max number of reports to submit (top-down order)')
    parser.add_argument('-o', '--output', required=False, dest='cef_output_file',
                        default='trustar.cef',
    ...
    for report_num in range(len(df)):
        current_content = ''
        current_title = ''
        ...
        for staged_report in all_reports:
            ...
            while not successful and attempts < 5:
                ...
                try:
                    ...
                    logger.exception(f"Problem submitting report: {e}")
                    time.sleep(5)
