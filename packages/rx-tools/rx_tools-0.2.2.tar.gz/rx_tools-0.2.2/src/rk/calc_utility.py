...
def get_json_table(year, polarity, evt):
    ...
    json_path=get_geo_acc_json(dirname, evt, prodID, f'{year}/{polarity}')
    if not os.path.isfile(json_path):
        return None

    return json_path
#------------------------------
def get_geo_acc_json(dirname, evt, prodID, dset):
    dbb_dir  =os.environ['DBBDIR']
    json_path=f'{dbb_dir}/gen_info/{dirname}/Evt{evt}-P{prodID}.json'
    if os.path.isfile(json_path):
        return json_path

    l_json_path= glob.glob(f'{dbb_dir}/gen_info/{dirname}/Evt{evt}-P*.json')
    njsn       = len(l_json_path) 
    if njsn == 1:
        json_path_backup = l_json_path[0]
        log.warning('-' * 20)
        log.warning(f'For: {dset}')
        log.warning(f'Failed to find {os.path.basename(json_path)}')
