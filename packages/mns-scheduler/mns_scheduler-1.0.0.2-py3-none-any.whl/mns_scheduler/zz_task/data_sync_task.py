...
def col_data_move():
    ...
    if trade_date_common_service_api.is_trade_day(str_day):
        if '192.168.31' in ip_util.get_host_ip():
...
def realtime_quotes_now_zt_kc_data_sync():
    ...
    if trade_date_common_service_api.is_trade_day(str_day):
        ...
        realtime_quotes_now_zt_kc_sync_api.sync_all_kc_zt_data(str_day, None)
        # 同步当日开盘数据
        realtime_quotes_now_zt_kc_sync_api.sync_one_day_open_data(str_day)
        # 涨停数据同步到excel
        export_open_data_to_excel_api.export_kc_zt_data(str_day)
...
def sync_toady_stock_zt_pool():
    ...
    now_date = datetime.now()
    str_day = now_date.strftime('%Y-%m-%d')
    today_high_chg_pool_sync_api.sync_one_day_zt_info(str_day)
...
# 同花顺有效概念清洗
def choose_effective_ths_concept():
    ths_effective_concept_clean_api.choose_effective_ths_concept()
...
# todo 当日k线信息
# 同步一天k线 涨停 数据
blockingScheduler.add_job(sync_daily_data_info, 'cron', hour='15,20', minute='19')
