...
import mns_common.component.k_line.k_line_common_service_api as k_line_patterns_service
...
# 日线 周线 月线 成交量  筹码信息
def calculate_k_line_info(str_day, symbol):
...
def handle_day_line(k_line_info, str_day, symbol):
    ...
    k_line_info = k_line_field(k_line_info.copy(), stock_qfq_daily_one.copy())
    ...
    # 排除最近有三板以上的股票
    k_line_info = exclude_recent_three_board_stock(str_day, k_line_info)
    return k_line_info
...
# 字段选择
def k_line_field(k_line_info, stock_qfq_daily_one):
...
# 排除最近10个交易日有三板以上的股票
def exclude_recent_three_board_stock(str_day, k_line_info):
    k_line_info.loc[:, 'exclude'] = False
    exclude_symbol_list = get_recent_three_board_stock(str_day)
    k_line_info.loc[k_line_info['symbol'].isin(exclude_symbol_list), 'exclude'] = True
    return k_line_info


# 排除最近有三个连板以上的股票  10天
@lru_cache(maxsize=None)
def get_recent_three_board_stock(str_day):
    recent_trade_day = trade_date_common_service_api.get_before_trade_date(str_day, EXCLUDE_DAYS)
    query = {"$and": [{'str_day': {"$gte": recent_trade_day}}, {'str_day': {"$lte": str_day}}],
             'connected_boards_numbers': {"$gte": MAX_CONTINUE_BOARDS}}
    stock_zt_connected_boards_pool = mongodb_util.find_query_data('stock_zt_pool', query)
    if data_frame_util.is_not_empty(stock_zt_connected_boards_pool):
        return list(stock_zt_connected_boards_pool['symbol'])
    else:
        return ['000001']
