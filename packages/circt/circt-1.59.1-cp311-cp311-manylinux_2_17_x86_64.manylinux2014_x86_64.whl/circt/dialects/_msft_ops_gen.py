...
def constructs_channel(input, clk, sym_name, default_stages, *, loc=None, ip=None) -> _ods_ir.OpResult:
  return _get_op_result_or_op_results(ChannelOp(input=input, clk=clk, sym_name=sym_name, defaultStages=default_stages, loc=loc, ip=ip))
...
def physical_region(sym_name, bounds, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(DeclPhysicalRegionOp(sym_name=sym_name, bounds=bounds, loc=loc, ip=ip))
...
def instance_dynamic(instance_ref, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(DynamicInstanceOp(instanceRef=instance_ref, loc=loc, ip=ip))
...
def instance_verb_attr(name, value, *, sub_path=None, ref=None, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(DynamicInstanceVerbatimAttrOp(name=name, value=value, subPath=sub_path, ref=ref, loc=loc, ip=ip))
...
def instance_hierarchy(top_module_ref, *, inst_name=None, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(InstanceHierarchyOp(topModuleRef=top_module_ref, instName=inst_name, loc=loc, ip=ip))
...
def hlc_linear(outs, clock, *, loc=None, ip=None) -> _ods_ir.OpResult:
...
class PDMulticycleOp(_ods_ir.OpView):
  ...
  def __init__(self, source, dest, cycles, *, loc=None, ip=None):
    ...
    issubclass(type(source), _ods_ir.Attribute) or
    not _ods_ir.AttrBuilder.contains('FlatSymbolRefAttr')) else
    ...
    issubclass(type(cycles), _ods_ir.Attribute) or
...
def pd_multicycle(source, dest, cycles, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(PDMulticycleOp(source=source, dest=dest, cycles=cycles, loc=loc, ip=ip))
...
def pd_location(loc_, *, sub_path=None, ref=None, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(PDPhysLocationOp(loc_=loc_, subPath=sub_path, ref=ref, loc=loc, ip=ip))
...
def pd_physregion(phys_region_ref, *, sub_path=None, ref=None, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(PDPhysRegionOp(physRegionRef=phys_region_ref, subPath=sub_path, ref=ref, loc=loc, ip=ip))
...
def pd_reg_location(locs, *, ref=None, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(PDRegPhysLocationOp(locs=locs, ref=ref, loc=loc, ip=ip))
...
def pe_output(output, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(PEOutputOp(output=output, loc=loc, ip=ip))
...
def systolic_array(pe_outputs, row_inputs, col_inputs, *, loc=None, ip=None) -> _ods_ir.OpResult:
  return _get_op_result_or_op_results(SystolicArrayOp(peOutputs=pe_outputs, rowInputs=row_inputs, colInputs=col_inputs, loc=loc, ip=ip))
