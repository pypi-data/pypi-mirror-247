...
def process_pubmed_central(text: str):
    ...
    for tag in soup.select('front article-categories subject'):
        if tag.text not in ('Article', 'Articles'):
    ...
    for part in list(body.select('abstract, back, front, journal-id, journal-meta,'
                                 ' license, properties, ref-list, related-article, supplementary-material')):
        part.extract()
