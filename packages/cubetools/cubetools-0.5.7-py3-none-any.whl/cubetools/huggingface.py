...
def download_model(model_id, hf_url='https://hf-mirror.com/', threads=10, exclude=None):
    if model_id.find('/') < 1:
        print('model_id格式不对！')
        return None
...
def download():
    ...
    parser.add_argument('model_id', default=None, help='HuggingFace model ID, like: model_owner/model_name')
    parser.add_argument('--hf-url', default='https://hf-mirror.com/', help='HuggingFace URL endpoint')
    parser.add_argument('--threads', default=10, type=int, help='Number of download threads for aria2c')
    parser.add_argument('--exclude', default=None, help='The patterns to match against filenames for exclusion, seperated with ","')
    args = parser.parse_args()

    download_model(model_id=args.model_id, hf_url=args.hf_url, threads=args.threads, exclude=args.exclude)
