...
from test_tele.pyrogram.reactor import inline_reactor, get_tp_file
...
async def run_pyrogram():
    ...
    async def inline_handler(app, inline_query: InlineQuery):
        ...
        # await inline_mode_path(app, inline_query)
        if inline_query.query.startswith('.px'):
            logging.warning("px jalan")
            await inline_pixiv(app, inline_query)
        elif inline_query.query.startswith('.tp'):
            logging.warning("tp jalan")
            await inline_reactor(app, inline_query)
        else:
            logging.warning("gb jalan")
            await inline_gelbooru(app, inline_query)


    @app.on_callback_query(filters.regex(r"(?:gb|px)"))
    async def callback_query_handler(app, callback_query: CallbackQuery):
        """Get callback query from inline keyboard"""
        # image_file = await callback_mode_path(app, callback_query)
        ...
        elif callback_query.data.startswith("tp"):
            image_file = await get_tp_file(callback_query.data.replace("tx ", ''))
