...
async def image_keyboard(query: str, my_list: list[str]) -> InlineKeyboardMarkup:
    buttons = [[
            ...
            #                          switch_inline_query_current_chat=f".md uploader:{my_list['uploader'].replace(' ', '+')}"),
            # ],[
                InlineKeyboardButton("📖" ,
...
async def set_url(query: str):
    url = query.lower().strip().replace('.md ', '')
...
async def generate_telegraph(id):
    ...
    gallery_dl_result = await gallery_dl(url, offset=200, filter='--range')
