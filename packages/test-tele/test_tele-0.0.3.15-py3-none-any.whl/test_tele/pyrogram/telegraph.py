...
async def generate_new_telepage(images: list, title: str, author_name: str):
    content = []
    compressed_image_path = await compress_image(images)

    async with aiohttp.ClientSession() as session:
        upload_tasks = [upload_image(session, image) for image in compressed_image_path]
    ...
    logging.warning(uploaded_results)

    for image, response in uploaded_results:
        logging.warning("ini image : ", image, "ini response : ", response)
    ...
    shutil.rmtree(f'temps/{title.split(" - ")[0]}')
...
async def compress_image(images):
    ...
    for image in images:
        with open(image, 'rb') as f:
            ...
            img.save(compressed_image_path, format='JPEG', quality=10)
...
async def images_in_folder(folder_path):
