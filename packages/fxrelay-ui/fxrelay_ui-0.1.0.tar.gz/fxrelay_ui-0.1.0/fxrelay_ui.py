...
class FakeEntry(factory.Factory):
    id = factory.Faker("pyint")
    full_address = factory.LazyFunction(lambda: f"fake-{random.randrange(1000, 9999)}@mozmail.com")
    description = factory.Faker("company")
    num_forwarded = factory.Faker("pyint", max_value=50)
    num_blocked = factory.Faker("pyint", max_value=50)
    enabled = factory.Faker("pybool")
    block_list_emails = factory.Faker("pybool")
    _created_at = factory.Faker("date_between", start_date=datetime.date(2021, 1, 1))
...
class FxRelayClient:
    ...
    def xlist_entries(self):
        return factory.build_batch(dict, 50, FACTORY_CLASS=FakeEntry)
        return [
            {
                "id": random.randrange(100_000, 999_999),
                "full_address": f"fake-{random.randrange(1000, 9999)}@mozmail.com",
                "description": "plop",
                "num_forwarded": random.randrange(0, 50),
                "num_blocked": random.randrange(0, 50),
                "enabled": True,
