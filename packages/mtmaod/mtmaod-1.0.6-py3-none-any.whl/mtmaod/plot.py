...
def density_chart(x, y, xlabel="AOD Measured", ylabel="AOD Retrieved", title=None, type="kernels", bins=25, xy_max_edge=2, save_path=None, dpi=300, figsize=(3, 3), style=None, fontsize=9, ticksize=8):
    ...
    if style in style_dict:
        figsize = style_dict[style].get("figsize", figsize)
        fontsize = style_dict[style].get("fontsize", fontsize)
        ticksize = style_dict[style].get("ticksize", ticksize)
    with mpl.rc_context():
        # 设置字体
        font = {'family': 'serif', 'serif': 'Times New Roman', 'weight': 'normal', 'size': fontsize}
        mpl.rc('font', **font)
        ...
        if type in ["kernels"]:
            ...
            # 生成散点图图层
            ax_data = axes.scatter(x, y, marker = 'o', c = z, s = 20, cmap = 'jet', vmin = 0)
        ...
        # 生成包络线图层
        axes.plot([-1 / 3, xy_max_edge+1], [-1 / 3, (xy_max_edge+1) - (0.05 + 0.15 * (xy_max_edge+1))], linestyle='--', color='black', label='EE envelopes', linewidth = 0.5)
        axes.plot([-1 / 3, xy_max_edge], [-1 / 3, xy_max_edge + (0.05 + 0.15 * xy_max_edge)], linestyle='--', color='black', linewidth = 0.6)
        ...
        axes.text(x=0, y=xy_max_edge, s=add_text, fontsize=ticksize, verticalalignment='top', bbox=dict(boxstyle='round', facecolor='white', alpha=0, linewidth=0.2))

        # 生成图例
        lg = axes.legend(loc=4, alignment="right", fontsize=ticksize, labelspacing=0.2, borderpad=0.2, handletextpad=0.2, handlelength=1)
        ...
        if save_path is not None:
            ...
            pic_dirpath = os.path.dirname(save_path)
