...
class LayerNextClient:
    ...
    def update_virtual_collection(
        ...
        print(f'create or update virtual collection - name: {virtual_collection_name}, id: {virtual_collection_id}')
    ...
    def find_chunk(
            ...
            adjacent_chunk_limit: int = 0

        ):
        print(f'find chunk - search_key_list: {search_key_list}, unique_name_list: {unique_name_list}, response_chunk_limit: {response_chunk_limit}, adjacent_chunk_limit: {adjacent_chunk_limit}')

        _datalake_client = datalake.DatalakeClient(
            self.encoded_key_secret, self.layernext_url)

        response = _datalake_client.find_chunk(
            search_key_list, unique_name_list, response_chunk_limit, adjacent_chunk_limit)
    ...
    """
    def execute_mongo_query(self, function, input_data, collection):
        _datalake_client = datalake.DatalakeClient(
            self.encoded_key_secret, self.layernext_url)

        return _datalake_client.execute_mongo_query(function, input_data, collection)
