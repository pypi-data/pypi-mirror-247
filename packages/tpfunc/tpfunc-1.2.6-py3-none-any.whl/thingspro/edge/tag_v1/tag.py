...
class Subscriber(Token):
    ...
    def __has_subscribed(self, provider, source, tags):
        key = "{}/{}/{}".format(provider, source, ",".join(tags))
    ...
    def subscribe(self, provider, source, tags):
        ...
        subscribed, topic = self.__has_subscribed(provider, source, tags)
        if subscribed:
            return

        url = self.__get_endpoint(provider, source, tags)
        self._threads[topic] = threading.Thread(target=self.__tag_monitoring, args=(url, ), daemon=True)
        if self._threads[topic] is not None:
            self._threads[topic].start()
    ...
    def unsubscribe(self):
        ...
        if hasattr(self, '_threads'):
            for topic, thread in self._threads.items():
