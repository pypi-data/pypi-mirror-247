...
def decode_video_using_imageio(data, format="mp4"):
...
def encode_video_using_imageio(data, format="mp4", fps=30):
...
def read_video_using_imageio(openfile, format="mp4"):
    reader = imageio.get_reader(openfile, format=format)
    return [im for im in reader]


def write_video_using_imageio(openfile, data, format="mp4", fps=30):
    temp_filename = "/tmp/iotools_temp." + format
    if format == "gif":
        kwargs = {"duration": 1000 * 1 / fps}
    else:
...
def load_video_using_imageio(filename):
    reader = imageio.get_reader(filename)
    return [im for im in reader]


def save_video_using_imageio(filename, data, format="mp4", fps=30):
    if format == "gif":
        kwargs = {"duration": 1000 * 1 / fps}
    else:
        kwargs = {"fps": fps}
