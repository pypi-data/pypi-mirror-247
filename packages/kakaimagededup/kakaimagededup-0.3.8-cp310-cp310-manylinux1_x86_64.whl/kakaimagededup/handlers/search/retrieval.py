...
from kakaimagededup.handlers.search.bktree import BKTree
from kakaimagededup.handlers.search.hash_block import HashBlock
from kakaimagededup.handlers.search.brute_force import BruteForce
from kakaimagededup.handlers.search.brute_force_cython import BruteForceCython
from kakaimagededup.utils.general_utils import parallelise
from kakaimagededup.utils.logger import return_logger
...
class HashEval:
    def __init__(
        ...
        elif search_method == 'hashblock':
            self._fetch_nearest_neighbors_hashblock()
        elif no_cython:
            print('======= 启用brute force search no-cython版 =========')
            self._fetch_nearest_neighbors_brute_force()
        else:
            print('======= 启用brute force search cython版 =========')
    ...
    def _fetch_nearest_neighbors_hashblock(self) -> None:
        """
        Wrapper function to retrieve results for all queries in dataset using a Hash-Block search.
        """
        logger.info('Start: Retrieving duplicates using Hash-Block algorithm')
        hash_block = HashBlock(self.test, self.distance_invoker)  # construct bktree
        self._get_query_results(hash_block)
        logger.info('End: Retrieving duplicates using Hash-Block algorithm')
