...
API_FILE_DOWNLOAD_PARALLEL_THRESHOLD = 500 * MiB
API_FILE_DOWNLOAD_PARALLELS = 20
...
def _http_get_model_file(
    ...
    *,
    ...
    logger.info("parallels %d , part_size %d B, chunk_size %d B", parallels, part_size, chunk_size)

    downloaded_size = temp_file.tell()
    if downloaded_size > 0 and parallels > 1:
        raise ValueError('conflict condition: parallels > 1 while using resume downloading')
    ...
    if not parallel_flag:
        ...
        logger.info("use accelerate extension")
        hf_transfer = None
        try:
            import hf_transfer
        except ImportError:
    ...
    *,
    ...
    for model_file in model_files:
        if model_file['file_type'] == 'FileType_Dir' or \
                any([fnmatch(model_file['name'], pattern) for pattern in ignore_patterns]):
            # TODO add log
            continue
        ...
        if not force_download and local_file_path.exists():
            ...
            if local_file_path.stat().st_size != int(model_file['size_bytes']):
                raise FileExistsError(f"file '{file_path}' exists and different from remote server, you can set 'force_download=True' to override it")
            else:
                continue
...
def create_and_upload_model(
    ...
    with tempfile.TemporaryDirectory(prefix='scoamp', dir=cache_root_dir) as tmp_git_dir_path:
