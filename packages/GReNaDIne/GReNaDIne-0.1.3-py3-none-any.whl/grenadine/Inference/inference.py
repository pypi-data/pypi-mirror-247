...
class gene_model:
    ...
    def get_scores(self):
        """
        Return the scores for the best self-expressive gene-level model 
...
class gxn_model:
    """
    Make a genme-level self expressive optimized model
    ...
    def fit(self,X,Y,regulators,to_inspect=None):  
        ...
        for gene in tqdm(to_inspect):
            self.gene_models_[gene] = gene_model(self.params_gridsearch)
            X_train = X[[e for e in tfs_ if e!=gene]]
            y_train = Y[gene]
    ...
    def score(self):
        ...
        """
        self.scores_ = pd.DataFrame({m: self.gene_models_[m].get_scores() for m in self.gene_models_})
        self.scores_ = self.scores_.T
        self.scores_.fillna(0,inplace=True)
        return self.scores_
    
    def plot_scores(self, metrics_list = None):
        """
        ...
        if metrics_list is None:
            metrics_list = [c for c in self.scores_.columns if "mean_test" in c]
        s = self.scores_[metrics_list]
        sns.boxplot(s,orient='h',notch=True, showcaps=False,
                    flierprops={"marker": "."},
