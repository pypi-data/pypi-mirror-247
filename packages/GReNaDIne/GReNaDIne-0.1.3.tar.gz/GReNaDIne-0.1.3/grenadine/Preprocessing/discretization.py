...
def discretize_genexp(data, method, nb_bins=2, axis=0):
    ...
    if method == 'kmeans':
        discr = KBinsDiscretizer(n_bins=nb_bins, encode='ordinal', strategy='kmeans')
    elif method == 'ewd':
        discr = KBinsDiscretizer(n_bins=nb_bins, encode='ordinal', strategy='uniform')
    elif method == 'efd':
        discr = KBinsDiscretizer(n_bins=nb_bins, encode='ordinal', strategy='quantile')
...
def bikmeans_original(data, nb_bins):
    ...
    """
    km = KBinsDiscretizer(nb_bins, 'ordinal', 'kmeans')
