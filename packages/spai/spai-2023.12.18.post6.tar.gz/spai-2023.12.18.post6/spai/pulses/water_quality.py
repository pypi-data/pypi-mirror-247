...
def water_quality(image_name,aoi_mask,storage,desired_images=2,date=None):
    ...
    median_water_mask, stored_images, water_masks_array = compute_median_water_mask(aoi_mask, desired_images, date, storage) # aoi_mask is needed for downloading

    # save raster median_water_mask
    raster_name_median_water_mask = f"median_water_mask_{date}.tif"
    median_water_mask_path = storage.create(median_water_mask, raster_name_median_water_mask, ds=ds)
    ...
    ndwi_masked, out_transform = mask_raster(raster_name_ndwi, shp, storage)
    ...
    ndti_masked, out_transform = mask_raster(raster_name_ndti, shp, storage)
    ndti_masked[ndti_masked == False] = np.nan # convert false to nan
    ...
    ndci_masked, out_transform = mask_raster(raster_name_ndci, shp, storage)
    ndci_masked[ndci_masked == False] = np.nan # convert false to nan
    ...
    doc_masked, out_transform = mask_raster(raster_name_doc, shp, storage)
    doc_masked[doc_masked == False] = np.nan # convert false to nan
    ...
    ndti_has = np.divide(ndti_px_counted, 100, out=np.zeros_like(ndti_px_counted, dtype=np.float64), where=100 != 0)
    ...
    timestamped_date = pd.Timestamp(date).date()
    save_table(
        data=ndti_has,
        columns=turbidity_columns,
        table_name=turbidity_table_name,
        date=timestamped_date,
        storage=storage,
    )
    ndti_percent = [np.divide(ndti_has[i],ndti_has[-1])*100 for i in range(len(ndti_has)-1)]
    ...
    ndci_has = np.divide(ndci_px_counted, 100, out=np.zeros_like(ndci_px_counted, dtype=np.float64), where=100 != 0)
    ...
    ndci_percent = [np.divide(ndci_has[i],ndci_has[-1])*100 for i in range(len(ndci_has)-1)]
    ...
    doc_categories = np.digitize(doc_masked, [min_to_N0, N0_to_N1, N1_to_N2, N2_to_N3, N3_to_max])
    raster_name_doc_categorized = f"DOC_categorized_{date}.tif"
    storage.create(doc_categories, raster_name_doc_categorized, ds=ds)

    doc_colorized = colorize_raster(doc_categories, colors=['green', 'yellow', 'orange', 'red'])
    ...
    doc_has = np.divide(doc_px_counted, 100, out=np.zeros_like(doc_px_counted, dtype=np.float64), where=100 != 0)
    ...
    doc_percent = [np.divide(doc_has[i],doc_has[-1])*100 for i in range(len(doc_has)-1)]
...
def compute_median_water_mask(aoi_mask, desired_images, from_date_to_earlier, storage):
    ...
    stored_images = download_missing_images(desired_images=desired_images, from_date_to_earlier=from_date_to_earlier, bbox=bbox, storage=storage)
...
def download_missing_images(desired_images, from_date_to_earlier, bbox, storage):
    ...
    while len(stored_images) < desired_images:
        time_interval = ((date_obj - timedelta(days=10)).strftime('%Y-%m-%d'), date_obj.strftime('%Y-%m-%d'))
        images_found = explore_satellite_images(bbox, time_interval=time_interval, cloud_cover=5)
        for image in images_found:
            if f"S2L2A_{image['date'].split('T')[0]}.tif" not in stored_images:
                path = download_satellite_image(storage, bbox, image['date'].split("T")[0])
...
def compute_doc(stored_images, storage):
    ...
    for path in stored_images:
        ...
        doc_values_dict = {"mean_doc": mean_doc, "max_doc": max_doc, "min_doc": min_doc, "std_doc": std_doc}
...
def update_extent_table(extent_has, date, storage):
    if f'table_water_extent.json' not in storage.list():
        df = pd.DataFrame({'Water [Has]': extent_has, 'Not Water [Has]': 0, 'Total [Has]': extent_has,'Percentage [%]': 100}, index=[date])
        df = update_df(df)
    else:
        ...
        if date in df.index:
            ...
            update_df(df)
            return df
        new_row = pd.DataFrame({'Water [Has]': extent_has, 'Not Water [Has]': 0, 'Total [Has]': 0,'Percentage [%]': 0}, index =[date])
