...
def download_waterways_from_osm(gdf: Union[str, gpd.GeoDataFrame], 
                                storage,
    ...
    final_waterways_gdf = final_waterways_gdf[final_waterways_gdf.geometry.type.isin(('LineString', 'MultiLineString'))]
    storage.create_from_parquet(final_waterways_gdf, name="waterways.parquet")


def download_roads_from_osm(gdf: Union[str, gpd.GeoDataFrame], 
                                storage,
                                waterway_tags: Optional[dict] = {
    ...
    final_roads_gdf = final_roads_gdf[final_roads_gdf.geometry.type.isin(('LineString', 'MultiLineString'))]
...
def download_buildings_from_osm(gdf: Union[str, gpd.GeoDataFrame], storage):
    ...
    final_buildings_gdf = final_buildings_gdf[final_buildings_gdf.geometry.type.isin(('Polygon', 'MultyPolygon'))]
    ...
    for _, row in gdf.iterrows():
        ...
        for key, values in tags.items():
            if isinstance(values, list):
                for value in values:
                    try:
                        waterway_gdf = ox.features_from_polygon(polygon, tags={key: value})
