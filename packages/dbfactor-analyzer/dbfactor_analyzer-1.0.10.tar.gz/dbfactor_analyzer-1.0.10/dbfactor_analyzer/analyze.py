...
# LRUCache（最近最少使用缓存）算法, 提高函数的执行效率
# 要求python在3.8以下
# from functools import lru_cache （备选，性能不如fastcache中的）
from cached_property import cached_property #用于实现属性的缓存,避免重复计算
...
from .prepare import (get_clean_factor_and_forward_returns, rate_of_return,
                      std_conversion,change_data_index_format)
...
class FactorAnalyzer(object):
    ...
    ----------
        ...
        calc_factor_alpha_beta:
            计算因子的 alpha 和 beta
        calc_factor_alpha_beta_tstatistic:
            计算因子的 alpha 和 beta的t统计量和pvalue
    ...
    def __init__(self, factor, prices, groupby=None, weights=1.0, 
                 quantiles=5, bins=None, periods=(14,),
                 binning_by_group=False, specify_industry=False,industry=None,
                 max_loss=0.25, zero_aware=False):
        
        if isinstance(factor, pd.DataFrame):
            date_str=factor.index[0]
            try:
                datetime.strptime(date_str, '%Y/%m/%d')
                self.factor=change_data_index_format(factor)
            except ValueError:
                self.factor = factor
        
        prices_copy=prices.copy()
        date_str=prices_copy.index[0]
        try:
            datetime.strptime(date_str, '%Y/%m/%d')
            self.prices=change_data_index_format(prices_copy)
        except ValueError:
        ...
        self.specify_industry=specify_industry
    ...
    def __gen_clean_factor_and_forward_returns(self):
        ...
        #stocks = list(factor_data.index.get_level_values(1).drop_duplicates()) #drop_duplicates()去重
        ...
        self._clean_factor_data = get_clean_factor_and_forward_returns(
            ...
            specify_industry=self.specify_industry,
    ...
    def calc_factor_alpha_beta(self, demeaned=True, group_adjust=False):
        ...
        return pef.factor_alpha_beta(self._clean_factor_data,
                                     demeaned=demeaned,
                                     group_adjust=group_adjust).iloc[0:2,:]
    @lru_cache(4)
    def calc_factor_alpha_beta_tstatistic(self, demeaned=True, group_adjust=False):
        """计算因子的 alpha 和 beta的t统计量和pvalue
        ...
        return pef.factor_alpha_beta(self._clean_factor_data,
                                     demeaned=demeaned,
                                     group_adjust=group_adjust).iloc[2:,:]
    ...
    def mean_return_by_group(self):
        ...
        返回值:
            ...
            index 分别是分位数、 行业名称,  column 是 period  (1,14)
        """
    ...
    def ic(self):
        ...
        返回 DataFrame, index 是时间,  column 是 period 的值 (1,14)
        """

        return self.calc_factor_information_coefficient()
    @property
    def rank_ic(self):
        return self.calc_factor_information_coefficient(method='rank')
    ...
    @property
    def rank_ic_by_group(self):
        return self.calc_mean_information_coefficient(by_group=True,method='rank')
    ...
    @property
    def rank_ic_monthly(self):
        ic_monthly = self.calc_mean_information_coefficient(group_adjust=False,
                                                            by_group=False,
                                                            by_time="M",method='rank').copy()
        ic_monthly.index = ic_monthly.index.map(lambda x: x.strftime('%Y-%m'))
        return ic_monthly   
    ...
    def plot_information_table(self, group_adjust=False, method=None):
        ...
        """
        if method=='rank':
            ic = self.calc_factor_information_coefficient(group_adjust=group_adjust,
                                                          by_group=False,method=method)
            pl.plot_rank_information_table(ic)
        else:
            ic = self.calc_factor_information_coefficient(group_adjust=group_adjust,
                                                          by_group=False,method=method)
    ...
    def plot_cumulative_returns_by_quantile_last(self,period=None, demeaned=False, group_adjust=False):
        """画指定调仓周期的各分位数累计收益
        ...
        for p in period:
            if p in self._periods:
                pl.plot_cumulative_returns_by_quantile_last(self._clean_factor_data,
    ...
    def plot_top_down_cumulative_returns(self, period=None, demeaned=False, group_adjust=False):
        ...
        for p in period:
            if p in self._periods:
                ...
                pl.plot_top_down_cumulative_returns(self._clean_factor_data,
    ...
    def plot_cumulative_returns_by_quantile(self, period=None, demeaned=False,
        ...
        for p in period:
            if p in self._periods:
                pl.plot_cumulative_returns_by_quantile(self._clean_factor_data,
                    mean_return_by_date[convert_to_forward_returns_columns(p)],
                    period=p
                )
    ...
    def create_full_tear_sheet(self, demeaned=False, group_adjust=False, by_group=False,
        ...
        self.plot_quantile_returns_bar(by_group=False,
                                       ...
                                       group_adjust=group_adjust)
        pl.plt.show()
        self.plot_cumulative_returns_by_quantile(period=turnover_periods,
                                                 demeaned=demeaned,
                                                 group_adjust=group_adjust)
        self.plot_cumulative_returns_by_quantile_last(demeaned=demeaned,group_adjust=group_adjust)
        pl.plt.show()
        self.plot_cumulative_returns(period=turnover_periods, demeaned=demeaned, group_adjust=group_adjust)
        pl.plt.show()
        self.plot_top_down_cumulative_returns(period=turnover_periods,
                                              demeaned=demeaned,
                                              group_adjust=group_adjust)
        pl.plt.show()
        '''
        ...
        if by_group:
            self.plot_quantile_returns_bar(by_group=True,
                                           ...
                                           group_adjust=group_adjust)
            pl.plt.show()
        #self.plot_quantile_returns_violin(demeaned=demeaned,group_adjust=group_adjust)
        #pl.plt.show()
        print("\n-------------------------\n")
        self.plot_information_table(group_adjust=group_adjust) #IC
        self.plot_information_table(group_adjust=group_adjust,method='rank') #Rank_IC
        self.plot_ic_ts(group_adjust=group_adjust, method=None)
        self.plot_ic_ts(group_adjust=group_adjust, method='rank')
        pl.plt.show()
        #self.plot_ic_qq(group_adjust=group_adjust)
        #pl.plt.show()
        if by_group:
    ...
    def ic_rankic_analyze(self,group_adjust=False,by_group=False):
        """输出因子的ic分析表"""
        print("\n-------------------------\n")
        self.plot_information_table(group_adjust=group_adjust) #IC
        self.plot_information_table(group_adjust=group_adjust,method='rank') #Rank_IC
