    ...
    def quantile_calc(x, _quantiles, _bins, _zero_aware, _no_raise):
        try:
            if _quantiles is not None and _bins is None and not _zero_aware:
                return pd.qcut(x, _quantiles, labels=False) + 1  #从小往大排，值越大组越高
            #后续使用报错maybe需要修改代码  return pd.qcut(x.rank(method='first'), _quantiles, labels=False,duplicates='drop') + 1
...
def industry_info_todict(industry_data):
    ''' 
    将读取的csv行业数据转换成dict(key为stock_code)
    参数：
    -------------
    industry_data: pd.read_csv('.\industry申万三级.csv', encoding='gbk')
    '''
    industry_data=industry_data.dropna()
    groupby=industry_data.set_index('code')['industry'].to_dict()
    return groupby
        ...
        if specify_industry: #指定行业
            ...
            if 'weights' in merged_data.columns:
                merged_data['weights'] = merged_data.set_index(
                    ...
                    ).droplevel(level=(2,4)).reset_index('factor_quantile',drop=True)
...
def industry_get(industry,stock):
    industry=industry.dropna()
    stock_industry=industry[industry['code']==stock]['industry'].values[0]
