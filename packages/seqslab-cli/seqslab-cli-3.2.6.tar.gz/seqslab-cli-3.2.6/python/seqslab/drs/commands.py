...
from seqslab.drs.utils import ProgressBarObject
...
class BaseDatahub:
    ...
    def _stdin() -> list:
        ...
        try:
            ...
            raise ValueError(
                f'Stdin format only supports json. Please read the document and make sure the format is valid.')
    ...
    @argument("non_interactive",
              type=bool,
              positional=False,
              description="Whether to run in non-interactive mode, i.e. without prompt (default = False).",
              aliases=['ni'])
    def upload(
            self, workspace: str, src: str, dst: str = '/', recursive: bool = False, output: str = 'json',
            concurrency: int = 0, multiprocessing: int = 1, chunk_size: str = '8', expiry_time: str = None,
        ...
        if expiry_time:
            ...
            if not re.fullmatch(pattern, expiry_time):
                cprint("Make sure expiry_time format is correct. e.g. 2023-12-31 00:00:00", "red")
                return errno.EINVAL

        dst = '/' if dst == "." else dst
        result = self._upload(src=URL(src, encoded=True), dst=URL(dst, encoded=True), recursive=recursive,
                              workspace=workspace, multiprocessing=multiprocessing, concurrency=concurrency,
                              chunk_size=self.size[chunk_size], proxy=self.proxy, non_interactive=non_interactive,
    ...
    def download(self, workspace: str, dst: str, id: str = None, uri: URL = None, overwrite: bool = False,
                 output: str = 'json', chunk_size: str = '4', concurrency: int = 120, multiprocessing: int = 1) -> int:
        """
        ...
        if not id:
            if not uri:
                logging.error("Invalid executions, Enter a valid drs_ids or a valid self_uri.")
                cprint("Invalid executions, Enter a valid drs_ids or a valid self_uri.", "red")
                return errno.EINVAL
            else:
                id = uri.name
                if uri.host:
            ...
            if uri:
                logging.error("Invalid executions, Enter a valid drs_id or a valid self_uri,"
                              "not both given.")
                cprint("Invalid executions, Enter a valid drs_id or a valid self_uri,"
                       "not both given.", "red")
                return errno.EINVAL

        tasks.append(
            self._download(drs_id=id, dst=dst, workspace=workspace, chunk_size=self.size[chunk_size],
                           bandwidth=concurrency, proxy=self.proxy, overwrite=overwrite,
    ...
    async def _download(drs_id: str, dst: str, **kwargs):
        ...
        kwargs['access_url'] = URL(download_info['access_url'])
    ...
    def register_blob(self, workspace: str, type: str, output: str = 'json', stdin: bool = False, **kwargs) -> int:
        """
    ...
    @argument("output",
              ...
              choices=['json', 'table', 'tsv'])
    def register_bundle(self, name: str, workspace: str, drs_id: List[str], output: str = 'json', **kwargs):
        """
        ...
        results = self._register_bundle(name=name, drs_ids=drs_id, workspace=workspace, **kwargs)
        if isinstance(results, int):
            return results
        __log(results=results, output=output)
        return 0
    ...
    @staticmethod
    @exception_handler
    def _register_bundle(name: str, drs_ids: List[str], workspace: str, stdin: bool = False, **kwargs) -> List[dict]:
        backend = drs_register().load_register(workspace)
        if stdin:
    ...
    @argument("names",
              type=List[str],
              positional=False,
              description="Locate a DRS object using a list of names "
                          "(optional).")
    @argument("tags",
              type=List[str],
              positional=False,
              description="Locate a DRS object using a list of labels "
                          "(optional).")
    def search(
            self,
            tags: List[str] = [],
    ...
    @staticmethod
    def find_fastq_paths(fastq_path: str, fq_sig: str) -> List[str]:
        return [os.path.join(root, f) for root, dirs, files in os.walk(fastq_path) for f in files
                if f.find(fq_sig) != -1 and f.rfind('fastq.gz') != -1]
    ...
    def create_drs_metadata(hdr_meta, sample_meta):
        ...
        for key in sample_meta:
            if sample_meta[key]:
                extra_properties.append(AtgxMetaData.extra_properties(key, sample_meta[key]))
        ...
        types.append(AtgxMetaData.data_type(platform_value=platform, method_value=method, instr_value=instrument))
    ...
    @staticmethod
    def match_fastq_files_to_runsheet(
            fastq_path: str,
            run_sheet: RunSheet,
            fastq_signature_expr: str,
            upload_dst: str
    ) -> list:
        """
        Create an upload list based on the Run Sheet and FASTQ path information.
        """
        hdr_meta = {k.replace(' ', '_'): v for k, v in run_sheet.SampleSheet.Header.items()}
        overall_idx = 0
        upload_info = []
        for s in run_sheet.SampleSheet.samples:
            ...
            fastq_signature = BaseDatahub._fastq_expr(fastq_signature_expr, {**hdr_meta, **sample_meta})
            ...
            if run_sheet.SampleSheet.is_single_end:
                ...
                upload_info.append({
                    "src": fq_paths[0],
                    "dst": f'{upload_dst}/{fastq_signature}_{sample_meta.get("Pair")}/',
                    "tags": [f"{s.get('Run_Name', run_sheet.SampleSheet.Header.Date)}/{s.get('Read1_Label', '')}"],
                    "metadata": BaseDatahub.create_drs_metadata(hdr_meta, sample_meta)
                })
                if id_rule:
                    upload_info[-1]['id'] = BaseDatahub._gen_drs_id(unquote(id_rule), upload_info[-1]['metadata'])
            ...
            if run_sheet.SampleSheet.is_paired_end:
                ...
                upload_info.append({
                    "src": fq_paths[1],
                    "dst": f'{upload_dst}/{fastq_signature}_{sample_meta.get("Pair")}/',
                    "tags": [f"{s.get('Run_Name', run_sheet.SampleSheet.Header.Date)}/{s.get('Read2_Label', '')}"],
                    "metadata": BaseDatahub.create_drs_metadata(hdr_meta, sample_meta)
                })
                if id_rule:
                    upload_info[-1]['id'] = BaseDatahub._gen_drs_id(unquote(id_rule), upload_info[-1]['metadata'])
                    upload_info[-2]['id'] = BaseDatahub._gen_drs_id(unquote(id_rule), upload_info[-2]['metadata'])
    ...
    @argument("input_dir",
              type=str,
              positional=False,
              description="Specify the FASTQ file path (required).")
    @argument("upload_dst",
              type=str,
              positional=False,
    ...
    @argument("non_interactive",
              ...
              description="Whether to run in non-interactive mode, i.e. without prompt (default = False).",
              aliases=['ni'])
    def upload_runsheet(self, workspace, input_dir: str, run_sheet: str, upload_dst: str = None,
                        fastq_signature: str = '~{Sample_ID}', concurrency: int = 0, multiprocessing: int = 1,
                        chunk_size: str = '8', expiry_time: str = None,
                        non_interactive: bool = False
                        ) -> int:
        """
        ...
        failed = False
        if not upload_dst:
            upload_dst = int(datetime.timestamp(datetime.now()))
        ...
        upload_payload = BaseDatahub.match_fastq_files_to_runsheet(input_dir, rs, fastq_signature, upload_dst)
        ret = []
