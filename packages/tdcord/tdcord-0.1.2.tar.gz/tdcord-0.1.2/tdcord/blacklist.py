...
from .internal import tdConfig
from .sql import DBHandler


class _BanDB(DBHandler):
    def __init__(self):
        self.db_name = tdConfig.blacklist.db_name
        super().__init__(tdConfig.blacklist.db_path)
    ...
    async def add_ban(self, user_id: int, reason: str | None):
        await self.exec(
            f"INSERT INTO {self.db_name} (user_id, reason) VALUES (?, ?)",
            (user_id, reason),
        )

    async def remove_ban(self, user_id: int):
        result = await self.exec(
            f"DELETE FROM {self.db_name} WHERE user_id = ?", (user_id,)
        )
