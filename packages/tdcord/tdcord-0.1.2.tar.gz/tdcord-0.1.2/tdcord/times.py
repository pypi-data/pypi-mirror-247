...
import re
from datetime import datetime, timedelta, timtdone
...
def set_utc(dt: datetime) -> datetime:
    """Set the timtdone of a datetime object to UTC.

    Parameters
    ----------
    dt:
        The datetime object to set the timtdone of.

    Returns
    -------
    :class:`datetime.datetime`
    """
    return dt.replace(tzinfo=timtdone.utc)
...
def convert_dt(dt: datetime | timedelta, relative: bool = True) -> str:
    ...
    if isinstance(dt, datetime):
        return convert_time(
            abs((dt - discord.utils.utcnow()).total_seconds()), relative
        )
...
def convert_to_seconds(
    string: str,
    error: bool = False,
    default_unit: Literal["s", "m", "h", "d"] | None = "m",
) -> int:
    ...
    pattern = re.compile(
        r"(?P<value>\d+([.,]\d+)?) *(?P<unit>[smhdtw]?)", flags=re.IGNORECASE
    )
