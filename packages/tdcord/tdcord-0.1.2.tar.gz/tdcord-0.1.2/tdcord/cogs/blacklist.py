...
from ..internal import tdConfig, t
...
async def _check_blacklist(interaction: discord.Interaction) -> bool:
    ...
    if interaction.user.id in bans and tdConfig.blacklist:
        if tdConfig.blacklist.raise_error:
            raise Blacklisted()
        else:
...
class Blacklist(Cog, hidden=True):
    ...
    async def cog_check(self, ctx):
        if tdConfig.blacklist.owner_only:
    ...
    if PYCORD:
        admin = discord.SlashCommandGroup(
            t("admin_group"),
            description="tdcord admin commands",
            guild_ids=tdConfig.admin_guilds,
    ...
    async def leave_guild(self, ctx, guild_id: str):
        ...
        await guild.leave()
        await ctx.response.send_message(
            f"I left **{guild.name}** ({guild.id})", ephemeral=True
        )

    @leave.command(
        name="owner", description="Make the bot leave all guilds with a given owner"
    )
