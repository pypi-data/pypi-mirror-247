...
.. note::
        ...
        inherit from :class:`~tdView` instead of :class:`discord.ui.View`.

        .. code-block:: python

            from tdcord import View
...
def _check_coro(func):
    if not asyncio.iscoroutinefunction(func):
        raise TypeError(
            f"Event registered must be a coroutine function, not {type(func)}"
        )
...
class tdView(discord.ui.View):
    ...
    async def on_error(
        ...
        Executes all registered error handlers with the ``@tdcord.event`` decorator.
        """
        if not PYCORD:
...
class tdModal(discord.ui.Modal):
