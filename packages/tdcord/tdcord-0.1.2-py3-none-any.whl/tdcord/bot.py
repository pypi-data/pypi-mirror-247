...
class Bot(_main_bot):  # type: ignore
    """The tdCord bot class. This is a subclass of :class:`discord.Bot` if you use Pycord.
    ...
    **kwargs:
    ...
    async def get_application_context(
        self, interaction: discord.Interaction, cls=tdContext
    ):
    ...
    def _cog_count_log(
        ...
        if directory and "{sum}" in log_format and "{directory}" in log_format:
            ...
            not directory and "{sum}" in log_format and "{directory}" not in log_format
        ):
            log_format = log_format.replace("{sum}", str(count))
        else:
            return
        ...
        *directories: str,
        ...
        for directory in directories:
            for root, dirs, files in os.walk(directory):
                ...
                self._cog_count_log(
                    custom_log_level, log, loaded_dir_cogs, log_color, path.stem
                )
    ...
    def ready(
        ...
        *,
        ...
        print_custom_ready(
            self, title, modifications, style, default_info, new_info, colors
        )
    ...
    async def _error_event(
        self, ctx: discord.Interaction, error: discord.DiscordException
    ):
        ...
        if isinstance(error, commands.CommandOnCooldown):
            ...
            if self.error_handler:
                ...
                try:
                    ...
                    if e.code != 10062:
                        self.logger.error(
                            "Could not send error message to user", exc_info=e
                        )
    ...
    def add_help_command(
        ...
        *,
        ...
        if not DPY:
            self.load_extension("tdcord.cogs.pyc.help_setup", package="tdcord")
    ...
    def add_status_changer(
        ...
        **kwargs:
        ...
        if not DPY:
            self.load_extension(
                "tdcord.cogs.pyc.status_changer_setup", package="tdcord"
            )
    ...
    def add_blacklist(
        ...
        *,
        ...
        tdConfig.blacklist = Blacklist(
            db_path,
            db_name,
            raise_error,
            owner_only,
        )
        tdConfig.admin_guilds = admin_server_ids

        self.enabled_extensions.append("blacklist")
        if not DPY:
            self.load_extension("tdcord.cogs.pyc.blacklist_setup", package="tdcord")
    ...
    async def setup_hook(self):
        ...
        for ext in self.enabled_extensions:
            await self.load_extension(f".cogs.dpy.{ext}_setup", package="tdcord")
            ...
            if error_webhook_url is not None:
                ...
                if not self.error_event_added:
                    if DPY:
                        ...
                        self.add_listener(
                            self._error_event, "on_application_command_error"
                        )
...
class PrefixBot(Bot, commands.Bot):
    ...
    This class can be used if you want to use tdCord with prefix commands.
...
class BridgeBot(Bot, bridge.Bot):
    ...
    This class can be used if you want to use tdCord with bridge commands.
    This is only needed for Pycord.
    """
