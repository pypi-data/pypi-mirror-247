...
async def get_settings_object_by_name(request: web.Request) -> web.Response:
    ...
    responses:
      ...
      '404':
        $ref: '#/components/responses/404'
    """
    obj = await mongoservicelib.get_by_name_dict(request, MONGODB_SETTINGS_COLLECTION)
    if obj is None:
        obj = await _default_settings_object_by_name(request, request.match_info['name'])

    return await response.get(request, obj)
...
async def get_all_settings_objects(request: web.Request) -> web.Response:
    ...
    responses:
      '200':
        $ref: '#/components/responses/200'
    """
    objs = await mongoservicelib.get_all_dict(request, MONGODB_SETTINGS_COLLECTION)
    missing = set(DEFAULT_SETTINGS_OBJECT_TEMPLATES.keys()).difference(obj.get('name') for obj in objs)
    if missing:
        objs_ = list(objs)
        for m in missing:
            if m in DEFAULT_SETTINGS_OBJECT_TEMPLATES:
                objs_.append(await _default_settings_object_by_name(request, m))
    else:
        objs_ = objs
    return await response.get_all(request, objs_)
...
async def _default_settings_object_by_name(request: web.Request, name: str) -> DesktopObjectDict | None:
    obj = deepcopy(DEFAULT_SETTINGS_OBJECT_TEMPLATES.get(name))
    if obj is not None:
        obj['user'] = request.headers.get(SUB, NONE_USER)
        obj['shares'][0]['user'] = request.headers.get(SUB, NONE_USER)
        id_ = await mongoservicelib.post_dict_return_id(request, obj, MONGODB_SETTINGS_COLLECTION, SettingsObject)
        if id_ is None:
...
async def _get_settings_object(request: web.Request) -> web.Response:
    obj = await mongoservicelib.get_dict(request, MONGODB_SETTINGS_COLLECTION)
    if obj is None:
        obj = await _default_settings_object_by_name(request, request.match_info['id'])
