...
class SimpleStreamFileLogger(object):
    ...
    '''
    def __init__(self, log_path, date_format='subdir', level='DEBUG', format="[%(asctime)s][%(levelname)s] %(message)s"):
...
class LoggerHandler(logging.Logger):
    ...
    '''
    def __init__(self, log_path=None, handles=None, handle_config=None, name='root', 
                 level='DEBUG', format="[%(asctime)s][%(filename)s][line:%(lineno)d][%(levelname)s] %(message)s"):
        super().__init__(name)
        # 日志器默认级别
        level_dict = {'DEBUG': logging.DEBUG, 'INFO': logging.INFO, 'WARNING': logging.WARNING, 
                      'ERROR': logging.ERROR, 'CRITICAL':logging.CRITICAL}
