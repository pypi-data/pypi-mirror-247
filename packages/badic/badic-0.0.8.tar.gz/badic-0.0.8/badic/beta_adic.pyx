...
cdef class BetaBase:
	...
	def Proj(self, DetAutomaton a, DetAutomaton b, t=0, m=1, pref=False, arel=None, int algo_inter=1, bint only_aut=True):
		self.m.a = a
		return self.m.proj(b, t=-t, m=m, pref=pref, arel=arel, aut=only_aut, algo_inter=algo_inter)

	def relations_automaton(self, t=0, bint isvide=False, list Ad=None,
							list A=None, list B=None,
							bint couples=False, bint ext=False,
							bint mirror=False,
							bint prune=True, int nhash=1000003, int prec=53, int algo=3,
							int coeff=1, bint verb=False):
...
#		return orbit_points
	...
	def relations_automaton(self, t=0, m=1, bint isvide=False, list Ad=None, list A=None, list B=None,
							 ...
							 bint prune=True, int nhash=1000003, int prec=53, int algo=1, int coeff=1, bint verb=False):
		r"""
		...
		TESTS::
			...
			sage: arel1 = m.relations_automaton(t=1, algo=1)
			sage: arel2 = m.relations_automaton(t=1, algo=2)
			sage: arel3 = m.relations_automaton(t=1, algo=3)
			sage: arel4 = m.relations_automaton(t=1, algo=3, keep_labels=True)
...
#		return (log(len(S)) / (niter * abs(log(m))))
	...
	def intersection(self, BetaAdicSet m, t=0, bint ext=False, int algo=3, int algo_inter=1, bint verb=False):
		r"""
		...
		EXAMPLES::
			...
			#. Compute the intersection of two Rauzy fractals (for the same beta)
				sage: from badic.beta_adic import DumontThomas
				sage: m = DumontThomas(WordMorphism("a->ab,b->ac,c->a")).mirror()	  # Tribonnacci
				sage: m2 = DumontThomas(WordMorphism("a->ab,b->ca,c->a")).mirror()	 # flipped Tribonnacci
		...
		TESTS::
			...
			sage: from badic.beta_adic import DumontThomas
			sage: m = DumontThomas(WordMorphism("a->ab,b->a")).mirror()
			sage: m2 = DumontThomas(WordMorphism("a->aab,b->a")).mirror()
			sage: mi = m.intersection(m2)
			Traceback (most recent call last):
			...
		...
		ai = ar.intersection(a, algo=algo_inter)
		if verb:
			print("ai = %s" % ai)
	...
	def intersection_words(self, list w1, list w2, bint ext=True, bint verb=False):
		r"""
	...
	def reduced_words_automaton(self, bint full=False, int step=100, alex=None,
		...
		if full:
			...
			if verb > 0:
				print("alex=%s" % alex)
			ai = ai.intersection(alex, algo=algo_inter, verb=verb-1)
			if verb > 0:
				print("ai=%s" % ai)
			ai = ai.proji(1)
			if verb > 0:
	...
	def critical_exponent_free(self, prec=None, int verb=1):
		r"""
	...
	def complete(self, list A=None,
				 bint ext=False, DetAutomaton arel=None, bint simplify=True, bint verb=False):
		r"""
	...
	def proj(self, a, t=0, m=1, DetAutomaton arel=None, int algo=3, int algo_inter=1, bint pref=False, bint simplify=True, bint aut=False):
		r"""
		Project m*self+t on the zero completion of a.

		INPUT:

		- ``a`` - DetAutomaton or BetaAdicSet giving a language on which to project

		- ``t`` - algebraic number (default : ``0``)
		  The translation.

		- ``m`` - algebraic number (default: ``1``)
			The scalar multiple.
		...
		- ``algo_inter`` -- int (default: ``1``) - algo used to compute intersections of languages

		- ``aut``  Bool -- (default: ``False``)
		  If True, returns only the DetAutomaton rather that the BetaAdicSet.

		- ``pref``  Bool -- (default: ``False``)
		  If True, take the prefix_closure of the result.

		- ``simplify``  Bool -- (default: ``True``)
		  If True, prune and minimize the DetAutomaton of the result.
		...
		EXAMPLES::
			...
			sage: from badic.beta_adic import DumontThomas
			sage: m1 = DumontThomas(WordMorphism('a->ab,b->ac,c->a')).mirror()
		...
		if arel is None:
			...
			arel = self.relations_automaton(t=t, m=m, couples=True, algo=algo,
											A=a.alphabet, B=self.a.alphabet)
		ai = arel.intersection(a.concat_zero_star().product(self.a.concat_zero_star()), algo=algo_inter)
		r = ai.proji(0)
		r.zero_complete_op()
		if pref:
			r = r.prefix_closure()
...
#
	...
	def domain_exchange(self, n=None, int algo=1, int algo_rel=3, int algo_inter=1, bint test_Pisot=True,
						int ndiam=30, bint verb=False):
		...
		- ``algo_inter`` - int -- (default: 2)
			algorithm used to compute intersection of languages
		...
		for t in it:
			...
			if verb:
				print("t=%s" % t)
			mi = m.intersection(m, -t, algo=algo_rel, algo_inter=algo_inter)
			mia = mi.a.intersection(a, algo=algo_inter)
			#mi = m.proj(a, -t, algo=algo_rel, algo_inter=algo_inter)
	...
	def substitution(self, DetAutomaton ap=None,
							  np=None, list lt=None, proj=False, bint need_included=True,
							  bint get_aut=False, step=None, int algo_inter=1, bint verb=False):
		r"""
		...
		- ``proj`` - Bool (default: ``False``) - if False, pieces of the domain exchange are assumed to be projected on self

		- ``algo_inter`` -- int (default: ``2``) - algo used to compute intersections of languages
		...
		if lt is None:
			...
			l = self.domain_exchange(test_Pisot=False, algo_inter=algo_inter)
		...
		if not a.included(ap):
			...
			if not m.Proj(ap, aa, algo_inter=algo_inter).has_same_language_as(aa) and need_included:
		...
		if np is None:
			for i in range(1, 300):
				ba = aa.unshift(0, i)
				ba.zero_complete_op()
				if m.Proj(aa, ba, algo_inter=algo_inter).has_same_language_as(ba):
					ba = m.Proj(ba, aa)
					np = i
					break
			if np is None:
			...
			ba = aa.unshift(0, np)
			ba.zero_complete_op()
			if not m.Proj(aa, ba, algo_inter=algo_inter).has_same_language_as(ba):
				raise ValueError('The g-beta-expansion must be b^np invariant (here np=%s).' % np)
			ba = m.Proj(ba, aa)
		if verb:
			print("np = %s" % np)
		if proj:
			# adapt the domain exchange
			lt2 = []
			for a, t in lt:
				lt2.append((m.Proj(a, ap, algo_inter=algo_inter), t))
			lt = lt2
		if verb:
		...
		if verb:
			print("d = %s" % d)
		lf = list(range(1, len(lm)))  # list of leaf
		if verb:
			print("lf = %s" % lf)
		...
		for i, (a1, t1) in enumerate(lm):
			...
			a = m.Proj(a, ap, t=-tr, algo_inter=algo_inter)
			if a.has_empty_language():
			...
			while True:
				...
				if j is None:
					...
					if verb:
						print("Subdivide into %s pieces..." % len(l))
					# compute intersections (split a1)
					for j in l:
						a2 = lm[j][0]
						# cut a according to a2
						# m.move2(t=tr, a=a2) #translate a2 de -tr
						a = m.Proj(a2, ap.unshift(0, np), t=tr, algo_inter=algo_inter)
						...
						if ab is None:
							...
							if abc is None:
								...
								if verb:
									print("tr=%s : subdivision of %s w.r.t. ba (new %s)..." % (tr, i, len(lm)))
					...
					a = m.Proj(a, ap, t=-lm[j][1], arel=arel[lm[j][1]], algo_inter=algo_inter)
					tr += lm[j][1]
				# split according to ba
...
def rauzy_fractal_projection(self, b=None, verb=False):
	r"""
