...
class Grid:
    ...
    def _thumbnail(self, cmap: ColorMap):
        with BytesIO() as buffer:
            figure = pyplot.figure(frameon=False)
            axes = figure.add_axes((0, 0, 1, 1))
            axes.axis("off")
            pyplot.imshow(self.data, cmap=cmap)
    ...
    def map(
        ...
        with warnings.catch_warnings():
            ...
            grid = self.project(4326)

        figure = folium.Figure(width=str(width), height=height)
        bounds = [[grid.ymin, grid.xmin], [grid.ymax, grid.xmax]]
...
class Stack:
    ...
    def _thumbnail(self, *rgb: int):
        with BytesIO() as buffer:
            figure = pyplot.figure(frameon=False)
            axes = figure.add_axes((0, 0, 1, 1))
            axes.axis("off")
            pyplot.imshow(
                np.dstack([self.grids[i - 1].type("uint8").data for i in rgb])
            )
            pyplot.savefig(buffer)
            pyplot.close(figure)
            image = b64encode(buffer.getvalue()).decode()
