...
class Grid:
    ...
    def focal(
        self, func: Callable[[np.ndarray, int], ndarray], buffer: int, circle: bool
    ):
        return _focal(func, buffer, circle, self)

    def focal_mean(self, buffer=1, circle: bool = False) -> "Grid":
        return self.focal(np.nanmean, buffer, circle)

    def focal_std(self, buffer=1, circle: bool = False) -> "Grid":
        return self.focal(np.nanstd, buffer, circle)

    def focal_min(self, buffer=1, circle: bool = False) -> "Grid":
        return self.focal(np.nanmin, buffer, circle)

    def focal_max(self, buffer=1, circle: bool = False) -> "Grid":
        return self.focal(np.nanmax, buffer, circle)

    def focal_sum(self, buffer=1, circle: bool = False) -> "Grid":
        return self.focal(np.nansum, buffer, circle)
