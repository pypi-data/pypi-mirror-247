...
class MainWindow(QMainWindow):
    ...
    def WindowToolbar(self, name, tool_items, movable, position, ID, size, border, orientation, newline):
    ...
    def WindowTitlebar(self, title, icon, widgets, alignment, backgound_color,  text_color,
                        height, default):
        ...
        if default == False:
            if current_window_style == SYSTEM_PLATFORM_FUSION:
                ...
                CONTROL_MINIMIZED, CONTROL_RESTORE, CONTROL_MAXIMIZED, CONTROL_CLOSE = WINDOWS_CONTROLS()
        ...
        if self._custom_title_bar == False and self._frame == False:
            ...
            def control_action(key: str = '', other: bool = False):
                state = window_state()
            ...
            def icon_controls(slot_one, slot_two, slot_three, size):
                ...
                def change_appearance_maximize():
                    state = window_state()
                    max = GET(MAXIMIZE_BUTTON_ID)
                ...
                maximize_switch = (
                    change_appearance_maximize,
                    call(change_appearance, MAXIMIZE_BUTTON_ID, MAXIMIZE_EMPTY)
                )
                
                control_buttons = [
                    Button(icon=MAXIMIZE_EMPTY, id=MAXIMIZE_BUTTON_ID,
                        func=call(control_action, 'max', True), focus=False, icon_size=18, hover=maximize_switch),
                    Button(icon=MINIMIZE_EMPTY, id=MINIMIZE_BUTTON_ID,
                        func=call(control_action, 'min', True), focus=False, icon_size=18, hover=minimize_switch),
            ...
            if default == False:
                if current_window_style == SYSTEM_PLATFORM_FUSION:
                    title_items = icon_controls(1, 0, 2, 17) 
                else:
                    title_items = [
                        titlebar_icon,
                        Text(title, id=TITLE_TEXT_ID,
            ...
            if widgets != None:
                if align == 'center':
                    ...
                    window_title = [title_items[0]]
                    controls = title_items[1:]
...
def handle_group(widgets, grouplayout):
    ...
    groupcontent.setExclusive(True)
    ...
    for items in widgets:
        ...
        for item in items:
            ...
            if widget_type not in containers:
                ...
                if widgets != None:
                    ...
                    for items in widgets:
                        ...
                        if widget_type == ELEM_TYPE_GROUP:
                            ...
                            NotRequired, widgets, group_layout = items
                            if widgets != None:
                ...
                Notrequired, widgets, group_layout = widget_items
...
class Titlebar:
    def __init__(self, title: str = path.basename(sys.argv[0]), icon: str = None, widgets: list = None, alignment: None = None, text_color: str = 'white',
                 background_color: str = 'rgb(32, 32, 32)', height: any = CUSTOM_TITLEBAR_HEIGHT, default: bool = True):
        '''
        Titlebar
        ...
        window.WindowTitlebar(title, icon, widgets, alignment,
                              background_color, text_color, height, default)
...
def thread(target, wait: bool = False):
    if wait == True:
        return lambda function: clera_multi_threading.start(target)
    elif wait == False:
        clera_multi_threading.start(target)
