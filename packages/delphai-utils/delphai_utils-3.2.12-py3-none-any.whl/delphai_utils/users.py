...
def get_user(context: ServicerContext) -> Dict:
    ...
    if decoded_access_token.get("sub"):
        user["user_id"] = decoded_access_token["sub"]
    elif decoded_access_token.get("payload", {}).get("sub"):
        user["user_id"] = decoded_access_token["payload"]["sub"]
