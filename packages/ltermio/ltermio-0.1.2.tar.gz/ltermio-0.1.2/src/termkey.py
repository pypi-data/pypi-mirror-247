...
def getch(timeout: int = BLOCKING_) -> str:
    ...
    Returns:
        A character in string if successfully reading. Otherwise returns
    ...
    new_flags = old_flags = termios.tcgetattr(in_fd)
        ...
        if seq in '[O':
            ...
            while key_ch:
                ...
                if 0x40 <= ord(key_ch) <= 0x7E:
                    # CSI terminal characters are in range of 0x40-0x7E.
                    fcode = _match_csi_sequence(seq)
                    if fcode != Key.NONE:
                        return fcode
                    break
                key_ch = getch(0)
        # Putback the left keys to the buffer.
        for key_ch in seq:
            _key_buffer.append(key_ch)
    return key_code
