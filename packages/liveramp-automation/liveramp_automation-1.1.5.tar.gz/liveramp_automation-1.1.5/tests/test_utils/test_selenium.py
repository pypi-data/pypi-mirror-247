...
def test_find_element_by_linktext(mock_chrome_driver):
    my_selenium_instance = SeleniumUtils(mock_chrome_driver)
    locator = 'Example Link Text'
    element = my_selenium_instance.find_element_by_linktext(locator)
    assert element.get_attribute("id") == "element_test"
    mock_chrome_driver.find_element.assert_called_once_with(By.LINK_TEXT, locator)


def test_find_element_by_partial_linktext(mock_chrome_driver):
    my_selenium_instance = SeleniumUtils(mock_chrome_driver)
    locator = 'Partial Link Text'
    element = my_selenium_instance.find_element_by_partial_linktext(locator)
    assert element.get_attribute("id") == "element_test"
...
def test_find_elements_by_partial_linktext(mock_chrome_driver):
    my_selenium_instance = SeleniumUtils(mock_chrome_driver)
    locator = 'example_partial_link_text'
    elements = my_selenium_instance.find_elements_by_partial_linktext(locator)
    assert len(elements) == 2
    assert elements[0].get_attribute("id") == "element_test_1"
    assert elements[1].get_attribute("id") == "element_test_2"
...
def test_find_elements_by_linktext(mock_chrome_driver):
    my_selenium_instance = SeleniumUtils(mock_chrome_driver)
    locator = 'example_link_text'
    elements = my_selenium_instance.find_elements_by_linktext(locator)
    assert len(elements) == 2
    assert elements[0].get_attribute("id") == "element_test_1"
    assert elements[1].get_attribute("id") == "element_test_2"
