...
class subscriber:
    ...
    Supported message types:
        - 'msgpack', 'bytes', 'images': Subscribe to all messages (no message filtering).
        - 'json', 'string', 'image': Subscribe to messages with a specific topic prefix..

    """
    def __init__(self, topic, node = "default", msg_type = "dict", conf =  {'transport': "ZMQ", 'network': "broker", 'mode':"many2many", "master_ip":"127.0.0.1"}, debug = False):
        ...
        try:
            ...
            if self.master_ip == "127.0.0.1":
                print ("SUB: " + topic + " in local-host") if self.debug and self.topic != "/nep_node" and not self.topic.startswith("/nepplus") else None
            else:
                print ("SUB: " + topic + " in " +  str(self.master_ip)) if self.debug and self.topic != "/nep_node" and not self.topic.startswith("/nepplus") else None
        except:
            pass
        ...
        if self.transport ==  "ZMQ":                                      #Use ZeroMQ
            if self.topic != "/nep_node":
                print ("SUB: " + topic + ", using ZMQ " + self.network) if self.debug and self.topic != "/nep_node" and not self.topic.startswith("/nepplus") else None
    ...
    def __network_selection(self):
        ...
        if self.network == "direct":
            ...
            print("SUB: " + self.topic + " waiting NEP master in " + self.master_ip) if self.debug and self.topic != "/nep_node" and not self.topic.startswith("/nepplus") else None
            ...
            print("SUB: " + self.topic + " socket ready")  if self.debug and self.topic != "/nep_node" and not self.topic.startswith("/nepplus") else None
    ...
    def __create_ZMQ_subscriber(self):
        ...
        Supported message types:
        - 'msgpack', 'bytes', 'ros', 'ros2', 'images': Subscribe to all messages (no message filtering).
        - 'json', 'string', 'image': Subscribe to messages with a specific topic prefix.
        ...
        if success:
            try:
                ...
                if self.msg_type in ["msgpack", "bytes", "images"]:
                    self.sock.setsockopt(zmq.SUBSCRIBE, b"")
                    self.sock.setsockopt(zmq.CONFLATE, 1)  # Only keeps last message in queue
                elif self.msg_type in ["json", "string", "image"]:
    ...
    def __connect_ZMQ_socket(self):
        ...
        if self.mode == "many2one":
            ...
            print("SUB: " + self.topic + " endpoint " + endpoint +  " bind") if self.debug and self.topic != "/nep_node" and not self.topic.startswith("/nepplus") else None
        elif self.mode == "one2many":

            endpoint = "tcp://" + self.ip + ":" + str(self.port)
            self.sock.connect(endpoint)
            print("SUB: " + self.topic + " endpoint " + endpoint +  " connect") if self.debug and self.topic != "/nep_node" and not self.topic.startswith("/nepplus") else None
    ...
    def listen(self, block_mode=False):
        ...
        """
        msg_type_functions = {
            "msgpack": self.listen_msgpack,
            "bytes": self.listen_bytes,
            "json": self.listen_json,
            "string": self.listen_string,
            "image": self.listen_image,
            "images": self.listen_images,
        }
        ...
        if self.msg_type in msg_type_functions:
            ...
            msg = f"Unsupported msg_type selected: '{self.msg_type}'"
            raise ValueError(msg)
    ...
    def listen_bytes(self, block_mode=False):
        """
    ...
    def listen_images(self, block_mode = False):
        """
