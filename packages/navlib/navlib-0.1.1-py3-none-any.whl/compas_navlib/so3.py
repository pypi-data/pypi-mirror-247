...
Functions:
    ...
    wrap360(angles): Wrap an array or list of angles in degrees to the range [-360, 360).
    wrap180(angles): Wrap an array or list of angles in degrees to the range [-180, 180).
    wrap2pi(angles): Wrap an array or list of angles in radians to the range [-2pi, 2pi).
    wrap1pi(angles): Wrap an array or list of angles in radians to the range [-pi, pi).
    unwrap2pi(angles): Unwrap an array or list of angles in radians from the range [-2pi, 2pi).
    unwrap1pi(angles): Unwrap an array or list of angles in radians from the range [-pi, pi).
    unwrap360(angles): Unwrap an array or list of angles in degrees from the range [-360, 360).
    unwrap180(angles): Unwrap an array or list of angles in degrees from the range [-180, 180).
    wrapunwrap360(angles): Wrap to and unwrap an array or list of angles in degrees to the range [-360, 360).
    wrapunwrap (angles): Wrap to and unwrap an array or list of angles in radians to the range [-2pi, 2pi).
...
def rot_inv(rot: np.ndarray) -> np.ndarray:
    ...
    Returns:
        np.ndarray: The inverse of the rotation matrix as a (3,3) numpy array.
...
def hpr2quat(hpr: Union[np.ndarray, Iterable[float]]) -> np.ndarray:
    ...
    Returns:
        np.ndarray: The quaternion using hpr with the convention ZYX in order (x, y, z, w)
...
def rph2quat(rph: Union[np.ndarray, Iterable[float]]) -> np.ndarray:
    ...
    Returns:
        np.ndarray: The quaternion using rph with the convention ZYX in order (x, y, z, w)
...
def quat2hpr(quat: Union[np.ndarray, Iterable[float]]) -> np.ndarray:
    ...
    Args:
        quat (Union[np.ndarray, Iterable[float]]): The quaternion as a numpy array or as a list.

    Returns:
        np.ndarray: The HPR angles in radians as a numpy array with the convention ZYX.
...
def quat2rph(quat: Union[np.ndarray, Iterable[float]]) -> np.ndarray:
    ...
    Returns:
        np.ndarray: The RPH angles in radians as a numpy array with the convention ZYX.
...
def hpr2rot(hpr: Union[np.ndarray, Iterable[float]]) -> np.ndarray:
    ...
    Returns:
        np.ndarray: The rotation matrix using hpr with the convention ZYX
...
def rph2rot(rph: Union[np.ndarray, Iterable[float]]) -> np.ndarray:
    ...
    Returns:
        np.ndarray: The rotation matrix using rph with the convention ZYX
...
def rot2hpr(rot: np.ndarray) -> np.ndarray:
    ...
    p = math.atan2(-rot[2, 0], math.sqrt(math.pow(rot[2, 1], 2) + math.pow(rot[2, 2], 2)))
...
def rot2quat(rot: np.ndarray) -> np.ndarray:
    ...
    Returns:
        np.ndarray: The quaternion as a numpy array with the convention ZYX in order (x, y, z, w)
...
def rot_diff(rot_1: np.ndarray, rot_2: np.ndarray) -> np.ndarray:
    ...
    The returned matrix is the attitude of 2 with respect to 1: R_12 = R_1w @ R_w2 = inv(R_w1) @ R_w2
    ...
    Returns:
        np.ndarray: The attitude of 2 with respect to 1 as a numpy array.
...
def vec_to_so3(vec: Union[np.ndarray, Iterable[float]]) -> np.ndarray:
    ...
    Returns:
        np.ndarray: The skew-symmetric matrix as a numpy array.
...
def so3_to_vec(mat: np.ndarray) -> np.ndarray:
    ...
    Returns:
        np.ndarray: The vector as a (3, ) numpy array.
...
def axis_ang3(vec: Union[Iterable[float], np.ndarray]) -> Tuple[np.ndarray, float]:
    """
    Converts a R3 representing a rotation into a exponential coordinates
    ...
    Returns:
        Tuple[np.ndarray, float]: Axis-Angle representation for the rotation as a tuple with the unit axis
...
def matrix_exp3(mat: np.ndarray) -> np.ndarray:
    ...
    Returns:
        np.ndarray: Matrix exponential of the exponential coordinates as a numpy array.
    ...
    if np.abs(np.linalg.norm(rotation)) < 1.00e-06:
        ...
        unit_mat = mat / theta
        return np.eye(3, dtype=np.float64) + np.sin(theta) * unit_mat + (1 - np.cos(theta)) * (unit_mat @ unit_mat)
...
def matrix_log3(R: np.ndarray) -> np.ndarray:
    ...
    Returns:
        np.ndarray: Matrix logarithm of the rotation matrix as a numpy array.
    ...
    # if theta = pi, three possible solutions.
        ...
        if not (np.abs(1 + R[2, 2]) < 1.00e-06):
            omg = (1.0 / np.sqrt(2 * (1 + R[2, 2]))) * (R[:, 2] + np.array([0.0, 0.0, 1.0]))
        elif not (np.abs(1 + R[1, 1]) < 1.00e-06):
            omg = (1.0 / np.sqrt(2 * (1 + R[1, 1]))) * (R[:, 1] + np.array([0.0, 1.0, 0.0]))
        else:
            omg = (1.0 / np.sqrt(2 * (1 + R[0, 0]))) * (R[:, 0] + np.array([1.0, 0.0, 0.0]))
...
def project_to_SO3(mat: np.ndarray) -> np.ndarray:
    ...
    Returns:
        np.ndarray: The closest matrix in SO(3) as a numpy array.
...
def distance_to_SO3(mat: np.ndarray) -> float:
    ...
    Returns:
        float: The frobenius norm as a float.
...
def unwrap1pi(angles: Union[Iterable[float], np.ndarray]) -> np.ndarray:
    ...
    # Unwrap each column in the array
    unwrapped = np.unwrap(angles, axis=0, period=np.pi) if angles.ndim > 1 else np.unwrap(angles, period=np.pi)
...
def unwrap360(angles: Union[Iterable[float], np.ndarray]) -> np.ndarray:
    ...
    # Unwrap each column in the array
    unwrapped = np.unwrap(angles, axis=0, period=360.0) if angles.ndim > 1 else np.unwrap(angles, period=360.0)
...
def unwrap180(angles: Union[Iterable[float], np.ndarray]) -> np.ndarray:
    ...
    # Unwrap each column in the array
    unwrapped = np.unwrap(angles, axis=0, period=180.0) if angles.ndim > 1 else np.unwrap(angles, period=180.0)
...
def wrapunwrap(angles: Union[Iterable[float], np.ndarray]) -> np.ndarray:
    """
    Wraps an input of angle between -pi and pi radians and then unwraps it.
...
def wrapunwrap360(angles: Union[Iterable[float], np.ndarray]) -> np.ndarray:
    """
    Wraps an input of angle between 0 and 360 degrees and then unwraps it.
