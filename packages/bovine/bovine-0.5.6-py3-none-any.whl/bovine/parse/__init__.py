...
class Actor(BovineObject):
    ...
    @property
    def cryptographic_identifiers(self) -> List[CryptographicIdentifier]:
        """Returns the cryptographic identifiers associated with the
        actor"""
        result = []
        logger.info(self.data)
        public_key = self.data.get("publicKey")

        logger.info(public_key)

        if isinstance(public_key, dict):
            result.append(CryptographicIdentifier.from_public_key(public_key))
        elif isinstance(public_key, list):
            result += [CryptographicIdentifier.from_public_key(x) for x in public_key]
