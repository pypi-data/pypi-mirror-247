...
def contour(function, points, resolution, value=0.0, buffer=1.5):
    ...
    if n_dim == 2:
        ...
        mesh.points += origin
        mesh.points[:, 0] *= (bounds[1] - bounds[0])
        mesh.points[:, 1] *= (bounds[3] - bounds[2])
        mesh.point_data["values"] = function(mesh.points)
        boundary = mesh.contour([value], method="marching_squares")
        boundary = boundary.reconstruct_surface()
    elif n_dim == 3:
