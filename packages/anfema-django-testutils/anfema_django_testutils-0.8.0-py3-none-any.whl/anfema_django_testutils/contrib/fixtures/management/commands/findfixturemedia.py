...
class Command(LabelCommand):
    help = "Finds the absolute paths for the given media file(s)."
    label = "FILE"
    ...
    def add_arguments(self, parser) -> None:
        ...
        parser.add_argument(
            ...
            action="store_false",
            dest="all",
            help="Only return the first match for each fixture media file.",
        )

    def log(self, msg: str, *, level: int = 2, style=None) -> None:
        """Small log helper"""
    ...
    def handle_label(self, path, **options):
        result = finders.media.find(path, all=self.all)
