...
from airfold_cli.options import DryRunOption, OverwriteOption, with_global_options
...
def delete(
    ...
    commands = api.project_source_delete(name=name, dry_run=dry_run)
...
def rename(
    ...
    config: Config = load_config()
    api = AirfoldApi(config.key, config.endpoint)
    commands = api.rename_source(name=name, new_name=new_name, dry_run=dry_run, overwrite=overwrite)
    print_plan(commands, console=source_app.console)
