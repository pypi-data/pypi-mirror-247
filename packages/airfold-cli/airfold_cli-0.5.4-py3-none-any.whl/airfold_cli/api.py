...
class AirfoldApi:
    ...
    def _resp_to_err(self, res: Response) -> APIError:
        ...
        if res.status_code == 401:
            ...
            return OverwriteConflictError(desc)
    ...
    def _rename(self, url_path: str, new_name: str, dry_run: bool, overwrite: OverwriteType) -> Response:
        params = {"new_name": new_name, "dry_run": dry_run, "overwrite": overwrite.value}
    ...
    @api_retry
    def rename_source(
        self, name: str, new_name: str, dry_run: bool = False, overwrite: OverwriteType = OverwriteType.auto
    ) -> List[Command]:
        res = self._rename(f"/v1/sources/{name}", new_name, dry_run, overwrite)
        if res.ok:
            return [Command(**cmd) for cmd in res.json()]
        raise self._resp_to_err(res)
    ...
    def rename_pipe(
        ...
        res = self._rename(f"/v1/pipes/{name}", new_name, dry_run, overwrite)
        if res.ok:
            return [Command(**cmd) for cmd in res.json()]
        raise self._resp_to_err(res)
