...
# 1 What if an in-situ measurement matchs 2+ Sat observations? We must construct a dataframe, row by row 
# 2 Must return sensor, add to the row, for later comparison with resampled in-situ data 
...
def run(sat_folder=None, is_file=None, AC=None, out_file=None, to_log = True):
    ...
    for sat_i, sat_file in enumerate(sat_files): 
        ...
        if AC=='POLYMER': 
            ...
            dt_sat_obj = AC_acolite.get_datetime(sat_file)
