...
def run(suite, environment, browser, account, workspace, auth_cookie, test_case):
    click.secho("V 1.60 - Running parameters:", fg='blue')
    click.secho("V 1.60 - Suite: %s" % (suite), fg='blue')
    click.secho("V 1.60 - Environment: %s" % (environment), fg='blue')
    click.secho("V 1.60 - Browser: %s" % (browser), fg='blue')
    ...
    if auth_cookie is not None:
        click.secho("Armar vtex-auth-cookie.js con: %s" % (auth_cookie), fg='red')
        ...
        for i, linea in enumerate(lineas):
            if "const _REMOTE_VtexIdclientAutCookie =" in linea:
                ...
                click.secho("Reemplazando lineas en authcookie", fg='red')
