...
class ProductAttribute(ModelSQL, ModelView):
    ...
    selection = fields.One2Many("product.attribute.selection_option",
...
class ProductProductAttribute(ModelSQL, ModelView):
    ...
    def get_value(self, name=None):
        ...
        if self.attribute_type == 'date':
            ...
            return str(getattr(self, 'value_' + self.attribute_type))
    ...
    @fields.depends('product', '_parent_product.id')
    ...
    def search_attribute_set(cls, name, clause):
        return [
            (('product.template.attribute_set',) + tuple(clause[1:])),
            ]
...
class Product(metaclass=PoolMeta):
    ...
    attributes = fields.One2Many("product.product.attribute", "product",
        "Attributes", domain=[(
                'attribute.sets', '=',
                Eval('_parent_template', {}).get(
                    'attribute_set', Eval('attribute_set', -1))
                )], states={
