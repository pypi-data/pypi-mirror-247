...
class ShardTransferInfo(BaseModel):
    ...
    var_from: conint(strict=True, ge=0) = Field(..., alias="from")
    to: conint(strict=True, ge=0) = Field(...)
    sync: StrictBool = Field(..., description="If `true` transfer is a synchronization of a replicas If `false` transfer is a moving of a shard from one peer to another")
    method: Optional[ShardTransferMethod] = None
    __properties = ["shard_id", "from", "to", "sync", "method"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ShardTransferInfo:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ShardTransferInfo:
        """Create an instance of ShardTransferInfo from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ShardTransferInfo.parse_obj(obj)

        _obj = ShardTransferInfo.parse_obj({
            "shard_id": obj.get("shard_id"),
            "var_from": obj.get("from"),
            "to": obj.get("to"),
            "sync": obj.get("sync"),
            "method": ShardTransferMethod.from_dict(obj.get("method")) if obj.get("method") is not None else None
        })
        return _obj
