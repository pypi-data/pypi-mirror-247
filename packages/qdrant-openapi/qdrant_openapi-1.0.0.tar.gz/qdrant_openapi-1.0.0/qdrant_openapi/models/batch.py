...
class Batch(BaseModel):
    ...
    Batch
    """
    ids: conlist(ExtendedPointId) = Field(...)
    vectors: BatchVectorStruct = Field(...)
    payloads: Optional[conlist(BatchPayloadsInner)] = None
    __properties = ["ids", "vectors", "payloads"]
    ...
    def to_dict(self):
        ...
        # set to None if payloads (nullable) is None
        # and __fields_set__ contains the field
        if self.payloads is None and "payloads" in self.__fields_set__:
            _dict['payloads'] = None

        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> Batch:
        """Create an instance of Batch from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return Batch.parse_obj(obj)

        _obj = Batch.parse_obj({
            "ids": [ExtendedPointId.from_dict(_item) for _item in obj.get("ids")] if obj.get("ids") is not None else None,
            "vectors": BatchVectorStruct.from_dict(obj.get("vectors")) if obj.get("vectors") is not None else None,
            "payloads": [BatchPayloadsInner.from_dict(_item) for _item in obj.get("payloads")] if obj.get("payloads") is not None else None
        })
        return _obj
