...
class CollectionTelemetry(BaseModel):
    ...
    """
    id: StrictStr = Field(...)
    init_time_ms: conint(strict=True, ge=0) = Field(...)
    config: CollectionConfig = Field(...)
    shards: conlist(ReplicaSetTelemetry) = Field(...)
    transfers: conlist(ShardTransferInfo) = Field(...)
    __properties = ["id", "init_time_ms", "config", "shards", "transfers"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CollectionTelemetry:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CollectionTelemetry:
        """Create an instance of CollectionTelemetry from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CollectionTelemetry.parse_obj(obj)

        _obj = CollectionTelemetry.parse_obj({
            "id": obj.get("id"),
            ...
            "shards": [ReplicaSetTelemetry.from_dict(_item) for _item in obj.get("shards")] if obj.get("shards") is not None else None,
            "transfers": [ShardTransferInfo.from_dict(_item) for _item in obj.get("transfers")] if obj.get("transfers") is not None else None
        })
        return _obj
