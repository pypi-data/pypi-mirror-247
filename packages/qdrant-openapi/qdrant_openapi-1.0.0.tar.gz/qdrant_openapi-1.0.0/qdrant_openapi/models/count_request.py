...
class CountRequest(BaseModel):
    """
    Count Request Counts the number of points which satisfy the given filter. If filter is not provided, the count of all points in the collection will be returned.  # noqa: E501
    """
    shard_key: Optional[ShardKeySelector] = None
    filter: Optional[Filter] = None
    exact: Optional[StrictBool] = Field(True, description="If true, count exact number of points. If false, count approximate number of points faster. Approximate count might be unreliable during the indexing process. Default: true")
    __properties = ["shard_key", "filter", "exact"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CountRequest:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CountRequest:
        """Create an instance of CountRequest from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CountRequest.parse_obj(obj)

        _obj = CountRequest.parse_obj({
            ...
            "exact": obj.get("exact") if obj.get("exact") is not None else True
        })
        return _obj
