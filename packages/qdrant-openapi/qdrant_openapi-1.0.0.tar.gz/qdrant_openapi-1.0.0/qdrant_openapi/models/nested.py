...
class Nested(BaseModel):
    """
    Select points with payload for a specified nested field  # noqa: E501
    """
    key: StrictStr = Field(...)
    filter: Filter = Field(...)
    __properties = ["key", "filter"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> Nested:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> Nested:
        """Create an instance of Nested from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return Nested.parse_obj(obj)

        _obj = Nested.parse_obj({
            "key": obj.get("key"),
