...
class DeletePayload(BaseModel):
    """
    This data structure is used in API interface and applied across multiple shards  # noqa: E501
    """
    keys: conlist(StrictStr) = Field(..., description="List of payload keys to remove from payload")
    points: Optional[conlist(ExtendedPointId)] = Field(None, description="Deletes values from each point in this list")
    filter: Optional[Filter] = None
    shard_key: Optional[ShardKeySelector] = None
    __properties = ["keys", "points", "filter", "shard_key"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> DeletePayload:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> DeletePayload:
        """Create an instance of DeletePayload from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return DeletePayload.parse_obj(obj)

        _obj = DeletePayload.parse_obj({
            "keys": obj.get("keys"),
            "points": [ExtendedPointId.from_dict(_item) for _item in obj.get("points")] if obj.get("points") is not None else None,
