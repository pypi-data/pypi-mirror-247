...
class WithLookup(BaseModel):
    ...
    WithLookup
    """
    collection: StrictStr = Field(..., description="Name of the collection to use for points lookup")
    with_payload: Optional[WithPayloadInterface] = None
    with_vectors: Optional[WithVector] = None
    __properties = ["collection", "with_payload", "with_vectors"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> WithLookup:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of with_vectors
        if self.with_vectors:
            _dict['with_vectors'] = self.with_vectors.to_dict()
        ...
        # and __fields_set__ contains the field
        if self.with_vectors is None and "with_vectors" in self.__fields_set__:
            _dict['with_vectors'] = None

        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> WithLookup:
        """Create an instance of WithLookup from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return WithLookup.parse_obj(obj)

        _obj = WithLookup.parse_obj({
            "collection": obj.get("collection"),
            "with_payload": WithPayloadInterface.from_dict(obj.get("with_payload")) if obj.get("with_payload") is not None else None,
            "with_vectors": WithVector.from_dict(obj.get("with_vectors")) if obj.get("with_vectors") is not None else None
        })
        return _obj
