...
class GroupsResult(BaseModel):
    ...
    GroupsResult
    """
    groups: conlist(PointGroup) = Field(...)
    __properties = ["groups"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> GroupsResult:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> GroupsResult:
        """Create an instance of GroupsResult from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return GroupsResult.parse_obj(obj)

        _obj = GroupsResult.parse_obj({
            "groups": [PointGroup.from_dict(_item) for _item in obj.get("groups")] if obj.get("groups") is not None else None
        })
        return _obj
