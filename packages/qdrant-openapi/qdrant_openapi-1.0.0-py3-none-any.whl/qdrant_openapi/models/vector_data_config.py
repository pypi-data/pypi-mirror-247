...
class VectorDataConfig(BaseModel):
    """
    Config of single vector data storage  # noqa: E501
    """
    size: conint(strict=True, ge=0) = Field(..., description="Size/dimensionality of the vectors used")
    distance: Distance = Field(...)
    storage_type: VectorStorageType = Field(...)
    index: Indexes = Field(...)
    quantization_config: Optional[QuantizationConfig] = None
    __properties = ["size", "distance", "storage_type", "index", "quantization_config"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> VectorDataConfig:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of storage_type
        if self.storage_type:
            _dict['storage_type'] = self.storage_type.to_dict()
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> VectorDataConfig:
        """Create an instance of VectorDataConfig from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return VectorDataConfig.parse_obj(obj)

        _obj = VectorDataConfig.parse_obj({
            "size": obj.get("size"),
            "distance": obj.get("distance"),
            "storage_type": VectorStorageType.from_dict(obj.get("storage_type")) if obj.get("storage_type") is not None else None,
            "index": Indexes.from_dict(obj.get("index")) if obj.get("index") is not None else None,
