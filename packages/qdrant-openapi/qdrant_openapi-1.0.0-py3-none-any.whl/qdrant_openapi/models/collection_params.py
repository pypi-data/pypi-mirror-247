...
class CollectionParams(BaseModel):
    ...
    shard_number: Optional[conint(strict=True, ge=1)] = Field(1, description="Number of shards the collection has")
    sharding_method: Optional[ShardingMethod] = None
    replication_factor: Optional[conint(strict=True, ge=1)] = Field(1, description="Number of replicas for each shard")
    write_consistency_factor: Optional[conint(strict=True, ge=1)] = Field(1, description="Defines how many replicas should apply the operation for us to consider it successful. Increasing this number will make the collection more resilient to inconsistencies, but will also make it fail if not enough replicas are available. Does not have any performance impact.")
    read_fan_out_factor: Optional[conint(strict=True, ge=0)] = Field(None, description="Defines how many additional replicas should be processing read request at the same time. Default value is Auto, which means that fan-out will be determined automatically based on the busyness of the local replica. Having more than 0 might be useful to smooth latency spikes of individual nodes.")
    on_disk_payload: Optional[StrictBool] = Field(False, description="If true - point's payload will not be stored in memory. It will be read from the disk every time it is requested. This setting saves RAM by (slightly) increasing the response time. Note: those payload values that are involved in filtering and are indexed - remain in RAM.")
    sparse_vectors: Optional[Dict[str, SparseVectorParams]] = Field(None, description="Configuration of the sparse vector storage")
    __properties = ["vectors", "shard_number", "sharding_method", "replication_factor", "write_consistency_factor", "read_fan_out_factor", "on_disk_payload", "sparse_vectors"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CollectionParams:
    ...
    def to_dict(self):
        ...
        # and __fields_set__ contains the field
        if self.read_fan_out_factor is None and "read_fan_out_factor" in self.__fields_set__:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CollectionParams:
        """Create an instance of CollectionParams from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CollectionParams.parse_obj(obj)

        _obj = CollectionParams.parse_obj({
            "vectors": VectorsConfig.from_dict(obj.get("vectors")) if obj.get("vectors") is not None else None,
            "shard_number": obj.get("shard_number") if obj.get("shard_number") is not None else 1,
            ...
            "write_consistency_factor": obj.get("write_consistency_factor") if obj.get("write_consistency_factor") is not None else 1,
            "read_fan_out_factor": obj.get("read_fan_out_factor"),
            "on_disk_payload": obj.get("on_disk_payload") if obj.get("on_disk_payload") is not None else False,
            "sparse_vectors": dict(
