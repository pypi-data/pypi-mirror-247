...
class CollectionParamsDiff(BaseModel):
    ...
    """
    replication_factor: Optional[conint(strict=True, ge=1)] = Field(None, description="Number of replicas for each shard")
    write_consistency_factor: Optional[conint(strict=True, ge=1)] = Field(None, description="Minimal number successful responses from replicas to consider operation successful")
    read_fan_out_factor: Optional[conint(strict=True, ge=0)] = Field(None, description="Fan-out every read request to these many additional remote nodes (and return first available response)")
    ...
    __properties = ["replication_factor", "write_consistency_factor", "read_fan_out_factor", "on_disk_payload"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CollectionParamsDiff:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CollectionParamsDiff:
        """Create an instance of CollectionParamsDiff from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CollectionParamsDiff.parse_obj(obj)

        _obj = CollectionParamsDiff.parse_obj({
