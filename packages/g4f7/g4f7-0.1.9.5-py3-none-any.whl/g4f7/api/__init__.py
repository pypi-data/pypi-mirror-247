...
class Api:
    def __init__(self, engine: g4f7, debug: bool = True, sentry: bool = False,
        ...
        async def models():
            model_list = []
            for model in g4f7.Model.__all__():
                model_info = (g4f7.ModelUtils.convert[model])
                model_list.append({
                'id': model,
                'object': 'model',
                'created': 0,
        ...
        async def model_info(model_name: str):
            try:
                model_info = (g4f7.ModelUtils.convert[model_name])
        ...
        async def chat_completions(request: Request, item: JSONStructure = None):
            ...
            provider = item_data.get('provider').replace('g4f7.Provider.', '')
            provider = provider if provider and provider != "Auto" else None
            if provider != None:
                provider = g4f7.Provider.ProviderUtils.convert.get(provider)
