...
class Session(Generic[InteractiveItemTypeT]):
    ...
    def query(self, query: str, timeout: int = 120) -> WQLResponseProxy[InteractiveItemTypeT]:
        ...
        self._web_driver.preprocess_dom()
    ...
    def on(self, event: Literal["popup"], callback: Callable[[dict], None]):
        """Emitted when there is a popup on the page."""
    ...
    def _detect_popup(self, tree: dict, known_popups: list[Popup]) -> list[Popup]:
        ...
        popup_list = []
        if tree_role == "dialog":
            popup = Popup(copy.deepcopy(tree), tree_name)
    ...
    def _handle_popup(self, popups: list[Popup]):
        ...
        if popups and self._event_listeners["popup"]:
