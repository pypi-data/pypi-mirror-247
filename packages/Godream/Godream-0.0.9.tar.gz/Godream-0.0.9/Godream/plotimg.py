...
def plot_raster(input_data, with_bounds=True, contour=False, contour_label_kws=None,
    ...
    # Check if the input_data is a DatasetReader object
        ...
        if input_data.count == 1:
            ...
            arr = input_data.read(1, masked=True)
        else:
            try:
                ...
                arr = input_data.read(rgb_indexes, masked=True)
                arr = reshape_as_image(arr)
...
def overlay_map(vector_file=None, raster_file=None, with_draw_tools=True,zoom=None ):
    """Create a map using Folium."""
    if zoom is None:
        zoom = 11
    ...
    if vector_file is None:
        ...
        with rasterio.open(raster_file[0]) as src:
            ...
            # # check num of bands
            # num_bands= src.count
            # er_message = (" Not enough bands, It need 3 bands to display rgb image")
            # if num_bands<2:
        ...
        for x in raster_file:
            with rasterio.open(x) as src:
                ...
                if src.count >= 3:
                    ...
                    if band1.min() == 0: #some raster image didn't preprocessing
                        ...
                        # brightness
                        red_b = brighten(band1)
                        green_b = brighten(band2)
                        blue_b = brighten(band3)
...
def plot_multibands(raster_file):
