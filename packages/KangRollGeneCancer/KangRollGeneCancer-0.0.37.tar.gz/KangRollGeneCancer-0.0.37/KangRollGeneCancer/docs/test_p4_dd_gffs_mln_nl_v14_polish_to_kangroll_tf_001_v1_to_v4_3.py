...
for i in range(3):
    # 调用函数
    MLN_output, NL_final = kangroll_tf_001_v1_to_v4(v1_example, v2_example, u_ini_example, mln_iterations=100, debug_mode=True)
...
plt.plot(MLN_output, label=f'MLN_output Iteration {i+1}')
plt.title(f'Time series of the two-layer network - Iteration {i+1}')
plt.xlabel('Month')
plt.ylabel('Value')
plt.legend()
plt.grid(True)
