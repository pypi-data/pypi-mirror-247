...
for index, v1_test in enumerate(v1_values):
    ...
    # 绘制每个损伤值的图像
    plt.plot(total_MLN_output, label=f'TP53 Damage: {v1_test}', color=colors[index])

# 设置图像标题和其他属性
plt.title('Cancer Dynamics Network Growth Rate for Different TP53 Damage Levels')
plt.xlabel('times(eg.hours)')
plt.ylabel('Value')
plt.legend()
plt.grid(True)
