...
class _ISO3746parallelPipedSurfaceWithTwoReflectingPlaneUsingRectangularSegments: 
    ...
    def _calculateBackGroundCorrection(self):
        ...
        if (meanSoundPressureLevelDifference > pe.variable(10, 'dB')):
            k1 = pe.variable(0, 'dB')
        elif (pe.variable(3, 'dB') <= meanSoundPressureLevelDifference <= pe.variable(10, 'dB')):
            k1 = pe.variable(- 10 * np.log10(1 - 10 **
                                            (-0.1 * meanSoundPressureLevelDifference.value)), 'dB')
        else:
            # DeltaLp < 3 dB
            warnings.warn(f'The difference of the mean sound pressure level of the test subject and the mean sound pressure level of the background is {meanSoundPressureLevelDifference}. This is less than 3 dB. The correction term for the background noise is 3 dB. It should be stated in the report that the results in represents an upper boundary of the sound power level of the noise source.', UserWarning)
            k1 = pe.variable(3, 'dB')
        self._k1 = k1
    ...
    def _checkData(self):
        ...
        ## More microphone positions are necessary if this difference is larger than twice the number of microphones used
        diff = np.max(self.data.LAeq) - np.min(self.data.LAeq)
        diff._uncert = 0
        if (diff > 2*len(self.data.LAeq)):
            raise ValueError(f'The differnce between the maximum and minimum value of the sound pressure levels, {diff}, is more than twice the number of measurement points, {len(self.data.LAeq)}. The measurement cannot be used as more measurement points are necessary.')
        ...
        diff._uncert = 0
        if (diff > 2*len(self.backgroundData.LAeq)):
            raise ValueError(f'The differnce between the maximum and minimum value of the background sound pressure levels, {diff}, is more than twice the number of measurement points, {len(self.data.LAeq)}. The measurement cannot be used as more measurement points are necessary.')
