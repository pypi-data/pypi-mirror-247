...
class TestDeployOperation(TestCase):
    ...
    def test_deploy(self):
        ...
        self.assertEqual(
            res[0].deployedAppAttributes[0].attributeValue, ami_credentials.password
        )
    ...
    def test_get_block_device_mappings_throws_max_storage_error(self):
        ...
        with self.assertRaisesRegex(
            ...
            self.deploy_operation._get_block_device_mappings(
                image=image, ami_deployment_model=ami, aws_ec2_resource_model=ec_model
            )
    ...
    def test_create_deployment_parameters_no_iam_role(self):
        ...
        self.assertTrue(
            not any(aws_model.iam_role)
        )  # not any(some_dict) => is empty dictionary
